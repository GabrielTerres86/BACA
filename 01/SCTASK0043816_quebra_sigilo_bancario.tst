...
DECLARE 
  ...
  vr_dircop     CONSTANT VARCHAR2(200) := '/micros/cecred/quisinski/carta';
  vr_dir_arq    VARCHAR2(300);

  -- Variaveis
  vr_referencia VARCHAR2(20);
  vr_dtinicio   DATE;
  vr_dtfim      DATE;
  vr_contador   NUMBER;
  ...
  BEGIN
    ...
    pr_tbpa('001_0015').dtabertu := to_date('30/09/2014','DD/MM/YYYY');
    ...
    pr_tbpa('002_0013').dtabertu := to_date('01/12/2006','DD/MM/YYYY');
    pr_tbpa('002_0013').dstelefo := '4734679950';
    ...
    pr_tbpa('009_0002').dstelefo := '4834622072';    
    ...
    pr_tbpa('011_0004').dtabertu := to_date('15/06/2011','DD/MM/YYYY');
    pr_tbpa('011_0004').dstelefo := '4733634025';
  ...
  BEGIN
    ...
    WHILE TRIM(vr_idx_cta_investigar) IS NOT NULL LOOP
      ...
      FOR rw_conta IN cr_contas (pr_cdcooper => pr_tbconta_investigar(vr_idx_cta_investigar).cdcooper
        ...
        /*vr_tbdoc(0).cdagenci := 42; --Agencia sem dig
        vr_tbdoc(0).nrcpfdeb := 50123696968;  
        vr_tbdoc(0).nrdconta := 3409;
        vr_tbdoc(0).nrctadeb := 1000415170;
        vr_tbdoc(0).dtmvtolt := TO_DATE('26/08/2016','DD/MM/RRRR');
        vr_tbdoc(0).cdbancod := 104;
        vr_tbdoc(0).nmclideb := 'DANIEL MARTINS';
        vr_tbdoc(0).valordoc := 3000;
        vr_tbdoc(0).flgativo := 0; --Não Utilizado
		
        vr_tbdoc(1).cdagenci := 42; --Agencia sem dig
        vr_tbdoc(1).nrcpfdeb := 50123696968;  
        vr_tbdoc(1).nrdconta := 116068;
        vr_tbdoc(1).nrctadeb := 1000415170;
        vr_tbdoc(1).dtmvtolt := TO_DATE('12/09/2016','DD/MM/RRRR');
        vr_tbdoc(1).cdbancod := 104;
        vr_tbdoc(1).nmclideb := 'DANIEL MARTINS';
        vr_tbdoc(1).valordoc := 3000;
        vr_tbdoc(1).flgativo := 0; --Não Utilizado

        vr_tbdoc(2).cdagenci := 190; --Agencia sem dig
        vr_tbdoc(2).nrcpfdeb := 00394460005887;  
        vr_tbdoc(2).nrdconta := 4138;
        vr_tbdoc(2).nrctadeb := 3325160028;
        vr_tbdoc(2).dtmvtolt := TO_DATE('14/03/2017','DD/MM/RRRR');
        vr_tbdoc(2).cdbancod := 1;
        vr_tbdoc(2).nmclideb := 'SECR. DA RECEITA FEDERAL';
        vr_tbdoc(2).valordoc := 1089.82;
        vr_tbdoc(2).flgativo := 0; --Não Utilizado
		
        vr_tbdoc(3).cdagenci := 384; --Agencia sem dig
        vr_tbdoc(3).nrcpfdeb := 36660680144;  
        vr_tbdoc(3).nrdconta := 116068;
        vr_tbdoc(3).nrctadeb := 25550;
        vr_tbdoc(3).dtmvtolt := TO_DATE('05/10/2017','DD/MM/RRRR');
        vr_tbdoc(3).cdbancod := 237;
        vr_tbdoc(3).nmclideb := 'JULIO CARLOS CERVO DE DAVID';
        vr_tbdoc(3).valordoc := 836;
        vr_tbdoc(3).flgativo := 0; --Não Utilizado

        vr_tbdoc(4).cdagenci := 179; --Agencia sem dig
        vr_tbdoc(4).nrcpfdeb := 21435819000151;  
        vr_tbdoc(4).nrdconta := 116068;
        vr_tbdoc(4).nrctadeb := 3000025152;
        vr_tbdoc(4).dtmvtolt := TO_DATE('28/12/2017','DD/MM/RRRR');
        vr_tbdoc(4).cdbancod := 104;
        vr_tbdoc(4).nmclideb := 'MADEIREIRA JOAO VITOR LEAL EIREL';
        vr_tbdoc(4).valordoc := 4000;
        vr_tbdoc(4).flgativo := 0; --Não Utilizado
		
        vr_tbdoc(5).cdagenci := 477; --Agencia sem dig
        vr_tbdoc(5).nrcpfdeb := 62319400982;  
        vr_tbdoc(5).nrdconta := 4138;
        vr_tbdoc(5).nrctadeb := 2005530;
        vr_tbdoc(5).dtmvtolt := TO_DATE('31/01/2018','DD/MM/RRRR');
        vr_tbdoc(5).cdbancod := 1;
        vr_tbdoc(5).nmclideb := 'ITELVINO SCHINAIDER';
        vr_tbdoc(5).valordoc := 600;
        vr_tbdoc(5).flgativo := 0; --Não Utilizado
        ...
        -- Percorrer todas as movimentaçoes da conta
        FOR rw_lancamento_doc IN cr_lancamentos_doc (pr_cdcooper     => rw_conta.cdcooper
                                                    ,pr_nrdconta     => rw_conta.nrdconta
                                                    ,pr_dtmvtolt_ini => pr_dtini_quebra
                                                    ,pr_dtmvtolt_fim => pr_dtfim_quebra) LOOP
                   
          vr_tbdoc(vr_contador).cdagenci := rw_lancamento_doc.cdagenci; --Agencia sem dig
          vr_tbdoc(vr_contador).nrcpfdeb := rw_lancamento_doc.cpfcgemi;  
          vr_tbdoc(vr_contador).nrdconta := rw_lancamento_doc.nrdconta;
          vr_tbdoc(vr_contador).nrctadeb := rw_lancamento_doc.nrctachq;
          vr_tbdoc(vr_contador).dtmvtolt := rw_lancamento_doc.dtmvtolt;
          vr_tbdoc(vr_contador).cdbancod := rw_lancamento_doc.cdbanchq;
          vr_tbdoc(vr_contador).nmclideb := rw_lancamento_doc.cdpesqbb;
          vr_tbdoc(vr_contador).valordoc := rw_lancamento_doc.vllanmto;
          vr_tbdoc(vr_contador).flgativo := 0;
    ...
    IF pr_tbarq_origem_destino.COUNT > 0 THEN
      FOR vr_idx IN pr_tbarq_origem_destino.FIRST..pr_tbarq_origem_destino.LAST LOOP
        vr_dstext_linha := pr_tbarq_origem_destino(vr_idx).idseqarq || chr(09) || 
                           ...
                           pr_tbarq_origem_destino(vr_idx).cdbandep || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).cdagedep || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).nrctadep || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).tpdconta || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).inpessoa || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).nrcpfcgc || chr(09) || 
                           pr_tbarq_origem_destino(vr_idx).nmprimtl || chr(09) || 
...
BEGIN
  ...
  */
  ...
  vr_referencia := 'SCTASK0043816'; 
  -- Data que deve ocorrer a quebra de sigilo bancário
  vr_dtinicio   := TO_DATE('01/01/2014','DD/MM/YYYY');
  vr_dtfim      := TO_DATE('31/08/2018','DD/MM/YYYY'); 

  vr_tbconta_investigar('007_0000123650_000006497892966').cdcooper := 7;
  vr_tbconta_investigar('007_0000123650_000006497892966').nrdconta := 123650;
  vr_tbconta_investigar('007_0000123650_000006497892966').nrcpfcgc := 06497892966;
