declare
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 3573;
  vr_nrctremp craplem.nrctremp%TYPE := 21585;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1705,1708,1711,3026','[^,]+', 1, LEVEL) as vr_cdhistor,
           REGEXP_SUBSTR('142.86,2.85,0.05,142.86','[^,]+', 1, LEVEL) as vr_vllanmto,
           REGEXP_SUBSTR('12,12,12,12','[^,]+', 1, LEVEL) as vr_nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('1705,1708,1711,3026','[^,]+', 1, LEVEL) IS NOT NULL;     
