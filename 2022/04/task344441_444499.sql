declare
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 444499;
  vr_nrctremp craplem.nrctremp%TYPE := 90846;
  vr_nrparepr crappep.nrparepr%TYPE := 14;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1041,1705,1708,3027','[^,]+', 1, LEVEL) as vr_cdhistor,
           REGEXP_SUBSTR('0.45,25.77,0.51,223.14','[^,]+', 1, LEVEL) as vr_vllanmto          
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('1041,1705,1708,3027','[^,]+', 1, LEVEL) IS NOT NULL;     
