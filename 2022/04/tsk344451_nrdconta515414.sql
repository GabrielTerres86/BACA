declare
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 215414;
  vr_nrctremp craplem.nrctremp%TYPE := 61817;
  vr_nrparepr crappep.nrparepr%TYPE := 22;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1041,1705,1708,3026','[^,]+', 1, LEVEL) as vr_cdhistor,
           REGEXP_SUBSTR('5.59,2.73,.06,533.94','[^,]+', 1, LEVEL) as vr_vllanmto          
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('1041,1705,1708,3026','[^,]+', 1, LEVEL) IS NOT NULL;     
