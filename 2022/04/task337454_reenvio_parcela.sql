DECLARE
  ...
  CURSOR cr_contas IS
	SELECT REGEXP_SUBSTR('1,1,1,1', '[^,]+', 1, LEVEL) AS cdcooper
		  ,REGEXP_SUBSTR('9612696,9612696,10338306,80476260','[^,]+',1,LEVEL) AS nrdconta
		  ,REGEXP_SUBSTR('2913350,2913350,3042647,2816090', '[^,]+', 1, LEVEL) AS nrctremp
		  ,REGEXP_SUBSTR('2022-04-07,2022-04-07,2022-04-11,2022-04-11', '[^,]+', 1, LEVEL) AS dtmvtolt
		  ,REGEXP_SUBSTR('7,19,18,20', '[^,]+', 1, LEVEL) AS nrparepr1
		  ,REGEXP_SUBSTR('10,34,18,20', '[^,]+', 1, LEVEL) AS nrparepr2
	FROM   DUAL
	CONNECT BY REGEXP_SUBSTR('1,1,1,1','[^,]+',1,LEVEL) IS NOT NULL;
