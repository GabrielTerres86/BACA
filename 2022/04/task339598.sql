DECLARE
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 166634;
  vr_nrctremp craplem.nrctremp%TYPE := 24549;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1041,1705,1708,1711,3026', '[^,]+', 1, LEVEL) as vr_cdhistor
          ,REGEXP_SUBSTR('28.76,146.15,2.92,0.01,146.15', '[^,]+', 1, LEVEL) as vr_vllanmto
...
BEGIN
  ...
  FOR rw_lancamento IN cr_lancamento LOOP
    ...
    EMPR0001.pc_cria_lancamento_lem(pr_cdcooper => vr_cdcooper,
                                    ...
                                    pr_vllanmto => TO_NUMBER(REPLACE(rw_lancamento.vr_vllanmto,
                                                                     ',',
                                                                     '.'),
                                                             '9999.99'),
