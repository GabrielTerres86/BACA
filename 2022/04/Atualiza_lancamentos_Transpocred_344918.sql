declare
  ...
  CURSOR CR_CONTAS IS
    SELECT REGEXP_SUBSTR('501948,503169,503665,506001,514640,515558','[^,]+', 1, LEVEL) as nrdconta,
           REGEXP_SUBSTR('21100009,21100075,21100124,20300050,20100508,20100495','[^,]+', 1, LEVEL) as nrctremp,
           REGEXP_SUBSTR('7,7,6,7,7,7','[^,]+', 1, LEVEL) as nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('501948,503169,503665,506001,514640,515558','[^,]+', 1, LEVEL) IS NOT NULL;  
