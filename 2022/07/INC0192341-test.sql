...
begin     
	...
	if	(qt_reg_arquivo	>= 10000) then
		...
		nm_arquivo_rollback_v	:= 'ROLLBACK_INC0192341_coop_' || pr_cdcooper || '_arq_' || nr_arquivo || '.sql';
	...
	for	rw_prestamista in
		...
		if	(rw_prestamista.nrctremp in (261250,241545,484247,504752,4740740,5684988,5221361,201428)) then
...
if	(upper(cecred.gene0001.fn_database_name) like '%AYLLOSP%' or upper(cecred.gene0001.fn_database_name) like '%AILOSTS%') then
	...
	ds_nome_diretorio_v	:= ds_nome_diretorio_v || 'cpd/bacas/INC0192341';
	...
	ds_nome_diretorio_v	:= ds_nome_diretorio_v || '/INC0192341';
