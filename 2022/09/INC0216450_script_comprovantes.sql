...
DECLARE
  ...
  CURSOR cr_craplau IS
     ...
     WHERE lau.cdcooper IN (1,2,5)
        AND lau.nrdconta IN (10457321,12791725,12637254,11332204,11890797,879487,10391401,11211881,2787075,8760675,8418551,576905,131318)
        AND lau.insitlau = 3
        AND lau.dsorigem = 'DEBAUT'
        AND lau.cdhistor = 3292
        AND lau.dtmvtopg = TO_DATE('09/09/2022', 'DD/MM/YYYY');
...
BEGIN   
    ...
    FOR rw_craplau IN cr_craplau LOOP
      ...
      GENE0006.pc_gera_protocolo(pr_cdcooper => rw_craplau.cdcooper
                                ,pr_dtmvtolt => TO_DATE('09/09/2022', 'DD/MM/YYYY')
