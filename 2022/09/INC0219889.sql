DECLARE
  ...
  vc_dstransaStatusCC             CONSTANT VARCHAR2(4000) := 'Alteracao da situacao de conta por script - INC0219889';
  vc_dstransaDevCotas             CONSTANT VARCHAR2(4000) := 'Alteração de Cotas e devolução - INC0219889';
  vc_dstransaDevDepVista          CONSTANT VARCHAR2(4000) := 'Alteracao da devolução do Depósito a Vista - INC0219889';
  ...
  vc_dstransaInternet             CONSTANT VARCHAR2(4000) := 'Baixa do Lancamento de Tarifa de Internet - INC0219889';
  vc_insitlau                     CONSTANT cecred.craplau.insitlau%TYPE := 3;
  

 CURSOR cr_internet(pr_globalname  in varchar2) IS
 select l.dtmvtolt,
        l.nrdconta,
        l.insitlau,
        l.dtdebito,
   ...
   from craplau l
  where 1=1
    and l.nrdconta = decode(pr_globalname, vc_bdprod, 3526267,96473673)
    and cdcooper = 1
 ...
 CURSOR cr_crapass(pr_globalname varchar2
   ...
   SELECT t.cdagenci
     ...
     FROM CECRED.CRAPASS t
        ,(select 1 as cdcooper, decode(pr_globalname, vc_bdprod, 3526267,96473673) as nrdconta, vc_cdsitdctEncerrada as cdsitdct, pr_dtmvtolt as dtdemiss, pr_dtmvtolt as dtelimin, pr_dtmvtolt as dtasitct, 11 as  cdmotdem from dual
         ) a
    WHERE 1=1
...
BEGIN
  ...
  FOR rw_internet in cr_internet(vr_globalname) LOOP

    vr_cdcooper := rw_internet.cdcooper;
    vr_nrdconta := rw_internet.nrdconta;   
    vr_nrdrowid := null;
    ...
    CECRED.GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid,
                                     ...
                                     pr_dsdadant => rw_internet.idlancto,
                                     pr_dsdadatu => rw_internet.idlancto);
    CECRED.GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid,
                                     pr_nmdcampo => 'CRAPLAU.INSITLAU',
                                     pr_dsdadant => rw_internet.insitlau,
                                     pr_dsdadatu => vc_insitlau);
    CECRED.GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid,
                                     pr_nmdcampo => 'CRAPLAU.DTDEBITO',
                                     pr_dsdadant => rw_internet.dtdebito,
                                     pr_dsdadatu => rw_internet.dtmvtolt);
    UPDATE CECRED.CRAPLAU t
       SET t.insitlau = vc_insitlau
         , t.dtdebito = t.dtmvtolt
     WHERE cdcooper = rw_internet.cdcooper
       AND nrdconta = rw_internet.nrdconta
       and idlancto = rw_internet.idlancto;
