    ...
    CURSOR cr_lancamento IS
      SELECT arq.idarquivo
       ...
       WHERE lct.idarquivo = arq.idarquivo
         ...
         AND arq.idarquivo IN (568654,568659,568660)
...
BEGIN
  ...
  BEGIN
    UPDATE CECRED.tbdomic_liqtrans_lancto
       SET insituacao = 1
     WHERE idarquivo IN (568654,568659,568660);
  END;

  BEGIN
    UPDATE CECRED.tbdomic_liqtrans_pdv
       SET cdocorrencia         = NULL
          ,dhretorno            = NULL
          ,cdocorrencia_retorno = NULL
          ,dserro               = NULL
          ,dsocorrencia_retorno = NULL
