   ...
   CURSOR cr_craprac IS
     SELECT rac.cdcooper, 
      ...
      WHERE rac.cdcooper = ass.cdcooper
        ...
        AND rac.dtmvtolt < to_date('01/06/2022','dd/mm/yyyy')
        AND rac.dtaniver in (to_date('10/07/2022','dd/mm/yyyy'),
                            to_date('11/07/2022','dd/mm/yyyy'),
                            to_date('12/07/2022','dd/mm/yyyy'),
                            to_date('13/07/2022','dd/mm/yyyy'),
                            to_date('14/07/2022','dd/mm/yyyy'));
...
BEGIN
  ...
  UPDATE cecred.craptxi 
     SET dtcadast = to_date('10/06/2022','dd/mm/yyyy')
   WHERE cddindex = 5
  ...
  FOR rw_craprac in cr_craprac LOOP 
    ...
    END IF;
     ...
     CECRED.pc_log_programa(pr_dstiplog      => 'O'
                           ...
                           ,pr_cdmensagem    => 444
                           ,pr_cdprograma    => 'INC0154388_IPCA22'
                           ,pr_cdcooper      => 3 
     ...
     vr_dtinical := to_date(EXTRACT(DAY FROM rw_craprac.dtmvtolt) || '/05/2022','dd/mm/yyyy');
  ...
  EXCEPTION 
     ...
     CECRED.pc_log_programa(pr_dstiplog      => 'O' 
                           ...
                           ,pr_dsmensagem    => vr_dslog
                           ,pr_cdmensagem    => 888
                           ,pr_cdprograma    => 'INC0154388_IPCA_ERRO'
                           ,pr_cdcooper      => 3 
                           ,pr_idprglog      => vr_idprglog);   
         ROLLBACK;    
     WHEN OTHERS then
