DECLARE
   ...
   CURSOR cr_craprac(pr_cdcooper IN craprac.cdcooper%TYPE ) IS
     SELECT rac.cdcooper, 
            ...
            rac.rowid      
       FROM craprac rac
      WHERE rac.dtaniver = to_date('01/03/2022','dd/mm/yyyy') 
...
BEGIN
  ...
  FOR  rw_crapdat in cr_crapdat LOOP 
      ...
      FOR rw_craprac in cr_craprac(rw_crapdat.cdcooper) LOOP
        ...
        BACKUP('UPDATE CRAPRAC SET DTANIVER = ''01/03/2022'' WHERE CDCOOPER = '||rw_craprac.cdcooper ||' AND '||' NRDCONTA = '||rw_craprac.nrdconta|| ' AND NRAPLICA = '||rw_craprac.nraplica||';'); 
      ...
      pr_percirrf := 0;
            ...
            pc_saldo_atual_poupanca(pr_cdcooper => rw_craprac.cdcooper,           
              ...
              APLI0012.pc_credita_poupanca(pr_cdcooper => rw_craprac.cdcooper
                                          ...
                                          ,pr_dtaniver => to_date('01/02/2022','dd/mm/yyyy')
