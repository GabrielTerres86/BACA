DECLARE
   ...
   CURSOR cr_craprac IS
     SELECT rac.cdcooper, 
      ...
      WHERE rac.cdcooper = ass.cdcooper
        ...
        AND rac.dtmvtolt < to_date('01/04/2022','dd/mm/yyyy')
   ...
   CURSOR cr_aporte (pr_cdcooper NUMBER
                     ,pr_nrdconta NUMBER
                     ,pr_nraplica NUMBER
                     ,pr_dtmvtotl DATE) IS 
    SELECT cdcooper,
     ...
     WHERE l.cdcooper = pr_cdcooper
       ...
       AND l.dtmvtolt in (to_date('05/05/2022','dd/mm/yyyy'),
                          pr_dtmvtotl)
       AND l.cdhistor = 3328;
   rw_aporte cr_aporte%ROWTYPE;
   ...
   where lac.cdcooper = his.cdcooper
     ...
     and lac.dtmvtolt between pr_dtmvtini and pr_dtmvtfim;  
...
BEGIN
  ...
  FOR rw_craprac in cr_craprac LOOP 
    ...
    vr_cdmensagem := 0;
     ...
     OPEN cr_aporte(rw_craprac.cdcooper
                    ,rw_craprac.nrdconta
                    ,rw_craprac.nraplica
                    ,rw_crapdat.dtmvtolt);
     FETCH cr_aporte INTO rw_aporte;
     
     IF cr_aporte%FOUND THEN
            
        vr_dslog := 'APORTE: ' || rw_craprac.cdcooper || ';'||rw_craprac.nrdconta||';'||rw_craprac.nraplica||';';
    ...
    OPEN cr_saldoapp(rw_craprac.cdcooper
     ...
     vr_dtinical := to_date(EXTRACT(DAY FROM rw_craprac.dtmvtolt) || '/03/2022','dd/mm/yyyy');
