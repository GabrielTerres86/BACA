declare
  ...
  vr_cdcooper crapcop.cdcooper%TYPE := 11;
  ...
  CURSOR CR_CONTAS IS
    SELECT REGEXP_SUBSTR('497509,730610','[^,]+', 1, LEVEL) as nrdconta,
           REGEXP_SUBSTR('152048,174655','[^,]+', 1, LEVEL) as nrctremp,
           REGEXP_SUBSTR('7,3','[^,]+', 1, LEVEL) as nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('497509,730610','[^,]+', 1, LEVEL) IS NOT NULL;  
  ...
  CURSOR CR_LANCAMENTO(PR_CDCOOPER IN CRAPLEM.CDCOOPER%TYPE
    ...
    SELECT PEP.VLPAREPR AS VLLANMTO,
           NVL((SELECT DECODE(INDAUTREPASSECC, 1, 3026, 3027)
             ...
             WHERE CDCOOPER = PR_CDCOOPER
               AND CDEMPRES = (SELECT CDEMPRES
                                ...
                                WHERE CDCOOPER = PR_CDCOOPER
                                  AND NRDCONTA = PR_NRDCONTA
                                  AND NRCTREMP = PR_NRCTREMP)),3027) CDHISTOR,
           PEP.NRPAREPR                                  
      FROM CRAPPEP PEP
     WHERE CDCOOPER = PR_CDCOOPER
       AND NRDCONTA = PR_NRDCONTA
