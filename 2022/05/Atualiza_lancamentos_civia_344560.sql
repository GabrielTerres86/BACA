declare
  ...
  CURSOR CR_CONTAS IS
    SELECT REGEXP_SUBSTR('419834,221546,388181,145998,80250,638315,548685,105295,186244,670324,207497','[^,]+', 1, LEVEL) as nrdconta,
           REGEXP_SUBSTR('55980,135155,143312,145708,139169,151150,138828,109809,58233,167935,51194','[^,]+', 1, LEVEL) as nrctremp,
           REGEXP_SUBSTR('24,6,5,4,5,3,5,10,24,1,27','[^,]+', 1, LEVEL) as nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('419834,221546,388181,145998,80250,638315,548685,105295,186244,670324,207497','[^,]+', 1, LEVEL) IS NOT NULL;  
