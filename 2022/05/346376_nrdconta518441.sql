declare
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 518441;
  vr_nrctremp craplem.nrctremp%TYPE := 21100089;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1041,1705,3027','[^,]+', 1, LEVEL) as vr_cdhistor,
           REGEXP_SUBSTR('28.62,.08,245.31','[^,]+', 1, LEVEL) as vr_vllanmto          
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('1041,1705,3027','[^,]+', 1, LEVEL) IS NOT NULL;     
