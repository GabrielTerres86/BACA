DECLARE
  ...
  vc_dstransaSensbCRAPSLD             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSLD) por script - INC0135197';
  vc_dstransaSensbCRAPSDA             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSDA) por script - INC0135197';
  vc_dtinicioCRAPSDA                  CONSTANT DATE           := to_date('12/04/2022','dd/mm/yyyy');
  vccdCooper                          CONSTANT NUMBER         := 1;
  ...
  CURSOR cr_crapsld is
    SELECT a.nrdconta
      ...
      from CECRED.crapsld a
          ,(select 14337614 as nrdconta, -22.42 as valor from dual) contas
     WHERE a.nrdconta = contas.nrdconta
       AND a.cdcooper = vccdCooper;
...
EXCEPTION
  WHEN vr_erro_geralog THEN
    ...
    RAISE_APPLICATION_ERROR(-20000,
                            ...
                            gr_cdcooper || '/' || vr_nrdconta || ')- ' ||  vr_dscritic);
  ...
  WHEN OTHERS THEN
    ...
    RAISE_APPLICATION_ERROR(-20000,
                            ...
                            gr_cdcooper || '/' || vr_nrdconta || ') - ' ||  ' - ' || v_code || ' - ' || v_errm);
END;
