...
vr_tab_nmarqtel			cecred.gene0002.typ_split;
...
type vr_tab_tbseg_nrproposta	is table of cecred.tbseg_nrproposta%ROWTYPE INDEX BY BINARY_INTEGER;
typ_tab_tbseg_nrproposta	vr_tab_tbseg_nrproposta;
vr_id_nrproposta		number := 0; 

cursor	cr_tbseg_nrproposta(pr_nrproposta	cecred.tbseg_nrproposta.nrproposta%type) is
select	1
from	cecred.tbseg_nrproposta
...
begin
	...
	if	(upper(cecred.gene0001.fn_database_name) like '%AYLLOSP%') then
		...
		vr_diretorio	:= cecred.gene0001.fn_diretorio(	pr_tpdireto => 'C',
								pr_cdcooper => 3);
	...
	cecred.gene0001.pc_lista_arquivos(	pr_path		=> vr_diretorio,
					pr_pesq		=> vr_nmarqmov,
	...
	vr_tab_nmarqtel	:= cecred.gene0002.fn_quebra_string(pr_string => vr_listadir);
	...
	for	idx in 1..vr_tab_nmarqtel.count loop
		...
		cecred.gene0001.pc_abre_arquivo(	pr_nmdireto	=> vr_diretorio||'/',
		...
		begin
			...
			begin
				...
				cecred.gene0001.pc_le_linha_arquivo(	pr_utlfileh	=> vr_arqhandle,
		...
		cecred.gene0001.pc_fecha_arquivo(vr_arqhandle);
			...
			for	vr_ind in typ_tab_tbseg_nrproposta.first .. typ_tab_tbseg_nrproposta.last loop
				...
				if	(cr_tbseg_nrproposta%notfound) then
					...
					begin
						...
						insert	into cecred.tbseg_nrproposta(nrproposta, tpcustei, dhseguro)
						...
						cecred.gene0002.pc_escreve_xml(	ds_dados_rollback_v,
										ds_texto_rollback_v,
										'delete	from cecred.tbseg_nrproposta a where a.rowid = ' || chr(39) || ds_rowid_proposta_v || chr(39) || ';' || chr(13), false);
						...
						if	(qt_registro_v	>= 10000) then
							...
							cecred.gene0002.pc_escreve_xml(ds_dados_rollback_v, ds_texto_rollback_v, chr(13) || 'commit;' || chr(13) || chr(13), FALSE);
