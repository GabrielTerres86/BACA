declare
  ...
  CURSOR CR_CONTAS IS
    SELECT REGEXP_SUBSTR('143421,149870,158593','[^,]+', 1, LEVEL) as nrdconta,
           REGEXP_SUBSTR('36375,42542,33015','[^,]+', 1, LEVEL) as nrctremp,
           REGEXP_SUBSTR('6,3,9','[^,]+', 1, LEVEL) as nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('143421,149870,158593','[^,]+', 1, LEVEL) IS NOT NULL;  
