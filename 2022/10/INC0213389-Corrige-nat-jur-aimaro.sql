DECLARE
  ...
  CURSOR cr_natjur_ctr (pr_cpfcgc  IN CECRED.tbcadast_pessoa.NRCPFCGC%TYPE) IS
    SELECT jc.cdnatureza_juridica   natjur_centralizado
      , p.nrcpfcgc
    FROM CECRED.TBCADAST_PESSOA P 
    JOIN CECRED.TBCADAST_PESSOA_JURIDICA JC ON P.IDPESSOA = JC.IDPESSOA
    WHERE P.NRCPFCGC = pr_cpfcgc
      AND NVL(JC.CDNATUREZA_JURIDICA, 0) <> 0;
  
  rw_natjur_ctr  cr_natjur_ctr%rowtype;
  
  CURSOR cr_crapdat(pr_cdcooper  IN CECRED.crapdat.CDCOOPER%TYPE) IS
    SELECT d.dtmvtolt
...
BEGIN
  ...
  LOOP
    ...
    OPEN cr_crapdat(rw_natjur.cdcooper);
    FETCH cr_crapdat INTO vr_dtmvtolt;
    CLOSE cr_crapdat;
    
    rw_natjur_ctr := NULL;
    
    OPEN cr_natjur_ctr(rw_natjur.nrcpfcgc);
    FETCH cr_natjur_ctr INTO rw_natjur_ctr;
    
    IF rw_natjur_ctr.natjur_centralizado IS NULL THEN
      
      gene0001.pc_escr_linha_arquivo(vr_ind_arqlog, rw_natjur.nrcpfcgc || ';Sem natureza jurídica no centralizado. ' 
                                                    || ';' || rw_natjur.nrdconta
                                                    || ';' || rw_natjur.nmrescop  );
      
    ELSE
      
      gene0001.pc_escr_linha_arquivo(vr_ind_arqlog, rw_natjur.nrcpfcgc || ';Sucesso' 
                                                    || ';' || rw_natjur.nrdconta
                                                    || ';' || rw_natjur.nmrescop
                                                    || ';' || rw_natjur_ctr.natjur_centralizado );
    ...
    BEGIN
      ...
      UPDATE CECRED.CRAPJUR
        SET natjurid = rw_natjur_ctr.natjur_centralizado
    ...
    CECRED.GENE0001.pc_gera_log_item( pr_nrdrowid => vr_nrdrowid
                                     ...
                                     ,pr_dsdadatu => to_char( rw_natjur_ctr.natjur_centralizado ) 
                                    );
