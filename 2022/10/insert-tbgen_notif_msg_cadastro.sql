...
BEGIN
  SELECT (max(cdmensagem) + 1) into codigoMensagem FROM tbgen_notif_msg_cadastro;
    ...
    (13, 53, 'PIX - Você enviou um Pix', codigoMensagem, 1, 0, '<br>#NomeRecebedor - Nome do Recebedor<br>#ValorPix - Valor do pagamento (Ex.: 5000,00)<br>#Instituicao - Nome da Instituição<br>#Identificacao - Identificação do Pix<br>#Descricao - Descrição do Pagamento');
    commit;
  ...
  SELECT (max(cdmensagem) + 1) into codigoMensagem FROM tbgen_notif_msg_cadastro;
    ...
    INSERT INTO tbgen_notif_automatica_prm (cdorigem_mensagem, cdmotivo_mensagem, dsmotivo_mensagem, cdmensagem, inmensagem_ativa, intipo_repeticao, dsvariaveis_mensagem) VALUES
    (13, 54, 'PIX - Você enviou um Pix', codigoMensagem, 1, 0, '<br>#NomeRecebedor - Nome do Recebedor<br>#ValorPix - Valor do pagamento (Ex.: 5000,00)<br>#ValorTarifaPix# - Valor da tarifa (Ex.: 5,00)<br>Instituicao - Nome da Instituição<br>#Identificacao - Identificação do Pix<br>#Descricao - Descrição do Pagamento');
    commit;
END;