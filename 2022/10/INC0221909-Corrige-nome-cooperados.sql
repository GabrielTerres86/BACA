DECLARE
  ...
  vr_nmarquiv           VARCHAR2(50) := 'INC0221909_cpfs.csv';
  vr_nmarqbkp           VARCHAR2(50) := 'INC0221909_cpfs_ROLLBACK.sql';
  vr_nmarqlog           VARCHAR2(50) := 'INC0221909_log_execucao.txt';
  ...
  CURSOR cr_crapjur ( pr_cdcooper IN CECRED.crapass.CDCOOPER%TYPE
    ...
    select j.nrdconta
      ...
      , A.NRCPFCGC
    from crapjur j
    JOIN CRAPASS A ON J.NRDCONTA = A.NRDCONTA AND J.CDCOOPER = A.CDCOOPER
...
BEGIN
  ...
  vr_nmdireto := gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cpd/bacas/INC0221909';
  ...
  LOOP
    ...
    IF vr_tabela = 'NOMETALAO' THEN
      
      OPEN cr_crapjur(vr_cdcooper, vr_nrdconta);
    ...
    IF vr_tabela = 'NOMETALAO' THEN
      ...
      UPDATE CECRED.CRAPJUR j
        SET j.nmtalttl = REPLACE( nmtalttl, CHR(26), '' )
      WHERE j.cdcooper = vr_cdcooper
        AND j.nrdconta = vr_nrdconta;
      
      gene0001.pc_escr_linha_arquivo(vr_ind_arquiv, '    UPDATE CECRED.CRAPJUR SET  '
                                                      || ' nmtalttl = '''    || rw_crapjur.nmtalttl || ''' '
                                                      || 'WHERE nrdconta = ' || vr_nrdconta
                                                      || '  AND cdcooper = ' || vr_cdcooper || '; ' );
      
      gene0001.pc_escr_linha_arquivo(vr_ind_arqlog, SQL%ROWCOUNT || ' registros atualizados na CRAPJUR para Conta x Cooperativa x CNPJ : ' || vr_nrdconta || ' x ' || vr_cdcooper || ' x ' || rw_crapjur.NRCPFCGC);
      
      
    ELSIF vr_tabela = 'CRAPASS' THEN
