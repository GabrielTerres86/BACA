DECLARE
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      raise_application_error(-20001, 'pc_efetiva_reg_pendentes '||vr_qtprocessados||' - ' ||vr_nrdconta||' - '|| SQLERRM);
...
BEGIN
  ...
  BEGIN
    UPDATE CECRED.tbdomic_liqtrans_lancto
       SET insituacao = 1
     WHERE idarquivo IN (607401);
  END;

  BEGIN
    UPDATE tbdomic_liqtrans_pdv pdv
       SET pdv.cdocorrencia = '00'
          ,pdv.DSERRO = NULL
