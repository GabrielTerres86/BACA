DECLARE
  ...
  BEGIN
    ...
    IF nvl(vr_cdcritic, 0) > 0 OR TRIM(vr_dscritic) IS NOT NULL THEN
      vr_cdcritic := 0;
      vr_dscritic := 'Erro ao debitar valores provisionados de juros +60 para conta ' ||
                     pr_nrdconta;
      RAISE vr_exc_saida;
    END IF;
  ...
  LOOP
    IF utl_file.IS_OPEN(vr_arquivo) THEN
      BEGIN
        ...
        vr_conta := cecred.GENE0002.fn_quebra_string(pr_string => vr_dslinha
                                                    ,pr_delimit => '|');
