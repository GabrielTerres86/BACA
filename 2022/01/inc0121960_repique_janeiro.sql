DECLARE 
  ...
  CURSOR cr_consor IS
   select * from craplau p where p.cdhistor in (1230, 1231, 1232, 1233, 1234, 2027)
         and p.dtmvtopg >= to_date('01/01/2022','dd/mm/yyyy')
...
--  vr_nmdireto := gene0001.fn_diretorio(pr_tpdireto => 'C', pr_cdcooper => 3);
  ...
  pc_valida_direto(pr_nmdireto => vr_nmdireto || '/INC0121960',
                   pr_dscritic => vr_dscritic);
  ...
  vr_nmdireto := vr_nmdireto || '/INC0121960';
  ...
  vr_nmarqbkp := 'ROLLBACK_INC0121960' || to_char(sysdate, 'hh24miss') ||
                 '.sql';
  ----------------------------------------------------------------------
