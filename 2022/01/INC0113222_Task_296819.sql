DECLARE
  ...
  /*
  Cooperativa: 14 - Evolua
  Conta: 103756
  Contrato: 41507

  Reenviar o pagamento das parcelas 38 a 48 com a dataPagamentoEstorno de 21/10/2021.
  Realizar isso em ambiente TEST ou individual e em seguida rodar a procedure que faz os lançamentos no extrato e me chamar para conferir.
  */
  ...
  CURSOR cr_craplcm IS       
    SELECT b.dtvencto,
           ...
           b.cdcooper,
           b.nrparepr,
           to_date('21/10/2021', 'DD/MM/YYYY') as dtmvtolt,
           min(b.idsequencia) idsequencia
      FROM tbepr_consignado_pagamento b,
          (SELECT cdcooper, nrdconta, nrctremp, nrparepr
             FROM crappep
            WHERE (cdcooper, nrdconta, nrctremp) IN ((14, 103756, 41507))
              AND nrparepr BETWEEN 38 AND 48
              AND inliquid = 0
           ) pep
