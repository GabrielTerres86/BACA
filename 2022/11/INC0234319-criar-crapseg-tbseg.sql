...
select w.*, e.cdagenci , e.cdbccxlt, e.cdopeori, e.cdageori,
       case when w.nrctrseg in (50305, 93577,266561) then
         1
       else 0 end idimpdps
...
if  (upper(cecred.gene0001.fn_database_name) like '%AYLLOSP%' or upper(cecred.gene0001.fn_database_name) like '%AILOSTS%') then
  ...
  ds_nome_diretorio_v := ds_nome_diretorio_v || 'cpd/bacas/INC0234319/rollback';
  ...
  ds_nome_diretorio_v := ds_nome_diretorio_v || '/INC0234319/rollback';
...
for r01 in c01 loop
  ...
  if  (qt_reg_arquivo >= 50000) then
    ...
    ds_nome_arquivo_bkp_v := 'ROLLBACK2_INC0234319_'||nr_arquivo||'.sql';
