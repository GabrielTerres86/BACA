...
BEGIN
   BEGIN
     ...
     vr_nmdirrec := gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/INC0130989';     
     vr_nmarq    := 'ROLLBACK_INC0130989.sql';   

     vr_index:= 1;
     vr_typ_tab_arquiv(vr_index).nrproposta := '770358617212';
     vr_typ_tab_arquiv(vr_index).cdcooper   := 1;
     vr_index := vr_index + 1;
     vr_typ_tab_arquiv(vr_index).nrproposta := '770357310415';
     vr_typ_tab_arquiv(vr_index).cdcooper   := 1;
     vr_index := vr_index + 1;
     vr_typ_tab_arquiv(vr_index).nrproposta := '770355055035';
     vr_typ_tab_arquiv(vr_index).cdcooper   := 1;
               ...
               FETCH cr_crawseg INTO rw_crawseg;
                 IF cr_crawseg%FOUND THEN
                   ...
                   UPDATE crawseg
                    ...
                    if rw_crawseg.nrproposta = '770357310415' then
                        vr_linha :=
                             '  UPDATE crapseg ' ||
                             '     SET cdsitseg = 2'||
                   ...
                   elsif rw_crawseg.nrproposta = '770358617212' then
                     vr_linha :=
                             '  UPDATE crapseg ' ||
                             '     SET cdsitseg = 5, '||
                             '         dtfimvig = '||'''22/02/2022'''||
