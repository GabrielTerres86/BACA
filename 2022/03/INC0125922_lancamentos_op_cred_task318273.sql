DECLARE
  ...
  FROM (
    SELECT REGEXP_SUBSTR('146471,163384,268135,269247,672130,673552,8637733','[^,]+',1,LEVEL) AS vr_nrdconta
       ,REGEXP_SUBSTR('129089,93695,70210,104701,166378,166691,4101609','[^,]+',1,LEVEL) AS vr_nrctremp
       ,REGEXP_SUBSTR('13,13,13,13,13,13,1', '[^,]+', 1, LEVEL) AS vr_cdcooper
       ,REGEXP_SUBSTR('0,0,0,0,0,0,0', '[^,]+', 1, LEVEL) AS vr_nrparepr
       ,REGEXP_SUBSTR('0.02,0.08,0.09,0.03,0.08,0.10,0.03','[^,]+',1,LEVEL) AS vr_vllanmto
       ,REGEXP_SUBSTR('1711,1711,1711,1711,1711,1711,1711','[^,]+',1,LEVEL) AS vr_cdhistor
   FROM DUAL
  CONNECT BY REGEXP_SUBSTR('146471,163384,268135,269247,672130,673552,8637733','[^,]+',1,LEVEL) IS NOT NULL );
