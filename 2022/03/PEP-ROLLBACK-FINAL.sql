DECLARE
  ...
  vr_script_retorno VARCHAR2(32767);
  vr_script CLOB;
...
BEGIN
  ...
  dbms_lob.createtemporary(vr_script, TRUE, dbms_lob.CALL);
  dbms_lob.open(vr_script,dbms_lob.lob_readwrite); 
  gene0002.pc_escreve_xml(vr_texto_carga,vr_texto_carga_aux,'CPF Pesquisado;' ||
                                                            ...
                                                            'Data Termino;' ||
                                                            'Log Execucao' || CHR(10));
  gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'BEGIN ' || CHR(10));
  FOR rw_crapass IN cr_crapass LOOP
    ...
    dbms_lob.createtemporary(vr_res_body, true);
    BEGIN
      BEGIN
        LOOP
          UTL_HTTP.read_text(vr_res, l_text, l_buffer);
          DBMS_LOB.writeappend (vr_res_body, LENGTH(l_text), l_text);
        END LOOP;
      EXCEPTION
        WHEN UTL_HTTP.end_of_body THEN
          UTL_HTTP.end_response(vr_res);
      END;
      v_xmltype := XMLTYPE(vr_res_body);
      ...
      IF instr(vr_res_body, '<DATA>') > 0 THEN
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/DS_RELACIONAMENTO', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_dsrelacmnto
               FROM dual;
      
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/CPF_CNPJ_STR', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_cpfpesquisa
               FROM dual; 
               
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/NM_PESSOA', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_nomepesquisa
               FROM dual;
               
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/CPF_PEP', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_cpfpep
               FROM dual;
               
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/PEP', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_nomepep
               FROM dual;
               
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/DS_CARGO', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_dsocupacao
               FROM dual;
               
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/DT_INICIO', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_dsinicio
               FROM dual;
             
        SELECT EXTRACTVALUE(v_xmltype, '//ConsultaResponse/ConsultaResult/DATA/PessoaPEP/DT_FIM', 'xmlns="http://tempuri.org/"')
               INTO vr_pep.vr_dstermino
               FROM dual;
          ...
          IF vr_count = 0 THEN
            ...
            IF vr_altera THEN
              ...
              VALUES
                ...
                gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'DELETE FROM tbcadast_pessoa_polexp WHERE idpessoa = ' || vr_pep.vr_idpessoa || ';' || CHR(10));
            END IF;
          ELSE 
        ...
        END IF;
          vr_pep.vr_log := vr_pep.vr_log || 'Alterando crapttl:' || rw_crapass.idseqttl || '/' || rw_crapass.cdcooper || '/' || rw_crapass.nrdconta || '|';
          IF vr_altera THEN            
           SELECT inpolexp
            INTO vr_inpolexp
            FROM crapttl
           WHERE idseqttl = rw_crapass.idseqttl
            ...
            UPDATE crapttl
             ...
             WHERE idseqttl = rw_crapass.idseqttl
               AND cdcooper = rw_crapass.cdcooper
               AND nrdconta = rw_crapass.nrdconta;
               gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'UPDATE crapttl
                       SET inpolexp = ' || vr_inpolexp || '
                     WHERE idseqttl = ' ||rw_crapass.idseqttl||'
                       AND cdcooper = ' ||rw_crapass.cdcooper||'
          ...
          SELECT COUNT(1)
           ...
           WHERE idseqttl = rw_crapass.idseqttl
             ...
             AND nrdconta = rw_crapass.nrdconta;
          IF vr_count = 0 THEN
            vr_pep.vr_log := vr_pep.vr_log || 'Incluindo tbcadast_politico_exposto:' || rw_crapass.idseqttl || '/' || rw_crapass.cdcooper || '/' || rw_crapass.nrdconta || '|';
            IF vr_altera THEN
              ...
              VALUES
                (rw_crapass.cdcooper,rw_crapass.nrdconta,rw_crapass.idseqttl,vr_pep.vr_tpexposto,vr_pep.vr_dtinicio,vr_pep.vr_dttermino
                ,vr_pep.vr_nomepep,vr_pep.vr_cdocupacao,vr_pep.vr_cdrelacmnto,vr_pep.vr_cpfpep);
            gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'DELETE FROM tbcadast_politico_exposto WHERE cdcooper = ' || rw_crapass.cdcooper || ' AND nrdconta = ' || rw_crapass.nrdconta || ' AND idseqttl = ' || rw_crapass.idseqttl || ';' || CHR(10));
            END IF;
          ELSE 
    ...
    utl_http.end_response(vr_res);
  END LOOP;
  gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'COMMIT; ' || CHR(10));
  gene0002.pc_escreve_xml(vr_script,vr_script_retorno,'END; ' || CHR(10));
  ...
  gene0002.pc_clob_para_arquivo(pr_clob     => vr_texto_carga
   ...
   gene0002.pc_clob_para_arquivo(pr_clob     => vr_script
   ,pr_caminho  => vr_dsdireto
  ...
  dbms_lob.freetemporary(vr_script);
  commit;
END;
