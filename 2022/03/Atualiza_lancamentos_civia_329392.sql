declare
  ...
  vr_nrdconta crapass.nrdconta%TYPE := 392413;
  vr_nrctremp craplem.nrctremp%TYPE := 129492;
  ...
  CURSOR cr_lancamento IS
    SELECT REGEXP_SUBSTR('1705,1705,1711,1711,1708,1708,1041,3027','[^,]+', 1, LEVEL) as vr_cdhistor,
           REGEXP_SUBSTR('173.33,75.89,0.06,0.03,3.46,1.52,4.55,249.22','[^,]+', 1, LEVEL) as vr_vllanmto,
           REGEXP_SUBSTR('7,7,7,7,7,7,0,7','[^,]+', 1, LEVEL) as vr_nrparepr           
      FROM DUAL
   CONNECT BY REGEXP_SUBSTR('1705,1705,1711,1711,1708,1708,1041,3027','[^,]+', 1, LEVEL) IS NOT NULL;     
