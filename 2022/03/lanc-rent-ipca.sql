DECLARE
   ...
   CURSOR cr_registro IS   
  SELECT substr(o.dsmensagem,1,INSTR(o.dsmensagem,'-')-1) AS registro  FROM tbgen_prglog l, tbgen_prglog_ocorrencia o 
     WHERE l.idprglog = o.idprglog
       AND l.idprglog = 44902921
       AND UPPER(o.dsmensagem) NOT LIKE '%UPDATE%';
   rw_registro cr_registro%ROWTYPE;
       
   CURSOR cr_craprac (pr_cdcooper NUMBER
                     ,pr_nrdconta NUMBER
...
BEGIN
  ...
  FOR rw_registro in cr_registro LOOP
     vr_split := gene0002.fn_quebra_string(rw_registro.registro,';');
     
     OPEN cr_craprac(vr_split(1)
                    ,vr_split(2)
                    ,vr_split(3));
     ...
     BEGIN
        UPDATE craprac SET dtinical = to_date(EXTRACT(DAY FROM rw_craprac.dtmvtolt) || '/01/2022','dd/mm/yyyy')
     ...
     apli0006.pc_posicao_saldo_aplicacao_pos( pr_cdcooper => rw_craprac.cdcooper,
      ...
      IF vr_vlultren > 0 THEN
        ...
        vr_dslog := rw_craprac.cdcooper || ';'||rw_craprac.nrdconta||';'||rw_craprac.nraplica||';'|| vr_vlultren || ' ';
        ...
        CECRED.pc_log_programa(pr_dstiplog      => 'O' 
                              ...
                              ,pr_dsmensagem    => vr_dslog
                              ,pr_cdmensagem    => 0
                              ,pr_cdprograma    => 'INC0128817_IPCA_LANC'
                              ,pr_cdcooper      => 3 
                              ,pr_idprglog      => vr_idprglog); 
  ...
  EXCEPTION 
     WHEN vr_excsaida then  
         CECRED.pc_log_programa(pr_dstiplog      => 'O' 
                           ...
                           ,pr_dsmensagem    => vr_dslog
                           ,pr_cdmensagem    => 0
                           ,pr_cdprograma    => 'INC0128817_IPCA_ERRO'
                           ,pr_cdcooper      => 3 
                           ,pr_idprglog      => vr_idprglog);   
         ROLLBACK;    
     WHEN OTHERS then
