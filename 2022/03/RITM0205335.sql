DECLARE
  ...
  vr_nrctremp NUMBER := 230031;
  vr_cdagenci NUMBER := 51;
  vr_vllanmto craplem.vllanmto%TYPE := 236875.30;

  vr_txmensal crapepr.txmensal%TYPE := 1.8;
  vr_cdhistor craplem.cdhistor%TYPE := 2409;
...
BEGIN
  ...
  UPDATE crapepr e
     SET e.vlsprjat = e.vlsdprej
        ,e.vlsdprej = e.vlsdprej + vr_vllanmto
        ,e.vljraprj = e.vljraprj + vr_vllanmto
        ,e.vljrmprj = vr_vllanmto
        ,e.txmensal = vr_txmensal
   WHERE e.cdcooper = vr_cdcooper
     AND e.nrdconta = vr_nrdconta 
     AND e.nrctremp = vr_nrctremp;
