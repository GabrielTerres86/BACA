/* Alterações: 05/09/2013 - Nova forma de chamar as agências, de PAC agora 
                            a escrita será PA (André Euzébio - Supero).
                            
               29/11/2013 - Adicionado "DO TRANSACTION" no CREATE craperr
                            na procedure estorna-faturas. (Reinert)
...
DEFINE VARIABLE p_cdchave   AS CHAR       NO-UNDO.
DEFINE VARIABLE p_autchave  AS INTE       NO-UNDO.
DEFINE VARIABLE p_valor     AS DEC        NO-UNDO.
DEFINE VARIABLE p_valordoc  AS DEC        NO-UNDO.
...
&Scoped-define WEB-FILE F:/web/crap081d.html

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.radio ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_codbarras ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_fat1 ab_unmap.v_fat2 ab_unmap.v_fat3 ab_unmap.v_fat4 ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_valordoc ab_unmap.v_valorpago 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.radio ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_codbarras ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_fat1 ab_unmap.v_fat2 ab_unmap.v_fat3 ab_unmap.v_fat4 ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_valordoc ab_unmap.v_valorpago 
...
------------------------------------------------------------------------*/
  ...
  ASSIGN radio:SCREEN-VALUE IN FRAME {&FRAME-NAME}  = radio.
  --*/ 
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
     ...
     * RUN assignFields. */
     ...
     IF  get-value("estorno") <> "" THEN DO:
        {&OUT}
            '<script> location="crap081.w" </script>'.
    END.
    ELSE  DO:
        c_radio           = get-value("v_pradio").
        ASSIGN radio       = c_radio.
        ASSIGN v_codbarras = get-value("v_pbarras").
        ASSIGN v_valordoc  = 
               STRING(DEC(get-value("v_pvalordoc")),"zzz,zzz,zzz,zz9.99").
        ASSIGN v_valorpago =
               STRING(DEC(get-value("v_pvalor")),"zzz,zzz,zzz,zz9.99").
        ASSIGN v_fat1      = 
               STRING(DEC(SUBSTR(v_codbarras,1,11)),"99999,99999,9")
               v_fat2      =
               STRING(DEC(SUBSTR(v_codbarras,12,11)),"99999,99999,9")
               v_fat3      =
               STRING(DEC(SUBSTR(v_codbarras,23,11)),"99999,99999,9")
               v_fat4      =
               STRING(DEC(SUBSTR(v_codbarras,34,11)),"99999,99999,9").
    
        
        IF  get-value("ok") <> "" THEN DO:

            /* Para obter campos p_autchave e p_cdchave */
            ...
            IF  RETURN-VALUE = "OK" THEN DO:
                ...
                RUN gera-estorno-correspondente 
                            IN h_b1crap81(INPUT v_coop,
                                          INPUT v_operador,
                                          INPUT INT(v_pac),    
                                          INPUT INT(v_caixa),
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE estorna-faturas w-html 
...
------------------------------------------------------------------------------*/
    ...
    IF  l-houve-erro = NO THEN DO:   
        ...
        {&OUT}
          '<script> window.location = "crap081.html" </script>'.
    END.
