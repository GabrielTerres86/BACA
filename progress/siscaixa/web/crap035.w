/* Alteracoes: 25/06/2018 - Alteracoes para usar as rotinas mesmo com o processo 
...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD v_lote AS CHARACTER FORMAT "X(256)":U 
...
DEFINE VARIABLE h-b1crap35  AS HANDLE     NO-UNDO.
DEF VAR l-imprime           AS LOG        NO-UNDO.
...
&Scoped-define WEB-FILE F:/web/crap035.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_lote ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_lote ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.v_data AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     ab_unmap.v_lote AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR fill-in ab_unmap.v_data IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR fill-in ab_unmap.v_lote IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("v_lote":U,"ab_unmap.v_lote":U,ab_unmap.v_lote:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
     ...
     * RUN assignFields. */
     ...
     ELSE DO:
         ...
         v_lote = STRING(22000 + INT(v_caixa)).
         ...
         ELSE DO:
            ...
            ELSE DO:
                ...
                RUN dbo/b1crap35.p PERSISTENT SET h-b1crap35.                   
        
                IF  get-value("botao1") = "Visualizar"  THEN  
                    ASSIGN l-imprime = no.
                ELSE 
                    ASSIGN l-imprime = yes.

                RUN impressao IN h-b1crap35 (INPUT  v_coop,
                                             INPUT  v_operador,
                                             INPUT  int(v_pac),
                                             INPUT  INT(v_caixa),
                                             INPUT  INT(v_lote),
                                             INPUT  l-imprime, 
                                             OUTPUT p-nome-arquivo).
                                                             
                DELETE PROCEDURE h-b1crap35.       
                IF RETURN-VALUE = "NOK" THEN  DO:
                   {include/i-erro.i}
                END.
                ELSE DO:
                   IF  l-imprime = NO  THEN DO:
                       ASSIGN vh_arquivo = HostURL + "/" + p-nome-arquivo.
                   END.
                   ELSE DO:
                       ASSIGN vh_arquivo = " ".
