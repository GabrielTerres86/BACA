...
DEFINE VARIABLE h-b1crap31 AS HANDLE     NO-UNDO.
DEFINE VARIABLE h-b1crap00 AS HANDLE     NO-UNDO.
DEF VAR l-detalhado        AS LOG        NO-UNDO.
...
&Scoped-define WEB-FILE crap031.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_msg ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_msg ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
    ...
    {include/assignfields.i} /* Colocado a chamada do assignFields dentro da include */
     ...
     ELSE DO:
        ...
        ELSE DO:
            ...
            RUN dbo/b1crap31.p PERSISTENT SET h-b1crap31.                   
        
            IF  get-value("botao") = "Visualizar" THEN
                ASSIGN l-imprime = no.
            ELSE 
                ASSIGN l-imprime = yes.

            IF  radio = "yes"  THEN
                ASSIGN l-detalhado = YES.
            ELSE
                ASSIGN l-detalhado = NO.
               
          

            RUN impressao IN h-b1crap31 (INPUT v_coop,
                                         INPUT  v_operador,
                                         INPUT  int(v_pac),
                                         INPUT  INT(v_caixa),
                                         INPUT  l-imprime, 
            ...
            DELETE PROCEDURE h-b1crap31.       
            IF RETURN-VALUE = "NOK" THEN  DO:
               {include/i-erro.i}
            END.
            ELSE DO:
                IF  l-imprime = NO  THEN DO:
                    IF  SEARCH(p-nome-arquivo) <> ? THEN do:
