/*---------------------------------------------------------------------------
Alteracoes: 13/12/2013 - Adicionado validate para tabela craperr (Tiago).

           19/06/2018 - Alterado para chamar a procedure valida-transacao2 e assim permitir
...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD v_sequencia AS CHARACTER FORMAT "X(256)":U 
...
DEFINE VARIABLE h-b1crap78 AS HANDLE     NO-UNDO.
...
DEF var p-nrcheque       AS DEC     FORMAT "zzz,zz9".           /* Cheque */
DEF var p-nrddigc3       AS INT     FORMAT "9".                  /* C3 */

DEF VAR p-sequencia AS INTE.
DEF VAR p-valor     AS DEC.
DEF VAR p-histor    AS INTE.
DEF VAR p-literal        AS CHAR    NO-UNDO.
DEF VAR p-ult-sequencia  AS INTE    NO-UNDO.
DEF VAR p-registro       AS RECID   NO-UNDO.
DEF VAR p-docto          AS INTE    NO-UNDO.
...
&Scoped-define WEB-FILE crap078.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_sequencia ab_unmap.v_cmc7 ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg  ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_foco ab_unmap.v_caixa  ab_unmap.v_sequencia ab_unmap.v_cmc7 ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.v_caixa AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     ab_unmap.v_sequencia AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR FILL-IN ab_unmap.v_caixa IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN ab_unmap.v_sequencia IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
RUN htmAssociate
    ("v_sequencia":U,"ab_unmap.v_sequencia":U,ab_unmap.v_sequencia:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
...
------------------------------------------------------------------------*/
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
    ...
    RUN valida-transacao2 IN h-b1crap00(INPUT v_coop,  /* 19/06/2018 - Alterado para a procedure valida-transacao2 - Everton Deserto(AMCOM). */
                                       INPUT v_pac,
                                       INPUT v_caixa).
    ...
    DO:
        ...
        DO:    
            ...
            ELSE
            DO:
                IF   get-value("manual") <> ""   THEN
                     DO:
                         {&OUT}
                            '<script>window.location = 
                              "crap078a.w?v_sequencia=" +"'get-value("v_sequencia")'"</script>'.
                     END.
                ELSE
                DO:
                    RUN dbo/b1crap78.p PERSISTENT SET h-b1crap78.
                    RUN critica-autenticacao 
                                IN h-b1crap78(INPUT v_coop,
                                              INPUT  v_pac,
                                              INPUT  v_caixa,
                                              INPUT  v_sequencia,
                                              OUTPUT p-valor,
                    ...
                    DO:  
                        RUN dbo/b1crap78.p PERSISTENT SET h-b1crap78.
                        RUN valida-codigo-cheque-valor
                                   IN h-b1crap78(INPUT v_coop,
                                                 INPUT v_pac,
                                                 INPUT v_caixa,
                                                 INPUT v_sequencia,
                        ...
                        DO:
                            ...
                            DO:  
                                ...
                                {&OUT}
                                  '<script>window.location = "crap078.html"
                                  </script>'.
                            END.
                        END.
                    END.
