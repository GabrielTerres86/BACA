...
DEFINE VARIABLE h-b1crap84       AS HANDLE                     NO-UNDO.

DEFINE VARIABLE v_comprovante    AS CHARACTER                  NO-UNDO.
DEFINE VARIABLE v_documento      AS DECIMAL                    NO-UNDO.

DEFINE VARIABLE p-literal        AS CHAR                       NO-UNDO.
DEFINE VARIABLE p-literal-cop    AS CHAR                       NO-UNDO.
DEFINE VARIABLE p-ult-sequencia  AS INTE                       NO-UNDO.
DEFINE VARIABLE p-registro       AS RECID                      NO-UNDO.
DEFINE VARIABLE p-registro-crapcbb AS RECID                    NO-UNDO.
...
&Scoped-define WEB-FILE F:/web/crap084.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_cartao ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_senha ab_unmap.v_valor 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_cartao ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_senha ab_unmap.v_valor 
...
DELETE PROCEDURE h-b1crap00.
 ...
 ELSE
     ASSIGN p-literal-cop = " "
            p-ult-sequencia = 9999.
            
/* Autenticacao Cooperativa + BB */
ASSIGN p-literal = p-literal-cop + v_comprovante.

RUN dbo/b1crap84.p PERSISTENT SET h-b1crap84.
RUN grava-comprovante IN h-b1crap84 (INPUT p-registro, INPUT p-literal).

/* se for Recebimento, gera os registros no lote e na crapcbb */
IF  GET-VALUE("ok") <> " "   THEN
    DO:
       RUN gera-recebimento
                         IN h-b1crap84 (INPUT v_coop,
                                        INPUT v_operador,
                                        INPUT INT(v_pac),
                                        INPUT INT(v_caixa),
                                        INPUT v_cartao,
                                        INPUT DECIMAL(v_valor),        
                                        INPUT 3, /* 1-tit, 2-fat, 3-inss */
                                        INPUT p-literal,
...
DELETE PROCEDURE h-b1crap84.

/* Abre a tela que faz a impressao do comprovante */
{&OUT} '<script>window.open("autentica.html?v_plit=" + "' p-literal '" + 
     ...
     * RUN assignFields. */
     ...
     ELSE
         DO:
             ...
             ELSE
                 DO:
                     IF  GET-VALUE("consulta") <> " "   THEN
                         DO:
                             /* Instancia a procedure */
                             RUN dbo/b1crap84.p PERSISTENT SET h-b1crap84.
                             RUN executa-consulta 
                                       IN h-b1crap84(INPUT v_coop,
                                                     INPUT INT(v_pac),
                                                     INPUT INT(v_caixa),
                                                     INPUT v_operador,
                     ...
                     IF  GET-VALUE("ok") <> " "   THEN
                         DO:
                             /* Instancia a procedure */
                             RUN dbo/b1crap84.p PERSISTENT SET h-b1crap84.
                             RUN executa-recebimento IN h-b1crap84(
                                                        INPUT v_coop,
                                                        INPUT INT(v_pac),
                                                        INPUT INT(v_caixa),
                                                        INPUT v_operador,
