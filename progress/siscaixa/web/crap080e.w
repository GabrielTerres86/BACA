...
&Scoped-define WEB-FILE F:/web/crap080e.html

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac 
 ...
 {&OUT}
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
     ...
     * Set any values that need to be set, then display them. */
     ...
     IF  AVAIL crapbcx THEN 
         DO:
            ASSIGN crapbcx.flgauten = YES.
            RELEASE crapbcx.
         END.    
         
     IF get-value("recibo") <> "" THEN
        DO:
            {&OUT}
             '<script> 
         bematech.ImpAjust("LPT1","' get-value
         ("v_pliteral_recibo") '",48,1,2,0,0,0,0) 
        </script>'.
     ...
     ELSE
        DO:
         ...
         {&OUT}
             '<script> 
      bematech.Imprimir("LPT1","'
               trim(get-value("v_pliteral")) '",2,1,0,0,0,0) 
               </script>'. 
  ...
  ELSE DO:
   ...
   FIND LAST crapbcx  EXCLUSIVE-LOCK WHERE 
              ...
              crapbcx.cdopecxa = v_operador          AND
              crapbcx.cdsitbcx = 1  NO-ERROR NO-WAIT.
   IF  AVAIL crapbcx THEN DO:
       ASSIGN crapbcx.flgauten = NO.
       RELEASE crapbcx.
    END.    
