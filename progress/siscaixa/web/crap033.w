/*-------------------------------------------------------------------------------------------------------------
   Alteracoes: 22/06/2018 - Alterado para chamar a procedure valida-transacao2 e assim permitir
...
DEFINE VARIABLE h-b1crap33  AS HANDLE     NO-UNDO.
DEF VAR p-nome-arquivo      AS CHAR       NO-UNDO.
DEF VAR p-nome-arquivo-res  AS CHAR       NO-UNDO.
...
&Scoped-define WEB-FILE crap033.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_msg ab_unmap.v_lote ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_arquivo ab_unmap.v_msg ab_unmap.v_lote ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
...
------------------------------------------------------------------------*/
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
    ...
    assign v_lote = string(11000 + int(v_caixa)).
    
    assign v_lote = string(int(v_caixa)).
     ...
     ELSE DO:
        ...
        RUN valida-transacao2 IN h-b1crap00(INPUT v_coop,  /* 22/06/2018 - Alterado para  procedure valida-transacao2 - Everton Deserto(AMCOM).*/
                                           INPUT v_pac,
                                           INPUT v_caixa).
        ...
        ELSE DO:
           ...
           ELSE DO:
              ...
              RUN dbo/b1crap33.p PERSISTENT SET h-b1crap33.                   
        
              IF  radio = "yes"  THEN DO:
                  RUN impressao IN h-b1crap33 (INPUT  v_coop,
                                               INPUT  v_operador,
                                               INPUT  int(v_pac),
                                               INPUT  INT(v_caixa),
                                               INPUT  INT(v_lote),
                                               INPUT yes,   /* Detalhado */ 
                                               OUTPUT p-nome-arquivo-res,
              ...
              ELSE DO:
                 IF  radio = "no"  THEN DO:
                     IF  SEARCH(p-nome-arquivo) <> ? THEN DO:
                         ASSIGN vh_arquivo = HostURL + "/" + p-nome-arquivo-res.
                         MESSAGE vh_arquivo.
                     END.
                     ELSE
                         ASSIGN vh_arquivo = "".
