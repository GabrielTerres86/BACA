...
Objetivo  : Pagamento de Emprestimos (Sem Conta-Corrente)
...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD v_contrato AS CHARACTER FORMAT "X(256)":U 
...
DEF VAR h-b1crap90 AS HANDLE          NO-UNDO.
...
&Scoped-define WEB-FILE F:/web/crap090.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.ok ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_conta ab_unmap.v_contrato ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_mensagem ab_unmap.v_msg ab_unmap.v_nome ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_valor 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.ok ab_unmap.vh_foco ab_unmap.v_caixa ab_unmap.v_conta ab_unmap.v_contrato ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_mensagem ab_unmap.v_msg ab_unmap.v_nome ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_valor 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.v_conta AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     ab_unmap.v_contrato AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR fill-in ab_unmap.v_conta IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR fill-in ab_unmap.v_contrato IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("v_contrato":U,"ab_unmap.v_contrato":U,ab_unmap.v_contrato:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
     ...
     ELSE DO:
         ...
         ELSE DO:
             ...
             ELSE DO:
                 ...
                 RUN dbo/b1crap90.p PERSISTENT SET h-b1crap90.
                 RUN valida-pagto-emprestimo    
                     IN h-b1crap90(INPUT v_coop,
                                   INPUT  INT(v_pac),
                                   INPUT  INT(v_caixa),
                                   INPUT  INT(v_conta),
                                   INPUT  INT(v_contrato),                      
                                   INPUT  0,  /* Valor */
                                   INPUT  v_operador,
                                   INPUT  1,  /* 1 - Validar Conta e Contrato */
                                   OUTPUT v_nome,
                                   OUTPUT v_mensagem).
                     ...
                     IF  get-value("ok") <> "" THEN DO:
                         RUN valida-pagto-emprestimo    
                             IN h-b1crap90(INPUT v_coop,
                                           ...
                                           INPUT INT(v_conta),
                                           INPUT INT(v_contrato),
                                           INPUT DEC(v_valor),   /* Valor */
                                           INPUT v_operador,
                                           INPUT 2,  /* 2 - Validar Valores */
                                           OUTPUT v_nome,
                                           OUTPUT v_mensagem).
                                ...
                                IF  RETURN-VALUE = "NOK" THEN DO:
                                    ...
                                    FOR EACH craperr NO-LOCK 
                                       WHERE craperr.cdcooper = 
                                             crapcop.cdcooper
                                         AND craperr.cdagenci = 
                                             INT(v_pac)        
                                         AND craperr.nrdcaixa = 
                                             INT(v_caixa):
