/*..............................................................................
   ...
   Sigla   : CRED
   Autor   : Lucas Lunelli
   Data    : 15/09/2014                         Ultima atualizacao: 31/05/2016
...
DEFINE TEMP-TABLE ab_unmap     
       ...
       FIELD tmp_cdempcon      AS CHARACTER FORMAT "X(256)":U       
...
DEFINE FRAME Web-Frame    
    ...
    ab_unmap.v_cdempcon AT ROW 1 COL 1 HELP
          ...
          VIEW-AS SELECTION-LIST SINGLE NO-DRAG 
          SIZE 20 BY 4
    ab_unmap.tmp_cdempcon AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("tmp_cdempcon":U,"ab_unmap.tmp_cdempcon":U,ab_unmap.tmp_cdempcon:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
     ...
     ELSE
        DO:
             ASSIGN opcao       = ""
                    ...
                    v_dssencrd  = ab_unmap.v_dssencrd
                    l-houve-erro = FALSE.
             ...
             IF  v_cartao <> '' THEN 
                DO:
                    ...
                    RUN retorna-conta-cartao IN h-b1crap30 (INPUT v_coop,
                                                            INPUT v_pac,
                                                            INPUT v_caixa,
                                                            INPUT DECI(v_cartao),
                                                           OUTPUT v_conta,
            ...
            {&OUT} '<script> tmp_objempcon = new Array();'.

             FOR EACH tt-convenios-codbarras NO-LOCK:
             
                ASSIGN aux_nmextcon = REPLACE(tt-convenios-codbarras.nmextcon, "'", "").
             
                IF  INTEGER(v_segmento) = 99 THEN 
                    DO:
                        vetor = vetor + "~{" +
                                "nmextcon:'" +  STRING(aux_nmextcon)   + "'," + 
                                "cdempcon:'" +  STRING(tt-convenios-codbarras.cdempcon)   + "/" +  STRING(tt-convenios-codbarras.cdsegmto) + "'"  + "~}".
                    END.
                ELSE
                    DO:
                        vetor = vetor + "~{" +
                                "nmextcon:'" +  STRING(aux_nmextcon)   + "'," + 
                                "cdsegmto:'" +  STRING(tt-convenios-codbarras.cdsegmto)   + "'," + 
                                "cdempcon:'" +  STRING(tt-convenios-codbarras.cdempcon)   + "'"  + "~}".
                    END.
             
                {&OUT} 'tmp_objempcon.push("' + STRING(vetor) + '");'.
            ...
            IF   v_btn_ok <> ""         AND
                 ...
                 DO:
                    ...
                    RUN valida_senha_cartao IN h-b1crap30 (INPUT v_coop,
                                                           INPUT v_pac,
                                                           INPUT v_caixa,
                                                           INPUT INT(v_conta),
                    ...
                    ELSE
                        DO:
                            ...
                            ELSE
                                DO:
                                    {include/i-erro.i}
                                    {&OUT} '<script> window.location = "crap030a.htm?v_conta=' + REPLACE(STRING(GET-VALUE("v_conta")), ".", "") + '";</script>'.
                                END.
                        END.
                 END.
        END.
  ...
  ELSE DO:
    ...
    IF   get-value("v_cdempcon") <> "" THEN
         DO:
             ...
             ASSIGN vh_foco       = "15"
                    tmp_cdempcon  = STRING(INTEGER(get-value("v_cdempcon"))).
