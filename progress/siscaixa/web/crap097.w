...
DEFINE TEMP-TABLE ab_unmap
	   ...
	   FIELD v_dtatendi AS CHARACTER FORMAT "X(256)":U
	   FIELD v_hratendi AS CHARACTER FORMAT "X(256)":U
	   FIELD v_flagpost AS CHARACTER FORMAT "X(256)":U.
...
DEFINE VARIABLE v_dsimpres   AS CHAR                            NO-UNDO.
...
&Scoped-define WEB-FILE crap097.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_foco ab_unmap.v_msg ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_dtatendi ab_unmap.v_hratendi ab_unmap.v_flagpost
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_foco ab_unmap.v_msg ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_dtatendi ab_unmap.v_hratendi ab_unmap.v_flagpost
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.v_pac AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1     
	 ab_unmap.v_dtatendi AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
	 ab_unmap.v_hratendi AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
	 ab_unmap.v_flagpost AT ROW 1 COL 1 HELP
          "0" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1		  
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("v_dtatendi":U,"ab_unmap.v_dtatendi":U,ab_unmap.v_dtatendi:HANDLE IN FRAME {&FRAME-NAME}).  
  RUN htmAssociate
    ("v_hratendi":U,"ab_unmap.v_hratendi":U,ab_unmap.v_hratendi:HANDLE IN FRAME {&FRAME-NAME}).  	
  RUN htmAssociate
    ("v_flagpost":U,"ab_unmap.v_flagpost":U,ab_unmap.v_flagpost:HANDLE IN FRAME {&FRAME-NAME}).  	
...
------------------------------------------------------------------------*/
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
    ...
    FIND crapcop WHERE crapcop.nmrescop = glb_nmrescop NO-LOCK NO-ERROR. 
	
	ASSIGN v_dsimpres = STRING(crapcop.cdagectl,"9999") + "-" + STRING(glb_cdagenci,"999") + "-" + STRING(glb_cdbccxlt,"999") +  " Atendimento: " + STRING(v_dtatendi) + " - " + STRING(v_hratendi).
	
	v_flagpost = "1". /*flg para evitar loop infinito na requisicao*/
	
	{&OUT} '<script language="javascript">window.open("autentica.html?v_plit=' + v_dsimpres + '&v_pseq=1&v_prec=NO&v_psetcook=yes&v_rotina=crap097","waut","width=250,height=145,scrollbars=auto,alwaysRaised=true");</script>'.
