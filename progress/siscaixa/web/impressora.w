/* .............................................................................
   ...
   Programa: Fontes/impressora.w
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Adriano
   Data    : 05/2011                                Ultima atualizacao: 

   Dados referentes ao programa: Mostra rotina IP - Impressoras.
...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD v_impres AS CHARACTER FORMAT "X(256)":U
       FIELD v_nvoimpres AS CHARACTER FORMAT "X(256)":U.
...
&Scoped-define WEB-FILE impressora.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_impres ab_unmap.v_nvoimpres 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_msg ab_unmap.v_operador ab_unmap.v_pac ab_unmap.v_impres ab_unmap.v_nvoimpres 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.v_pac AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
    ab_unmap.v_impres AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
    ab_unmap.v_nvoimpres AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR FILL-IN ab_unmap.v_pac IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN ab_unmap.v_impres IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN ab_unmap.v_nvoimpres IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("v_impres":U,"ab_unmap.v_impres":U,ab_unmap.v_impres:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
    ("v_nvoimpres":U,"ab_unmap.v_nvoimpres":U,ab_unmap.v_nvoimpres:HANDLE IN FRAME {&FRAME-NAME}).
END PROCEDURE.
...
------------------------------------------------------------------------*/
  ...
  IF REQUEST_METHOD = "POST":U THEN DO:
     ...
     * Set any values that need to be set, then display them. */
     ...
     FIND crapcop WHERE crapcop.nmrescop = v_coop
                        NO-LOCK NO-ERROR.
     ...
     FIND crapope WHERE crapope.cdcooper = crapcop.cdcooper AND
                        crapope.cdoperad = v_operador
                        NO-LOCK NO-ERROR.

     v_impres = crapope.dsimpres. 

     IF get-value("ok") <> "" THEN
        DO:
           ASSIGN aux_grep = ""
 	              aux_nmimpres = "^" + v_nvoimpres + ":".
           ...
           IF SEARCH("/var/spool/lp/interface/" +
              ...
              DO:
                 RUN cria-erro (INPUT v_coop,
                                ...
                                INPUT INT(v_caixa),
                                INPUT 0,
                                INPUT "Impressora nao cadastrada.",
                                INPUT YES).
           ...
           ELSE
              DO:
                 DO aux_contador = 1 TO 10:
                    ...
                    FIND CURRENT crapope EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
                    ...
                    DO:
                       ...
                       DO:
                           PAUSE(1) NO-MESSAGE.
                           aux_dscritic = "Tabela sendo alterada p/ outro terminal.".
                           NEXT.
                       END.
                       ELSE
                       DO:
                          aux_dscritic = "Nao foi possivel alterar a impressora.".
                          LEAVE.
                 ...
                 ELSE
                 DO:
                     ASSIGN v_impres = v_nvoimpres
                            crapope.dsimpres = v_nvoimpres
                            v_nvoimpres = "".
                 END.
  ...
  ELSE DO:
    ...
    ASSIGN v_impres = crapope.dsimpres.
