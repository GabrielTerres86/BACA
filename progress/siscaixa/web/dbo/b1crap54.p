...
PROCEDURE valida-cheque-avulso-conta:
    ...
    ELSE     /* Cartao */
         DO:
             ...
             RUN valida_cartao IN h-b1wgen0032 (INPUT crapcop.cdcooper,
                                                INPUT STRING(p-cartao),
...
PROCEDURE valida-permissao-saldo-conta:
    ...
    IF   p-opcao = "C"   THEN
         DO:
             FIND crapcrm WHERE crapcrm.cdcooper = crapcop.cdcooper   AND
                                crapcrm.nrcartao = p-nrcartao 
                                NO-LOCK NO-ERROR.
            ...
            DO:
                FIND FIRST craptco WHERE  craptco.cdcooper = crapcop.cdcooper AND
                                          craptco.nrctaant = INTE(SUBSTR(STRING(p-nrcartao), 8, 8)) AND
                ...
                DO:
                    ...
                    DO: 
                        ASSIGN i-cod-erro  = 0
                        c-desc-erro = "Cartao nao encontrado.".
            ...
            END.
             ...
             IF   crapcrm.dssencar <> ENCODE(p-senha-cartao)   THEN
                  DO:
                      ASSIGN i-cod-erro  = 0
                             c-desc-erro = "Senha do cartao incorreta.".
