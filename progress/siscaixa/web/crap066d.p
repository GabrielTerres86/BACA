/*** crap066d.p = AUTENTICACAO DE CHEQUES DA COOPERATIVA ***/
...
/*------------------------------------------------------------------------
  ...
  Description: Lançamento de cheques (adaptacao da rotina 51 + lanchq)
...
DEF TEMP-TABLE ab_unmap
    ...
    FIELD vh_cont-autentica AS CHAR.
...
-----------------------------------------------------------------------------*/
   ...
   */
    ...
    DEFINE VARIABLE cContAutentica AS CHARACTER  NO-UNDO.
  ...
  {&OUT}
    ...
    '<title>AUTENTICAÇÃO DE CHEQUE COOPERATIVA</title>' SKIP
    "</HEAD>":U SKIP
    ...
    '<div align="center">' SKIP
      ...
      '<table width="100%" class=cabtab>' SKIP
        '<tr>' SKIP
          ...
          '</td>' SKIP
          '<td width="20%" align="center"> PAC&nbsp;' ab_unmap.v_pac SKIP
          '</td>' SKIP
  ...
  {&OUT}
    '<div align="center">' SKIP
     '<center>' SKIP
      '<table width="100%" class=cabtab>' SKIP
       ' <tr>' SKIP
         '  <td class=titulo>AUTENTICAÇÃO DE CHEQUE COOPERATIVA</td>' SKIP
         '  <td width="21%" class=programa align="right">P.051d&nbsp; V.1.00</td>' SKIP
       ' </tr>' SKIP
      '</table>' SKIP
     '</center>' SKIP
    '</div>' SKIP.
         ...
         '  <center>' SKIP
          '  <table width="100%" cellspacing = 0 cellpadding = 1 class=tcampo>' SKIP
           ...
           ' </tr>'
            ...
            '</tr>'
            '<tr>'
              '<td align="right" class="linhaform" width="23%">Agência Cheque:</td>'
              '<td align="left" class="linhaform" width="77%">'
            ...
            '</tr>' 
            '<tr>'
               '<td align="right" class="linhaform" width="23%">Conta Cheque:</td>'
                 '<td align="left" class="linhaform" width="77%">'
                 '<input type="text" name="v_contacheque" size="10" class="input" maxlength="10" style="text-align: right" onKeyDown="f_formatacontaX(event)" onKeyUp="f_trataX(event)">'
                 '&nbsp;'
               '</td>'
            '</tr>' 
            '<tr>'
              '<td align="right" class="linhaform" width="23%">Número Cheque:</td>'
              '<td align="left" class="linhaform" width="77%">'
            ...
            '<tr><td align="right" class="tcampo" style="BACKGROUND-COLOR:#FFFFCE; border=0" colspan=7>&nbsp;</td></tr>'
              '<tr>'
               '<td align="center" class="tcampo" style="BACKGROUND-COLOR:#FFFFCE; border=0" colspan=7>'
                ...
                '<input type="button" value="Retornar" name="fecha" class="button" onClick="JavaScript:fechaAutentica ();">&nbsp;'
              '</tr>' SKIP.
   {&OUT}
 ...
 DO:
     ...
     DO WHILE TRUE:
         FIND FIRST crapmdw EXCLUSIVE-LOCK
              WHERE crapmdw.cdcooper = crapcop.cdcooper
                ...
                AND crapmdw.nrctabdb = INT(get-value("v_contacheque")) 
     ...
     DO:
         ...
         DO:
             ...
             DO:
                 ASSIGN aux_nrcheque = 
                            INTE(STRING(crapmdw.nrcheque,"zzz,zz9") +                                             STRING(crapmdw.nrddigc3,"9")).
                 ...
                 RUN grava-autenticacao
                       IN h-b1crap00 (INPUT v_coop,
                                      ...
                                      INPUT "1",  /* On-line            */                                             INPUT NO,   /* Nao estorno        */
                                      INPUT 386, 
                                      INPUT ?, /* Data off-line */
...
/* Vander */
             ...
             DO:
                 ...
                 IF get-value("ContAutentica") = "" THEN
                     ASSIGN cContAutentica = "1".
                 ELSE
                     ASSIGN cContAutentica = STRING(INTEGER(get-value("ContAutentica")) + 1).
             END.
         END.
     END.
 END.
 ...
 {&OUT}
   ...
   '<input type="hidden" name="ContAutentica" value="' cContAutentica '">'

   '<script language="JavaScript">' SKIP

   'function fechaAutentica () ~{' SKIP
   '  opener.reabilitaForm();' SKIP
   '  window.close();' SKIP
   '  if (window.document.forms[0].ContAutentica.value > 0)' SKIP
   '    opener.document.forms[0].sok.click();' SKIP
   '~}' SKIP

   'function f_formatacontaX(teclapres) ~{' SKIP
   '  var tecla = teclapres.keyCode;' SKIP
   '  vr = window.document.forms[0].v_contacheque.value;' SKIP 
   '  vr = vr.replace( ".", "" );' SKIP
   '  vr = vr.replace( ".", "" );' SKIP
   '  vr = vr.replace( ".", "" );' SKIP
   '  vr = vr.replace( ".", "" );' SKIP
   '  contacheque = vr;' SKIP 
   '  tam = vr.length;' SKIP
   '  if (tam < 10 && tecla != 8) ~{ tam = vr.length + 1 ; ~}' SKIP
   '  if (tecla == 8 ) ~{ tam = tam - 1 ; ~}' SKIP
   '  if ( tecla == 8 || tecla == 88 || (tecla >= 48 && tecla <= 57) || (tecla >= 96 && tecla <= 105))  ~{' SKIP
   '     if ( tam <= 1 ) ~{ contacheque = vr ; ~}' SKIP
   '     if ( (tam > 1) && (tam <= 4) ) ~{ ' SKIP
   '        contacheque = vr.substr( 0, tam - 1 ) + "." + vr.substr( tam - 1, tam ) ; ~}' SKIP
   '     if ( (tam >= 5) && (tam <= 9) ) ~{' SKIP
   '        contacheque = vr.substr( 0, tam - 4 ) + "." + vr.substr( tam - 4, 3 ) + "." + vr.substr( tam - 1, tam ) ; ~}' SKIP
   '     window.document.forms[0].v_contacheque.value = contacheque; ~}' SKIP
   '  else ~{ if (tecla != 9 && tecla != 46 && (tecla < 37 || tecla > 40)) ~{ event.returnValue = false; ~} ~}' SKIP
   '~}' SKIP

   'function f_trataX(teclapres) ~{' SKIP
   '   var tecla = teclapres.keyCode;' SKIP
   '  if (tecla == 88) ~{' SKIP
   '     vr = window.document.forms[0].v_contacheque.value;' SKIP 
   '     vr = vr.replace("X", "0");' SKIP
   '     vr = vr.replace("x", "0");' SKIP
   '     window.document.forms[0].v_contacheque.value = vr; ~}' SKIP
   '~}' SKIP
