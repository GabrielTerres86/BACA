...
DEF VAR  h-b1crap34 AS HANDLE     NO-UNDO.
...
DEF VAR c-arquivo-url AS CHAR FORMAT "x(50)"          NO-UNDO.
...
&Scoped-define WEB-FILE crap034.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS ab_unmap.vh_arquivo ab_unmap.vh_foco ab_unmap.v_msg ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.vh_arquivo ab_unmap.vh_foco ab_unmap.v_msg ab_unmap.radio ab_unmap.v_caixa ab_unmap.v_coop ab_unmap.v_data ab_unmap.v_operador ab_unmap.v_pac 
     ...
     IF  get-value("cancela") = "" THEN DO:
         ...
         ELSE DO:
             ...
             ELSE do:   
                 ...
                 IF  get-value("botao") = "Visualizar"  THEN DO:
                     
                     FIND crapcop WHERE crapcop.nmrescop = v_coop NO-LOCK NO-ERROR.
                     
                     ASSIGN i-dia =  inte(radio).
                     ASSIGN c-arquivo     = "/usr/coop/sistema/siscaixa/web/spool/" + 
                                            crapcop.dsdircop + 
                                            string(int(v_pac),"999") + 
                                            string(int(v_caixa),"999") + 
                                            c-dia[i-dia] + ".txt"  /* Nome Fixo  */
                            c-arquivo-url = "spool/" + 
                                            crapcop.dsdircop +
                                            string(int(v_pac),"999") + 
                                            string(int(v_caixa),"999") + c-dia[i-dia] + ".txt".

                     IF  SEARCH(c-arquivo) <> ? THEN do: 
                         ASSIGN vh_foco = STRING(i-dia + 6).
                         ASSIGN vh_arquivo = HostURL + "/" + c-arquivo-url.
                     END.
                 END.
      
                 IF  get-value("botao") = "Gerar"  THEN DO:

                     RUN dbo/b1crap34.p PERSISTENT SET h-b1crap34.

                     ASSIGN i-dia =  inte(radio).
         
                     RUN gera-arquivo-texto IN h-b1crap34(INPUT v_coop,
                                                          INPUT v_pac,
                                                          INPUT v_caixa,
                                                          INPUT i-dia,
                                                          OUTPUT c-arquivo).
                     DELETE PROCEDURE h-b1crap34.
                     IF   SEARCH(c-arquivo) <> ? THEN do: 
                         ASSIGN vh_foco = STRING(i-dia + 6).
                         ASSIGN vh_arquivo = HostURL + "/" + c-arquivo.
                     END.
                END.
             END.
         END.
