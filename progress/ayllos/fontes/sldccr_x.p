/* ............................................................................
   ...
   Programa: Fontes/sldccr_x.p
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Odair
   Data    : Abril/97.                          Ultima atualizacao: 09/04/2015

   Dados referentes ao programa:

   Frequencia: Diario (on-line)
   Objetivo  : Rotina para cancelamento de cartoes de credito.

   Alteracoes: 26/08/98 - Alterado para a nova tabela de limites de cartao -  
                          craptlc (Deborah).
               01/09/98 - Ajustes nos motivos de cancelamento (Deborah).

              17/01/2001 - Substituir CCOH por COOP (Margarete/Planner).

              07/05/2004 - So altera status de desfeito cancelamento apos
                           validacao da administradora (Julio).

              20/05/2004 - Impressao do termo de cancelamento (Julio).  
              
              18/06/2004 - Inibicao da validacao do cancelamento do cartao
                           para administradora desativada (Julio).

              27/01/2006 - Unificacao dos Bancos - SQLWorks - Luciane.
              
              20/06/2006 - Alterado para atualizar campo flgctitg ao cancelar
                           cartao (Diego).
                           
              11/08/2006 - Atualizar o campo flgctitg de acordo com a
                           administradora do cartao (Evandro).
                           
              12/09/2006 - Excluida opcao "TAB" (Diego).
              
              19/03/2007 - Excluido campo tel_nrprotoc (Elton).
              
              29/01/2008 - Alterado de cancelamento para bloqueio quando for
                           Cartao BB (Guilherme).
              ...
              18/05/2010 - Incluido novo motivo para cancelamento de cartao
                           "Por fraude" (Elton).
                           
              30/07/2010 - Mostrar mensagem de alerta retornada pela procedure
                           desfaz_cancblq_cartao (David).
                           
              20/10/2010 - Alteracao para imprimir termo de pessoa jurica
                          (Gati - Daniel).                          
                          
              09/04/2015 - #272659 Adicionado alerta para bloqueio de cartão BB
                           (Carlos)
............................................................................ */
...
DEF VAR tel_dscancel AS CHAR INIT "Cancelar"                           NO-UNDO.
...
FORM SKIP(1)
     ...
     NO-LABELS TITLE COLOR NORMAL " Cancelamento/Bloqueio " FRAME f_tit.

FORM SKIP(1)
     par_nrcrcard  LABEL "Numero do cartao" FORMAT "9999,9999,9999,9999"
     HELP "Entre com o numero do cartao"           AT 10 "   "
     SKIP(1)
     tel_dsmotivo  LABEL "Motivo do cancelamento"  AT 4
     HELP "Use as setas para escolher o motivo do cancelamento"
     SKIP(1)
     WITH SIDE-LABELS ROW 11 OVERLAY CENTERED 
     TITLE COLOR NORMAL " Cancelamento/Bloqueio " FRAME f_canc.
...
FORM SKIP(1)
     ...
     SKIP(1)
     tel_dsmotivo  LABEL "Motivo do cancelamento"  AT 5
     HELP "Use as setas para escolher o motivo do cancelamento"
     SKIP(1)
     WITH SIDE-LABELS ROW 11 OVERLAY CENTERED 
     TITLE COLOR NORMAL " Cancelamento/Bloqueio " FRAME f_canc_pj.
...
DO WHILE TRUE ON ENDKEY UNDO, LEAVE:
    ...
    RUN verifica_acesso_cancblq IN h_b1wgen0028 (INPUT glb_cdcooper,
                                                 INPUT 0, 
                                                 INPUT 0, 
                                                 INPUT glb_cdoperad,
                                                 INPUT tel_nrdconta,
    ...
    IF  aux_flgadmbb  THEN
        ASSIGN tel_dscancel                       = "Bloqueio"
               tel_dsdesfaz                       = "Desbloqueio"
               tel_dsmotivo:LABEL IN FRAME f_canc = "Motivo do Bloqueio"
               tel_dsmotivo:LABEL IN FRAME f_canc_pj = "Motivo do Bloqueio".
    
    DISPLAY tel_dscancel tel_dsdesfaz WITH FRAME f_tit.
    
    CHOOSE FIELD tel_dscancel tel_dsdesfaz WITH FRAME f_tit.
    ...
    IF  FRAME-VALUE = tel_dscancel  THEN
        DO:
            ...
            RUN verifica_canc_cartao IN h_b1wgen0028 (INPUT glb_cdcooper,
                                                      INPUT 0, 
                                                      INPUT 0, 
                                                      INPUT glb_cdoperad,
                                                      INPUT tel_nrdconta,
            ...
            IF   aux_tipopess = 2 THEN  
                DISPLAY tel_repsolic par_nrcrcard WITH FRAME f_canc_pj.
            ELSE
                DISPLAY par_nrcrcard WITH FRAME f_canc.
            ...
            DO WHILE TRUE ON ENDKEY UNDO, LEAVE:
                ...
                IF   aux_tipopess = 2 THEN 
                     DO:
                        UPDATE tel_repsolic tel_dsmotivo WITH FRAME f_canc_pj
                                ...
                                DO:
                                    ...
                                    IF  KEYFUNCTION(LASTKEY) = "CURSOR-RIGHT"  THEN
                                        DO:
                                            ...
                                            DISPLAY tel_repsolic WITH FRAME f_canc_pj.
                                        END.
                                ...
                                DO:
                                    IF  KEYFUNCTION(LASTKEY) = "CURSOR-RIGHT"  THEN
                                        DO:
                                            ...
                                            DISPLAY tel_dsmotivo WITH FRAME f_canc_pj.
                                        END.
                                    ELSE
                                    IF  KEYFUNCTION(LASTKEY) = "CURSOR-LEFT"  THEN
                                        DO:
                     ...
                     END.
                ELSE DO:
                    UPDATE tel_dsmotivo WITH FRAME f_canc
                            ...
                            DO:
                                IF  (KEYFUNCTION(LASTKEY) = "CURSOR-UP"     OR
                                    ...
                                    DO:
                                        ...
                                        DISPLAY tel_dsmotivo WITH FRAME f_canc.
                                    END.
                                ELSE
            ...
            IF  KEYFUNCTION(LASTKEY) = "END-ERROR"  THEN
                DO:
                    HIDE FRAME f_canc NO-PAUSE.
                    HIDE FRAME f_canc_pj NO-PAUSE.
                    NEXT.
                END.
            ...
            RUN cancela_bloqueia_cartao IN h_b1wgen0028
                             (INPUT glb_cdcooper,
                              INPUT 0, 
                              INPUT 0, 
                              INPUT glb_cdoperad,
            ...
            IF   aux_tipopess = 1 THEN 
                RUN fontes/cancelcrd.p (INPUT par_nrctrcrd,
                                        INPUT par_nrcrcard,
                                        INPUT par_cdadmcrd).
            ...
            IF   aux_tipopess = 2 THEN
                 DO:
                     ...
                     RUN termo_cancela_cartao IN h_termos ( INPUT glb_cdcooper,
                                                            INPUT glb_cdoperad,
                                                            INPUT glb_nmdatela,
                                                            INPUT tel_nrdconta,
                                                            INPUT glb_dtmvtolt,
    ...
    IF  FRAME-VALUE = tel_dsdesfaz  THEN
        DO:  
            ...
            RUN desfaz_cancblq_cartao IN h_b1wgen0028
                             (INPUT glb_cdcooper,
                              INPUT 0,
                              INPUT 0,
                              INPUT glb_cdoperad,
