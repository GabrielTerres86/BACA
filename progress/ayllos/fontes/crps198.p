...
FORM rel_dsempres[rel_cdempres]     AT   1 FORMAT "x(25)"
...
FOR EACH crapemp NO-LOCK WHERE crapemp.cdcooper = glb_cdcooper:
    ...
    rel_dsempres[crapemp.cdempres] = STRING(crapemp.cdempres, "99999") + " - " 
                                     + crapemp.nmresemp.
END.
...
FOR EACH cratest NO-LOCK BREAK BY cratest.cdagenci 
    ...
    IF   cratest.qtestemp > 0   THEN
         DO:        
             DISPLAY STREAM str_1 rel_dsempres[rel_cdempres]     
                                  cratest.qtestemp
                                  cratest.vlestemp
