...
on return of b_buscas in frame f_fonte do:
   ...
   if   aux_opened <> aux_caminho   OR
        tel_dsarquiv = ""           then
        do:
            tel_dsarquiv:read-file(aux_caminho).
            aux_opened = aux_caminho.
        end.
...
if   search("/usr/coop/cecred/log/check_cdcooper.lst") = ?   then
     do:
...
input stream str_1 from 
     value("/usr/coop/cecred/log/check_cdcooper.lst") no-echo.
...
aux_confirma = false.
do  while true on endkey undo, retry:
    message "O programa foi inteiramente verificado, deseja libera-lo?"
            update aux_confirma format "S/N".
    leave.
end.
