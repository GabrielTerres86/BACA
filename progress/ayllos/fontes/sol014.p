/* .............................................................................
   ...
   Objetivo  : Mostrar a tela SOL014.

   Alteracao : 29/04/97 - Permitir solicitar por agencia (Odair)
...
DEF        VAR tel_nrseqsol AS INT     FORMAT "z9"                   NO-UNDO.
...
DEF        VAR aux_nrsolici AS INT     FORMAT "z9" INIT 14           NO-UNDO.
...
FORM SKIP (3)
     ...
     SKIP(2)
     tel_cdagenci AT 19 LABEL "PA        "
                  HELP "Entre com o codigo do PA ou 999 para todos"
                  VALIDATE (tel_cdagenci > 0,"134 - PA errado")
     SKIP (2)
     ...
     WITH ROW 4 COLUMN 1 OVERLAY TITLE COLOR MESSAGE tel_dstitulo
          SIDE-LABELS NO-ATTR-SPACE WIDTH 80 FRAME f_sol014.

glb_cddopcao = "I".

FIND craprel WHERE craprel.cdcooper = glb_cdcooper  AND
                   craprel.cdrelato = 23            NO-LOCK NO-ERROR.
...
DO WHILE TRUE:
        ...
        DISPLAY glb_cddopcao WITH FRAME f_sol014.
        NEXT-PROMPT tel_nrseqsol WITH FRAME f_sol014.

        DO WHILE TRUE ON ENDKEY UNDO, LEAVE:

           PROMPT-FOR glb_cddopcao tel_nrseqsol WITH FRAME f_sol014.
           LEAVE.
        ...
        IF   KEYFUNCTION(LASTKEY) = "END-ERROR"   THEN     /*   F4 OU FIM   */
             DO:
                 ...
                 IF   CAPS(glb_nmdatela) <> "SOL014"   THEN
                      DO:
                          HIDE FRAME f_sol014.
                          RETURN.
                      END.
             ...
             DO:
                 ...
                 IF   NOT AVAILABLE crapsol   THEN
                      DO:
                          ...
                          CLEAR FRAME f_sol014.
                          ASSIGN tel_nrseqsol = aux_nrseqsol.
                          DISPLAY tel_nrseqsol WITH FRAME f_sol014.
                          NEXT.
                      END.

                 ASSIGN  tel_cdagenci = INTEGER(crapsol.dsparame)
                         tel_nrdevias = crapsol.nrdevias.

                 DISPLAY tel_cdagenci tel_nrdevias WITH FRAME f_sol014.
             ...
             DO:
                 DO TRANSACTION ON ENDKEY UNDO, LEAVE:
                    ...
                    ASSIGN tel_nrdevias = crapsol.nrdevias
                           tel_cdagenci = INTEGER(crapsol.dsparame).
             ...
             DO:
                 ...
                 DO TRANSACTION ON ENDKEY UNDO, LEAVE:
                    ...
                    DO WHILE TRUE:
                       ...
                       SET tel_cdagenci tel_nrdevias WITH FRAME f_sol014.

                       IF   NOT CAN-FIND(crapage WHERE crapage.cdcooper = 
                                                       glb_cdcooper   AND
                                                       crapage.cdagenci =
                                               tel_cdagenci) AND
                                         tel_cdagenci <> 999 THEN
                            DO:
                                glb_cdcritic = 15.
                                BELL.
                                RUN fontes/critic.p.
                                MESSAGE glb_dscritic.
                                glb_cdcritic = 0.
                                NEXT-PROMPT tel_cdagenci WITH FRAME f_sol014.
                                NEXT.
                            END.
