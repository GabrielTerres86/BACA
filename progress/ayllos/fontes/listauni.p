/* .............................................................................
   ...
   Programa: Fontes/listauni.p
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Edson
   Data    : Outubro/95.                         Ultima atualizacao: 27/01/2006

   Dados referentes ao programa:

   Frequencia: Sempre que chamado por outros programas (on-line)
   Objetivo  : Mostrar a lista de aplicacoes rdca.
...
DEFINE VARIABLE s_column   AS INTEGER                                NO-UNDO.
DEFINE VARIABLE s_hide     AS LOGICAL                                NO-UNDO.
DEFINE VARIABLE s_row      AS INTEGER                                NO-UNDO.
DEFINE VARIABLE s_title    AS CHARACTER                              NO-UNDO.
DEFINE VARIABLE s_wide     AS LOGICAL                                NO-UNDO.

DEFINE VARIABLE s_chcnt    AS INTEGER                                NO-UNDO.
DEFINE VARIABLE s_chextent AS INTEGER                                NO-UNDO.
DEFINE VARIABLE s_chlist   AS CHARACTER EXTENT 500                   NO-UNDO.
DEFINE VARIABLE s_choice   AS INTEGER   EXTENT 50                    NO-UNDO.
DEFINE VARIABLE s_multiple AS LOGICAL                                NO-UNDO.
DEFINE VARIABLE s_dbfilenm AS CHARACTER                              NO-UNDO.

DEFINE VARIABLE p_down   AS INTEGER                                  NO-UNDO.
DEFINE VARIABLE p_flag   AS LOGICAL   INITIAL FALSE EXTENT 500       NO-UNDO.
...
DEFINE VARIABLE aux_nrctatos AS INT                                  NO-UNDO.
DEFINE VARIABLE aux_nraplica AS INT                                  NO-UNDO.
DEFINE VARIABLE rda_nraplica AS INT EXTENT 72                        NO-UNDO.
...
FOR EACH crapuni WHERE crapuni.cdcooper = glb_cdcooper   AND
                       crapuni.nrdconta = par_nrdconta   NO-LOCK
                       BREAK BY crapuni.nruniapl:

    IF   FIRST-OF(crapuni.nruniapl)   THEN
         DO:

             FIND craprda WHERE craprda.cdcooper = glb_cdcooper       AND
                                craprda.nrdconta = crapuni.nrdconta   AND
                                craprda.nraplica = crapuni.nruniapl
                                NO-LOCK NO-ERROR.

             IF   NOT AVAILABLE craprda   THEN
                  ASSIGN aux_nrctatos = aux_nrctatos + 1
                         aux_nraplica = crapuni.nruniapl

                         rda_nraplica[aux_nrctatos] = crapuni.nruniapl

                         s_chextent           = s_chextent + 1
                         s_chlist[s_chextent] =
                                  STRING(crapuni.nruniapl,"zzz,zz9") + "   " +
                                  "Unificacao a ser efetuada.".
             ELSE
                  ASSIGN aux_nrctatos = aux_nrctatos + 1
                         aux_nraplica = craprda.nraplica

                         rda_nraplica[aux_nrctatos] = craprda.nraplica

                         s_chextent           = s_chextent + 1
                         s_chlist[s_chextent] =
                                STRING(craprda.nraplica,"zzz,zz9") + "   " +
                                STRING(craprda.dtmvtolt,"99/99/9999") + "  " +
                                STRING(craprda.vlaplica,"zzz,zzz,zzz,zz9.99").
         END.

END.  /*  Fim do FOR EACH  --  Leitura das aplicacoes RDCA  */

IF   aux_nrctatos = 0   THEN
     DO:
         ASSIGN par_nraplica = 0
                glb_cdcritic = 465.
...
FORM p_flag[p_recid] FORMAT "* /  " SPACE(0) s_chlist[p_recid] FORMAT "x(40)"
...
IF   s_chcnt > 0   THEN
     par_nraplica = rda_nraplica[s_choice[s_chcnt]].
