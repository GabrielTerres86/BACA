/* .............................................................................
   ...
   Programa: Fontes/lancapa.p
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Edson
   Data    : Junho/92.                           Ultima atualizacao: 05/09/2013

   Dados referentes ao programa:

   Frequencia: Diario (on-line)
   Objetivo  : Processar a rotina de alteracao da tela LANCAP.

   Alteracoes: 15/03/95 - Eliminada a rotina de leitura dos lancamentos
                          (Deborah).

               01/04/98 - Tratamento para milenio e troca para V8 (Margarete).
               
               18/02/99 - Nao permitir saques de capital de ativos que         
                          resultem em capital inferior a R$1,00 (Deborah).
                          
               21/05/1999 - Dividir o lancamento em capital inicial e
               ...
               24/01/2002 - Nao eliminar lote 10002 (Margarete).
...
DO WHILE TRUE:
   ...
   DO TRANSACTION:
      ...
      DO WHILE TRUE ON ENDKEY UNDO, LEAVE:
         ...
         SET tel_cdhistor tel_nrdocmto tel_vllanmto
             WITH FRAME f_lancap
         ...
         IF   tel_nrdocmto = 0   THEN
              ...
              END.
         ELSE
         IF   tel_nrdocmto <> craplct.nrdocmto   THEN
              DO:
                  IF   CAN-FIND(craplct WHERE craplct.cdcooper = glb_cdcooper
                                          AND craplct.dtmvtolt = tel_dtmvtolt
                                          AND craplct.cdagenci = tel_cdagenci
                                          AND craplct.cdbccxlt = tel_cdbccxlt
                                          AND craplct.nrdolote = tel_nrdolote
                                          AND craplct.nrdconta = tel_nrdconta
                                          AND craplct.nrdocmto = tel_nrdocmto
                                          USE-INDEX craplct1)   THEN
                       glb_cdcritic = 92.
              END.
         ELSE
              IF   (craphis.inhistor = 17) AND
                   (CAN-FIND(FIRST craplct WHERE 
      ...
      IF   KEYFUNCTION(LASTKEY) = "END-ERROR"   THEN     /*   F4 OU FIM   */
           UNDO, NEXT.  /* Volta pedir a conta para o operador */
      ...
      ELSE
           IF   aux_inhistor = 16   THEN
                crapcot.vldcotas = crapcot.vldcotas + aux_vllanmto.
           ELSE
                IF   NOT CAN-DO("7,8,17,18",STRING(aux_inhistor))   THEN
                     DO:
                         glb_cdcritic = 214.
                         RUN fontes/critic.p.
                         BELL.
      ...
      IF   glb_cdcritic = 0   THEN
           DO:
               ...
               IF   aux_inhistor = 17   THEN
                    ...
                    END.
               ELSE
               IF   aux_cdhistor = 18   THEN
                    DO:
                        IF   tel_vllanmto > aux_vlcmicot   THEN
                             glb_cdcritic = 205.
                    END.
               ELSE
               IF   NOT CAN-DO("7,8",STRING(aux_inhistor))   THEN
                    glb_cdcritic = 214.
           END.
