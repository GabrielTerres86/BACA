...
FOR EACH crapcop 
   ...
   WHERE crapcob.cdcooper = crapceb.cdcooper
     ...
     AND crapcob.dtmvtolt <= crapdat.dtmvtolt
   ...
   WHERE crapsab.cdcooper = crapcob.cdcooper
    ...
    /* se titulo Cooperativa/EE e nao foi enviado ainda
       para a PG, não enviar ao DDA */
    IF crapcob.inemiten = 3 AND 
       crapcob.inemiexp <> 2 THEN NEXT.

    /* tratamento para não enviar para a cabine
    pois a cabine existe essa limitação de valor */
    IF crapcob.vltitulo > 9999999.99 THEN 
    DO:
        ASSIGN aux_insitpro = 0.
        ...
        RUN cria-log-cobranca IN h-b1wgen0088
              (INPUT ROWID(crapcob),
               INPUT glb_cdoperad,
               INPUT TODAY,
               INPUT "Falha no envio para a CIP (valor superior ao suportado)" ).
    ...
    FOR EACH  tt-retorno-dda 
        ...
        DO:
            ...
            IF  AVAIL(crapret) THEN 
                ASSIGN crapret.cdmotivo = crapret.cdmotivo + "A4".
...
PROCEDURE p_cria_titulo:
    ...
    ASSIGN tt-remessa-dda.cdlegado = "LEG"
           ...
           tt-remessa-dda.dtlipgto = (IF crapcob.flgdprot = TRUE THEN
                 ...
                 ELSE IF crapcco.dsorgarq = "EMPRESTIMO" THEN
                      INTE(STRING(YEAR(crapcob.dtvencto),"9999") +
                           STRING(MONTH(crapcob.dtvencto), "99") + 
                           STRING(DAY(crapcob.dtvencto), "99"))
                 ELSE
                      INTE(STRING(YEAR(crapcob.dtvencto + 52),"9999") +
                           ...
                           STRING(DAY(crapcob.dtvencto + 52), "99")))
           ...
           tt-remessa-dda.dtdmulta = (IF crapcob.tpdmulta = 3 OR NOT CAN-DO("1,2", STRING(crapcob.tpdmulta)) THEN ?
                                               ELSE INTE(STRING(YEAR(crapcob.dtvencto + 1),"9999") +
                                 STRING(MONTH(crapcob.dtvencto + 1), "99") + 
                                 STRING(DAY(crapcob.dtvencto + 1), "99")))
           tt-remessa-dda.cddmulta = (IF crapcob.tpdmulta = 3 OR NOT CAN-DO("1,2", STRING(crapcob.tpdmulta)) THEN "3"
                                               ELSE STRING(crapcob.tpdmulta))
           tt-remessa-DDA.vlrmulta = (IF crapcob.tpdmulta = 3 OR NOT CAN-DO("1,2", STRING(crapcob.tpdmulta)) THEN 0
