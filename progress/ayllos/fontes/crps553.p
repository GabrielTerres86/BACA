/* ............................................................................
   ...
   Programa: Fontes/crps553.p
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Guilherme/Supero
   Data    : Janeiro/2010.                    Ultima atualizacao: 28/08/2013.
                                                                          
   Dados referentes ao programa:

   Frequencia: Diario (Batch).
   Objetivo  : Relatorio de Tarifas da Compe - Por PAC - Mensal
               Gera relatorio crrl542
               Roda somente nas cooperativas - Cadeia Paralela - Sol. 4

   Alteracoes: 16/06/2010 - Acertos Gerais (Ze).

               07/07/2010 - Acertos no posicionamento do relatorio e inclusao
                            da descricao do PAC na primeira coluna
                            (Guilherme/Supero)
                            
               04/08/2010 - Alteracao na Soma dos Valores CHQ/TIT/DOC (Ze).
               
               14/09/2010 - Alteracao na solicitacao do programa (Ze).
               
               28/09/2010 - Alteracao no layout do relatorio 542 e incluido
                            devolucao dos cheques (Adriano).

               17/05/2011 - Inclusao do calculo de COB SR e no layout do
                            relatorio 542. (Guilherme/Supero)
               
               10/06/2011 - Alteracao acima nao estava fazendo a soma dos totais
                            se Cob SR (Guilherme)
                            
               16/07/2012 - Ajuste no layout do relatório (David Kruger).
...
DEF        VAR aux_qtdcdedi AS IN    INIT 0                          NO-UNDO.
...
DEF        VAR aux_fill_207 AS CHAR  FORMAT "x(207)"                 NO-UNDO.
...
DEF TEMP-TABLE w-relat-1                                             NO-UNDO
    ...
    INDEX w-relat-11 AS PRIMARY dsagenci.
...
FORM SKIP(1) 
     ...
     WITH NO-BOX NO-LABELS WIDTH 230 FRAME f_cabec_3. /*de 2 para 3*/
...
FORM SKIP
     w-relat-1.dsagenci   FORMAT "x(19)"                    AT 001
     /* CHEQUE */
...
FORM SKIP
     ...
     w-relat-1.qtdcdedi  FORMAT ">>>,>>9"                   AT 118
     w-relat-1.totcdedi  FORMAT ">>>,>>>,>>9.99"            AT 125
     w-relat-1.totchqde  FORMAT ">>>,>>>,>>9.99"            AT 140
     /* ICF */
...
/* Será executado sempre no primeiro dia do mes, */
/* sendo assim, a variavel glb_dtmvtoan conterá  */
/* a data do ultimo dia do mes anterior          */

ASSIGN aux_dataini = DATE(MONTH(glb_dtmvtoan),1,YEAR(glb_dtmvtoan))
       aux_datafim = glb_dtmvtoan.
       
/*............................................................................*/
/* Processa todos os lancamentos do periodo informado */
FOR EACH gntarcp WHERE gntarcp.cdcooper  = crapcop.cdcooper AND
                       gntarcp.dtmvtolt >= aux_dataini      AND 
                       gntarcp.dtmvtolt <= aux_datafim NO-LOCK
                       BREAK BY gntarcp.cdagenci:

    FIND FIRST crapage WHERE crapage.cdcooper = gntarcp.cdcooper AND
                             crapage.cdagenci = gntarcp.cdagenci
                             NO-LOCK NO-ERROR.
    IF   AVAIL crapage THEN
         aux_dsagenci = TRIM(STRING(crapage.cdagenci,"999") + "-" + 
                        crapage.nmresage).
    ELSE
         aux_dsagenci = "000-Sem Identific.".


    FIND FIRST w-relat-1 WHERE w-relat-1.dsagenci = aux_dsagenci
                               NO-LOCK NO-ERROR.
    
    IF   NOT AVAIL w-relat-1 THEN 
         DO:
             CREATE w-relat-1.
             ASSIGN w-relat-1.dsagenci = aux_dsagenci.
         END.
    ...
    CASE gntarcp.cdtipdoc:
        ...
        WHEN 9 OR WHEN 10 THEN 
         ...
         /* DOC NR */
         WHEN 5 THEN 
             ASSIGN w-relat-1.qtddocnr = w-relat-1.qtddocnr + gntarcp.qtdocmto
                    w-relat-1.totdocnr = w-relat-1.totdocnr - gntarcp.vldocmto
                    w-relat-1.totaldoc = w-relat-1.totaldoc - gntarcp.vldocmto.
                                                  
         /* DOC SR */
         WHEN 8 THEN 
             ASSIGN w-relat-1.qtddocsr = w-relat-1.qtddocsr + gntarcp.qtdocmto
                    w-relat-1.totdocsr = w-relat-1.totdocsr + gntarcp.vldocmto
                    w-relat-1.totaldoc = w-relat-1.totaldoc + gntarcp.vldocmto.
    ...
    IF   LAST-OF (gntarcp.cdagenci) THEN 
...
/* GERAR RELATORIO */
ASSIGN aux_nmarqimp    = "rl/crrl542.lst"
       glb_cdrelato[1] = 542
       glb_cdempres    = 11.
...
FOR EACH w-relat-1
    BREAK BY w-relat-1.dsagenci:

    DISPLAY STREAM str_1 w-relat-1.dsagenci w-relat-1.qtdchqnr
...
FOR EACH w-relat-1
    ...
    DISPLAY STREAM str_1 w-relat-1.dsagenci
...
FOR EACH w-relat-1
    ...
    DISPLAY STREAM str_1 w-relat-1.dsagenci  w-relat-1.qtdcronr
