...
/* limpa o arquivo */
unix silent value("> /usr/coop/cecred/log/check_cdcooper.lst").
...
if not aux_flgexist then
   unix silent value("rm /usr/coop/cecred/log/check_cdcooper.lst").
...
procedure verifica_programa:
  ...
  compile value(par_nmprogra) xref
          value("/usr/coop/cecred/log/check_cdcooper.xref") NO-ERROR.
          
  /* Se teve erro de compilacao */
  IF   error-status:num-messages <> 0   THEN
       return "NOK".
          
  input from value("/usr/coop/cecred/log/check_cdcooper.xref").
  ...
  /* remove o .xref */
  unix silent value("rm /usr/coop/cecred/log/check_cdcooper.xref").
  ...
  IF   NUM-RESULTS("qSearch") <> 0   THEN
       do:
           ...
           unix silent value("echo 'Programa: " + ttSearch.programa + "' " +
                             ">> /usr/coop/cecred/log/check_cdcooper.lst").
