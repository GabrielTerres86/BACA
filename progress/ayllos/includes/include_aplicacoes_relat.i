...
FIND craplot WHERE craplot.cdcooper = glb_cdcooper AND
              ...
              do:
                message "dtmvtolt " rd2_dtdolote skip
                        "cdagenci " rd2_cdagenci skip
                        "cdbccxlt " rd2_cdbccxlt skip
                        "nrdolote " rd2_nrdolote
                        view-as alert-box.
              end.
              ...
              find craplap where craplap.cdcooper = glb_cdcooper and
                                 ...
                                 no-lock no-error.
/*                                 
              if not avail craplap then
              do:
                  message "not avail craplap" view-as alert-box.
                  
                  message "nrdconta " craprda.nrdconta skip
                          "nraplica " craprda.nraplica skip
                          "dtmvtolt " craplot.dtmvtolt skip
                          "nrdolote " craplot.nrdolote skip
                          "cdbccxlt " craplot.cdbccxlt skip
                          "cdagenci " craplot.cdagenci skip
                          "vlrentot " rd2_vlrentot
                          view-as alert-box.
              end.
*/                                 
              find crablap where crablap.cdcooper = glb_cdcooper and
                                 ...
                                 no-lock no-error.
/*                                 
              if not avail crablap then
              do:
                  message "not avail crablap" view-as alert-box.
                  ...
                  message "nrdconta " craprda.nrdconta skip
                          ...
                          "cdbccxlt " craplot.cdbccxlt skip
                          "cdagenci " craplot.cdagenci skip
                          "vlirrdca " aux_vlirrdca
                          view-as alert-box.
              end.
  */
/* Saldo liquido Mês Anterior */
ASSIGN rd2_vlsdrdca = rd2_vlsdrdca - craplap.vllanmto + crablap.vllanmto
       aux_vlsldant = rd2_vlsdrdca.
...
DO WHILE TRUE:
   ...
   IF   craprda.inaniver = 0 /*1*/   THEN                             /* SC */
        ...
        FIND FIRST craptrd WHERE craptrd.cdcooper  = crapcop.cdcooper     AND
                                 craptrd.dtiniper  = ADD-INTERVAL(craprda.dtiniper, -1, 'months') AND
     ...
     DO:
       /*  DO WHILE TRUE:
         ...
         IF   aux_vlirrdca > 0   THEN      
              ...
              if not avail craplot then
              do:
                message "not avail craplot" view-as alert-box.
...
/* Relatorio 09/06 ****                 
          FIND craplot WHERE craplot.cdcooper = glb_cdcooper AND
                               craplot.dtmvtolt = 06/06/2014 AND
                               craplot.cdagenci = rd2_cdagenci AND
                               craplot.cdbccxlt = rd2_cdbccxlt AND
                               craplot.nrdolote = rd2_nrdolote
                               no-lock no-error.
          ...
          find craplap where craplap.cdcooper = glb_cdcooper and
                                 craplap.nrdconta = craprda.nrdconta and
                                 craplap.nraplica = craprda.nraplica and
                                 craplap.dtmvtolt = 06/06/2014 and
                                 craplap.nrdolote = craplot.nrdolote and
                                 craplap.cdbccxlt = craplot.cdbccxlt and
                                 craplap.cdagenci = craplot.cdagenci and
                                 craplap.cdhistor = 179 /* rendimento */
          ...
          do:                                 
            ASSIGN aux_vlrendim_dif = aux_vlrendim_dif - craplap.vllanmto.
            
            if aux_vlrendim_dif > 0 then            
                assign craplap.vllanmto = craplap.vllanmto + aux_vlrendim_dif               
                craplot.vlinfocr = craplot.vlinfocr + aux_vlrendim_dif
                craplot.vlcompcr = craplot.vlcompcr + aux_vlrendim_dif.
          end.
          ...
          find crablap where crablap.cdcooper = glb_cdcooper and
                                 crablap.nrdconta = craprda.nrdconta and
                                 crablap.nraplica = craprda.nraplica and
                                 crablap.dtmvtolt = 06/06/2014       and
                                 crablap.nrdolote = craplot.nrdolote and
                                 crablap.cdbccxlt = craplot.cdbccxlt and
                                 crablap.cdagenci = craplot.cdagenci and
                                 crablap.cdhistor = 862 /* IR */
          ...
          if avail crablap then
          do:
            assign aux_vlirrdca_dif = aux_vlirrdca_dif - crablap.vllanmto.
            
            if aux_vlirrdca_dif > 0 then            
                assign crablap.vllanmto = crablap.vllanmto + aux_vlirrdca_dif
                
                       craplot.vlinfocr = craplot.vlinfocr + aux_vlirrdca_dif
                       craplot.vlcompcr = craplot.vlcompcr + aux_vlirrdca_dif.
          end.
*********** */
