...
DEF TEMP-TABLE cratdir
    ...
    INDEX cratdir1 AS PRIMARY
          cdagenci fldemiss nrdconta.
    /* Valores flstatus:        */
    /*  1 - ATIVOS              */
    /*  2 - DEMITIDOS EM 2010   */
    /*  3 - DEMITIDOS           */
...
DEF VAR aux_anoatual    AS  CHAR  FORMAT "x(4)"  EXTENT 2  NO-UNDO.
...
ASSIGN aux_dtmvtolt = DATE (1,1,YEAR(glb_dtmvtolt))
       aux_anoatual = STRING(YEAR(aux_dtmvtolt))
       aux_qtdtotal = 0
       aux_qtdativo = 0
       aux_qtddemit = 0
       aux_qtdantig = 0
...
FOR EACH crapdir WHERE crapdir.cdcooper = glb_cdcooper
    ...
    ASSIGN aux_smposano = ROUND(aux_smposano / 12,2).
...
FOR EACH cratdir BREAK BY cratdir.cdagenci
                       BY cratdir.fldemiss
                       BY cratdir.nrdconta:
    ...
    IF cratdir.smposano > 0 THEN 
       ASSIGN aux_qtdtotal = aux_qtdtotal + 1
              aux_vlrtotal = aux_vlrtotal + cratdir.smposano.
    
    IF FIRST-OF(cratdir.cdagenci) THEN
    DO:
        IF  cratdir.cdagenci <> 1 THEN
            PAGE STREAM str_1.
    ...
    IF LAST-OF(cratdir.cdagenci) THEN
       DO:
          DISP STREAM str_1 aux_qtdativo aux_vlrativo aux_anoatual[1]
                            aux_qtddemit aux_vlrdemit aux_anoatual[2]
