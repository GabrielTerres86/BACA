...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD dseixtem AS CHARACTER FORMAT "X(256)":U 
...
ab_unmap.aux_stdopcao ab_unmap.dseixtem ab_unmap.idevento 
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.aux_cddopcao ~
ab_unmap.aux_dsendurl ab_unmap.aux_dsretorn ab_unmap.aux_lspermis ~
ab_unmap.aux_nrdrowid ab_unmap.aux_stdopcao ab_unmap.dseixtem ~
ab_unmap.idevento 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.aux_stdopcao AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     ab_unmap.dseixtem AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR fill-in ab_unmap.aux_stdopcao IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR fill-in ab_unmap.dseixtem IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE CriaListaXXXX w-html 
PROCEDURE CriaListaXXXX :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
/* Carrega o array */
FOR EACH xxxxxxx WHERE xxxxxx.campo = INTEGER(ab_unmap.campo) NO-LOCK BY xxxxxxx.campo:
    
    IF   vetorxxxxxxx = ""   THEN
         vetorxxxxxxx = "~{" + "nome do campo:" + "'" + STRING(xxxxxxx.campo) + "',nome do campo:" + "'" + xxxxxxx.campo + "'~}".
    ELSE
         vetorxxxxxxx = vetorxxxxxxx + "," + 
                        "~{" + "nome do campo:" + "'" + STRING(xxxxxxx.campo) + "',nome do campo:" + "'" + xxxxxxx.campo + "'~}".

END. /* for each */
          
RUN RodaJavaScript("var mxxxxxxx=new Array();mxxxxxxx=["  + vetorxxxxxxx + "]"). 
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("dseixtem":U,"ab_unmap.dseixtem":U,ab_unmap.dseixtem:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
      ...
      DO WITH FRAME {&FRAME-NAME}:
         IF opcao = "inclusao" THEN
            DO: 
                ...
                RUN inclui-registro IN h-b1wpgdXXXX(INPUT TABLE cratxxx, OUTPUT msg-erro, OUTPUT ab_unmap.aux_nrdrowid).
            END.
         ELSE  /* alteracao */
            DO:
                ...
                RUN altera-registro IN h-b1wpgdXXXX(INPUT TABLE cratxxx, OUTPUT msg-erro).
            END.    
...
FIND gnapses WHERE gnapses.idsessao = v-identificacao NO-LOCK NO-ERROR.
...
IF REQUEST_METHOD = "POST":U THEN 
   DO:
      ...
      IF VALID-HANDLE(h-b1wpgdXXXX) THEN
         DO:
            RUN posiciona-registro IN h-b1wpgdXXXX(INPUT TO-ROWID(ab_unmap.aux_nrdrowid), OUTPUT msg-erro).
            DELETE PROCEDURE h-b1wpgdXXXX NO-ERROR.
         END.
