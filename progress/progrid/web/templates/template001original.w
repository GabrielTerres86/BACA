...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION ValidaDados w-html 
FUNCTION ValidaDados RETURNS CHARACTER
    (INPUT tipo-validacao AS CHARACTER):

DEFINE VARIABLE m-erros AS CHARACTER.
ASSIGN m-erros = "".
...
DO WITH FRAME {&FRAME-NAME}:
   CASE tipo-validacao:
        WHEN "alteracao" THEN
             DO: 
                FIND FIRST {&SECOND-ENABLED-TABLE} WHERE {&SECOND-ENABLED-TABLE}.campo = INPUT {&SECOND-ENABLED-TABLE}.campo NO-LOCK NO-ERROR.
                IF NOT AVAIL {&SECOND-ENABLED-TABLE} THEN
                    ASSIGN m-erros    = m-erros + "XXXXXXXXXX não cadastrado"
                           v-qtdeerro = v-qtdeerro + 1.
        ...
        WHEN "inclusao" THEN
             DO:
                ...
                IF AVAIL {&SECOND-ENABLED-TABLE} THEN 
                    ASSIGN m-erros    = m-erros + "XXXXXXXXXX já cadastrado"
                           v-qtdeerro = v-qtdeerro + 1.
        ...
        WHEN "exclusao" THEN
             DO:
                ...
                /* verifica integridade em relação a outras tabelas relacionadas com o registro que será excluído */
                FIND FIRST crapxxx WHERE crapxxx.campo = {&SECOND-ENABLED-TABLE}.campo NO-LOCK NO-ERROR.

                IF  AVAILABLE crapxxx  THEN
                    ASSIGN m-erros    = m-erros + "XXXXXXXXX possui XXXXXXXX relacionados"
                           v-qtdeerro = v-qtdeerro + 1.
             END.
   END CASE.
END.
