/* .............................................................................
   Programa: progrid/web/dbo/b1wpgd0018.p
   Sistema : Progrid
   Sigla   :   
   Autor   : Rosangela
   ...
   Frequencia: 
   Objetivo  : Criação/Alteração/Exclusão de Custos dos Eventos
   
   Alteração : 29/08/2013 - Nova forma de chamar as agências, de PAC agora 
                              a escrita será PA (André Euzébio - Supero).

               06/01/2014 - Busca da critica 962, para crapage nao encontrada,
                            incluida (Carlos)
............................................................................ */
&SCOPED-DEFINE tttabela cratcdp
&SCOPED-DEFINE tabela   crapcdp
...
PROCEDURE valida-inclusao:
    ...
    */
    ...
    IF {&tttabela}.vlcuseve > 99999.99 THEN
    DO:
    ...
    FIND FIRST {&tabela} WHERE {&tabela}.idevento = {&tttabela}.idevento    AND
                               ...
                               {&tabela}.cdcuseve = {&tttabela}.cdcuseve    NO-LOCK NO-ERROR.

    IF AVAIL {&tabela} THEN
    DO:
        ASSIGN m-erros = m-erros + "Registro já existe com a chave informada.".
        RETURN "NOK".
    END.
...
PROCEDURE valida-alteracao:
    ...
    */
    ...
    /* Verificar se a flag FECHAR está sendo "aberta" */
    IF {&tttabela}.flgfecha = NO AND {&tabela}.flgfecha = YES THEN
    DO:
        /* Verifica se a agenda já foi enviada ao PA */
        /* Verifica se a agenda já foi enviada ao PA */
        FIND FIRST crapagp WHERE crapagp.idevento = {&tttabela}.idevento    AND
                                 crapagp.cdcooper = {&tttabela}.cdcooper    AND
                                 crapagp.dtanoage = {&tttabela}.dtanoage    AND
                                 crapagp.cdagenci = {&tttabela}.cdagenci    NO-LOCK NO-ERROR.
        

        IF AVAIL crapagp AND crapagp.idstagen <> 0 THEN
        DO:
            FIND FIRST crapage WHERE crapage.cdagenci = {&tttabela}.cdagenci    AND
                                     crapage.cdcooper = {&tttabela}.cdcooper    NO-LOCK NO-ERROR.
            ASSIGN m-erros = m-erros + "Agenda já foi enviada ao PA " + crapage.nmresage + ". ".
            RETURN "NOK".
        END.
    END.
...
PROCEDURE altera-registro:
    ...
    FIND FIRST {&tabela} WHERE {&tabela}.idevento = {&tttabela}.idevento    AND
                               ...
                               {&tabela}.tpcuseve = {&tttabela}.tpcuseve    AND
                               {&tabela}.cdevento = {&tttabela}.cdevento    AND
                               {&tabela}.cdcuseve = {&tttabela}.cdcuseve    EXCLUSIVE-LOCK NO-ERROR.
