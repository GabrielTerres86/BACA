...
DEFINE BUFFER crabcdp FOR crapcdp.
DEFINE BUFFER bf-crabedp FOR crapedp.
            ...
            CASE crapcdp.cdcuseve:
              ...
              WHEN 8 THEN /* Outros */
                DO:   
                  ...
                  FIND crabcdp WHERE crabcdp.cdevento = INT(ab_unmap.aux_cdevento)
                                 AND crabcdp.idevento = INT(ab_unmap.aux_idevento)
                                 AND crabcdp.dtanoage = INT(ab_unmap.aux_dtanoage)
                                 AND crabcdp.tpcuseve = 4                         
                                 AND crabcdp.cdcuseve = 1                         
                                 AND crabcdp.cdagenci = crapage.cdagenci          
                                 AND crabcdp.cdcooper = INT(ab_unmap.aux_cdcooper) EXCLUSIVE-LOCK.
                                 
                  IF AVAILABLE crabcdp THEN               
                    DO:
                      ASSIGN crabcdp.vlcuseve = aux_vlrtotal.
                    END.
                END.
            END CASE. /* CASE crapcdp.cdcuseve */  
...
PROCEDURE process-web-request :
  ...
  FIND FIRST crapeap WHERE crapeap.cdcooper = INT(ab_unmap.aux_cdcooper)
                       AND crapeap.idevento = INT(ab_unmap.aux_idevento)                                     
                       AND crapeap.cdevento = INT(ab_unmap.aux_cdevento)
                       AND crapeap.dtanoage = INT(ab_unmap.aux_dtanoage) NO-LOCK NO-ERROR NO-WAIT.
  ...
  IF AVAILABLE crapeap THEN
    DO:       
      ...
      ELSE
        DO:
          /* para a frequencia minima */
          FIND FIRST bf-crabedp WHERE bf-crabedp.idevento = INT(ab_unmap.aux_idevento) 
                               AND bf-crabedp.cdcooper = INT(ab_unmap.aux_cdcooper)
                               AND bf-crabedp.dtanoage = INT(ab_unmap.aux_dtanoage) 
                               AND bf-crabedp.cdevento = INT(ab_unmap.aux_cdevento) NO-LOCK NO-ERROR NO-WAIT.
                               
          IF AVAILABLE bf-crabedp THEN
            DO: 
              IF bf-crabedp.qtmaxtur > 0 THEN
                DO:
                  ASSIGN aux_qtmaxtur = STRING(bf-crabedp.qtmaxtur).
                END.       
              ELSE
                DO:
                  FIND FIRST bf-crabedp WHERE bf-crabedp.idevento = INT(ab_unmap.aux_idevento)
                                       AND bf-crabedp.cdcooper = 0
                                       AND bf-crabedp.dtanoage = 0
                                       AND bf-crabedp.cdevento = INT(ab_unmap.aux_cdevento) NO-LOCK NO-ERROR NO-WAIT.
  ...
  ELSE /*NOT AVAILABLE crapeap */
    DO:      
      /* para a frequencia minima */
      FIND FIRST bf-crabedp WHERE bf-crabedp.idevento = INT(ab_unmap.aux_idevento) AND 
                               bf-crabedp.cdcooper = INT(ab_unmap.aux_cdcooper) AND
                               bf-crabedp.dtanoage = INT(ab_unmap.aux_dtanoage) AND 
                               bf-crabedp.cdevento = INT(ab_unmap.aux_cdevento) NO-LOCK NO-ERROR NO-WAIT. 
    ...
    IF aux_qtmaxtur = "" THEN
      DO:
          RUN RodaJavaScript("alert('O evento precisa constar na Seleção de Sugestões:" + aux_qtmaxtur + ".');top.close();").
          RETURN "NOK".
      END.
