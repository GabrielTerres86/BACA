...
DEFINE VARIABLE ProgramaEmUso AS CHARACTER INITIAL ["wpgd0060b"].
DEFINE VARIABLE NmeDoPrograma AS CHARACTER INITIAL ["wpgd0060b.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0060b.htm
...
PROCEDURE CriaListaLocal :
	...
	DEF VAR vetorlocal  AS CHAR  NO-UNDO.
	
	DEFINE BUFFER bf-crapldp FOR crapldp.

	RUN RodaJavaScript("var mlocal = new Array();").
	
	/* Carrega o array com todos os Locais da Cooperativa e do PAC */
	FOR EACH bf-crapldp NO-LOCK WHERE bf-crapldp.cdcooper = INT(ab_unmap.aux_cdcooper)
	  														AND bf-crapldp.idloceva = 1:
    
    IF INT(ab_unmap.aux_cdagenci) <> 0 AND bf-crapldp.cdagenci <> INT(ab_unmap.aux_cdagenci) THEN
			NEXT.
    ...
    ASSIGn aux_dsestloc = "".
		...
		DO aux_contador = 1 TO LENGTH(bf-crapldp.dsestloc):
			...
			IF KEYCODE(SUBSTRING(bf-crapldp.dsestloc,aux_contador,1)) = 10 THEN
				ASSIGN aux_dsestloc = aux_dsestloc + "~\n".
			ELSE
				ASSIGN aux_dsestloc = aux_dsestloc + SUBSTRING(bf-crapldp.dsestloc,aux_contador,1).
    END.
    
		ASSIGN vetorlocal =  "~{nrseqdig:'" + STRING(bf-crapldp.nrseqdig) +
												 ...
												 "',idestaci:'" + (IF bf-crapldp.idestaci > 0 THEN "SIM" ELSE "NÃO") +
												 "',idclimat:'" + (IF bf-crapldp.idclimat > 0 THEN "SIM" ELSE "NÃO") +
												 "',idacecad:'" + (IF bf-crapldp.idacecad > 0 THEN "SIM" ELSE "NÃO") +
												 "',idcadeir:'" + (IF bf-crapldp.idcadeir > 0 THEN "SIM" ELSE "NÃO") +
												 "',idtelpro:'" + (IF bf-crapldp.idtelpro > 0 THEN "SIM" ELSE "NÃO") +
												 "',idprojet:'" + (IF bf-crapldp.idprojet > 0 THEN "SIM" ELSE "NÃO") +
												 "',idsonori:'" + (IF bf-crapldp.idsonori > 0 THEN "SIM" ELSE "NÃO") +																 
												 "',nmconloc:'" + STRING(bf-crapldp.nmconloc) +
												 "',nrtelefo:'" + STRING(bf-crapldp.nrtelefo) + "'~}".
												 
		RUN RodaJavaScript("mlocal.push(" + TRIM(STRING(vetorlocal)) + ");").
  ...
  IF (TRIM(aux_dscritic) <> "") THEN
    DO:
      ...
      DO aux_contador = 2 TO (NUM-ENTRIES(aux_dscritic, "#")):
        ASSIGN aux_msgalert = aux_msgalert + STRING(ENTRY(aux_contador, aux_dscritic,"#")) + "\\r".
      END.
    
      RUN RodaJavaScript("alert('" + aux_msgalert + "');").
  ...
  DO:
      DO WITH FRAME {&FRAME-NAME}:
          ...
          ELSE DO:
			  ...
			  CREATE cratadp.
              BUFFER-COPY crapadp EXCEPT cdlocali cdcopope cdoperad TO cratadp.
              ...
              IF msg-erro = "" THEN
			   DO:              
				   ...
				   FIND FIRST crapcop WHERE crapcop.cdcooper = cratadp.cdcooper NO-LOCK NO-ERROR.

				   FIND FIRST crapage WHERE crapage.cdcooper = cratadp.cdcooper
										AND crapage.cdagenci = cratadp.cdagenci NO-LOCK NO-ERROR.

				   FIND FIRST crapedp WHERE crapedp.cdevento = cratadp.cdevento
										AND crapedp.cdcooper = cratadp.cdcooper
										AND crapedp.dtanoage = cratadp.dtanoage
										AND crapedp.idevento = cratadp.idevento NO-LOCK NO-ERROR.
			   END.
