...
DEFINE VARIABLE aux_nmtiprec AS CHARACTER.
...
FUNCTION montaTela RETURNS LOGICAL ().    
  ...
  DEFINE VARIABLE conta AS INTEGER INIT 0.
  
    {&out} '<html>' SKIP
           '<head>' SKIP
           '<title>Progrid - Cheklist</title>' SKIP.
    ...
    FOR EACH craprep WHERE craprep.idevento = idevento
      ...
      IF AVAILABLE gnaprdp THEN
        DO:
          CREATE gnatrdp.
          ASSIGN gnatrdp.cdtiprec = gnaprdp.cdtiprec
								 gnatrdp.dsrecurs = gnaprdp.dsrecurs.
        END.
    END.
   ...
   FOR EACH crapree NO-LOCK WHERE crapree.nrsdieve	= nrSeqEve
		,EACH gnaprdp WHERE gnaprdp.idevento = idEvento
                    AND gnaprdp.cdcooper = 0                                
    ...
    FOR EACH gnatrdp NO-LOCK BREAK BY gnatrdp.cdtiprec
																   BY gnatrdp.dsrecurs:
      ASSIGN conta = conta + 1.
			
			IF FIRST-OF(gnatrdp.cdtiprec) THEN
				DO:
					ASSIGN aux_nmtiprec = "".
					IF gnatrdp.cdtiprec = 1 THEN
						ASSIGN aux_nmtiprec = "BRINDES".
					ELSE IF gnatrdp.cdtiprec = 2 THEN
						ASSIGN aux_nmtiprec = "DIVULGA&Ccedil;&Atilde;O".
					ELSE IF gnatrdp.cdtiprec = 3 THEN
						ASSIGN aux_nmtiprec = "EQUIPAMENTOS".
					ELSE IF gnatrdp.cdtiprec = 4 THEN
						ASSIGN aux_nmtiprec = "MATERIAIS".
					ELSE IF gnatrdp.cdtiprec = 5 THEN
						ASSIGN aux_nmtiprec = "LEMBRETE".
					ELSE IF gnatrdp.cdtiprec = 6 THEN
						ASSIGN aux_nmtiprec = "RECREA&Ccedil;&Atilde;O".
						
					/*ASSIGN conta = 1.*/
					{&out} '      <tr>' SKIP
             '         <td class="tdDados" align="center"></td>' SKIP
             '         <td class="tdDados"><b>' + STRING(aux_nmtiprec) + '<b></td>' SKIP
             '         <td class="tdDados" align="center"> &nbsp; </td>' SKIP
             '         <td class="tdDados" align="center"> &nbsp; </td>' SKIP
             '      </tr>' SKIP.
				END.
