...
------------------------------------------------------------------------------*/
   ...
   DO:
       FOR EACH crapsdp WHERE string(crapsdp.dtanoage) = ?      AND 
          ...
          /* só podem ser atribuidos custos aos eventos selecionados na tela de seleção da sugestão (wpgd0014)*/
          /* alteração efetuada em 30/01/2007 - Rosangela)*/
          /*FIND FIRST crapeap WHERE crapeap.cdcooper = INTEGER(ab_unmap.aux_cdcooper)   AND
                                   crapeap.idevento = INTEGER(ab_unmap.aux_idevento)   AND
                                   crapeap.dtanoage = INTEGER(ab_unmap.aux_dtanoage)   AND 
                                   crapeap.cdevento = crapedp.cdevento    NO-LOCK NO-ERROR. 
          ...
          DO WITH FRAME {&FRAME-NAME}:
             IF opcao = "inclusao" THEN
                DO: 
                    ...
                    /*RUN RodaJavaScript('alert("'+ ab_unmap.aux_cdevesel + '"); ').*/
                    ...
                    DO aux_qtevesel = 1 TO NUM-ENTRIES(ab_unmap.aux_cdevesel):
                        ...
                        DO aux_tpcuseve = 1 TO (NUM-ENTRIES(craptab.dstextab) / 2):
                           ...
                           /* RUN RodaJavaScript('alert("'+ craptab.dstextab + '"); ').*/
