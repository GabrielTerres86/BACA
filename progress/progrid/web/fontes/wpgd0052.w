...
DEFINE VARIABLE ProgramaEmUso AS CHARACTER INITIAL ["wpgd0052"].
DEFINE VARIABLE NmeDoPrograma AS CHARACTER INITIAL ["wpgd0052.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0052.htm
...
PROCEDURE CriaListaEventos :
  ...
  DEF VAR vetoreventos AS CHAR        NO-UNDO.
  ...
  FOR EACH crapedp WHERE crapedp.cdcooper = INTEGER(ab_unmap.aux_cdcooper)  
      ...
      FOR EACH  crapeap WHERE crapeap.cdcooper = crapedp.cdcooper
                          AND crapeap.idevento = crapedp.idevento
                          AND crapeap.cdevento = crapedp.cdevento
                          AND crapeap.dtanoage = crapedp.dtanoage NO-LOCK,
          
          /* Busca o nome do PA */
          EACH cratagp WHERE cratagp.cdagenci = crapeap.cdagenci NO-LOCK by cratagp.nmresage:
                      
          IF AVAILABLE cratagp THEN
            DO:
              ASSIGN aux_qtpaccad = aux_qtpaccad + 1.
                     aux_nmpaccad = aux_nmpaccad + STRING(aux_qtpaccad) + ":'- " + cratagp.nmresage + "',".
              IF cratagp.idstagen = 0 THEN
                DO:
      ...
      RUN RodaJavaScript("meventos.push(~{aux_nrdrowid:'" + STRING(ROWID(crapedp))   
...
PROCEDURE CriaListaPacs:
  ...
  FOR EACH crapagp WHERE crapagp.idevento = INTEGER(ab_unmap.aux_idevento)
                     ...
                     AND crapagp.cdageagr = crapagp.cdagenci 
    ...
    IF NOT AVAILABLE cratagp THEN
      DO:
        ...
        ASSIGN cratagp.cdagenci = crapagp.cdagenci
               ...
               cratagp.idstagen = crapagp.idstagen.
      END.
  END.
  ...
  FOR EACH crapagp WHERE crapagp.idevento = INTEGER(ab_unmap.aux_idevento)
                     ...
                     AND crapagp.dtanoage = INTEGER(ab_unmap.aux_dtanoage) 
                     AND crapagp.cdageagr <> crapagp.cdagenci 
                     AND crapagp.cdageagr <> 0 NO-LOCK:
  ...
  FOR EACH cratagp NO-LOCK BY cratagp.nmresage:
      ...
      FIND crapagp WHERE crapagp.idevento = INTEGER(ab_unmap.aux_idevento)  AND
                         ...
                         crapagp.cdagenci = cratagp.cdagenci                AND 
                         crapagp.cdagenci <> 0 NO-LOCK NO-ERROR.
...
PROCEDURE envia-email:
  ...
  DEF VAR aux_dscritic AS LONGCHAR NO-UNDO.
  
  FOR FIRST crapage FIELDS(nmresage dsdemail) WHERE crapage.cdcooper = INTEGER(ab_unmap.aux_cdcooper)
                                                AND crapage.cdagenci = INTEGER(par_cdagenci) /*INTEGER(ab_unmap.aux_cdagenci)*/
                                                AND crapage.cdagenci <> 0 NO-LOCK. END.
  ...
  ELSE
    DO:
      ASSIGN msg-erro = "Email nao cadastrado para a PA: " + STRING(crapage.cdagenci)
             msg-erro-aux = 5.
      RETURN "NOK".
    END.
  ...
  ASSIGN aux_dsdemail = REPLACE(aux_dsdemail,";",",")
         aux_dsconteu = "Prezado(a)," + 
							...
							"\nFaça a seleção dos eventos para o seu PA acessando o Sistema de Gestão de Eventos em: Assembléias -> Processos -> Seleção Eventos - Pré." +
    ...
    DO:    					
			...
			RUN STORED-PROCEDURE pc_solicita_email_prog
				aux_handproc = PROC-HANDLE NO-ERROR(INPUT  INTEGER(ab_unmap.aux_cdcooper)                 /* par_cdcooper        */
																					 ,INPUT  "wpgd0052"                                     /* par_cdprogra        */
																					 ,INPUT  aux_dsdemail                                   /* par_des_destino     */
																					 ,INPUT  "ASSEMBLEIAS - DEFINA A NOVA AGENDA DO SEU PA" /* par_des_assunto     */
			...
			ASSIGN aux_dscritic = ""
						 aux_dscritic = pc_solicita_email_prog.pr_des_erro WHEN pc_solicita_email_prog.pr_des_erro <> ?.

			IF (TRIM(aux_dscritic) <> "" AND aux_dscritic <> ?) THEN
				DO:
  ...
  IF VALID-HANDLE(h-b1wpgd0014)   THEN
    DO:
      ...
      FOR EACH crapagp WHERE crapagp.idevento  = INTEGER(ab_unmap.aux_idevento) AND
                             ...
                             0 = INTEGER(ab_unmap.aux_cdagenci)) AND crapagp.cdagenci <> 0) AND
                             crapagp.idstagen  = 0 NO-LOCK
        ...
        CASE msg-erro-aux:
             ...
             WHEN 5 THEN
                  RUN RodaJavaScript('alert("' + msg-erro + '")').
						 WHEN 6 THEN
                  RUN RodaJavaScript('alert("Erro ao enviar email.")').			
             WHEN 10 THEN
