...
DEFINE VARIABLE ProgramaEmUso AS CHARACTER INITIAL ["wpgd0023b"].
DEFINE VARIABLE NmeDoPrograma AS CHARACTER INITIAL ["wpgd0023b.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0023b.htm
...
PROCEDURE CriaListaFornecedor:
  ...
  DEF VAR aux_regselec AS CHAR NO-UNDO.
	...
	RUN RodaJavaScript("var mfornece = new Array();").
  ...
  FOR EACH gnapfdp WHERE gnapfdp.cdcooper = 0        
    ...
    FOR EACH gnappdp WHERE gnappdp.idevento = gnapfdp.idevento
      ...
      RUN RodaJavaScript("mfornece.push(~{nrcpfcgc:'" + STRING(gnapfdp.nrcpfcgc) + "'" + 
...
PROCEDURE valida-facilitador:
  ...
  MESSAGE "aux_cdfacili = " + STRING(aux_cdfacili).
...
IF REQUEST_METHOD = "POST":U THEN 
  DO:
    ...
    CASE msg-erro-aux:
     ...
     WHEN 3 THEN
        DO:
             RUN RodaJavaScript('alert("Custo Não Encontrtado");').
        END.
