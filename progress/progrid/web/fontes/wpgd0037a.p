...
FUNCTION montaTela RETURNS LOGICAL ().
    ...
    {&out} '   <table border="1" cellspacing="1" cellpadding="3" width="100%" class="tab1">' SKIP
        ...
        FOR EACH Craprep WHERE Craprep.Idevento = idEvento AND
                           Craprep.CdCooper = 0 AND
                           Craprep.CdEvento = cdEvento 
                                                   NO-LOCK:
        
            FIND Gnaprdp WHERE Gnaprdp.IdEvento = Craprep.Idevento AND
                           Gnaprdp.CdCooper = 0 AND
                           Gnaprdp.NrSeqDig = Craprep.NrSeqDig NO-LOCK NO-ERROR.
        
       
        IF AVAILABLE Gnaprdp
           THEN
               DO:
                  ASSIGN conta = conta + 1.
                  {&out} '      <tr>' SKIP
                         '         <td class="tdDados" align="center">' conta FORMAT "99" '</td>' SKIP
                         '         <td class="tdDados">' Gnaprdp.dsRecurs '</td>' SKIP
