...
DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0047"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0047.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0047.htm
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.tipoDeRelatorio AT ROW 1 COL 1 HELP
          ...
          LIST-ITEM-PAIRS "Analisadas por PA","1",
                     "Analisadas por Evento","2",
                     "Analisadas por Quantidade","3",
                     "Todas Cadastradas","4"
          SIZE 20 BY 4
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH crapeap WHERE crapeap.cdcooper = INT(ab_unmap.cdcooper)         AND
                          ...
                          (crapeap.cdagenci = INT(ab_unmap.cdagenci)         OR
                           INT(ab_unmap.cdagenci) = 0)                       AND /* Todos os PAC's */
       ...
       FIRST crapedp WHERE crapedp.cdevento = crapeap.cdevento               AND
        ...
        /* Mostra somente 1 evento de cada */
        IF   FIRST-OF(crapedp.nmevento)   THEN
             DO:
                ...
                CREATE ttEventos.
                ASSIGN ttEventos.CdCooper = crapeap.cdcooper
                       ttEventos.CdAgenci = crapeap.cdagenci
                       ttEventos.CdEvento = crapeap.cdevento
                       ttEventos.NmEvento = aux_nmevento
                       ttEventos.NrSeqEve = aux_nrseqeve.
        END.
...
IF   INT(ab_unmap.cdagenci) = 0 AND GET-VALUE("cdagenci") = ""   THEN
