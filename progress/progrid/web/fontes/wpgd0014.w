...
DEFINE VARIABLE aux_flpacenv          AS LOGICAL                        NO-UNDO.
DEFINE VARIABLE aux_dsdemail          LIKE crapage.dsdemail.
...
FOR EACH crapcop NO-LOCK BY crapcop.cdcooper:
    aux_nmrescop = aux_nmrescop + crapcop.nmrescop + "," + STRING(crapcop.cdcooper) + ",".
END.

/* tira a última virgula */
aux_nmrescop = SUBSTRING(aux_nmrescop,1,LENGTH(aux_nmrescop) - 1).

/* Preenche as listas com os elementos encontrados */
ASSIGN ab_unmap.aux_nmrescop:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = aux_nmrescop.
******/
...
FOR EACH crapedp WHERE crapedp.cdcooper = INTEGER(ab_unmap.aux_cdcooper)   AND
                       ...
                       crapedp.dtanoage = INTEGER(ab_unmap.aux_dtanoage)   NO-LOCK
                       BY crapedp.nmevento:
...
FOR EACH cratagp NO-LOCK BY cratagp.nmresage:
    ...
    IF   aux_qtevepac > 0   THEN
         aux_nmevepac = SUBSTRING(aux_nmevepac,1,LENGTH(aux_nmevepac) - 1).
    ELSE
         aux_nmevepac = "0:''".
    ...
    IF   NOT AVAILABLE crapagp  OR
         crapagp.idstagen = 0   THEN
         aux_flpacenv = FALSE. /* Eventos não enviados para o PA */
    ELSE
         aux_flpacenv = TRUE.  /* Eventos já enviados para o PA */

    /********************************************************************************************************/
    /********************************************************************************************************/
    ASSIGN aux_conteudo = "~{cdagenci:'" + STRING(cratagp.cdagenci)          + 
                          "',flpacenv:'" + STRING(aux_flpacenv,"true/false") + 
                          "',nmresage:'" + cratagp.nmresage                  + 
                          "',qtevepac:'" + STRING(aux_qtevepac)              + 
                          "'," + aux_nmevepac + "~}".

    /* Se a quantidade de caracteres armazenada + o dobro da qtde. do registro atual for superior a 32K  */
    IF   (LENGTH(vetorpacs) + (LENGTH(aux_conteudo) * 2)) >= 31900  THEN
         DO:
             ...
             ASSIGN vetorpacs    = aux_conteudo
                    aux_nrovetor = aux_nrovetor + 1.

         END.
    ELSE 
         ASSIGN vetorpacs = vetorpacs + (IF vetorpacs = "" THEN "" ELSE ",") + 
                            aux_conteudo.
    /********************************************************************************************************/
    /********************************************************************************************************/
/*
     IF   vetorpacs = ""   THEN                                                                                                                                                                                                             
          vetorpacs =             "~{cdagenci:'"  + STRING(cratagp.cdagenci) + "',flpacenv:'" + STRING(aux_flpacenv,"true/false") + "',nmresage:'" + cratagp.nmresage + "',qtevepac:'" + STRING(aux_qtevepac) + "'," + aux_nmevepac + "~}". 
     ELSE                                                                                                                                                                                                                                   
          vetorpacs = vetorpacs + ",~{cdagenci:'" + STRING(cratagp.cdagenci) + "',flpacenv:'" + STRING(aux_flpacenv,"true/false") + "',nmresage:'" + cratagp.nmresage + "',qtevepac:'" + STRING(aux_qtevepac) + "'," + aux_nmevepac + "~}". 
...
FOR EACH crapsdp WHERE crapsdp.idevento = INT(ab_unmap.aux_idevento) AND
    ...
    FIND crapedp WHERE crapedp.idevento = crapsdp.idevento           AND
                       ...
                       crapedp.cdevento = crapsdp.cdevento           NO-LOCK NO-ERROR.
...
FIND crapage WHERE crapage.cdcooper = INTEGER(ab_unmap.aux_cdcooper)  AND
                   crapage.cdagenci = INTEGER(ab_unmap.aux_cdagenci)  
                   NO-LOCK NO-ERROR.
                           
IF  AVAIL crapage   AND   crapage.dsdemail <> ?   THEN
    aux_dsdemail = crapage.dsdemail.
          
IF  aux_dsdemail <> ? THEN DO:
        
    RUN sistema/generico/procedures/b1wgen0011.p PERSISTENT SET b1wgen0011.

    IF   VALID-HANDLE (b1wgen0011) THEN DO:
         RUN enviar_email IN b1wgen0011 (INPUT INTEGER(ab_unmap.aux_cdcooper),
                                         INPUT "wpgd0014",
                                         INPUT aux_dsdemail,
                                         INPUT "PROGRID - PREPARE " + 
                                               "A NOVA AGENDA DE SEU PA",
                                         INPUT "",
                                         INPUT FALSE).
    ...
    DO:
      ...
      FOR EACH crapeap WHERE crapeap.cdcooper = INTEGER(ab_unmap.aux_cdcooper)   AND
        ...
        IF NOT AVAILABLE crapcdp   THEN
          DO:
            msg-erro = "Um ou mais eventos não possuem o custo 'fechado', impossível enviar ao PA".
            RETURN.
          END.
      END.
...
IF   VALID-HANDLE(h-b1wpgd0014a)   THEN
     DO:
        FIND crapedp WHERE crapedp.cdcooper = INTEGER(ab_unmap.aux_cdcooper)  AND
                           ...
                           crapedp.idevento = INTEGER(ab_unmap.aux_idevento)  AND
                           crapedp.dtanoage = INTEGER(ab_unmap.aux_dtanoage)  NO-LOCK NO-ERROR.
