...
DEFINE VARIABLE vetoread              AS CHAR                           NO-UNDO.
...
DEF VAR aux_registros AS INT                         NO-UNDO.
  ...
  FOR EACH crapcdp NO-LOCK WHERE crapcdp.cdagenci = 0                          AND
                                 crapcdp.cdcooper = INT(ab_unmap.aux_cdcooper) AND
                                 crapcdp.idevento = INT(ab_unmap.aux_idevento) AND
                                 crapcdp.dtanoage = INT(ab_unmap.aux_dtanoage) BREAK BY crapcdp.idevento
                                         BY crapcdp.cdcooper
                                           BY crapcdp.dtanoage
                                             BY crapcdp.cdevento:
    ...
    IF FIRST-OF(crapcdp.cdevento) THEN
      DO:
        ...
        FOR EACH crapedp WHERE crapedp.cdcooper = crapcdp.cdcooper AND
                               crapedp.dtanoage = crapcdp.dtanoage AND
                               crapedp.idevento = crapcdp.idevento AND
                               crapedp.cdevento = crapcdp.cdevento NO-LOCK
                               BY crapedp.nmevento:
  ...
  FOR EACH tt-eventos NO-LOCK WHERE (tt-eventos.nmevento MATCHES "*" + TRIM(ab_unmap.aux_dscpesqu) + "*" OR TRIM(ab_unmap.aux_dscpesqu) = "") BY tt-eventos.nmevento:
    ...
    IF NOT vetorevento = "" THEN
      ...
      vetorevento = vetorevento + "~{" + "cdcooper:'" + STRING(tt-eventos.cdcooper) + "'," +
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR aux_registro AS INT INIT 0.
  ...
  DEF VAR aux_vlrtotal AS DEC INIT 0.
  ...
  DEF VAR aux_vlrtotal_tot AS DEC INIT 0.
  ...
  FOR EACH tt-eventos NO-LOCK BY tt-eventos.nmevento:
    ...
    vetoread = vetoread + "~{" + "dtanoage:'" + TRIM(STRING(tt-eventos.dtanoage))
    ...
    IF aux_registro > 50 THEN
      DO:
        RUN RodaJavaScript("mead.push(" + STRING(vetoread) + ");").
        ASSIGN vetoread = ""
               aux_registro = 0.
      END.
  END.
  ...
  ASSIGN vetoread = ""
         vetoread = "~{" + "qtocoeve:'" + TRIM(STRING(aux_qtocoeve_tot))
