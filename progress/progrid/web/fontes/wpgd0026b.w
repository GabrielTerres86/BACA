 /*-----------------------------------------------------------------------------
    ...
    Alteracoes: 20/10/2008 - Chamar BO que envia os e-mails na procedure 
                             local-assign-record (Gabriel).

                10/12/2008 - Melhoria de performance para a tabela gnapses (Evandro).
                
                17/03/2009 - Acerto do problema de limite de locais mostrados na tabela HTML (Martin)
                06/04/2009 - Acerto do problema de limite de locais mostrados na tabela HTML (Martin)
...
DEFINE VARIABLE ProgramaEmUso AS CHARACTER INITIAL ["wpgd0026b"].
DEFINE VARIABLE NmeDoPrograma AS CHARACTER INITIAL ["wpgd0026b.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0026b.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-FIELDS crapldp.idevento 
&Scoped-define ENABLED-TABLES ab_unmap crapldp
&Scoped-define FIRST-ENABLED-TABLE ab_unmap
&Scoped-define SECOND-ENABLED-TABLE crapldp
&Scoped-Define ENABLED-OBJECTS ab_unmap.aux_nrseqeve ab_unmap.aux_dtanoage ab_unmap.aux_cdagenci ab_unmap.aux_cdcooper ab_unmap.aux_cddopcao ab_unmap.aux_cdevento ab_unmap.aux_dsendurl ab_unmap.aux_dsretorn ab_unmap.aux_idevento ab_unmap.aux_lslocali ab_unmap.aux_lspermis ab_unmap.aux_nrdrowid ab_unmap.aux_stdopcao 
&Scoped-Define DISPLAYED-FIELDS crapldp.idevento 
&Scoped-define DISPLAYED-TABLES ab_unmap crapldp
&Scoped-define FIRST-DISPLAYED-TABLE ab_unmap
&Scoped-define SECOND-DISPLAYED-TABLE crapldp
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.aux_nrseqeve ab_unmap.aux_dtanoage ab_unmap.aux_cdagenci ab_unmap.aux_cdcooper ab_unmap.aux_cddopcao ab_unmap.aux_cdevento ab_unmap.aux_dsendurl ab_unmap.aux_dsretorn ab_unmap.aux_idevento ab_unmap.aux_lslocali ab_unmap.aux_lspermis ab_unmap.aux_nrdrowid ab_unmap.aux_stdopcao 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.aux_stdopcao AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     crapldp.idevento AT ROW 1 COL 1 NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR FILL-IN ab_unmap.aux_stdopcao IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN crapldp.idevento IN FRAME Web-Frame
   ALIGN-L EXP-LABEL                                                    */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE CriaListaLocal w-html 
PROCEDURE CriaListaLocal :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR aux_dsestloc AS CHAR NO-UNDO.

DEFINE BUFFER bf-crapldp FOR crapldp.

/* Carrega o array com todos os Locais da Cooperativa e do PAC */
EMPTY TEMP-TABLE tt-javaFrase.
ASSIGN iseqJava = 1.
FOR EACH bf-crapldp NO-LOCK WHERE
    bf-crapldp.cdcooper = INT(ab_unmap.aux_cdcooper):
    IF INT(ab_unmap.aux_cdagenci) <> 0 AND 
       bf-crapldp.cdagenci <> INT(ab_unmap.aux_cdagenci) THEN NEXT.

    aux_dsestloc = "".
    DO aux_contador = 1 TO LENGTH(bf-crapldp.dsestloc):

        /* Troca o ENTER por '\n' */
        IF   KEYCODE(SUBSTRING(bf-crapldp.dsestloc,aux_contador,1)) = 10   THEN
             aux_dsestloc = aux_dsestloc + "~\n".
        ELSE
             aux_dsestloc = aux_dsestloc + SUBSTRING(bf-crapldp.dsestloc,aux_contador,1).
    END.

    CREATE tt-javaFrase.
    ASSIGN iseqJava = iseqJava + 1
           tt-javaFrase.seq = iseqJava
           tt-javaFrase.agregado = (IF iseqJava > 2 THEN "," ELSE "mlocal=[")
           tt-javaFrase.jump = NO
           tt-javaFrase.frase =  "~{" + "nrseqdig:" + "'" + STRING(bf-crapldp.nrseqdig) + "'" + 
                                 ",dslocali:" + "'" + STRING(bf-crapldp.dslocali) + "'" +
                                 ",vldialoc:" + "'" + STRING(bf-crapldp.vldialoc, "->>>,>>9.99") + "'" +
                                 ",qtmaxpes:" + "'" + STRING(bf-crapldp.qtmaxpes) + "'" +
                                 ",dsendloc:" + "'" + STRING(bf-crapldp.dsendloc) + "'" +
                                 ",dsestloc:" + "'" + STRING(aux_dsestloc) + "'" +
                                 ",nmconloc:" + "'" + STRING(bf-crapldp.nmconloc) + "'" +
                                 ",nrtelefo:" + "'" + STRING(bf-crapldp.nrtelefo) + "'" + "~}".

/*
    IF   vetorlocal = ""   THEN
         vetorlocal = "~{" + "nrseqdig:" + "'" + STRING(bf-crapldp.nrseqdig) + "'" + 
                            ",dslocali:" + "'" + STRING(bf-crapldp.dslocali) + "'" +
                            ",vldialoc:" + "'" + STRING(bf-crapldp.vldialoc, "->>>,>>9.99") + "'" +
                            ",qtmaxpes:" + "'" + STRING(bf-crapldp.qtmaxpes) + "'" +
                            ",dsendloc:" + "'" + STRING(bf-crapldp.dsendloc) + "'" +
                            ...
                            ",nrtelefo:" + "'" + STRING(bf-crapldp.nrtelefo) + "'" + "~}".
    ELSE
         vetorlocal = vetorlocal + "," + 
                      "~{" + "nrseqdig:" + "'" + STRING(bf-crapldp.nrseqdig) + "'" + 
                            ",dslocali:" + "'" + STRING(bf-crapldp.dslocali) + "'" +
                            ",vldialoc:" + "'" + STRING(bf-crapldp.vldialoc, "->>>,>>9.99") + "'" +
                            ",qtmaxpes:" + "'" + STRING(bf-crapldp.qtmaxpes) + "'" +
                            ",dsendloc:" + "'" + STRING(bf-crapldp.dsendloc) + "'" +
...
FIND LAST tt-javaFrase.
ASSIGN tt-javaFrase.frase = tt-javaFrase.frase + "]"
       tt-javafrase.jump  = YES.
          
CREATE tt-javaFrase.
ASSIGN tt-javaFrase.seq = 1
       tt-javaFrase.frase = "var mlocal=new Array();"
       tt-javaFrase.jump = YES
       tt-javaFrase.agregado = "".

RUN RodaJavaScriptLong.
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("idevento":U,"crapldp.idevento":U,crapldp.idevento:HANDLE IN FRAME {&FRAME-NAME}).
END PROCEDURE.
          ...
          ELSE DO:
              ...
              IF   msg-erro = "" THEN
                   DO:              
                       ...
                       /* Envia e-mail para a coordenação do progrid */
                                            
                       IF   AVAILABLE crapcop   AND  AVAILABLE crapage   AND
                            AVAILABLE crapedp   THEN
                            DO:
...
/*
/* Instancia a BO para executar as procedures */
RUN dbo/b1wpgd0026b.p PERSISTENT SET h-b1wpgd0030.
