...
Alterações: 10/12/2008 - Melhoria de performance para a tabela gnapses (Evandro).
            ...
            17/11/2009 - Inicializar a tela com o ano da agenda atual (Diego).
...
DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0025"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0025.w"].
...
DEFINE VARIABLE vetorkit              AS CHAR                           NO-UNDO.
DEFINE VARIABLE vetorbri              AS CHAR                           NO-UNDO.
DEFINE VARIABLE vetorque              AS CHAR                           NO-UNDO.
...
&Scoped-define WEB-FILE fontes/wpgd0025.htm

/* Name of first Frame and/or Browse and/or first Query                 */
&Scoped-define FRAME-NAME Web-Frame

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-FIELDS crapkbq.dtanoage 
&Scoped-define ENABLED-TABLES ab_unmap crapkbq
&Scoped-define FIRST-ENABLED-TABLE ab_unmap
&Scoped-define SECOND-ENABLED-TABLE crapkbq
&Scoped-Define ENABLED-OBJECTS ab_unmap.cdagenci ab_unmap.cdcooper ~
...
ab_unmap.aux_nrdrowid ab_unmap.aux_stdopcao ab_unmap.pagina 
&Scoped-Define DISPLAYED-FIELDS crapkbq.dtanoage 
&Scoped-define DISPLAYED-TABLES ab_unmap crapkbq
&Scoped-define FIRST-DISPLAYED-TABLE ab_unmap
&Scoped-define SECOND-DISPLAYED-TABLE crapkbq
&Scoped-Define DISPLAYED-OBJECTS ab_unmap.cdagenci ab_unmap.cdcooper ~
ab_unmap.aux_cdagenci ab_unmap.aux_cdcooper ab_unmap.aux_cddopcao ~
ab_unmap.aux_dsendurl ab_unmap.aux_dsretorn ab_unmap.aux_dtanoage ~
ab_unmap.aux_idevento ab_unmap.aux_idexclui ab_unmap.aux_lspermis ~
ab_unmap.aux_nrdrowid ab_unmap.aux_stdopcao ab_unmap.pagina 
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.aux_stdopcao AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     crapkbq.dtanoage AT ROW 1 COL 1 NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
...
/* SETTINGS FOR FILL-IN ab_unmap.cdcooper IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN crapkbq.dtanoage IN FRAME Web-Frame
   ALIGN-L EXP-LABEL                                                    */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE VARIABLE aux_dtdenvio AS CHAR NO-UNDO.
    DEFINE VARIABLE aux_qtdenvio AS CHAR NO-UNDO.
    DEFINE VARIABLE aux_vlrenvio AS CHAR NO-UNDO.
    
    IF NOT AVAIL gnpapgd THEN NEXT.
    
    FOR EACH crapkbq WHERE crapkbq.dtanoage = INT(ab_unmap.aux_dtanoage)        AND
                           crapkbq.cdcooper = INTEGER(ab_unmap.aux_cdcooper)    AND
                           crapkbq.cdagenci = INTEGER(ab_unmap.cdagenci)        NO-LOCK
                           BY crapkbq.dtsolici DESCENDING
                              BY crapkbq.dtdenvio DESCENDING:
    
        FIND FIRST crapage WHERE crapage.cdcooper = crapkbq.cdcooper    AND
                                 crapage.cdagenci = crapkbq.cdagenci    NO-LOCK NO-ERROR.
    
        FIND FIRST crapcop WHERE crapcop.cdcooper = crapkbq.cdcooper    NO-LOCK NO-ERROR.
        ...
        IF crapkbq.dtdenvio <> ? THEN
            aux_dtdenvio = STRING(crapkbq.dtdenvio, "99/99/9999").
    
        IF crapkbq.qtdenvio <> ? THEN
            aux_qtdenvio = STRING(crapkbq.qtdenvio, "->>>,>>9").
    
        IF crapkbq.vlrenvio <> ? THEN
            aux_vlrenvio = STRING(crapkbq.vlrenvio, "->>>,>>9.99").
    
        CASE crapkbq.tpdeitem:
            /* Kits */
            WHEN 1 THEN
            DO:
                IF  vetorkit = "" THEN
                    vetorkit = "~{" + "cdagenci:"  + "'" + STRING(crapkbq.cdagenci)               + "'"
                                    + ",nmresage:" + "'" + STRING(crapage.nmresage)               + "'"
                                    + ",cdcooper:" + "'" + STRING(crapkbq.cdcooper)               + "'"
                                    + ",nmrescop:" + "'" + STRING(crapcop.nmrescop)               + "'"
                                    + ",idregist:" + "'" + STRING(ROWID(crapkbq))                 + "'"
                                    + ",dtsolici:" + "'" + STRING(crapkbq.dtsolici, "99/99/9999") + "'"
                                    + ",qtsolici:" + "'" + STRING(crapkbq.qtsolici, "->>>,>>9")   + "'"
                                    + ",nmsolici:" + "'" + STRING(crapkbq.nmsolici)               + "'"
                                    + ",dtdenvio:" + "'" + STRING(aux_dtdenvio)                   + "'"
                                    + ",qtdenvio:" + "'" + STRING(aux_qtdenvio)                   + "'"
                                    + ",vlrenvio:" + "'" + STRING(aux_vlrenvio)                   + "'" 
                               + "~}".
                ELSE
                    vetorkit = vetorkit + "," + 
                               "~{" + "cdagenci:"  + "'" + STRING(crapkbq.cdagenci)               + "'"
                                    + ",nmresage:" + "'" + STRING(crapage.nmresage)               + "'"
                                    + ",cdcooper:" + "'" + STRING(crapkbq.cdcooper)               + "'"
                                    + ",nmrescop:" + "'" + STRING(crapcop.nmrescop)               + "'"
                                    + ",idregist:" + "'" + STRING(ROWID(crapkbq))                 + "'"
            ...
            DO:
                IF  vetorbri = "" THEN
                    vetorbri = "~{" + "cdagenci:"  + "'" + STRING(crapkbq.cdagenci)               + "'"
                                    + ",nmresage:" + "'" + STRING(crapage.nmresage)               + "'"
                                    + ",cdcooper:" + "'" + STRING(crapkbq.cdcooper)               + "'"
                                    + ",nmrescop:" + "'" + STRING(crapcop.nmrescop)               + "'"
                                    + ",idregist:" + "'" + STRING(ROWID(crapkbq))                 + "'"
            ...
            DO:
                IF  vetorque = "" THEN
                    vetorque = "~{" + "cdagenci:"  + "'" + STRING(crapkbq.cdagenci) + "'"
                                    + ",nmresage:" + "'" + STRING(crapage.nmresage) + "'"
                                    + ",cdcooper:" + "'" + STRING(crapkbq.cdcooper) + "'"
                                    + ",nmrescop:" + "'" + STRING(crapcop.nmrescop) + "'"
                                    + ",idregist:" + "'" + STRING(ROWID(crapkbq))   + "'"
    ...
    /* Se não teve registro, pelo menos cria o array vazio */
    IF vetorkit = ? THEN ASSIGN vetorkit = "".
    IF vetorbri = ? THEN ASSIGN vetorbri = "".
    IF vetorque = ? THEN ASSIGN vetorque = "".
    
    RUN RodaJavaScript("var mkit=new Array();mkit=["  + vetorkit + "]").
    RUN RodaJavaScript("var mbrinde=new Array();mbrinde=["  + vetorbri + "]").
    RUN RodaJavaScript("var mquest=new Array();mquest=["  + vetorque + "]").
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("dtanoage":U,"crapkbq.dtanoage":U,crapkbq.dtanoage:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
       ...
       DO:
          DO WITH FRAME {&FRAME-NAME}:
             IF opcao = "inclusao" THEN
                DO: 
                    CREATE cratkbq.
                    ASSIGN cratkbq.campo = INPUT crapkbq.campo.
             ...
             ELSE  /* alteracao */
                DO:
                    ...
                    CREATE cratkbq.
                    BUFFER-COPY crapkbq EXCEPT crapkbq.campox TO cratkbq.
...
IF REQUEST_METHOD = "GET":U THEN 
   DO:
      /*IF ab_unmap.aux_cdcooper = "" THEN */
      FIND FIRST crapcop WHERE crapcop.flgdopgd = YES NO-LOCK NO-ERROR.
      IF AVAIL  crapcop THEN
         ASSIGN ab_unmap.aux_cdcooper = STRING(crapcop.cdcooper).
   END.
...
IF REQUEST_METHOD = "POST":U THEN 
   DO:
      ...
      IF VALID-HANDLE(h-b1wpgd0025) THEN
         DO:
            RUN posiciona-registro IN h-b1wpgd0025(INPUT TO-ROWID(ab_unmap.aux_nrdrowid), OUTPUT msg-erro).
            DELETE PROCEDURE h-b1wpgd0025 NO-ERROR.
         END.
      ...
      CASE opcao:
           ...
           DO:
               FIND {&SECOND-ENABLED-TABLE} WHERE ROWID({&SECOND-ENABLED-TABLE}) = TO-ROWID(ab_unmap.aux_idexclui) NO-LOCK NO-WAIT NO-ERROR.
               ...
               ELSE
                  ASSIGN 
                      msg-erro = msg-erro + "Registro não encontrado para exclusão."
                      msg-erro-aux = 3. /* erros da validação de dados */
