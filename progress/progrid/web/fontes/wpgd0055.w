...
DEFINE TEMP-TABLE ab_unmap
       ...
       FIELD aux_dtlimite AS CHARACTER FORMAT "X(256)":U.

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DEFINITIONS w-html 

CREATE WIDGET-POOL.

DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0055"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0055.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0055.htm
...
ab_unmap.aux_cdcooper ab_unmap.aux_cddopcao ab_unmap.aux_dsendurl ab_unmap.aux_dsretorn ab_unmap.aux_idevento ab_unmap.aux_lspermis ~
...
ab_unmap.aux_cdcopope ab_unmap.aux_eventsel ab_unmap.aux_dtlimite
...
DEFINE FRAME Web-Frame
     ...
     ab_unmap.aux_cdcopope AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     ab_unmap.aux_dtlimite AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1          
...
PROCEDURE CriaListaEventos:
  ...
  DEFINE VARIABLE aux_dtlimite AS INTEGER NO-UNDO INIT 0.
	DEFINE VARIABLE aux_dtlimass AS DATE NO-UNDO.
  ...
  FOR EACH crapeap WHERE crapeap.dtanoage = INT(ab_unmap.aux_dtanoage)
    ...
    /* Bloqueia as Flags, se o status da agenda não for 1 */
		FOR FIRST crapppc FIELDS(dtlimass) WHERE crapppc.cdcooper = crapeap.cdcooper
																				 AND crapppc.dtanoage = INT(ab_unmap.aux_dtanoage). END.
		
		IF AVAILABLE crapppc THEN
			DO:
				ASSIGN aux_dtlimass = crapppc.dtlimass.
        IF aux_dtlimass < TODAY THEN
          DO:
            ASSIGN aux_dtlimite = 1.
          END.
			END.
		ELSE
			DO:
    ...
    IF crapagp.idstagen < 1 OR aux_dtlimass < TODAY OR crapagp.idstagen >= 2 THEN
      ASSIGN aux_flgevobr = "disabled"
             aux_flgcusfe = "2". /* nao está pronto */   
		...
		RUN RodaJavaScript("mevento.push(~{cdagenci:'" + STRING(crapeap.cdagenci)
                                  ...
                                  + "',dtlimass:'" + STRING(aux_dtlimite) + "'~});").
		
    
  END.
  
  IF aux_dtlimite = 1 THEN
    DO:
      ASSIGN ab_unmap.aux_dtlimite = "1".
      RUN RodaJavaScript('alert("Não é possível informar a sugestão de datas pois a data limite (' + STRING(crapppc.dtlimass) + ') já foi alcançada.");').      
    END.
...
PROCEDURE envia-email :
  ...
  ASSIGN aux_dsconteu = "A lista de seleção dos eventos(AGO/AGE), já foi concluida." +
                        "\n\nVerifique e confirme através da tela Menu Assembleias > Processos > Confirma Eventos - AGO/AGE.".
  ...
  IF AVAILABLE crapcop THEN
    DO:
		...
		IF VALID-HANDLE (b1wgen0011)   THEN
			DO:   
				FOR FIRST crapppc FIELDS(dsemlsdf) WHERE crapppc.cdcooper = INTEGER(ab_unmap.aux_cdcooper)
																						 AND crapppc.dtanoage = INTEGER(ab_unmap.aux_dtanoage) NO-LOCK. END.
													 
				IF AVAILABLE crapppc AND crapppc.dsemlsdf <> ? THEN
					DO:
						IF aux_dsdemail <> ? THEN
							DO:
								aux_dsdemail = aux_dsdemail + ";" + crapppc.dsemlsdf.
							END.
						ELSE
							DO:
								aux_dsdemail = crapppc.dsemlsdf.
							END.
					END.                
				ELSE
					DO:
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("aux_dtlimite":U,"ab_unmap.aux_dtlimite":U,ab_unmap.aux_dtlimite:HANDLE IN FRAME {&FRAME-NAME}).
    ...
    DO WITH FRAME {&FRAME-NAME}:
      ...
      DO i = 1 TO NUM-ENTRIES(ab_unmap.aux_lscuseve, ";"):
          FIND FIRST crapeap WHERE crapeap.idevento = INT(ab_unmap.aux_idevento)               
                               ...
                               AND crapeap.cdagenci = 0 EXCLUSIVE-LOCK NO-ERROR.
