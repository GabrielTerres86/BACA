...
DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0033"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0033.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0033.htm
...
PROCEDURE CriaListaEventos :
    ...
    FOR EACH crapeap WHERE (crapeap.idevento = INT(ab_unmap.aux_idevento)   AND
						   ...
						   (crapeap.idevento = 2                            AND 
						    crapeap.cdcooper = INT(ab_unmap.cdcooper)       AND
							crapeap.dtanoage = INT(ab_unmap.aux_dtanoage)   AND
                            crapeap.cdagenci = 0                            AND
							crapeap.flgevsel = YES                          AND
							INT(ab_unmap.aux_idevento) = 2)  NO-LOCK,  
