...
DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0052b"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0052b.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0052b.htm
...
------------------------------------------------------------------------------*/
    ...
    If available crapedp Then
      vetorpacs = "~{nmevento:'" + crapedp.nmevento + "',nmresage:'PA',cdagenci:'0'" + "~}".    
    Else
      vetorpacs = "~{nmevento:' ',nmresage:'PA',cdagenci:'0'" + "~}".    
    ...
    FOR EACH cratagp NO-LOCK BY cratagp.nmresage:
        ...
        IF AVAILABLE crapeap THEN 
          DO:
            ASSIGN vetorpacs = vetorpacs + ",flgativo:'yes'~}".
          END.
        /* Obrigatoriedade escolhida para a COOP */
        ELSE 
          DO:
            ASSIGN vetorpacs = vetorpacs + ",flgativo:'no'~}".
          END.  
    END.
					 ...
					 IF opcao = "inclusao" THEN
							DO: 
									...
									DO i = 1 TO NUM-ENTRIES(ab_unmap.aux_lsagenci,";"):
										 ...
										 CREATE crateap.
										 ASSIGN crateap.cdagenci = INTEGER(ENTRY(i,ab_unmap.aux_lsagenci,";"))
										 ...
										 FIND crapeap WHERE crapeap.cdagenci = INTEGER(ENTRY(i,ab_unmap.aux_lsagenci,";"))
										 ...
										 IF NOT AVAILABLE crapeap THEN                         
											 DO:
												 ...
												 FIND FIRST crapadp WHERE crapadp.cdagenci = INTEGER(ENTRY(i,ab_unmap.aux_lsagenci,";"))
																							...
																							AND crapadp.idevento = 2 NO-LOCK NO-ERROR NO-WAIT.
												...
												FOR EACH crapsde WHERE crapsde.cdagenci = INTEGER(ENTRY(i,ab_unmap.aux_lsagenci,";"))
																					...
																					AND crapsde.idevento = 2 NO-LOCK :
												...
												END.
												 ...
												 CREATE crapeap.
												 ASSIGN crapeap.cdagenci = INTEGER(ENTRY(i,ab_unmap.aux_lsagenci,";"))
