...
DEFINE VARIABLE aux_flgsuges  AS LOGICAL      INIT YES        NO-UNDO.
...
FUNCTION EmBranco RETURNS LOGICAL ():
    ...
    FOR EACH  craprap WHERE craprap.idevento = aux_idEvento   AND
        ...
        FIRST crapgap WHERE crapgap.idevento = craprap.idevento   AND
                            ...
                            crapgap.cdgruava = craprap.cdgruava   NO-LOCK,
    ...
    IF   NOT aux_flgexist   THEN
         msgsDeErro = msgsDeErro + "-> Não há Tópicos e/ou Itens a serem avaliados.<br>".
...
FUNCTION Tabulada RETURNS LOGICAL ():
    ...
    FOR EACH  craprap WHERE craprap.idevento = aux_idEvento      AND
        ...
        FIRST crapgap WHERE crapgap.idevento = craprap.idevento         AND
                            ...
                            crapgap.tpiteava = 1 /* Item Alternativo */ NO-LOCK,
    ...
    IF   aux_flgexist   THEN
         DO:    */
             ...
             FOR EACH craprap WHERE craprap.idevento = aux_idEvento      AND
                                   ...
                                   (craprap.nrseqeve = aux_nrseqdig      OR
                                   (aux_nrseqdig = 0 AND craprap.cdevento = aux_cdEvento)) NO-LOCK
                                    BREAK BY craprap.cdagenci
                                            BY craprap.nrseqeve:

                 IF   FIRST-OF(craprap.cdagenci)   OR
                      FIRST-OF(craprap.nrseqeve)   THEN
                      tot_qtavares = tot_qtavares + craprap.qtavares.
             END.
             ...
             {&OUT} '<table class="tab2" border="1" borderColor="#000000" width="100%">'   SKIP
                    ...
                    '</table>'                                                             SKIP
                    '<br>'                                                                 SKIP.
        /* END.*/ /*comentado para poder imprimir qdo tiver somente comentarios descritivos*/
    /*ELSE
         DO:
             msgsDeErro = msgsDeErro + "-> Não há Tópicos e/ou Itens a serem avaliados.<br>".
             RETURN FALSE.
    ...
    FOR EACH  craprap WHERE craprap.idevento = aux_idEvento      AND
        ...
        FIRST crapgap WHERE crapgap.idevento = craprap.idevento         AND
                            crapgap.cdcooper = 0                        AND
                            crapgap.cdgruava = craprap.cdgruava         AND 
                            crapgap.tpiteava = 2 /* Item Descritivo */ NO-LOCK,
    ...
    {&out} '<div id="div_analisadas">'                              SKIP
    ...
    IF   NOT aux_flgexist   THEN
         {&out} '<script language="JavaScript">' SKIP
                '  document.all.div_analisadas.style.display="none";' SKIP
                '</script>' SKIP.
   ...
   /* Sugestões Analisadas Modelo Antigo*/
    ...
    {&out} '<div id="div_nao_analisadas">' SKIP
           '<table cellspacing="0" cellpadding="0" width="100%">'    SKIP
           '  <tr>'                                                  SKIP
           '    <td>'                                                SKIP
    ...
    IF   NOT aux_flgexist   THEN
         {&out} '<script language="JavaScript">' SKIP
                '  document.all.div_nao_analisadas.style.display="none";' SKIP
                '</script>' SKIP.
...
FUNCTION montaTela RETURNS LOGICAL ():
    ...
    {&out} '   <table border="0" cellspacing="0" cellpadding="0" width="100%" class="tab1">' SKIP
           ...
           '         <td class="tdTitulo1" colspan="4" align="center">' nomeDaCooperativa ' - ' aux_tldatela[tipoDeRelatorio] ' - ' aux_dtAnoAge '</td>' SKIP
           ...
           '      </tr>' SKIP
           '      <tr>' SKIP
           '         <td align="center" class="tdTitulo2" colspan="6">' ENTRY(crapedp.tpevento * 2 - 1,craptab.dstextab) ': ' crapedp.nmevento '</td>' SKIP
           '      </tr>' SKIP
           '      <tr>' SKIP 
           '        <td colspan="6">&nbsp;</td>' SKIP
           '      </tr>' SKIP
   ...
   ELSE
       DO:
          ASSIGN aux_idEvento    = INTEGER(GET-VALUE("parametro1"))
                 ...
                 aux_cdevento    = INTEGER(GET-VALUE("parametro7")) NO-ERROR.          
