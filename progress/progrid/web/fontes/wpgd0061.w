  ...
  RUN STORED-PROCEDURE pc_lista_inscritos_car
    aux_handproc = PROC-HANDLE NO-ERROR (INPUT DECIMAL(ab_unmap.aux_cdcooper) /* Codigo da Cooperativa        */
                                        ...
                                        ,INPUT DECIMAL(ab_unmap.aux_nriniseq) /* Registro Inicial             */
																				,INPUT 0 /*DECIMAL(ab_unmap.aux_nrficpre)*/ /* Numero da Ficha de Presença */
...
PROCEDURE EncerraMatricula:
  ...
  IF NOT AVAILABLE crapedp THEN
    DO:
      ASSIGN msg-erro = "Registro de Evento não encontrado!".
      RETURN "NOK". 
    END.                     
  ...
  IF crapedp.tpevento = 7 OR crapedp.tpevento = 12 THEN
    DO:
      FOR EACH crapfpa NO-LOCK:
        ...
        IF NOT AVAILABLE crapfpa THEN
          DO:
            ASSIGN msg-erro = "Existem fichas de presenças relacionadas ao evento e que não possuem inscritos lançados!".
            RETURN "NOK". 
          END.                           
    ...
    IF AVAILABLE crapadp THEN
      DO:
	    ...
	    END.     
    ELSE
      msg-erro = msg-erro + "Problemas para encerrar matrículas. AGE: " + string(aux_cdagenci) + ", nrseqeve: " + string(ab_unmap.nrseqeve) + ", IDEVE: " + string(INT(ab_unmap.aux_idevento)) + ", CDCOOPER: " + string(INT(ab_unmap.aux_cdcooper)) + ", Dt: " + string(INT(ab_unmap.aux_dtanoage)).
    ...
    DO:
      ...
      FOR EACH crapfpa WHERE crapfpa.cdcopfic = INT(ab_unmap.aux_cdcooper)
        ...
        IF NOT AVAILABLE crapidp THEN
          DO:
            ASSIGN msg-erro = "Existem fichas de presença relacionadas ao evento e que não possuem inscritos lançados! Para confirmar o encerramento pressione o botão 'OK'. Para visualizar as informações das fichas, pressione o botão 'Cancelar' e o botão 'Vizualizar Relatório' na tela de confirmações."
									 msg-erro-aux = 15.
            RETURN "NOK". 
          END.                           
      ...
      DO WITH FRAME {&FRAME-NAME}:
        ...
        IF opcao = "inclusao" THEN
          DO: 
            ...
            IF crapedp.tpevento = 7 OR crapedp.tpevento = 12 THEN /* é assembléia */
...
PROCEDURE NomeCooperado :
 ...
 ASSIGN vetorNome = "". /* PJ332 - SM 7 - Mateus*/
...
/*Fim PJ 322 - SM 7 - Mateus */
    ...
    RUN RodaJavaScript("vetNome = ["  + vetorNome + "];"). /*PJ 322 - SM 7 - Mateus*/
END PROCEDURE.
  ...
  ELSE
    DO:
      ASSIGN msg-erro-aux = 12
             msg-erro = "Limite máximo de inscritos(25), para esta Ficha de Presença foi excedido.".
  ...
  IF AVAILABLE crapidp THEN
    DO:
      ASSIGN msg-erro-aux = 14
             msg-erro = "Já existe uma inscrição realizada para esta pessoa. Inclusão duplicada nao permitida!".
      RETURN "NOK".
    END.
  ELSE
    DO:
...
PROCEDURE process-web-request :
  ...
  ASSIGN ab_unmap.aux_tpfiltro:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = "Conta,2,Pré-Inscrito,1".
  /*PJ 332 - SM 7 - Mateus Inclusão do  ,Comentário,4*/
  ASSIGN ab_unmap.aux_pesqorde:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = "Pré-Inscrito,1,Conta,2,Situação,3,Comentário,4".
  ...
  IF REQUEST_METHOD = "POST":U THEN
    DO:
     ...
     CASE msg-erro-aux:
           ...
           WHEN 12 THEN RUN RodaJavaScript('alert("Limite máximo de inscritos(25), para esta Ficha de Presença foi excedido.");'). 
           
           /* Ficha de Inscricao nao existente */
           WHEN 13 THEN RUN RodaJavaScript('alert("Ficha de Presença não cadastrada.");'). 
           ...
           WHEN 15 THEN
            ...
            END.
					/* Existem inscrições confirmadas sem ficha de presença */
           WHEN 16 THEN
            DO:
