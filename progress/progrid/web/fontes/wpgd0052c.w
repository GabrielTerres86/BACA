...
DEFINE VARIABLE ProgramaEmUso         AS CHARACTER INITIAL ["wpgd0052c"].
DEFINE VARIABLE NmeDoPrograma         AS CHARACTER INITIAL ["wpgd0052c.w"].
...
&Scoped-define WEB-FILE fontes/wpgd0052c.htm
...
PROCEDURE CriaListaEventos :
  ...
  DEF VAR aux_nmagenci AS CHAR NO-UNDO.
	...
	FOR EACH crapagp WHERE crapagp.idevento = INTEGER(ab_unmap.aux_idevento)
		...
		EACH crapage WHERE crapage.cdcooper = crapagp.cdcooper
									 AND crapage.cdagenci = crapagp.cdagenci NO-LOCK:
	
		FIND cratagp WHERE cratagp.cdagenci = crapagp.cdagenci NO-LOCK NO-ERROR NO-WAIT.
    ...
    FOR EACH crapagp WHERE crapagp.idevento = INTEGER(ab_unmap.aux_idevento)  AND
        ...
        IF AVAILABLE cratagp THEN
          DO:
            ASSIGN cratagp.nmresage = cratagp.nmresage + " / " + (IF AVAILABLE crapage THEN crapage.nmresage ELSE "").
          END.
    END.
    ...
    /* cabeçalho */
    RUN RodaJavaScript("meventos.push(~{nmresage:'" + cratagp.nmresage + "',nmevento:'Evento',cdevento:'0'~});").
              ...
              DO i = 1 TO NUM-ENTRIES(ab_unmap.aux_lsevento,";"):
                 ...
                 IF NOT AVAILABLE crapeap THEN                         
                   DO:
                     ...
                     FIND crapedp WHERE crapedp.cdcooper = INT(ab_unmap.aux_cdcooper)                 
                                    ...
                                    AND crapedp.tpevento = 2 /*integração*/ NO-LOCK NO-ERROR. 
                     ...
                     ASSIGN crapeap.cdagenci = INTEGER(ab_unmap.aux_cdagenci)
                            ...
                            crapeap.flgevsel = IF AVAIL crapedp THEN TRUE ELSE FALSE /* PAC ainda não escolheu */
