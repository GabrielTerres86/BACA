/*****************************************************************************
  ...
  Programa wpgd0062a.w
 
  Criação: 25/09/2017 - Criaçao da Tela para utilização pela OQS FILIADA  
                        Somente Eventos do tipo 13 - Pré-Assembleia Empresa
		 		        cópia da tela WPGD0023, Prj. 322 (Andrei)..
...
DEFINE VARIABLE ProgramaEmUso AS CHARACTER INITIAL ["wpgd0062"].
DEFINE VARIABLE NmeDoPrograma AS CHARACTER INITIAL ["wpgd0062a.w"].
...
&Scoped-define WEB-FILE P:/web/fontes/wpgd0062a.htm
...
DEF VAR aux_meses AS CHAR EXTENT 12 INIT ["JANEIRO","FEVEREIRO","MARÇO","ABRIL","MAIO","JUNHO","JULHO","AGOSTO","SETEMBRO","OUTUBRO","NOVEMBRO","DEZEMBRO"].
  ...
  FIND FIRST crapedp WHERE crapedp.idevento = {&SECOND-ENABLED-TABLE}.idevento
                       ...
                       AND crapedp.cdevento = {&SECOND-ENABLED-TABLE}.cdevento 
					   AND crapedp.tpevento = 13 /* ANDREI*/ NO-LOCK NO-ERROR.
...
ASSIGN vetorhistorico = "".
    ...
    ELSE
			vetorhistorico = vetorhistorico + "," + "~{" + "dthist:" + "'" + STRING(craphep.dtmvtolt) + "',dshist:" + "'" + craphep.dshiseve + "'~}".        
END.                                    
        ...
        ELSE  /* Alteracao */
          DO:
            ...
            FIND crapedp WHERE crapedp.idevento = crapadp.idevento
						   ...
						   AND crapedp.tpevento = 13 /* ANDREI */ NO-LOCK NO-ERROR NO-WAIT.
				  ...
				  IF GET-VALUE("dtinieve") <> ? AND GET-VALUE("dtinieve") <> "" AND GET-VALUE("dtinieve") <> "/  /" THEN
				    DO:	
				      ...
				      FIND crapedp WHERE crapedp.idevento = INT(ab_unmap.aux_idevento)
									 ...
									 AND crapedp.dtanoage = INT(GET-VALUE("aux_dtanoage"))
                                     AND crapedp.tpevento = 13 /* ANDREI */	 NO-LOCK NO-ERROR NO-WAIT.
