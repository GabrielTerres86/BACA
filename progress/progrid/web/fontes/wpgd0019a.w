...
DEFINE TEMP-TABLE ab_unmap
     ...
     FIELD aux_verfacil AS CHARACTER FORMAT "X(256)":U
     ...
     FIELD aux_nmfacili AS CHARACTER FORMAT "X(256)":U 
...
ab_unmap.aux_verregio ab_unmap.aux_nrpropos ab_unmap.aux_nmfacili ~
ab_unmap.aux_cdevento ab_unmap.aux_dtanoage ab_unmap.aux_nrcpfcgc ~
ab_unmap.aux_verfacil ab_unmap.aux_semanadi ab_unmap.aux_semapost ~
...
DEFINE FRAME Web-Frame
   ...
   ab_unmap.aux_nrpropos AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
   ab_unmap.aux_nmfacili AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
   ...
   ab_unmap.aux_prefermu AT ROW 1 COL 1 HELP
	 ...
	 ab_unmap.aux_posferna AT ROW 1 COL 1 HELP
          ...
          VIEW-AS FILL-IN 
          SIZE 20 BY 1	  
   ab_unmap.aux_verfacil AT ROW 1 COL 1 HELP
          "" NO-LABEL FORMAT "X(256)":U
          VIEW-AS FILL-IN 
          SIZE 20 BY 1	  
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      FIELD aux_dsdiaeve AS LOGICAL
      FIELD aux_cdcopope AS CHARACTER FORMAT "X(256)":U
      FIELD tel_dsdiaeve AS CHARACTER
      FIELD aux_mesevent AS CHARACTER FORMAT "X(256)":U
      END-FIELDS.
   END-TABLES.
 */
...
/* SETTINGS FOR FILL-IN ab_unmap.aux_posferna IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */   
/* SETTINGS FOR FILL-IN ab_unmap.aux_verfacil IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */   
...
/* SETTINGS FOR FILL-IN ab_unmap.tel_cdoperad IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN ab_unmap.aux_nrpropos IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
/* SETTINGS FOR FILL-IN ab_unmap.aux_nmfacili IN FRAME Web-Frame
   ALIGN-L EXP-LABEL EXP-FORMAT EXP-HELP                                */
...
PROCEDURE carregaDescricao :
  ...
  IF AVAILABLE crapcdp THEN 
    DO:
      ASSIGN ab_unmap.aux_nrpropos = IF STRING(crapcdp.nrpropos) = ? THEN "" ELSE STRING(crapcdp.nrpropos).
    END.
  ...
  IF AVAILABLE gnapfep THEN
    DO:
      ASSIGN ab_unmap.aux_nmfacili = gnapfep.nmfacili
             ab_unmap.aux_nrcpfcgc = STRING(crapcdp.nrcpfcgc).
    END.
...
------------------------------------------------------------------------------*/
  ...
  RUN htmAssociate
    ("aux_nmfacili":U,"ab_unmap.aux_nmfacili":U,ab_unmap.aux_nmfacili:HANDLE IN FRAME {&FRAME-NAME}).
  RUN htmAssociate
  ...
  RUN htmAssociate
    ("aux_verfacil":U,"ab_unmap.aux_verfacil":U,ab_unmap.aux_verfacil:HANDLE IN FRAME {&FRAME-NAME}).	
  RUN htmAssociate
...
PROCEDURE valida-facilitador:
  ...
  FIND LAST gnpapgd WHERE gnpapgd.idevento = INT(ab_unmap.aux_idevento)
                      AND gnpapgd.cdcooper = INT(GET-VALUE("cdcooper"))
  ...
  FIND crapadp WHERE crapadp.idevento = INT(ab_unmap.aux_idevento)
                 AND crapadp.cdcooper = INT({&SECOND-ENABLED-TABLE}.cdcooper)
                 AND crapadp.dtanoage = INT(ab_unmap.aux_dtanoage)
                 AND crapadp.cdagenci = INT({&SECOND-ENABLED-TABLE}.cdagenci)
  ...
  ELSE
    DO:
      ASSIGN aux_dsdiaeve = GET-VALUE("aux_dsdiaeve").
      IF DATE(GET-VALUE("dtinieve")) = ? OR
         DATE(GET-VALUE("dtfineve")) = ? THEN
        DO:
          LEAVE.
        END.
    END.  
      
      /* select * from crapcdp c where c.idevento = 1 and c.cdcooper = 1 and c.dtanoage = 2015 and c.cdevento = 13 and c.cdagenci = 27*/

      
  ASSIGN aux_nrcpfcgc = DEC(GET-VALUE("aux_nrcpfcgc"))
         aux_nrpropos = GET-VALUE("aux_nrpropos").
  ...
  FOR EACH crabadp WHERE crabadp.idevento  = crapadp.idevento
                     ...
                     AND crabadp.dtinieve >= ADD-INTERVAL(DATE(GET-VALUE("dtinieve")),-10,"DAY")
                     AND crabadp.dtfineve <= ADD-INTERVAL(DATE(GET-VALUE("dtfineve")),10,"DAY")
    ...
    /*MESSAGE "crabadp: " + STRING(crabadp.idevento).*/
    IF ((DATE(GET-VALUE("dtinieve")) >= crabadp.dtinieve) OR (DATE(GET-VALUE("dtfineve")) <= crabadp.dtfineve)) THEN
    DO:
    ...
    FIND gnfacep WHERE gnfacep.idevento = INT(ab_unmap.aux_idevento)
                   AND gnfacep.cdcooper = 0
                   AND gnfacep.nrcpfcgc = crapcdp.nrcpfcgc
                   AND gnfacep.nrpropos = crapcdp.nrpropos NO-LOCK NO-ERROR NO-WAIT.
            ...
            DO aux_contador = 1 TO NUM-ENTRIES(aux_dsdiaeve,","):
              IF CAN-DO(aux_dsdiapro,ENTRY(aux_contador,aux_dsdiaeve,",")) THEN
                DO:
                  ...
                  ", Data: " + STRING(GET-VALUE("dtinieve")) + ".".
...
PROCEDURE process-web-request :
	...
	IF REQUEST_METHOD = "POST":U THEN 
      ...
      /* Verifica Facilitador */
			IF GET-VALUE("aux_verfacil") = "N" THEN
				DO:
          ...
          IF vr_errfacil <> ? THEN
            DO:
              ASSIGN msg-erro-aux = 20
                     msg-erro     = "Validacao de Facilitador".
            END.
        END.
              ...
              IF aux_contacop >= crapppc.qtevedia THEN
                DO:
                  ...
                  ASSIGN msg-erro-aux = 18
                         msg-erro = "Quantidade de eventos por cooperativa ultrapassado. Já existem " + STRING(aux_contacop) + " eventos cadastrados.".
                END.          
            END.
        END.
      ...
      /*RUN RodaJavaScript("diasEvento();"). */
        ...
        WHEN 18 THEN 
					DO:
						RUN RodaJavaScript('var conf = confirm("A cooperativa já possui (' + STRING(aux_contacop) + ') eventos nesta data. O limite é de (' + STRING(aux_concopdi) + ') eventos por dia. Deseja confirmar?");'). 
						RUN RodaJavaScript('((!conf) ? document.form.aux_vercoope.value = "N" : document.form.aux_vercoope.value = "S")').
						RUN RodaJavaScript('((!conf) ? limpaDados() : document.getElementById("form").submit())').
					END. 
        WHEN 19 THEN
					DO:
						RUN RodaJavaScript('var conf = confirm("A regional já possui (' + STRING(aux_contareg) + ') eventos nesta data. O limite é de (' + STRING(aux_conregdi)+ ') eventos por dia. Deseja confirmar?");'). 
        ...
        WHEN 20 THEN
            ...
            RUN RodaJavaScript('var conf = confirm("' + STRING(vr_errfacil) + '");'). 
						RUN RodaJavaScript('((!conf) ? document.form.aux_verfacil.value = "N" : document.form.aux_verfacil.value = "S")').
						RUN RodaJavaScript('((!conf) ? limpaDados() : document.getElementById("form").submit())').
					END.
        WHEN 21 THEN
					DO:
