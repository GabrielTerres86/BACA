...
PROCEDURE valida-facilitador:
  ...
  FOR EACH crabadp WHERE crabadp.idevento  = crapadp.idevento
    ...
    /* JMD 29/06/2016 */
    ASSIGN aux_datinite = DATE(GET-VALUE("dtinieve"))
           aux_datcontr = DATE(crabadp.dtinieve)
           aux_flgachou = FALSE
           aux_datachou = TODAY.
    
    DO WHILE TRUE:
    
      IF aux_datinite > DATE(GET-VALUE("dtfineve")) THEN
        LEAVE.
          
        DO WHILE TRUE:
              
          IF aux_datcontr > crabadp.dtfineve THEN
            LEAVE.
      ...
      ASSIGN aux_datcontr = crabadp.dtinieve
             aux_datinite = aux_datinite + 1.
    END.
    
    /* JMD FIM 29/06/2016 */
    
    IF aux_flgachou = TRUE THEN
      DO:
