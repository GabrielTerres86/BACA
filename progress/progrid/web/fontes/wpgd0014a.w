...
Alterações: 10/12/2008 - Melhoria de performance para a tabela gnapses (Evandro).
            ...
            29/11/2011 - Inclusão do SUBSTRING na função PUSH para desconsiderar
                         a vírgula no início da função e carregar corretamente os
            ...
            17/12/2015 -  Alterado crapeap.flgevsel = FALSE para 
                          IF crapedp.tpevento = 2 THEN TRUE -- Integração já é escolhida 
                                                 ELSE FALSE -- PA ainda não escolheu (Vanessa)
            ...
            13/05/2016 - Alterado funcao CriaListaEventos pois ocorria erro de memória.
                         (Jean Michel)
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN vetoreventos = "~{qtdcolun:'4',dseixtem:'Eixo Temático',dstemeix:'Tema',cdevento:'0',nmevento:'Evento',flgtdpac:'Obrig.'~}".
    
    RUN RodaJavaScript("meventos.push("  + vetoreventos + ");").
    
    ASSIGN vetoreventos = "".
    
    /* Carrega o array com todos os Eventos Ativos e sugestoes*/
    FOR EACH crapedp WHERE crapedp.cdcooper = 0                                 AND
                           crapedp.idevento = INTEGER(ab_unmap.aux_idevento)    AND 
                           crapedp.dtanoage = 0                                 AND
                           crapedp.flgativo = TRUE                              AND
       ...
       FIRST gnapetp WHERE gnapetp.cdcooper = craptem.cdcooper                                 AND
         ...
         ELSE
          ...
          IF vetoreventos <> "" THEN
            ASSIGN vetoreventos = vetoreventos + ",".
         ...
         ASSIGN vetoreventos = vetoreventos + "~{dseixtem:'" + gnapetp.dseixtem + "',dstemeix:'" +
                                              ...
                                              ",flgtdpac:'" + STRING(crapedp.flgtdpac,"Sim/Nao") + "'}".
                        ...
                        IF NOT AVAILABLE crapeap THEN         
                          DO:
                            ...
                            ASSIGN crapeap.idevento = INTEGER(ab_unmap.aux_idevento)
                                   ...
                                   crapeap.qtmaxtur = crapedp.qtmaxtur.
                            VALIDATE crapeap.       
