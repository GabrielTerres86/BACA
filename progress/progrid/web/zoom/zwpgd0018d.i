/******************************** INCLUDE ZOOM *********************************
PROGRAMA.....:zwpgd0018d.i
AUTOR........:B&T/Solusoft 
DESCRIÇÃO....:Zoom da Tabela de Custos Diversos

Alterações...: 11/12/2008 - Utilizar variaveis ValorCampo (Guilherme).
*******************************************************************************/

&GLOBAL-DEFINE TituloDoPrograma Tabela de Custos Diversos 
&GLOBAL-DEFINE NomeDoProgramaDeZoom zwpgd0018d.w
&GLOBAL-DEFINE TabelaPadrao crapcdi

/* *** campos para pesquisa. Devem ser preenchidos mesmo não sendo usados *** */
&GLOBAL-DEFINE CampoDePesquisa  crapcdi.dscusind
&GLOBAL-DEFINE CampoDePesquisa1 crapcdi.dscusind
&GLOBAL-DEFINE CampoDePesquisa2 crapcdi.dscusind
&GLOBAL-DEFINE CampoDePesquisa3 crapcdi.dscusind
...
&GLOBAL-DEFINE CamposDisplay "<td class='txtNormal' height='22px'>&nbsp; &nbsp;" crapcdi.dscusind format "x(40)" "</td>" 
&GLOBAL-DEFINE ListaDeCampos crapcdi.idevento crapcdi.nrseqdig crapcdi.dscusind 
&GLOBAL-DEFINE NomeDoProgramaPrincipal  /wpgd0018d.w
&GLOBAL-DEFINE CampoChaveParaRetorno    crapcdi.idevento 
&GLOBAL-DEFINE CampoCompltoParaRetorno  crapcdi.nrseqdig 
&GLOBAL-DEFINE CampoCompltoParaRetorno2 crapcdi.dscusind
...
DEFINE VARIABLE aux_idevento        LIKE crapcdi.idevento NO-UNDO.
  ...
  IF vtippesquisa = "{&NomeCampoDePesquisa}"  AND "{&NomeCampoDePesquisa}" <> ""  THEN DO:
    IF vcriterio = "{&NomeCriterio}" THEN DO:
      FIND FIRST {&TabelaPadrao} WHERE {&CampoDePesquisa} {&Criterio} pesquisa AND
           {&TabelaPadrao}.idevento = aux_idevento USE-INDEX crapcind1 NO-LOCK NO-ERROR.
...
PROCEDURE OpenQuery:
  ...
  IF vtippesquisa = "{&NomeCampoDePesquisa}" AND "{&NomeCampoDePesquisa}" <> ""  THEN DO:
    IF vcriterio = "{&NomeCriterio}" THEN
      OPEN QUERY QueryPadrao FOR EACH {&TabelaPadrao} USE-INDEX crapcind1
