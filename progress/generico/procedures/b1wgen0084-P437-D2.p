...
/*............................................................................
    ...
    Alteracoes: 27/06/2011 - Incluidos procedimentos para tratativas de
              ...
              26/02/2019 - prj450 - chama pc_busca_status_rating para validar rating antes da efetivaçao
                           da proposta (Fabio Adriano - AMcom).
       ...
       ***********************/
       ...
       { includes/PLSQL_altera_session_antes.i &dboraayl={&scd_dboraayl} }
        ...
        RUN STORED-PROCEDURE pc_busca_status_rating aux_handproc = PROC-HANDLE
           ...
           ,INPUT  par_nrctremp /* Número do contrato  */
           ,OUTPUT 0            /* Status do Rating */
           ,OUTPUT 0
           ,OUTPUT "").
            
        CLOSE STORED-PROCEDURE pc_busca_status_rating WHERE PROC-HANDLE = aux_handproc.
        
        { includes/PLSQL_altera_session_depois.i &dboraayl={&scd_dboraayl} }
        
        ASSIGN aux_dscritic = pc_busca_status_rating.pr_dscritic WHEN pc_busca_status_rating.pr_dscritic <> ?.
        ASSIGN aux_cdcritic = pc_busca_status_rating.pr_cdcritic WHEN pc_busca_status_rating.pr_cdcritic <> ?.
        ...
        DO:
            ...
            UNDO EFETIVACAO, LEAVE EFETIVACAO.
        END.
        ELSE IF pc_busca_status_rating.pr_strating <> 2 THEN
             DO: 
                 CREATE tt-erro.
                 ASSIGN tt-erro.cdcritic = 0
                        tt-erro.dscritic = "Contrato nao pode ser efetivado, pois nao há Rating Analisado".
                 UNDO EFETIVACAO, LEAVE EFETIVACAO.
             END.
...
/*-----------inicio p450 rating------------------------------------*/
	   MESSAGE "Antes IF p450".				
       IF craplcr.flgdisap        or    /* Dispensa aprovação             */
	      crawepr.cdfinemp = 68   or    /* finalidade 68 - PRE-APROVADO   */ 
	 	  craplcr.cdlcremp = 6901 THEN  /* Cessões de Cartão              */
          DO: 
/*----------------------------*/
             MESSAGE "Cpf Base"  Crapass.nrcpfcnpj_base.	

			{ includes/PLSQL_altera_session_antes_st.i &dboraayl={&scd_dboraayl} }

			/* Efetuar a chamada a rotina Oracle, para Rating calculado como é feito na contingencia, buscando da Central de Risco do dia anterior -P450 Rating */
			RUN STORED-PROCEDURE pc_habilita_contigencia
				 aux_handproc = PROC-HANDLE NO-ERROR (INPUT par_cdcooper
			...
			/* Fechar o procedimento para buscarmos o resultado */ 
			CLOSE STORED-PROC pc_habilita_contigencia
					aux_statproc = PROC-STATUS WHERE PROC-HANDLE = aux_handproc. 
			...
			ASSIGN aux_cdcritic          = 0
                   aux_dscritic          = ""
                   aux_nivrisco_contigen = 0				   
			       aux_cdcritic = pc_habilita_contigencia.pr_cdcritic
									 WHEN pc_habilita_contigencia.pr_cdcritic <> ?
				   aux_dscritic = pc_habilita_contigencia.pr_dscritic
									 WHEN pc_habilita_contigencia.pr_dscritic <> ?
                   aux_nivrisco_contigen = pc_habilita_contigencia.pr_innivris									 
									 WHEN pc_habilita_contigencia.pr_innivris <> ?. 
			IF aux_cdcritic > 0 OR aux_dscritic <> '' THEN
			  DO:
            ...
            MESSAGE "Risco Conting: "  aux_nivrisco_contigen.		
            ASSIGN aux_nrctrempr = par_nrctremp.		
            message "aux_nrctrempr:  " aux_nrctrempr.			
/*--------------------------*/
			{ includes/PLSQL_altera_session_antes_st.i &dboraayl={&scd_dboraayl} }
			...
			/*RUN STORED-PROCEDURE pc_grava_rating_operacao
				 aux_handproc = PROC-HANDLE NO-ERROR (INPUT par_cdcooper
													 ...
													 ,INPUT par_nrctremp
													 ,INPUT aux_nivrisco_contigen
													 ,INPUT ?             /* null para pr_ntrataut */
													 ,INPUT par_dtmvtolt  /* pr_dtrating */
                                                     ,INPUT 3             /* pr_strating => 3 -- Expirado */
													 ,INPUT 4             /* pr_orrating => 4 --contingencia */
				                                     ,INPUT par_cdoperad
		  ...
		  MESSAGE "Depois grava op aux_dsctrliq: " aux_dsctrliq.			  

        /* chama a obtem_emprestimo_risco, para buscar o risco para gravar o risco inclusão - P450 Rating */   
        ...
        RUN obtem_emprestimo_risco IN h-b1wgen0043
                                (INPUT par_cdcooper,
                                 ...
                                 INPUT FALSE,
                                 INPUT crawepr.cdfinemp,
                                 INPUT craplcr.cdlcremp, /*tt-proposta-epr.cdlcremp, */
                                 INPUT crawepr.nrctrliq, /* antes era: aux_nrctrliq, */
                                 INPUT aux_dsctrliq,     /* par_dsctrliq,   tt-proposta-epr.dsctrliq,  */
                                 OUTPUT TABLE tt-erro,
                                 OUTPUT aux_nivrisco).   /*tt-proposta-epr.nivrisco).    */ 
        ...
        IF   RETURN-VALUE <> "OK"   THEN
             do:
               ...
               ELSE
                    aux_dscritic = "Ocorreram erros na obtem_emprestimo_risco".
               EMPTY TEMP-TABLE tt-erro.
               UNDO EFETIVACAO, LEAVE EFETIVACAO.
             end.

        MESSAGE "Risco p/ INcluir: "  aux_nivrisco.			  
		...
		END CASE.
		MESSAGE "Depois converte: "  aux_nivrisco_num.		
