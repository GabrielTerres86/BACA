...
/*............................................................................
  ...
  Autor   : Gabriel
  Data    : 08/12/2010                        Ultima Alteracao: 23/11/2015
...
PROCEDURE valida-exclusao-convenio:
    ...
    IF   par_flgerlog   THEN
         ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
                aux_dstransa = "Validar cancelamento do convenio de cobranca.".
    ...
    DO WHILE TRUE:
        ...
        FIND FIRST crapcob WHERE crapcob.cdcooper = par_cdcooper   AND
                                 ...
                                 crapcob.nrcnvcob = par_nrconven   
                                 NO-LOCK NO-ERROR.

        IF   AVAIL crapcob   THEN
             DO:
                 aux_dscritic = 
                     "Existem boletos cadastrados para este convenio CEB.".
                 LEAVE.
             END.

        FIND crapcco WHERE crapcco.cdcooper = par_cdcooper   AND
                           crapcco.nrconven = par_nrconven   NO-LOCK NO-ERROR.
...
PROCEDURE exclui-convenio:
    ...
    DEF  INPUT PARAM par_nrdconta AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_nrconven AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_nrcnvceb AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_idseqttl AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_dtmvtolt AS DATE                            NO-UNDO.
    DEF  INPUT PARAM par_flgerlog AS LOGI                            NO-UNDO.
    ...
    IF   par_flgerlog   THEN
         ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
                aux_dstransa = "Efetuar o cancelamento do convenio de cobranca.".
    ...
    DO aux_contador = 1 TO 10:
        ...
        FIND crapceb WHERE crapceb.cdcooper = par_cdcooper   AND
                           crapceb.nrdconta = par_nrdconta   AND
                           crapceb.nrconven = par_nrconven   AND
                           crapceb.nrcnvceb = par_nrcnvceb 
                           EXCLUSIVE-LOCK NO-ERROR NO-WAIT.

        IF   NOT AVAIL crapceb   THEN
             IF   LOCKED crapceb   THEN
                  DO:
                      aux_cdcritic = 341.
                      PAUSE 1 NO-MESSAGE.
                      NEXT.
    ...
    IF   par_flgerlog  THEN
         DO:
             ...
             RUN proc_gerar_log_item (INPUT aux_nrdrowid,
                                      INPUT "nrconven",
                                      INPUT TRIM(STRING(par_nrconven,
                                                        "zzz,zzz,zzz")),
                                      INPUT "").

             RUN proc_gerar_log_item (INPUT aux_nrdrowid,
                                      INPUT "nrcnvceb",
                                      INPUT TRIM(STRING(par_nrcnvceb,
                                                        "zzz,zzz,zzz")),
                                      INPUT "").
         END.
...
PROCEDURE valida-habilitacao:
    ...
    DEF OUTPUT PARAM TABLE FOR tt-erro.
    DEF OUTPUT PARAM par_dsorgarq AS CHAR                            NO-UNDO.
    DEF OUTPUT PARAM par_flgregis AS CHAR                            NO-UNDO.
    DEF OUTPUT PARAM par_cddbanco AS INTE                            NO-UNDO.

    DEF OUTPUT PARAM par_flserasa AS INTE                            NO-UNDO. 


    IF   par_flgerlog   THEN
         ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
                aux_dstransa = "Validar habilitacao de cadastro de cobranca.".
    ...
    DO WHILE TRUE:
        ...
        /* Verificar se existe o convenio */
        FIND crapcco WHERE crapcco.cdcooper = par_cdcooper   AND
                           crapcco.nrconven = par_nrconven   
                           NO-LOCK NO-ERROR.
        ...
        IF   NOT AVAIL crapcco   THEN
         ...
         /* Validar se o convenio está desativado */
         IF  NOT crapcco.flgativo THEN
             DO:
                 /* Não permitir que seja habilitado um convenio que
                    está desativado */
                 ASSIGN aux_cdcritic = 949.
                 LEAVE.
             END.

        ASSIGN par_dsorgarq = crapcco.dsorgarq
               par_flgregis = IF  crapcco.flgregis  THEN
                                  "SIM"
                              ELSE "NAO"
               par_cddbanco = crapcco.cddbanco.
       
        ASSIGN par_flserasa = INT(crapcco.flserasa).
       
        FIND FIRST crapceb WHERE crapceb.cdcooper = par_cdcooper   AND 
                                 crapceb.nrdconta = par_nrdconta   AND
                                 crapceb.nrconven = par_nrconven
                                 NO-LOCK NO-ERROR.

        IF   AVAIL crapceb   THEN
              DO:
                  ASSIGN aux_cdcritic = 793.
                  LEAVE.
              END.
...
PROCEDURE valida-dados-limites:
    ...
    DEF  INPUT PARAM par_inarqcbr AS INTE                           NO-UNDO.
    ...
    IF   par_flgerlog   THEN
         ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
                aux_dstransa = "Validar dados de inclusao/alteracao do convenio.".
    ...
    DO WHILE TRUE:
        ...
        IF  par_inarqcbr < 0 OR par_inarqcbr > 3  THEN
            DO:
                ASSIGN aux_dscritic = "Tipo de arquivo de retorno invalido.".
                LEAVE.
            END.
                    
        IF  par_cddemail = 0 AND par_inarqcbr > 0  THEN
            DO:
                ASSIGN aux_dscritic = "Informe email para arquivo de retorno.".
                LEAVE.
            END.

        /* Se tem arquivo de retorno e email nao cadastrado na crapcem */
        IF  par_inarqcbr > 0                  AND 
            NOT CAN-FIND (crapcem WHERE crapcem.cdcooper = par_cdcooper   AND
                                        crapcem.nrdconta = par_nrdconta   AND
                                        crapcem.idseqttl = par_idseqttl   AND
                                        crapcem.cddemail = par_cddemail)  THEN
            DO:
        ...
        IF   par_dsorgarq = "INTERNET"   THEN /* Se convenio INTERNET */
             DO:

                 IF crapass.idastcjt = 0 THEN
                 DO:
                     
                 /* Senha do 1. deve estar ativa , para a proxima verificacao*/
    ...
    END. /* Fim do tratamento do Critico */
...
PROCEDURE habilita-convenio:
    ...
    DEF  INPUT PARAM par_nrdconta AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_nrconven AS INTE                            NO-UNDO.    
    DEF  INPUT PARAM par_dssitceb AS LOGI                            NO-UNDO.
    DEF  INPUT PARAM par_inarqcbr AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_cddemail AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_flgcruni AS LOGI                            NO-UNDO.
    DEF  INPUT PARAM par_flgcebhm AS LOGI                            NO-UNDO.
    DEF  INPUT PARAM par_dsdregis AS CHAR                            NO-UNDO.
    DEF  INPUT PARAM par_idseqttl AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_dtmvtolt AS DATE                            NO-UNDO.
    DEF  INPUT PARAM par_flcooexp AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_flceeexp AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_flserasa AS INTE                            NO-UNDO.
    DEF  INPUT PARAM par_flgerlog AS LOGI                            NO-UNDO.

    DEF OUTPUT PARAM TABLE FOR tt-erro.
    DEF OUTPUT PARAM par_dsdmesag AS CHAR                            NO-UNDO.
    DEF OUTPUT PARAM par_dsdtitul AS CHAR                            NO-UNDO.
    DEF OUTPUT PARAM par_flgimpri AS LOGI                            NO-UNDO.
    ...
    DEF VAR aux_newdeceb AS LOGI                                     NO-UNDO.
    DEF VAR aux_flgtrans AS LOGI                                     NO-UNDO.
    DEF VAR aux_flgfound AS LOGI INIT FALSE                          NO-UNDO.
                                                      
    DEF VAR log_nrconven AS INTE                                     NO-UNDO.
    DEF VAR log_nrcnvceb AS INTE                                     NO-UNDO.
    DEF VAR log_insitceb AS INTE INIT 2                              NO-UNDO.
    DEF VAR log_inarqcbr AS INTE                                     NO-UNDO.
    DEF VAR log_cddemail AS INTE                                     NO-UNDO.
    DEF VAR log_flgcruni AS LOGI                                     NO-UNDO.
    ...
    DO WHILE TRUE TRANSACTION:
       ...
       ASSIGN aux_dsoperac = "Tentativa de habilitacao de cobranca na conta " +
       ...
       IF crapcco.cddbanco = 85 THEN
          DO:
              IF par_flcooexp = 0 AND par_flceeexp = 0 THEN
                DO:
                    ASSIGN aux_dscritic = "Campo Cooperativa Emite e Expede ou "
                                        + "Cooperado Emite e Expede devem ser "
                                        + "preenchidos".
                    LEAVE.
                END.                
          END.
       ...
       IF NOT AVAIL crapope THEN
        ...
        IF  par_dssitceb THEN
            DO:
                IF  crapcco.dsorgarq = "INTERNET" THEN
                    DO:
                        FOR EACH crabcco WHERE crabcco.cdcooper = par_cdcooper      AND
                                               crabcco.nrconven <> crapcco.nrconven AND
                                               crabcco.flgregis = crapcco.flgregis  AND
                                               crabcco.cddbanco = crapcco.cddbanco  AND
                                               crabcco.dsorgarq = "INTERNET"  AND
                                               crabcco.flginter = TRUE 
                                               NO-LOCK,
        
                           EACH crabceb WHERE crabceb.cdcooper = crabcco.cdcooper AND
                                              crabceb.nrconven = crabcco.nrconven AND
                                              crabceb.nrdconta = par_nrdconta     AND
                                              crabceb.insitceb = 1
                                              NO-LOCK:
        
                            ASSIGN aux_flgfound = TRUE.
        
                        END.
        
                        IF aux_flgfound THEN
                           DO:
                               ASSIGN aux_dscritic = "Ja existe um convenio " + 
                                                     "INTERNET habilitado.".
                               LEAVE.
                           END.
                    END.
       ...
       IF  AVAIL crapceb THEN
           DO:
               IF  crapcco.cddbanco = 001 AND
                   par_inarqcbr = 3   THEN
                   DO:
                       ASSIGN aux_dscritic = "Convenio nao suporta " + 
                                             "arquivo de retorno CNAB400.".
                       LEAVE.
                   END.
           END.
        
       IF crapcco.flgutceb   THEN /* Se tem identificador de sequencia */
          DO:
             FIND FIRST crapceb WHERE crapceb.cdcooper = par_cdcooper AND
                                      crapceb.nrdconta = par_nrdconta AND
                                      crapceb.nrconven = par_nrconven  
             ...
             IF NOT AVAIL crapceb THEN
                DO:
                   ...
                   IF aux_nrcnvceb > 9999 THEN
                      DO:
                         ASSIGN aux_dscritic = "Numero CEB superior a 9999.".
                         LEAVE.
                      END.
       ...
       ASSIGN par_flgimpri = TRUE. /* Perguntar se quer imprimir */

   /* Se ele ja tem um convenio deste tipo , nao perguntar se quer imprimir */
       FOR EACH crapceb WHERE crapceb.cdcooper = par_cdcooper AND
                              crapceb.nrdconta = par_nrdconta 
                              NO-LOCK,
           
           FIRST crabcco WHERE crabcco.cdcooper = par_cdcooper     AND
                               crabcco.nrconven = crapceb.nrconven 
                               NO-LOCK:

           /* Se ja tem , nao imprime */
           IF crabcco.flgregis = crapcco.flgregis THEN
              ASSIGN par_flgimpri = FALSE.
       ...
       DO aux_contador = 1 TO 10:
          ...
          /* Foi despresado tratamento do Oracle ja que seria trabalhoso e 
             a possibilidade de ocorrer o caso de habilitacao simultanea eh
             quase zero. */
          FIND FIRST crapceb WHERE crapceb.cdcooper = par_cdcooper AND
                                   crapceb.nrdconta = par_nrdconta AND
                                   crapceb.nrconven = par_nrconven 
          ...
          IF NOT AVAIL crapceb   THEN
             ...
             ELSE
                DO:
                    ...
                    ASSIGN crapceb.cdcooper = par_cdcooper
                           crapceb.nrdconta = par_nrdconta
                           crapceb.nrconven = par_nrconven
                ...
                END.
          ELSE 
             ASSIGN log_nrconven = crapceb.nrconven
                    log_nrcnvceb = crapceb.nrcnvceb
                    log_insitceb = crapceb.insitceb 
                    log_inarqcbr = crapceb.inarqcbr
                    log_cddemail = crapceb.cddemail
                    log_flgcruni = crapceb.flgcruni. 
       ...
       END. /* Fim LOCK crapceb */     

       /* Permitir a reativação de convênios de cobrança sem registro (Renato - Supero - SD 194301  ) */
       IF NOT CAN-DO("SUPORTE,COBRANCA",crapope.dsdepart) THEN DO:
           /* regra para nao permitir ativar um convenio sem registro 
               do BB inativo */
           IF  par_dssitceb = TRUE AND 
               crapceb.insitceb = 2 AND
               crapcco.flgregis = FALSE AND
               crapcco.cddbanco = 001  THEN DO:
             
               ASSIGN aux_dscritic = "Nao eh permitido habilitar " + 
                                     "convenio BB sem registro inativado.".
               LEAVE.
       ...
       ASSIGN crapceb.dtcadast = par_dtmvtolt
              crapceb.inarqcbr = par_inarqcbr /* Se nao recebe, limpa email */
              crapceb.cddemail = IF   par_inarqcbr = 0   THEN   0 
                                 ELSE par_cddemail 
              crapceb.flgcruni = par_flgcruni
              crapceb.flgcebhm = par_flgcebhm
              crapceb.flcooexp = IF par_flcooexp = 1 THEN TRUE ELSE FALSE
              crapceb.flceeexp = IF par_flceeexp = 1 THEN TRUE ELSE FALSE
              crapceb.flserasa = IF par_flserasa = 1 THEN TRUE ELSE FALSE
              crapceb.insitceb = IF   par_dssitceb   THEN 
                                      1
                                 ELSE
                                      2.

       /* Verificar se existe informação no campo cdoperad */
       IF crapceb.cdoperad = ? OR crapceb.cdoperad = "" THEN
           /* Se não existir grava o operador */
           ASSIGN crapceb.cdoperad = par_cdoperad.
       ELSE 
           /* Se existir, atualiza o operador que homologou */
           ASSIGN crapceb.cdhomolo = par_cdoperad.

       IF NEW crapceb  THEN
          DO: 
              /* Mensagem com o numero do Convenio CEB , se inclusao  */
              IF aux_nrcnvceb > 0  THEN 
                 ASSIGN par_dsdmesag = "Convenio CEB de numero "            + 
                                       TRIM(STRING(aux_nrcnvceb,"zzz,zz9")) + 
                                       " habilitado com sucesso!".
              
              /* 
                ATENCAO! As 8 posicoes destinadas ao numero do convenio no log
                de inclusao, estao sendo utilizadas na tela CONGPR. Se houver
                alteracao neste log a tela devera ser corrigida.
              */
              IF par_flgerlog THEN 
                 ASSIGN aux_dsorigem = 
                            TRIM(ENTRY(par_idorigem,des_dorigens,","))
                        aux_dstransa = 
                            "Incluir convenio de cobranca " +
                            STRING(crapcco.nrconven,"zzzzzzz9") + ".".
          ...
          DO:
             IF par_flgerlog  THEN 
                ASSIGN aux_dsorigem = 
                           TRIM(ENTRY(par_idorigem,des_dorigens,","))
                       aux_dstransa = 
                           "Alterar convenio de cobranca " + 
                           STRING(crapcco.nrconven,"zzzzzzz9") + ".".

          END.

       ASSIGN aux_newdeceb = NEW (crapceb)
              aux_flgtrans = TRUE.
       LEAVE.
    ...
    IF par_flgerlog  THEN /* Logar campos alterados */
       DO:
           RUN proc_gerar_log (INPUT par_cdcooper,
                               INPUT par_cdoperad,
                               INPUT "",
           ...
           IF log_insitceb <> crapceb.insitceb   OR
              log_inarqcbr <> crapceb.inarqcbr   OR 
              log_cddemail <> crapceb.cddemail   OR
              log_flgcruni <> crapceb.flgcruni   OR 
              aux_newdeceb                       THEN
              RUN proc_gerar_log_item        /* Numero convenio */
                      (INPUT aux_nrdrowid,
                       INPUT "nrconven",
                       INPUT TRIM(STRING(log_nrconven,"zzz,zzz,zzz")),
                       INPUT TRIM(STRING(crapceb.nrconven,"zzz,zzz,zzz"))).
           ...
           IF log_insitceb <> crapceb.insitceb   OR
              ...
              log_flgcruni <> crapceb.flgcruni   OR 
              aux_newdeceb                       THEN     
              RUN proc_gerar_log_item        /* Convenio CEB */
                      (INPUT aux_nrdrowid,
                       INPUT "nrcnvceb",
                       INPUT TRIM(STRING(log_nrcnvceb,"zzz,zzz,zzz")),
                       INPUT TRIM(STRING(crapceb.nrcnvceb,"zzz,zzz,zzz"))).

           IF log_insitceb <> crapceb.insitceb   THEN  /* Sit. CEB*/
              RUN proc_gerar_log_item 
                      (INPUT aux_nrdrowid,
                       INPUT "insitceb",
                       INPUT STRING(log_insitceb = 1,"ATIVO/INATIVO"),
                       INPUT STRING((crapceb.insitceb = 1),"ATIVO/INATIVO")).
              
           IF log_inarqcbr <> crapceb.inarqcbr   THEN /* Arquivo Retorno */
              RUN proc_gerar_log_item 
                      (INPUT aux_nrdrowid,
                       INPUT "inarqcbr",
                       INPUT STRING(log_inarqcbr,"9"),
                       INPUT STRING(crapceb.inarqcbr,"9")).

           IF log_cddemail <> crapceb.cddemail   THEN /* Email retorno */
              RUN proc_gerar_log_item 
                      (INPUT aux_nrdrowid,
                       INPUT "cddemail",
                       INPUT TRIM(STRING(log_cddemail,"zz9")),
                       INPUT TRIM(STRING(crapceb.cddemail,"zz9"))).  

           IF log_flgcruni <> crapceb.flgcruni   THEN /* Credito Unificado*/
              RUN proc_gerar_log_item 
                      (INPUT aux_nrdrowid,
                       INPUT "flgcruni",
                       INPUT STRING(log_flgcruni,"SIM/NAO"),
                       INPUT STRING(crapceb.flgcruni,"SIM/NAO")).  
           ...
           DO:
               ...
               aux_dstransa = "Convenio " +
                              STRING(crapceb.insitceb = 1,"ativado/desativado") + 
                              " pelo operador " + crapceb.cdhomolo + " - " + 
                              crapope.nmoperad.

               /* Gerar log de alteracao da habilitacao do convênio */
               RUN proc_gerar_log (INPUT par_cdcooper,
                                   INPUT par_cdoperad,
                                   INPUT "",
                                   INPUT aux_dsorigem,
...
PROCEDURE monta-arquivo-registrada:
   ...
   FORM "Pelo presente instrumento, o COOPERADO acima identificado e"
    ...
    FIND FIRST crapcco WHERE crapcco.cdcooper = tt-crapcop.cdcooper   AND
                             crapcco.nrconven = par_nrconven NO-LOCK NO-ERROR.
    
    /* Se encontrar o registro */
    IF AVAILABLE crapcco AND crapcco.qtdfloat > 0 THEN
      ASSIGN aux_qtdfloat = crapcco.qtdfloat
