...
PROCEDURE integra_proposta:
    ...
    DEF  INPUT PARAM par_nrctaloj AS DECI                           NO-UNDO.
    ...
    GRAVA: DO TRANSACTION ON ERROR  UNDO GRAVA, LEAVE GRAVA
       ...
       RUN STORED-PROCEDURE pc_valida_dados_proposta
        aux_handproc = PROC-HANDLE NO-ERROR (INPUT par_cdcoploj,
                                             INPUT par_nrctaloj,
                                             INPUT par_cdcooper,
                                             INPUT par_tpemprst,
                                             INPUT par_cdlcremp,
          ...
          IF CAN-DO("MOTO,AUTOMOVEL,CAMINHAO",TRIM(CAPS(tt-bens-alienacao.dscatbem))) THEN
            DO:
               RUN valida-dados-alienacao IN h-b1wgen0002 (INPUT par_cdcooper,
                                                           ...
                                                           INPUT IF tt-bens-alienacao.dstipbem = "2" THEN /* Usado */
                                                                    CAPS(tt-bens-alienacao.nrdplaca)
                                                                 ELSE "" /* ZERO KM */,
                                                           ...
                                                           INPUT IF tt-bens-alienacao.dstipbem = "2" THEN /* Usado */
                                                                    CAPS(tt-bens-alienacao.ufdplaca)
                                                                 ELSE "" /* ZERO KM */,
...
PROCEDURE integra_dados_avalista:
    ...
    IF RETURN-VALUE <> "OK"   THEN
      DO:
        ...
        ELSE
          par_dscritic = "Ocorreu um erro interno no sistema.".
