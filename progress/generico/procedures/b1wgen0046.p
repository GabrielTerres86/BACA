...
/***************************************************************************
 Procedure que sera chamada na importacao dos dados vindos do arquivo da JD
 contendo as transacoes diarias
***************************************************************************/
PROCEDURE cria_gnmvspb:

    DEF INPUT PARAM par_cdcooper AS INTE                             NO-UNDO.
    DEF INPUT PARAM par_cdagenci AS INTE                             NO-UNDO.
    DEF INPUT PARAM par_nrdcaixa AS INTE                             NO-UNDO.

    DEF INPUT PARAM par_cdbancmp AS INTE                             NO-UNDO. 
    DEF INPUT PARAM par_cdagecmp AS INTE                             NO-UNDO.
    DEF INPUT PARAM par_dtmvtolt AS DATE                             NO-UNDO.
    DEF INPUT PARAM par_dsdgrupo AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dsorigem AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dsdebcre AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dsareneg AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dsmensag AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dtmensag AS DATE                             NO-UNDO.
    DEF INPUT PARAM par_vllantmo AS DECI                             NO-UNDO.
    
    DEF INPUT PARAM par_dsinstcr AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_nrcnpjcr AS DECI                             NO-UNDO.
    DEF INPUT PARAM par_nmcliecr AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dstpctcr AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_cdagencr AS INTE                             NO-UNDO.
    DEF INPUT PARAM par_dscntacr AS CHAR                             NO-UNDO.
    
    DEF INPUT PARAM par_dsinstdb AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_nrcnpjdb AS DECI                             NO-UNDO.
    DEF INPUT PARAM par_nmcliedb AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dstpctdb AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_cdagendb AS INTE                             NO-UNDO.
    DEF INPUT PARAM par_dscntadb AS CHAR                             NO-UNDO.
    DEF INPUT PARAM par_dsfinmsg AS CHAR                             NO-UNDO.

    DEF OUTPUT PARAM TABLE FOR tt-erro.

    EMPTY TEMP-TABLE tt-erro.

    /*Criticar importacao de mensagem (ja existentes) em datas posteriores*/
    FIND gnmvspb WHERE gnmvspb.dtmensag =  par_dtmensag AND 
                       gnmvspb.dtmvtolt <> par_dtmvtolt 
                       NO-LOCK NO-ERROR.

    IF  AVAIL gnmvspb  THEN
        DO:
            ASSIGN aux_cdcritic = 0
                   aux_dscritic = "Esta mensagem ja foi importada.".
    ...
    CREATE gnmvspb.
    ASSIGN gnmvspb.cdcooper = par_cdbancmp
           gnmvspb.cdagenci = par_cdagecmp

           gnmvspb.dtmvtolt = par_dtmvtolt
           gnmvspb.dsdgrupo = par_dsdgrupo
           gnmvspb.dsorigem = par_dsorigem
           gnmvspb.dsdebcre = par_dsdebcre
           gnmvspb.dsareneg = par_dsareneg
           gnmvspb.dsmensag = par_dsmensag
           gnmvspb.dtmensag = par_dtmensag
           gnmvspb.vllanmto = par_vllantmo
                                          
           gnmvspb.dsinstcr = par_dsinstcr
           gnmvspb.nrcnpjcr = par_nrcnpjcr
           gnmvspb.nmcliecr = par_nmcliecr
           gnmvspb.dstpctcr = par_dstpctcr
           gnmvspb.cdagencr = par_cdagencr
           gnmvspb.dscntacr = par_dscntacr
                                          
           gnmvspb.dsinstdb = par_dsinstdb
           gnmvspb.nrcnpjdb = par_nrcnpjdb
           gnmvspb.nmcliedb = par_nmcliedb
           gnmvspb.dstpctdb = par_dstpctdb
           gnmvspb.cdagendb = par_cdagendb
           gnmvspb.dscntadb = par_dscntadb
           gnmvspb.dsfinmsg = par_dsfinmsg.
    VALIDATE gnmvspb.
    ...
    IF  par_nmmsgenv = "STR0005" OR par_nmmsgenv = "PAG0107"  THEN
        ASSIGN aux_textoxml[9]  = "<" + par_nmmsgenv + ">"
               ...
               aux_textoxml[19] = "<ISPBIFCredtd>" + par_ispbcred + 
                                  "</ISPBIFCredtd>"
               aux_textoxml[20] = "<AgCredtd>" + aux_cdagenbc + 
                                  "</AgCredtd>"
      ...
      DO:
          ...
          ASSIGN aux_textoxml[9]  = "<" + par_nmmsgenv + ">"
                 ...
                 aux_textoxml[20] = "<ISPBIFCredtd>" + par_ispbcred + 
                                    "</ISPBIFCredtd>"
                 aux_textoxml[21] = "<AgCredtd>" + aux_cdagenbc + 
                                    "</AgCredtd>"
