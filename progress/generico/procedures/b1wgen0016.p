...
/*..............................................................................
    ...
    Autor   : Evandro/David
    Data    : Abril/2006                     Ultima Atualizacao: 19/09/2016
    ...
    Alteracoes: 22/08/2007 - Incluida procedure de Convenios Aceitos (Diego).
                ...
                06/03/2015 - Correçao da passagem de valor para temp-table 
                ...
                05/08/2015 - Adicionar parametros para gravaçao do campo 
                ...
                30/09/2015 - Remover as procedures debita-agendamento-transferencia
                             e debita-agendamento-pagamento que estao convertidas 
                             em Oracle, e nenhum fonte utiliza mais essas procedures.
                             Removidas na versao 1042 do fonte.
                ...
                19/11/2015 - As procedures paga_convenio e paga_titulo foram
                             ajustadas para nao registrar dados do preposto na 
                             geraçao do comprovante, PRJ. Ass. Conjunta. (Jean Michel)
                ...
                04/12/2015 - Prj 131. Ajustadas procedures verifica_convenio,
                             ...
                             estrutura de aprovaçao conjunta. (Reinert)
			   ...
			   06/05/2016 - Ajuste realizados:
							...
							-> Retirar as rotinas agendamento-recorrente, cadastrar-agendamento
							   pois elas já estao convertidas e os fontes que as chamaao foram ajustados
							   para utilizar a sua respectiva conversao
						    (Adriano - M117).
	
	          19/05/2016 - Ajustes realizados:
					       -> Realizado a inclusao da rotina cadastrar-agendamento;
						   -> Incluido aprovaçao de agendamento de TED para assintura conjunta						   
						   (Adriano - M117).
                            ...
                            DO:
                                ...
                                RUN STORED-PROCEDURE pc_lista_aplicacoes_car
                                 aux_handproc = PROC-HANDLE NO-ERROR (INPUT tbcapt_trans_pend.cdcooper,    /* Código da Cooperativa */
                                                                      ...
                                                                      INPUT tbcapt_trans_pend.nraplicacao, /* Número da Aplicaçao - Parâmetro Opcional */
...
PROCEDURE convenios_aceitos:
    ...
    FOR EACH crapcon NO-LOCK WHERE crapcon.cdcooper = par_cdcooper  AND
        ...
        IF (((crapcon.cdempcon = 24 OR crapcon.cdempcon = 98) AND 
            ...
            tt-convenios_aceitos.hhoracan = "Estorno nao permitido para este convenio".
        ELSE
      ...
      DO:
        ...
        DO  aux_cont_raiz = 1 TO xRoot:NUM-CHILDREN: 
          ...
          DO aux_cont = 1 TO xRoot2:NUM-CHILDREN:
            ...
            ASSIGN tt-tbpagto_darf_das_trans_pend.nrcpfcgc             =  DEC(xText:NODE-VALUE) WHEN xField:NAME = "nrcpfcgc".
                ...
                ELSE IF tbgen_trans_pend.tptransacao = 6 THEN /* Pré-Aprovado */
                    DO:
                        ...
                        ELSE
                            ...
                            END.
                    END.
                ELSE IF tbgen_trans_pend.tptransacao = 7 THEN /* Aplicaçoes */
                    DO:
        ...
        DO TRANSACTION ON ERROR UNDO TRANSACAO, LEAVE TRANSACAO:
                    ...
                    DO:
                        ...
                        RUN STORED-PROCEDURE pc_verifica_operacao_prog
                            aux_handproc = PROC-HANDLE NO-ERROR      
                              ...
                              ,INPUT  par_nrcpfope /* chw tt-tbgen_trans_pend.nrcpf_operador */
                ...
                ELSE IF tt-tbgen_trans_pend.tptransacao = 6 THEN /* Pré-Aprovado */
                	DO:
                        ...
                        IF  par_indvalid = 0 AND aux_conttran = 1 THEN
                            ...
                            END.
                	END.
                ELSE IF tt-tbgen_trans_pend.tptransacao = 7 THEN /* Aplicaçoes */
                	DO:
                        ...
                        /* Efetivar Cancelamento Aplicaçao */
                        ...
                        IF CAN-DO("1",STRING(tt-tbcapt_trans_pend.tpoperacao)) THEN
                            ...
                            END. /* Fim efetivar Cancelamento Aplicaçao */
                                    ...
                                    DO:
                                        ...
                                        RUN STORED-PROCEDURE pc_val_solicit_resg
                                           aux_handproc = PROC-HANDLE NO-ERROR (INPUT par_cdcooper,                          /* Código da Cooperativa */
                                                                                ...
                                                                                INPUT tt-tbcapt_trans_pend.nraplicacao,         /* Número da Aplicaçao */
                                                                                ...
                                                                                INPUT 0,                                     /* Resgate na Conta Investimento (Identificador informado em tela, 0 – Nao) */
                                                                                INPUT 1,                                     /* Identificador de validaçao do bloqueio judicial (0 – Nao / 1 – Sim) */
                                                                                INPUT 0,                                     /* Identificador de Log (Fixo no código, 0 – Nao / 1 - Sim) */
                                                                                INPUT '',                                    /* Operador */
                                                                                INPUT '',                                    /* Senha */
                ...
                ELSE IF tt-tbgen_trans_pend.tptransacao = 8 THEN /* Débito Automático */
                	DO:
                        ...
                        IF tt-tbconv_trans_pend.tpoperacao = 1 THEN /* Efetivaçao Cadastro Autorizaçao Débito Automático */
                            DO:
                                /* Validaçao */
                                ...
                                IF par_indvalid = 1 AND aux_conttran = 1 THEN
                                    ...
                                    END.
                            END.
                        ELSE IF CAN-DO("2,3",STRING(tt-tbconv_trans_pend.tpoperacao)) THEN /* Efetivaçao Bloqueio Lançamento Débito Automático | Efetivaçao Desbloqueio Lançamento Débito Automático */
                            DO:
                                /* Efetivaçao */
                                IF par_indvalid = 1 AND aux_conttran = 1 THEN
                                    DO:
...
FUNCTION IdentificaMovCC RETURNS INTEGER 
    ...
    ELSE
    IF par_tptransacao = 1 OR   /* Transferencia Intracoop. */
    ...
    IF (par_tptransacao = 6) OR   /* Crédito Pré-Aprovado        */
       (par_tptransacao = 7 AND 
        par_tpoperacao  = 1) OR   /* Cancelamento Nova Aplicaçao */
       (par_tptransacao = 7 AND 
        par_tpoperacao  = 2) THEN /* Resgate Aplicaçao           */
       RETURN 1. /* Créditos */
    ELSE
       RETURN 4. /* Sem Movimentaçao na CC */
END FUNCTION.
