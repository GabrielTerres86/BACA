/*.............................................................................
  ...
  Autor   : Gabriel
  Data    : Setembro/2009                       Ultima Atualizacao: 11/10/2012
...
PROCEDURE verifica_atualizacao:
    ...
    DEF  INPUT PARAM par_dsretorn AS LOG                              NO-UNDO.
    ...
    IF  par_flgerlog  THEN
        ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
               aux_dstransa = "Verificar se o Rating em questao pode " +
                              "ser atualizado.".
    ...
    DO WHILE TRUE:
       ...
       FIND crapnrc WHERE crapnrc.cdcooper = par_cdcooper   AND
                          ...
                          crapnrc.tpctrrat = par_tpctrrat  
                          NO-LOCK NO-ERROR.

       IF   NOT AVAILABLE crapnrc   THEN
            DO:
                aux_cdcritic = 925. /* Rating nao encontrado */
                LEAVE.
            END.
       ...
       IF  par_cdcooper <> 3 THEN
           DO:
       IF   MONTH(par_dtmvtolt) <> MONTH(crapnrc.dtmvtolt)   OR
            YEAR (par_dtmvtolt) <> YEAR (crapnrc.dtmvtolt)   THEN     
            DO: 
                aux_dtmvtolt = ADD-INTERVAL(crapnrc.dtmvtolt,6,"MONTHS").
             
                /* Se hoje nao é maior do que a ult. alteraçao  mais 6 meses*/

                IF   NOT par_dtmvtolt >= aux_dtmvtolt   THEN
                     DO:
                         IF   NOT ( MONTH(par_dtmvtolt) = MONTH (aux_dtmvtolt)   AND
                                    YEAR (par_dtmvtolt) = YEAR  (aux_dtmvtolt) ) THEN
                              DO:
                                  aux_dscritic = "Atualizacao só permitida " +
                                                 "6 meses após a alteraçao.". 
                                  LEAVE.
                              END.                         
                     END.
            END.
       ...
       /* Se deu erro, ja criou critica entao retorna */
       IF   RETURN-VALUE <> "OK" THEN
            RETURN "NOK".
                     
       /* O Rating efetivo de agora porem "atualizado"  */
       FIND FIRST tt-impressao-risco NO-LOCK NO-ERROR.

       /* O pior rating proposto */
       FIND crabnrc WHERE ROWID (crabnrc) = par_rowidnrc NO-LOCK NO-ERROR.

       /* Usar a pior nota */
       IF   NOT AVAILABLE crabnrc OR
            tt-impressao-risco.vlrtotal >= crabnrc.nrnotrat THEN
            DO: 
                ASSIGN par_nrnotrat = tt-impressao-risco.vlrtotal
                       par_indrisco = tt-impressao-risco.dsdrisco
                       par_rowidnrc = ROWID(crapnrc).
            END.
       ELSE
            DO:
                /* O indrisco e rowid nao precisa atribuir pois */
                /* ja veio da procedure procura_pior_nota */
                ASSIGN par_nrnotrat = crabnrc.nrnotrat. 
            END.
...
PROCEDURE obtem_emprestimo_risco:
    ...
    ASSIGN aux_dtrefere = par_dtmvtolt - DAY (par_dtmvtolt).

    DO WHILE TRUE:

       IF   CAN-DO("1,7",STRING(WEEKDAY(aux_dtrefere)))               OR
            CAN-FIND(crapfer WHERE crapfer.cdcooper = par_cdcooper    AND
                                   crapfer.dtferiad = aux_dtrefere)   THEN
            DO:
    ...
    /* Estouro de conta */
    FIND LAST crapris WHERE 
              crapris.cdcooper = par_cdcooper    AND
              crapris.nrdconta = par_nrdconta    AND
              crapris.dtrefere >= aux_dtrefere   AND
              crapris.inddocto = 1               AND /* Dcto 3020*/
              crapris.cdmodali = 0101            AND /*Adiant. a Depos*/
              crapris.cdorigem = 1                  /* Conta */             
              NO-LOCK NO-ERROR.

    
    IF   AVAIL crapris   THEN /* Risco Por conta Estourada */
         DO:
             RUN descricoes_risco(INPUT par_cdcooper,
                                  INPUT 0,
                                  INPUT 0,
             ...
             par_nivrisco = tt-impressao-risco.dsdrisco.   

             RETURN "OK".

         END.
     
    /* Se chegou aqui nao tem atraso por conta Estourada */

    FIND LAST crapris WHERE crapris.cdcooper = par_cdcooper    AND
                            crapris.nrdconta = par_nrdconta    AND
                            crapris.dtrefere >= aux_dtrefere   AND
                            crapris.inddocto = 1               AND /*Dcto 3020*/
                            crapris.cdmodali = 0299            AND /*Emprest. */
                            crapris.cdorigem = 3               AND  /*Emprest  */
                            crapris.qtdiaatr > 0
                            NO-LOCK NO-ERROR.

    /* Se atraso por emprestimo */
    IF    AVAIL crapris   THEN
          DO: 
              RUN descricoes_risco
                         (INPUT par_cdcooper,
                          INPUT 0,
                          INPUT 0,
                          INPUT 0,
    ...
    FIND LAST crapris WHERE crapris.cdcooper = par_cdcooper    AND
                            crapris.nrdconta = par_nrdconta    AND
                            crapris.dtrefere >= aux_dtrefere   AND
                            crapris.inddocto = 1               AND /*Docto 3020*/
                            crapris.cdmodali = 0499            AND /*Financ. */
                            crapris.cdorigem = 3               AND /*Financ. */
                            crapris.qtdiaatr > 0
                            NO-LOCK NO-ERROR.
    ...
    /* Se chegou aqui nao existe atraso por emprestimo */

    /* Todos os Emprestimos em aberto */
    FOR EACH crapepr WHERE crapepr.cdcooper = par_cdcooper   AND
        ...
        /* Tem que ter 6 riscos na central pra ir com 'A' */
        FOR EACH crapris WHERE crapris.cdcooper = crapepr.cdcooper   AND
                               crapris.nrdconta = crapepr.nrdconta   AND
                               crapris.nrctremp = crapepr.nrctremp   AND
                               crapris.cdorigem = 3                  NO-LOCK:

            aux_qtdmeses = aux_qtdmeses + 1.

            IF   aux_qtdmeses >= 6   THEN
                 LEAVE.
    ...
    /* Pelo menos um emprestimo com 6 ou mais meses*/
    IF   aux_qtdmeses >= 6   THEN 
         DO:
             ASSIGN par_nivrisco = "A".         
         END.
    ELSE    
         DO:
             /* Risco da central do ultimo mes */   
             FIND LAST crapris WHERE 
                       crapris.cdcooper = par_cdcooper    AND 
                       crapris.nrdconta = par_nrdconta    AND
                       crapris.dtrefere >= aux_dtrefere   AND
...
/*****************************************************************************
 Traz as contas que possuem risco sem um Rating criado.
 Baseado na includes crps280.i 
*****************************************************************************/

PROCEDURE riscos_sem_rating:
    ...
    DEF  VAR         aux_contador AS INTE                            NO-UNDO.
    DEF  VAR         aux_dtrefere AS DATE                            NO-UNDO.
    DEF  VAR         aux_qtpreatr AS INTE                            NO-UNDO.   
    DEF  VAR         aux_vldivida AS DECI                            NO-UNDO.
    DEF  VAR   aux_vlprejuz_conta AS DECI                            NO-UNDO. 
    DEF  VAR         aux_percentu AS DECI                            NO-UNDO.
    DEF  VAR         aux_qtdmeses AS INTE                            NO-UNDO.

    DEF  VAR         rel_percentu AS DECI EXTENT 999                 NO-UNDO.
    ...
    IF  par_flgerlog  THEN
        ASSIGN aux_dsorigem = TRIM(ENTRY(par_idorigem,des_dorigens,","))
               aux_dstransa = "Trazer os riscos que estao sem Rating.". 
    ...
    FOR EACH craptab WHERE craptab.cdcooper = par_cdcooper AND 
        ...
        ASSIGN aux_contador = INT(SUBSTR(craptab.dstextab,12,2))
               rel_percentu[aux_contador] = DEC(SUBSTR(craptab.dstextab,1,6)).

    END.

    FOR EACH crapris WHERE crapris.cdcooper = par_cdcooper           AND

                          (crapris.nrdconta = par_nrdconta           OR
                           par_nrdconta     = 0 )                    AND 
                          
                          (crapris.nrctremp = par_nrctremp           OR
                           par_nrctremp = 0)                         AND

                          (crapris.cdagenci = par_cdagenci           OR
                           par_cdagenci = 0)                         AND  

                           crapris.cdorigem = 3  /* Emprestimo */    AND 
                           crapris.dtrefere >= aux_dtrefere          AND
                           crapris.inddocto = 1  /* Docto 3020 */    AND
                           crapris.vldivida <> 0                     AND 
                           crapris.vldivida < 5000000                NO-LOCK: 
        ...
        /* Somente se ele nao tem Rating */
        IF   CAN-FIND (crapnrc WHERE 
                       crapnrc.cdcooper = par_cdcooper      AND
                       crapnrc.nrdconta = crapris.nrdconta  AND
                       crapnrc.insitrat = 2)               THEN
             NEXT.
        ...
        /* So emprestimos com riscos piores que o A */
        IF   crawepr.dsnivris = "A" THEN
             NEXT.
                
             
        /* Estouro de conta */
        FIND LAST crabris WHERE 
                  crabris.cdcooper = par_cdcooper       AND
        ...
        /* Quantidade de meses */
        FOR EACH crabris WHERE crabris.cdcooper = par_cdcooper       AND
                               crabris.nrdconta = crapris.nrdconta   AND
                               crabris.nrctremp = crapris.nrctremp   AND 
                               crabris.cdorigem = 3 /* Emprestimo*/  NO-LOCK:

            aux_qtdmeses = aux_qtdmeses + 1.

        END.

        /* Tem que ter no minimo 6 meses */
        IF   aux_qtdmeses < 6   THEN
             NEXT.
              
        FOR EACH crapvri WHERE crapvri.cdcooper = par_cdcooper         AND
                               ...
                               crapvri.nrseqctr = crapris.nrseqctr     NO-LOCK:
            ...
            IF  crapvri.cdvencto = 310 OR
                ...
                ASSIGN aux_vlprejuz_conta = aux_vlprejuz_conta + crapvri.vldivida.
        ...
        IF  aux_qtpreatr <= 0     AND /* Em dia*/
            ...
            aux_vldivida > 0      THEN /* Nao listar Prejuizo */
            DO:  
                FIND crapepr WHERE crapepr.cdcooper = par_cdcooper      AND
                ...
                CREATE tt-ratings.
                ASSIGN tt-ratings.cdagenci = crapris.cdagenci
                       tt-ratings.nrdconta = crapris.nrdconta
                       tt-ratings.nrctrrat = crapris.nrctremp
                       tt-ratings.vloperac = crawepr.vlemprst 
                       tt-ratings.indrisco = crawepr.dsnivris
                       tt-ratings.dtmvtolt = crapepr.dtmvtolt
                                             WHEN AVAIL crapepr. 
                ...
                RUN obtem_risco IN h-b1wgen9999 (INPUT par_cdcooper,
                                                 ...
                                                 OUTPUT tt-ratings.dsdrisco,
                                                 OUTPUT tt-ratings.dtrefere).
...
/*****************************************************************************
 Atualizar o Risco das Operaçoes de emprestimo para "A". Atraves da opcao
 'S" da tela ATURAT. 
*****************************************************************************/

PROCEDURE atualiza_risco_operacao:
    ...
    DO WHILE TRUE:
        ...
        /* Nivel da operacao passa a ser A */
        ASSIGN crawepr.dsnivris = "A"
               crawepr.dtaltniv = par_dtmvtolt.   
...
PROCEDURE calcula_rating_juridica:
    ...
    ********************************************************************/
    ...
    FOR EACH crapavt WHERE crapavt.cdcooper = par_cdcooper         AND
                           ...
                           crapavt.dsproftl = "SOCIO/PROPRIETARIO" NO-LOCK
    ...
    IF   CAN-DO ("27,39,44,45,46,48",STRING(crapjur.natjurid))   THEN
         DO:
    ...
    ********************************************************************/
    ...
    ELSE                         /* Limites / Descontos */
         DO:
             ...
             ELSE
                  ASSIGN aux_nrseqite = 2.   /* Descontos*/
         END.
...
PROCEDURE natureza_operacao:
    ...
    IF   par_tpctrato = 90   THEN  /* Emprestimo / Financiamento */ 
         DO:
             IF   par_idquapro > 1   THEN  
                  CASE par_idquapro:
                                 /* Renovacao de credito de divida */ 
                      WHEN   2                THEN   par_nrseqite = 2.
                                 /* Renegociacao / Composicao de divida */
         ...
         DO:                                         
             ...
             FOR EACH crablim WHERE crablim.cdcooper = par_cdcooper   AND
                                   ...
                                   ((crablim.tpctrlim = 3             AND   
                                    ...
                                    (crablim.tpctrlim = 1             AND
                                     crablim.dtfimvig = ?))           NO-LOCK:
...
PROCEDURE proc_calcula:

    DEF INPUT PARAM par_flgcriar AS LOG                       NO-UNDO.
    DEF INPUT PARAM par_nrdconta AS INT                       NO-UNDO.
    DEF INPUT PARAM par_tpctrrat AS INT                       NO-UNDO.
    DEF INPUT PARAM par_nrctrrat AS INT                       NO-UNDO.
    ...
    DEF OUTPUT PARAM par_indrisco AS CHAR                     NO-UNDO.
    DEF OUTPUT PARAM par_nrnotrat AS DEC                      NO-UNDO.
    DEF OUTPUT PARAM TABLE FOR tt-erro.
    DEF OUTPUT PARAM TABLE FOR tt-impressao-risco-tl.
    ...
    IF par_insitrat = 1 THEN
       DO:  
          ...
          IF AVAIL tt-impressao-risco THEN
             ASSIGN par_indrisco = tt-impressao-risco.dsdrisco
                    par_nrnotrat = tt-impressao-risco.vlrtotal.
    ...
    ELSE
      DO:
        IF par_flgcriar THEN
           DO:
              RUN atualiza_rating (INPUT  par_cdcooper,
                                    INPUT  0,
                                    INPUT  0,
                                    INPUT  par_cdoperad,
                                    INPUT  par_dtmvtolt,
