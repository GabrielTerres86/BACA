...
/******************************************************************************
  ...
  |  carrega_dados_tarifa_cobranca   |  .pc_carrega_dados_tarifa_cobranca     |
...
/*.............................................................................
    ...
    Alteracoes: 24/07/2013 - Retirado na leitura crapfco a verificacao da cdcooper 
                ...
                07/07/2016 - Na procedure rel-receita-tarifa e rel-receita-tarifa-resumido,
                             alterado na busca da craplat o campo dtmvtolt para dtefetiv
                             (Lucas Ranghetti #449309)
...
PROCEDURE buscar-cadcat:
    ...
    FIND crapcat WHERE crapcat.cdcatego = par_cdcatego NO-LOCK NO-ERROR.
    ...
    FIND craptic WHERE craptic.cdtipcat = crapcat.cdtipcat NO-LOCK NO-ERROR.

    IF  NOT AVAIL(craptic) THEN
        DO:
            ASSIGN aux_cdcritic = 0
                   aux_dscritic = "Tipo de Categoria inexistente!".
    ...
    ASSIGN aux_cdsubgru = crapcat.cdsubgru.

    IF ( aux_cdsubgru > 0 ) THEN
        DO:

            FIND crapsgr WHERE crapsgr.cdsubgru = crapcat.cdsubgru NO-LOCK NO-ERROR.
        
            IF  AVAIL(crapsgr) THEN
                DO:
                    ASSIGN aux_dssubgru = crapsgr.dssubgru.
                END.
            ...
            IF  AVAIL(crapgru) THEN
                DO:
                    ASSIGN aux_cddgrupo = crapgru.cddgrupo
                           aux_dsdgrupo = crapgru.dsdgrupo.
                END.

        END.


    ASSIGN  par_dscatego = crapcat.dscatego
            par_cddgrupo = aux_cddgrupo
            par_dsdgrupo = aux_dsdgrupo
            par_cdsubgru = aux_cdsubgru
            par_dssubgru = aux_dssubgru
            par_dstipcat = craptic.dstipcat
            par_cdtipcat = craptic.cdtipcat.
...
PROCEDURE incluir-cadcat:
    ...
    CREATE crapcat.
    ASSIGN crapcat.cdcatego = par_cdcatego
           crapcat.dscatego = par_dscatego
           crapcat.cdsubgru = par_cdsubgru
           crapcat.cdtipcat = par_cdtipcat.
    VALIDATE crapcat.
...
PROCEDURE excluir-cadcat:
    ...
    FIND FIRST craptar WHERE craptar.cdcatego = par_cdcatego NO-LOCK NO-ERROR.

    IF AVAIL craptar THEN
    DO:
        ASSIGN aux_cdcritic = 0
               aux_dscritic = "Operacao nao permitida. Categoria vinculada a uma tarifa!".
...
PROCEDURE alterar-cadcat:
    ...
    DO aux_contador = 1 TO 10:
		...
		ELSE
            DO:
			   ASSIGN crapcat.dscatego = par_dscatego
                      crapcat.cdsubgru = par_cdsubgru
                      crapcat.cdtipcat = par_cdtipcat.
...
PROCEDURE carrega_dados_tarifa_cobranca:
    ...
    FIND FIRST crapint WHERE UPPER(crapint.dsinctar) 
                             MATCHES(UPPER(par_dsincide) + '*')
                             NO-LOCK NO-ERROR.

    IF  NOT AVAIL(crapint) THEN
        DO:
            ASSIGN aux_cdcritic = 0
				   aux_dscritic = "Tipo de incidencia de tarifa nao cadastrada!".
    ...
    IF TRIM(par_cdmotivo) = "0" THEN
        par_cdmotivo = "".

    IF TRIM(par_cdmotivo) <> "" THEN
    DO:
        IF LENGTH(par_cdmotivo) < 2 THEN
            ASSIGN par_cdmotivo = "0" + par_cdmotivo.
    END.

    FIND FIRST  craptar WHERE craptar.cdinctar = crapint.cdinctar AND
                              craptar.cdocorre = par_cdocorre     AND
                              craptar.cdmotivo = par_cdmotivo     AND
                              craptar.inpessoa = par_inpessoa
                              NO-LOCK NO-ERROR.
    ...
    IF  NOT AVAIL(craptar) THEN
        DO: 
            ...
            IF par_cdmotivo = "" THEN DO:
                ...
                FIND FIRST craptar WHERE craptar.cdinctar = crapint.cdinctar AND
                                         craptar.cdocorre = par_cdocorre     AND
                                         craptar.cdmotivo = "00"             AND
                                         craptar.inpessoa = par_inpessoa
                                         NO-LOCK NO-ERROR.

                IF NOT AVAIL(craptar) THEN DO:
    ...
    DO:
        ...
        FOR EACH crapfco WHERE crapfco.cdcooper = par_cdcooper     AND
            ...
            ASSIGN tt-vigenc.cdfvlcop = crapfco.cdfvlcop
                   ...
                   tt-vigenc.nrconven = crapfco.nrconven
        ...
        DO:
            ...
            FOR EACH tt-vigenc NO-LOCK BY tt-vigenc.qtdiavig:
                ...
                UNIX SILENT VALUE("echo " + STRING(TIME,"HH:MM:SS")     +
                                  ...
                                  " Cnv: " + STRING(tt-vigenc.nrconven) +
                                  " Oco: " + STRING(tt-vigenc.cdocorre) +
        ...
        DO:             
            ...
            ASSIGN aux_cdcritic  = 0
				           aux_dscritic  = "Tarifa: "  + STRING(craptar.cdtarifa) + chr(13) +
                                   ...
                                   "\nConvenio: " + STRING(par_nrconven).
                   aux_dsassunto = "\nErro Faixa Valor Coop.: " + crapcop.nmrescop.
...
PROCEDURE rel-receita-tarifa:                                   
    ...
    FOR EACH craplat WHERE craplat.insitlat  = 2             AND
                          (craplat.dtefetiv >= par_dtinicio  AND
                           craplat.dtefetiv <= par_dtafinal) NO-LOCK:
        ...
        CREATE tt-receita.
        ASSIGN tt-receita.dtmvtolt = craplat.dtefetiv
