...
PROCEDURE grava-proposta-completa:
    ...
    Grava: DO TRANSACTION ON ERROR  UNDO Grava, LEAVE Grava
        ...
        /* Estou chamando a criacao dos avalistas antes da grava dados */
        /* proposta, pois eu preciso do crapavl criado antes de alimentar */
        /* os dados da SCR* (GABRIEL-RKAM) */
...
PROCEDURE altera-valor-proposta:
	...
	DEF VAR          aux_flgporta AS LOGI                           NO-UNDO.       
    ...
    DO WHILE TRUE TRANSACTION
       ON ERROR UNDO Grava_valor, LEAVE Grava_valor:
        ...
        IF AVAIL crappre AND crawepr.cdfinemp = crappre.cdfinemp THEN
           DO:
               ...
               IF AVAIL crapcpa THEN
                  DO:
                      ASSIGN par_vllimapv = crapcpa.vlcalpre
                             par_flgcmtlc = FALSE.
        ...
        FIND crapfin WHERE crapfin.cdcooper = crawepr.cdcooper AND
                           crapfin.cdfinemp = crawepr.cdfinemp
                           NO-LOCK NO-ERROR.
        ...
        ELSE            
            DO:
        FIND craplcr WHERE craplcr.cdcooper = crawepr.cdcooper AND
                           craplcr.cdlcremp = crawepr.cdlcremp
                           NO-LOCK NO-ERROR.
        ...
        IF AVAIL craplcr AND craplcr.flgdisap THEN
           ...
           END.
        ELSE
        IF   par_flgcmtlc   THEN  /** Comite local de aprovacao **/
             DO:
                 IF   par_dsdopcao = "SVP" THEN
                 DO:
                
                 /** Novo valor maior que valor original da proposta **/
                         IF   par_vlemprst > par_vleprori AND NOT aux_flgporta THEN
                      ASSIGN crawepr.insitapr = 0
                             crawepr.cdopeapr = ""
                             crawepr.dtaprova = ?
                             crawepr.hraprova = 0.

                 IF   crawepr.insitapr <> 1 AND
                      crawepr.insitapr <> 3 THEN
                      DO:
                          FIND crapope WHERE
                          ...
                          IF   NOT AVAIL crapope   THEN
                               DO:
                                   ...
                                   ASSIGN tt-msg-confirma.inconfir = 1
                                      tt-msg-confirma.dsmensag =
                                       "067 - Operador nao cadastrado.".

                                   CREATE tt-msg-confirma.
                                   ASSIGN tt-msg-confirma.inconfir = 2
                                          tt-msg-confirma.dsmensag =
                                          "Essa proposta deve ser" +
                                          " aprovada pelo COMITE SEDE.".
                               END.
                          ELSE
                               DO:
                                   FIND crapage WHERE
                                   ...
                                   IF   NOT AVAIL crapage   THEN
                                       ...
                                       "Essa proposta deve ser aprovada pelo" +
                                       " COMITE SEDE.".
                                        END.
                                   ELSE
                                        DO:
                                            IF  crapage.vllimapv <
                                                (par_vlutiliz + par_vlemprst)
                                                THEN
                                                DO:
                                                    CREATE tt-msg-confirma.
                                                    ASSIGN
                      ...
                      DO:
                          ...
                          ASSIGN tt-msg-confirma.inconfir = 1
                                 tt-msg-confirma.dsmensag =
                                    ...
                                    "Essa proposta esta aprovada.".
                      END.
                 END.

             END.
        ELSE      /** Sem comite local de aprovacao **/
             DO:
                 ASSIGN crawepr.insitapr = 0
                        crawepr.cdopeapr = ""
                        crawepr.dtaprova = ?
                 ...
                 IF   par_vllimapv < par_vlemprst   THEN
                      DO:
                          FIND crapope WHERE
                               crapope.cdcooper = par_cdcooper  AND
                               crapope.cdoperad = par_cdoperad
                          ...
                          ELSE
                               DO:
                                   IF   crapope.vlapvcre <
                                        (par_vlutiliz + par_vlemprst)  THEN
                                        DO:

                                            CREATE tt-msg-confirma.
                                            ASSIGN tt-msg-confirma.inconfir = 1
                                                   tt-msg-confirma.dsmensag =
                                         "Essa proposta deve ser aprovada na " +
                                         "tela CMAPRV.".
    ...
    *********************************************************************/
    ...
    FIND FIRST craplcm 
         WHERE craplcm.cdcooper = par_cdcooper
           ...
           AND craplcm.nrdocmto = crapepr.nrctremp
           NO-LOCK NO-ERROR.

    IF AVAIL(craplcm) AND crapepr.dtmvtolt = par_dtmvtolt THEN
        aux_liquidia = 1.
    ...
    ASSIGN tt-dados-epr.nrdconta = par_nrdconta
           ...
           tt-dados-epr.dstpempr = "Cálculo Atual,Pré-Fixada"
