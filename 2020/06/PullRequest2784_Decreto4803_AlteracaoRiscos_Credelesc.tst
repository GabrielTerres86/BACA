...
BEGIN
  ...
  -- Ativa para cooperativa 8 o decreto
  UPDATE crappco SET dsconteu = '1' WHERE cdpartar = 80 AND cdcooper = 8;
  COMMIT;

  vr_incidente := 'RITM0081876';
...
/*
          dbms_output.put_line(rw_decreto.cdcooper || ',' ||
                               rw_decreto.nrdconta || ',' ||
                               rw_decreto.nrctremp || ',' ||
                               rw_decreto.dsnivris || ',' ||
                               risc0004.fn_traduz_risco(rw_decreto.inrisco_rating) || ',' ||
                               risc0004.fn_traduz_risco(vr_risco_atualizado)
                               );
*/
