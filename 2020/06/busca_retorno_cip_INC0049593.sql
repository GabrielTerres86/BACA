-- Baca para gerar movimentação de cobrança e envio de boletos 
-- para a CIP, dos boletos que deram erro EDDA0814 entre dia 11/05/2020 e 01/06/2020
...
DECLARE
  ...
  PROCEDURE pc_trata_retorno_erro(pr_idtabcob IN ROWID --> rowid crapcob
                                 ...
                                 ,pr_iddopejd IN VARCHAR2) IS
    --> Identificador da operadao da JD
    ---------------------------------------------------------------------------------------------------------------
    --
  ...
  BEGIN
    ...
    FOR rw_optiterr IN cr_optiterr LOOP
      vr_dscritic := rw_optiterr.nmcoluna || ': ' || rw_optiterr.codderro ||
                     ' - ' || rw_optiterr.dsdoerro;
    
      paga0001.pc_cria_log_cobranca(pr_idtabcob => pr_idtabcob
                                   ,pr_cdoperad => '1'
                                   ,pr_dtmvtolt => SYSDATE
                                   ,pr_dsmensag => TRIM(substr(vr_dscritic
                                                              ,1
                                                              ,350))
                                   ,pr_des_erro => vr_des_erro
                                   ,pr_dscritic => vr_dscritic);
...
BEGIN
  ...
  FOR rw_crapcob IN cr_crapcob LOOP
    BEGIN
      ...
      ELSIF rw_retnpc.cdstiope = 'RC' THEN
      ...
      BEGIN
        UPDATE crapcob cob
           SET cob.insitpro = nvl(vr_insitpro
                                 ,cob.insitpro)
              ,cob.flgcbdda = nvl(vr_flgcbdda
                                 ,cob.flgcbdda)
              ,cob.inenvcip = nvl(vr_inenvcip
                                 ,cob.inenvcip)
              ,cob.dhenvcip = nvl(vr_dhenvcip
                                 ,cob.dhenvcip)
              ,cob.inregcip = nvl(vr_inregcip
                                 ,cob.inregcip)
              ,cob.nrdident = nvl(rw_retnpc.idtitnpc
                                 ,cob.nrdident)
              ,cob.nratutit = nvl(rw_retnpc.nratutit
                                 ,cob.nratutit)
