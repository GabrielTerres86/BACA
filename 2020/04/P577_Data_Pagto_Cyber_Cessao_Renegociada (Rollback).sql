/*****************************************************************************************************************
21/04/2020 - (Script ROLLBACK) P577 - INC0043759 - Cessão Renegociada.
...
BEGIN
  ...
  FOR rw_emp_cessao_reneg IN cr_emp_cessao_reneg LOOP
    ...
    IF Nvl(vr_reneg_efetivada,0) = 1 THEN
      ...
      BEGIN
        UPDATE crapcyb  cyb 
        SET    cyb.dtdpagto  = rw_emp_cessao_reneg.dtvencto_original --(Script ROLLBACK)                            
        ...
        AND    cyb.dtdpagto <> rw_emp_cessao_reneg.dtvencto_original; --Se a data estiver diferente (Script ROLLBACK)             
      EXCEPTION
        WHEN OTHERS THEN
