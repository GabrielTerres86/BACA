CREATE OR REPLACE PROCEDURE CECRED.pc_crps660_wag(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Cooperativa solicitada
...
BEGIN
  /* .............................................................................
     ...
     Alteracoes: 02/12/2014 - Conversão Progress >> PLSQL (Petter-Supero e Andrino-RKAM)
                 ...
                 10/11/2019 - P437.3 - Migração de contratos Consignado TR>PP deve ser levado o novo numero de contrato
                              para o tipo de saída 0307 (Miguel Rodrigues - Supero)
  ............................................................................ */
  ...
  DECLARE
    ...
    BEGIN
      DECLARE
        ...
        vr_cdcritic   PLS_INTEGER;       --> Código da crítica
        vr_dscritic   VARCHAR2(4000);    --> Descricao da critica
      ...
      BEGIN
        ...
        IF pr_cdmodali = 0299 OR pr_cdmodali = 0499 THEN
          ...
          ELSE
            ...
            -- Testa migracao P437.2
            -- Verifica se o contrato atual foi migrado e se foi, retorna o numero do atual
            EMPR9999.pc_verifica_empr_migrado(pr_cdcooper => pr_cdcooper,
                                              pr_nrdconta => pr_nrdconta, 
                                              pr_nrctrnov => pr_nrctremp, 
                                              pr_tpempmgr => 1, -- 1 ele busca atraves do original para retornar o migrado 
                                              pr_nrctremp => vr_nrctrnov, 
                                              pr_cdcritic => vr_cdcritic, 
                                              pr_dscritic => vr_dscritic);
            IF vr_nrctrnov <> 0 THEN
      ...
      DECLARE
        ...
        CURSOR cr_crapris_mes_anterior(pr_cdcooper IN crapris.cdcooper%TYPE      --> Código da cooperativa
          ...
          WHERE cr.cdcooper = pr_cdcooper
            ...
            AND NOT  ((cdcooper = 1 and nrdconta = 7677251 and nrctremp = 1916441) or
                      (cdcooper = 1  and nrdconta = 7683693 and nrctremp = 2004938) or
                      (cdcooper = 1  and nrdconta = 8861579 and nrctremp = 1586929) or
                      (cdcooper = 1  and nrdconta = 10198229 and nrctremp = 1885115) or
                      (cdcooper = 16 and nrdconta = 427675 and nrctremp = 135633) or
                      (cdcooper = 16 and nrdconta = 524514 and nrctremp = 128774) or
                      (cdcooper = 16 and nrdconta = 571415 and nrctremp = 140121) or
                      (cdcooper = 14 and nrdconta = 61140 and nrctremp = 12753))
  ...
  BEGIN
   ...
   /* OPEN cr_craptab(pr_cdcooper);
    ...
    -- Atribui data (Wagner)
    vr_dtrefere := to_date('31/03/2020', 'DD/MM/RRRR');
      ...
      IF rw_crapris.inddocto IN (1,3,4,5) AND nvl(rw_crapris.cdinfadi,' ') <> '0301'  THEN
        ...
        ELSE
          ...
          /*IF rw_crapris.cdorigem = 3 THEN
            vr_vldivida_contrato := empr0021.fn_valor_divida_contrato(pr_cdcooper => pr_cdcooper
                                                                     ,pr_nrdconta => rw_crapris.nrdconta
                                                                     ,pr_nrctremp => rw_crapris.nrctremp
                                                                     ,pr_dtrefere => vr_dtrefere
                                                                     );
          ELSE
            vr_vldivida_contrato := (rw_crapris.vldivida - rw_crapris.vljura60); -- PRJ577
          END IF;*/
          vr_vldivida_contrato := (rw_crapris.vldivida - rw_crapris.vljura60 - rw_crapris.vljurantpp); -- PRJ577
            -- Fim -- PRJ577
        END IF;      
      END IF;
    ...
    AND to_char(vr_dtrefere, 'mm') IN ('03', '06', '09', '12') THEN
      ...
      FOR rw_crapris IN cr_crapris_BNDES(vr_dtrefere) LOOP
        ...
        IF rw_crapris.inddocto IN (1,3,4,5) AND nvl(rw_crapris.cdinfadi,' ') <> '0301'  THEN
          ...
          ELSE
            /*IF rw_crapris.cdorigem = 3 THEN
              vr_vldivida_contrato := empr0021.fn_valor_divida_contrato(pr_cdcooper => rw_crapris.cdcooper
                                                                       ,pr_nrdconta => rw_crapris.nrdconta
                                                                       ,pr_nrctremp => rw_crapris.nrctremp
                                                                       ,pr_dtrefere => vr_dtrefere
                                                                       );
