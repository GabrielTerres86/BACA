-- Created on 08/04/2020 by F0030248 - Rafael Cechet
-- Baca para gerar movimentação de cobrança e envio de boletos 
-- para a CIP, dos boletos que deram problema nos dias 03/04 e 06/04
  ...
  CURSOR cr_crapcob IS
    SELECT cob.nrdocmto, cob.cdcooper, cob.nrcnvcob, cob.rowid, cob.idtitleg, cob.idopeleg, decode(cob.cdtpinsc,1,'F','J') tppessoa, cob.nrinssac, cob.nrdconta
     ...
     WHERE cob.cdcooper >= 1
       AND cob.dtmvtolt = to_date('03/04/2020','DD/MM/RRRR')
       ...
       AND ret.dtocorre(+) = to_date('03/04/2020','DD/MM/RRRR')
       AND ret.cdocorre(+) = 2
    ...
    SELECT cob.nrdocmto, cob.cdcooper, cob.nrcnvcob, cob.rowid, cob.idtitleg, cob.idopeleg, decode(cob.cdtpinsc,1,'F','J') tppessoa, cob.nrinssac, cob.nrdconta
      FROM crapcob cob, crapret ret
     WHERE cob.cdcooper >= 1
       AND cob.dtmvtolt = to_date('06/04/2020','DD/MM/RRRR')
       AND cob.idlottck IS NOT NULL
       AND cob.incobran = 0
       AND cob.inenvcip = 2
       AND ret.cdcooper(+) = cob.cdcooper
       ...
       AND ret.nrcnvcob(+) = cob.nrcnvcob
       AND ret.nrdocmto(+) = cob.nrdocmto
       AND ret.dtocorre(+) = to_date('06/04/2020','DD/MM/RRRR')
       AND ret.cdocorre(+) = 2
       AND ret.nrdocmto IS NULL;
