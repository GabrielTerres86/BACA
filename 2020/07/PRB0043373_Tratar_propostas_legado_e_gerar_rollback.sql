    ...
    SELECT w.nrdconta
     ...
     WHERE w.cdcooper = t.cdcooper
       ...
       AND EXISTS (
           SELECT 1
             FROM crawcrd win
            WHERE win.cdcooper = w.cdcooper
              AND win.nrdconta = w.nrdconta
              AND win.nrcpftit = w.nrcpftit
              AND win.cdadmcrd = w.cdadmcrd
              AND win.insitcrd = 6
       )
