declare
--CCR30201343401740309201975632650260870005127070233583533CHARLES RODRIGO        14121985201020104710094900    000200                                                                                               000BD32ECBEF1B098A750068520       SSPSCCHARLES RODRIGO DA SILVA                                                        7563265000000651567N
  ...
  CURSOR cr_crawcrd_ant (pr_cdcooper  IN crawcrd.cdcooper%TYPE
    ...
    SELECT crd.dddebito
     ...
     WHERE crd.cdcooper = pr_cdcooper
       ...
       AND crd.insitcrd = 4; -- situação Em Uso
...
begin
  BEGIN
    vr_des_text := 'CCR30201343401740309201975632650260870005127070233583533CHARLES RODRIGO        14121985201020104710094900    000200                                                                                               000BD32ECBEF1B098A750068520       SSPSCCHARLES RODRIGO DA SILVA                                                        7563265000000651567N';
    ...
    IF substr(vr_des_text,5,2) = '02'  THEN
      ...
      vr_cdadmcrd := 12; -- (nao vem no arquivo, foi identificado no SIPAG)
      vr_dddebito := 0;
      vr_vllimcrd := 0;
      vr_tpdpagto := 0;
      vr_flgdebcc := 0;
      vr_cdgraupr := 0;
      vr_dtlibera := NULL;
      vr_dtcancel := '06/11/2019';
      vr_nrcpfcpj := 4710094900;
      --
      -- Como é um cartão adicional, buscar os dados do cartão do titular.
      ...
      vr_dddebito:= 3;--rw_crawcrd_ant.dddebito;
      vr_vllimcrd:= 600; -- Informação está no sipag
  ...
  commit;
  dbms_output.put_line('Sucesso ao inserir registro de cartão' );
