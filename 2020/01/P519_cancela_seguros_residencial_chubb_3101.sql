...
CURSOR cr_crapseg IS  
	...
	FROM crapseg seg
     ...
     WHERE seg.cdcooper = 1 -- 1-VIACREDI 
       AND seg.cdsegura  = 5011
       AND seg.tpseguro  = 11
       AND seg.cdsitseg  = 2
	   AND seg.dtcancel = trunc(sysdate)
	   AND seg.dtfimvig > trunc(sysdate) -- desconsiderar os seguros renovados e que serão alterados para 'Vencido' no final do mes(crps441)  
	   AND seg.cdmotcan = 1 
	   AND seg.cdopecnl = '1'
...
BEGIN
  ...
  UPDATE crapseg
     SET crapseg.cdsitseg = 2
        , crapseg.dtcancel = trunc(sysdate) 
   ...
   WHERE crapseg.cdcooper = 1 -- 1-VIACREDI
     AND crapseg.cdsitseg IN (1,3) -- novo, renovado  
     AND crapseg.tpseguro = 11     -- CASA 
     AND crapseg.cdsegura = 5011  -- CHUBB  
	 AND crapseg.dtfimvig > trunc(sysdate); -- para desconsiderar os seguros renovados no mes que passarao para vencido no processo mnsal(crps441)
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_dslobdev, '/micros/cecred/diego/', 'HDI_cancelados_31012020' || '.txt', NLS_CHARSET_ID('UTF8'));    
