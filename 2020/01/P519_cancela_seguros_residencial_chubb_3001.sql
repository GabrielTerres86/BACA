...
CURSOR cr_crapseg IS  
	...
	FROM crapseg seg
     ...
     WHERE seg.cdcooper = 1 -- 1-VIACREDI 
	   ...
	   AND seg.dtfimvig = '31/01/2020' -- desconsiderar os seguros renovados e que serão alterados para 'Vencido' no final do mes(crps441)  
	   AND seg.cdmotcan = 1 
	   AND seg.cdopecnl = '1'
...
BEGIN
  ...
  UPDATE crapseg
   ...
   WHERE crapseg.cdcooper = 1 -- 1-VIACREDI
	 ...
	 AND crapseg.dtfimvig = '31/01/2020'; 
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_dslobdev, '/micros/cecred/diego/', 'HDI_cancelados_30012020' || '.txt', NLS_CHARSET_ID('UTF8'));    
