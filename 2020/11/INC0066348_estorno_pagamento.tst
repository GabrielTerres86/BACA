...
686
-- Script para estornar pagamento de emprestimo
declare 

  vr_cdcooper  NUMBER := 13;
  vr_cdagenci  NUMBER := 1;
  vr_nrdconta  NUMBER := 140082;
  vr_nrctremp  NUMBER := 20381;
  vr_dtmvtolt_ext DATE := to_date('30/10/2020','DD/MM/RRRR');
...
BEGIN
  ...
  IF vr_des_reto = 'NOK' THEN
    ...
    ELSE
      vr_cdcritic := 0;
      vr_cdcritic := 'Não foi possivel executar estorno do Pagamento do Prejuízo.';
    END IF;
    RAISE vr_exc_erro;
  END IF;  
