declare
  ...
  CURSOR cr_crapcop IS
    SELECT cop.cdcooper
     ...
     WHERE cop.cdcooper = dat.cdcooper
       AND cop.cdcooper in (1,6,7,9,11,12,13)
       AND cop.flgativo = 1
     ;
  --
  ...
  CURSOR cr_parcelas(pr_cdcooper crappep.cdcooper%TYPE
    ...
    SELECT epr.cdcooper
     ...
     WHERE epr.cdcooper = wpr.cdcooper
       ...
       --
       --AND epr.cdcooper = 1
       --AND epr.nrdconta = 10974172
       --AND epr.nrctremp = 3074870
       --
       AND epr.tpemprst = 2
       AND epr.inliquid = 0
       -- Somente estas contas
       and (epr.cdcooper,epr.nrdconta,epr.nrctremp)  in ((1, 6286984, 1226556)
                                                        ,(1, 7211325, 1416895)
                                                        ,(1, 7259697, 2311468)
                                                        ,(1, 8325545, 2882434)
                                                        ,(1, 8387087, 1663772)
       ...
       AND EXISTS (   SELECT 1
                       ...
                       WHERE pepX.cdcooper = eprX.cdcooper
                         ...
                         AND lemX.cdhistor IN (2330,2331,2332,2333,2334,2335,2338,2339)
                         AND lemX.nrparepr = pepX.nrparepr
                         AND lemX.dtpagemp < gene0005.fn_valida_dia_util(pr_cdcooper => pepX.cdcooper
                                                              ,pr_dtmvtolt => pepX.dtvencto
                                                              ,pr_tipo     => 'P')
  ...
  --
  CURSOR cr_contratos_ignorar(pr_dtmvtolt crapdat.dtmvtolt%TYPE) IS
    -- Ignora contratos com divergência no pagamento (crapep x craplem)
    SELECT DISTINCT cdcooper
      ...
      FROM( SELECT pep.cdcooper
             ...
             WHERE pep.cdcooper = epr.cdcooper
               ...
               AND pep.inliquid = 1
               AND lem.dtmvtolt >= '01/07/2020'
               AND lem.cdhistor IN(2330, 2331, 2334, 2335, 2338, 2339)
               AND epr.inliquid = 0
               AND epr.tpemprst = 2
               --
               --AND epr.cdcooper = 1
     ...
     WHERE vlpagpar <> vllanmto
     UNION ALL
    -- Ignora contratos que o valor de ajuste aumentou com o recalculo de parcelas
    SELECT epr.cdcooper
          ,epr.nrdconta
          ,epr.nrctremp
          ,epr.rowid
      FROM crapepr epr
     WHERE (epr.cdcooper, epr.nrdconta, epr.nrctremp) IN((1, 9849491, 2721148)
                                                        ,(2, 239631, 264964)
                                                        ,(7, 1953, 25271)
                                                        ,(7, 143669, 35701)
                                                        ,(7, 276898, 36966)
       ...
       ;
  --
  rw_contratos_ignorar cr_contratos_ignorar%ROWTYPE;
  ...
  --
  TYPE typ_ignorar IS RECORD(cdcooper  crappep.cdcooper%TYPE
                            ,nrdconta  crappep.nrdconta%TYPE
                            ,nrctremp  crappep.nrctremp%TYPE
                            ,ds_rowid  VARCHAR2(100)
                            );
  --
  TYPE typ_tab_ignorar IS TABLE OF typ_ignorar INDEX BY VARCHAR2(100);
  vr_tab_ignorar typ_tab_ignorar;
  vr_indice_ignorar VARCHAR(100);
  -- CRPS723
  ...
  BEGIN
    ...
    WHILE vr_indice_registro IS NOT NULL LOOP
      -- Só atualiza os contratos liberados até a data de processamento
    ...
    BEGIN
      ...
      FOR idx IN 1..pr_tab_parc_pep.count LOOP
        ...
        IF vr_vlparepr <> pr_tab_parc_pep(idx).vlparepr OR vr_vlsdvpar <> vr_vlsdvpar_novo THEN
          -- Ajusta a data de vencimento para um dia útil
  ...
  BEGIN
    ...
    WHILE vr_indice_registro IS NOT NULL LOOP
      ...
      IF vr_tab_registro(vr_indice_registro).dtlibera <= pr_dtmvtolt THEN
        --
        BEGIN
          -- O script faz o processamento retroativo, simulando como se estivesse na data de vencimento (dia útil) da parcela
          -- sendo necessário pegar também o dia útil anterior para buscar a taxa
...
BEGIN
  ...
  LOOP
    ...
    --
    OPEN cr_contratos_ignorar(vr_data_atual);
    --
    LOOP
      --
      FETCH cr_contratos_ignorar INTO rw_contratos_ignorar;
      EXIT WHEN cr_contratos_ignorar%NOTFOUND;
      --
      IF NOT vr_tab_ignorar.exists(rw_contratos_ignorar.ds_rowid) THEN
        --
        vr_tab_ignorar(rw_contratos_ignorar.ds_rowid).ds_rowid  := rw_contratos_ignorar.ds_rowid;
        vr_tab_ignorar(rw_contratos_ignorar.ds_rowid).cdcooper  := rw_contratos_ignorar.cdcooper;
        vr_tab_ignorar(rw_contratos_ignorar.ds_rowid).nrdconta  := rw_contratos_ignorar.nrdconta;
        vr_tab_ignorar(rw_contratos_ignorar.ds_rowid).nrctremp  := rw_contratos_ignorar.nrctremp;
        --
      END IF;
      --
    END LOOP;
    --
    CLOSE cr_contratos_ignorar;
    --
    ...
    LOOP
      ...
      --
      IF NOT vr_tab_registro.exists(rw_parcelas.rowid) AND
         NOT vr_tab_ignorar.exists(rw_parcelas.rowid) THEN
        --
        recp0001.pc_verifica_acordo_ativo(pr_cdcooper => rw_parcelas.cdcooper
                                         ,pr_nrdconta => rw_parcelas.nrdconta
                                         ,pr_nrctremp => rw_parcelas.nrctremp
                                         ,pr_cdorigem => 0
                                         ,pr_flgativo => vr_flgativo
                                         ,pr_cdcritic => vr_cdcritic
                                         ,pr_dscritic => vr_dscritic
                                         );
        -- Ignora contratos em acordo
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_log, vr_dsdireto, 'log_erros_antecipada.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crapepr_prox, vr_dsdireto, 'crapepr_prox_parc_antecipada.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crappep_prox, vr_dsdireto, 'crappep_prox_parc_antecipada.csv', NLS_CHARSET_ID('UTF8'));
