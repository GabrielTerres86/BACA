  ...
  CURSOR cr_parcelas(pr_cdcooper crappep.cdcooper%TYPE
    ...
    SELECT epr.cdcooper
     ...
     WHERE epr.cdcooper = wpr.cdcooper
       ...
       AND epr.tpemprst = 2
       AND epr.inliquid = 0
       AND (epr.cdcooper, epr.nrdconta, epr.nrctremp) IN((1, 3522970, 2358233)
                                                        ,(1, 6762395, 2407042)
                                                        ,(1, 7143559, 2443166)
                                                        ,(1, 8136912, 2334943)
                                                        ,(1, 9002650, 2385210)
...
BEGIN
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_log, vr_dsdireto, 'log_erros_pagto.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crapepr_prox, vr_dsdireto, 'crapepr_prox_parc_pagto.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crappep_prox, vr_dsdireto, 'crappep_prox_parc_pagto.csv', NLS_CHARSET_ID('UTF8'));
