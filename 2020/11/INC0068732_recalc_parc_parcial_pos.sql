DECLARE
  ...
  CURSOR cr_parcelas(pr_cdcooper crappep.cdcooper%TYPE
    ...
    SELECT epr.cdcooper
     ...
     WHERE epr.cdcooper = wpr.cdcooper
       ...
       AND epr.tpemprst = 2
       AND epr.inliquid = 0
       AND EXISTS( SELECT DISTINCT epr2.cdcooper
                                  ,epr2.nrdconta
                                  ,epr2.nrctremp
                              FROM crappep pep2
                                  ,crapepr epr2
                             WHERE pep2.cdcooper = epr2.cdcooper
                               AND pep2.nrdconta = epr2.nrdconta
                               AND pep2.nrctremp = epr2.nrctremp
                               AND epr2.cdcooper = epr.cdcooper
  ...
  CURSOR cr_contratos_ignorar(pr_dtmvtolt crapdat.dtmvtolt%TYPE) IS
    ...
    SELECT DISTINCT cdcooper
     ...
     UNION ALL
    -- Ignora contratos com antecipação de pagamentos
    SELECT pep.cdcooper
          ,pep.nrdconta
          ,pep.nrctremp
          ,epr.rowid ds_rowid
...
BEGIN
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_log, vr_dsdireto, 'log_erros_parcial.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crapepr_prox, vr_dsdireto, 'crapepr_prox_parc_parcial.csv', NLS_CHARSET_ID('UTF8'));
  ...
  vr_dsdireto := SISTEMA.obternomedirectory(gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cecred/james');
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_crappep_prox, vr_dsdireto, 'crappep_prox_parc_parcial.csv', NLS_CHARSET_ID('UTF8'));
