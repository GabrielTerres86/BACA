declare 
  ...
  vr_nmdireto        VARCHAR2(4000) := vr_rootmicros||'cpd/bacas/INC0064982';
  vr_nmarqimp        VARCHAR2(100)  := 'rollback_INC0064982.txt';
  ...
  CURSOR cr_contas IS
     ...
     FROM (
          SELECT ass.cdcooper cooperativa
           ...
           WHERE ass.nrdconta IN (1835793, 2036878, 165522, 2005000, 184373, 276448,
                                  584592, 2534380, 2689189, 447897, 2886278, 37958, 85880,
                                  167746, 349615, 6556078, 186350, 323713, 895784, 222127,
                                  245011, 343242, 383732, 408590, 448206, 585904, 324825, 504270)
             AND ass.cdcooper = 16
