...
CURSOR cr_crapseg IS  
	...
	FROM crapseg seg
     ...
     WHERE seg.cdcooper = 1 /* Ajustar - conforme cada migracao */ 
	  ...
	  -- AND seg.cdmotcan = 1 
	   AND seg.cdopecnl = '1'
	   AND wseg.cdcooper = seg.cdcooper
	   AND wseg.nrdconta = seg.nrdconta
	   AND wseg.nrctrseg = seg.nrctrseg
	   AND wseg.tpseguro = seg.tpseguro
...
BEGIN
  ...
  DBMS_XSLPROCESSOR.CLOB2FILE(vr_dslobdev, '/micros/cecred/diego/', 'ICATU_cancelados_31032020' || '.txt', NLS_CHARSET_ID('UTF8'));    
