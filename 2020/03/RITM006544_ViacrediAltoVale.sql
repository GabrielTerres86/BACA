--RITM006544 - Miguel - 13/03/2020 -- Viacredi Alto Vale
DECLARE 
 
  vr_arq_path  VARCHAR2(1000):= gene0001.fn_diretorio(pr_tpdireto => 'M', 
                                                      pr_cdcooper => 0, 
                                                      pr_nmsubdir => 'cpd/bacas/ritm006544/viacredialtovale'); 


  vr_nmarquiv  VARCHAR2(100) := 'ViacrediAltoVale_ITG.txt';
  vr_nmarqbkp  VARCHAR2(100) := 'ROLLBACK_ViacrediAltoVale_ITG.txt';
  ...
  CURSOR cr_crapass (pr_nrdconta crapass.nrdconta%TYPE) IS
    ...
    FROM   CRAPASS 
    WHERE  cdcooper = 16  -- Viacredi Alto Vale
