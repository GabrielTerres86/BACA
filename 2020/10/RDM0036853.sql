DECLARE
  ...
  PROCEDURE PC_ATUALIZA_REGISTRO(PR_DSDIRETO IN VARCHAR2 -- Diretorio onde vai gravar o backup
    ...
    NOVO_nmmaeptl VARCHAR2(60);
    NOVO_nmpaiptl VARCHAR2(58);
    CN_REGEXSTR CONSTANT VARCHAR2(200) := '[ÀÁÂÃÄÅàáâãäåÒÓÔÕÖØòóôõöÈÉÊËèéêëÇçÌÍÎÏìíîïÙÚÛÜùúûüÿÑñ\.\,\/\:\(\)\;\=\+\°\º\ª\*\_\>\<\$\#\"\!\@\%\{\}\?\&\-]';
  
    CURSOR CR_nmmaeptl IS
      SELECT C.*
        FROM CRAPASS C
       WHERE REGEXP_LIKE( nmmaeptl , CN_REGEXSTR);

  
    CURSOR CR_nmpaiptl IS
      SELECT C.*
        FROM CRAPASS C
       WHERE REGEXP_LIKE( nmpaiptl, CN_REGEXSTR);
  ...
  BEGIN
    ...
    FOR RW_nmmaeptl IN CR_nmmaeptl LOOP
    
           
      GENE0001.PC_ESCR_LINHA_ARQUIVO(VR_IND_ARQLOG,'UPDATE CRAPASS ' || 'SET CRAPASS.nmmaeptl = q''[' ||
                   RW_nmmaeptl.nmmaeptl || ']'' ' ||
                   'WHERE CRAPASS.PROGRESS_RECID = ''' ||
                   RW_nmmaeptl.PROGRESS_RECID || ''';' || CHR(10));           
      ...
      BEGIN
        ...
        -- Remove caracteres especiais do campo RW_nmmaeptl.nmmaeptl
        NOVO_nmmaeptl := RW_nmmaeptl.nmmaeptl;
        NOVO_nmmaeptl := REGEXP_REPLACE(NOVO_nmmaeptl, 'Ã§{1,}', 'ç');
        NOVO_nmmaeptl := REGEXP_REPLACE(NOVO_nmmaeptl, CN_REGEXSTR, '');
        NOVO_nmmaeptl := TRIM(NOVO_nmmaeptl);
      
        IF NOVO_nmmaeptl = '' THEN
          NOVO_nmmaeptl := '-';
        END IF;
      
        UPDATE CRAPASS
           SET CRAPASS.nmmaeptl = NOVO_nmmaeptl -- Saida somente com caracteres validos
         WHERE CRAPASS.PROGRESS_RECID = RW_nmmaeptl.PROGRESS_RECID;
      
      EXCEPTION
        WHEN OTHERS THEN
          PR_DSCRITIC := 'Erro ao atualizar o registro de endereço ( ' ||
                         RW_nmmaeptl.PROGRESS_RECID || ' ):' || SQLERRM;
          RAISE VR_EXC_ERRO;
      END;
    ...
    -- Faz o loop dos endereços com caracteres invalidos
    FOR RW_nmpaiptl IN CR_nmpaiptl LOOP
    

      GENE0001.PC_ESCR_LINHA_ARQUIVO(VR_IND_ARQLOG,'UPDATE CRAPASS ' || 'SET CRAPASS.nmpaiptl = q''[' ||
                  RW_nmpaiptl.nmpaiptl || ']'' ' ||
                  'WHERE CRAPASS.PROGRESS_RECID = ''' ||
                  RW_nmpaiptl.PROGRESS_RECID || ''';' || CHR(10));

      BEGIN
      
        -- Remove caracteres especiais do campo RW_nmpaiptl.nmpaiptl
        NOVO_nmpaiptl := RW_nmpaiptl.nmpaiptl;
        NOVO_nmpaiptl := REGEXP_REPLACE(NOVO_nmpaiptl, 'Ã§{1,}', 'ç');
        NOVO_nmpaiptl := REGEXP_REPLACE(NOVO_nmpaiptl, CN_REGEXSTR, '');
        NOVO_nmpaiptl := REGEXP_REPLACE(NOVO_nmpaiptl, ' {2,}', ' ');
        NOVO_nmpaiptl := TRIM(NOVO_nmpaiptl);
      
        IF NOVO_nmpaiptl = '' THEN
          NOVO_nmpaiptl := '-';
        END IF;
      
        UPDATE CRAPASS
           SET CRAPASS.nmpaiptl = NOVO_nmpaiptl -- Saida somente com caracteres validos
         WHERE CRAPASS.PROGRESS_RECID = RW_nmpaiptl.PROGRESS_RECID;
      
      EXCEPTION
        WHEN OTHERS THEN
          PR_DSCRITIC := 'Erro ao atualizar o registro de endereço ( ' ||
                         RW_nmpaiptl.PROGRESS_RECID || ' ):' || SQLERRM;
          RAISE VR_EXC_ERRO;
      END;
