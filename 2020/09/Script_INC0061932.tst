...
-- 2408 - Quando LANÇA, Diminui o Saldo Prejuizo      [C] 2408 - TRF. PREJUIZO-> TBCC_PREJUIZO_DETALHE
  ...
  BEGIN
    ...
    IF (prm_cdcooper = 1 AND prm_nrdconta in ( 9519297,9460624)) THEN
...
---------------------------------------------------
  -- INC0061932 - Viacredi: 1 - Conta: 9519297
  -- => Solução: Exluir um 2408 no valor de R$ 42,37.
  vr_incidente := 'INC0061932';
  vr_cdcooper  := 1;
  vr_nrdconta  := 9519297;
  vr_vllanmto  := 42.37;
  vr_cdhistor  := 2408;
...
---------------------------------------------------
  -- INC0062542 - Viacredi: 1 - Conta: 9460624 
  -- => Solução: Exluir 2408 no valor de R$ 0,24.
  vr_incidente := 'INC0062542';
  vr_cdcooper  := 1;
  vr_nrdconta  := 9460624;
  vr_vllanmto  := 28.34;
  vr_cdhistor  := 2408;
...
---------------------------------------------------
  ...
  dbms_output.put_line('-------- Atualizar PRM HISTOR_PREJ_N_SALDO --------');
  dbms_output.put_line(' ');
  
  update crapprm x
     set x.dsvlrprm = x.dsvlrprm || ';3207'
   WHERE x.cdacesso = 'HISTOR_PREJ_N_SALDO';
