<?php 
	...
	/************************************************************************
	 Fonte: resgate_cancelamento.php                                  
	 Autor: David                                                     
	 Data : Outubro/2009              		  Última alteração: 03/03/2013
	                                                                  
	 Objetivo  : Mostrar opcao CANCELAMENTO da rotina de Aplicações   
	 			 da tela ATENDA                            
	 ...
	 Alterações: 01/12/2010 - Alterado a chamada da BO b1wgen0004.p 
				 ...
				 03/03/2015 - Inclusão de procedures via OCI para projeto
	...
	************************************************************************/
	...
	if($tpaplica == 'A'){
		...
		$xmlGetResgates .= "		<flgcance>yes</flgcance>";
		$xmlGetResgates .= "	</Dados>";
		$xmlGetResgates .= "</Root>";
		...
		if (strtoupper($xmlObjResgates->roottag->tags[0]->name) == "ERRO") {
			...
			$msgErro = $xmlObjResgates->roottag->tags[0]->cdata;
			if($msgErro == null || $msgErro == ''){
				$msgErro = $xmlObjResgates->roottag->tags[0]->tags[0]->tags[4]->cdata;
			}			
			exibeErro($msgErro);		
		} 
		$resgates   = $xmlObjResgates->roottag->tags;	
		$qtResgates = count($resgates);
	}	
...
<div class="divRegistros">	
	<table>
		...
		<tbody>
			...
			?>
			<tr id="trResgate<?php echo $i + 1; ?>" onClick="selecionaResgate(<?php echo $i + 1; ?>,<?php echo $qtResgates; ?>,'<?php echo $resgates[$i]->tags[0]->cdata; ?>','<?php echo $resgates[$i]->tags[1]->cdata; ?>');">
...
<?php if ($qtResgates > 0) { ?>
idLinhaResg = 0;
selecionaResgate(1,<?php echo $qtResgates; ?>,'<?php echo $resgates[0]->tags[0]->cdata; ?>','<?php echo $resgates[0]->tags[1]->cdata; ?>');
<?php } ?>
