...
$tipos  = $xmlObjeto->roottag->tags[0]->attributes['DSTIPSEG'];
$tipos  = explode(';',$tipos);
if(is_array($tipos) && $tipos){
	foreach($tipos as $tipo){
		echo '<option value="'.getValue($tipo).'">'.$tipo.'</option>';
	}
}
function getValue($tipo){
	switch(strtoupper($tipo)){
		case 'CASA'			: return 11; break;
		case 'VIDA'			: return 3;  break;
		case 'PRESTAMISTA'  : return 4;  break;
	}
	return null;
}
