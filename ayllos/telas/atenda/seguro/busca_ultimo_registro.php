<? 
/*!
 * FONTE        : buscar_ultimo_registro.php
 * CRIAÇÃO      : Michel M Candido
 * DATA CRIAÇÃO : 18/09/2011
 * OBJETIVO     : retorna  o ultimo registro
 */
session_start();
...
$xml .= "		<Proc>buscar_proposta_seguro_r</Proc>";
$xml .= "	</Cabecalho>";
$xml .= "	<Dados>";
...
$tpseguro  = getByTagName($ultimo->tags,'tpseguro');
$tpplaseg  = getByTagName($ultimo->tags,'tpplaseg');
$cdsegura  = getByTagName($ultimo->tags,'cdsegura');

echo "
		$('#nrctrseg').val('{$nrctrseg}');
		$('#tpseguro').val('{$tpseguro}');
		$('#cdsegura').val('{$cdsegura}');
	";
