<?php 
	...
	/************************************************************************  
	  Fonte: imprimir_pdf_dsctit.php                                               
	  Autor: Guilherme                                                          
	  Data : Março/2008                         Última Alteração: 19/08/2015
	                                                                            
	  Objetivo  : Gerar PDF das impressoes de desconto de títulos              
	                                                                            	 
	  Alterações: 11/06/2010 - Adaptar para novo RATING (David).     
				  19/03/2012 - Adicionada área para uso da Digitalização (Lucas).
				  08/10/2013 - Alteração da sigla PAC para PA. (Carlos)
	...
	class PDF extends FPDF {
		...
		function geraImpresso($dadosImpressos,$idImpresso) {		
			if ($idImpresso == "1") {
				...
				$this->geraTitulosBorder($dadosImpressos["CABECTIT"],$dadosImpressos["TITSBORD"],$dadosImpressos["TBRESTRI"],$dadosImpressos["SACADNPG"]);
			} elseif ($idImpresso == "6") {
				$this->geraPropostaBordero($dadosImpressos["PROPBORD"],$dadosImpressos["EMPRESTI"]);
			} elseif ($idImpresso == "7"){
				$this->geraTitulosBorder($dadosImpressos["CABECTIT"],$dadosImpressos["TITSBORD"],$dadosImpressos["TBRESTRI"],$dadosImpressos["SACADNPG"]);
		...
		function geraPropostaLimite($dadosProposta,$Emprestimos,$dadosRating) {
			...
			$this->Cell(18,0.4,"PROPOSTA DE LIMITE DE DESCONTO DE TÍTULOS",0,1,"L",0,"");
			...
			$this->Cell(18,0.4,"Desconto de Cheques: ".number_format(str_replace(",",".",$dadosProposta["VLLIMCHQ"]),2,",","."),0,1,"L",0,"");
			...
			$this->Cell(6,0.4,"Valor Médio dos Títulos",0,1,"R",0,"");
			...
			$this->Cell(6,0.4,number_format(str_replace(",",".",$dadosProposta["VLMEDTIT"]),2,",","."),0,1,"R",0,"");			
			...
			$this->Cell(1,0.4,"",0,0,"L",0,"");
			$this->Cell(8.5,0.4,$dadosProposta["NMCIDADE"].", ".$dadosProposta["DDMVTOLT"]." de ".$dadosProposta["DSMESREF"]." de ".$dadosProposta["AAMVTOLT"].".",0,1,"C",0,"");							
		...
		function geraContratoLimite($dadosContrato,$dadosAvalistas) {
			...
			$this->Cell(18,0.4,"CONTRATO DE DESCONTO DE TÍTULOS",0,1,"C",0,"");			
			...
			$this->Cell(18,0.4,"3. DA LINHA DE DESCONTO DE TÍTULOS: ",0,1,"L",0,"");
			$this->Ln();
			$this->SetFont("Courier","",10);
			...
			$this->Ln();
			$this->SetFont("Courier","",10);
			$this->MultiCell(18,0.4,"Declaram as partes, abaixo assinadas, que a presente Folha de Rosto é parte integrante das CONDIÇÕES GERAIS do Contrato de Desconto de Títulos cujas condições aceitam, outorgam e prometem cumprir.",0,"J",0);																		
		...
		function geraNPLimite($dadosAvalistas,$dadosNP) {
			...
			$this->SetFont("Courier","B",10);
			$this->Cell(18,0.4,"CONTRATO DE DESCONTO DE TÍTULOS",0,1,"L",0,"");
			$this->SetFont("Courier","",10);
		...
		function geraPropostaBordero($dadosProposta,$Emprestimos) {
			...
			$this->Ln();
			$this->SetFont("Courier","",11);
			$this->Cell(10,0.4,"PROPOSTA DE DESCONTO DE TÍTULOS",0,0,"L",0,"");
			...
			$this->Cell(0.8,0.4,"",0,0,"R",0,"");		
			$this->Cell(0.8,0.4,formataNumericos("zz9",$dadosProposta["CDAGENCI"],"."),0,1,"R",0,"");			
			...
			$this->Cell(9,0.4,"Desconto de Títulos: ".number_format(str_replace(",",".",$dadosProposta["VLLIMPRO"]),2,",","."),0,1,"L",0,"");
			
			$this->Ln();
			
			$this->SetFont("Courier","B",10);
			$this->Cell(18,0.4,"CARTEIRA DE COBRANÇA:",0,1,"L",0,"");
			$this->SetFont("Courier","",10);
			$this->Ln();
			
			$this->Cell(18,0.4,"Quantidade de Boletos da Carteira de cobrança: ".$dadosProposta["QTDBOLET"],0,1,"L",0,"");
			$this->Cell(18,0.4,"Valor médio da Carteira de cobrança: ".number_format(str_replace(",",".",$dadosProposta["VLMEDBOL"]),2,",","."),0,1,"L",0,"");
			
			$this->Ln();
			
			$this->Cell(18,0.4,"Valor médio por título descontado: ".number_format(str_replace(",",".",$dadosProposta["VLMEDDSC"]),2,",","."),0,1,"L",0,"");
			$this->Cell(18,0.4,"Quantidade de títulos não pago por pagador: ".$dadosProposta["QTTITSAC"].($dadosProposta["NRMESPSQ"] > 0 ? "  (últimos " : "  (último ").$dadosProposta["NRMESPSQ"].($dadosProposta["NRMESPSQ"] > 0 ? " meses)" : "mês)"),0,1,"L",0,"");
			$this->Cell(18,0.4,"Percentual de títulos não pago por beneficiario: ".$dadosProposta["PERCEDEN"],0,1,"L",0,"");
			...
			$this->SetFont("Courier","",10);
			$this->Cell(13,0.4,number_format(str_replace(",",".",$dadosProposta["VLTOTTIT"]),2,",","."). " (".formataNumericos("zzz.zz9",$dadosProposta["QTTOTTIT"],"."). ($dadosProposta["QTTOTTIT"] > 0 ? " Títulos)" : " Título)"),0,1,"L",0,"");
			...
			if (count($Emprestimos) == 0) {
				...
				for ($i = 0;$i < count($Emprestimos); $i++){
					...
					if ($i == 0){
						...
						$this->Cell(13,0.4,number_format(str_replace(",",".",$dadosProposta["VLSLDDSC"]),2,",",".")."           ".formataNumericos("zz.zz9",$dadosProposta["QTDSCSLD"],".")." Títulos",0,1,"L",0,"");						
						$this->Cell(18,0.4,"Maior Título: ".number_format(str_replace(",",".",$dadosProposta["VLMAXDSC"]),2,",","."),0,1,"L",0,"");
						$this->Cell(18,0.4,"Valor Médio: ".number_format(str_replace(",",".",$dadosProposta["VALORMED"]),2,",","."),0,1,"L",0,"");
						...
						$this->Cell(2,0.4,"Contrato",0,0,"R",0,"");
						$this->Cell(3,0.4,"Saldo Devedor",0,0,"R",0,"");
						$this->Cell(4.3,0.4,"Prestacoes",0,0,"L",0,"");
		...
		function geraTitulosBorder($dadosCabecalho,$dadosTitulos,$dadosRestricoes,$dadosSacadNPG) {
			$this->nrLinhas = 12;
			...
			$this->Cell(1,0.4,"".$dadosCabecalho["CDAGENCI"],0,1,"L",0,"");
			...
			$this->Cell(18,0.4,"TÍTULOS ENTREGUES PARA DESCONTO:",0,1,"L",0,"");			

			$this->Ln();
			$this->SetFont("Courier","",7);
			
			for ($i = 0;$i < count($dadosTitulos); $i++){
				if ($i == 0){
					$this->Cell(1.7,0.4,"Vencimento","B",0,"L",0,"");
					$this->Cell(3,0.4,"Nosso Número","B",0,"L",0,"");
					$this->Cell(2,0.4,"Valor","B",0,"R",0,"");
					$this->Cell(2,0.4,"Vlr Líquido","B",0,"R",0,"");
					$this->Cell(0.7,0.4,"Prz","B",0,"L",0,"");
					$this->Cell(5.6,0.4,"Pagador","B",0,"L",0,"");
					$this->Cell(3,0.4,"CPF/CNPJ","B",1,"L",0,"");
				}
				$vlrtotal += doubleval(str_replace(",",".",$dadosTitulos[$i]["VLTITULO"]));
				$qttotal += 1;
				$vltotliq += doubleval(str_replace(",",".",$dadosTitulos[$i]["VLLIQUID"]));				
				$this->controlaLinhas(1);
				$this->Cell(1.7,0.4,$dadosTitulos[$i]["DTVENCTO"],0,0,"L",0,"");
				$this->Cell(3,0.4,$dadosTitulos[$i]["NOSSONUM"],0,0,"L",0,"");
				$this->Cell(2,0.4,number_format(str_replace(",",".",$dadosTitulos[$i]["VLTITULO"]),2,",","."),0,0,"R",0,"");
				$this->Cell(2,0.4,number_format(str_replace(",",".",$dadosTitulos[$i]["VLLIQUID"]),2,",","."),0,0,"R",0,"");
				$this->Cell(0.7,0.4,$dadosTitulos[$i]["DTLIBBDT"] != "" ? diffData($dadosTitulos[$i]["DTVENCTO"],$dadosTitulos[$i]["DTLIBBDT"]) : "",0,0,"L",0,"");
				$this->Cell(5.6,0.4,substr($dadosTitulos[$i]["NMSACADO"],0,34),0,0,"L",0,"");
				$this->Cell(3,0.4,strlen($dadosTitulos[$i]["NRINSSAC"]) > 11 ? formataNumericos('99.999.999/9999-99',$dadosTitulos[$i]["NRINSSAC"],'./-') : formataNumericos('999.999.999-99',$dadosTitulos[$i]["NRINSSAC"],'.-'),0,1,"L",0,"");
				for ($j = 0;$j < count($dadosRestricoes); $j++){
					if ($dadosRestricoes[$j]["NRDOCMTO"] == $dadosTitulos[$i]["NRDOCMTO"]){
						$qtrestricoes += 1;
						$this->controlaLinhas(1);
			...
			$this->controlaLinhas(3);
			$this->Ln();
			$this->Cell(4.7,0.4,"TOTAL: ".intval(count($dadosTitulos))." TÍTULO(S)",0,0,"L",0,"");
			$this->Cell(2,0.4,number_format(str_replace(",",".",$vlrtotal),2,",","."),0,0,"R",0,"");
			$this->Cell(2,0.4,number_format(str_replace(",",".",$vltotliq),2,",","."),0,0,"R",0,"");
			$this->Cell(9.3,0.4,"        VALOR MÉDIO: " .number_format(doubleval($vlrtotal / $qttotal),2,",",".")."          ".intval($qtrestricoes)." RESTRIÇÃO(ÕES)",0,1,"L",0,"");
			
			$this->Ln();
			
			$this->SetFont("Courier","",10);
			for ($i = 0;$i < count($dadosSacadNPG); $i++){
				if ($i == 0){
					$this->controlaLinhas(3);
					$this->Cell(12,0.4,"RELAÇÃO DE TÍTULOS NÃO PAGOS POR PAGADOR:",0,1,"L",0,"");
					$this->Ln();
					$this->Cell(12.5,0.4,"Nome","B",0,"L",0,"");
					$this->Cell(2.5,0.4,"Quantidade","B",0,"R",0,"");
					$this->Cell(3,0.4,"Valor","B",1,"R",0,"");
				}
				$this->controlaLinhas(1);
				$this->Cell(12.5,0.4,$dadosSacadNPG[$i]["NMSACADO"],0,0,"L",0,"");
				$this->Cell(2.5,0.4,$dadosSacadNPG[$i]["QTDTITUL"],0,0,"R",0,"");
				$this->Cell(3,0.4,number_format(str_replace(",",".",$dadosSacadNPG[$i]["VLRTITUL"]),2,",","."),0,1,"R",0,"");
			}			
