...
function acessaOpcaoAba(nrOpcoes,id,opcao) {
	...
	$.ajax({		
		type: 'POST', 
		dataType: 'html',
		url: UrlSite + 'telas/cadcta/informativos/principal.php',
		data: {
			nrdconta: nrdconta,
			idseqttl: idseqttl,
			nrdrowid: nrdrowid,
...
function controlaOperacao(operacao) {
    ...
    if ( operacao == 'TC'){  
        ...
        /* Buscar rowid selecionado na tela CADCTA*/    
        nrdrowid = '';
        nrdrowid    = $('#nrdrowid_infor', '#frmCabCadcta').val();        

    }
        
	if ( in_array(operacao,['CA','CX']) ) {
		nrdrowid = '';
		nrdrowid 	= $('#nrdrowid','#frmDadosInformativos').val();
		if ( nrdrowid == '' ) { return false; }	
	}
	
	var msgOperacao = '';
	if (in_array(operacao,['FA','','EC','TC'])) {
		//alert('Op vazia');
		...
		showConfirmacao('Deseja cancelar opera&ccedil;&atilde;o?','Confirma&ccedil;&atilde;o - Ayllos','controlaOperacao(\'TC\')','bloqueiaFundo(divRotina)','sim.gif','nao.gif');
		return false;
	} else  if ( operacao == 'IC' ) {
		...
		$('table > tbody > tr', 'div.divRegistrosInformativos').each( function() {
...
function manterRotina(operacao) {
	...
	$.ajax({		
		type: 'POST',
		url: UrlSite + 'telas/cadcta/informativos/manter_rotina.php', 		
		data: {
...
function controlaLayout( operacao ) {	
	...
	if ( in_array(operacao,['AC','IC','FI','FA','FE','','EC']) ) {	
		...
		var divRegistro = $('div.divRegistrosInformativos');		
