...
<?
	...
	$termoAssTl = ( count($xmlObjeto->roottag->tags[25]->tags) > 0 ) ? $xmlObjeto->roottag->tags[25]->tags[0]->tags : "" ;
...
<?
	...
	if ( count($termoIdent) != 0 ) {		
		...
		if ( getByTagName($termoAssTl,'nmextttl') != '' ){
			escreveLinha( preencheString('________________________________________',76) );
			escreveLinha( preencheString(getByTagName($termoAssTl,'nmextttl'),76) );
			escreveLinha( preencheString('CPF/CNPJ: '.getByTagName($termoAssTl,'nrcpfttl'),76) );
		}
