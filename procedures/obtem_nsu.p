...
Procedure: obtem_nsu.p 
Objetivo : Obter o sequencial único (do servidor) usado como chave nas tabelas
Autor    : Evandro
Data     : Março 2010
...
DEFINE OUTPUT PARAMETER par_nrsequni    AS INTEGER                  NO-UNDO.
...
RUN procedures/grava_log.p (INPUT "Obtendo NSU...").
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "NSU - Sem comunicação com o servidor.").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "NSU - " + xText:NODE-VALUE).
            ...
            IF  xField:NAME = "NRSEQUNI"  THEN
                par_nrsequni = INT(xText:NODE-VALUE).
...
RUN procedures/grava_log.p (INPUT "NSU: " + STRING(par_nrsequni)).
