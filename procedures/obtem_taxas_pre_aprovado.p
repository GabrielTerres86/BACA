/* ..............................................................................
 ...
 Procedure: obtem_taxas_pre_aprovado.p 
 Objetivo : Busca as taxas do Credito Pre-Aprovado
...
DEFINE INPUT  PARAMETER par_vlparepr    AS DECIMAL              NO-UNDO.
DEFINE INPUT  PARAMETER par_nrparepr    AS INTEGER              NO-UNDO.
DEFINE INPUT  PARAMETER par_dtvencto    AS DATE                 NO-UNDO.

DEFINE OUTPUT PARAMETER par_vlrtarif    AS DECIMAL  INIT 0      NO-UNDO.
...
RUN procedures/grava_log.p (INPUT "Busca as taxas do pre-aprovado...").
...
DO:
    ...
    xDoc:CREATE-NODE(xText,"","TEXT").
    xText:NODE-VALUE = STRING(par_vlparepr).
    xField:APPEND-CHILD(xText).
    ...
    xDoc:CREATE-NODE(xText,"","TEXT").
    xText:NODE-VALUE = STRING(par_nrparepr).
    xField:APPEND-CHILD(xText).
    ...
    DO  WHILE TRUE:
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "VLRTARIF"  THEN
                DO:
                    ASSIGN par_vlrtarif = DECI(xText:NODE-VALUE).
                END.

            IF  xField:NAME = "PERCETOP"  THEN
                DO:
                    ASSIGN par_percetop = DECI(xText:NODE-VALUE).
                END.
                
            IF  xField:NAME = "VLTAXIOF"  THEN
                DO:
                    ASSIGN par_vltaxiof = DECI(xText:NODE-VALUE).
                END.    
...
RUN procedures/grava_log.p (INPUT "Busca as taxas do credito pre-aprovado com " +
                                  "sucesso.").
