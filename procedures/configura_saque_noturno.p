/* ..............................................................................
 ...
 Procedure: configura_saque_noturno.p 
 Objetivo : Configura os valores de saque noturno
 Autor    : Evandro
 Data     : Agosto 2011
...
DEFINE  INPUT PARAMETER aux_hhininot    AS INTEGER              NO-UNDO.    
DEFINE  INPUT PARAMETER aux_mmininot    AS INTEGER              NO-UNDO.    
DEFINE  INPUT PARAMETER aux_hhfimnot    AS INTEGER              NO-UNDO.    
DEFINE  INPUT PARAMETER aux_mmfimnot    AS INTEGER              NO-UNDO.    
DEFINE  INPUT PARAMETER aux_vlsaqnot    AS DECIMAL              NO-UNDO.    
...
DEFINE VARIABLE aux_hrininot    AS INT      NO-UNDO.
DEFINE VARIABLE aux_hrfimnot    AS INT      NO-UNDO.


/* Valida os campos */

    /* hora inicial */     /* minuto inicial */
IF  aux_hhininot > 23  OR  aux_mmininot > 59  OR
    /* hora final */       /* minuto final */
    aux_hhfimnot > 23  OR  aux_mmfimnot > 59  THEN
    DO:
        IF  xfs_painop_em_uso  THEN
            DO:
                ...
                ASSIGN buff[2] = "           HORARIO INVALIDO"
                       buff[4] = "   VERIFIQUE OS VALORES INFORMADOS".
        ...
        ELSE
            DO:
                RUN mensagem.w (INPUT YES,
                                ...
                                INPUT "",
                                INPUT "",
                                INPUT "Horário Inválido! Verifique os",
                                INPUT "valores informados.",
                                INPUT "").
    ...
    DO:
        IF  xfs_painop_em_uso  THEN
            DO:
                ...
                ASSIGN buff[2] = "       VALOR DE SAQUE INVALIDO"
                       buff[4] = "     VERIFIQUE O VALOR INFORMADO".
        ...
        ELSE
            DO:
                RUN mensagem.w (INPUT YES,
                                INPUT "    ATENÇÃO",
                                INPUT "",
                                INPUT "Valor de Saque Inválido!",
                                INPUT "Verifique o valor informado.",
                                INPUT "",
                                INPUT "").
...
/* monta o horário em segundos */
ASSIGN aux_hrininot = aux_hhininot * 3600 + aux_mmininot * 60
       aux_hrfimnot = aux_hhfimnot * 3600 + aux_mmfimnot * 60.
...
RUN procedures/grava_log.p (INPUT "Configurando saque noturno...").

RUN procedures/grava_log.p (INPUT "Inicio: " + STRING(aux_hrininot,"HH:MM") + " " +
                                     "Fim: " + STRING(aux_hrfimnot,"HH:MM") + " " +
                                   "Valor: " + STRING(aux_vlsaqnot)).

CREATE "ADODB.Command" comando.
comando:ActiveConnection = conexao.

/* No firebird, separação decimal é por "." */
comando:CommandText = "UPDATE CRAPTFN " +
                      "SET HRININOT = " + STRING(aux_hrininot) + ", " +
                          "HRFIMNOT = " + STRING(aux_hrfimnot) + ", " +
                          "VLSAQNOT = " + REPLACE(STRING(aux_vlsaqnot),",",".").
