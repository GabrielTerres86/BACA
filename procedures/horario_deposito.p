...
Procedure: horario_deposito.p 
Objetivo : Verificar se ainda esta no horario para deposito
Autor    : Evandro
Data     : Maio 2010
...
RUN procedures/grava_log.p ("Verificando horário para depósitos...").
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Horario Deposito - Sem comunicação com o servidor").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "HRDEPOSITO"  THEN
                aux_flghorar = LOGICAL(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Horario Deposito - " + xText:NODE-VALUE).
...
IF  NOT aux_flghorar  THEN
    DO:
        RUN procedures/grava_log.p ("Horário já finalizado.").
        RETURN "NOK".
    END.


RUN procedures/grava_log.p ("Horário OK.").
RETURN "OK".
