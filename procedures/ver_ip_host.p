...
Procedure: ver_ip_host.p 
Objetivo : Captura o nome e ip do TAA
Autor    : Evandro
Data     : Janeiro 2010

     Obs.: Lógica retirada do KBASE da Progress.
...
DEFINE OUTPUT PARAMETER par_nmdohost    AS CHAR     NO-UNDO.
DEFINE OUTPUT PARAMETER par_ipdohost    AS CHAR     NO-UNDO.
...
RUN gettcpinfo (OUTPUT par_nmdohost, OUTPUT par_ipdohost).
...
PROCEDURE GetTcpInfo:
    ...
    Sample usage: RUN GetTcpInfo (OUTPUT w-TcpName, OUTPUT w-TcpAddr).
    ...
    INPUT GET-POINTER-VALUE(ptr-WsaData),OUTPUT w-Return).
