...
Procedure: obtem_telefone_sms_debaut.p
Objetivo : Obter telefone cadastrado para SMS DEBAUT [PROJ320]
Autor    : Lucas Lunelli
Data     : Abril/2016
...
DEFINE OUTPUT PARAM par_nrDDD        AS DECI                        NO-UNDO.
DEFINE OUTPUT PARAM par_nrtelefo     AS DECI                        NO-UNDO.
DEFINE OUTPUT PARAM par_dsmsgsms     AS CHAR                        NO-UNDO.
DEFINE OUTPUT PARAM par_flgderro     AS LOGI        INIT NO         NO-UNDO.
...
RUN procedures/grava_log.p (INPUT "Obtendo telefone de envio SMS DEBAUT...").
...
RUN mensagem.w (INPUT NO,
                ...
                INPUT "",
                INPUT "",
                INPUT "Obtendo telefone para envio de SMS...",
                INPUT "",
                INPUT "").
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Obtendo telefone de envio SMS DEBAUT - Sem comunicação com o servidor.").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Obtendo telefone de envio SMS DEBAUT - " + xText:NODE-VALUE).
            ...
            IF  xField:NAME = "NRDDD"  THEN
                par_nrDDD = DEC(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "NRTELEFO"  THEN
                par_nrtelefo = DEC(xText:NODE-VALUE).                                                   
            ELSE
            IF  xField:NAME = "DSMSGSMS"  THEN
                par_dsmsgsms = xText:NODE-VALUE.
...
RUN procedures/grava_log.p (INPUT "Obtido telefone de envio SMS DEBAUT com sucesso.").
