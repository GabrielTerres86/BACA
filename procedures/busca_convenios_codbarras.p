...
Procedure: busca_convenios_codbarras.p
Objetivo : Obter os convenios disponíveis para arrecadação com cod de barras
Autor    : Lucas Lunelli
Data     : Agosto/2014
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Verificação de Convênio para Aut. de Déb. - Sem comunicação com o servidor.").
        ...
        DO  aux_qtcompro = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "NMEXTCON"   THEN
                ASSIGN par_nmextcon = xText:NODE-VALUE
                       par_flgderro = NO.
            ELSE
            IF  xField:NAME = "FLGOFATR"   THEN
                DO:
                    IF  xText:NODE-VALUE = "N" AND 
                        par_cdoperac = "P"     THEN
                        ASSIGN par_nmextcon = ""
                               par_flgderro = NO.
                END.
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    ...
                    RUN procedures/grava_log.p (INPUT "Verificação de Convênio para Aut. de Déb. - " + xText:NODE-VALUE).

                    IF  par_cdoperac <> "P" THEN /* Pagamento */
                        DO:
                            RUN mensagem.w (INPUT YES,
                                            INPUT "      ERRO!",
                                            INPUT "",
