...
Procedure: efetua_pagamento_titulo.p 
Objetivo : Efetuar o pagamento do titulo
Autor    : Evandro
Data     : Maio 2010
...
DEFINE  INPUT PARAM par_dtvencto    AS DATE             NO-UNDO.
...
IF  NOT par_flagenda THEN
    DO:
        ...
        IF  par_flgderro   OR
            ...
            DO:
                RUN procedures/grava_log.p (INPUT "Pagamento de Título - Saldo Insuficiente").
        ...
        RUN procedures/grava_log.p (INPUT "Efetuando Pagamento de título...").
...
ELSE
    DO:
        ...
        RUN procedures/grava_log.p (INPUT "Efetuando Agendamento de título...").
...
/* Se é agendado TPDTRANS = 12, senão TPDTRANS = 9 */
IF par_flagenda THEN
    DO:
...
DO:
    ...
    IF  par_dtvencto <> ?  THEN
        DO:
            ...
            xDoc:CREATE-NODE(xText,"","TEXT").
            xText:NODE-VALUE = STRING(par_dtvencto).
            xField:APPEND-CHILD(xText).
        END.
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Pagamento de Título - Sem comunicação com o servidor.").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Pagamento de Título - " + xText:NODE-VALUE).
...
IF par_flagenda THEN
    RUN procedures/grava_log.p (INPUT "Agendamento de título efetuado com sucesso.").
ELSE
    RUN procedures/grava_log.p (INPUT "Pagamento de título efetuado com sucesso.").
