...
Procedure: valida_cartao.p 
Objetivo : Verificar se o cartao esta ok
Autor    : Evandro
Data     : Janeiro 2010
...
DEFINE OUTPUT PARAMETER par_idsenlet    AS LOGICAL                  NO-UNDO.
...
RUN procedures/grava_log.p (INPUT "Passou Cartão: " + par_dscartao).
...
DO:
    ...
    /* ---------- */
    xDoc:CREATE-NODE(xField,"DSCARTAO","ELEMENT").
    xRoot:APPEND-CHILD(xField).
    
    xDoc:CREATE-NODE(xText,"","TEXT").
    xText:NODE-VALUE = REPLACE(par_dscartao,"ç","c").
    xField:APPEND-CHILD(xText).
...
DO:
    ...
    DO  WHILE TRUE:
		...
		RUN procedures/grava_log.p (INPUT "Cartão " + par_dscartao + " - servidor respondeu").
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Captura Cartão - Sem comunicação com o servidor").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "CDCOOPER"  THEN
                glb_cdcooper = INT(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "NRDCONTA"  THEN
                glb_nrdconta = INT(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "NRCARTAO"  THEN
                glb_nrcartao = DEC(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "INPESSOA"  THEN
                glb_inpessoa = INT(xText:NODE-VALUE).
            ELSE
            IF  xField:NAME = "IDSENLET" THEN
                par_idsenlet = LOGICAL(xText:NODE-VALUE).
            ELSE
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Captura Cartão - " + xText:NODE-VALUE).
                    ...
                    IF  xfs_painop_em_uso  THEN
                        DO:
                            ...
                            ASSIGN buff[2] = "          CARTAO CRITICADO"
...
/* Transacao MultiCooperativa */
IF  glb_cdcoptfn <> glb_cdcooper  THEN
    RUN procedures/grava_log.p (INPUT "Transação MultiCooperativa - COOP: " +
                                      STRING(glb_cdcooper)).
									  
RUN procedures/grava_log.p (INPUT "Cartão lido - dados carregados").		
