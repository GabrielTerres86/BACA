...
Procedure: obtem_informacoes_comprovante.p
Objetivo : Obter as informações para composição do comprovante
Autor    : Lucas Lunelli
Data     : Agosto/2015
...
DEFINE OUTPUT PARAMETER par_nrtelsac AS CHARACTER.
DEFINE OUTPUT PARAMETER par_nrtelouv AS CHARACTER.
...
RUN procedures/grava_log.p (INPUT "Obtendo informacoes do comprovante...").
...
RUN mensagem.w (INPUT NO,
                ...
                INPUT "",
                INPUT "",
                INPUT "Obtendo dados do comprovante...",
                INPUT "",
                INPUT "").
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Obter informacoes do comprovante - Sem comunicação com o servidor.").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "DSCRITIC" THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Obter informacoes do comprovante - " + xText:NODE-VALUE).
            ...
            ELSE
                DO:
                    IF  xField:NAME = "NRTELSAC"  THEN
                        ASSIGN par_nrtelsac = STRING(xText:NODE-VALUE)
                               tmp_vlposesp = INTE((20 - LENGTH(par_nrtelsac)) / 2)
                               par_nrtelsac = FILL(" ", tmp_vlposesp) + par_nrtelsac
                               par_nrtelsac = par_nrtelsac + IF (tmp_vlposesp MOD 2) = 0 THEN
                                                                 FILL(" ", tmp_vlposesp)
                                                             ELSE 
                                                                 FILL(" ", (tmp_vlposesp - 1)).

                    IF  xField:NAME = "NRTELOUV"  THEN
                        ASSIGN par_nrtelouv = STRING(xText:NODE-VALUE)
                               tmp_vlposesp = INTE((20 - LENGTH(par_nrtelouv)) / 2)
                               par_nrtelouv = FILL(" ", tmp_vlposesp) + par_nrtelouv
                               par_nrtelouv = par_nrtelouv + IF (tmp_vlposesp MOD 2) = 0 THEN
                                                                 FILL(" ", tmp_vlposesp)
                                                             ELSE 
                                                                 FILL(" ", (tmp_vlposesp - 1)).
                END.
