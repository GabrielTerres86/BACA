...
Procedure: efetua_fechamento.p 
Objetivo : Efetuar fechamento do TAA
Autor    : Evandro
Data     : Fevereiro 2010
...
RUN procedures/grava_log.p (INPUT "Efetuando fechamento do terminal...").
...
IF  xfs_painop_em_uso  THEN
    DO:
        ...
        ASSIGN buff[2] = "             AGUARDE..."
               buff[4] = "     EFETUANDO FECHAMENTO DO TAA".
        RUN procedures/atualiza_painop.p (INPUT buff).
...
ELSE
    DO:
        RUN mensagem.w (INPUT NO,
                        ...
                        INPUT "",
                        INPUT "",
                        INPUT "Efetuando fechamento do TAA.",
                        INPUT "",
                        INPUT "").
...
/* Atualiza o fechamento no Servidor */
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Fechamento - Sem comunicação com o servidor.").
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "FECHAMENTO"  AND
                xText:NODE-VALUE = "OK"   THEN
                par_flgderro = NO.
            ELSE
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Fechamento - " + xText:NODE-VALUE).
                    ...
                    IF  xfs_painop_em_uso  THEN
                        DO:
                            ...
                            ASSIGN buff[2] = "         FECHAMENTO CRITICADO"
...
RUN procedures/grava_log.p (INPUT "Confimando fechamento no terminal...").
...
RUN procedures/grava_log.p (INPUT "Terminal fechado com sucesso.").
