...
Procedure: inclui_autorizacao_debito.p
Objetivo : Incluir autorização de débito na conta do cooperado
Autor    : Lucas Lunelli
Data     : Agosto/2014

Ultima alteração: 24/10/2014 - Inclusão de mensagem de sucesso (Lunelli)  
...
RUN procedures/grava_log.p (INPUT "Incluindo autorização de débito...").
...
RUN mensagem.w (INPUT NO,
                ...
                INPUT "",
                INPUT "",
                INPUT "Incluindo autorização de débito...",
                INPUT "",
                INPUT "").
...
DO:
    ...
    DO  WHILE TRUE:
        ...
        IF  xDoc:NUM-CHILDREN = 0  OR
            xRoot:NAME <> "TAA"    THEN
            DO:
                RUN procedures/grava_log.p (INPUT "Inclusão de Autorização de Débito - Sem comunicação com o servidor.").
        ...
        DO  aux_qtcompro = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Inclusão de Autorização de Débito - " + xText:NODE-VALUE).
...
IF  NOT par_flgderro THEN
    DO:
        RUN procedures/grava_log.p (INPUT "Autorização de Débito incluída com sucesso.").

        RUN mensagem.w (INPUT NO,
                        INPUT "    ATENÇÃO",
                        INPUT "",
                        INPUT "Autorização de débito",
                        INPUT "cadastrada com sucesso.",
                        INPUT "",
                        INPUT "").
    END.
