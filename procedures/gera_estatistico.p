...
Procedure: gera_estatistico.p 
Objetivo : Contabilizar tarifas interCooperativas pelo uso do TAA
Autor    : Gabriel
Data     : Agosto 2011
...
DEF OUTPUT PARAM par_flgderro AS LOGI                               NO-UNDO. 
...
RUN procedures/grava_log.p (INPUT "Contabilizar tarifas ...").
...
DO:
    ...
    xField:APPEND-CHILD(xText).
     ...
     /* ---------- */
    xDoc:CREATE-NODE(xField,"TPEXTRAT","ELEMENT").
    xRoot:APPEND-CHILD(xField).
    
    xDoc:CREATE-NODE(xText,"","TEXT").
    xText:NODE-VALUE = STRING(par_tpextrat).
    xField:APPEND-CHILD(xText).
...
DO:   
    ...
    DO  WHILE TRUE:
        ...
        DO  aux_contador = 1 TO xRoot:NUM-CHILDREN:
            ...
            IF  xField:NAME = "COMPROVANTE"    AND
                xText:NODE-VALUE = "OK"        THEN
                par_flgderro = NO.
            ELSE
            IF  xField:NAME = "DSCRITIC"  THEN
                DO:
                    RUN procedures/grava_log.p (INPUT "Impressao Comprovante - "+ xText:NODE-VALUE).
