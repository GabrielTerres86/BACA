...
function controlaOperacaoProcuradores( operacao ){
	...
	/* tela nao precisa validar permissao 
	...
	*/
    ...
    if ( in_array(operacao,['TC','TA','TX','TP']) ) {
		...
		nrcpfcgc 	= normalizaNumero($('#nrcpfcgc','#frmDadosProcuradores').val());
		nrdctato 	= normalizaNumero($('#nrdctato','#frmDadosProcuradores').val());
		nrdrowid 	= $('#nrdrowid','#frmDadosProcuradores').val();
        
        if (operacao != 'TP'){        
            nrdrowid    = $('#nrdrowid_procurador', '#frmCabCadcta').val();                
        }        

		if ( nrcpfcgc == '' && nrdctato == '' && nrdrowid == '' ) { return false; }
	...
	switch (operacao) {
		...
		case 'AT': 
			...
			showConfirmacao('Deseja cancelar altera&ccedil;&atilde;o?','Confirma&ccedil;&atilde;o - Ayllos','controlaOperacaoProcuradores(\'TC\')','bloqueiaFundo(divRotina)','sim.gif','nao.gif');
			return false;
			break;
		// Formulario em modo inclusão para tabela consulta		
		case 'IT': 
           showConfirmacao('Deseja cancelar inclus&atilde;o?','Confirma&ccedil;&atilde;o - Ayllos','fechaRotina(divRotina,\'\',\'obtemCabecalho();\');','bloqueiaFundo(divRotina)','sim.gif','nao.gif');
			return false;
            break;	
	...
	$.ajax({		
		type: 'POST',
		dataType: 'html',
		url: UrlSite + 'telas/cadcta/procuradores/principal.php', 
		data: {
...
function manterRotinaProcuradores(operacao) {
    ...
    dsproftl = $('#dsproftl','#frmDadosProcuradores').val();
    dtadmsoc = $('#dtadmsoc','#frmDadosProcuradores').val();
    persocio = $('#persocio','#frmDadosProcuradores').val();
    
        
    //se for pessoa juridica enviar seq 0
    if (inpessoa > 1 ){
        if (dsproftl == ''){
            dsproftl = 'PROCURADOR';
        }
	...
	$.ajax({		
		type: 'POST',
		url: UrlSite + 'telas/cadcta/procuradores/manter_rotina.php', 		
		data: {			
			...
			dsproftl: dsproftl, dtadmsoc: dtadmsoc, persocio: persocio,            
            operacao: operacao, 
...
function controlaLayoutProcuradores( operacao ) {
	...
	if ( in_array(operacao,['I','A','TX','CF','P','CT']) ) {
	...
	// FIELDSET IDENTIFICAÇÃO
		...
		cTpDocumento.css('width','65px');
		cDocumento.addClass('alphanum').css('width','170px').attr('maxlength','15');
...
function controlaPesquisasProcuradores() {
	...
	$('#cdnacion','#'+nomeFormProcuradores).unbind('change').bind('change',function() {
		...
		titulo      = ' Nacionalidade';
		filtrosDesc = '';
		buscaDescricao('ZOOM0001',procedure,titulo,$(this).attr('name'),'dsnacion',$(this).val(),'dsnacion',filtrosDesc,nomeFormProcuradores);        
		return false;
	}); 
...
function mostraTabelaBens( operacao ) {
	...
	$.ajax({		
		type: 'POST',
		dataType: 'html',
		url: UrlSite + 'telas/cadcta/procuradores/bens.php', 
		data: {
			operacao: operacao,
			redirect: 'html_ajax'			
			}, 
...
function salvarPoderes(){
	...
	$('table > tbody > tr', 'div.divRegistros').each(function () {
        ...
        if  ( $('input[name="hdnCodPoder"]', $(this)).val() != undefined) {
            if (strPoderes != "") {
                strPoderes += "/"
            }
	...
	$.ajax({		
		...
		success: function(response) {
			...
			if (response != "" &&
                ...
                $('#nrdrowid_procurador', '#frmCabCadcta').val($('#nrdrowid','#frmDadosProcuradores').val()); 
				//controlaOperacaoProcuradores('TC');
                encerraRotina(true);
                return false;
