...
CREATE OR REPLACE PROCEDURE CECRED.pc_quebra_sigilo_bancario IS 
  ...
  vr_dtfim      DATE;
  vr_contador   NUMBER;
  vr_dacaojud  crapicf.dacaojud%TYPE := 'INFORMAR_DESC_ACAO_JUD'; -- Informar descrição da ação judicial (este campo é utilizado na tabela CRAPICF)
  ...
  BEGIN
    ...
    WHILE TRIM(vr_idx_cta_investigar) IS NOT NULL LOOP
      ...
      FOR rw_conta IN cr_contas (pr_cdcooper => pr_tbconta_investigar(vr_idx_cta_investigar).cdcooper
        ...
        vr_tbdoc(1).nrcpfdeb := 01414471000109;
        vr_tbdoc(1).nrdconta := 4023617;
        vr_tbdoc(1).nrctadeb := 130035323;
        vr_tbdoc(1).dtmvtolt := TO_DATE('19/07/16','DD/MM/RRRR');
        vr_tbdoc(1).cdbancod := 33;
        vr_tbdoc(1).nmclideb := 'PROCOMEX CONSULTORIA EM NEGOCIOS INTERNA';
        vr_tbdoc(1).valordoc := 1000;
        vr_tbdoc(1).flgativo := 0;
        
        vr_tbdoc(2).cdagenci := 1;
        vr_tbdoc(2).nrcpfdeb := 01414471000109;
        vr_tbdoc(2).nrdconta := 4023617;
        vr_tbdoc(2).nrctadeb := 984580;
        vr_tbdoc(2).dtmvtolt := TO_DATE('22/12/16','DD/MM/RRRR');
        vr_tbdoc(2).cdbancod := 341;
        vr_tbdoc(2).nmclideb := 'PROCOMEX CONS EM NEGOCIOS INT';
        vr_tbdoc(2).valordoc := 2700;
        vr_tbdoc(2).flgativo := 0;

        vr_tbdoc(3).cdagenci := 1;
        vr_tbdoc(3).nrcpfdeb := 00004808688751;
        vr_tbdoc(3).nrdconta := 4023617;
        vr_tbdoc(3).nrctadeb := 304085;
        vr_tbdoc(3).dtmvtolt := TO_DATE('20/02/17','DD/MM/RRRR');
        vr_tbdoc(3).cdbancod := 1;
        vr_tbdoc(3).nmclideb := 'PABLO PINHEIRO V SANTOS';
        vr_tbdoc(3).valordoc := 2000;
        vr_tbdoc(3).flgativo := 0;

        vr_tbdoc(4).cdagenci := 1;
        vr_tbdoc(4).nrcpfdeb := 01414471000109;
        vr_tbdoc(4).nrdconta := 4023617;
        vr_tbdoc(4).nrctadeb := 130035323;
        vr_tbdoc(4).dtmvtolt := TO_DATE('07/11/17','DD/MM/RRRR');
        vr_tbdoc(4).cdbancod := 33;
        vr_tbdoc(4).nmclideb := 'PROCOMEX CONSULTORIA EM NEGOCIOS INTERNA';
        vr_tbdoc(4).valordoc := 500;
...
BEGIN 
  ...
  */
  ...
  vr_referencia := 'RITM0019698';
  -- Data que deve ocorrer a quebra de sigilo bancário
  vr_dtinicio   := TO_DATE('01/01/2015','DD/MM/YYYY');
  vr_dtfim      := TO_DATE('31/12/2017','DD/MM/YYYY'); 

  vr_tbconta_investigar('001_0004023617_001414471000109').cdcooper := 1;
  vr_tbconta_investigar('001_0004023617_001414471000109').nrdconta := 4023617;
  vr_tbconta_investigar('001_0004023617_001414471000109').nrcpfcgc := 01414471000109;
