...
DECLARE
  ...
  CURSOR cr_craptel IS
    SELECT * 
      FROM craptel
     WHERE UPPER(craptel.nmdatela) IN ('LCREDI');
  ...
  CURSOR cr_crapace(pr_cdcooper crapace.cdcooper%TYPE
    ...
    SELECT * 
     ...
     WHERE crapace.cdcooper = pr_cdcooper
       ...
       AND (UPPER(crapace.nmrotina) = UPPER(pr_nmrotina) OR TRIM(pr_nmrotina) IS NULL)
...
BEGIN 
  ...
  -- Iniciar Variáveis     
  vr_arq_path := '/micros/cpd/bacas/SCTASK0021195/';
  ...
  -- Efetuar abertura do arquivo para processamento
  gene0001.pc_abre_arquivo(pr_nmdireto => vr_arq_path   --> Diretorio do arquivo
                          ,pr_nmarquiv => 'processar_operadores.csv'  --> Nome do arquivo
                          ,pr_tipabert => 'R'           --> Modo de abertura (R,W,A)
                          ,pr_utlfileh => vr_hutlfile   --> Handle do arquivo aberto
                          ,pr_des_erro => vr_dscritic); --> Erro
...
--CDCOOPER	CDAGENCI	CDOPERAD	NMTELA	NMROTINA	NMOPCAO	NMOPERAD	DSCARGORH	NOMECENTROCUSTO	TIPO	OP_NIVEL	TELA_NIVEL
              ...
              IF vr_tab_opcao_nome.EXISTS(TRIM(vr_tab_ace(vr_chave).nmdatela)||UPPER(TRIM(REPLACE(vr_txtauxil,chr(13),'')))||trim(vr_tab_ace(vr_chave).nmrotina))THEN
                 vr_tab_ace(vr_chave).cddopcao := vr_tab_opcao_nome(vr_tab_ace(vr_chave).nmdatela||UPPER(TRIM(REPLACE(vr_txtauxil,chr(13),'')))||(vr_tab_ace(vr_chave).nmrotina)).cdopptel;
              ELSE
        ...
        BEGIN
          ...
          DELETE 
           ...
           WHERE crapace.cdcooper = rw_crapace.cdcooper
             ...
             AND (UPPER(crapace.nmrotina) = UPPER(rw_crapace.nmrotina) OR TRIM(rw_crapace.nmrotina) IS NULL)
