DECLARE
  ...
  vr_dsdireto          VARCHAR2(50) := '/micros/cpd/bacas/RITM0339318';
  vr_nmarquivo_entrada VARCHAR(200) := 'bacen_set_2023.csv';
  ...
  BEGIN
    ...
    IF cr_consulta_benefic_cip%FOUND
    THEN
      ...
      IF vr_des_erro = 'NOK'
      THEN
        vr_des_tit := vr_cooper || ';' || vr_nrdcta || ';' || vr_datmov || ';' || vr_vlrpag || ';' ||
                      vr_codigo_barras || ';' || ';' || ';' || ';' || 'XML consulta CIP em branco';
      ELSE
        ...
        vr_des_tit := vr_cooper || ';' || vr_nrdcta || ';' || vr_datmov || ';' || vr_vlrpag || ';' ||
                      vr_codigo_barras || ';' || vr_tpcpfCnpjBen || ';' || vr_nrcpfCnpjBen || ';' ||
                      vr_tbtitulo.CNPJ_CPFPagdr || ';' || pr_mensagem;
      END IF;
    ELSE
      CLOSE cr_consulta_benefic_cip;
      vr_des_tit := vr_cooper || ';' || vr_nrdcta || ';' || vr_datmov || ';' || vr_vlrpag || ';' ||
                    vr_codigo_barras || ';' || ';' || ';' || ';' || 'Consulta CIP nao encontrada';
    END IF;
...
BEGIN
  ...
  IF utl_file.IS_OPEN(vr_utlfileh)
  THEN
    LOOP
      vr_dsdlinha := NULL;
      BEGIN
        cecred.gene0001.pc_le_linha_arquivo(pr_utlfileh => vr_utlfileh, pr_des_text => vr_dsdlinha);
        vr_cooper        := NULL;
        vr_nrdcta        := NULL;
        vr_datmov        := NULL;
        vr_vlrpag        := NULL;
        ...
        FOR i IN 1 .. LENGTH(vr_dsdlinha)
        LOOP
          BEGIN
            IF SUBSTR(vr_dsdlinha, i, 1) = ';'
            THEN
              IF vr_cooper IS NULL
              THEN
                vr_cooper := SUBSTR(vr_dsdlinha, 1, i - 1);
                vr_pos    := i + 1;
              ELSE
                IF vr_nrdcta IS NULL
                THEN
                  vr_nrdcta := SUBSTR(vr_dsdlinha, vr_pos, i - vr_pos);
                  vr_pos    := i + 1;
                ELSE
                  IF vr_datmov IS NULL
                  THEN
                    vr_datmov := SUBSTR(vr_dsdlinha, vr_pos, i - vr_pos);
                    vr_pos    := i;
                  ELSE
                    IF vr_vlrpag IS NULL
                    THEN
                      vr_vlrpag := SUBSTR(vr_dsdlinha, vr_pos + 1, (i - 1) - vr_pos);
                      vr_pos    := i;
                    ELSE
                      IF vr_codigo_barras IS NULL
                      THEN
                        vr_codigo_barras := SUBSTR(vr_dsdlinha, vr_pos + 1, (i - 1) - vr_pos);
                        vr_pos           := i;
                      END IF;
                    END IF;
                  END IF;
        ...
        vr_nrcpf_cnpj := SUBSTR(vr_dsdlinha, vr_pos + 1, LENGTH(vr_dsdlinha) - vr_pos);
        ...
        THEN
          ...
          OPEN cr_consulta_tit_barras(TO_NUMBER(rw_cop.cdcooper), vr_codigo_barras);
          FETCH cr_consulta_tit_barras
            INTO rw_consulta_tit_barras;
          IF cr_consulta_tit_barras%FOUND
          THEN
          ...
          ELSE
            ...
            IF cr_cdctrlcs_titulo_estornado%FOUND
            THEN
              CLOSE cr_cdctrlcs_titulo_estornado;
            
              pc_consulta_benefic_cip(rw_consulta_tit_barras.cdctrlcs, 'Titulo estornado');
            ELSE
              CLOSE cr_cdctrlcs_titulo_estornado;
              vr_des_tit := vr_cooper || ';' || vr_nrdcta || ';' || vr_datmov || ';' || vr_vlrpag || ';' ||
                            vr_codigo_barras || ';' || ';' || ';' || ';' || 'Titulo nao encontrado';
            END IF;
          END IF;
        ELSE
        ...
        gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_ind_arqlog, pr_des_text => vr_des_tit);
      EXCEPTION
...
EXCEPTION
  WHEN OTHERS THEN
    CECRED.pc_internal_exception(pr_cdcooper => 1,
                                 pr_compleme => to_char(vr_cdcritic) || '-' || vr_dscritic ||
                                                ' - Conta:' || to_char(vr_nrdcta) || '- Erro: ' ||
                                                SQLERRM);
