...
BEGIN
  ...
  vr_tab_param(1).nmpartar := 'Central de Risco - Quantidade Paralelo Retorno Aimaro';
  vr_tab_param(1).tpdedado := 2;
  vr_tab_param(1).cdprodut := 12;
  vr_tab_param(1).cdbattar := 'QTPARRETAIMCTLR';
  vr_tab_param(1).nmidenti := 'Central de Risco - Quantidade Paralelo Retorno Aimaro';
  vr_tab_param(1).cdprogra := ' ';
  vr_tab_param(1).tpcadast := 2;
  vr_tab_param(1).cdcooper := 3;
  vr_tab_param(1).dsconteu := '30';
