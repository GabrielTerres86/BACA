DECLARE
  ...
  vr_dtinicio         DATE;
  vr_vlmora60         cecred.craplem.vllanmto%TYPE;
  vr_vlsaldo59        cecred.craplem.vllanmto%TYPE;
  vr_flgremun         BOOLEAN;
  vr_vltaxatu         cecred.crappep.vltaxatu%TYPE;
  vr_txdiaria         cecred.craplcr.txdiaria%TYPE;
  vr_vljurcor         cecred.crapepr.vljuratu%TYPE;
  vr_qtdedias         PLS_INTEGER;
  vr_taxa_periodo     NUMBER(25,8);
  vr_dtvencto         cecred.crappep.dtvencto%TYPE;
  vr_qtdiaatr         INTEGER;
  vr_dtrefere60       DATE;
  ...
  CURSOR cr_crapepr(pr_cdcooper IN cecred.crapcop.cdcooper%TYPE
  ...
  CURSOR cr_craplcr(pr_cdcooper IN cecred.craplcr.cdcooper%TYPE) IS
    SELECT cdlcremp
          ,dsoperac
          ,perjurmo
          ,flgcobmu
  ...
  CURSOR cr_renegociacao_saldo(pr_cdcooper IN cecred.crappep.cdcooper%TYPE) IS
    SELECT /*+ parallel */
           (r.vljurmor - r.vlpgjrmr) vljurmor, 
           (r.vljurcor - r.vlpgjrco) vljurcor, 
           (r.vlmultap - r.vlpgmult) vlmultap, 
           (r.vltariof - r.vlpgtiof) vltariof, 
           (r.vljura60 - r.vlpgjr60) vljura60, 
           (r.vljuremu - r.vlpgjrem) vljuremu, 
           r.cdcooper,
           r.nrdconta,
           r.nrctremp
      FROM cecred.tbepr_renegociacao_saldo r
          ,cecred.crapepr e
  ...
  CURSOR cr_parcelas(pr_cdcooper IN cecred.crapcop.cdcooper%TYPE) IS
    SELECT /*+ parallel */
           pep.cdcooper
          ,pep.nrdconta
          ,pep.nrctremp
  ...
  CURSOR cr_crapris(pr_cdcooper IN cecred.crapcop.cdcooper%TYPE
  ...
  CURSOR cr_verifica_reneg(pr_cdcooper crapass.cdcooper%TYPE) IS
    SELECT tr.cdcooper
     ...
     ORDER BY tr.dtlibera ASC;

  TYPE typ_reg_contrato_reneg IS
    RECORD(cdcooper cecred.tbepr_renegociacao.cdcooper%TYPE
          ,nrdconta cecred.tbepr_renegociacao.nrdconta%TYPE
          ,nrctrepr cecred.tbepr_renegociacao.nrctremp%TYPE
          ,nrdiaatr cecred.tbepr_renegociacao_contrato.nrdiaatr%TYPE);
  TYPE typ_tab_contrato_reneg_bulk  IS TABLE OF typ_reg_contrato_reneg INDEX BY PLS_INTEGER;
  TYPE typ_tab_contrato_reneg       IS TABLE OF typ_reg_contrato_reneg INDEX BY VARCHAR2(25); 
  vr_reg_contrato_reneg_bulk           typ_tab_contrato_reneg_bulk;
  vr_reg_contrato_reneg                typ_tab_contrato_reneg;

  
  PROCEDURE calcularMoraPosFixado(pr_cdcooper IN  cecred.crappep.cdcooper%TYPE   
                                 ,pr_dtcalcul IN  cecred.crapdat.dtmvtolt%TYPE   
                                 ,pr_dtvencto IN  cecred.crappep.dtvencto%TYPE   
                                 ,pr_dtultpag IN  cecred.crappep.dtultpag%TYPE   
                                 ,pr_vlsdvpar IN  cecred.crappep.vlsdvpar%TYPE   
                                 ,pr_perjurmo IN  cecred.craplcr.perjurmo%TYPE   
                                 ,pr_txmensal IN  cecred.crapepr.txmensal%TYPE   
                                 ,pr_vlperidx IN  cecred.crawepr.vlperidx%TYPE   
                                 ,pr_vltaxatu IN  cecred.crappep.vltaxatu%TYPE   
                                 ,pr_qttolatr IN  cecred.crapepr.qttolatr%TYPE   
  ...
  BEGIN
    ...
    cecred.EMPR9999.pc_calc_dias_atraso_jur(pr_cdcooper => pr_cdcooper
  ...
  EXCEPTION 
    WHEN vr_exc_erro THEN
      IF nvl(vr_cdcritic,0) > 0 AND TRIM(vr_dscritic) IS NULL THEN
        vr_dscritic := sistema.obterCritica(vr_cdcritic);
      END IF;
      pr_dscritic := vr_dscritic;
  ...
  PROCEDURE relatorioJuros60Contrato(pr_cdcooper  IN cecred.crapcop.cdcooper%TYPE
                                    ,pr_dscritic OUT cecred.crapcri.dscritic%TYPE) IS
  ...
  BEGIN
    ...
    vr_nmdireto := cecred.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cpd/bacas/RELATO_CENTRAL';
    ...
    IF vr_tab_crapepr.count > 0 THEN
        ...
        IF nvl(vr_qtdiaatr, 0) >= 60 THEN
          ...
          FOR rw_crappep IN cr_crappep(pr_cdcooper => vr_tab_crapepr(idx).cdcooper
            ...
            calcularMoraPosFixado(pr_cdcooper => vr_tab_crapepr(idx).cdcooper
                                 ...
                                 ,pr_dtultpag => nvl(rw_crappep.dtultpag, rw_crappep.dtvencto)
      ...
      cecred.gene0002.pc_clob_para_arquivo(pr_clob     => vr_desclob
...
BEGIN
  ...
  vr_cdcooper := 8;
  vr_dtinicio := SYSDATE;
  rw_crapdat  := SISTEMA.datascooperativa(vr_cdcooper);
