...
BEGIN
  ...
  vr_tab_param(1).nmpartar := 'CB093 - Horario limite aguardar arquivo Bancoob';
  vr_tab_param(1).tpdedado := 2;
  vr_tab_param(1).cdprodut := 12;
  vr_tab_param(1).cdbattar := 'HRLIMIMARQCB093';
  vr_tab_param(1).nmidenti := 'CB093 - Horario limite aguardar arquivo Bancoob';
  vr_tab_param(1).cdprogra := ' ';
  vr_tab_param(1).tpcadast := 2;
  vr_tab_param(1).cdcooper := 3;
  vr_tab_param(1).dsconteu := '08:15';
