DECLARE
  ...
  ORDER BY 1;
    ...
    BEGIN
       ...
       IF vr_aux_contador > 0 THEN
         ...
         cecred.gene0001.pc_oscommand_shell(pr_des_comando => 'ux2dos '||vr_nom_diretorio||'/'||vr_nmarqdat_prejuizo||' > '||vr_dsdircop||'/novacentral/erro/'||vr_nmarqdat_prejuizo_nov||' 2>/dev/null',
  ...
  /*  FOR rw_craplcm IN cr_craplem2 (pr_cdcooper,
   ...
   /*vr_cdhistor := 2386;
    ...
    cecred.gene0001.pc_oscommand_shell(pr_des_comando => 'ux2dos '||vr_nom_diretorio||'/'||vr_nmarqdat_ope_cred||' > '||vr_dsdircop||'/novacentral/erro/'||vr_nmarqdat_ope_cred_nov||' 2>/dev/null',
                                pr_typ_saida   => vr_typ_said,
                                pr_des_saida   => vr_dscritic);
    if vr_typ_said = 'ERR' then
       vr_cdcritic := 1040;
