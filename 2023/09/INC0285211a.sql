Declare 
  ...
  vc_dstransaSensbCRAPSLD             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSLD) por script - INC0285211a';
  vc_dstransaSensbCRAPSDA             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSDA) por script - INC0285211a';
  vc_dtinicioCRAPSDA                  CONSTANT DATE           := to_date('03/07/2023','dd/mm/yyyy');
  ...
  CURSOR cr_crapsld is
    SELECT a.nrdconta
      ...
      from CECRED.crapsld a
          ,(select 1 as cdcooper, 12151327    as nrdconta, 9.72   as valor from dual
            union all
            select 1 as cdcooper, 13159380    as nrdconta, 543.53   as valor from dual
			union all
			select 1 as cdcooper, 6168752    as nrdconta, 89.62   as valor from dual
            union all
			select 1 as cdcooper, 12373273    as nrdconta, 271.62   as valor from dual
			union all
			select 1 as cdcooper, 10421920   as nrdconta, 228.16   as valor from dual
			union all
			select 1 as cdcooper, 10494103   as nrdconta, 692.47   as valor from dual
			union all
			select 1 as cdcooper, 10616969   as nrdconta, 0.28   as valor from dual
			union all
			select 1 as cdcooper, 11367008   as nrdconta, 1.80   as valor from dual
			union all
			select 1 as cdcooper, 12602752   as nrdconta, 0.20    as valor from dual
			union all
			select 1 as cdcooper, 13050559 as nrdconta, 367.73    as valor from dual
			) contas
