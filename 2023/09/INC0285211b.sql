Declare 
  ...
  vc_dstransaSensbCRAPSLD             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSLD) por script - INC0285211b';
  vc_dstransaSensbCRAPSDA             CONSTANT VARCHAR2(4000) := 'Sensibilizacao do Saldo (CRAPSDA) por script - INC0285211b';
  ...
  CURSOR cr_crapsld is
    SELECT a.nrdconta
      ...
      from CECRED.crapsld a
          ,(select 1 as cdcooper, 12191507    as nrdconta, 639.24   as valor from dual
            union all
            select 1 as cdcooper, 11047496    as nrdconta, 974.36   as valor from dual
			union all
			select 1 as cdcooper, 12083771    as nrdconta, 1153.35   as valor from dual
            union all
			select 1 as cdcooper, 10869956    as nrdconta, 24.17   as valor from dual
			union all
			select 1 as cdcooper, 10572589   as nrdconta, 163.61   as valor from dual
			union all
			select 1 as cdcooper, 10910310   as nrdconta, 101.37   as valor from dual
			union all
			select 1 as cdcooper, 11810513   as nrdconta, 0.67   as valor from dual
			union all
			select 1 as cdcooper, 2948527   as nrdconta, 355.10   as valor from dual
			) contas
