...
CURSOR cr_teds IS
  SELECT DISTINCT decode(tvl.flgpesdb,1,lpad(tvl.cpfcgemi,11,'0'),lpad(tvl.cpfcgemi,14,'0')) CNPJ_CPFCLIDEBTD
   ...
   WHERE tvl.tpdoctrf = 3 
     AND tvl.dtmvtolt >= add_months(trunc(sysdate),-12)
     AND tvl.flgopfin = 1
  UNION
  SELECT DISTINCT 
   ...
   WHERE cti.cddbanco     = 85
     AND cti.dttransa    >= add_months(trunc(sysdate),-12)
  ...
  SELECT DISTINCT decode(a.TPPESSOA_PAGADOR,1,lpad(a.NRCPF_CNPJ_PAGADOR,11,'0'),lpad(a.NRCPF_CNPJ_PAGADOR,14,'0')) CNPJ_CPFCLIDEBTD
   ...
   WHERE a.IDTIPO_TRANSACAO = 'P'
     AND a.IDSITUACAO = 'P'
     AND a.DHTRANSACAO BETWEEN ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -12) AND SYSDATE
