DECLARE
    ...
    CURSOR cr_lancamento IS
      SELECT arq.idarquivo
       ...
       WHERE lct.idarquivo = arq.idarquivo
         ...
         AND arq.idarquivo IN (665705,665736,665759,665772,665798,665810,665822,665831,665857,665858)
    ...
    CURSOR cr_tabela(pr_idlancto       tbdomic_liqtrans_lancto.idlancto%TYPE
      ...
      SELECT pdv.nrliquidacao
       ...
       WHERE ctz.idlancto = pr_idlancto
         ...
         AND ctz.cdagencia_centraliza <> 805
       ORDER BY pdv.vlpagamento
               ,ctz.cdagencia_centraliza
               ,ctz.nrcta_centraliza
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      raise_application_error(-20001,
                              'pc_efetiva_reg_pendentes ' || vr_qtprocessados || ' - ' ||
                              vr_nrdconta || ' - ' || SQLERRM);
...
BEGIN
  ...
  BEGIN
    UPDATE CECRED.tbdomic_liqtrans_lancto
       SET insituacao = 1
     WHERE idarquivo IN (665705,665736,665759,665772,665798,665810,665822,665831,665857,665858);
  END;
