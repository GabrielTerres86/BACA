...
CURSOR cr_teds IS
  SELECT DISTINCT decode(tvl.flgpesdb,1,lpad(tvl.cpfcgemi,11,'0'),lpad(tvl.cpfcgemi,14,'0')) CNPJ_CPFCLIDEBTD
   ...
   WHERE tvl.tpdoctrf = 3 
     AND tvl.dtmvtolt >= add_months(trunc(sysdate),-12) and tvl.dtmvtolt < to_date('08/06/2023')
     AND tvl.flgopfin = 1
  UNION
  SELECT DISTINCT 
   ...
   WHERE cti.cddbanco     = 85
     ...
     and cti.dttransa < to_date('08/06/2023')
     AND ass_dbt.cdcooper = cti.cdcooper
     AND ass_dbt.nrdconta = cti.nrdconta
     AND ass_dbt.cdcooper = cti.cdcooper
     and cop.cdcooper     = cti.cdcooper
     AND cop.cdagectl     = cti.cdageban
     AND ass_cdt.cdcooper = cop.cdcooper
     AND ass_cdt.nrdconta = cti.nrctatrf
   UNION
  SELECT  /*+ INDEX(a TBPIX_TRANSA_IDX_08) */
       DISTINCT decode(a.TPPESSOA_PAGADOR,1,lpad(a.NRCPF_CNPJ_PAGADOR,11,'0'),lpad(a.NRCPF_CNPJ_PAGADOR,14,'0')) CNPJ_CPFCLIDEBTD
   ...
   WHERE a.IDTIPO_TRANSACAO IN ('P','PC')
     AND a.IDSITUACAO = 'P'
     AND a.CDBCCXLT_RECEBEDOR <> 0
     AND a.DTMVTOLT >= ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -12) 
     AND a.DTMVTOLT < to_date('08/06/2023');
