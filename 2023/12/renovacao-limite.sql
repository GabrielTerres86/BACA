DECLARE
   ...
   vr_aux_diretor     VARCHAR2(100) := 'RITM0349885';     
  ...
  BEGIN
    ...
    ELSE
      ...
      IF rw_craplim.dtfimvig > pr_dtmvtolt THEN
        vr_dscritic := 'Nao e possivel realizar a renovacao do limite. Contrato nao esta vencido. ' || rw_craplim.dtfimvig;
        RAISE vr_exc_saida;
      END IF;
    ...
    UPDATE cecred.craplim 
     ...
     gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_handle
                                   ,pr_des_text => 'UPDATE cecred.craplim lim ' ||
                                                   '   SET lim.dtrenova = ' ||''''||to_char(rw_craplim.dtrenova,'dd/mm/rrrr')||''''||
                                                   '      ,lim.tprenova = ' ||''''||rw_craplim.tprenova||''''||
                                                   '      ,lim.dtfimvig = ' ||''''||to_char(rw_craplim.dtfimvig_rollback,'dd/mm/rrrr')||''''||
                                                   '      ,lim.qtrenova = ' || rw_craplim.qtrenova ||
     ...
     gene0001.pc_gera_log(pr_cdcooper =>  rw_craplim.cdcooper
                         ...
                         ,pr_dstransa => 'Alteracao via script renovacao limite de credito - ' || vr_aux_diretor
...
BEGIN
    IF vr_aux_ambiente = 1 THEN 
      vr_nmarq_regs     := '/progress/t0035419/micros/script_renovacao/'|| vr_aux_diretor ||'/'|| vr_aux_arquivo ||'.csv';  
