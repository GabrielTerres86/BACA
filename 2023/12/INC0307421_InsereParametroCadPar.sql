...
BEGIN
  ...
  vr_tab_param(1).nmpartar := 'Cyber - Habilitar Rotina Atualização Arquivo MC';
  vr_tab_param(1).tpdedado := 2;
  vr_tab_param(1).cdprodut := 12;
  vr_tab_param(1).cdbattar := 'INEXECATUMCCYB';
  vr_tab_param(1).nmidenti := 'Cyber - Habilitar Rotina Atualização Arquivo MC';
  vr_tab_param(1).cdprogra := ' ';
  vr_tab_param(1).tpcadast := 2;
  vr_tab_param(1).cdcooper := 3;
  vr_tab_param(1).dsconteu := '1';
