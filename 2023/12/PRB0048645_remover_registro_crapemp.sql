DECLARE 
  vr_nrdrowid        ROWID;
  vr_nmprograma      CECRED.tbgen_prglog.cdprograma%TYPE := 'Excluir registros com erros PRB0048645';
   
  CURSOR cr_crapemp IS 
    SELECT *
      FROM CECRED.crapemp emp
     WHERE emp.cdempres IN (441, 442)
       AND emp.cdcooper = 12
  ...
  BEGIN
    FOR rw_crapemp IN cr_crapemp LOOP
      CECRED.GENE0001.pc_gera_log(pr_cdcooper => rw_crapemp.cdcooper
                                 ...
                                 ,pr_idseqttl => rw_crapemp.cdempres
      ...
      CECRED.GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid
                                      ,pr_nmdcampo => 'crapemp.cdempres'
                                      ,pr_dsdadant => rw_crapemp.cdempres
                                      ,pr_dsdadatu => NULL);    
      CECRED.GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    CECRED.pc_internal_exception(pr_cdcooper => 0
                                ,pr_compleme => ' Script: => ' || vr_nmprograma ||
                                                ' Etapa: 1 - Salvar log informacoes do registro antes de deletar'); 
  END;
  
  BEGIN 
   FOR rw_crapemp IN cr_crapemp LOOP
    DELETE FROM CECRED.crapemp emp
     WHERE emp.cdempres = rw_crapemp.cdempres
       AND emp.cdcooper = rw_crapemp.cdcooper
       AND emp.nrdconta = rw_crapemp.nrdconta;
   END LOOP;                                             
  EXCEPTION WHEN OTHERS THEN
    ROLLBACK;
    CECRED.pc_internal_exception(pr_cdcooper => 0
                                ,pr_compleme => ' Script: => ' || vr_nmprograma ||
                                                ' Etapa: 2 - Delete crapemp');          
