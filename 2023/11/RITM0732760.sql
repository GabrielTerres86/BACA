DECLARE
  ...
  vr_arq_path            VARCHAR2(1000):= cecred.gene0001.fn_param_sistema('CRED',0,'ROOT_MICROS') || 'cpd/bacas/RITM0732760';
  vr_nmarquiv            VARCHAR2(100) := 'contas_viacredi_HML.txt';
  vr_nmarqbkp            VARCHAR2(100) := 'RITM0732760_script_rollback_test.sql';
  vr_nmarqcri            VARCHAR2(100) := 'RITM0732760_script_log_test.txt';
  ...
  PROCEDURE TrocaTipoConta(pr_cdcooper  IN  crapass.cdcooper%TYPE
    ...
    vr_cdtipcta    CONSTANT NUMBER := 22;
  ...
  if cr_crapass%found THEN
     BEGIN 
         UPDATE CECRED.crapass t SET t.cdtipcta = vr_cdtipcta
                                   , t.dtultalt = vr_dtmvtolt
          WHERE ROWID = conta.dsdrowid;
        
          pc_escreve_xml_rollback(chr(10)||'  UPDATE cecred.crapass SET cdtipcta = '||to_char(conta.cdtipcta)||'  WHERE rowid = '''||conta.dsdrowid||'''; ');
     EXCEPTION
          WHEN OTHERS THEN
       ...
       gene0001.pc_gera_log_item(pr_nrdrowid => vr_lgrowid
                                ...
                                ,pr_dsdadant => conta.cdtipcta
                                ,pr_dsdadatu => vr_cdtipcta);
       ...
       ELSE
          ...
          BEGIN
            UPDATE cecred.crapalt SET dsaltera = rg_altera.dsaltera||'PA '||conta.cdtipcta||'-'||vr_cdtipcta||','
