  ...
  BEGIN    
    ...
    BEGIN
      ...
      vr_dslog := 'DELETE cecred.craplct ' ||
                  ' WHERE CDCOOPER = ' ||pr_cdcooper ||
                    ' AND NRDCONTA = '|| pr_nrdconta ||
                    ' AND NRDOLOTE = 900039 AND CDHISTOR = 2417 AND DTMVTOLT = ' || pr_dtmvtolt ||';'; 
      CECRED.pc_log_programa(pr_dstiplog      => 'O'
                             ,pr_dsmensagem    => vr_dslog
                             ,pr_cdmensagem    => 222
                             ,pr_cdprograma    => vr_cdprograma
      ...
      vr_dslog := 'DELETE cecred.craplcm ' ||
                 ' WHERE CDCOOPER = ' ||pr_cdcooper ||
                   ' AND NRDCONTA = '|| pr_nrdconta ||
                   ' AND NRDOLOTE = 900039 AND CDHISTOR = 2418 AND DTMVTOLT = ' || pr_dtmvtolt ||';'; 
      CECRED.pc_log_programa(pr_dstiplog      => 'O'
                             ,pr_dsmensagem    => vr_dslog
                             ,pr_cdmensagem    => 333
                             ,pr_cdprograma    => vr_cdprograma
...
BEGIN
  ...
  CECRED.pc_log_programa(pr_dstiplog      => 'O'
                            ,pr_dsmensagem    => 'Inicio script'
                            ,pr_cdmensagem    => 999
                            ,pr_cdprograma    => vr_cdprograma
                            ,pr_cdcooper      => 3 
                            ,pr_idprglog      => vr_idprglog);
                              
  vr_nmdireto := gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS') || 'cpd/bacas/RITM0340990/';
