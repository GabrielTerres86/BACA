DECLARE
  ...
  vr_nmdir               VARCHAR2(4000) := CECRED.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/INC0255088';
  vr_nmarq               VARCHAR2(100)  := 'ROLLBACK_INC0255088.sql';
    ...
    SELECT p.idseqtra
          ...
          ,NVL((SELECT j.vldivida 
                ...
                WHERE j.cdcooper = p.cdcooper 
                  ...
                  AND j.dtrefere = TO_DATE('28/02/2023','DD/MM/RRRR')
     ...
     WHERE p.cdcooper = pr_cdcooper
       ...
       AND TRUNC(p.dtinivig) < TO_DATE('01/03/2023','DD/MM/RRRR')
       AND p.tpregist <> 0
       AND p.tpcustei = 1
      ...
      ELSE 
        IF vr_vlsdeved >= vr_vlminimo AND vr_tpregist = 2 and vr_vlsdatua > 0 THEN            
          ...
          pc_replica_cancelado(pr_cdcooper   => pr_cdcooper,
                               ...
                               pr_tpregist   => rw_prestamista.tpregist,
          ...
          IF cr_nrproposta%FOUND THEN
             BEGIN
              ...
              EXCEPTION
                WHEN OTHERS THEN
                  vr_cdcritic := 0;
                  vr_dscritic := 'Erro ao atualizar numero de proposta já atualizada - cdcooper: ' || pr_cdcooper || ' - nrdconta: ' || rw_prestamista.nrdconta || ' - nrproposta: ' || vr_NRPROPOSTA || ' - '  || SQLERRM;
                  RAISE vr_exc_saida;
              END;    
          END IF;
        CLOSE cr_nrproposta;
