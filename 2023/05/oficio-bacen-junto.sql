DECLARE
  ...
  pr_cdprogcotas   VARCHAR2(40) := 'BACA-CREDITO';

  vr_indsobra VARCHAR2(2000);
  vr_inderro  VARCHAR2(2000);
  ...
  vr_inprejuz_cc VARCHAR2(400);
  vr_infimsol    INTEGER;
  vr_cooppreju   INTEGER := NULL;
  ...
  TYPE typ_reg_contrato_erro IS RECORD(
     cdcooper cecred.crapepr.cdcooper%TYPE
    ,nrdconta cecred.crapepr.nrdconta%TYPE
    ,contrato cecred.crapepr.nrctremp%TYPE);
  TYPE typ_tab_contrato_erro IS TABLE OF typ_reg_contrato_erro INDEX BY VARCHAR(2000);
  vr_tab_contrato_erro typ_tab_contrato_erro;

  CURSOR cr_atraso_pp(pr_cdcooper IN cecred.crappep.cdcooper%TYPE
                     ,pr_nrdconta IN cecred.crappep.nrdconta%TYPE
                     ,pr_dtmvtolt IN cecred.crappep.dtvencto%TYPE
                     ,pr_dtmvtoan IN cecred.crappep.dtvencto%TYPE) IS
    SELECT nvl((SELECT 1
                 FROM cecred.crapbpr
                WHERE crapbpr.cdcooper = crappep.cdcooper
                      ...
                      AND
                      grvm0001.fn_valida_categoria_alienavel(crapbpr.dscatbem) = 'S'
          ...
          ,crappep.rowid
          ,crapepr.tpemprst
          ,crapass.inprejuz inprejuz_cc
      FROM cecred.crawepr, cecred.crapass, cecred.crappep, cecred.crapepr
  ...
  CURSOR cr_sobra_antecipa_pp(pr_cdcooper cecred.crappep.cdcooper%TYPE
  ...
  BEGIN    
    ...
    BEGIN
      ...
      IF cr_crapcot%NOTFOUND THEN
        CLOSE cr_crapcot;
        vr_dscritic := 'Nao encontrado conta em cotas - ' ;
        RAISE vr_exc_erro;
      END IF;
      ...
      IF nvl(pr_vllanmto,0) = 0 THEN
        vr_dscritic := 'Nao possui valor para lancar: Cooperativa: ' ;
        RAISE vr_exc_erro;
      END IF;
    
      IF nvl(pr_vllanmto,0) > rw_crapcot.vldcotas THEN  
        vr_dscritic := 'Valor para saque maior que o saldo disponível: Cooperativa ' ;
        RAISE vr_exc_erro;
      END IF;
       
      IF (rw_crapcot.vldcotas - nvl(pr_vllanmto,0)) < 1 THEN
        
        vr_dscritic := 'Nao e possivel saque total, deve deixar pelo menos 1,00 em cotas: Cooperativa ';
        RAISE vr_exc_erro;
      END IF;
    ...
    EXCEPTION
      WHEN vr_exc_erro THEN
        ...
        ELSE
          ...
          pr_dscritic := vr_dscritic ||  ' Cooperativa ' ||  pr_cdcooper || ' Conta: ' || pr_nrdconta;
        END IF;
  ...
  PROCEDURE calcularParcPosePp(pr_cdcooper        crapepr.cdcooper%TYPE
                              ...
                              ,pr_tab_pgto_parcel OUT empr0001.typ_tab_pgto_parcel
                              ,pr_cdcritic        OUT crapcri.cdcritic%TYPE
                              ,pr_dscritic        OUT crapcri.dscritic%TYPE) IS
  ...
  BEGIN
      ...
      IF rw_contrato.tpemprst = 1 THEN
        ...
        FOR idx IN vr_tab_pgto_parcel_pp.FIRST .. vr_tab_pgto_parcel_pp.LAST LOOP
          ...
          IF vr_tab_pgto_parcel_pp(idx).nrparepr = pr_nrparepr THEN
            ...
            vr_tab_pgto_parc(1).cdcooper := vr_tab_pgto_parcel_pp(idx).cdcooper;
            vr_tab_pgto_parc(1).nrdconta := vr_tab_pgto_parcel_pp(idx).nrdconta;
            vr_tab_pgto_parc(1).nrctremp := vr_tab_pgto_parcel_pp(idx).nrctremp;
            vr_tab_pgto_parc(1).nrparepr := vr_tab_pgto_parcel_pp(idx).nrparepr;
            vr_tab_pgto_parc(1).vlparepr := vr_tab_pgto_parcel_pp(idx).vlparepr;
            vr_tab_pgto_parc(1).vljinpar := vr_tab_pgto_parcel_pp(idx).vljinpar;
            vr_tab_pgto_parc(1).vlmrapar := vr_tab_pgto_parcel_pp(idx).vlmrapar;
            vr_tab_pgto_parc(1).vlmtapar := vr_tab_pgto_parcel_pp(idx).vlmtapar;
            vr_tab_pgto_parc(1).vliofcpl := vr_tab_pgto_parcel_pp(idx).vliofcpl;
            vr_tab_pgto_parc(1).dtvencto := vr_tab_pgto_parcel_pp(idx).dtvencto;
            vr_tab_pgto_parc(1).dtultpag := vr_tab_pgto_parcel_pp(idx).dtultpag;
            vr_tab_pgto_parc(1).vlpagpar := vr_parcela;
            vr_tab_pgto_parc(1).vlpagmta := vr_tab_pgto_parcel_pp(idx).vlpagmta;
            vr_tab_pgto_parc(1).vlpagmra := vr_tab_pgto_parcel_pp(idx).vlpagmra;
            vr_tab_pgto_parc(1).vldespar := vr_tab_pgto_parcel_pp(idx).vldespar;
            vr_tab_pgto_parc(1).inliquid := vr_tab_pgto_parcel_pp(idx).inliquid;
            vr_tab_pgto_parc(1).vlpagjin := vr_tab_pgto_parcel_pp(idx).vlpagjin;
            vr_tab_pgto_parc(1).vlatupar := vr_tab_pgto_parcel_pp(idx).vlatupar;
            vr_tab_pgto_parc(1).vlsdvpar := vr_tab_pgto_parcel_pp(idx).vlsdvpar;
            vr_tab_pgto_parc(1).vlatrpag := vr_tab_pgto_parcel_pp(idx).vlatrpag;
            vr_tab_pgto_parc(1).perhonor := vr_tab_pgto_parcel_pp(idx).perhonor;
            vr_tab_pgto_parc(1).flgantec := vr_tab_pgto_parcel_pp(idx).flgantec;
            vr_tab_pgto_parc(1).vljinp59 := vr_tab_pgto_parcel_pp(idx).vljinp59;
            vr_tab_pgto_parc(1).vljinp60 := vr_tab_pgto_parcel_pp(idx).vljinp60;
    ...
    pr_tab_pgto_parcel := vr_tab_pgto_parc;
  ...
  PROCEDURE CRPS750_PAGAMENTO(pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    CURSOR cr_crappep(pr_cdcooper IN crappep.cdcooper%TYPE
      ...
      SELECT crappep.cdcooper
            ...
            ,crappep.vlmrapar
            ,crappep.vliofcpl
        FROM cecred.crawepr, cecred.crapass, cecred.crappep
       WHERE crawepr.cdcooper(+) = crappep.cdcooper
             AND crawepr.nrdconta(+) = crappep.nrdconta
             AND crawepr.nrctremp(+) = crappep.nrctremp
             AND crapass.cdcooper = crappep.cdcooper
    ...
    CURSOR cr_craplcmC(pr_cdcooper IN craplcm.cdcooper%TYPE
      ...
      SELECT a.vllanmto
        FROM cecred.craplcm a, cecred.craphis b
       WHERE a.cdcooper = pr_cdcooper
             AND a.nrdconta = pr_nrdconta
    ...
    CURSOR cr_cde(pr_cdcooper IN crapcob.cdcooper%TYPE
      ...
      SELECT cob.nrdocmto
       ...
       WHERE cob.cdcooper = pr_cdcooper
             AND cob.incobran = 0
             AND (cob.nrdconta, cob.nrcnvcob, cob.nrctasac, cob.nrctremp,
              cob.nrdocmto) IN
             (SELECT DISTINCT nrdconta_cob
                                 ,nrcnvcob
                                 ,nrdconta
                                 ,nrctremp
                                 ,nrboleto
                    FROM cecred.tbrecup_cobranca cde
    ...
    CURSOR cr_ret(pr_cdcooper IN crapcob.cdcooper%TYPE
      ...
      SELECT cob.nrdocmto
        FROM cecred.crapcob cob, cecred.crapret ret
       WHERE cob.cdcooper = pr_cdcooper
             AND cob.incobran = 5
    ...
    CURSOR cr_ctr_acordo(pr_cdcooper tbrecup_acordo.cdcooper%TYPE
      ...
      SELECT tbrecup_acordo_contrato.nracordo
        ...
        FROM cecred.tbrecup_acordo_contrato
        JOIN tbrecup_acordo
    ...
    EXCEPTION
      WHEN OTHERS THEN
        vr_cdcritic := 0;
        vr_dscritic := 'Erro ao limpar tabelas de memoria. Rotina pc_CRPS750.pc_limpa_tabela. ' ||
                       SQLERRM;
        RAISE vr_exc_saida;
    END pc_limpa_tabela;
  ...
  BEGIN
    ...
    FOR rw_crappep IN cr_crappep(pr_cdcooper => pr_cdcooper
      ...
      vr_flgemdia := rw_crappep.dtvencto > rw_crapdat.dtmvtoan AND
                     rw_crappep.dtvencto <= rw_crapdat.dtmvtolt;
    
      vr_flgantec := rw_crappep.dtvencto > rw_crapdat.dtmvtolt;
      ...
      FETCH cr_blqDebConsignado
        INTO rw_blqDebConsignado;
      CLOSE cr_blqDebConsignado;
    
      vr_blqconsig := FALSE;
      IF rw_crapepr.tpdescto = 2 AND
         NVL(rw_blqDebConsignado.DSCONTEU, 0) = 1 THEN
        vr_blqconsig := TRUE;
            ...
            BEGIN
              UPDATE crapepr
                 SET crapepr.diarefju = nvl(vr_diarefju, 0)
                    ...
                    ,crapepr.vlsdeved = nvl(crapepr.vlsdeved, 0) +
                                        nvl(vr_vljurmes, 0)
                    ,crapepr.vljuratu = nvl(crapepr.vljuratu, 0) +
                                        nvl(vr_vljurmes, 0)
                    ,crapepr.vljuracu = nvl(crapepr.vljuracu, 0) +
                                        nvl(vr_vljurmes, 0)
               WHERE crapepr.rowid = rw_crapepr.rowid;
            EXCEPTION
              WHEN OTHERS THEN
      ...
      ELSIF vr_flgantec THEN
        ...
        calcularParcPosePp(pr_cdcooper        => rw_crappep.cdcooper
                          ,pr_nrdconta        => rw_crappep.nrdconta
                          ,pr_nrctremp        => rw_crappep.nrctremp
                          ,pr_nrparepr        => rw_crappep.nrparepr
                          ,pr_vlparcela       => vr_vlapagar
        ...
        vr_tab_pgto_parcel(1).vlpagpar := vr_vlsomato;
        ...
        ELSE
          EMPR0001.pc_gera_pagamentos_parcelas(pr_cdcooper        => pr_cdcooper
                                              ...
                                              ,pr_nrctremp        => rw_crappep.nrctremp
                                              ,pr_dtmvtoan        => rw_crapdat.dtmvtoan
                                              ,pr_totatual        => vr_vlsdeved
                                              ,pr_totpagto        => vr_vlsomato
                                              ,pr_nrseqava        => 0
                                              ,pr_tab_pgto_parcel => vr_tab_pgto_parcel
                                              ,pr_des_reto        => vr_des_reto
                                              ,pr_tab_erro        => vr_tab_erro);
        ...
        IF vr_des_reto = 'NOK' THEN
          IF vr_tab_erro.exists(vr_tab_erro.first) THEN
            IF vr_tab_erro(vr_tab_erro.first).cdcritic = 995 THEN
              vr_cdcritic := vr_tab_erro(vr_tab_erro.first).cdcritic;
              vr_dscritic := GENE0001.fn_busca_critica(pr_cdcritic => vr_tab_erro(vr_tab_erro.first).cdcritic);
            ELSE
              vr_cdcritic := 0;
      ...
      ELSE
        ...
        IF rw_crapepr.tpdescto = 2 THEN
          ...
          ELSIF to_char(rw_crapdat.dtmvtolt, 'DD') <
                (vr_nrdialimiterepasse) THEN
            vr_vlsomato := 0;
        ...
        IF vr_tab_acordo.EXISTS(vr_cdindice) THEN
          vr_vlsomato := 0;
          vr_dscritic := 'Acordo encontrado, Contrato: ' ||
                         rw_crappep.nrctremp;
        END IF;
      
        IF (vr_vlsomato) <= 0 THEN
          vr_dscritic := vr_dscritic ||
                         ' Pulou Parcela erro Se o saldo mais o valor de resgate são menores ou igual a zero';
          pr_dscritic := vr_dscritic;
          CONTINUE;
        END IF;
        ...
        IF vr_index_pgto_parcel IS NOT NULL THEN
          IF nvl(vr_vlsomato, 0) >
             nvl(vr_tab_pgto_parcel(vr_index_pgto_parcel).vlatrpag, 0) THEN
            vr_vlsomato := nvl(vr_tab_pgto_parcel(vr_index_pgto_parcel).vlatrpag
                              ,0);
          END IF;
        END IF;
        ...
        IF rw_crapepr.tpdescto = 2 THEN
          EMPR0020.pc_gera_pagto_deb_uni_consig(pr_cdcooper => pr_cdcooper
                                               ...
                                               ,pr_nrparepr => rw_crappep.nrparepr
                                               ,pr_vlparpag => vr_vlsomato
                                               ,pr_vlsdvpar => nvl(vr_tab_pgto_parcel(vr_index_pgto_parcel).vlatrpag
                                                                  ,0)
                                               ,pr_dtvencto => rw_crappep.dtvencto
                                               ,pr_cdlcremp => rw_crapepr.cdlcremp
                                               ,pr_tppagmto => 'A'
  ...
  PROCEDURE CRPS724_PAGAMENTO(pr_cdcooper crapepr.cdcooper%TYPE
    ...
    FUNCTION fun_identifica_tipo_critica(pr_cdcritic IN crapcri.cdcritic%TYPE)
      ...
      CURSOR cur_crapcri IS
        SELECT tpcritic FROM cecred.crapcri WHERE cdcritic = pr_cdcritic;
  ...
  BEGIN
    ...
    FOR rw_epr_pep IN cr_epr_pep(pr_cdcooper => pr_cdcooper
      ...
      vr_txdiaria := POWER(1 + (NVL(rw_epr_pep.txmensal, 0) / 100)
                          ,(1 / 30)) - 1;
...
BEGIN
  ...
  LOOP
    ...
    vr_indsobra := GENE0002.fn_char_para_number(vr_campo(1)) ||
                   GENE0002.fn_char_para_number(vr_campo(2));
    ...
    IF TRIM(vr_tab_contrato(vr_indsobra).contratos) IS NOT NULL THEN
      vr_tab_contrato(vr_indsobra).contratos := vr_tab_contrato(vr_indsobra)
                                                .contratos || ' , ' ||
                                                 vr_campo(3);
    ELSE
      vr_tab_contrato(vr_indsobra).contratos := vr_campo(3);
    END IF;
    vr_tab_contrato(vr_indsobra).cotas := NVL(vr_tab_contrato(vr_indsobra).cotas
                                             ,0) + NVL(GENE0002.fn_char_para_number(vr_campo(4))
                                                      ,0);
  END LOOP;
  ...
  WHILE vr_indsobra IS NOT NULL LOOP
    ...
    IF vr_cotas > 0 THEN
      FOR rw_atraso_pp IN cr_atraso_pp(pr_cdcooper => vr_tab_contrato(vr_indsobra).cdcooper
        ...
        vr_inderro := to_char(vr_seqerro,'fm0000000') || to_char(rw_atraso_pp.nrctremp,'fm0000000000');
             
        IF vr_tab_contrato_erro.exists(vr_inderro) THEN
          IF vr_tab_contrato_erro(vr_inderro).cdcooper = rw_atraso_pp.cdcooper 
            AND vr_tab_contrato_erro(vr_inderro).nrdconta = rw_atraso_pp.nrdconta 
            AND vr_tab_contrato_erro(vr_inderro).contrato = rw_atraso_pp.nrctremp THEN
            CONTINUE;
          END IF;
        END IF;
        ...
        gene0001.pc_gera_log(pr_cdcooper => rw_atraso_pp.cdcooper
                            ...
                            ,pr_dscritic => ' '
                            ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                            ,pr_dstransa => 'PAGAMENTO ATRASO - SEQ CONTRATO: ' ||
                                            vr_seq
                            ,pr_dttransa => trunc(SYSDATE)
                            ,pr_flgtrans => 1
                            ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
        ...
        IF rw_atraso_pp.inprejuz_cc = 1 THEN
          gene0001.pc_gera_log_item(pr_nrdrowid => vr_rowidlog
                                   ,pr_nmdcampo => 'Critica ao fazer Pagamento'
                                   ,pr_dsdadant => ' '
                                   ,pr_dsdadatu => 'CC em Prejuizo!');
        ...
        IF vr_cotas <= 0 THEN
          ...
          gene0001.pc_gera_log(pr_cdcooper => rw_atraso_pp.cdcooper
                              ...
                              ,pr_dscritic => ' '
                              ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                              ,pr_dstransa => 'Sem saldo para continuar pagamentos de ATRASO - SEQ CONTRATO: ' ||
                                              vr_seq
                              ,pr_dttransa => trunc(SYSDATE)
                              ,pr_flgtrans => 1
                              ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
    ...
    IF vr_cotas > 0 THEN
      vr_cursor := 'SELECT crappep.cdcooper
                             ...
                             ,crapass.inprejuz inprejuz_cc
                             , crapepr.txmensal
                             , NVL((SELECT COUNT(pep.nrparepr) FROM cecred.crappep pep
                                                  WHERE pep.cdcooper = crappep.cdcooper
                                                   AND pep.nrdconta = crappep.nrdconta
                                                   AND pep.nrctremp = crappep.nrctremp
                                                   AND pep.dtvencto >= ''' ||
      ...
      dbms_sql.define_column(vr_nrcursor, 7, vr_inprejuz_cc, 4000);
      ...
      LOOP
        ...
        ELSE
          ...
          dbms_sql.column_value(vr_nrcursor, 7, vr_inprejuz_cc);
          ...
          vr_inprejuz_cc := GENE0002.fn_char_para_number(vr_inprejuz_cc);
        
          vr_inderro := to_char(vr_seqerro,'fm0000000') || to_char(vr_nrctremp,'fm0000000000');
                   
          IF vr_tab_contrato_erro.exists(vr_inderro) THEN
            IF vr_tab_contrato_erro(vr_inderro).cdcooper = vr_cdcooper AND vr_tab_contrato_erro(vr_inderro)
               .nrdconta = vr_nrdconta AND vr_tab_contrato_erro(vr_inderro)
               .contrato = vr_nrctremp THEN
              CONTINUE;
            END IF;
          END IF;
          ...
          gene0001.pc_gera_log(pr_cdcooper => vr_cdcooper
                              ...
                              ,pr_dscritic => ' '
                              ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                              ,pr_dstransa => 'PAGAMENTO CONTRATO PLANILHA - SEQ CONTRATO: ' ||
                                              vr_seq
                              ,pr_dttransa => trunc(SYSDATE)
                              ,pr_flgtrans => 1
                              ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
          ...
          IF vr_cotas <= 0 THEN
            ...
            gene0001.pc_gera_log(pr_cdcooper => vr_cdcooper
                                ...
                                ,pr_dscritic => ' '
                                ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                                ,pr_dstransa => 'Sem saldo para continuar pagamentos de PLANILHA - SEQ CONTRATO: ' ||
                                                vr_seq
                                ,pr_dttransa => trunc(SYSDATE)
                                ,pr_flgtrans => 1
                                ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
    ...
    IF vr_cotas > 0 THEN
      FOR rw_sobra_antecipa_pp IN cr_sobra_antecipa_pp(pr_cdcooper => vr_tab_contrato(vr_indsobra).cdcooper
        ...
        vr_inderro := to_char(vr_seqerro,'fm0000000') || to_char(rw_sobra_antecipa_pp.nrctremp,'fm0000000000');
        
        IF vr_tab_contrato_erro.exists(vr_inderro) THEN
          IF vr_tab_contrato_erro(vr_inderro)
           .cdcooper = rw_sobra_antecipa_pp.cdcooper AND vr_tab_contrato_erro(vr_inderro)
             .nrdconta = rw_sobra_antecipa_pp.nrdconta AND vr_tab_contrato_erro(vr_inderro)
             .contrato = rw_sobra_antecipa_pp.nrctremp THEN
            CONTINUE;
          END IF;
        END IF;
        ...
        gene0001.pc_gera_log(pr_cdcooper => rw_sobra_antecipa_pp.cdcooper
                            ...
                            ,pr_dscritic => ' '
                            ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                            ,pr_dstransa => 'PAGAMENTO CONTRATO ANTECIPADO GERAL - SEQ CONTRATO: ' ||
                                            vr_seq
                            ,pr_dttransa => trunc(SYSDATE)
                            ,pr_flgtrans => 1
                            ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
        ...
        IF rw_sobra_antecipa_pp.inprejuz_cc = 1 THEN
          gene0001.pc_gera_log_item(pr_nrdrowid => vr_rowidlog
                                   ,pr_nmdcampo => 'Critica ao fazer Pagamento'
                                   ,pr_dsdadant => ' '
                                   ,pr_dsdadatu => 'CC em Prejuizo!');
        ...
        IF vr_cotas <= 0 THEN
          ...
          gene0001.pc_gera_log(pr_cdcooper => rw_sobra_antecipa_pp.cdcooper
                              ...
                              ,pr_dscritic => ' '
                              ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                              ,pr_dstransa => 'Sem saldo para continuar pagamentos de ANTECIPACAO GERAL - SEQ CONTRATO: ' ||
                                              vr_seq
                              ,pr_dttransa => trunc(SYSDATE)
                              ,pr_flgtrans => 1
                              ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
    ...
    gene0001.pc_gera_log(pr_cdcooper => vr_tab_contrato(vr_indsobra).cdcooper
                        ...
                        ,pr_dscritic => ' '
                        ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                        ,pr_dstransa => 'Pagamento Total - SEQ CONTRATO: ' ||
                                        vr_seq
                        ,pr_dttransa => trunc(SYSDATE)
                        ,pr_flgtrans => 1
                        ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
    ...
    gene0001.pc_gera_log(pr_cdcooper => vr_tab_contrato(vr_indsobra).cdcooper
                        ...
                        ,pr_dscritic => ' '
                        ,pr_dsorigem => GENE0001.vr_vet_des_origens(7)
                        ,pr_dstransa => 'Sobrou de COTAS Final - SEQ CONTRATO: ' ||
                                        vr_seq
                        ,pr_dttransa => trunc(SYSDATE)
                        ,pr_flgtrans => 1
                        ,pr_hrtransa => to_char(SYSDATE, 'SSSSS')
   ...
   pc_lancamento_capital(pr_cdcooper => vr_tab_contrato(vr_indsobra).cdcooper
                        ,pr_nrdconta => vr_tab_contrato(vr_indsobra).nrdconta
                        ,pr_vllanmto => vr_tab_contrato(vr_indsobra).cotas
                        ,pr_dtmvtolt => rw_crapdat.dtmvtolt
                        ,pr_cdcritic => vr_cdcritic
                        ,pr_dscritic => vr_dscritic);
    ...
    IF vr_cdcritic IS NOT NULL OR TRIM(vr_dscritic) IS NOT NULL THEN
      CECRED.pc_log_programa(pr_dstiplog      => 'O' 
                            ...
                            ,pr_cdmensagem    => 444
                            ,pr_cdprograma    => pr_cdprogcotas
                            ,pr_cdcooper      => vr_tab_contrato(vr_indsobra).cdcooper
                            ,pr_idprglog      => vr_idprglog);      
      ROLLBACK;
  ...
  FOR rec IN (SELECT cop.cdcooper 
                FROM cecred.crapcop cop 
               WHERE cop.flgativo = 1 
                 AND cop.cdcooper = NVL(vr_cooppreju,cop.cdcooper) 
                 AND cop.cdcooper NOT IN (1,3,7,10,12)) LOOP
    
    cecred.pc_crps770(pr_cdcooper => rec.cdcooper 
                    , pr_cdagenci => 0 
                    , pr_nmdatela => 'CRPS770' 
