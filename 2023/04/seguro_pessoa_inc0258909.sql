DECLARE
  ...
  vr_nmdir               VARCHAR2(4000) := CECRED.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/INC0258909';
  vr_nmarq               VARCHAR2(100)  := 'ROLLBACK_INC0258909.sql';
  vr_ind_arq             utl_file.file_type;
  vr_linha               VARCHAR2(32767);
  vr_flentrou            VARCHAR2(1);
  ...
  CURSOR cr_crapcop IS
    SELECT c.cdcooper
     ...
     WHERE c.flgativo = 1
       AND c.cdcooper <> 3
       AND EXISTS (SELECT 1
                     FROM  CECRED.crapepr e,  CECRED.tbseg_prestamista p
                    WHERE p.tpcustei = 1
                      AND p.vldevatu = 0
                      AND p.tpregist = 1
                      AND e.inliquid = 0
                      ...
                      AND p.nrproposta IN ('770613642803',
                                           '770657271276',
                                           '770658248324',
                                           '770658441060',
                                           '770658256955',
                                           ...
                                           '770658500562')
                      AND e.vlsdeved >= CASE
                            WHEN p.cdcooper = 1 THEN
                             40000
                            WHEN p.cdcooper = 5 THEN
                             10000
                            WHEN p.cdcooper = 9 THEN
                             999999999.99
                            WHEN p.cdcooper = 16 THEN
                             19999.99
                            ELSE
                             0.01
                          END);

  CURSOR cr_prestamista(pr_cdcooper IN NUMBER,
    ...
    FOR rw_crapcop IN cr_crapcop LOOP
      ...
      FOR rw_prestamista IN cr_prestamista(pr_cdcooper => rw_crapcop.cdcooper,
        ...
        UPDATE CECRED.tbseg_prestamista p
           SET p.vldevatu = rw_prestamista.vldevatu
         WHERE p.idseqtra = rw_prestamista.idseqtra;
        ...
        vr_flgnvenv   := FALSE;
        vr_ultimoDia :=  ADD_MONTHS(TRUNC(rw_prestamista.dtinivig,'MONTH'),1)-1;
      ...
      vr_nmarqdat        := vr_dtmvtolt_yymmdd||'_'||lpad(rw_crapcop.cdcooper,2,0)||'_PRESTAMISTA_COMPLEMENTAR.txt';
      vr_linhadet        := '';
      ...
      IF vr_tab_lancarq_815.count > 0 THEN
         ...
         vr_nmarqnov := vr_dtmvtolt_yymmdd||'_'||LPAD(TO_CHAR(rw_crapcop.cdcooper),2,0)||'_PRESTAMISTA_COMPLEMENTAR.txt';
