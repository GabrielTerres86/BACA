            ...
            IF cr_nrproposta%FOUND THEN
               BEGIN
                ...
                EXCEPTION
                  WHEN OTHERS THEN
                    vr_cdcritic := 0;
                    vr_dscritic := 'Erro ao atualizar numero de proposta já atualizada - cdcooper: ' || rw_crapcop.cdcooper || ' - nrdconta: ' || rw_prestamista.nrdconta || ' - nrproposta: ' || vr_NRPROPOSTA || ' - '  || SQLERRM;
                    RAISE vr_exc_saida;
                END;
            END IF;
          CLOSE cr_nrproposta;
