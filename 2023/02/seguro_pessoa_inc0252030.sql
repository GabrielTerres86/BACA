DECLARE
  ...
  vr_nmdir          VARCHAR2(4000) := CECRED.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/INC0252030';
  vr_nmarq          VARCHAR2(100)  := 'ROLLBACK_INC0252030.sql';
      ...
      FOR rw_crapcop IN cr_crapcop LOOP
        ...
        FOR rw_principal IN cr_principal(pr_cdcooper => rw_crapcop.cdcooper) LOOP
          ...
          vr_linha :=    ' UPDATE CECRED.craplau p   '
                      || '    SET p.insitlau = ''' || rw_principal.insitlau || ''''
                      || '       ,p.nrcrcard = ''' || REPLACE(rw_principal.nrcrcard,',','.') || ''''
                      || '       ,p.dtdebito = ''' || rw_principal.dtdebito || ''''
