DECLARE
  ...
  vr_nmdir          VARCHAR2(4000) := CECRED.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/PRB0047827';
  vr_nmarq          VARCHAR2(100)  := 'ROLLBACK_PRB0047827.sql';
  ...
  CURSOR cr_principal(pr_cdcooper CECRED.crapseg.cdcooper%TYPE) IS
    SELECT w.cdcooper
     ...
     WHERE w.cdcooper = pr_cdcooper
       ...
       AND e.inliquid = 0
       AND NOT EXISTS (SELECT p.nrproposta
  ...
  BEGIN
      ...
      FOR rw_crapcop IN cr_crapcop LOOP
        ...
        FOR rw_principal IN cr_principal(pr_cdcooper => rw_crapcop.cdcooper) LOOP
          ...
          FOR rw_crawseg IN cr_crawseg(pr_cdcooper => rw_principal.cdcooper
                                      ,pr_nrdconta => rw_principal.nrdconta
                                      ,pr_nrctrseg => rw_principal.nrctrseg) LOOP
            ...
            vr_linha := 'UPDATE CECRED.crawseg w ' ||
                        '    SET w.nrproposta = ''' || rw_crawseg.nrproposta || '''' ||
          ...
          FOR rw_crapseg IN cr_crapseg(pr_cdcooper => rw_principal.cdcooper
                                      ,pr_nrdconta => rw_principal.nrdconta
                                      ,pr_nrctrseg => rw_principal.nrctrseg) LOOP
