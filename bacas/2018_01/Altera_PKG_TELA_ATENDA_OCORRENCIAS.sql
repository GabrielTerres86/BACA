...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_ATENDA_OCORRENCIAS IS
  ...
  ---------------------------------------------------------------------------------------------------------------
	...
	FUNCTION busca_rating(pr_cdcooper NUMBER, pr_nrdconta NUMBER) 
	...
	FUNCTION busca_riscos_contrato(pr_cdcooper NUMBER, pr_nrdconta NUMBER, pr_nrctremp NUMBER, pr_dtmvtoan DATE) 
	...
	BEGIN
	   ...
	   RETURN rw_risco;
	END busca_riscos_contrato;
	
	FUNCTION busca_risco_agravado(pr_cdcooper NUMBER, pr_nrdconta NUMBER) 
	...
	BEGIN
		...
		RETURN vr_risco_agr;         
	END busca_risco_agravado;
	
	FUNCTION busca_grupo_economico(pr_cdcooper NUMBER, pr_nrdconta NUMBER, pr_nrcpfcgc NUMBER) 
	...
	BEGIN
     ...
     WHERE aux.cdcooper = pr_cdcooper
	  ...
	  RETURN rw_grp;
	END busca_grupo_economico;
	
	FUNCTION calcula_risco_atraso(qtdiaatr NUMBER) 
  ...
  BEGIN
      risco_atraso :=  CASE WHEN qtdiaatr <   15 THEN 'A'
                            ...
                            ELSE 'H' END;
			RETURN risco_atraso;
	END calcula_risco_atraso;
	
	PROCEDURE busca_risco_ult_central(pr_cdcooper          IN crawepr.cdcooper%TYPE
			  ...
			  WHEN NO_DATA_FOUND THEN
				  SELECT uc.innivris, uc.dtdrisco, uc.vldivida
					 ...
					 WHERE uc.cdcooper = pr_cdcooper
						 ...
						 AND ROWNUM = 1;
			END;
	END busca_risco_ult_central;
	
	FUNCTION traduz_risco(innivris NUMBER) RETURN crawepr.dsnivris%TYPE AS dsnivris crawepr.dsnivris%TYPE;
	BEGIN
	...
	PROCEDURE monta_reg_conta_xml(pr_retxml       IN OUT NOCOPY XMLType
	...
	BEGIN
          ...
          gene0007.pc_insere_tag(pr_xml      => pr_retxml,
                             ...
                             pr_des_erro => pr_dscritic);
	END monta_reg_conta_xml;
	
	PROCEDURE monta_reg_central_risco(pr_retxml           IN OUT NOCOPY XMLType
															     ,pr_dscritic         IN OUT VARCHAR2
    ...
    BEGIN
      ...
      FOR rw_outras_contas_do_titular 
       IN cr_outras_contas_do_titular(rw_cbase) LOOP
          ...
          rw_grp := busca_grupo_economico(rw_outras_contas_do_titular.cdcooper
                                        , rw_outras_contas_do_titular.nrdconta
                                        , rw_outras_contas_do_titular.nrcpfcgc);
          
          -- Busca o rating da conta
          vr_rating := busca_rating(rw_outras_contas_do_titular.cdcooper
                                  , rw_outras_contas_do_titular.nrdconta);               
          
          -- Busca os riscos para o contrato
          rw_risco := busca_riscos_contrato(rw_outras_contas_do_titular.cdcooper
                                          , rw_outras_contas_do_titular.nrdconta
                                          , rw_outras_contas_do_titular.nrctremp
                                          , rw_dat.dtmvtoan);

          -- Busca o risco agravado para a conta
          vr_risco_agr := busca_risco_agravado(rw_outras_contas_do_titular.cdcooper
                                             , rw_outras_contas_do_titular.nrdconta);
           
          -- Adiciona registro para a conta no XML de retorno
          monta_reg_conta_xml(pr_retxml
                            , vr_auxconta
                            , vr_dscritic
                            , rw_outras_contas_do_titular.nrdconta
                            , rw_outras_contas_do_titular.nrcpfcgc
                            ...
                            , rw_grp.dsdrisgp
                            , vr_rating
                            , calcula_risco_atraso(rw_risco.qtdiaatr)
                            , traduz_risco(vr_risco_agr)
      ...
      FOR rw_contas_grupo_economico 
        ...
        -- Busca o rating da conta
          vr_rating := busca_rating(rw_contas_grupo_economico.cdcooper
                                  , rw_contas_grupo_economico.nrdconta);               
          
          -- Busca os riscos para o contrato
          rw_risco := busca_riscos_contrato(rw_contas_grupo_economico.cdcooper
                                          , rw_contas_grupo_economico.nrdconta
                                          , rw_contas_grupo_economico.nrctremp
                                          , rw_dat.dtmvtoan);

          -- Busca o risco agravado para a conta
          vr_risco_agr := busca_risco_agravado(rw_contas_grupo_economico.cdcooper
                                             , rw_contas_grupo_economico.nrdconta);
      ...
      END LOOP;
			...
			busca_risco_ult_central(pr_cdcooper
			                      , pr_nrdconta
														, rw_dat.dtultdma
      ...
      monta_reg_central_risco(pr_retxml
                            , vr_dscritic
                            , traduz_risco(vr_risco_ult_central)
