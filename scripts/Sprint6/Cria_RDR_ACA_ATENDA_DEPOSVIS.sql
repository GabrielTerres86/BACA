...
begin
  ...
  -- Mensageria
  OPEN cr_craprdr(pr_nmprogra => 'TELA_ATENDA_DEPOSVIS');
  ...
  IF cr_craprdr%NOTFOUND THEN
    ...
    INSERT INTO craprdr(nmprogra
                       ,dtsolici) 
                 values('TELA_ATENDA_DEPOSVIS'
                       ,SYSDATE);
    -- Posiciona no registro criado
    OPEN cr_craprdr(pr_nmprogra => 'TELA_ATENDA_DEPOSVIS');
    FETCH cr_craprdr INTO rw_craprdr;

    dbms_output.put_line('Insere CRAPRDR -> TELA_ATENDA_DEPOSVIS: ' || rw_craprdr.nrseqrdr);
  ...
  OPEN cr_crapaca(pr_nmdeacao => 'BUSCA_SALDOS_DEVEDORES'
                 ,pr_nmpackag => 'TELA_ATENDA_DEPOSVIS'
                 ,pr_nmproced => 'pc_busca_saldos_devedores'
  ...
  IF cr_crapaca%NOTFOUND THEN
    ...
    INSERT INTO crapaca(nmdeacao, 
                        ...
                        lstparam, 
                        nrseqrdr) 
                 VALUES('BUSCA_SALDOS_DEVEDORES',
                        'TELA_ATENDA_DEPOSVIS',
                        'pc_busca_saldos_devedores',
                        'pr_nrdconta',
                        rw_craprdr.nrseqrdr);
                             
    dbms_output.put_line('Insere CRAPACA -> BUSCA_SALDOS_DEVEDORES -> TELA_ATENDA_DEPOSVIS.pc_busca_saldos_devedores');
