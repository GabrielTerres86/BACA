...
<form id="frmRating" name="frmRating" class="formulario" style="display:none;">
	...
	<fieldset id="fsetDetalhesNota" name="fsetDetalhesNota" style="padding:0px; margin:0px; padding-bottom:10px;">
		
		<legend>Dados</legend>
		
		<div id="divDados">
		
			<label for="vlrdnota"><? echo utf8ToHtml('Nota:') ?></label>
			<input id="vlrdnota" name="vlrdnota" type="text" value="<?echo $vlrdnota;?>"></input>
			
			<br /> 

			<label for="nivrisco"><? echo utf8ToHtml('Risco:') ?></label>
			<input id="nivrisco" name="nivrisco" type="text" value="<?echo $nivrisco;?>"></input>
			
			<br /> 

			<label for="datatual"><? echo utf8ToHtml('Data:') ?></label>
			<input id="datatual" name="datatual" type="text" value="<?echo $datatual;?>"></input>
						...
						<? for($i = 0; $i < count($registros); $i++){
							...
							for($j = 0; $j < count($registros[$i]->tags); $j++){?>
								...
								<script type="text/javascript">
									...
									regRating["reg_craptor"] = "<? echo getByTagName($registros[$i]->tags[$j]->tags,'reg_craptor');?>";
									regRating["reg_crapitr"] = "<? echo getByTagName($registros[$i]->tags[$j]->tags,'reg_crapitr');?>";
									regRating["reg_crapsir"] = "<? echo getByTagName($registros[$i]->tags[$j]->tags,'reg_crapsir');?>";
						...
						<td>
							<? if($qtregist > ($nriniseq + $nrregist - 1)) { ?>
								  ...
								  <script type="text/javascript">									
									...
									$('#btConcluir','#divBotoesRating').css('display','none');									
							...
							<? }else{ ?>
									<script type="text/javascript">
										...
										if('<?echo $cddopcao;?>' == 'A'){
											$('#btConcluir','#divBotoesRating').css('display','visible');
										}else{
...
<div id="divBotoesRating" style='text-align:center; margin-bottom: 10px; margin-top: 10px; display:none;' >
...
<script type="text/javascript">
	...
	<?if($cddopcao == "C"){?>
		$("#flgativo","#divRatings").prop("disabled","true");
	<?}?> 
	
	$('a.paginacaoAnt').unbind('click').bind('click', function() {
		buscaRatings( <? echo "'".($nriniseq - $nrregist)."','".$nrregist."'"; ?> );
    });
	...
	formataFormularioRating();
	formataTabelaRatings();
	$('#divTabela').css('display','block');	
	$('#divBotoesFiltro').css('display','none');
	$('#divBotoesRating').css('display','block');
    $('#btVoltar','#divBotoesRating').css('visible');	
