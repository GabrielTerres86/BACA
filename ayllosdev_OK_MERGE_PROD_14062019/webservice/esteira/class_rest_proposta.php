...
class RestProposta extends RestServerJson{
    ...
    public function __construct(){
        $this->aParamsRequired = array('cdcooper','nrdconta','nrctremp','insitapr','dsobscmt','dsscore','dtscore','tpprodu');
    }
    ...
    private function validaParametrosRecebidos($oDados){
        ...
        if (!is_object($oDados)){
            $this->processaRetornoErro(412,'Parecer da proposta nao foi atualizado, parametros incorretos.');
            return false;
        }
    ...
    public function processaRequisicao(){
        try{
            ...
            $xml .= "   <cdcooper>".$oDados->cdcooper."</cdcooper>";
            $xml .= "   <nrdconta>".$oDados->nrdconta."</nrdconta>";
            $xml .= "   <nrctremp>".$oDados->nrctremp."</nrctremp>";
            $xml .= "   <insitapr>".$oDados->insitapr."</insitapr>";
            $xml .= "   <dsobscmt>".$oDados->dsobscmt."</dsobscmt>";
            $xml .= "   <dsdscore>".$oDados->dsscore."</dsdscore>";
            $xml .= "   <dtdscore>".$oDados->dtscore."</dtdscore>";
            $xml .= "   <tpprodut>".$oDados->tpprodu."</tpprodut>";
            $xml .= "   <dsrequis>".$this->getFileContents()."</dsrequis>";
            $xml .= "   <namehost>".$this->getNameHost()."</namehost>";
            $xml .= " </Dados>";
            $xml .= "</Root>";
            $sXmlResult = mensageria($xml, "WEBS0001", "WEBS0001_ATUARETORN_PROPOSTA_ESTEIRA", 0, 0, 0, 5, 0, "</Root>");
