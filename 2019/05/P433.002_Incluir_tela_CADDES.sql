...
DECLARE
  ...
  aux_rsprogra  VARCHAR2(50) := 'CADASTRO DE DESENVOLVEDORES';
  aux_nmdatela  VARCHAR2(50) := 'CADDES';
  aux_dsprogra  VARCHAR2(50) := 'CADASTRO DE DESENVOLVEDORES';
  aux_cdopcoes  VARCHAR2(50) := '@,A,I,C,E,U';
  aux_dsopcoes  VARCHAR2(90) := 'ACESSO,ALTERACAO,INCLUSAO,CONSULTA,EXCLUSAO,GERENCIA UUID';
  ...
  FOR rw_crapcop IN cr_crapcop LOOP
    ...
    BEGIN
      SELECT NVL(MAX(prg.nrordprg),0) + 1 
       ...
       WHERE prg.cdcooper = rw_crapcop.cdcooper
         AND prg.nrsolici = 50;
    EXCEPTION
      WHEN no_data_found THEN  -- Se o select não encontrar registros
        aux_nrordprg := 1;
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20000,'Erro ao buscar NRORDPRG: '||SQLERRM);
    END;
         
    -- Inserir a tela TIPCTA na tabela de programas
    BEGIN
    ...
    EXCEPTION
      WHEN dup_val_on_index THEN
        RAISE_APPLICATION_ERROR(-20001,'Programa já cadastrado: '||SQLERRM);
      WHEN OTHERS THEN
