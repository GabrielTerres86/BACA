    ...
    DECLARE
      ...
      CURSOR cr_crapris_microcredito(pr_cdcooper IN crapris.cdcooper%TYPE      --> Código da cooperativa
        ...
        SELECT cr.nrdconta          -- Mesmos campos do crps660 para criar o registro de saida
              ...
              ,cr.dsinfaux
              ,cr.rowid
        FROM craplcr l, crapepr e, crapris cr
        ...
        WHERE cr.cdcooper = pr_cdcooper
          ...
          AND l.cdmodali = 04
          AND l.cdsubmod = 03
          AND (LPAD(l.cdcooper,3,'0') || LPAD(l.cdlcremp,5,'0') ) IN 
                     ( 00100401,00100402,00100403,00100411,00100416,00100464,00100546
                      ,00100551,00100564,00100836,00100837,00100839,00101403,00101411
                      ,00101416,00101546,00101551,00102403,00102546,00103403,00200907
                      ,00200908,00200909,00200910,00200911,00200912,00200952,01600401
                      ,01600402,01600403,01600464,01600564,01601100,01601403,01602100
                      ,01602403)
    ...
    BEGIN
      ...
      FOR rw_crapris IN cr_crapris_microcredito(pr_cdcooper => pr_cdcooper
        ...
        IF NOT vr_fltemris THEN
          ...
          -- Para contratos das linhas anexo a estoria
          -- devem ter seu motivo de saida 0307 - Troca de Modalidade
