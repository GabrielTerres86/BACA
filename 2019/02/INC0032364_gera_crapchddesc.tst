...
DECLARE 
    ...
    vr_nrdconta          crapchd.nrdconta%TYPE;
    vr_nrctachq          crapchd.nrdconta%TYPE;
  ...
  BEGIN
    ...
    FOR rw_crapcst IN (SELECT * FROM CRAPCST a
     ...
     -- exit;
    END LOOP; -- Loop Custodi
    
    vr_cdcooper := 0;
    vr_nrdconta := 0;
    tab_vlchqmai := 0;
  
    vr_comando := '--- Cheques descontados ---'; 
          
    pc_escreve_xml(vr_comando || chr(10));    
    -- Cheque descontados
    FOR rw_crapcdb IN (SELECT * FROM CRAPCDB a
                        WHERE NOT EXISTS (SELECT 1 FROM crapchd b
                                            WHERE a.cdcooper = b.cdcooper
                                              AND a.cdcmpchq = b.cdcmpchq
                                              AND a.cdbanchq = b.cdbanchq
       ...
       IF vr_cdcooper <> rw_crapcdb.cdcooper THEN
    ...
    END IF;
      ...
      if vr_inserir then
        ...
        cheq0001.pc_dig_cmc7(rw_crapcdb.dsdocmc7, vr_nrdcampo, vr_lsdigctr);  
         vr_nrddigv1 := TO_NUMBER(TO_CHAR(gene0002.fn_busca_entrada(1,vr_lsdigctr,',')));   
         vr_nrddigv2 := TO_NUMBER(TO_CHAR(gene0002.fn_busca_entrada(2,vr_lsdigctr,','))); 
         vr_nrddigv3 := TO_NUMBER(TO_CHAR(gene0002.fn_busca_entrada(3,vr_lsdigctr,','))); 
       
         IF   tab_vlchqmai <= rw_crapcdb.vlcheque THEN
           aux_tpdmovto := 1;
        ELSE        
           aux_tpdmovto := 2;
        END IF;  
     ...
     -- Cheques acolhidos para depositos nas contas dos associados.
      INSERT INTO crapchd (cdagechq
                   ...
                   VALUES (rw_crapcdb.cdagechq
                          ...
                          ,1  -- Situacao atual
                          ,rw_crapcdb.dsdocmc7
                          ,gene0005.fn_valida_dia_util(rw_crapcdb.cdcooper, rw_crapcdb.dtlibera)
                          ,rw_crapcdb.inchqcop
                          ,rw_crapcdb.insitchq
                          ,rw_crapcdb.nrcheque
                          ,vr_nrctachq
                          ...
                          ,aux_tpdmovto  -- Tipo de movimento para troca eletronica de documentos
                          ,rw_crapcdb.vlcheque
                          ,rw_crapcdb.cdcooper) RETURNING ROWID INTO vr_rowidchd; 
