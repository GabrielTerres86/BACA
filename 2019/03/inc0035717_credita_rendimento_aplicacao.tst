...
declare 
  ...
  pr_dtiniano date := to_date('23/11/2018','dd/mm/rrrr');
...
begin
  ...
  FOR rw_craprda IN cr_craprda LOOP
    ...
    rw_craplap.txaplica := 108;
    
    IF rw_craprda.tpaplica = 8 THEN
      /* RDC POS */
      vr_cdhistor := 529;
      
      vr_vlrendim := rw_craprda.vlaplica + (rw_craprda.vlaplica * vr_cdidiari * rw_craplap.txaplica / 100 / 100);
      vr_dia      := pr_dtiniano;
      LOOP      
    ...
    vr_vltotal := vr_vlrendim - rw_craprda.saldo_cal_2018;
