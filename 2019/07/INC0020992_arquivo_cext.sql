  ...
  BEGIN
    ...
    DECLARE
     ...
     rw_craphis cr_craphis%ROWTYPE;
      ...
      vr_hora NUMBER;
      --UPPER(TPMENSAG), NRNSUCAP, DTDTRGMT, HRDTRGMT, CDCOOPER, NRDCONTA
      CURSOR cr_crapdcb_1 (pr_cdcooper IN crapdcb.cdcooper%TYPE
                          ,pr_nrdconta IN crapdcb.nrdconta%TYPE 
                          ,pr_dtdtrgmt IN crapdcb.dtdtrgmt%TYPE
                          ,pr_nrnsucap IN crapdcb.nrnsucap%TYPE
                          ,pr_tpmansag IN crapdcb.tpmensag%TYPE) IS  
       SELECT MAX(c.hrdtrgmt) + 1 AS hora FROM crapdcb c
        WHERE c.cdcooper = pr_cdcooper
          AND c.nrdconta = pr_nrdconta
          AND c.dtdtrgmt = pr_dtdtrgmt
          AND c.nrnsucap = pr_nrnsucap
          AND upper(c.tpmensag) = upper(pr_tpmansag);
                  ...
                  EXCEPTION
                    WHEN OTHERS THEN
                      ...
                      BEGIN
                       ...
                       -- buscar max hora e incrementar + 1 para não ocorrer chave duplicada
                       -- UPPER(TPMENSAG), NRNSUCAP, DTDTRGMT, HRDTRGMT, CDCOOPER, NRDCONTA                       
                          
                       OPEN cr_crapdcb_1 (vr_tab_crapdcb(vr_idxdcb).det.cdcooper, 
                                          vr_tab_crapdcb(vr_idxdcb).det.nrdconta,
                                          vr_tab_crapdcb(vr_idxdcb).det.dtdtrgmt,
                                          vr_tab_crapdcb(vr_idxdcb).det.nrnsucap,
                                          vr_tab_crapdcb(vr_idxdcb).det.tpmensag);
                       FETCH cr_crapdcb_1 INTO vr_hora;   
                       CLOSE cr_crapdcb_1;                     
                    ...
                    VALUES
                     ...
                     EXCEPTION 
                         WHEN OTHERS THEN
                             vr_dscritic := 'Erro ao inserir crapdcb dub_val: '||SQLERRM 
                                            || 'nsu: '|| vr_tab_crapdcb(vr_idxdcb).det.nrnsucap
                                            || 'cop: '||vr_tab_crapdcb(vr_idxdcb).det.cdcooper
                                            || 'con: ' ||vr_tab_crapdcb(vr_idxdcb).det.nrdconta;
                             dbms_output.put_line(vr_dscritic);
...
begin
  ...
  pc_proces_arq_cet_bancoob(pr_cdcooper => 3,
                            pr_dtmvtolt => to_date('17/07/2019','dd/mm/yyyy'),
