...
PROCEDURE pc_crps388_i(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Cooperativa solicitada
  ...
  BEGIN
    ...
    -- datas
    rw_crapdat.dtmvtolt := to_date('11062019','ddmmyyyy');
    rw_crapdat.dtmvtopr := to_date('12062019','ddmmyyyy'); 
    rw_crapdat.dtmvtoan := to_date('10062019','ddmmyyyy'); 
    rw_crapdat.dtmvtocd := to_date('11062019','ddmmyyyy');
  ...
  EXCEPTION
    ...
    WHEN vr_excsaida THEN
      -- Se foi retornado apenas códigoa
      IF vr_cdcritic > 0 AND vr_dscritic IS NULL THEN
        -- Buscar a descrição
        vr_dscritic := gene0001.fn_busca_critica(vr_cdcritic);
      END IF;
