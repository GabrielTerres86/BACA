...
declare 
  ...
  vr_nmarqimp        VARCHAR2(100)  := 'demais-log.txt';
  vr_nmarqimp2       VARCHAR2(100)  := 'demais-sucesso.txt';
  vr_nmarqimp3       VARCHAR2(100)  := 'demais-falha.txt';
  vr_nmarqimp4       VARCHAR2(100)  := 'demais-backup.txt';
  ...
  select ass.cdcooper
        ...
        ,ass.inpessoa
    from crapass ass
   where ass.cdcooper in (select cdcooper from crapcop where cdcooper not in (1,3) and flgativo = 1)
     and exists (select 1 
                   from craprac rac
  ...
  select lac.cdcooper, lac.nrdconta, lac.dtmvtolt, lac.vllanmto
   ...
   where rac.cdcooper = lac.cdcooper
     ...
     and rac.nrctrrpp > 0
     and lac.cdcooper in (select cdcooper from crapcop where cdcooper not in (1,3) and flgativo = 1)
