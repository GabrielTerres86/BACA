declare 
  ...
  cursor cr_usuario is
    select decode(a.inpessoa,1,lpad(a.nrcpfcgc,11,'0'),lpad(a.nrcpfcgc,14,'0')) login
     ...
     where a.cdcooper=2
       and a.nrdconta in (269808, 382485);
     
  cursor cr_valida (pr_dslogin in tbepr_cdc_usuario.dslogin%type
                   ,pr_idcooperado_cdc in tbepr_cdc_usuario_vinculo.idcooperado_cdc%type) is
    select *
      from tbepr_cdc_usuario u
      join tbepr_cdc_usuario_vinculo v on (u.idusuario=v.idusuario and v.idcooperado_cdc=pr_idcooperado_cdc)
...
begin
  ...
  for rw_usuario in cr_usuario loop
    ...
    OPEN cr_valida  (pr_dslogin => rw_usuario.login
                    ,pr_idcooperado_cdc => rw_usuario.idcooperado_cdc);
    ...
    IF cr_valida%NOTFOUND THEN
      ...
      begin
        ...
        update tbepr_cdc_usuario u set u.flgativo=0 where u.dslogin=rw_usuario.login and u.idusuario!=vr_idusuario;
        
        commit;
        
        dbms_output.put_line ('Usuário criado com sucesso!');
