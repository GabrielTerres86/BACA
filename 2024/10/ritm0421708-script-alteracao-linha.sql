DECLARE
   ...
   vr_aux_diretor     VARCHAR2(100) := 'RITM0421708';     
    ...
    LOOP 
      BEGIN
        ...
        BEGIN
           ...
           vr_aux_cddlinha := TO_NUMBER(vr_vet_campos(4));
        EXCEPTION
          WHEN OTHERS THEN
            gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_handle_log
        ...
        IF vr_tab_carga.exists(vr_idx1) THEN
         ...
         ELSE  
             ...
             IF cr_conta_ambiente%NOTFOUND THEN
                ...
                gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_handle_log
                                              ,pr_des_text => 'Coop: ' || vr_tab_carga(vr_idx1).cdcooper ||' - Contrato nao encontrado: ' || vr_tab_carga(vr_idx1).nrctrlim);
                CONTINUE;
             END IF;
             CLOSE cr_conta_ambiente;
