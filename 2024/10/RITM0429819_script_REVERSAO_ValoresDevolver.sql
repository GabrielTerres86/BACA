DECLARE
  ...
  vc_cdcooper       CONSTANT NUMBER := 1;
  vc_cdhistor_PF    CONSTANT NUMBER := 4610;
  vc_cdhistor_PJ    CONSTANT NUMBER := 4611;
  vc_cdhistor_caixa CONSTANT NUMBER := 4612;
  ...
  CURSOR cr_retorno_full IS
    SELECT dev.ROWID dsdrowid
         ...
         , 0 iddifval
      FROM contacorrente.tbcc_controle_devolucao dev
...
BEGIN
  ...
  FOR coop IN cd_cooperativa LOOP
    ...
    vr_nrseqdig := fn_sequence(pr_nmtabela => 'CRAPLOT'
                              ...
                              ,pr_flgdecre => 'N');
    BEGIN
      INSERT INTO cecred.craplcx(cdagenci
                         ,cdhistor
                         ,cdopecxa
                         ,dtmvtolt
                         ,nrdcaixa
    ...
    FOR ind IN vr_tbregistros.FIRST..vr_tbregistros.LAST LOOP
      ...
      IF vr_tbregistros(ind).iddifval = 0 THEN
        ...
        BEGIN
          DELETE contacorrente.tbcc_controle_devolucao
           WHERE ROWID = vr_tbregistros(ind).dsdrowid;
        EXCEPTION
          WHEN OTHERS THEN
            raise_application_error(-20022,'Erro ao excluir tbcc_controle_devolucao('||vr_tbregistros(ind).cdcooper||'/'||vr_tbregistros(ind).nrdconta||'): '||SQLERRM);
        END;
      ELSE
        BEGIN
          UPDATE cecred.tbcotas_devolucao dev
        ...
        BEGIN
          UPDATE contacorrente.tbcc_controle_devolucao t
             SET t.vldsaldo = t.vldsaldo - vr_tbregistros(ind).vldsaldo
           WHERE ROWID      = vr_tbregistros(ind).dsdrowid;
        EXCEPTION
          WHEN OTHERS THEN
            raise_application_error(-20002,'Erro ao incluir tbcc_controle_devolucao('||vr_tbregistros(ind).cdcooper||'/'||vr_tbregistros(ind).nrdconta||'): '||SQLERRM);
        END;
      END IF;
