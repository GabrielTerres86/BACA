  ...
  CURSOR cr_tbrecup_acordo_contrato IS
    SELECT b.nracordo
      ...
      FROM cecred.tbrecup_acordo          a
          ...
          ,cecred.tbdsct_titulo_cyber     c
     WHERE a.nracordo = b.nracordo
       AND a.cdcooper = c.cdcooper
       AND a.nrdconta = c.nrdconta
       AND b.nrctremp = c.nrctrdsc
       AND a.cdsituacao = 1
       AND (a.nracordo, c.nrborder) IN ((652604, 24274), (652290, 248645))
     ORDER BY a.cdcooper
             ,a.nrdconta;
...
BEGIN
  ...
  sistema.escreveLinhaArquivo(pr_utlfileh => vr_arqrollback, pr_des_text => 'BEGIN');
  ...
  LOOP
    ...
    FOR vr_indice IN vr_tbrecup_acordo_contrato.first .. vr_tbrecup_acordo_contrato.last LOOP
      vr_rollback := 'UPDATE cecred.tbrecup_acordo_contrato' || ' SET cdmodelo       = ' || vr_tbrecup_acordo_contrato(vr_indice).cdmodelo ||
      ...
      sistema.escreveLinhaArquivo(pr_utlfileh => vr_arqrollback, pr_des_text => vr_rollback);
    END LOOP;
      ...
      WHEN OTHERS THEN
        FOR vr_indice IN 1 .. SQL%bulk_exceptions.count LOOP
          vr_log := 'nracordo: ' || vr_tbrecup_acordo_contrato(SQL%BULK_EXCEPTIONS(vr_indice).error_index).nracordo ||
                    ...
                    ' Erro: ' || SQLERRM;
          sistema.escreveLinhaArquivo(pr_utlfileh => vr_arqlog, pr_des_text => vr_log);
        END LOOP;
    END;
    COMMIT;
  END LOOP;
