DECLARE
  ...
  vr_nmdir          VARCHAR2(4000) := CECRED.gene0001.fn_param_sistema('CRED',3,'ROOT_MICROS')||'cpd/bacas/ritm0365001';
  vr_nmarq          VARCHAR2(100)  := 'ROLLBACK_ritm0365001.sql';
  ...
  CURSOR cr_principal(pr_cdcooper CECRED.crapseg.cdcooper%TYPE) IS
    SELECT x.*,xp.idseqtra,xw.progress_recid crawseg_progress_recid,xs.progress_recid crapseg_progress_recid
     FROM (SELECT p.cdcooper,p.nrdconta,p.nrctremp, max(p.nrctrseg) nrctrseg
          ...
          CECRED.crapseg xs,
          CECRED.crawseg xw,
          CECRED.tbseg_prestamista xp
    WHERE x.cdcooper = xp.cdcooper
      AND x.nrdconta = xp.nrdconta
      AND x.nrctrseg = xp.nrctrseg
      AND x.nrctremp = xp.nrctremp
      AND x.cdcooper = xw.cdcooper
      AND x.nrdconta = xw.nrdconta
      AND x.nrctrseg = xw.nrctrseg
      AND x.cdcooper = xs.cdcooper
      AND x.nrdconta = xs.nrdconta
      AND x.nrctrseg = xs.nrctrseg;
