declare 
  ...
  cursor cr_crapcar IS
     SELECT crd.rowid, crd.nrconta_cartao, crd.dtmvtolt, crd.cdcooper, crd.nrdconta, 
           (select car.cdcooper from cecred.tbcrd_conta_cartao car where  crd.nrconta_cartao = car.nrconta_cartao) coop_certa,
           (select car.nrdconta from cecred.tbcrd_conta_cartao car where crd.nrconta_cartao = car.nrconta_cartao) conta_certa,
           crd.qttransa_debito, crd.qttransa_credito, crd.vltransa_debito, crd.vltransa_credito 
    from cecred.tbcrd_utilizacao_cartao crd
    where not exists(Select 1 from cecred.tbcrd_conta_cartao car where crd.cdcooper = car.cdcooper and crd.nrdconta = car.nrdconta and crd.nrconta_cartao = car.nrconta_cartao)
...
BEGIN
  ...
  FOR rw_crapcar IN cr_crapcar LOOP
    ...
    loga := 'update cecred.tbcrd_utilizacao_cartao set nrdconta = '|| rw_crapcar.nrdconta || ', cdcooper = ' || rw_crapcar.cdcooper ||
    ...
    BEGIN
      UPDATE cecred.tbcrd_utilizacao_cartao
