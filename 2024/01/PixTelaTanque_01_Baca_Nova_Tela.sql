declare
  vr_nrseqrdr number;
BEGIN
  FOR rw_crapcop IN (SELECT cdcooper FROM CECRED.crapcop where cdcooper = 3) LOOP 
  ...
  insert into CECRED.craprdr(nmprogra,dtsolici) values('TELA_TANQUE',sysdate) returning nrseqrdr into vr_nrseqrdr;
