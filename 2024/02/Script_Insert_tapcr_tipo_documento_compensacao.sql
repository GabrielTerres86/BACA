BEGIN
  INSERT INTO cobranca.tapcr_tipo_documento_compensacao
  ...
  UPDATE cobranca.tapcr_tipo_documento_compensacao TD
  SET TD.IDTIPO_DOCUMENTO_COMPENSACAO_RCO = (SELECT T.IDTIPO_DOCUMENTO_COMPENSACAO FROM cobranca.tapcr_tipo_documento_compensacao T 
                                 WHERE T.DSTIPO_DOCUMENTO_COMPENSACAO = 'RCO de Cobranca')
  WHERE TD.IDTIPO_DOCUMENTO_COMPENSACAO IN (SELECT T.IDTIPO_DOCUMENTO_COMPENSACAO FROM cobranca.tapcr_tipo_documento_compensacao T 
...
EXCEPTION
  WHEN OTHERS THEN
    sistema.excecaointerna(pr_cdcooper => 3, pr_compleme => 'PRJ0024441-TipoDocCompe');
END;
