DECLARE
  ...
  vr_dtreferencia DATE := to_date('07022024', 'ddmmyyyy');
  ...
  BEGIN
    ...
    LOOP
      ...
      IF nvl(vr_cdcartor, 0) <> to_number(rw_craprem.cdcartor) THEN
        ...
        END IF;
        vr_arquivo  := vr_arquivo || htf.escape_sc('<cartorio><numero_cartorio>' || TRIM(rw_craprem.cdcartor) || '</numero_cartorio>');
    ...
    IF vr_qtregist > 1 THEN
      ...
      vr_arquivo := '<soapenv:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:protesto_brIntf-Iprotesto_br">' ||
                    ...
                    '<user_dados xsi:type="xsd:string">' || vr_arquivo || '</user_dados>' ||
                    '</urn:Cancelamento>' || '</soapenv:Body>' || '</soapenv:Envelope>';
    
      vr_nmdirtxt := gene0001.fn_param_sistema(pr_nmsistem => 'CRED', pr_cdcooper => 3, pr_cdacesso => 'DIR_IEPTB_REMESSA');
      gene0001.pc_abre_arquivo(pr_nmdireto => vr_nmdirtxt
                              ,pr_nmarquiv => vr_nmarqtxt
                              ,pr_tipabert => 'W'
      ...
      IF pr_dscritic IS NOT NULL THEN
        RAISE vr_exc_erro;
      END IF;
      gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_input_file, pr_des_text => vr_arquivo);
      BEGIN
