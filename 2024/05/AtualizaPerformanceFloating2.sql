...
BEGIN
  ...
  BEGIN
    EXECUTE IMMEDIATE 'create index PAGAMENTO.TB_BAIXA_PCR_REMESSA_IDX7 on PAGAMENTO.TB_BAIXA_PCR_REMESSA (NRCONTROLE_PARTICIPANTE) PARALLEL 20 tablespace TBS_PAGAMENTO_I';
    dbms_output.put_line('create PAGAMENTO.TB_BAIXA_PCR_REMESSA_IDX7');
  EXCEPTION
    WHEN already_exists OR columns_indexed THEN
      dbms_output.put_line('skipped crate PAGAMENTO.TB_BAIXA_PCR_REMESSA_IDX7:'||sqlerrm);
  END;
  BEGIN
    EXECUTE IMMEDIATE 'ALTER INDEX pagamento.TB_BAIXA_PCR_REMESSA_IDX7 NOPARALLEL';
    dbms_output.put_line('alter parallel PAGAMENTO.TB_BAIXA_PCR_REMESSA_IDX7');
  EXCEPTION
    WHEN already_exists OR columns_indexed THEN
      dbms_output.put_line('skipped alter parallel PAGAMENTO.TB_BAIXA_PCR_REMESSA_IDX7:'||sqlerrm);
  END;
