DECLARE
  ...
  vr_cdprogra VARCHAR2(50) := 'exp_tbcrd_score_';
...
BEGIN
  ...
  FOR rw_crapcop IN cr_crapcop LOOP
    vr_jobname := 'exp_tbscore_' || to_char(rw_crapcop.cdcooper) || '_K';
    vr_dsplsql := 'DECLARE                                                     ' || chr(13) ||
                  '  DATA_LIMITE DATE;                                         ' || chr(13) ||
                  'BEGIN                                                       ' || chr(13) ||
                  '  SELECT (MAX(DTBASE) - INTERVAL ''12'' MONTH) AS DATA_LIMITE ' || chr(13) ||
                  '    INTO DATA_LIMITE                                        ' || chr(13) ||
