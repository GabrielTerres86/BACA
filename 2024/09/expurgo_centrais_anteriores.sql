  ...
  FOR rw_crapcop IN cr_crapcop LOOP
        ...
        BEGIN
          DELETE FROM cecred.crapvri
           WHERE cdcooper = ' || rw_crapcop.cdcooper || '
             AND dtrefere = to_date(''' || '27/03/2024' || ''' ,''dd/mm/rrrr'');
          COMMIT;
        EXCEPTION
          WHEN OTHERS THEN
    ...
    vr_dsplsql := '        
        ...
        BEGIN
          DELETE FROM cecred.crapvri
           WHERE cdcooper = ' || rw_crapcop.cdcooper || '
             AND dtrefere = to_date(''' || '26/04/2024' || ''' ,''dd/mm/rrrr'');
          COMMIT;
        EXCEPTION
          WHEN OTHERS THEN
            cecred.pc_internal_exception(pr_compleme => ''Erro ao efetuar a limpeza de registros. Tabela de Vencimentos (CRAPVRI).'');
    ...
    dbms_scheduler.create_job(job_name        => 'LIMPA_VRI_ANT_2_' || rw_crapcop.cdcooper
                             ,job_type        => 'PLSQL_BLOCK' 
                             ,job_action      => vr_dsplsql    
                             ,start_date      => SYSDATE       
                             ,auto_drop       => TRUE          
    ...
    dbms_scheduler.create_job(job_name        => 'LIMPA_RIS_ANT_2_' || rw_crapcop.cdcooper
                             ,job_type        => 'PLSQL_BLOCK' 
                             ,job_action      => vr_dsplsql    
                             ,start_date      => SYSDATE       
                             ,auto_drop       => TRUE          
    ...
    vr_dsplsql := '        
        ...
        BEGIN
          DELETE FROM cecred.crapvri
           WHERE cdcooper = ' || rw_crapcop.cdcooper || '
             AND dtrefere = to_date(''' || '17/05/2024' || ''' ,''dd/mm/rrrr'');
          COMMIT;
        EXCEPTION
          WHEN OTHERS THEN
            cecred.pc_internal_exception(pr_compleme => ''Erro ao efetuar a limpeza de registros. Tabela de Vencimentos (CRAPVRI).'');
    ...
    dbms_scheduler.create_job(job_name        => 'LIMPA_VRI_ANT_3_' || rw_crapcop.cdcooper
                             ,job_type        => 'PLSQL_BLOCK' 
                             ,job_action      => vr_dsplsql    
                             ,start_date      => SYSDATE       
                             ,auto_drop       => TRUE          
    ...
    dbms_scheduler.create_job(job_name        => 'LIMPA_RIS_ANT_3_' || rw_crapcop.cdcooper
                             ,job_type        => 'PLSQL_BLOCK' 
                             ,job_action      => vr_dsplsql    
                             ,start_date      => SYSDATE       
                             ,auto_drop       => TRUE          
