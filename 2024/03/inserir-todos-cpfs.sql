DECLARE
  ...
  vr_nmarqimp  VARCHAR2(100) := 'RITM0334430.csv';
...
BEGIN
  ...
  BEGIN
    ...
    USING (SELECT vr_cdcooper AS cdcooperativa, vr_cpf AS cpf, vr_nome AS nome, vr_cdvaga AS cdvaga, vr_tpcolaborador AS tipocolaborador, vr_valor AS valor, vr_valorvaga AS valorvaga, vr_relac AS relac, vr_dsrelac AS dsrelac, vr_cidade AS cidade, vr_UF AS UF, vr_valorpendencia AS valorpendencia, vr_valorendividamento AS valorendividamento, vr_data AS data, 'A' AS inst FROM dual) src
    ON (dest.NRCPFCGC = cpf AND dest.CDIDENTIFICADOR_VAGA = cdvaga)
    WHEN MATCHED THEN
      UPDATE SET dest.CDCOOPER = cdcooperativa,
                 dest.NMPESSOA = nome,
    ...
    WHEN NOT MATCHED THEN
      INSERT (CDCOOPER, NRCPFCGC, NMPESSOA, CDIDENTIFICADOR_VAGA, TPCOLABORADOR, VLSALARIO, VLSALARIO_VAGA, TPRELACIONAMENTO, DSRELACIONAMENTO, DSCIDADE, DSUF, VLPENDENCIA, VLENDIVIDAMENTO, DHRECEBIMENTO, INSITUACAO)
      VALUES (cdcooperativa, cpf, nome, cdvaga, tipocolaborador, valor, valorvaga, relac, dsrelac, cidade, UF, valorpendencia, valorendividamento, data, inst);
  EXCEPTION
    WHEN OTHERS THEN
      vr_dscritic := 'Erro ao processar MERGE na CALCULADORA COL: ' || SQLERRM;
