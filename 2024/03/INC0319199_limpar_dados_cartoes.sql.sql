create index GESTAODERISCO.TBRISCO_OPERACAO_EMPRESTIMO_IDX1 on GESTAODERISCO.TBRISCO_OPERACAO_EMPRESTIMO(IDCARGA_OPERACAO) parallel 48;
BEGIN

  DELETE FROM crapvri x
  WHERE x.rowid IN(SELECT DISTINCT r.rowid
                     FROM tbcrd_risco t, crapvri r
                    WHERE t.IDARQUIVO IN ( SELECT t.idarquivo
