  ...
  CURSOR cr_dados (pr_cpf_rep     IN CECRED.crapass.NRCPFCGC%TYPE
    ...
    WHERE l.nrcpf = pr_cpf_rep
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM CRAPAVT AV
        WHERE AV.CDCOOPER = L.CDCOOPER
          AND AV.NRDCONTA = L.NRDCONTA
          AND AV.NRCPFCGC = pr_cpf_rep
      );
...
BEGIN
  ...
  IF NVL(vr_nrcpf, 0) > 0 AND NVL(vr_nrcnpj, 0) > 0 THEN
  ...
  END IF;
  vr_comments := 'END vr_nrcpf > 0 AND vr_nrcnpj > 0 THEN';
