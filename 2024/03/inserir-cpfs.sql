DECLARE
  ...
  vr_nmarqimp  VARCHAR2(100) := 'inserir-cpfs.csv';
...
BEGIN
  ...
  BEGIN
    ...
    USING (SELECT vr_cdcooper AS cdcooperativa, vr_cdvaga AS cdvaga, vr_nome AS nome, vr_cpf AS cpf, vr_tpcolaborador AS tipocolaborador, vr_valor AS valor, vr_valorvaga AS valorvaga, vr_relac AS relac, vr_dsrelac AS dsrelac, vr_cidade AS cidade, vr_UF AS UF, vr_valorpendencia AS valorpendencia, vr_valorendividamento AS valorendividamento, SYSDATE AS data, 'A' AS inst FROM dual)
