...
BEGIN
  ...
  vr_tab_param(1).nmpartar := 'Carga Cyber - Horario limite aguardar Central de Risco';
  vr_tab_param(1).tpdedado := 2;
  vr_tab_param(1).cdprodut := 12;
  vr_tab_param(1).cdbattar := 'HRLIMCENTRALCYB';
  vr_tab_param(1).nmidenti := 'Carga Cyber - Horario limite aguardar Central de Risco';
  vr_tab_param(1).cdprogra := ' ';
  vr_tab_param(1).tpcadast := 2;
  vr_tab_param(1).cdcooper := 3;
  vr_tab_param(1).dsconteu := '12:00';
