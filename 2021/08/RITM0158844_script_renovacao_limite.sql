...
DECLARE
  ...
  /* 
  --LOCAL
  vr_nmarq_carga    VARCHAR2(200) := '/progress/t0032597/micros/script_renovacao/renovar_civia.csv';                     -- Arquivo a ser lido
  vr_nmarq_log      VARCHAR2(200) := '/progress/t0032597/micros/script_renovacao/Limites_a_renovar_civia_LOG.txt';       -- Arquivo de Log
  vr_nmarq_rollback VARCHAR2(200) := '/progress/t0032597/micros/script_renovacao/Limites_a_renovar_civia_ROLLBACK.sql';  -- Arquivo de Rollback
  */
  ...
  */
  ...
  DECLARE
    ...
    vr_habrat       VARCHAR2(1) := 'N'; -- Paramentro para Habilitar Novo Ratin (S/N)
    vr_strating     NUMBER;
    vr_flgrating    NUMBER;
...
*/
   ...
   GENE0001.pc_gera_log(pr_cdcooper =>  pr_cdcooper
                       ...
                       ,pr_dstransa => 'Renovacao manual Limite (RITM0158844)'
          ...
          IF vr_tab_carga.exists(vr_idx1) THEN         
             ...
             pc_renovar_limite_cred_manual(pr_cdcooper => vr_tab_carga(vr_idx1).cdcooper
              ...
              END IF;
               ...
               --##################################################################################### 
               --INICIO Troca de linha de credito
               
               -- Se nao houver erro na renovacao e linha antiga diferente de linha nova
               IF vr_dscritic IS NULL AND vr_tab_carga(vr_idx1).linhaold <> vr_tab_carga(vr_idx1).linhanew THEN
                 
                  
                  BEGIN -- Armazenar cddlinha para gravar posteriormente no arquivo de rollback
                        SELECT lim.cddlinha INTO vr_aux_cddlinha
                          FROM craplim lim
                    ...
                    UPDATE crawlim lim
                     ...
                     gene0001.pc_escr_linha_arquivo(pr_utlfileh => vr_handle
                                                  ,pr_des_text => 'UPDATE craplim lim '    ||
                                                                  '   SET lim.cddlinha = ' || vr_aux_cddlinha                ||
