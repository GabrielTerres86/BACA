declare 
  ...
  vr_nrdconta      crapass.nrdconta%TYPE := 10473130;
  vr_nrctremp      craplem.nrctremp%TYPE := 3036755;
  vr_vllanmto      craplem.vllanmto%TYPE := 6802.79;
  vr_cdhistor      craplem.cdhistor%TYPE := 3273;
  vr_vlsdeved      crapepr.vlsdeved%TYPE := 7114.76;
  vr_vlsprojt      crapepr.vlsprojt%TYPE := 7114.76;
  vr_vlparepr      crappep.vlparepr%TYPE := 433.75;
  ...
  BEGIN
    UPDATE crappep c
       SET inliquid = 0, 
           vldespar = 0, 
           vlpagpar = 0, 
           vlsdvpar = vr_vlparepr,
           dtultpag = null, 
           dtdstjur = null
     WHERE c.cdcooper = vr_cdcooper
       AND c.nrdconta = vr_nrdconta
       AND c.nrctremp = vr_nrctremp
       and (c.nrparepr between 23 and 26 or c.nrparepr between 28 and 47);
  EXCEPTION
    WHEN OTHERS THEN
      RAISE vr_exc_saida;
  END;
