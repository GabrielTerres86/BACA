DECLARE
  ...
  CURSOR cr_crawcrd(pr_cdcooper crapcop.cdcooper%TYPE) IS
    SELECT w.nrcctitg, w.nrcrcard, w.rowid
      FROM crawcrd w
  ...
  CURSOR cr_crapcrd(pr_cdcooper crapcop.cdcooper%TYPE) IS
    SELECT w.nrcctitg, p.nrcrcard, p.rowid
      FROM crapcrd p
          ,crawcrd w 
     WHERE p.cdcooper = w.cdcooper
       AND p.nrdconta = w.nrdconta
       AND p.nrcrcard = w.nrcrcard
       AND p.cdcooper = pr_cdcooper
       AND NVL(p.cdadmcrd, 0) = 0; 
...
BEGIN
  ...
  vr_nmdireto := vr_nmdireto || '/INC0090090';
