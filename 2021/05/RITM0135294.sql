...
declare 
  ...
  vr_nmdireto        VARCHAR2(4000) := vr_rootmicros||'cpd/bacas/RITM0135294';
  vr_nmarqimp        VARCHAR2(100)  := 'rollback_RITM0135294.sql';
  ...
  FOR rw_contas IN cr_contas LOOP
       ...
       UPDATE CRAPASS
          SET DTADMISS = rw_contas.DTMVTOLT
		    , DTABTCCT = rw_contas.DTMVTOLT
        WHERE CDCOOPER = rw_contas.CDCOOPER
          AND NRDCONTA = rw_contas.NRDCONTA;
             
        vr_bkp_dep_vista_1 := 'UPDATE CRAPASS SET DTADMISS = TO_DATE( ''' || RW_CONTAS.DTADMISS || ''',''DD/MM/YYYY''), DTABTCCT = '''' WHERE CDCOOPER = ' || RW_CONTAS.CDCOOPER || ' AND NRDCONTA = ' || RW_CONTAS.NRDCONTA ||';';                         
