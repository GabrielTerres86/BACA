...
vr_nmdireto        VARCHAR2(4000) := vr_rootmicros||'cpd/bacas/INC0076748'; 
  ...
  CURSOR cr_aplica IS   
   SELECT 13 cdcooper, 3000   nrdconta, 19274  nrctrrpp, 2  nraplica,  'AABTD3AABAAESJ7AAr' rowidlac FROM dual;
...
BEGIN
  ...
  FOR rw_aplica IN cr_aplica LOOP
    ...
    UPDATE craplac
     ...
     WHERE rowid =  rw_aplica.rowidlac;
    EXCEPTION
    WHEN OTHERS THEN
    pr_dscritic := 'Erro ao atualizar craplac.';
    RAISE vr_excsaida;
    END;
    backup3('update from craplac set cdhistor = 2747 where rowid = '||rw_aplica.rowidlac||';'); 
