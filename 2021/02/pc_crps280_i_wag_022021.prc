CREATE OR REPLACE PROCEDURE CECRED.PC_CRPS280_I_WAG(pr_cdcooper   IN crapcop.cdcooper%TYPE --> Coop. conectada
                                                   ...
                                                   -- CYBER
                                                   ,pr_flgrelat IN PLS_INTEGER DEFAULT 0 --> Flag padrão para somente gerar os relat??s
...
BEGIN
  /* ..........................................................................
    ...
    12/01/2021 - INC0069256: Não validar finalidade do empréstimo para contabilização do programa BNDES PESE.
                  (Heitor - Mouts)
  ............................................................................. */
  ...
  DECLARE
	...
	vr_idx_crapepr VARCHAR2(100); --indice pl-table vr_tab_crapepr
    ...
    vr_tab_crapris_bulk typ_crapris;           
    ...
    --Buscar emprestimos com base na crapris
    CURSOR cr_crapepr IS   
      ...
      FROM crapass ass
       ...
       WHERE ris.cdcooper  = ass.cdcooper
         ...
         AND ris.inddocto  = 1 -- Docto 3020
         AND ris.vldivida > 0
         AND ris.cdorigem = 3 -- Emprestimos/Financiamentos
         AND ris.dsinfaux <> 'BNDES'
       -- P450 - Alterado agencia do decode de alias= ris para alias= ass
         AND ass.cdagenci = decode(pr_cdagenci, 0, ass.cdagenci, pr_cdagenci); -- Ligeirinho.

    TYPE typ_crapepr IS TABLE OF cr_crapepr%ROWTYPE INDEX BY VARCHAR2(50);       
    vr_tab_crapepr typ_crapepr;             
    ...
    BEGIN
        ...
        -- 1ª linha
				-- PRJ Microcredito
        /* Lançamento não é mais necessário. Solicitado por Keyt (Contabilidade) e tratado através da RITM0045405
    ...
    begin
      ...
      -- popular em memória cursor crapepr
      FOR rw_crapepr IN cr_crapepr LOOP
        vr_idx_crapepr := LPAD(pr_cdcooper,3,'0') || 
                          LPAD(rw_crapepr.nrdconta,10,'0') || 
                          LPAD(rw_crapepr.nrctremp,10,'0');  
                                     
        vr_tab_crapepr(vr_idx_crapepr).qtmesdec := rw_crapepr.qtmesdec;
        vr_tab_crapepr(vr_idx_crapepr).dtdpagto := rw_crapepr.dtdpagto;
        vr_tab_crapepr(vr_idx_crapepr).qtprecal := rw_crapepr.qtprecal;
        vr_tab_crapepr(vr_idx_crapepr).vlsdeved := rw_crapepr.vlsdeved;
        vr_tab_crapepr(vr_idx_crapepr).qtpreemp := rw_crapepr.qtpreemp;
        vr_tab_crapepr(vr_idx_crapepr).vlpreemp := rw_crapepr.vlpreemp;
        vr_tab_crapepr(vr_idx_crapepr).cdlcremp := rw_crapepr.cdlcremp;
        vr_tab_crapepr(vr_idx_crapepr).tpemprst := rw_crapepr.tpemprst;
        vr_tab_crapepr(vr_idx_crapepr).flgpagto := rw_crapepr.flgpagto;
        vr_tab_crapepr(vr_idx_crapepr).qtlcalat := rw_crapepr.qtlcalat;
        vr_tab_crapepr(vr_idx_crapepr).qtpcalat := rw_crapepr.qtpcalat;
        vr_tab_crapepr(vr_idx_crapepr).txmensal := rw_crapepr.txmensal;
        vr_tab_crapepr(vr_idx_crapepr).vlsdevat := rw_crapepr.vlsdevat;
        vr_tab_crapepr(vr_idx_crapepr).inliquid := rw_crapepr.inliquid;
        vr_tab_crapepr(vr_idx_crapepr).txjuremp := rw_crapepr.txjuremp;
        vr_tab_crapepr(vr_idx_crapepr).nrdconta := rw_crapepr.nrdconta;
        vr_tab_crapepr(vr_idx_crapepr).nrctremp := rw_crapepr.nrctremp;
        vr_tab_crapepr(vr_idx_crapepr).vlpapgat := rw_crapepr.vlpapgat;
        vr_tab_crapepr(vr_idx_crapepr).inprejuz := rw_crapepr.inprejuz;
        vr_tab_crapepr(vr_idx_crapepr).cdfinemp := rw_crapepr.cdfinemp;
        vr_tab_crapepr(vr_idx_crapepr).dtmvtolt := rw_crapepr.dtmvtolt;
        vr_tab_crapepr(vr_idx_crapepr).vlemprst := rw_crapepr.vlemprst;
        vr_tab_crapepr(vr_idx_crapepr).tpdescto := rw_crapepr.tpdescto;
        vr_tab_crapepr(vr_idx_crapepr).vlppagat := rw_crapepr.vlppagat;
        vr_tab_crapepr(vr_idx_crapepr).vljurmes := rw_crapepr.vljurmes;
        vr_tab_crapepr(vr_idx_crapepr).cdagenci := rw_crapepr.cdagenci;
        vr_tab_crapepr(vr_idx_crapepr).dtvencto_original := rw_crapepr.dtvencto_original;                                       
      END LOOP;
      ...
      LOOP
        FETCH cr_crapris BULK COLLECT INTO vr_tab_crapris_bulk LIMIT 1000;
        EXIT WHEN vr_tab_crapris_bulk.COUNT = 0;
        FOR idx IN vr_tab_crapris_bulk.first..vr_tab_crapris_bulk.last LOOP
          BEGIN
            -- Verifica migração/incorporação
            IF NOT
                fn_verifica_conta_migracao(pr_nrdconta => vr_tab_crapris_bulk(idx).nrdconta) THEN
              CONTINUE;
            END IF;
            ...
            IF vr_tab_crapris_bulk(idx).cdorigem = 3 THEN
              -- Para empréstimos BNDES
              IF vr_tab_crapris_bulk(idx).dsinfaux = 'BNDES' THEN
                -- Buscar informações da crapebn
                OPEN cr_crapebn(pr_nrdconta => vr_tab_crapris_bulk(idx).nrdconta,
                                pr_nrctremp => vr_tab_crapris_bulk(idx).nrctremp);
                FETCH cr_crapebn
                  INTO vr_vlpreemp;
                -- Se encontrou
                IF cr_crapebn%FOUND THEN
                  ...
                  IF vr_tab_crapris_bulk(idx).qtdiaatr > 0 THEN
                    vr_qtatraso := TRUNC(vr_tab_crapris_bulk(idx).qtdiaatr / 30, 0) + 1;
                    vr_qtdiaatr := vr_tab_crapris_bulk(idx).qtdiaatr; -- paralelismo.
                  ELSE
                    vr_qtatraso := 0;
              ...
              ELSE
                ...
                -- montar indice para busar emprestimo ALCEMIR
                vr_idx_crapepr := LPAD(pr_cdcooper,3,'0') || 
                                       LPAD(vr_tab_crapris_bulk(idx).nrdconta,10,'0') || 
                                       LPAD(vr_tab_crapris_bulk(idx).nrctremp,10,'0');  
                  
                -- Somente se encontrar
                IF vr_tab_crapepr.EXISTS(vr_idx_crapepr) THEN
                                                     
                  -- Indicador se é um emprestimo de cessao
                  vr_fleprces := case
                                   when vr_tab_crapepr(vr_idx_crapepr).dtvencto_original is null then
                                    0
                                   else
                                    1
                                 end;
                  ...
                  vr_qtmesdec := vr_tab_crapepr(vr_idx_crapepr).qtmesdec;

                  /* Efetuar correcao quantidade parcelas em atraso */

                  vr_qtprecal_intei := vr_tab_crapepr(vr_idx_crapepr).qtprecal;

                  -- Se existir data do primeiro pagamento
                  IF vr_tab_crapepr(vr_idx_crapepr).dtdpagto IS NOT NULL THEN
                    ...
                    IF (to_char(vr_tab_crapepr(vr_idx_crapepr).dtdpagto, 'dd') >
                       to_char(pr_rw_crapdat.dtmvtolt, 'dd') AND
                       TRUNC(vr_tab_crapepr(vr_idx_crapepr).dtdpagto, 'mm') =
                       TRUNC(pr_rw_crapdat.dtmvtolt, 'mm')) OR
                       (TRUNC(pr_rw_crapdat.dtmvtolt, 'mm') =
                       TRUNC(pr_rw_crapdat.dtmvtopr, 'mm') AND
                       vr_tab_crapepr(vr_idx_crapepr).dtdpagto > pr_rw_crapdat.dtmvtolt) THEN
                      -- Decrementar -1 mês
                  ...
                  OPEN cr_craplcr(pr_cdcooper => pr_cdcooper,
                                  pr_cdlcremp => vr_tab_crapepr(vr_idx_crapepr).cdlcremp);
                  FETCH cr_craplcr
                    INTO rw_craplcr;
                  -- Se não encontrar
                  ...
                  vr_txmenemp := vr_tab_crapepr(vr_idx_crapepr).txmensal;

                  /* Diario */
                  IF to_char(pr_rw_crapdat.dtmvtolt, 'MM') =
                     to_char(pr_rw_crapdat.dtmvtopr, 'MM') THEN
                    -- verifica o tipo do emprestimo 0-atual
                    IF vr_tab_crapepr(vr_idx_crapepr).tpemprst = 0 THEN
                      vr_qtprecal_decim := vr_tab_crapepr(vr_idx_crapepr).qtlcalat;
                    ELSE
                      /* END IF crapepr.tpemprst = 0 */
                      vr_qtprecal_decim := vr_tab_crapepr(vr_idx_crapepr).qtpcalat;
                    END IF;
                    ...
                    vr_qtprecal_intei := vr_qtprecal_intei +
                                         vr_tab_crapepr(vr_idx_crapepr).qtprecal;
                    vr_qtprecal_decim := vr_qtprecal_decim +
                                         vr_tab_crapepr(vr_idx_crapepr).qtprecal;
                    -- Guardar o saldo devedor atual
                    vr_vlsdeved_atual := vr_tab_crapepr(vr_idx_crapepr).vlsdevat;

                  ELSE
                    /* Mensal */
                    vr_qtprecal_intei := vr_tab_crapepr(vr_idx_crapepr).qtprecal;
                    vr_vlsdeved_atual := vr_tab_crapepr(vr_idx_crapepr).vlsdeved;
                    vr_qtprecal_decim := vr_tab_crapepr(vr_idx_crapepr).qtprecal;
                  END IF;
                  ...
                  empr0001.pc_config_empresti_empresa(pr_cdcooper => pr_cdcooper --> Código da Cooperativa
                                                     ...
                                                     ,pr_nrdconta => vr_tab_crapris_bulk(idx).nrdconta --> Numero da conta do empréstim
                  ...
                  vr_txdjuros := vr_tab_crapepr(vr_idx_crapepr).txjuremp;
                  ...
                  vr_indice := LPAD(vr_tab_crapepr(vr_idx_crapepr).nrdconta, 10, '0') ||
                               LPAD(vr_tab_crapepr(vr_idx_crapepr).nrctremp, 10, '0');
                  -- Alimentando a tabela de emprestimos
                  vr_tab_dados_epr(vr_indice).cdagenci := vr_tab_crapepr(vr_idx_crapepr).cdagenci; -- Ligeirinho.
                  vr_tab_dados_epr(vr_indice).nrdconta := vr_tab_crapepr(vr_idx_crapepr).nrdconta;
                  vr_tab_dados_epr(vr_indice).nrctremp := vr_tab_crapepr(vr_idx_crapepr).nrctremp;
                  vr_tab_dados_epr(vr_indice).vlpapgat := vr_tab_crapepr(vr_idx_crapepr).vlpapgat;
                  vr_tab_dados_epr(vr_indice).flgpagto := vr_tab_crapepr(vr_idx_crapepr).flgpagto;
                  vr_tab_dados_epr(vr_indice).dtdpagto := vr_tab_crapepr(vr_idx_crapepr).dtdpagto;
                  vr_tab_dados_epr(vr_indice).vlsdevat := vr_tab_crapepr(vr_idx_crapepr).vlsdevat;
                  vr_tab_dados_epr(vr_indice).qtpcalat := vr_tab_crapepr(vr_idx_crapepr).qtpcalat;
                  vr_tab_dados_epr(vr_indice).txmensal := vr_txmenemp;
                  vr_tab_dados_epr(vr_indice).txjuremp := vr_txdjuros;
                  vr_tab_dados_epr(vr_indice).vlppagat := vr_tab_crapepr(vr_idx_crapepr).vlppagat;
                  vr_tab_dados_epr(vr_indice).qtmdecat := vr_tab_crapepr(vr_idx_crapepr).qtmdecat;
                  vr_tab_dados_epr(vr_indice).inprejuz := vr_tab_crapepr(vr_idx_crapepr).inprejuz;
                  vr_tab_dados_epr(vr_indice).inliquid := vr_tab_crapepr(vr_idx_crapepr).inliquid;
                  vr_tab_dados_epr(vr_indice).qtpreemp := vr_tab_crapepr(vr_idx_crapepr).qtpreemp;
                  vr_tab_dados_epr(vr_indice).cdlcremp := vr_tab_crapepr(vr_idx_crapepr).cdlcremp;
                  vr_tab_dados_epr(vr_indice).cdfinemp := vr_tab_crapepr(vr_idx_crapepr).cdfinemp;
                  vr_tab_dados_epr(vr_indice).dtmvtolt := vr_tab_crapepr(vr_idx_crapepr).dtmvtolt;
                  vr_tab_dados_epr(vr_indice).vlemprst := vr_tab_crapepr(vr_idx_crapepr).vlemprst;
                  vr_tab_dados_epr(vr_indice).tpdescto := vr_tab_crapepr(vr_idx_crapepr).tpdescto;
                  vr_tab_dados_epr(vr_indice).vljurmes := vr_tab_crapepr(vr_idx_crapepr).vljurmes;
                  -- Para microcrédito
                  ...
                  vr_tab_dados_epr(vr_indice).dtinictr := vr_tab_crapris_bulk(idx).dtinictr;
                  ...
                  IF vr_fleprces = 1 THEN
                    ...
                    --Somente popular a dtdpagto com a data da cessão do cartão se o contrato de cessão não foi renegociado,
                    --se foi renegociado, a dtdpagto é a data de pagamento da primeira prestação da renegociação. 
                    IF empr0021.fn_retorna_qtd_renegociacoes(pr_cdcooper => pr_cdcooper
                                                            ,pr_nrdconta => vr_tab_crapepr(vr_idx_crapepr).nrdconta
                                                            ,pr_nrctremp => vr_tab_crapepr(vr_idx_crapepr).nrctremp
                                                            ,pr_tplibera => 1) = 0 THEN                                                
                      IF Nvl(vr_tab_dados_epr(vr_indice).qtpreemp,0) = 1 THEN
                        vr_tab_dados_epr(vr_indice).dtdpagto := vr_tab_crapepr(vr_idx_crapepr).dtvencto_original;
                      --24/07/2020 - INC0055731
                      --Para Cessão de Cartão com mais de uma Parcela, somente popular a dtdpagto com a data da cessão se tiver
                      --atraso da primeira parcela, nas demais parcelas a dtdpagto deve ser a data da parcela do empréstimo.  
                      ELSE 
                  ...
                  IF vr_tab_crapepr(vr_idx_crapepr).tpemprst IN (1, 2) THEN
                    -- Utilizaremos o valor da tabela
                    vr_qtdiaatr := vr_tab_crapris_bulk(idx).qtdiaatr;
                  ELSE
                    -- Calcular os dias em atraso com base nos meses decorridos
                    vr_qtdiaatr := (vr_qtmesdec - vr_qtprecal_decim) * 30;

                    IF vr_qtdiaatr <= 0 AND vr_tab_crapepr(vr_idx_crapepr).flgpagto = 0 THEN
                      -- Conta corrente
                      vr_qtdiaatr := pr_rw_crapdat.dtmvtolt -
                                     vr_tab_crapepr(vr_idx_crapepr).dtdpagto;
                    END IF;
                  ...
                  IF vr_qtmesdec > vr_tab_crapepr(vr_idx_crapepr).qtpreemp THEN
                    vr_qtmesdec := vr_tab_crapepr(vr_idx_crapepr).qtpreemp;
                  END IF;

                  -- Atribuir aos campos da tabela o valor da parcela e linha de credito
                  vr_vlpreemp := vr_tab_crapepr(vr_idx_crapepr).vlpreemp;
                  vr_cdlcremp := to_char(vr_tab_crapepr(vr_idx_crapepr).cdlcremp);
                  vr_cdfinemp := vr_tab_crapepr(vr_idx_crapepr).cdfinemp;
                  vr_cdusolcr := rw_craplcr.cdusolcr;
                  vr_dsorgrec := rw_craplcr.dsorgrec;

                  CASE vr_tab_crapepr(vr_idx_crapepr).tpemprst
                    WHEN 0 THEN
                      vr_tpemprst := 'TR';
                    WHEN 1 THEN
                      vr_tpemprst := 'PP';
                  ...
                  IF vr_tab_crapepr(vr_idx_crapepr).tpemprst IN (1, 2) THEN
                    ...
                    vr_nroprest := vr_tab_crapepr(vr_idx_crapepr).qtmesdec - nvl(vr_tab_crapepr(vr_idx_crapepr).qtpcalat, 0);
                    ...
                    IF vr_tab_crapris_bulk(idx).qtdiaatr > 0 THEN
                      ...
                      vr_qtatraso := TRUNC(vr_tab_crapris_bulk(idx).qtdiaatr / 30) + 1;
                    ELSE
                      -- Sem atraso
                      vr_qtatraso := 0;
                    END IF;
...
--                  CLOSE cr_crapepr;
              ...
              -- NrdConta(10) + DtRefere(8) + InNivRis(5)
              vr_des_chave_crapris := LPAD(vr_tab_crapris_bulk(idx).cdagenci, 5, '0') ||
                                      to_char(999999.9999 - nvl(vr_nroprest, 0), '000000d0000') ||
                                      RPAD(vr_tab_crapris_bulk(idx).nmprimtl, 60, ' ') ||
                                      LPAD(vr_tab_crapris_bulk(idx).nrctremp, 10, '0') ||
                                      LPAD(vr_tab_crapris_bulk(idx).nrseqctr, 5, '0') ||
                                      LPAD(vr_tab_crapris_bulk(idx).nrdconta, 10, '0') ||
                                      to_char(vr_tab_crapris_bulk(idx).dtrefere, 'ddmmyyyy') ||
                                      LPAD(vr_tab_crapris_bulk(idx).innivris, 5, '0');

              -- Para conta corrente
            ELSIF vr_tab_crapris_bulk(idx).cdorigem = 1 THEN
              ...
              OPEN cr_crapsld(pr_nrdconta => vr_tab_crapris_bulk(idx).nrdconta);
              FETCH cr_crapsld
                INTO rw_crapsld;
              -- Se tiver encontrado
            ...
            vr_tab_crapris(vr_des_chave_crapris).cdagenci := vr_tab_crapris_bulk(idx).cdagenci;
            vr_tab_crapris(vr_des_chave_crapris).nroprest := vr_nroprest;
            vr_tab_crapris(vr_des_chave_crapris).nmprimtl := vr_tab_crapris_bulk(idx).nmprimtl;
            vr_tab_crapris(vr_des_chave_crapris).nrdconta := vr_tab_crapris_bulk(idx).nrdconta;
            vr_tab_crapris(vr_des_chave_crapris).nrcpfcgc := vr_tab_crapris_bulk(idx).nrcpfcgc;
            vr_tab_crapris(vr_des_chave_crapris).dtrefere := vr_tab_crapris_bulk(idx).dtrefere;
            vr_tab_crapris(vr_des_chave_crapris).innivris := vr_tab_crapris_bulk(idx).innivris;
            vr_tab_crapris(vr_des_chave_crapris).nrctremp := vr_tab_crapris_bulk(idx).nrctremp;
            vr_tab_crapris(vr_des_chave_crapris).cdmodali := vr_tab_crapris_bulk(idx).cdmodali;
            vr_tab_crapris(vr_des_chave_crapris).nrseqctr := vr_tab_crapris_bulk(idx).nrseqctr;
            vr_tab_crapris(vr_des_chave_crapris).vldivida := vr_tab_crapris_bulk(idx).vldivida;
            vr_tab_crapris(vr_des_chave_crapris).vljura60 := vr_tab_crapris_bulk(idx).vljura60;
            vr_tab_crapris(vr_des_chave_crapris).cdorigem := vr_tab_crapris_bulk(idx).cdorigem;
            vr_tab_crapris(vr_des_chave_crapris).inpessoa := vr_tab_crapris_bulk(idx).inpessoa;
            vr_tab_crapris(vr_des_chave_crapris).qtdriclq := vr_tab_crapris_bulk(idx).qtdriclq;
            ...
            vr_tab_crapris(vr_des_chave_crapris).dtinictr := vr_tab_crapris_bulk(idx).dtinictr;
            vr_tab_crapris(vr_des_chave_crapris).inprejuz := vr_tab_crapris_bulk(idx).inprejuz;
            vr_tab_crapris(vr_des_chave_crapris).vliofmes := rw_crapsld.vliofmes;

            -- P577 - Inicio           
            vr_tab_crapris(vr_des_chave_crapris).vljurantpp := vr_tab_crapris_bulk(idx).vljurantpp;
            vr_tab_crapris(vr_des_chave_crapris).vljurmorpp := vr_tab_crapris_bulk(idx).vljurmorpp;
            vr_tab_crapris(vr_des_chave_crapris).vljurmulpp := vr_tab_crapris_bulk(idx).vljurmulpp;           
            -- P577 - Fim

            IF vr_tab_crapris_bulk(idx).cdorigem = 1 THEN
              -- conta corrente
              vr_tab_crapris(vr_des_chave_crapris).qtdiaatr := vr_qtatraso;
            ELSIF (vr_tab_crapris_bulk(idx).cdorigem IN (4,5)) THEN
              vr_tab_crapris(vr_des_chave_crapris).qtdiaatr := vr_tab_crapris_bulk(idx).qtdiaatr;
            ELSE
              vr_tab_crapris(vr_des_chave_crapris).qtdiaatr := vr_qtdiaatr;
            END IF;

            vr_tab_crapris(vr_des_chave_crapris).dsinfaux := vr_tab_crapris_bulk(idx).dsinfaux;
            ...
            IF vr_tab_crapris_bulk(idx).cdorigem in (4,5) THEN -- Títulos C/Registro e S/Registro
              ...
              FOR rw_craptdb IN cr_craptdb(pr_cdcooper => pr_cdcooper
                                          ,pr_nrdconta => vr_tab_crapris_bulk(idx).nrdconta
                                          ,pr_nrctremp => vr_tab_crapris_bulk(idx).nrctremp
                                          --,pr_dtinictr => vr_tab_crapris_bulk.dtinictr
          ...
          EXCEPTION
            WHEN vr_exc_ignorar THEN
              ...
              NULL;
          END;          
        END LOOP; --FOR idx IN vr_tab_crapris_bulk.first..vr_tab_crapris_bulk.last LOOP --> Termino leitura dos riscos
        vr_tab_crapris_bulk.DELETE;
      END LOOP;-- fim loop fetch bulk
        ...
        IF vr_tab_crapvri_index.exists(vr_chave_index) THEN
          ...
          vr_vljuro60_desconto := vr_vljuro60_desconto +
                                  CASE WHEN vr_tab_crapris(vr_des_chave_crapris).cdmodali = 101 
                                        AND vr_tab_crapris(vr_des_chave_crapris).innivris >= 10 --> Apenas operações que estejam em prejuizo
        ...
        IF vr_vlprejuz_conta <> vr_tab_crapris(vr_des_chave_crapris).vldivida THEN
          ...
          -- Calcular o valor de prestação em atraso
            vr_vlpreatr := ROUND( (vr_vldivida - vr_tab_crapris(vr_des_chave_crapris).vljura60
                                               - nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0) -- PRJ577 -- O cálculo da provisão deve considerar o Valor do Juros+60 Anterior 
                                  ) * vr_vlpercen ,2);
          ...
          IF vr_tab_crapris(vr_des_chave_crapris).inpessoa = 1 THEN
            ...
            ELSE
              vr_tab_totrispac(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpf60 := vr_tab_crapris(vr_des_chave_crapris).vljura60 + nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0); -- PRJ577 -- Considerar no resumo do relatório crrl227 o valor do Juros+60 anterior
            END IF;
          ELSE
            IF vr_tab_totrispac.EXISTS(vr_tab_crapris(vr_des_chave_crapris).innivris) THEN
            ...
            ELSE
              vr_tab_totrispac(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpj60 := vr_tab_crapris(vr_des_chave_crapris).vljura60 + nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0); -- PRJ577 -- Considerar no resumo do relatório crrl227 o valor do Juros+60 anterior
            END IF;
          END IF;
          ...
          IF vr_tab_crapris(vr_des_chave_crapris).inpessoa = 1 THEN
            ...
            ELSE
              vr_tab_totrisger(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpf60 := vr_tab_crapris(vr_des_chave_crapris).vljura60 + nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0); -- PRJ577 -- Considerar no resumo do relatório crrl227 o valor do Juros+60 anterior
            END IF;
          ELSE
            IF vr_tab_totrisger.EXISTS(vr_tab_crapris(vr_des_chave_crapris).innivris) THEN
              vr_tab_totrisger(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpj60 := vr_tab_totrisger(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpj60 + vr_tab_crapris(vr_des_chave_crapris).vljura60 + nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0); -- PRJ577 -- Considerar no resumo do relatório crrl227 o valor do Juros+60 anterior
            ELSE
              vr_tab_totrisger(vr_tab_crapris(vr_des_chave_crapris).innivris).vljrpj60 := vr_tab_crapris(vr_des_chave_crapris).vljura60 + nvl(vr_tab_crapris(vr_des_chave_crapris).vljurantpp, 0); -- PRJ577 -- Considerar no resumo do relatório crrl227 o valor do Juros+60 anterior
            END IF;
          END IF;
          --
          ...
          ELSIF vr_tab_crapris(vr_des_chave_crapris).cdorigem = 1 THEN
            ...
            ELSE
              ...
              -- P450 - Desconsiderar do vr_vldivida, o juro 60. São casos em que esse juro é de nivelrisco >= 10 (prejuizo).  
