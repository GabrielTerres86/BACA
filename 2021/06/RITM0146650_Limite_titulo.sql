---atualizar cdageimp no limite desconto titulo e bordero de titulo.
DECLARE
  ...
  CURSOR c1 (pr_cdcooper IN crapcop.cdcooper%TYPE)  IS
    SELECT craplim.cdopelib
     ...
     WHERE craplim.cdcooper = pr_cdcooper
       ...
       AND craplim.tpctrlim = 3  -- limite desconto titulo
  ...
  --Busca Borderôs de Títulos 
  CURSOR c3 (pr_cdcooper IN crapcop.cdcooper%TYPE)  IS
...
BEGIN
  ...
  -- caminho para produção:
     ...
     vr_nmarq_rollback := vr_dsdireto||'RITM0146650_Limite_titulo_ROLLBACK.sql';
    ...
    FOR rw_crapcop IN (SELECT cop.cdcooper
      ...
      -- bordero de titulo
      FOR r3 IN c3(pr_cdcooper => rw_crapcop.cdcooper)  LOOP                                  
        BEGIN
        ...
        EXCEPTION
          WHEN OTHERS THEN
            vr_dsmensagem := 'Erro ao atualizar crapbdt. ' || SQLERRM;
            RAISE vr_exc_erro;
        END;
