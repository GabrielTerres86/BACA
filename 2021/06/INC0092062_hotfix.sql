declare
  ...
  PROCEDURE pc_risco_k(pr_cdcooper   IN crapcop.cdcooper%TYPE
    ...
    Data    : Dezembro/2014                       Ultima Alteracao: 16/04/2021
    ...
    Alterações:
     ...
     09/03/2021 - RISCOS - Tratamento carta circular 4092 - Limite Credito 1902 (Guilherme/AMcom)
       ...
       15/04/2021 - RITM0130707 - RITM0130707 - Incluir na package RISC0001_TEMP.pc_risco_k a geração de uma linha com o total da operação no último dia do mês 
    ...
    CURSOR cr_crapris_lnu(pr_cdcooper IN crapris.cdcooper%TYPE
                           ,pr_dtrefere IN crapris.dtrefere%TYPE
                           ,pr_cdmodali IN crapris.cdmodali%TYPE) IS
        SELECT ris.cdcooper
              ,ris.dtrefere
              ,ris.vldivida
              ,ris.nrdconta
      ...
      --> Rendas a apropriar (juros+60)
       CURSOR cr_juros_60  (pr_cdcooper IN crapepr.cdcooper%TYPE,         
                            pr_dtrefere IN crapris.dtrefere%TYPE,
                            pr_cdlcremp IN crapepr.cdlcremp%TYPE) is 
        SELECT nvl(ROUND(SUM(vljura60 + vljurantpp),2),0)
            FROM CRAPRIS, CRAPEPR
           WHERE crapris.cdcooper  = pr_cdcooper -- COOPERATIVA
             AND crapris.dtrefere  = pr_dtrefere --data base
  ...
  --ROUND(((CRAPRIS.VLDIVIDA - CRAPRIS.VLJURA60 - CRAPRIS.VLJURANTPP) *  PERCENTUAL_CONFORME_RISCO), 2)
      ...
      ROUND((crapris.vldivida - crapris.vljura60 - crapris.vljurantpp) * (n_risco.percentu)/100,2) v_provisao_perda
        FROM CRAPRIS, CRAPEPR,
      ...
      cursor cr_craplcr_6901(pr_cdcooper IN crapepr.cdcooper%TYPE
        ...
        select aux.innivris
          ...
          from (select c.innivris
                     , sum(c.vldivida) vldivida_normal
                     , 0 vldivida_vencida
                     , sum(case c.inpessoa when 1 then c.vljura60 else 0 end) vljura60_pf
                     , sum(case c.inpessoa when 1 then 0 else c.vljura60 end) vljura60_pj
                     , sum(case c.inpessoa when 1 then c.vljurantpp else 0 end) vljurantpp_pf
                     , sum(case c.inpessoa when 1 then 0 else c.vljurantpp end) vljurantpp_pj
                  from crapass a
                     , crapepr e
                     , crapris c
                ...
                select c.innivris
                     , 0 vldivida_normal
                     , sum(c.vldivida) vldivida_vencida
                     , sum(case c.inpessoa when 1 then c.vljura60 else 0 end) vljura60_pf
                     , sum(case c.inpessoa when 1 then 0 else c.vljura60 end) vljura60_pj
                     , sum(case c.inpessoa when 1 then c.vljurantpp else 0 end) vljurantpp_pf
                     , sum(case c.inpessoa when 1 then 0 else c.vljurantpp end) vljurantpp_pj
      ...
      --  Saldo da operação Linha de crédito 2600
         ...
         --Operação no último dia do mês. Operação       
             vr_linhadet := TRIM(vr_con_dtmvtolt) || ',' ||
                           ...
                           ',5210,'||
                           '"OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE – PRONAMPE"';
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);                         
             -- primeiro dia do mês subsequente. Linha para a Reversão 
              ...
              vr_linhadet := TRIM(vr_con_dtmvtopr) || ',' ||
                             ...
                             TRIM(to_char(vr_saldo_opera, '99999999999990.00')) ||        
                             ',5210,'||
                             '"REVERSAO OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE - PRONAMPE"';
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);  
         ...
         IF vr_total_jur60 > 0 THEN       
             --Operação no último dia do mês. Operação
              vr_linhadet := TRIM(vr_con_dtmvtolt) || ',' ||
                             ...
                             ',5210,'||
                             '"RENDAS A APROPRIAR OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE – PRONAMPE"';
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);  
             -- primeiro dia do mês subsequente. Linha para a Reversão        
              vr_linhadet := TRIM(vr_con_dtmvtopr) || ',' ||
                             ...
                             TRIM(to_char(vr_total_jur60, '99999999999990.00')) ||        
                             ',5210,'||
                             '"REVERSAO RENDAS A APROPRIAR OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE - PRONAMPE"';
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);  
      ...
      -- Gerar lançamento apenas quando o valor de Provisão de perdas for maior que zero 
         IF vr_prov_perdas > 0 THEN 
             ...
             vr_linhadet := TRIM(vr_con_dtmvtolt) || ',' ||
                           ...
                           ',5210,'||
                           '"PROVISAO DAS OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE - PRONAMPE"';
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);                         
             -- primeiro dia do mês subsequente. Linha para a Reversão        
              vr_linhadet := TRIM(vr_con_dtmvtopr) || ',' ||
                             ...
                             TRIM(to_char(vr_prov_perdas, '99999999999990.00')) ||        
                             ',5210,'||
                             '"REVERSAO PROVISAO DAS OPERACOES DE CREDITO CONFORME INSTRUCAO NORMATIVA BCB 89 2021 PROGRAMA NACIONAL DE APOIO AS MICROEMPRESAS E EMPRESAS DE PEQUENO PORTE - PRONAMPE"';                       
              -- Gravar Linha
              pc_gravar_linha(vr_linhadet);          
   ...
   /**s**/
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        pr_dscritic := 'Erro em RISC0001_TEMP.pc_risco_k: ' || SQLERRM;
...
begin
  for rw_crapcop in (select cdcooper
                       from crapcop
                      where flgativo = 1) loop
    vr_retfile  := null;
    vr_dscritic := null;

    -- Call the procedure
    pc_risco_k(pr_cdcooper => rw_crapcop.cdcooper,
               pr_dtrefere => to_char('31/05/2021'),
