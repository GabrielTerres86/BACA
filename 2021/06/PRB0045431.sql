DECLARE
  ...
  CURSOR cr_tbcrd_conta_cartao(pr_cdcooper crapcop.cdcooper%TYPE) IS
    SELECT t.*, t.rowid
     ...
     WHERE t.cdcooper = pr_cdcooper
       AND NVL(t.nrconta_cartao,0) = 0; 
 
  rw_tbcrd_conta_cartao  cr_tbcrd_conta_cartao%ROWTYPE;
  
  CURSOR cr_tbcrd_conta_cartao_adm(pr_cdcooper crapcop.cdcooper%TYPE) IS
  ...
  rw_tbcrd_conta_cartao_adm  cr_tbcrd_conta_cartao_adm%ROWTYPE;
...
BEGIN
  BEGIN
    ...
    values
      ('CRED',
       3,
       'EMAIL_ERRO_CRPS672',
       'Email de criticas da rotina CRD_CRPS672',
       'otavio.theiss@ailos.coop.br;alessandra.baessa@ailos.coop.br',
       NULL);
  ...
  vr_nmdireto := vr_nmdireto || '/PRB0045431';
  ...
  FOR rw_crapcop IN cr_crapcop LOOP  
    ...
    vr_nmarqbkp  := 'BKP_TBCRD_CONTA_CARTAO_DELETE_'||upper(rw_crapcop.dsdircop)||'_'||to_char(sysdate,'hh24miss')||'.sql';
    ...
    FOR rw_tbcrd_conta_cartao IN cr_tbcrd_conta_cartao(rw_crapcop.cdcooper) LOOP
      ...
      gene0002.pc_escreve_xml(vr_dados_rollback
                             ,vr_texto_rollback
                             ,'INSERT INTO tbcrd_conta_cartao '                    || chr(13) || 
                              'VALUES ('||rw_tbcrd_conta_cartao.cdcooper           || chr(13) ||
                              ','||rw_tbcrd_conta_cartao.nrdconta                  || chr(13) ||
                              ','||NVL(rw_tbcrd_conta_cartao.nrconta_cartao,'0')   || chr(13) ||
                              ','||rw_tbcrd_conta_cartao.vllimite_global           || chr(13) ||
                              ','||rw_tbcrd_conta_cartao.cdadmcrd||');'            || chr(13) ||chr(13), FALSE);
    ...
    gene0002.pc_escreve_xml(vr_dados_rollback, vr_texto_rollback, 'BEGIN'||chr(13), FALSE);
    vr_cdcooper  := rw_crapcop.cdcooper;     
    vr_nmarqbkp  := 'BKP_TBCRD_CONTA_CARTAO_ADM_'||upper(rw_crapcop.dsdircop)||'_'||to_char(sysdate,'hh24miss')||'.sql';
                     
    FOR rw_tbcrd_conta_cartao_adm IN cr_tbcrd_conta_cartao_adm(rw_crapcop.cdcooper) LOOP
      
      OPEN  cr_cdamdcrd(rw_tbcrd_conta_cartao_adm.nrconta_cartao);
      FETCH cr_cdamdcrd
       INTO rw_cdamdcrd;
       
      IF NVL(rw_cdamdcrd.cdadmcrd,0) = 0 THEN
        open cr_bin(rw_crapcop.cdcooper, substr(rw_tbcrd_conta_cartao_adm.nrcrcard,1,6));
        fetch cr_bin
        into rw_bin;
        ...
        BEGIN
           UPDATE tbcrd_conta_cartao
            ...
            WHERE rowid = rw_tbcrd_conta_cartao_adm.rowid;
        END;                  
        ...
        gene0002.pc_escreve_xml(vr_dados_rollback
                               ...
                               ,'UPDATE tbcrd_conta_cartao '      || chr(13) || 
                                ...
                                ' WHERE rowid = '''    || rw_tbcrd_conta_cartao_adm.rowid  || '''; ' ||chr(13)||chr(13), FALSE);
