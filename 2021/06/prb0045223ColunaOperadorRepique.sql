DECLARE
  ...
  CURSOR cr_consor IS
    select * from TBCNS_REPIQUE a where a.cdopecan =1;
...
--  vr_nmdireto       := vr_rootmicros|| 'cpd/bacas';  
  pc_valida_direto(pr_nmdireto => vr_nmdireto || '/prb0045223',
                   pr_dscritic => vr_dscritic);
  ...
  vr_nmdireto := vr_nmdireto || '/prb0045223';
  ...
  vr_nmarqbkp := 'ROLLBACK_prb0045223' || to_char(sysdate, 'hh24miss') ||
                 '.sql';
  ----------------------------------------------------------------------
  for rw_cons in cr_consor loop
              update TBCNS_REPIQUE cns
               set cns.cdopecan = null
             where cns.idrepiqu = rw_cons.idrepiqu;
    
      ---------------------------------------------------------------------
      gene0002.pc_escreve_xml(vr_dados_rollback,
                              vr_texto_rollback,
                              'UPDATE TBCNS_REPIQUE      ' || chr(13) ||
                              '   SET cdopecan =   1 '||chr(13) ||                              
                              ' WHERE idrepiqu = ' || rw_cons.idrepiqu || chr(13) || ';' || chr(13)
                              ,
                              FALSE);
      commit;                        
