...
--      GENE0001.pc_escr_linha_arquivo(vr_ind_arquiv3, pr_msg);
  ...
  PROCEDURE abre_arquivos(pr_cooperativa INTEGER) IS
  BEGIN
    vr_nmarqimp2 := 'cooperativa_' || pr_cooperativa || '_' || '_' || vr_nmarqimp22 ||to_char(sysdate,'HH24MISS');
    vr_nmarqimp3 := 'cooperativa_' || pr_cooperativa || '_' || '_' || vr_nmarqimp33 ||to_char(sysdate,'HH24MISS');
