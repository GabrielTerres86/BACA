...
order by b.cdcooper, b.nrdconta, b.nrctremp, b.nrparepr;
	...
	v_nrdconta2 tbgen_evento_soa.NRDCONTA%type := 80438792;
	v_cdcooper2 tbgen_evento_soa.CDCOOPER%type := 1;
	v_nrctrprp2 tbgen_evento_soa.NRCTRPRP%type := 2956201;
	v_vlparpag2 VARCHAR2(10) := '893.38';
	...
	conta_80438792_01_21 CLOB := '<?xml version="1.0"?>
                              <Root>
                                <convenioCredito>
                                  <cooperativa>
                                    <codigo>' ||to_char(v_cdcooper2)|| '</codigo>
                                  </cooperativa>
                                  <numeroContrato>' ||to_char(v_nrctrprp2)|| '</numeroContrato>
                                </convenioCredito>
                                <propostaContratoCredito>
                                  <emitente>
                                    <contaCorrente>
                                      <codigoContaSemDigito>' ||to_char(v_nrdconta2)|| '</codigoContaSemDigito>
                                    </contaCorrente>
                                  </emitente>
                                </propostaContratoCredito>
                                <lote>
                                ...
                                <parcela>
                                  ...
                                  <valor>' ||to_char(v_vlparpag2)|| '</valor>
                                </parcela>
                                <motivoEnvio>REENVIARPAGTO</motivoEnvio>
                                <interacaoGrafica>
                                  <dataAcaoUsuario>' || dataAcaoUsuario || '</dataAcaoUsuario>
...
BEGIN
	...
	(CDCOOPER, NRDCONTA, NRCTRPRP, TPEVENTO, TPRODUTO_EVENTO, TPOPERACAO, DHOPERACAO, DSPROCESSAMENTO,
	 DSSTATUS, DHEVENTO, DSERRO, NRTENTATIVAS, DSCONTEUDO_REQUISICAO)
	VALUES
	(v_cdcooper, v_nrdconta, v_nrctrprp, 'PAGTO_PAGAR', 'CONSIGNADO', 'INSERT', SYSDATE,
	 NULL, NULL, NULL, NULL, NULL, conta_525774_09_21);
	 
	INSERT INTO tbgen_evento_soa
	(CDCOOPER, NRDCONTA, NRCTRPRP, TPEVENTO, TPRODUTO_EVENTO, TPOPERACAO, DHOPERACAO, DSPROCESSAMENTO,
	 DSSTATUS, DHEVENTO, DSERRO, NRTENTATIVAS, DSCONTEUDO_REQUISICAO)
	VALUES
	(v_cdcooper2, v_nrdconta2, v_nrctrprp2, 'PAGTO_PAGAR', 'CONSIGNADO', 'INSERT', SYSDATE,
	 NULL, NULL, NULL, NULL, NULL, conta_80438792_01_21	);
	...
	UPDATE crappep
	   SET vlpagpar = 897.98,
		   ...
		   vlsdvpar = 0,
		   vlsdvatu = 0
	 WHERE cdcooper = v_cdcooper2
	   AND nrdconta = v_nrdconta2
	   AND nrctremp = v_nrctrprp2
	   AND nrparepr = 14
	   AND inliquid = 0;	 
