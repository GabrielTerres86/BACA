...
CURSOR cr_craplcm IS       
     SELECT b.dtvencto
      ...
      FROM crappep                    pep
          ...
          ,tbepr_consig_parcelas_tmp  t
     WHERE (pep.nrdconta, pep.nrctremp, pep.cdcooper, pep.nrparepr) IN
           ((529885, 20100503, 9, 1),
            (519936, 21100077, 9, 2),
            (38970, 9548, 8, 5),
            (376744, 78331, 13, 12),
       ...
       AND t.cdcooper = pep.cdcooper
       AND t.nrdconta = pep.nrdconta
       AND t.nrctremp = pep.nrctremp
       AND t.nrparcela = pep.nrparepr
       AND t.dtmovimento = CASE pep.nrdconta
                           WHEN 38970 THEN '01/11/2021'
                           WHEN 376744 THEN '01/11/2021'
                           ELSE to_char(TRUNC(b.dtincreg))
                           END
       AND b.instatus <> 2
       AND pep.inliquid = 0
       AND pep.nrdconta = b.nrdconta
...
BEGIN
    ...
    FOR rw_craplcm IN cr_craplcm LOOP
        ...
        vr_tipo_pagto := ' <valor>'||trim(to_char(rw_craplcm.vlsaldoparc,'999999990.00'))||'</valor>' ;
        vr_xml_parcela := ' <parcela>
                            <dataEfetivacao>'||to_char(rw_craplcm.dtmovimento,'yyyy-mm-dd')||'T'||to_char(sysdate,'hh24:mi:ss')||'</dataEfetivacao> 
