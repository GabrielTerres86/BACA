CREATE OR REPLACE PROCEDURE CECRED.pc_operacoa_diaria_pix_retro_viacredi(pr_cdcooper IN crapcop.cdcooper%TYPE
  ...
  vr_dataini           DATE;
  vr_dtini             DATE := to_date('01/' || to_char(pr_cdmes,'fm00') || '/2021', 'DD/MM/RRRR');
...
--      GENE0001.pc_escr_linha_arquivo(vr_ind_arquiv3, pr_msg);
  ...
  BEGIN
    vr_nmarqimp2 := 'viacredi_' || pr_cdmes || '_' || vr_nmarqimp22 ||to_char(sysdate,'HH24MISS');
    vr_nmarqimp3 := 'viacredi_' || pr_cdmes || '_' || vr_nmarqimp33 ||to_char(sysdate,'HH24MISS');
