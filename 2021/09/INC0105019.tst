...
150
-- Created on 09/09/2021 by T0032717 
DECLARE
  ...
  vr_cdcooper crapepr.cdcooper%TYPE := 13;
  vr_nrdconta crapepr.nrdconta%TYPE := 340693;
  vr_nrctremp crapepr.nrctremp%TYPE := 133468;
...
BEGIN
  ...
  BEGIN
    INSERT INTO craplcm (cdcooper ,dtmvtolt, cdagenci, cdbccxlt, nrdolote, nrdconta, nrdctabb
       ...
       , vllanmto)
    VALUES  
       (vr_cdcooper, rw_craplot.dtmvtolt, rw_craplot.cdagenci, rw_craplot.cdbccxlt, rw_craplot.nrdolote, vr_nrdconta, vr_nrdconta
       , GENE0002.fn_mask(vr_nrdconta,'99999999')
       , GENE0002.fn_mask(vr_nrctremp || TO_CHAR(sysdate,'SSSSS') || (nvl(rw_craplot.nrseqdig,0) + 1),'999999999999999') -- rw_crabepr.nrctremp
       , 15 -- CR.EMPRESTIMO
       , nvl(rw_craplot.nrseqdig,0) + 1
       , GENE0002.fn_mask(vr_nrctremp,'99999999')
  ...
  gene0002.pc_escreve_xml(vr_dados_rollback
                        , vr_texto_rollback
                        , 'DELETE FROM craplcm WHERE cdcooper = '||vr_cdcooper||' AND nrdconta = '||vr_nrdconta||' AND cdhistor = 15 AND progress_recid = '||vr_progress_recid||';' || chr(13), FALSE); 
