   ...
   begin
    ...
    BEGIN 
      ...
      BEGIN
        ...
        INSERT INTO tbseg_prestamista (
           ...
           ,rw_prest.nrtelefo
           ,rw_prest.dtdevend
           ,to_date(trunc(sysdate,'MONTH')-1,'DD/MM/RRRR')
           ,rw_prest.nrctremp
           ,rw_prest.cdcobran
           ,rw_prest.cdadmcob
           ,rw_prest.tpfrecob
    ...
    BEGIN
        ...
        CURSOR cr_prestamista(pr_cdcooper IN crapcop.cdcooper%TYPE) IS
          SELECT p.idseqtra
           ...
           WHERE p.cdcooper = pr_cdcooper    
             ...
             and TRUNC(p.dtdevend) <= to_date('31/08/2021','DD/MM/RRRR')
      ...
      BEGIN
        ...
        vr_ultimoDia := to_date(trunc(sysdate,'MONTH')-1,'DD/MM/RRRR');    
        ...
        FOR rw_prestamista IN cr_prestamista(pr_cdcooper => pr_cdcooper) LOOP
          ...
          -- Se data da venda ou inicio de vigencia for o proximo mês e for uma adesao devemos enviar somente no proximo mês
          if (trunc(vr_dtdevend) >= TO_DATE('01/09/2021','DD/MM/RRRR') OR
              trunc(vr_dtinivig) >= TO_DATE('01/09/2021','DD/MM/RRRR')) and vr_tpregist = 1 then                          
            continue;
          end if;

          -- Tratamento para registros criados de cancelados no dia 01/09 na alto vale
          if trunc(vr_dtinivig) >= TO_DATE('01/09/2021','DD/MM/RRRR') and vr_tpregist = 3 and pr_cdcooper = 16 then
            vr_tpregist:= 1;
          end if;
          ...
          ELSE 
            ...
            IF vr_vlsdeved >= vr_vlminimo AND vr_tpregist = 2 and vr_vlsdatua > 0 THEN            
              ...
              END IF;
              -- novos sets
              vr_dtinivig := vr_ultimoDia; -- Ultimo dia do mês anterior
