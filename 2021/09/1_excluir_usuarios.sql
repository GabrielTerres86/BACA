declare
  ...
  CURSOR cr_usuario ( pr_idcooperado_cdc   tbepr_cdc_usuario_vinculo.idcooperado_cdc%TYPE
                     ,pr_nrcpf             tbepr_cdc_vendedor.nrcpf%TYPE ) IS
    SELECT  tcuv.idusuario
           ,tcv.idvendedor
    FROM    tbepr_cdc_usuario_vinculo   tcuv
           ,tbepr_cdc_usuario           tcu
           ,tbepr_cdc_vendedor          tcv
    WHERE tcuv.idusuario = tcu.idusuario
    AND   tcuv.idcooperado_cdc = pr_idcooperado_cdc
    AND   tcu.idusuario = tcuv.idusuario
    AND   tcv.idvendedor = tcuv.idvendedor
    AND   tcv.nrcpf      = pr_nrcpf;
begin
  OPEN cr_loja;
  FETCH  cr_loja
