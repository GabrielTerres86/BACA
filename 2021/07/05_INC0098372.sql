BEGIN
   ...
   ORDER BY  tcm.cdcooper,
    ...
    CURSOR cr_juros_rem (pr_cdcooper IN tbepr_consig_movimento_tmp.cdcooper%TYPE,
     ...
     SELECT NVL(SUM(nvl(tcm.vlsaldo,0)),0) vljurosrem
      ...
      WHERE tcm.cdcooper    = pr_cdcooper
        ...
        AND tcm.dtmovimento<= pr_dtmovimento
        AND NVL(tcm.instatusproces,'W') <> 'P'
        AND tcm.intplancamento = 10; -- Lancamentos de Juros remuneratï¿½rios
    ...
    CURSOR cr_contrato_liquidado(pr_cdcooper crappep.cdcooper%TYPE
      ...
      select nvl(max(0),1) inliquid
        from crappep cpp
       where cpp.inliquid = 0 -- Se inliquid for zero, a parcela não está liquidada, e havendo alguma parcela não liquidada o contrato não deve ser liquidado.
    ...
    vr_tab_pagto_ctr        typ_tab_pagto_ctr;
     ...
     -- Definiï¿½ï¿½o de tipo para armazenar os dados da data do movimento da FIS
    ...
    -- Funcao para buscar o hsitï¿½rico do estorno
  ...
  BEGIN    
    ...
    vr_dtmvtoan:= to_date('21/07/2021', 'DD/MM/YYYY');
    vr_dtmvtolt:= to_date('22/07/2021', 'DD/MM/YYYY');--rw_crapdat.dtmvtolt;    
    vr_dtmvtopr:= to_date('23/07/2021', 'DD/MM/YYYY');--rw_crapdat.dtmvtolt;
         ...
         --Limpar parametros saida
          ...
          LOOP
            ...
            IF cr_craplcrepr%NOTFOUND THEN
               ...
               CLOSE cr_craplcrepr;
               -- Gerar erro
               vr_cdcritic := 1178;-- linha de credito nï¿½o encontrada
            ...
            -- os lanï¿½amentos 1,8 e 9 jï¿½ sï¿½o criados na efetivaï¿½ï¿½o da proposta (b1wgen0084.grava_efetivacao_proposta)
            -- 1 - Valor liberado para o cliente (Debito)
            -- 8 - Valor de IOF cobrado (Credito)
            -- 9 - Valor de TAC cobrada (Credito)
            -- O lanï¿½amento do Juros remuneratï¿½rios (10), ï¿½ somado e lanï¿½ado somente no pagamento, no vencimento ou na mensal
            -- 16 - O lanï¿½amento de estorno de desconto nï¿½o ï¿½ lanï¿½ado no extrato de emprestimo
            -- Os lanï¿½amento 17 e 18 nï¿½o ï¿½ lanï¿½ado no extrato (craplem), pois nï¿½o financia IOF/TAXA
            -- 17 - Valor de IOF cobrado - dï¿½bito em conta corrente (bancado) (Dï¿½bito)
            -- 18 - Valor de TAC cobrada - dï¿½bito em conta corrente (bancado) (Dï¿½bito
            ...
            ELSIF rw_consig_movimento.intplancamento IN (1,8,9,10,16,17,18) THEN
              -- os lanï¿½amentos 1,8 e 9 jï¿½ sï¿½o criados na efetivaï¿½ï¿½o da proposta (b1wgen0084.grava_efetivacao_proposta)
              -- o lanï¿½amento 10, ï¿½ somado e lanï¿½ado somente no pagamento, no vencimento ou na mensal
              -- O lanï¿½amento 17 e 18, nï¿½o ï¿½ feito no extrato ( craplem), sï¿½o lanï¿½amentos refrente a ao Nï¿½o financia IOF
              -- O lanï¿½amento 16 - estorno de desconto nï¿½o ï¿½ lanï¿½ado
              vr_cdhistor:= NULL;
            END IF;
            ...
            IF (rw_consig_movimento.intplancamento = 2 OR -- Valor de Pagamento - Valor Principal amortizada
                ...
                IF vr_jurosrem > 0 THEN
                  ...
                  empr0001.pc_cria_lancamento_lem(pr_cdcooper => rw_consig_movimento.cdcooper --Codigo Cooperativa
                                                  ...
                                                  ,pr_vllanmto => vr_jurosrem -- Valor do lanï¿½amento
                    ...
                    -- Atualiza a data do lanï¿½amento do juros na crapepr
                    BEGIN
              ...
              -- P437.3 - Quando o contrato foi transferido para prejuï¿½zo, nï¿½o atualiza mais nada. 
            ...
            IF vr_cdhistor IS NOT NULL THEN
               ...
               -- Verifica se o lanï¿½amento existe no Aimaro
               ...
               ELSE
                   vr_cdorigem:= 5; --Aimaro web
               END IF;
               -- Armazenar na tabela de memï¿½ria a origem do movimento
