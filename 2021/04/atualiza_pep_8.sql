DECLARE--589,024
  ...
  vr_nmarquiv   VARCHAR2(100)  := 'atualizacao_pep_8.csv';
  ...
  FOR rw_crapass IN cr_crapass LOOP
    ...
    BEGIN
      ...
      IF instr(vr_res_body, '<DATA>') > 0 THEN
        ...
        BEGIN
          IF instr(vr_pep.vr_dsrelacmnto, 'IRMÃ(O)') > 0 THEN
            vr_pep.vr_cdrelacmnto := 9;
            vr_pep.vr_relacmnto   := 'IRMÃ(O) - Parente';
        ...
        IF instr(vr_pep.vr_dsocupacao, 'SUPLENTE') > 0 THEN
          vr_pep.vr_log := vr_pep.vr_log || 'Suplente, não altera dados|';
          CONTINUE;
        END IF;
        ...
        EXCEPTION
           WHEN OTHERS THEN
             vr_pep.vr_idpessoapep := NULL;
             vr_pep.vr_log := vr_pep.vr_log || 'Não foi encontrado esse cpf do pep na tbcadast_pessoa, não incluirá o idpessoa_politico|';
        END;
        ...
        IF vr_pep.vr_idpessoa IS NOT NULL THEN
          ...
          IF vr_count = 0 THEN
            ...
            END IF;
          ELSE 
            vr_pep.vr_log := vr_pep.vr_log || 'Já existe tbcadast_pessoa_polexp, não altera dados|';
          END IF;
        ELSE
          vr_pep.vr_log := vr_pep.vr_log || 'Não foi encontrado esse cpf cadastrado na tbcadast_pessoa, não altera tbcadast_pessoa_polexp|';
        END IF;
        ...
        FOR rw_crapttl IN cr_crapttl(vr_pep.vr_cpfpesquisa) LOOP
          ...
          IF vr_count = 0 THEN
            ...
            END IF;
          ELSE 
            vr_pep.vr_log := vr_pep.vr_log || 'Já existe tbcadast_politico_exposto, não altera dados|';
          END IF;
        END LOOP;
        
        gene0002.pc_escreve_xml(vr_texto_carga,vr_texto_carga_aux,rw_crapass.nrcpfcgc || ';' ||
                                                                  rw_crapass.nmextttl || ';' ||
                                                                  CASE WHEN vr_pep.vr_flgpep THEN 'Sim' ELSE 'Não' END || ';' ||
                                                                  vr_pep.vr_cpfpep || ';' ||
                                                                  vr_pep.vr_nomepep || ';' ||
                                                                  vr_pep.vr_tpexposto || ';' ||
