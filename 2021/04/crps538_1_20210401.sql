DECLARE
  ...
  --
  vr_data_aux_compefora DATE := TO_DATE('01/04/2021', 'DD/MM/YYYY');
  --
  vr_cdcritic INTEGER;
  vr_dscritic VARCHAR2(4000);
  --
  PROCEDURE pc_carga_cratrej(pr_cdcooper crapcop.cdcooper%TYPE) IS
  BEGIN
    ...
    BEGIN
      ...
      FOR rw_craprej IN cr_craprej(pr_cdcooper    => pr_cdcooper
                                  ...
                                  ,pr_dtmvtolt    => vr_data_aux_compefora)
      LOOP
...
PROCEDURE baca_crps538_1(pr_cdcooper    IN crapcop.cdcooper%TYPE   --> Codigo Cooperativa
  ...
  /* .............................................................................
   ...
   13/01/2021 - Consultar o título na CIP antes de executar a instrução de protesto - PRB0044089 (Jose Dill - Mouts)
   ...
   .............................................................................*/
     ...
     DECLARE
       ...
       vr_typ_craprej_array   TYP_CRAPREJ_ARRAY := TYP_CRAPREJ_ARRAY();
       TYPE typ_tab_crapmot IS TABLE 
                              OF crapmot.dsmotivo%type 
    ...
    BEGIN         
      ...
      CECRED.pc_log_programa( pr_dstiplog      => pr_dstiplog 
                             ...
                             ,pr_dsmensagem    => pr_dscritic ||
                                                         ...
                                                         ', vr_inreproc:' || 
                                                         CASE vr_inreproc WHEN true THEN 'true' ELSE 'false' END  ||                                                         
    ...
    BEGIN
         ...
         FOR rw_crapcco IN cr_crapcco_relat (pr_cdcooper => pr_cdcooper
           ...
           --INC0085009
           vr_nmarqimp := 'crrl605_20210401_' || gene0002.fn_mask(rw_crapcco.nrconven, '9999999') || '_' ||
...
*/
           ...
           IF vr_typ_craprej_array(vr_index_cratrej).cdcooper = pr_cdcooper THEN   
             --Verificar convenio      
             IF SUBSTR(vr_typ_craprej_array(vr_index_cratrej).cdpesqbb,20,6) = gene0002.fn_mask(rw_crapcco.nrconven,'999999') THEN
               --Marcar rejeitado
               vr_rejeitad:= TRUE;
               ...
               '<lancto>
                  <nrseqdig>'||to_char(vr_typ_craprej_array(vr_index_cratrej).nrseqdig,'fm999g990')||'</nrseqdig>
                  <nrdconta>'||to_char(vr_typ_craprej_array(vr_index_cratrej).nrdconta,'fm9g999g999g9')||'</nrdconta>
                  <nrdocmto>'||to_char(vr_typ_craprej_array(vr_index_cratrej).nrdocmto,'fm999g999g990')||'</nrdocmto>
                  <cdpesqbb>'||substr(vr_typ_craprej_array(vr_index_cratrej).cdpesqbb,1,44)||'</cdpesqbb>
                  <vllanmto>'||to_char(vr_typ_craprej_array(vr_index_cratrej).vllanmto,'fm999g999g990d00')||'</vllanmto>
                  <cdbccxlt>'||gene0002.fn_mask(vr_typ_craprej_array(vr_index_cratrej).cdbccxlt,'zz9')||'</cdbccxlt>
                  <cdagenci>'||gene0002.fn_mask(vr_typ_craprej_array(vr_index_cratrej).cdagenci,'zzz9')||'</cdagenci>
...
--             vr_dtmvtolt:= rw_crapdat.dtmvtoan;
             --INC0085009
             vr_dtmvtolt:= TO_DATE('01/04/2021', 'DD/MM/YYYY');
           ELSE
...
--             vr_dtmvtaux:= rw_crapdat.dtmvtoan;
             --INC0085009
             vr_dtmvtaux:= TO_DATE('01/04/2021', 'DD/MM/YYYY');
           ELSE
...
--       vr_dtdpagto_rel:= rw_crapdat.dtmvtoan;
        --INC0085009
        vr_dtdpagto_rel:= TO_DATE('01/04/2021', 'DD/MM/YYYY');
      ELSE
       --Data Atual
      ...
      FOR rw_crapret IN cr_crapret_sld (pr_cdcooper => pr_cdcooper
                                      ...
                                      --INC0085009
                                      ,pr_dtmvtopr => TO_DATE('05/04/2021', 'DD/MM/YYYY')) LOOP
--                                      ,pr_dtmvtopr => rw_crapdat.dtmvtopr) LOOP
      ...
      --INC0085009
      vr_nmarqimp := 'crrl686_20210401.lst';
        /*
        IF vr_inreproc THEN
    ...
    BEGIN
      ...
      IF NOT vr_inreproc THEN
        ...
        FOR rw_crapcco IN cr_crapcco_ativo(pr_cdcooper => rw_crapcop.cdcooper
          ...
          vr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => vr_cdcritic) ||
                         ' convenio:' || to_char(rw_crapcco.nrconven);
              ...
              FOR rw_crapcob IN cr_crapcob_prot (pr_cdcooper => rw_crapcco.cdcooper
                ...
                IF nvl(rw_crapcob.flgdprot,0) = 1 AND nvl(rw_crapcob.flgregis,0) = 1 THEN
                  ...
                  IF (rw_crapcob.dtvencto + rw_crapcob.qtdiaprt) <= vr_dtmvtaux THEN
                    ...
                    npcb0001.pc_consulta_sit_titulo_cip (pr_rowidcob => rw_crapcob.rowid
                                                        , pr_idorigem => 1 
                                                        , pr_cdoperad => vr_cdoperad
                                                        , pr_situacao => vr_sit_titulo_cip);
                    --
                    IF npcb0001.fn_verifica_bloq_inst_cip(vr_sit_titulo_cip) = 'N' THEN
                    ...
                    ELSE
                      ...
                      ,pr_cdoperad => vr_cdoperad --Operador
                      ,pr_dtmvtolt => rw_crapdat.dtmvtolt --Data movimento
                      ,pr_dsmensag => 'Protesto nao efetuado: Baixa operacional encontrada na CIP'||
                      vr_dscritic --Descricao Mensagem
                      ,pr_des_erro => vr_des_erro --Indicador erro
                      ,pr_dscritic => vr_dscritic);
        ...
        vr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => vr_cdcritic) || ' Lancamento Tarifas.';
        -- Incluido controle de Log - Chamado 714566 - 11/08/2017  
        pc_controla_log_programa( pr_dstiplog => 'O'
                                 ,pr_tpocorre => 4
                                 ,pr_dscritic => vr_dscritic
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        pr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => pr_cdcritic) ||
                       ' pr_cdcooper:' || pr_cdcooper ||
                       ', pc_atualiza_protesto_baixa - ' || SQLERRM; 
...
BEGIN
  --
  FOR rw_crapcop IN (SELECT cdcooper FROM crapcop cop WHERE cop.cdcooper IN (2, 5, 6, 7, 9, 10, 11, 12, 13, 14, 16)) LOOP
    --
    vr_typ_craprej_array := TYP_CRAPREJ_ARRAY();
    vr_cdcritic := 0;
    vr_dscritic := '';
    --
    pc_carga_cratrej(pr_cdcooper => rw_crapcop.cdcooper);
    --
    baca_crps538_1(pr_cdcooper    => rw_crapcop.cdcooper
                  ,pr_nmtelant    => 'COMPEFORA'
                  ,pr_dsreproc    => 'false'
                  ,pr_nmarquiv    => '29999401.RET'
