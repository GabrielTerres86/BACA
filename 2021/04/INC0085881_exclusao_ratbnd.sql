-- Created on 22/04/2021 by T0033801 
DECLARE 

CURSOR C1 IS
SELECT REGEXP_SUBSTR('36374,50636,255238,128678,234885,130125,242403,18147','[^,]+', 1, LEVEL) AS PR_NRDCONTA,
       REGEXP_SUBSTR('13755,24771,1,1,1,18344,1,1','[^,]+', 1, LEVEL) AS PR_NRCTRATO,
       '14' AS PR_CDCOOPER 
  FROM DUAL 
CONNECT BY REGEXP_SUBSTR('36374,50636,255238,128678,234885,130125,242403,18147','[^,]+', 1, LEVEL) IS NOT NULL;
