...
PROCEDURE pc_crps538_20210405(pr_cdcooper IN crapcop.cdcooper%TYPE --> Codigo Cooperativa
...
BEGIN
  ...
  Data    : Novembro/2009.                   Ultima atualizacao: 09/09/2020
  ...
  DECLARE
    ...
    CURSOR cr_boletos_pagos_acordos(pr_cdcooper IN crapret.cdcooper%TYPE
      ...
      SELECT aco.cdcooper
       ...
       WHERE ret.cdcooper = pr_cdcooper
         ...
         AND cob.rowid IN ('AA9oheABeAAHoqYAAN', 'AA9oheABkAAC6VvAAE', 'AA9oheABkAACe7QAAR');
    ...
    PROCEDURE pc_baixa_eft_interbca_local(pr_cdcooper IN INTEGER,      --> Cooperativa
      ...
      /* ..........................................................................
       ...
       Programa : pc_baixa_eft_interbca_local       
            ...
            SELECT cob.rowid idrowid_cob
              FROM crapcob cob
                  ,(SELECT (SELECT ceb.cdcooper
                      ...
                      FROM gncptit tit
                     WHERE tit.cdcooper = 1
                       AND tit.dtmvtolt >= to_date('01042021', 'ddmmyyyy')
                       AND tit.flgconci = 0
                       AND tit.cdbandst = 85
                       AND tit.nmarquiv = '80101401.090') a
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        -- definir retorno
        pr_cdcritic := 0;
        pr_dscritic := 'Erro inesperado ao pc_baixa_eft_interbca_local:' || SQLERRM;
        
    END pc_baixa_eft_interbca_local;
      ...
      LOOP
        ...
        IF vr_index_rel618 = 1 OR vr_cdbanpag_qbr <> vr_cdbanpag THEN
          ...
          ELSE
            --Nome arquivo Impressao
            vr_nmarqimp := 'crrl618_' || rw_crapcop.dsdircop || '_' ||
                           LTRIM(gene0002.fn_mask(vr_cdbanpag, '999')) ||
                           GENE0002.fn_busca_time || '.lst';
          END IF;
          --Descricao da Origem
    ...
    PROCEDURE pc_integra_cobranca_registrada(pr_cdcritic OUT INTEGER
      ...
      vr_exc_saida   EXCEPTION;
      --
      CURSOR cr_crapcob_210401(pr_cdcooper crapcob.cdcooper%TYPE
                              ,pr_dtcredit crapret.dtcredit%TYPE) IS
        SELECT cob.rowid idrowid_cob
              ,a.*
          FROM crapcob cob
              ,crapret ret
        ...
        IF (rw_crapdat.inproces > 2 -- Processo Batch
          ...
          LOOP
            ...
            *************************************************************/
            ...
            FOR rw_crapcob_210401 IN cr_crapcob_210401(pr_cdcooper => rw_crapcco.cdcooper
              ...
              PAGA0001.pc_valores_a_creditar(pr_cdcooper            => rw_crapcco.cdcooper
                                            ...
                                            ,pr_idtabcob            => rw_crapcob_210401.idrowid_cob
          ...
          IF NOT vr_inreproc THEN
            ...
            --> Comandar baixa efetiva de boletos pagos no dia fora da cooperativo(interbancaria)
            pc_baixa_eft_interbca_local(pr_cdcooper => pr_cdcooper
                                               , --> Cooperativa
        ...
        IF (rw_crapdat.inproces > 2 -- 1-on line, 2-agenda, >2-Batch
          ...
          LOOP
            ...
            ************************************************************************/
            ...
            IF rw_crapcco.dsorgarq IN ('EMPRESTIMO', 'ACORDO', 'DESCONTO DE TITULO') AND
               vr_inreproc THEN
              ...
              EXCEPTION
                WHEN OTHERS THEN
                  ...
                  vr_cdcritic := 1035;
                  vr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => vr_cdcritic) ||
                                 ' crapret(3):' || ' flcredit:' || '2' || ', cdcooper:' ||
                                 rw_crapcco.cdcooper || ', nrcnvcob:' || rw_crapcco.nrconven ||
                                 ', dtcredit:' || vr_dtcredit || ', flcredit:' || '1' || '. ' ||
                                 SQLERRM;
                  RAISE vr_exc_saida;
              END;
            END IF;
            ...
            IF rw_crapcco.dsorgarq IN ('EMPRESTIMO', 'ACORDO', 'DESCONTO DE TITULO') AND
              ...
              EXCEPTION
                WHEN OTHERS THEN
                  ...
                  vr_cdcritic := 1035;
                  vr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => vr_cdcritic) ||
                                 ' crapret(4):' || ' flcredit:' || '1' || ', cdcooper:' ||
                                 rw_crapcco.cdcooper || ', nrcnvcob:' || rw_crapcco.nrconven ||
                                 ', dtcredit:' || vr_dtcredit || ', flcredit:' || '2' || '. ' ||
                                 SQLERRM;
                  RAISE vr_exc_saida;
              END;
            END IF;
  ...
  END;
  --
END pc_crps538_20210405;

BEGIN

  FOR rw_crapcop IN (select cdcooper
                      from crapcop cop
                      where cop.cdcooper in (2,5,6,7,9,10,11,12,13,14,16)) LOOP
    vr_stprogra := 0;
    vr_infimsol := 0;
    vr_cdcritic := 0;
    vr_dscritic := ' ';
    pc_crps538_20210405(pr_cdcooper => rw_crapcop.cdcooper,
                        pr_cdagenci => 0,
                        pr_idparale => 0,
                        pr_flgresta => 0,
                        ...
                        pr_cdcritic => vr_cdcritic,
                        pr_dscritic => vr_dscritic);
    dbms_output.put_line( rw_crapcop.cdcooper ||' - '|| vr_cdcritic ||' - '|| vr_dscritic );
  END LOOP;
