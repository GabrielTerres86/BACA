...
DECLARE
  ...
  BEGIN
    ...
    registrarVERLOG(pr_cdcooper  => prm_cdcooper
                   ,pr_nrdconta  => prm_nrdconta
                   ,pr_dstransa  => 'Ajuste Contabil - prc_gera_acerto_transit'
  ...
  BEGIN
    ...
    registrarVERLOG(pr_cdcooper  => prm_cdcooper
                   ,pr_nrdconta  => prm_nrdconta
                   ,pr_dstransa  => 'Ajuste Contabil - prc_gera_acerto'
  ...
  PROCEDURE prc_gera_acerto_debt (prm_cdcooper IN craplcm.cdcooper%TYPE,
                                  prm_nrdconta IN craplcm.nrdconta%TYPE,
                                  prm_vllanmto IN craplcm.vllanmto%TYPE) IS
  ...
  BEGIN
    ...
    dbms_output.put_line('   Parâmetros (prc_gera_acerto_debt):');
    dbms_output.put_line('     Cooperativa....: '||prm_cdcooper);
    dbms_output.put_line('     Conta..........: '||prm_nrdconta);
    dbms_output.put_line('     Data...........: '||To_Char(vr_dtmvtolt,'dd/mm/yyyy'));
    dbms_output.put_line('     Valor..........: '||prm_vllanmto);
   ...
   prej0003.pc_gera_debt_cta_prj(pr_cdcooper  => prm_cdcooper
    ...
    IF Nvl(vr_cdcritic,0) > 0 OR Trim(vr_dscritic) IS NOT NULL THEN
      RAISE vr_erro; 
    ELSE
      dbms_output.put_line('   Lançamento debito Transitoria efetuado com Sucesso na Coop\Conta '||prm_cdcooper||'\'||prm_nrdconta||'.');  
    END IF;

    registrarVERLOG(pr_cdcooper  => prm_cdcooper
                   ,pr_nrdconta  => prm_nrdconta
                   ,pr_dstransa  => 'Ajuste Contabil - prc_gera_acerto_debt'
                   ,pr_dsCampo   => 'pc_gera_lcto_extrato_prj'
                   ,pr_antes     => ''
                   ,pr_depois    => prm_vllanmto);
  ...
  EXCEPTION
    WHEN vr_erro THEN
      dbms_output.put_line('prc_gera_acerto_debt: '||vr_dscritic);
      ROLLBACK;
      dbms_output.put_line('Efetuado Rollback.');
      Raise_Application_Error(-20001,vr_dscritic);
    WHEN OTHERS THEN
      dbms_output.put_line('prc_gera_acerto_debt: Erro: '||SubStr(SQLERRM,1,255));
      ROLLBACK;
      dbms_output.put_line('Efetuado Rollback.');
      Raise_Application_Error(-20002,'Erro Geral no prc_gera_acerto_debt. Erro: '||SubStr(SQLERRM,1,255));    
  END prc_gera_acerto_debt;
 ...
 PROCEDURE prc_gera_lct (prm_cdcooper IN craplcm.cdcooper%TYPE,
  ...
  BEGIN
    ...
    registrarVERLOG(pr_cdcooper  => prm_cdcooper
                   ,pr_nrdconta  => prm_nrdconta
                   ,pr_dstransa  => 'Ajuste Contabil - prc_gera_lct'
                   ,pr_dsCampo   => 'pc_gera_lcto_extrato_prj'
                   ,pr_antes     => ''
                   ,pr_depois    => prm_vllanmto);
...
-- 2408 - Quando EXCLUI, Diminui o Saldo Prejuizo     [C] 2408 - TRF. PREJUIZO-> TBCC_PREJUIZO_DETALHE
    ...
    registrarVERLOG(pr_cdcooper  => prm_cdcooper
                   ,pr_nrdconta  => prm_nrdconta
                   ,pr_dstransa  => 'Ajuste Contabil - prc_atlz_prejuizo'
...
-- ## 01
---------------------------------------------------
  -- INC0079464 - Credcrea - C/C 266914  - R$ 280.00
  -- -> Solução: Incluir um 2408 no valor de R$ 280.00
---------------------------------------------------
  vr_incidente := 'INC0079464';
  vr_cdcooper  := 7;
  vr_nrdconta  := 266914;
  vr_vllanmto  := 280.00;
  vr_cdhistor  := 2408;
