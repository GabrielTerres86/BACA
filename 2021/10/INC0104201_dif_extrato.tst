...
DECLARE
  ...
  CURSOR cr_crapepr IS
    SELECT a.cdcooper, a.nrdconta, a.nrctremp, a.vlsdprej, a.dif_multa,a.dif_mora,a.dif_iof
      FROM (SELECT cdcooper, nrdconta, nrctremp, vlsdprej, 
          nvl((vlttmupr - vlpgmupr),0) dif_multa, 
          nvl((vlttjmpr - vlpgjmpr),0) dif_mora,
          nvl((vltiofpr - vlpiofpr),0) dif_iof 
              FROM crapepr 
             WHERE cdcooper = 12
               AND nrdconta in ( 299,119830,70580,90557,128830)
               AND nrctremp in (23400,23576,16059,17191,13849,13859,16061,16327,17204)
               AND vlsdprej > 0
      ) a;
...
BEGIN
  ...
  vr_nmdireto := vr_nmdireto||'cpd/bacas/INC0104201'; 
  vr_nmarqbkp  := 'ROLLBACK_VIA_INC0104201_'||to_char(sysdate,'ddmmyyyy_hh24miss')||'.sql';
  ...
  FOR rw_crapepr IN cr_crapepr LOOP
    ...
    CLOSE cr_craplem;
      -- atualiza o valor 
      UPDATE crapepr 
         SET vlsdprej = rw_craplem.lancamento - (rw_crapepr.dif_multa + rw_crapepr.dif_mora + rw_crapepr.dif_iof) --essa subtração ocorre apenas em contratos pós-fixado, conforme está na geração do extrato
