DECLARE
  pr_dtmvtolt  DATE := to_date('30/04/2021', 'dd/mm/rrrr');
  pr_dtmvtolt2 DATE := to_date('30/06/2021', 'dd/mm/rrrr');
  vr_dsjobnam  VARCHAR2(100) := 'JBCRD_IMPORTA_UTLZ_CARTAO - VIA BACA';

  TYPE typ_reg_utlz_crd IS RECORD(
    dtmvtolt         tbcrd_intermed_utlz_cartao.dtmvtolt%TYPE,
  ...
  TYPE typ_tab_utlz_crd IS TABLE OF typ_reg_utlz_crd;

  -- Definicao do tipo de tabela para as conta cartao
  TYPE typ_reg_conta_crd IS RECORD(
    cdcooper crapass.cdcooper%TYPE,
    nrdconta crapass.nrdconta%TYPE);
  TYPE typ_tab_conta_crd IS TABLE OF typ_reg_conta_crd INDEX BY VARCHAR2(15);
  ...
  CURSOR cr_intermedcrd IS
    SELECT inmedcrd.dtmvtolt,
     ...
     WHERE inmedcrd.dtmvtolt = pr_dtmvtolt
        OR inmedcrd.dtmvtolt > pr_dtmvtolt2;

  -- Busca conta e cooper do cooperado
  CURSOR cr_contacrd IS
    SELECT tbcc.cdcooper, tbcc.nrdconta, tbcc.nrconta_cartao
...
BEGIN
  ...
  OPEN cr_intermedcrd;
  FETCH cr_intermedcrd BULK COLLECT
    INTO vr_tab_utlz_crd;
  ...
  ELSE
    FOR idx IN vr_tab_utlz_crd.FIRST .. vr_tab_utlz_crd.LAST LOOP
      IF vr_tab_conta_crd.EXISTS(vr_tab_utlz_crd(idx).nrconta_cartao) =
         FALSE THEN
        -- Quando não encontrar o cooperado, gera log e continua para o próximo
        vr_cdcritic := 9;
        vr_dscritic := gene0001.fn_busca_critica(pr_cdcritic => vr_cdcritic) ||
                       'Nr Conta Cartao = ' || vr_tab_utlz_crd(idx).nrconta_cartao;
