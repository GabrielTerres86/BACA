...
/* Se não for encontrado o arquivo de Rollback no diretório especificado, pode-se resgatar o script retornado no campo DSXMLDAD dessa query:
	...
	select	a.dsxmldad
	from	crapslr a
	where	a.dsarqsai	like '%INC0110361%'
	and	a.dtsolici	=
		(select	max(x.dtsolici)
		from	crapslr x
		where	x.dsarqsai like '%INC0110361%');
...
from	(
	...
	from	tbcrd_linha_arquivo_item j,							/* Log importação arquivo CCR3 - campo código de rejeição */
		tbcrd_linha_arquivo_item i,							/* Log importação arquivo CCR3 - campo número do cartão */
	...
	where	nvl(trim(j.dsvalor),'000')		<> '004'				/* Somente se o código de rejeição for diferente de 004 */
	and	j.nmcampo				= 'CODREJEI'
	and	d.idlinha_arquivo			= j.idlinha_arquivo
	and	i.nmcampo				= 'NRCRCARD'
	...
	and	b.nrcctitg				= 0					/* Número da conta cartão zerada */
	and	b.insitcrd				in (1,2)
	and	b.dtpropos				between to_date('17/09/2021','dd/mm/yyyy') and to_date('20/09/2021 23:59:59','dd/mm/yyyy hh24:mi:ss')	/* Considerar casos da última execução da nova rotina 672 */
	) a
...
/* Diretório de Produção: /micros/cpd/bacas/INC0110361 */
if	(gene0001.fn_database_name like '%AYLLOSP%') then
	...
	ds_nome_diretorio_v	:= ds_nome_diretorio_v || 'cpd/bacas/INC0110361';

/* Diretório de Homologação: /progress/f0033552/usr/coop/cecred/INC0110361 */
else

	ds_nome_diretorio_v	:= gene0001.fn_diretorio(	pr_tpdireto => 'C',
								pr_cdcooper => 3);

	ds_nome_diretorio_v	:= ds_nome_diretorio_v || '/INC0110361';
...
gene0002.pc_escreve_xml(ds_dados_rollback_v, ds_texto_rollback_v, '-- Programa para rollback das informacoes INC0110361'||chr(13), false);
gene0002.pc_escreve_xml(ds_dados_rollback_v, ds_texto_rollback_v, 'begin'||chr(13), false);  
ds_nome_arquivo_bkp_v	:= 'ROLLBACK_INC0110361_'||to_char(sysdate,'hh24miss')||'.sql';
