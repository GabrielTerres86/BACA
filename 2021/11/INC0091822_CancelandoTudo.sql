DECLARE
  ...
  CURSOR cr_consor IS
    select c.progress_recid from crapcns c where c.flgativo = 1;
...
BEGIN
  ...
  pc_valida_direto(pr_nmdireto => vr_nmdireto || '/INC0091822',
                   pr_dscritic => vr_dscritic);
  ...
  vr_nmdireto := vr_nmdireto || '/INC0091822';
  ...
  vr_nmarqbkp := 'ROLLBACK_INC0091822' || to_char(sysdate, 'hh24miss') ||
                 '.sql';
  ----------------------------------------------------------------------
  for rw_cons in cr_consor loop
      update crapcns c  set c.flgativo = 0 where c.progress_recid = rw_cons.progress_recid; 

      ---------------------------------------------------------------------
      gene0002.pc_escreve_xml(vr_dados_rollback,
                              vr_texto_rollback,
                              'UPDATE crapcns      ' || chr(13) ||
                              '   SET flgativo =   1 '||chr(13) ||
