DECLARE
  ...
  CURSOR cr_consor IS
    select c.progress_recid, c.cdsitcns  from cecred.crapcns c where c.flgativo = 0 and c.cdsitcns = ' ';
...
--  vr_nmdireto := gene0001.fn_diretorio(pr_tpdireto => 'C', pr_cdcooper => 3);
  ...
  vr_nmarqbkp := 'ROLLBACK_INC0091822v2' || to_char(sysdate, 'hh24miss') ||
                 '.sql';
  ----------------------------------------------------------------------
  for rw_cons in cr_consor loop
      update cecred.crapcns c  set c.cdsitcns = 'DES' where c.progress_recid = rw_cons.progress_recid; 

      ---------------------------------------------------------------------
      gene0002.pc_escreve_xml(vr_dados_rollback,
                              vr_texto_rollback,
                              'UPDATE cecred.crapcns      ' || chr(13) ||
                              '   SET cdsitcns =   '||chr(13) || '''' ||rw_cons.cdsitcns|| '''' ||
