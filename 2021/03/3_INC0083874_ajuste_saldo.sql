...
and (e.cdcooper, e.nrdconta, e.nrctremp) in ( (13,  66940, 59732)
                                       ,(13, 217000, 67252)
                                      )
              JOIN craphis   h
...
BEGIN
  ...
  FOR rw_saldo_extrato IN cr_saldo_extrato LOOP
     ...
     rw_saldo_fis.vlsdev_empratu_d0 := 0;
     vr_dif_saldo := rw_saldo_extrato.saldo - rw_saldo_fis.vlsdev_empratu_d0;
   
     IF rw_saldo_extrato.nrdconta = 66940 THEN
       vr_dif_saldo := vr_dif_saldo - 0.60;
     ELSIF rw_saldo_extrato.nrdconta = 217000 THEN
       vr_dif_saldo := vr_dif_saldo + 0.02;
     END IF;
