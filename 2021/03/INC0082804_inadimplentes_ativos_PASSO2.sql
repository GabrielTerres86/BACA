DECLARE
  ...
  CURSOR cr_consInad IS
    select *
      from tbconsor_situacao a
     inner join crapcns b
        on b.cdsitcns = a.cdsituacao
     where a.nmsituacao = 'Inadimplente'
       and b.flgativo = 0;
...
--  vr_nmdireto := gene0001.fn_diretorio(pr_tpdireto => 'C', pr_cdcooper => 3);
  ...
  pc_valida_direto(pr_nmdireto => vr_nmdireto || '/INC0082804',
                   pr_dscritic => vr_dscritic);
  ...
  vr_nmdireto := vr_nmdireto || '/INC0082804';
  ...
  vr_nmarqbkp := 'ROLLBACK_INC0082804' || to_char(sysdate, 'hh24miss') ||
                 '.sql';
  ----------------------------------------------------------------------
  for rw_cons in cr_consInad loop
      ...
      gene0002.pc_escreve_xml(vr_dados_rollback,
                              ...
                              '   SET flgativo =   ' || rw_cons.flgativo    ||chr(13) ||                              
