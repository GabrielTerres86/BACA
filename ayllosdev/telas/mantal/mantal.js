...
function estadoInicial() {
	...
	$('input', '#'+ formDados ).limpaFormulario().removeClass('campoErro');
...
function controlaLayout() {
	...
	// Se mudar o valor do campo BANCO, verificar a tecla pressionada e toda a devida ação
	cBanco.unbind('blur').bind('blur', function() { 	
		if ( divError.css('display') == 'block' ) { return false; }	
		
		if ( cBanco.val() != '' )  {  
			if (  cBanco.val() != cBanco.attr('aux') ) { 
				operacao = operacao.charAt(0) + '1';
				cBanco.attr( 'aux', cBanco.val() );	
				manterRotina();
			}
		} else {
			cBanco.attr( 'aux', '' );	
	...
	if ( operacao == 'B0' || operacao == 'D0' ) {
		...
		$('input', '#'+ formDados ).habilitaCampo().limpaFormulario();
