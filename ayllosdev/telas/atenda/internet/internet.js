...
function obtemCntsPendentes( nriniseq, nrregist ) {
	...
	$.ajax({		
		type: "POST", 
		url: UrlSite + "telas/atenda/internet/contas_pendentes.php",
		dataType: "html",
		data: {
			nrdconta: nrdconta,
			idseqttl: idseqttl,
...
function detalhesPendentes(seq) {

	//Nome do form
	var objForm = $('#frmDetalhes');
	
	$('#divPesquisaRodape').css("display","none");
	$('#divBotoes').css("display","none");

	//Aumenta o div do conteúdo da opção
	$("#divConteudoOpcao").css("width","500");
	
	//Mostra o frame de Detalhes e esconde a lista de contas
	$("#frmDetalhes").css("display","block");
	$("#frmPendentes").css("display","none");
	
	//Esconte campos de CPF/CNPJ
	$('#CNPJ',objForm).css("display","none");
	$('#CPF',objForm).css("display","none");
	
	
	//Exibe elementos do Form de acordo com o Tp Pessoa
	if (reg[seq].inpessoa == 1){
	
		$('#CPF',objForm).css("display","block");
		$('#CPF',objForm).css("padding-left","23");
		
	} else {
	
		$('#CNPJ',objForm).css("display","block");
		$('#CNPJ',objForm).css("padding-left","17");
		
	}
	
	//Associa valore aos campos do form
	$('#cddbanco',objForm).val(reg[seq].cddbanco);
	$('#cdageban',objForm).val(reg[seq].cdageban);
	$('#nrctatrf',objForm).val(reg[seq].dsctatrf);
	$('#nmtitula',objForm).val(reg[seq].nmtitula);
	$('#nrcpfcgc',objForm).val(reg[seq].nrcpfcgc);
	$('#dstransa',objForm).val(reg[seq].dstransa);
	$('#dsprotoc',objForm).val(reg[seq].dsprotoc);
...
function escondeDetalhesPend() {
	
	//Retorna os forms à suas propriedades iniciais
	$("#divConteudoOpcao").css("width","650");
	$("#frmDetalhes").css("display","none");
	$("#frmPendentes").css("display","block");
...
function confirmaContas() {

	showMsgAguardo("Aguarde, confirmando contas ...")
	...
	$('#flgselec','#frmPendentes').each(function() {	
		if ($(this).prop('checked')) {
		
			//Armazena informações em um Array com índice igual ao seu tamanho
			seq = arrayConfirma.length;
			arrayConfirma[seq] = arrayPend[$(this).val()];
			
		}
	});
			
	//Verifica se alguma conta pendente foi selecionada
	if (arrayConfirma.length == 0) {
		hideMsgAguardo();
		showError("error","Nenhuma conta selecionada.","Alerta - Ayllos","bloqueiaFundo(divRotina)");
		return false;				
	}
	
	camposDc  = retornaCampos( arrayConfirma, '|' );
	dadosDc   = retornaValores( arrayConfirma, ';', '|',camposDc );
	
	$.ajax({		
		type: "POST", 
		dataType: "html",
		url: UrlSite + "telas/atenda/internet/contas_pendentes_confirmar.php",
		data: {
			nrdconta: nrdconta,
			idseqttl: idseqttl,
			camposDc: camposDc,
	...
	//Limpa Arrays
	arrayConfirma = new Array();
	arrayPend = new Array();
	
}


// Função para formatar o layout
function controlaListaPendentes() {
...
function controlaLayout( nomeForm ){
	...
	if(nomeForm != 'divPrincipalPJ' && nomeForm != 'divResponsaveisAss' ){
		...
		nomeForm = 'frmPendentes';
		$('#'+nomeForm).addClass('formulario');
		
		nomeForm = 'frmDetalhes';
		$('#'+nomeForm).addClass('formulario');
