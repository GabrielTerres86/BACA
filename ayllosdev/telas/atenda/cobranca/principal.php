			...
			<tbody>
				<?  for ($i = 0; $i < count($convenios); $i++) {					   
					   ...
					   $flsercco =  getByTagName($convenios[$i]->tags,'flsercco');
						...
						$mtdClick = "selecionaConvenio( '".$i."', '".$nrconven."','".$dsorgarq."','".$nrcnvceb."','".$dssitceb."','".$dtcadast."','".$cdoperad."','".$inarqcbr."','".$cddemail."' ,'".$dsdemail."','".$flgcruni."','".$flgcebhm."','".$flgregis."','".$flcooexp."','".$flceeexp."','".$cddbanco."','".$flserasa."','".$flsercco."');";
					?>
...
<div id="divBotoes">
	...
	<input type="image" src="<?php echo $UrlImagens; ?>botoes/cancelamento.gif"   <? if (in_array("X",$glbvars["opcoesTela"])) { ?> onClick="validaExclusao();return false;" <? } else { ?> style="cursor: default;" <? } ?> />
	...
	<input type="image" src="<?php echo $UrlImagens; ?>botoes/habilitar.gif" <? if (in_array("H",$glbvars["opcoesTela"])) { ?> onClick="habilita();return false;" <? } else { ?> style="cursor: default;" <? } ?> />
...
(executandoProdutos == true) ? habilita() : '';
