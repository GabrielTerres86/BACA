 ...
 **************************************************************************************/
	...
	$xmlResult = mensageria($xmlRenovaLimite, "TELA_ATENDA_DESCTO", "RENOVA_LIMITE_TIT", $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
	...
	if (strtoupper($xmlObjRenovaLimite->roottag->tags[0]->name) == "ERRO") {
		...
		exibirErro('error',htmlentities($msgErro),'Alerta - Ayllos',"blockBackground(parseInt($('#divUsoGenerico').css('z-index')))",false);
	}
	
	echo "showError(\"inform\",\"Opera&ccedil;&atilde;o efetuada com sucesso!\",\"Alerta - Ayllos\",\"fechaRotina($('#divUsoGenerico'),divRotina);carregaTitulos();\");";
