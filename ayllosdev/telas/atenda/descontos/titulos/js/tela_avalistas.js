// PRJ 438 - Sprint 16 - Variaveis relacionadas aos avalistas
var nomeDiv = 'divDscTit_Avalistas';
...
function formataAvalista() {
    ...
    cInpessoa.unbind('keydown').bind('keydown', function(e) {
        ...
        if (e.keyCode == __ARROW_DOWN || e.keyCode == __ARROW_LEFT || e.keyCode == __ARROW_RIGHT || e.keyCode == __ARROW_UP) {

            var cDesabilita = $('#nmdavali,#dtnascto,#cdnacion,#dsnacion,#nrctacjg,#nrcpfcjg,#nmconjug,#vlrencjg,#nrcepend,#dsendre1,#nrendere,#complend,#nrcxapst,#dsendre2,#cdufresd,#nmcidade,#nrfonres,#dsdemail,#vlrenmes', '#' + nomeDiv);
            cCPF.val('');
            cDesabilita.val('').desabilitaCampo();
    ...
    var rRotulo_1 = $('label[for="nmconjug"],label[for="nrctacjg"]', '#' + nomeDiv);
    var rCpf_1 = $('label[for="nrcpfcjg"]', '#' + nomeDiv);
    var rVlrencjg = $('label[for="vlrencjg"]', '#' + nomeDiv);
    
    var cConj = $('#nmconjug', '#' + nomeDiv);
    var cCPF_1 = $('#nrcpfcjg', '#' + nomeDiv);
    var cNrctacjg = $('#nrctacjg', '#' + nomeDiv);
    var cVlrencjg = $('#vlrencjg', '#' + nomeDiv);
...
function Busca_Associado(nrctaava, nrcpfcgc) { 
    ...
    $.ajax({
        type: 'POST',
        url: UrlSite + 'telas/atenda/descontos/titulos/busca_avalista.php',
        data: {
            nrdconta: nrdconta, 
            nrctaava: nrctaava, 
            nrcpfcgc: nrcpfcgc,
...
function controlaCamposTelaAvalista(cooperado){
    ...
    if (operacao != "C") {
        $('#nrctaava','#divDscTit_Avalistas').habilitaCampo();
    } else {
        $('#nrctaava','#divDscTit_Avalistas').desabilitaCampo();
    }

    $('#nrcpfcgc','#divDscTit_Avalistas').desabilitaCampo();
    ...
    if (inpessoa == 1) {
        ...
        //$('#nrcpfcgc', '#' + nomeDiv).removeClass('cnpj').addClass('cpf');
        ...
        //$('#nrcpfcgc', '#' + nomeDiv).removeClass('cpf').addClass('cnpj');
...
function buscarContasPorCpfCnpj(tipoConsulta){
	...
	$.ajax({
        type: 'POST',
        dataType: 'html',
        url: UrlSite + 'telas/atenda/descontos/titulos/busca_contas_por_cpf_cnpj.php',
        data: {
...
function buscarDadosContaConjuge(){
	...
	$.ajax({
        type: 'POST',
        dataType: 'html',
        url: UrlSite + 'telas/atenda/descontos/titulos/busca_dados_conta_conjuge.php',
        data: {
        	nrdconta: nrdconta,
        	nomeForm: nomeDiv,
            redirect: 'html_ajax'
...
function controlaVoltarAvalista(){
    // Verifica se está no Avalista 2, se sim, entao deve voltar para o Avalista 1, senão volta para a tela Observações
    if(contAvalistas == 2){
        contAvalistas = 1;
        ...
        if ($('#dsobserv','#divDscTit_Observacao').val() != '') {
            ...
            dscShowHideDiv('','divDscTit_Avalistas;divBotoesAval');
...
function controlaContinuarAvalista(){
    ...
    if (aux_conta == 0 && aux_cpf == 0 && arrayAvalistas.length == 0) {
		abrirTelaDemoDescontoTitulo(true);
        return false;
    }
...
function validaDadosAval() {
    ...
    $.ajax({
        type: 'POST',
        async: false,
        url: UrlSite + 'telas/atenda/descontos/titulos/valida_avalistas.php',
        data: {
            nrdconta: nrdconta, 
            nrcpfav1: nrcpfav1, 
            idavalis: idavalis,
...
function continuarSemValidarAvalistas(){
    ...
    if (operacao == "I") { // Incluir
        ...
        showConfirmacao("Deseja alterar os dados do limite de desconto de t&iacute;tulos?","Confirma&ccedil;&atilde;o - Aimaro","gravaLimiteDscTit(\'A\')","blockBackground(parseInt($(\'#divRotina\').css(\'z-index\')))","sim.gif","nao.gif");
    }     
}
