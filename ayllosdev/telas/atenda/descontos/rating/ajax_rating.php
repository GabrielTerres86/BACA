...
if ($flgopcao == 'salvarRating') { //salva Rating
    ...
    if ($contrato > 0 && $nrdconta > 0) {
        ...
        // Tipo de produto (0-Emprestimo/Financiamento, 1-Limite Desconto Cheque, 2-Limite Credito, 3-Desconto Titulo, 4-Cartao de Credito)
        $xml = "<Root>";
        $xml .= "<Dados>";
        $xml .= "	<nrcpfcgc></nrcpfcgc>";
        $xml .= "	<nrdconta>".$nrdconta."</nrdconta>";
        $xml .= "	<nrctro>".$contrato."</nrctro>";
        $xml .= "	<tpproduto>".$tipoproduto."</tpproduto>";
        ...
        if (strtoupper($xmlObj->roottag->tags[0]->name) == 'ERRO') {
            $msgErro = trim($xmlObj->roottag->tags[0]->cdata);
