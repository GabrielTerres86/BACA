<?php
	...
	************************************************************************************/
	...
	if (isset($_POST['ServerMonitoracao']) && trim($_POST['ServerMonitoracao']) <> '') $ServerMonitoracao = $_POST['ServerMonitoracao'];
	...
	for ($i = 0; $i < count($rotinasTela); $i++) {
		...
		switch ($rotinasTela[$i]) {
			...
			case "SEGURO": {
				$nomeRotina = "Seguro";    
				$urlRotina  = "seguro";    
				$strValue   = number_format(str_replace(",",".",$valores[16]->cdata),2,",",".");
				break;	
			}			
			case "TELE ATEN": {
				$nomeRotina = "Tele Atendimento";    
				$urlRotina  = "tele_atendimento"; 
				if($valores[10]->cdata == "") {	$strValue = "INATIVA"; }
				else { $strValue   = $valores[10]->cdata; }
				break;	
			}
			case "TELEFONE": {
				$nomeRotina = "Telefone";
