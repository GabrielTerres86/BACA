...
function valida_inclusao(reciprocidade,nrdconta,idparame_reciproci,inpessoa){
	...
	if ($("#cdpacote").val() == "") {
		showError('error','Informe um pacote!', 'Alerta - Ayllos', 'bloqueiaFundo(divRotina)');
		return false;
	}
...
function verificaPctCancelado(nrdconta, rotina, dtadesao) {
	...
	$.ajax({		
		...
		success: function(response) {
			if(response == "N") {
				if (rotina == 'C'){
					showConfirmacao('Deseja cancelar o pacote de servi&ccedil;os atual?',
