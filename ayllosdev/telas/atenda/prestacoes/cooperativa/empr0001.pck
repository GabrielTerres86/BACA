CREATE OR REPLACE PACKAGE CECRED.empr0001 AS
		...
		TYPE typ_reg_dados_epr IS RECORD(
				...
				,nrdiacar INTEGER
				,vliofcpl crapepr.vliofepr%TYPE);
...
CREATE OR REPLACE PACKAGE BODY CECRED.empr0001 AS
		...
		CURSOR cr_craptab(pr_cdcooper IN craptab.cdcooper%TYPE
				...
				SELECT tab.dstextab, tab.tpregist, tab.ROWID
				FROM craptab tab
		...
		CURSOR cr_crawepr(pr_cdcooper IN crapepr.cdcooper%TYPE
										 ,pr_nrdconta IN crapepr.nrdconta%TYPE
										 ,pr_nrctremp IN crapepr.nrctremp%TYPE) IS
				SELECT crawepr.cdlcremp, crawepr.vlpreemp, crawepr.dtlibera, crawepr.tpemprst
				FROM crawepr
		...
		BEGIN
				...
				DECLARE
						...
						CURSOR cr_crapemp IS
								SELECT emp.flgpagto, emp.flgpgtib
								FROM crapemp emp
						...
						CURSOR cr_craplem(pr_dtmvtolt IN craplem.dtmvtolt%TYPE) IS
								...
								ORDER BY lem.dtmvtolt, lem.cdhistor;
				...
				BEGIN
						...
						IF cr_crapemp%FOUND AND (vr_flgpagto_emp = 0 OR vr_flgpgtib_emp = 0) THEN
						...
						IF pr_diapagto > 0 AND rw_crapepr.flgpagto = 0 THEN
						...
						IF pr_rw_crapdat.inproces > 2 AND pr_cdprogra IN ('CRPS080', 'CRPS085', 'CRPS120') THEN
						...
						IF rw_crapepr.inliquid = 1 AND NVL(pr_vlsdeved, 0) = 0 THEN
								-- Verificar se existe registro de conta transferida entre
								-- cooperativas com tipo de transferência = 1 (Conta Corrente)
								OPEN cr_craptco;
								FETCH cr_craptco
										...
										IF rw_craplem.cdhistor IN
											 (88, 91, 92, 93, 94, 95, 120, 277, 349, 353, 392, 393, 507) THEN
												...
												IF rw_craplem.vlpreemp > 0 THEN
														...
														vr_qtprepag := ROUND(rw_craplem.vllanmto / rw_craplem.vlpreemp, 4);
												END IF;
												-- Para os movimentos
												------ --------------------------------------------------
												--  88 ESTORNO PAGTO
										...
										IF rw_craplem.cdhistor IN (91, 92, 94, 277, 349, 353, 392, 393) THEN
												...
												IF pr_vlsdeved > 0 THEN
														...
														IF vr_nrdiacal > rw_craplem.ddlanmto THEN
																...
																vr_vljurmes := vr_vljurmes + (rw_craplem.vllanmto * pr_txdjuros *
						...
						IF pr_rw_crapdat.inproces > 2 THEN
								...
								ELSE
										...
										IF TRUNC(vr_dtmvtolt, 'mm') <> TRUNC(pr_rw_crapdat.dtmvtopr, 'mm') THEN
												...
												vr_nrdiacal := to_char(pr_rw_crapdat.dtultdia, 'dd') - vr_nrdiacal;
										ELSE
												-- Dia para cálculo recebe o dia enviado - o dia dalculado
												vr_nrdiacal := to_char(vr_dtmvtolt, 'dd') - vr_nrdiacal;
										END IF;
						...
						IF pr_vlsdeved > 0 THEN
								...
								vr_vljurmes := vr_vljurmes + (pr_vlsdeved * pr_txdjuros * vr_nrdiacal);
						END IF;
						...
						IF pr_dtcalcul IS NOT NULL AND pr_vlsdeved > 0 THEN
								...
								vr_vljurmes := vr_vljurmes + (pr_vlsdeved * pr_txdjuros * vr_nrdiames);
						...
						ELSE
								...
								IF to_char(vr_dtmvtolt, 'dd') < pr_diapagto AND pr_rw_crapdat.inproces < 3 AND
									 pr_vlsdeved > 0 AND NOT vr_inhst093 THEN
						...
						IF vr_nrdiacal > 0 AND rw_crapepr.dtdpagto <= pr_rw_crapdat.dtultdia THEN
								...
								IF pr_vlsdeved > rw_crapepr.vlpreemp THEN
										...
										vr_vljurmes := vr_vljurmes + (rw_crapepr.vlpreemp * pr_txdjuros * vr_nrdiacal);
								ELSE
						...
						IF pr_vlsdeved > 0 AND rw_crapepr.inliquid > 0 THEN
								-- Se estiver rodando o processo batch no programa crps078
								IF pr_rw_crapdat.inproces > 2 AND pr_cdprogra = 'CRPS078' THEN
										...
										ELSE
												-- Gerar crítica
												vr_des_erro := 'ATENCAO: NAO FOI POSSIVEL ZERAR O SALDO - ' || ' CONTA = ' ||
		...
		BEGIN
				...
				BEGIN
						...
						IF rw_crapepr.flgpagto = 0 THEN
								...
								ELSE
										...
										IF ((rw_crapepr.dtdpagto > rw_crapdat.dtmvtolt) OR
											 (rw_crapepr.dtdpagto < rw_crapdat.dtmvtolt AND
											 to_char(rw_crapepr.dtdpagto, 'dd') <= to_char(rw_crapdat.dtmvtolt, 'dd'))) THEN
												-- Incrementar a quantidade de parcelas
												vr_qtmesdec := rw_crapepr.qtmesdec + 1;
										ELSE
												-- Consideramos a quantidade já calculadao
						...
						IF rw_crapepr.qtprecal > rw_crapepr.qtmesdec AND
							 rw_crapepr.dtdpagto <= rw_crapdat.dtmvtolt AND rw_crapepr.flgpagto = 0 THEN
        ...
        ............................................................................. */
				...
				BEGIN
						...
						ELSIF pr_dtdpagto > 28 THEN
								...
								IF vr_mes_datfinal = 2 AND vr_dia_datfinal >= 28 AND vr_dia_datfinal <> pr_dtdpagto THEN
						...
						IF ABS(vr_ano_datfinal - vr_ano_dtinicio) = 0 THEN
								...
								pr_qtdedias := (vr_mes_datfinal - vr_mes_dtinicio) * 30 + vr_dia_datfinal -
															 vr_dia_dtinicio;
						ELSE
        ...
        ............................................................................. */
				BEGIN
						...
						pr_vljinpar := apli0001.fn_round(pr_vlpagpar *
																						 apli0001.fn_round(POWER((1 + pr_txmensal / 100),
																																		 (pr_qtdiajur / 30)),
																															 10),
																						 10);
        ...
        ............................................................................. */
				...
				BEGIN
						...
						pr_vlatupar := ROUND(pr_vlsdvpar *
																 POWER(1 + (pr_txmensal / 100), vr_ndiasant / 30 * -1),
																 2);
				...
				EXCEPTION
						WHEN OTHERS THEN
								...
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_calc_antecipa_parcela> ' || SQLERRM;
        ...
        ............................................................................. */
				...
				BEGIN
						...
						-- ROUND(VALOR_PARCELA * (  EXP(1+(TAXA_MENSAL*100) , - DIAS_ADIANTAMENTO /30),2)
						vr_vlpresen := ROUND(pr_vlpagpar * POWER(1 + (pr_txmensal / 100), vr_ndiasant / 30), 2);
						-- Valor do desconto é igual ao valor devido - valor atualizado
				...
				EXCEPTION
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_calc_antec_parcel_parci. ' ||
															 SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => 0,
        ...
        ............................................................................. */
				DECLARE
						...
						CURSOR cr_crapepr IS
								SELECT epr.dtdpagto, epr.cdlcremp, epr.txmensal, epr.qttolatr
								FROM crapepr epr
								WHERE epr.cdcooper = pr_cdcooper
											AND epr.nrdconta = pr_nrdconta
											AND epr.nrctremp = pr_nrctremp;
						...
						CURSOR cr_craplcr(pr_cdlcremp IN craplcr.cdlcremp%TYPE) IS
								SELECT lcr.perjurmo, lcr.flgcobmu
								FROM craplcr lcr
				...
				BEGIN
						...
						BEGIN
								...
								IF rw_crappep.dtultpag IS NULL OR rw_crappep.dtultpag < rw_crappep.dtvencto THEN
								...
								empr0001.pc_calc_dias360(pr_ehmensal => FALSE -- Indica se juros esta rodando na mensal
																				,
																				 pr_dtdpagto => to_char(rw_crapepr.dtdpagto, 'dd') -- Dia do primeiro vencimento do emprestimo
																				,
                ...
                END IF;*/
								...
								ELSE
										...
										vr_txdiaria := ROUND((100 *
																				 (POWER((rw_craplcr.perjurmo / 100) + 1, (1 / 30)) - 1)),
																				 10);
										-- Dividimos por 100
								...
								IF pr_vlpagpar <> 0 THEN
										...
										pr_vlpagsld := pr_vlpagpar - (ROUND(pr_vlmtapar, 2) + ROUND(pr_vlmrapar, 2));
								ELSE
				...
				EXCEPTION
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_calc_atraso_parcela> ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_nrdcaixa,
        ...
        ............................................................................. */
				...
				CURSOR cr_crawepr(pr_cdcooper crawepr.cdcooper%TYPE
						...
						SELECT wpr.dtlibera, epr.inliquid
						FROM crapepr epr, crawepr wpr
				...
				CURSOR cr_crappep(pr_cdcooper crawepr.cdcooper%TYPE
												 ,pr_nrdconta crawepr.nrdconta%TYPE
												 ,pr_nrctremp crawepr.nrctremp%TYPE) IS
						SELECT nrparepr, dtvencto, vlsdvpar, dtultpag, vlparepr, vlpagmta, vlsdvsji, vlpagmra
						FROM crappep
						WHERE crappep.cdcooper = pr_cdcooper
									AND crappep.nrdconta = pr_nrdconta
									AND crappep.nrctremp = pr_nrctremp
		...
		BEGIN
				...
				BEGIN
						...
						FOR rw_crappep IN cr_crappep(pr_cdcooper => pr_cdcooper,
								...
								ELSIF rw_crappep.dtvencto > pr_rw_crapdat.dtmvtoan AND
                    ...
                    aux_nrdiamta é quantidade de dias de tolerância parametrizada */
										...
										-- Valor da Multa
										vr_vlmtapar := vr_vlmtapar + apli0001.fn_round(nvl(rw_crappep.vlparepr, 0) *
										...
										vr_vlatupar := apli0001.fn_round(rw_crappep.vlsdvpar *
																										 POWER((1 + nvl(pr_txmensal, 0) / 100),
																													 (nvl(vr_qtdedias, 0) / 30)),
																										 2);
								
										vr_vlpreapg := apli0001.fn_round(nvl(vr_vlpreapg, 0) + nvl(vr_vlatupar, 0), 2);
										-- Valor a Vencer
                    ...
                    aux_prtljuro é quantidade de dias de tolerância parametrizada */
										...
										ELSE
												vr_txdiaria := apli0001.fn_round((100 * (POWER((nvl(rw_craplcr.perjurmo, 0) / 100) + 1,
																																			 (1 / 30)) - 1)),
																												 10);
												vr_txdiaria := vr_txdiaria / 100;
												vr_vlmrapar := vr_vlmrapar + (nvl(rw_crappep.vlsdvsji, 0) *
								...
								ELSIF rw_crappep.dtvencto > pr_dtmvtolt THEN
										...
										IF ((to_char(rw_crappep.dtvencto, 'MM') = to_char(pr_dtmvtolt, 'MM')) AND
											 (to_char(rw_crappep.dtvencto, 'RRRR') = to_char(pr_dtmvtolt, 'RRRR'))) THEN
												vr_vlpraven := nvl(vr_vlpraven, 0) + nvl(rw_crappep.vlsdvpar, 0);
										END IF;
						...
						IF pr_dtmvtolt <= rw_crawepr.dtlibera AND rw_crawepr.inliquid <> 1 THEN
								/* Nao liberado */
				...
				BEGIN
						...
						BEGIN
								...
								FOR rw_crappep IN cr_crappep LOOP
										...
										ELSIF rw_crappep.dtvencto > pr_dtmvtoan AND rw_crappep.dtvencto <= pr_dtmvtolt THEN
												-- Parcela em dia
												...
												/* A regularizar */
												vr_vlpreapg := vr_vlpreapg + pr_tab_pgto_parcel(vr_ind_pag).vlatupar;
										...
										ELSIF rw_crappep.dtvencto < pr_dtmvtolt THEN
												...
												vr_vlpreapg := vr_vlpreapg + pr_tab_pgto_parcel(vr_ind_pag).vlatupar;
												-- Guardar quantidades calculadas
												vr_vlsdvctr := vr_vlsdvctr + pr_tab_pgto_parcel(vr_ind_pag).vlatupar;
										...
										IF NOT pr_dtmvtolt <= rw_crawepr.dtlibera THEN
												...
												pr_tab_pgto_parcel(vr_ind_pag).vlatrpag := NVL(pr_tab_pgto_parcel(vr_ind_pag)
																																			 .vlatupar,
																																			 0) + NVL(pr_tab_pgto_parcel(vr_ind_pag)
																																								.vlmtapar,
																																								0) +
        ...
        ............................................................................. */
				DECLARE
						...
						CURSOR cr_crapass IS
								SELECT ass.inpessoa, ass.cdtipsfx
								FROM crapass ass
				...
				BEGIN
						...
						IF NOT vr_tab_diadopagto.EXISTS(pr_cdcooper) AND rw_crapdat.inproces > 1 THEN
								...
								FOR rw_craptab IN cr_craptab(pr_cdcooper => pr_cdcooper,
										...
										vr_tab_diadopagto(vr_nrindice).diapgtoh := NVL(TRIM(SUBSTR(rw_craptab.dstextab,
																																							 7,
																																							 2)),
																																	 0);
										vr_tab_diadopagto(vr_nrindice).diapgtom := NVL(TRIM(SUBSTR(rw_craptab.dstextab,
																																							 4,
																																							 2)),
																																	 0);
										vr_tab_diadopagto(vr_nrindice).flgfolha := NVL(TRIM(SUBSTR(rw_craptab.dstextab,
																																							 14,
																																							 1)),
																																	 0);
										vr_tab_diadopagto(vr_nrindice).ddmesnov := NVL(TRIM(SUBSTR(rw_craptab.dstextab,
																																							 1,
																																							 2)),
																																	 0);
								END LOOP;
						...
						IF vr_tab_diadopagto(vr_nrindice)
						 .diapgtoh = 0 OR vr_tab_diadopagto(vr_nrindice).diapgtom = 0 OR vr_tab_diadopagto(vr_nrindice)
							 .ddmesnov = 0 THEN
								-- Gerar erro com critica
								pr_cdcritic := 0;
								vr_des_erro := 'Falta de dados no cadastro da empresa ' || to_char(vr_cdempres) || '.' ||
															 ' Conta: ' || to_char(pr_nrdconta) || '. ' || '(' || vr_tab_diadopagto(vr_nrindice)
															.diapgtoh || ',' || vr_tab_diadopagto(vr_nrindice).diapgtom || ',' || vr_tab_diadopagto(vr_nrindice)
															.ddmesnov || ')';
								RAISE vr_exc_erro;
						END IF;
						...
						CURSOR cr_craplem(pr_dtmvtolt IN craplem.dtmvtolt%TYPE) IS
								...
								WHERE lem.cdcooper = pr_cdcooper
										 ...
										 -- 443 MULTA S/ATRAS
											AND
											lem.cdhistor IN
											(88, 91, 92, 93, 94, 95, 120, 277, 349, 353, 392, 393, 507, 395, 441, 443)
				...
				BEGIN
						...
						ELSIF rw_crapepr.tpemprst = 2 THEN
								...
								EMPR0011.pc_busca_prest_pago_mes_pos(pr_cdcooper => pr_cdcooper,
																										 pr_nrdconta => pr_nrdconta,
																										 pr_nrctremp => pr_nrctremp,
																										 pr_dtmvtolt => TO_CHAR(pr_rw_crapdat.dtmvtolt,
																																						'DD/MM/RRRR'),
								...
								EMPR0011.pc_busca_pagto_parc_pos_prog(pr_cdcooper => pr_cdcooper,
																											pr_dtcalcul => TO_CHAR(pr_rw_crapdat.dtmvtolt,
																																						 'DD/MM/RRRR'),
																											pr_nrdconta => pr_nrdconta,
																											pr_nrctremp => pr_nrctremp,
        ...
        ............................................................................. */
				DECLARE
						...
						CURSOR cr_gnmodal(pr_cdmodali IN gnmodal.cdmodali%TYPE) IS
								SELECT gnmodal.cdmodali, gnmodal.dsmodali
								FROM gnmodal
						...
						CURSOR cr_gnsbmod(pr_cdmodali IN gnsbmod.cdmodali%TYPE
								...
								SELECT gnsbmod.cdsubmod, gnsbmod.dssubmod
								FROM gnsbmod
            ...
            ,pr_tpregist => 01);*/
						...
						FOR rw_crapepr IN cr_crapepr LOOP
								BEGIN
										...
										IF NOT (UPPER(pr_nmdatela) IN ('EXTEMP', 'IMPRES') OR RW_crapepr.inliquid = 0 OR
												rw_crapepr.inprejuz = 1 OR
												(pr_flgcondc AND rw_crapepr.inliquid = 1 AND rw_crapepr.inprejuz = 0 AND
												rw_crapepr.dtultpag + rw_craplcr.nrdialiq >= pr_rw_crapdat.dtmvtolt)) THEN
										...
										IF pr_inusatab AND rw_crapepr.inliquid = 0 THEN
										...
										IF rw_crapepr.inliquid = 0 OR rw_crapepr.tpemprst IN (1, 2) THEN
												--tratamento b1wgen0002a.i
										...
										IF rw_crapepr.tpemprst = 0 AND rw_crapepr.inliquid = 0 THEN
										...
										IF trunc(rw_crapepr.dtmvtolt, 'mm') = trunc(pr_rw_crapdat.dtmvtolt, 'mm') THEN
										...
										FOR vr_ind IN 1 .. 2 LOOP
												...
												IF vr_nrctaavg > 0 THEN
														...
														IF cr_crapass%FOUND THEN
																-- Adicionar o nome
																vr_tab_avalist(vr_ind).nmdavali := ': ' || SUBSTR(rw_crapass.nmprimtl,
																																									1,
																																									14) || '...';
														ELSE
										...
										IF rw_crapepr.nrctaav1 = 0 OR rw_crapepr.nrctaav2 = 0 THEN
										...
										IF vr_ind_avalist > 0 THEN
												...
												FOR vr_ind_avalist IN vr_tab_avalist.FIRST .. vr_tab_avalist.LAST LOOP
														...
														IF vr_ind_avalist = 1 THEN
																...
																vr_dsdaval1 := vr_tab_avalist(vr_ind_avalist)
																							 .nrgeneri || vr_tab_avalist(vr_ind_avalist).nmdavali;
																-- Preencher o campo avalistas por extenso
																vr_dsdavali := 'Aval ' ||
																							 TRIM(vr_tab_avalist(vr_ind_avalist).nrgeneri);
														ELSIF vr_ind_avalist = 2 THEN
																-- Utilizar o campo vr_dsdaval2
																vr_dsdaval2 := vr_tab_avalist(vr_ind_avalist)
																							 .nrgeneri || vr_tab_avalist(vr_ind_avalist).nmdavali;
																-- Preencher o campo avalistas por extenso
																vr_dsdavali := vr_dsdavali || '/Aval ' || vr_tab_avalist(vr_ind_avalist)
																							.nrgeneri;
														END IF;
										...
										IF pr_idorigem = 5 AND pr_nriniseq <> 0 AND pr_nrregist <> 0 AND
											 pr_nrctremp = 0 THEN
										
												pr_qtregist := nvl(pr_qtregist, 0) + 1;
										
												/* controles da paginação */
												IF (vr_nrregist < 1) OR (pr_qtregist < pr_nriniseq) OR
										...
										pr_tab_dados_epr(vr_indadepr).nrdiacar := rw_crawepr.dtdpagto -
																															rw_crapepr.dtmvtolt;
										...
										IF rw_crawepr.indachou = 'S' THEN
												...
												pr_tab_dados_epr(vr_indadepr).txanual := TRUNC((POWER(1 +
																																							(rw_crawepr.txdiaria / 100),
																																							360) - 1) * 100,
																																			 5);
										END IF;
										...
										IF pr_tab_dados_epr(vr_indadepr).inprejuz > 0 THEN
												...
												IF pr_tab_dados_epr(vr_indadepr)
												 .vlttmupr - pr_tab_dados_epr(vr_indadepr).vlpgmupr <= 0 AND pr_tab_dados_epr(vr_indadepr)
                        ...
                        diminuir do valor total pago da multa e juros de mora      */
										...
										IF rw_crapepr.tpemprst = 1 OR rw_crapepr.tpemprst = 2 THEN
										...
										ELSE
												...
												IF rw_crapepr.flgpagto = 0 THEN
														...
														ELSE
																...
																IF rw_crapepr.dtdpagto > pr_rw_crapdat.dtmvtolt OR
																	 (rw_crapepr.dtdpagto < pr_rw_crapdat.dtmvtolt AND
										...
										pr_tab_dados_epr(vr_indadepr).dspreapg := '   ' ||
																															to_char(vr_qtprecal, '990d0000') || '/' ||
																															to_char(rw_crapepr.qtpreemp, 'fm000') ||
																															' ->' ||
																															to_char(vr_qtpreapg, '990d0000');
										...
										ELSE
												...
												IF pr_tab_dados_epr(vr_indadepr).qtmesdec > rw_crapepr.qtpreemp OR pr_tab_dados_epr(vr_indadepr)
													 .vlpreapg > vr_vlsdeved THEN
										...
										pr_tab_dados_epr(vr_indadepr).vltotpag := nvl(pr_tab_dados_epr(vr_indadepr)
																																	.vlpreapg,
																																	0) + nvl(pr_tab_dados_epr(vr_indadepr)
																																					 .vlmrapar,
																																					 0) +
		...
		BEGIN
				...
				WHILE vr_index IS NOT NULL LOOP
						pc_escreve_xml('<inf>' || '<nrdconta>' || vr_tab_dados_epr(vr_index).nrdconta ||
													 '</nrdconta>' || '<cdagenci>' || vr_tab_dados_epr(vr_index).cdagenci ||
													 '</cdagenci>' || '<nmprimtl>' || vr_tab_dados_epr(vr_index).nmprimtl ||
													 '</nmprimtl>' || '<nrctremp>' || vr_tab_dados_epr(vr_index).nrctremp ||
													 '</nrctremp>' || '<vlemprst>' || vr_tab_dados_epr(vr_index).vlemprst ||
													 '</vlemprst>' || '<vlsdeved>' || vr_tab_dados_epr(vr_index).vlsdeved ||
													 '</vlsdeved>' || '<vlpreemp>' || vr_tab_dados_epr(vr_index).vlpreemp ||
													 '</vlpreemp>' || '<vlprepag>' ||
													 nvl(vr_tab_dados_epr(vr_index).vlprepag, 0) || '</vlprepag>' ||
													 '<txjuremp>' ||
													 to_char(vr_tab_dados_epr(vr_index).txjuremp, '990D00000000') ||
													 '</txjuremp>' || '<vljurmes>' || vr_tab_dados_epr(vr_index).vljurmes ||
													 '</vljurmes>' || '<vljuracu>' || vr_tab_dados_epr(vr_index).vljuracu ||
													 '</vljuracu>' || '<vlprejuz>' || vr_tab_dados_epr(vr_index).vlprejuz ||
													 '</vlprejuz>' || '<vlsdprej>' || vr_tab_dados_epr(vr_index).vlsdprej ||
													 '</vlsdprej>' || '<dtprejuz>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtprejuz, 'DD/MM/RRRR') ||
													 '</dtprejuz>' || '<vljrmprj>' || vr_tab_dados_epr(vr_index).vljrmprj ||
													 '</vljrmprj>' || '<vljraprj>' || vr_tab_dados_epr(vr_index).vljraprj ||
													 '</vljraprj>' || '<inprejuz>' || vr_tab_dados_epr(vr_index).inprejuz ||
													 '</inprejuz>' || '<vlprovis>' || vr_tab_dados_epr(vr_index).vlprovis ||
													 '</vlprovis>' || '<flgpagto>' ||
													 (CASE vr_tab_dados_epr(vr_index).flgpagto WHEN 1 THEN 'yes' ELSE 'no' END) ||
													 '</flgpagto>' || '<dtdpagto>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtdpagto, 'DD/MM/RRRR') ||
													 '</dtdpagto>' || '<cdpesqui>' || vr_tab_dados_epr(vr_index).cdpesqui ||
													 '</cdpesqui>' || '<dspreapg>' || vr_tab_dados_epr(vr_index).dspreapg ||
													 '</dspreapg>' || '<cdlcremp>' || vr_tab_dados_epr(vr_index).cdlcremp ||
													 '</cdlcremp>' || '<dslcremp>' || vr_tab_dados_epr(vr_index).dslcremp ||
													 '</dslcremp>' || '<cdfinemp>' || vr_tab_dados_epr(vr_index).cdfinemp ||
													 '</cdfinemp>' || '<dsfinemp>' || vr_tab_dados_epr(vr_index).dsfinemp ||
													 '</dsfinemp>' || '<dsdaval1>' || vr_tab_dados_epr(vr_index).dsdaval1 ||
													 '</dsdaval1>' || '<dsdaval2>' || vr_tab_dados_epr(vr_index).dsdaval2 ||
													 '</dsdaval2>' || '<vlpreapg>' ||
													 nvl(vr_tab_dados_epr(vr_index).vlpreapg, 0) || '</vlpreapg>' ||
													 ...
													 '<dtmvtolt>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtmvtolt, 'DD/MM/RRRR') ||
													 '</dtmvtolt>' || '<qtpreemp>' || vr_tab_dados_epr(vr_index).qtpreemp ||
													 '</qtpreemp>' || '<dtultpag>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtultpag, 'DD/MM/RRRR') ||
													 '</dtultpag>' || '<vlrabono>' || vr_tab_dados_epr(vr_index).vlrabono ||
													 '</vlrabono>' || '<qtaditiv>' || vr_tab_dados_epr(vr_index).qtaditiv ||
													 '</qtaditiv>' || '<dsdpagto>' || vr_tab_dados_epr(vr_index).dsdpagto ||
													 '</dsdpagto>' || '<dsdavali>' || vr_tab_dados_epr(vr_index).dsdavali ||
													 '</dsdavali>' || '<qtmesatr>' || vr_tab_dados_epr(vr_index).qtmesatr ||
													 '</qtmesatr>' || '<qtpromis>' || vr_tab_dados_epr(vr_index).qtpromis ||
													 '</qtpromis>' || '<flgimppr>' ||
													 ...
													 '</perjurmo>' || '<dtpripgt>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtpripgt, 'DD/MM/RRRR') ||
													 '</dtpripgt>' || '<inliquid>' || vr_tab_dados_epr(vr_index).inliquid ||
													 '</inliquid>' || '<txmensal>' || vr_tab_dados_epr(vr_index).txmensal ||
													 '</txmensal>' || '<flgatras>' ||
													 ...
													 '</tpdocged>' || '<vlpapgat>' || vr_tab_dados_epr(vr_index).vlpapgat ||
													 '</vlpapgat>' || '<vlsdevat>' || vr_tab_dados_epr(vr_index).vlsdevat ||
													 ...
													 '</qtlemcal>' || '<vlppagat>' || vr_tab_dados_epr(vr_index).vlppagat ||
													 '</vlppagat>' || '<vlmrapar>' || vr_tab_dados_epr(vr_index).vlmrapar ||
													 '</vlmrapar>' || '<vlmtapar>' || vr_tab_dados_epr(vr_index).vlmtapar ||
													 '</vlmtapar>' || '<vltotpag>' || vr_tab_dados_epr(vr_index).vltotpag ||
													 '</vltotpag>' || '<vlprvenc>' || vr_tab_dados_epr(vr_index).vlprvenc ||
													 '</vlprvenc>' || '<vlpraven>' || vr_tab_dados_epr(vr_index).vlpraven ||
													 '</vlpraven>' || '<flgpreap>' ||
													 (CASE WHEN vr_tab_dados_epr(vr_index).flgpreap THEN 1 ELSE 0 END) ||
													 '</flgpreap>' || '<cdorigem>' || vr_tab_dados_epr(vr_index).cdorigem ||
													 '</cdorigem>' || '<vlttmupr>' || vr_tab_dados_epr(vr_index).vlttmupr ||
													 '</vlttmupr>' || '<vlttjmpr>' || vr_tab_dados_epr(vr_index).vlttjmpr ||
													 '</vlttjmpr>' || '<vlpgmupr>' || vr_tab_dados_epr(vr_index).vlpgmupr ||
													 '</vlpgmupr>' || '<vlpgjmpr>' || vr_tab_dados_epr(vr_index).vlpgjmpr ||
													 '</vlpgjmpr>' || '<liquidia>' || vr_tab_dados_epr(vr_index).liquidia ||
													 '</liquidia>' || '<qtimpctr>' || vr_tab_dados_epr(vr_index).qtimpctr ||
													 '</qtimpctr>' || '<portabil>' || vr_tab_dados_epr(vr_index).portabil ||
													 '</portabil>' || '<tpatuidx>' || vr_tab_dados_epr(vr_index).tpatuidx ||
													 '</tpatuidx>' || '<idcarenc>' || vr_tab_dados_epr(vr_index).idcarenc ||
													 '</idcarenc>' || '<dtcarenc>' ||
													 to_char(vr_tab_dados_epr(vr_index).dtcarenc, 'DD/MM/RRRR') ||
													 '</dtcarenc>' || '<nrdiacar>' || vr_tab_dados_epr(vr_index).nrdiacar ||
													 '</nrdiacar>' || '<vliofcpl>' || vr_tab_dados_epr(vr_index).vliofcpl ||
													 '</vliofcpl>' || '</inf>');
        ...
        ............................................................................. */
				...
				BEGIN
						...
						-- Se houve erro na rotina
						IF vr_des_erro IS NOT NULL OR pr_cdcritic IS NOT NULL THEN
								-- Levantar exce¿¿o
								RAISE vr_exc_erro;
						END IF;
						-- Busca dos detalhes do empréstimo
						...
						ELSIF rw_crapepr.tpemprst = 2 THEN
								...
								EMPR0011.pc_busca_pagto_parc_pos_prog(pr_cdcooper => pr_cdcooper,
																											pr_dtcalcul => pr_rw_crapdat.dtmvtolt,
																											pr_nrdconta => rw_crapepr.nrdconta,
																											pr_nrctremp => rw_crapepr.nrctremp,
        ...
        ............................................................................. */
				...
				BEGIN
						...
						IF rw_crapepr.tpemprst = 0 THEN
								...
								IF rw_crapepr.dtdpagto IS NOT NULL THEN
										...
										IF to_number(to_char(rw_crapepr.dtdpagto, 'DD')) >
											 to_number(to_char(pr_rw_crapdat.dtmvtolt, 'DD')) AND
											 to_char(rw_crapepr.dtdpagto, 'YYYYMM') =
								...
								IF vr_qtdias <= 0 AND rw_crapepr.dtdpagto IS NOT NULL AND rw_crapepr.flgpagto = 0 AND /* Conta Corrente */
						...
						IF upper(pr_cdprogra) = 'CRPS398' AND vr_vlsdeved <= 0 THEN
								--Levantar Excecao
								RAISE vr_exc_saida;
						END IF;
						...
						IF rw_crapepr.tpemprst = 0 THEN
								...
								IF vr_qtdias <= 0 AND rw_crapepr.flgpagto = 0 THEN
										/* Conta Corrente */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								pr_cdcritic := nvl(pr_cdcritic, 0);
								pr_des_erro := 'Erro na rotina EMPR0001.pc_calc_dias_atraso. ' || SQLERRM;
				END;
		END;
        ...
        ............................................................................. */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								--Variavel de erro recebe erro ocorrido
								pr_cdcritic := 0;
								pr_dscritic := 'Erro na rotina EMPR0001.pc_inclui_altera_lote. ' || SQLERRM;
				END;
        ...
        ............................................................................. */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_cria_lancamento_cc ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_cdbccxlt,
        ...
        ............................................................................. */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								--Variavel de erro recebe erro ocorrido
								pr_cdcritic := 0;
								pr_dscritic := 'Erro na rotina EMPR0001.pc_cria_lancamento_lem. ' || SQLERRM;
				END;
        ...
        ............................................................................. */
				...
				BEGIN
						...
						BEGIN
								...
								vr_index_crawepr := lpad(pr_cdcooper, 10, '0') || lpad(pr_nrdconta, 10, '0') ||
								...
								OPEN cr_craplcr(pr_cdcooper => pr_cdcooper, pr_cdlcremp => rw_crabepr.cdlcremp);
								FETCH cr_craplcr
										INTO rw_craplcr;
								--Se nao encontrou
								IF cr_craplcr%NOTFOUND THEN
								...
								IF rw_crabepr.diarefju <> 0 AND rw_crabepr.mesrefju <> 0 AND
									 rw_crabepr.anorefju <> 0 THEN
										--Setar Dia/mes?ano
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_lanca_juro_contrato. ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_nrdcaixa,
        ...
        ............................................................................. */
				...
				BEGIN
						...
						IF NOT vr_flgtrans AND nvl(vr_cdcritic, 0) <> 0 THEN
								-- Retorno não OK
								pr_des_reto := 'NOK';
        ...
        ............................................................................. */
				...
				DECLARE
						...
						CURSOR cr_crapope(pr_crapope  IN crapope.cdcooper%TYPE
								...
								SELECT crapope.cdoperad, crapope.vlpagchq
								FROM crapope
						...
						CURSOR cr_crapass(pr_cdcooper IN crapass.cdcooper%TYPE
								...
								SELECT ass.inpessoa, ass.vllimcre
								FROM crapass ass
								WHERE ass.cdcooper = pr_cdcooper
											AND ass.nrdconta = pr_nrdconta;
						rw_crapass cr_crapass%ROWTYPE;
				...
				BEGIN
						...
						BEGIN
								...
								--Selecionar Operadores
								OPEN cr_crapope(pr_crapope => pr_cdcooper, pr_cdoperad => pr_cdoperad);
								FETCH cr_crapope
										INTO rw_crapope;
        ...
        ............................................................................. */
						...
						BEGIN
								...
								IF nvl(rw_crapepr.vlsdeved, 0) <> 0 THEN
										...
										IF cr_craplem_estorno%NOTFOUND THEN
												BEGIN
														UPDATE crapepr
														SET crapepr.vlajsdev = rw_crapepr.vlsdeved, crapepr.vlsdeved = 0
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_grava_liquidacao_empr ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_nrdcaixa,
				...
				BEGIN
						...
						BEGIN
								...
								IF nvl(vr_vlmtapar, 0) > 0 THEN
										...
										BEGIN
												UPDATE crappep
												SET crappep.vlpagmta = nvl(crappep.vlpagmta, 0) + nvl(vr_vlmtapar, 0)
								...
								IF nvl(vr_vlmrapar, 0) > 0 AND nvl(vr_vlpagpar, 0) >= 0 THEN
										IF vr_floperac THEN
												/* Financiamento */
												vr_nrdolote := 600023;
										ELSE
										...
										BEGIN
												UPDATE crappep
												SET crappep.vlpagmra = nvl(crappep.vlpagmra, 0) + nvl(vr_vlmrapar, 0)
												WHERE crappep.rowid = rw_crappep.rowid;
										EXCEPTION
												WHEN OTHERS THEN
														vr_cdcritic := 0;
								...
								/* Juros normais */
								IF nvl(vr_vljinpar, 0) > 0 AND nvl(vr_vlpagpar, 0) > 0 THEN
										IF vr_floperac THEN
												/* Financiamento */
												vr_nrdolote := 600027;
												vr_cdhistor := 1051;
										...
										BEGIN
												UPDATE crappep
												SET crappep.vlpagjin = nvl(crappep.vlpagjin, 0) + nvl(vr_vljinpar, 0)
												WHERE crappep.rowid = rw_crappep.rowid;
										EXCEPTION
												WHEN OTHERS THEN
														vr_cdcritic := 0;
								...
								IF nvl(vr_vlpagpar, 0) > 0 THEN
										...
										IF apli0001.fn_round(vr_vlatupar, 2) = apli0001.fn_round(vr_vlpagpar, 2) THEN
												vr_inliquid := 1;
												--Zerar saldo devedor
										...
										--Valor Multa + Juros
										vr_vlmuljur := nvl(vr_vlmtapar, 0) + nvl(vr_vljinpar, 0) + nvl(vr_vlmrapar, 0);
								...
								BEGIN
										...
										SET crapepr.diarefju = rw_crapepr.diarefju,
												...
												crapepr.vlsdeved = apli0001.fn_round(crapepr.vlsdeved, 10) -
																					 apli0001.fn_round(nvl(vr_vlpagpar, 0), 10) +
																					 apli0001.fn_round(nvl(vr_vljurmes, 0), 10),
        ...
        ............................................................................. */
				...
				BEGIN
						...
						BEGIN
								...
								/* Pagamento de juros de mora */
								IF nvl(vr_vlatraso, 0) > 0 AND nvl(vr_vlpagsld, 0) >= 0 THEN
				...
				BEGIN
						...
						BEGIN
								...
								IF nvl(pr_nrseqava, 0) = 0 THEN
										...
										vr_pag_cdhistor := CASE vr_floperac
																					 WHEN TRUE THEN
																						1039
																					 ELSE
																						1044
								...
								vr_vlsdvpar := nvl(rw_crappep.vlsdvpar, 0) -
															 (nvl(pr_vlpagpar, 0) + nvl(vr_vldespar, 0));
						
								/* Verificar se liquidou a parcela */
								vr_inliquid := CASE nvl(vr_vlsdvpar, 0)
																	 WHEN 0 THEN
																		1
																	 ELSE
																		0
								...
								BEGIN
										UPDATE crappep
										SET crappep.vldespar = nvl(crappep.vldespar, 0) + nvl(vr_vldespar, 0),
        ...
        ............................................................................. */
				...
				BEGIN
						...
						--Se nao ocorreu a transacao
						IF vr_flgtrans AND pr_flgerlog = 'S' THEN
								-- Se foi solicitado o envio de LOG
								gene0001.pc_gera_log(pr_cdcooper => pr_cdcooper,
																		 pr_cdoperad => pr_cdoperad,
																		 pr_dscritic => '',
        ...
        ............................................................................. */
				...
				BEGIN
						...
						BEGIN
								...
								rw_crappep.vlpagpar := nvl(rw_crappep.vlpagpar, 0) + nvl(pr_vlparepr, 0);
								rw_crappep.vlsdvpar := nvl(rw_crappep.vlsdvpar, 0) - nvl(pr_vlparepr, 0);
								rw_crappep.vlsdvsji := nvl(rw_crappep.vlsdvsji, 0) - nvl(pr_vlparepr, 0);
								...
								IF rw_crappep.dtvencto > vr_dtcalcul THEN
										...
										rw_crapepr.vlsdeved := nvl(rw_crapepr.vlsdeved, 0) + nvl(vr_vljurmes, 0);
								ELSE
										--Diminuir valor parcela e somar juros
										rw_crapepr.vlsdeved := nvl(rw_crapepr.vlsdeved, 0) - nvl(pr_vlparepr, 0) +
																					 nvl(vr_vljurmes, 0);
								END IF;
								--Valor Juros Atual
								rw_crapepr.vljuratu := nvl(rw_crapepr.vljuratu, 0) + nvl(vr_vljurmes, 0);
								--Valor Juros Acumulados
								rw_crapepr.vljuracu := nvl(rw_crapepr.vljuracu, 0) + nvl(vr_vljurmes, 0);
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_efetiva_pagto_parc_lem ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_nrdcaixa,
        ...
        ............................................................................. */
				...
				EXCEPTION
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_efetiva_pagto_parcela ' || SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
																			pr_nrdcaixa => pr_nrdcaixa,
        ...
        ..............................................................................*/
				DECLARE
						...
						CURSOR cr_craplem(pr_cdcooper IN craplem.cdcooper%TYPE
														 ,pr_nrdconta IN craplem.nrdconta%TYPE
														 ,pr_nrctremp IN craplem.nrctremp%TYPE) IS
								SELECT lem.nrparepr, lem.dtpagemp, lem.vllanmto, pep.dtvencto
								FROM craplem lem, crappep pep
								WHERE lem.cdcooper = pr_cdcooper
											AND lem.nrdconta = pr_nrdconta
											AND lem.nrctremp = pr_nrctremp
											AND lem.cdhistor IN (1044, -- PG. EMPR. C/C
																					 1039, -- PG.FINANC.C/C
																					 1057, -- PG.AVAL.FINA
																					 1045) -- PG.AVAL.EMPR.
								...
								ORDER BY lem.dtpagemp DESC, pep.dtvencto, lem.nrctremp;
				...
				BEGIN
						...
						FOR rw_craplem IN cr_craplem(pr_cdcooper => vr_cdcooper,
								...
								gene0007.pc_insere_tag(pr_xml      => pr_retxml,
																			 ...
																			 pr_tag_cont => to_char(rw_craplem.dtvencto, 'DD/MM/RRRR'),
																			 pr_des_erro => vr_dscritic);
								gene0007.pc_insere_tag(pr_xml      => pr_retxml,
																			 pr_tag_pai  => 'parcela',
																			 pr_posicao  => vr_contador,
																			 pr_tag_nova => 'dtpagemp',
																			 pr_tag_cont => to_char(rw_craplem.dtpagemp, 'DD/MM/RRRR'),
																			 pr_des_erro => vr_dscritic);
        ...
        ..............................................................................*/
				DECLARE
						...
						CURSOR cr_crapass(pr_cdcooper IN crapass.cdcooper%TYPE
														 ,pr_nrdconta IN crapass.nrdconta%TYPE) IS
								SELECT ass.nrdconta, SUBSTR(ass.nmprimtl, 1, 40) nmprimtl, ass.cdagenci
								FROM crapass ass
								WHERE ass.cdcooper = pr_cdcooper
											AND ass.nrdconta = pr_nrdconta;
						rw_crapass cr_crapass%ROWTYPE;
				...
				BEGIN
						...
						vr_strbuffer := vr_strbuffer || '<associ>' || '<nmrescop>' || rw_crapcop.nmrescop ||
														'</nmrescop>' || '<nrdconta>' ||
														TRIM(gene0002.fn_mask_conta(rw_crapass.nrdconta)) || '</nrdconta>' ||
														'<cdagenci>' || to_char(rw_crapass.cdagenci) || '</cdagenci>' ||
														'<nmprimtl>' || rw_crapass.nmprimtl || '</nmprimtl>' || '<nrctremp>' ||
														TRIM(gene0002.fn_mask_contrato(to_char(pr_nrctremp))) || '</nrctremp>' ||
														'<nmoperad>' || rw_crapope.nmoperad || '</nmoperad>' || '<dtmvtolt>' ||
														to_char(SYSDATE, 'DD/MM/RRRR') || '</dtmvtolt>' || '</associ>';
						...
						LOOP
								...
								vr_nrparepr := GENE0002.fn_busca_entrada(vr_auxqtd, pr_lstparepr, ';');
								vr_dtvencto := GENE0002.fn_busca_entrada(vr_auxqtd, pr_lstdtvcto, ';');
								vr_dtpagemp := GENE0002.fn_busca_entrada(vr_auxqtd, pr_lstdtpgto, ';');
								...
								-- Inicializar tag empresa
								vr_strbuffer := vr_strbuffer || '<parcela>' || '<nrparepr>' || vr_nrparepr ||
																'</nrparepr>' || '<dtvencto>' || vr_dtvencto || '</dtvencto>' ||
																'<dtpagemp>' || vr_dtpagemp || '</dtpagemp>' || '<vllanmto>' ||
						...
						IF dbms_lob.getlength(vr_xml) > 0 THEN
								...
								vr_nmarqimp := 'crrl684_' || pr_nrdconta || '_' || pr_nrctremp || '.pdf';
		...
		PROCEDURE pc_valida_inclusao_tr(pr_cdcooper IN craplcr.cdcooper%TYPE --> Código da cooperativa
				...
				CURSOR cr_craplcr(pr_cdcooper IN craplcr.cdcooper%TYPE
												 ,pr_cdlcremp IN craplcr.cdlcremp%TYPE) IS
						SELECT cdlcremp, dslcremp, cdusolcr, tpdescto
						FROM craplcr
						WHERE cdcooper = pr_cdcooper
									AND cdlcremp = pr_cdlcremp;
				rw_craplcr cr_craplcr%ROWTYPE;
		...
		BEGIN
				...
				-- Buscar informações da linha de crédito
				OPEN cr_crapfin(pr_cdcooper => pr_cdcooper, pr_cdfinemp => pr_cdfinemp);
				FETCH cr_crapfin
						INTO rw_crapfin;
				-- Se não encontrar
				IF cr_crapfin%NOTFOUND THEN
				...
				IF pr_dtmvtolt >= TO_DATE('02/26/2015') THEN
						...
						IF ((rw_craplcr.cdusolcr = 1) AND
							 (INSTR(',' || vr_dslcremp || ',', ',' || rw_craplcr.cdlcremp || ',') > 0)) THEN
								RAISE vr_exc_saida;
						END IF;
        ...
        ............................................................................. */
				...
				BEGIN
						...
						vr_index := lpad(pr_cdcooper, 10, '0') || lpad(pr_nrctremp, 10, '0') ||
												lpad(pr_cdhistor, 10, '0') || to_char(pr_dtmvtolt, 'YYYYMMDD') ||
												lpad(pr_cdpactra, 10, '0') || lpad('100', 10, '0') ||
												lpad(pr_nrdolote, 10, '0') || lpad(pr_nrdconta, 10, '0');
						--Se nao encontrou
						IF pr_tab_lancconta.EXISTS(vr_index) THEN
								--Atualizar Lancamento
								pr_tab_lancconta(vr_index).vllanmto := nvl(pr_tab_lancconta(vr_index).vllanmto, 0) +
																											 pr_vllanmto;
						ELSE
        ...
        ............................................................................. */
				...
				BEGIN
						...
						BEGIN
										...
										IF rw_crappep.dtvencto > pr_dtmvtoan AND rw_crappep.dtvencto <= pr_dtmvtolt THEN
												...
												EMPR0001.pc_efetiva_pagto_parc_lem(pr_cdcooper    => pr_cdcooper --> Cooperativa conectada
																													...
																													,
																													 pr_nrparepr    => rw_crappep.nrparepr --> Número parcelas empréstimo
																													,
																													 pr_vlparepr    => vr_tab_pgto(vr_index_char)
																																						 .vlatupar --> Valor da parcela emprestimo
																													,
																													 pr_tab_crawepr => pr_tab_crawepr --Tabela com Contas e Contratos
																													,
																													 pr_nrseqava    => pr_nrseqava --> Pagamento: Sequencia do avalista
												...
												IF nvl(pr_ehprcbat, 'X') = 'N' THEN
														...
														pc_cria_atualiza_ttlanconta(pr_cdcooper      => pr_cdcooper --> Cooperativa conectada
																											 ...
																											 ,
																												pr_nrdconta      => pr_nrdconta --> Número da conta
																											 ,
																												pr_vllanmto      => vr_tab_pgto(vr_index_char)
																																						.vlatupar --> Valor lancamento
																											 ,
																												pr_nrseqava      => pr_nrseqava --> Pagamento: Sequencia do avalista
																											 ,
										...
										ELSIF rw_crappep.dtvencto < pr_dtmvtolt THEN
												...
												EMPR0001.pc_efetiva_pag_atr_parcel_lem(pr_cdcooper    => pr_cdcooper --> Cooperativa conectada
																															...
																															,
																															 pr_nrparepr    => rw_crappep.nrparepr --> Número parcelas empréstimo
																															,
																															 pr_vlpagpar    => vr_tab_pgto(vr_index_char)
																																								 .vlatrpag --> Valor da parcela emprestimo
																															,
																															 pr_tab_crawepr => pr_tab_crawepr --Tabela com Contas e Contratos
																															,
																															 pr_nrseqava    => pr_nrseqava --Pagamento: Sequencia do avalista
								...
								WHILE vr_index_lanc IS NOT NULL LOOP
										...
										/* Lanca em C/C e atualiza o lote */
										EMPR0001.pc_cria_lancamento_cc(pr_cdcooper => vr_tab_lanc(vr_index_lanc)
																																	.cdcooper --> Cooperativa conectada
																									,
				...
				EXCEPTION
						WHEN OTHERS THEN
								...
								pr_des_erro := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_efetua_liquidacao_empr. ' ||
															 SQLERRM;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
        ...
        ............................................................................. */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								pr_des_reto := 'NOK';
								-- Montar descrição de erro não tratado
								vr_dscritic := 'Erro não tratado na EMPR0001.pc_liquida_mesmo_dia ' || SQLERRM;
								ROLLBACK;
								-- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
								gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper,
																			pr_cdagenci => pr_cdagenci,
        ...
        ............................................................................. */
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								...
								-- Montar descrição de erro não tratado
								pr_dscritic := 'Erro não tratado na EMPR0001.pc_liquida_mesmo_dia ' || SQLERRM; /*
                                                                                                        -- Gerar rotina de gravação de erro avisando sobre o erro não tratavo
                                                                                                        gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper
                                                                                                                             ,pr_cdagenci => pr_cdagenci
                                                                                                                             ,pr_nrdcaixa => pr_nrdcaixa
		...
		EXCEPTION
				...
				WHEN OTHERS THEN
						pr_des_reto := 'NOK';
						pr_tab_erro(pr_tab_erro.FIRST).dscritic := 'Erro PR_EFETIVA_PAGTO_ANTEC_PARCELA: ' ||
																											 SQLERRM;
        ...
        ............................................................................. */
				DECLARE
						...
						CURSOR cr_crappep(pr_cdcooper IN crappep.cdcooper%TYPE
														 ...
														 ,pr_nrctremp IN crappep.nrctremp%TYPE
														 ,pr_nrparepr IN crappep.nrparepr%TYPE) IS
								SELECT pep.nrparepr, pep.dtvencto, pep.inliquid, pep.nrctremp
								FROM crappep pep
								WHERE pep.cdcooper = pr_cdcooper
											AND pep.nrdconta = pr_nrdconta
											AND pep.nrctremp = pr_nrctremp
						...
						CURSOR cr_cde(pr_cdcooper IN crapcob.cdcooper%TYPE
								...
								SELECT cob.nrdocmto, cob.dtvencto, cob.vltitulo
								FROM crapcob cob
				...
				BEGIN
						...
						IF vr_blqresg_cc = 'S' AND vr_flgativo = 0 THEN
								...
								IF nvl(rw_cde.nrdocmto, 0) > 0 THEN
										...
										vr_dscritic := 'Boleto do contrato ' || to_char(pr_nrctremp) || ' em aberto.' ||
																	 ' Vencto ' || to_char(rw_cde.dtvencto, 'DD/MM/RRRR') || ' R$ ' ||
																	 to_char(rw_cde.vltitulo,
																					 'fm999g999g990d00',
																					 'NLS_NUMERIC_CHARACTERS = '',.''') || '.';
										-- Gera exceção
										RAISE vr_exc_erro;
								...
								ELSE
										...
										IF nvl(rw_ret.nrdocmto, 0) > 0 THEN
												...
												vr_dscritic := 'Boleto do contrato ' || to_char(pr_nrctremp) ||
																			 ' esta pago pendente de processamento.' || ' Vencto ' ||
																			 to_char(rw_ret.dtvencto, 'DD/MM/RRRR') || ' R$ ' ||
																			 to_char(rw_ret.vltitulo,
																							 'fm999g999g990d00',
																							 'NLS_NUMERIC_CHARACTERS = '',.''') || '.';
												-- Gera exceção
												RAISE vr_exc_erro;
						...
						IF pr_totatual = pr_totpagto THEN
								-- Liquida Emprestimo
						...
						ELSE
								...
								OPEN cr_crappep_menor(pr_cdcooper => pr_cdcooper,
																			pr_nrdconta => pr_nrdconta,
																			pr_nrctremp => pr_nrctremp);
								FETCH cr_crappep_menor
										INTO rw_crappep_menor;
								...
								FOR idx IN vr_tab_pgto_parcel.FIRST .. vr_tab_pgto_parcel.LAST LOOP
										...
										ELSIF rw_crappep.dtvencto < pr_dtmvtolt THEN
												...
												pc_efetiva_pagto_atr_parcel(pr_cdcooper    => pr_cdcooper,
																										...
																										pr_nrctremp    => rw_crappep.nrctremp,
																										pr_nrparepr    => rw_crappep.nrparepr,
																										pr_vlpagpar    => vr_tab_pgto_parcel(idx)
																																			.vlpagpar,
																										pr_tab_crawepr => vr_tab_crawepr,
																										pr_nrseqava    => pr_nrseqava,
																										pr_des_reto    => vr_des_reto,
										...
										ELSIF rw_crappep.dtvencto > pr_dtmvtolt THEN
												...
												pr_efetiva_pagto_antec_parcela(pr_cdcooper    => pr_cdcooper --> Cooperativa conectada
																											...
																											,
																											 pr_vlpagpar    => vr_tab_pgto_parcel(idx)
																																				 .vlpagpar --> Valor da parcela emprestimo
																											,
        ...
        ............................................................................. */
				...
				BEGIN
						...
						IF cr_crappep_2%FOUND AND cr_crappep_3%FOUND THEN
				...
				EXCEPTION
						WHEN vr_exc_saida THEN
								...
								IF cr_crappep_2%ISOPEN THEN
										CLOSE cr_crappep_2;
								END IF;
								IF cr_crappep_3%ISOPEN THEN
										CLOSE cr_crappep_3;
								END IF;
						...
						WHEN OTHERS THEN
								...
								pr_dscritic := 'Erro nao tratado na procedure EMPR0001.pc_verifica_parcelas_antecipa -> ' ||
															 SQLERRM;
								pr_des_reto := 'NOK';
    ...
    para chamadas diretamente atraves de rotinas progress */
		PROCEDURE pc_valida_pagto_geral_prog(pr_cdcooper IN crapcop.cdcooper%TYPE --Codigo Cooperativa
																				...
																				,pr_dscritic OUT VARCHAR --Descricao da Critica
																				,pr_inconfir OUT INTEGER --Codigo da Confirmacao
																				,pr_dsmensag OUT VARCHAR) IS
				--Descricao da Confirmacao
		...
		BEGIN
				...
				IF (pr_vlemprst * vr_tipsplit(1)) <= pr_vlmerbem THEN
						pr_dsmensag := 'Atencao! Valor do bem superior ou igual a ' || vr_tipsplit(1) ||
													 ' vezes o valor do emprestimo!';
				END IF;
				...
				IF (pr_vlemprst * vr_tipsplit(2)) <= pr_vlmerbem THEN
						pr_dsmensag := 'Atencao! Valor do bem superior ou igual a ' || vr_tipsplit(2) ||
													 ' vezes o valor do emprestimo!';
						pr_flgsenha := 1;
				END IF;
				...
				IF (pr_vlmerbem * vr_tipsplit(1)) <= pr_vlemprst THEN
						pr_dsmensag := 'Atencao! Valor do bem inferior ou igual a ' || vr_tipsplit(1) ||
													 ' vezes o valor do emprestimo!';
				END IF;
				...
				IF (pr_vlmerbem * vr_tipsplit(2)) <= pr_vlemprst THEN
						pr_dsmensag := 'Atencao! Valor do bem inferior ou igual a ' || vr_tipsplit(2) ||
													 ' vezes o valor do emprestimo!';
						pr_flgsenha := 1;
				END IF;
        ...
        ............................................................................. */
				...
				DECLARE
						CURSOR cr_crapbpr(pr_cdcooper IN crapbpr.cdcooper%TYPE
								...
								SELECT crapbpr.vlmerbem, crapbpr.dscatbem
								FROM crapbpr
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								-- Montar descrição de erro não tratado
								pr_dscritic := 'Erro não tratado na EMPR0001.pc_valida_alt_valor_prop --> ' ||
															 SQLERRM;
				END;
        ...
        ............................................................................. */
				...
				BEGIN
						...
						gene0007.pc_insere_tag(pr_xml      => pr_retxml,
																	 pr_tag_pai  => 'inf',
																	 pr_posicao  => 0,
																	 pr_tag_nova => 'flgsenha',
																	 pr_tag_cont => vr_flgsenha,
																	 pr_des_erro => vr_dscritic);
						gene0007.pc_insere_tag(pr_xml      => pr_retxml,
																	 pr_tag_pai  => 'inf',
																	 pr_posicao  => 0,
				...
				EXCEPTION
						...
						WHEN OTHERS THEN
								-- Montar descrição de erro não tratado
								pr_dscritic := 'Erro não tratado na EMPR0001.pc_valida_alt_valor_prop_web ' ||
															 SQLERRM;
				END;
        ...
        */
		...
		BEGIN
				...
				IF NOT gene0001.fn_exis_arquivo(pr_caminho => vr_nm_arquivo) THEN
						vr_des_erro := 'Erro rotina pc_gera_arq_saldo_devedor: Arquivo inexistente!' || SQLERRM;
						pr_cdcritic := 3;
						RAISE vr_exc_erro;
				END IF;
				...
				IF vr_des_erro IS NOT NULL THEN
						vr_des_erro := 'Rotina pc_gera_arq_saldo_devedor: Erro abertura arquivo saida!' ||
													 SQLERRM;
						pr_cdcritic := 5;
						RAISE vr_exc_erro;
				END IF;
				...
				IF vr_des_erro IS NOT NULL THEN
						vr_des_erro := 'Rotina pc_gera_arq_saldo_devedor: Erro abertura arquivo LOG!' ||
													 SQLERRM;
						pr_cdcritic := 6;
						RAISE vr_exc_erro;
				END IF;
		...
		EXCEPTION
				...
				WHEN OTHERS THEN
						raise_application_error(-20150,
																		'erro na rotina pc_gera_arq_saldo_devedor: ' || SQLERRM);
        ...
        ............................................................................. */
				...
				CURSOR cr_crapepr IS
						SELECT 1
						FROM crapbpr b, craplcr r, crawepr t
        ...
        ............................................................................. */
				...
				TYPE typ_reg_parcela IS RECORD(
		...
		BEGIN
				...
				IF pr_tpemprst = 1 THEN
						...
						FOR vr_ind IN 1 .. pr_qtpreemp LOOP
								IF vr_ind = 1 THEN
										vr_saldo_devedor := ROUND(pr_vlemprst *
																							(POWER((1 + vr_txdiaria),
																										 ((pr_dtdpagto - pr_dtmvtolt) - 30))),
																							2);
								ELSE
								...
								-- Valor Saldo Devedor
								vr_tab_parcela(vr_ind).vlsaldodevedor := vr_saldo_devedor - vr_tab_parcela(vr_ind)
																												.vlprincipal;
								-- Valor do IOF
								pr_valoriof := pr_valoriof +
															 ROUND(vr_qtdias * vr_tab_parcela(vr_ind).vlprincipal * vr_taxaiof, 2);
						END LOOP;
				
				ELSIF pr_tpemprst = 0 THEN
						-- Se o tipo do empréstimo for TR
				...
				ELSIF pr_tpemprst = 2 THEN
						-- Se o emprestimo for Pos-Fixado
