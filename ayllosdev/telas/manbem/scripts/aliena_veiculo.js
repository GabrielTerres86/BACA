...
function validaCamposAditiv() {
    ...
    if ($('#dsmarbem', '#frmTipo').val() == '-1' || dsmarbem == "") {
        ...
        if (!validaCampo('dsbemfinC', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
        if (!validaCampo('nrmodbemC', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
    } else {
        if (!validaCampo('dsmarbem', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
        if (!validaCampo('dsbemfin', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
        if (!validaCampo('nrmodbem', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
    }
    if (!validaCampo('dstipbem', '#frmTipo')) {
        invalidos = invalidos + 1;
    }
    if (!validaCampo('nranobem', '#frmTipo')) {
        invalidos = invalidos + 1;
    }
    if (!validaCampo('vlrdobem', '#frmTipo')) {
        invalidos = invalidos + 1;
    }

    if (!validaCampo('tpchassi', '#frmTipo')) {
        invalidos = invalidos + 1;
    }
    if (!validaCampo('dschassi', '#frmTipo')) {
        invalidos = invalidos + 1;
    }
    if (!validaCampo('dscorbem', '#frmTipo')) {
        invalidos = invalidos + 1;
    }

    if (!$('#ufdplaca').prop("disabled"))
    {
        if (!validaCampo('ufdplaca', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
        if (!validaCampo('nrdplaca', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
        if (!validaCampo('nrrenava', '#frmTipo')) {
            invalidos = invalidos + 1;
        }
    }
    if (!validaRadio($('input[name=nrbem]'))) {
        invalidos = invalidos + 1;
    }
