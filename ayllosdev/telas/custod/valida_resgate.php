<?php
	/*!
	 * FONTE        : valida_resgate.php
	 * CRIAÇÃO      : Lucas Reinert
	 * DATA CRIAÇÃO : 05/09/2016
	 * OBJETIVO     : Rotina para validar os cheques informados para resgate
	 * --------------
	 * ALTERAÇÕES   : 
	 * -------------- 
	 */		
	...
	$xmlResult = mensageria($xml, "TELA_CUSTOD", "CUSTOD_VALIDA_RESGATE", $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
	...
	if(isset($xmlObj->roottag->tags[1]->name) && strtoupper($xmlObj->roottag->tags[1]->name == 'VALIDAR_CMC7')){	
		...
		$msgErro = 'Não é possível efetuar o resgate pois existem cheques com críticas. <br> Remova-os e tente novamente.';
	...
	if ($inresgte == 1){
		...
		$xmlResult = mensageria($xml, "TELA_CUSTOD", "CUSTOD_EFETUA_RESGATE", $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
		$xmlObj = getObjectXML($xmlResult);		
		...
		if(isset($xmlObj->roottag->tags[1]->name) && strtoupper($xmlObj->roottag->tags[1]->name == 'VALIDAR_CMC7')){	
			...
			$msgErro = 'Não foi possível efetuar o resgate de ' || $contErro || ' cheque(s). <br> Erro na cobrança de tarifa. Verifique cheque(s) destacado(s)..';
