<?php
	/*!
	 * FONTE        : cadastrar_emitente_I.php
	 * CRIAÇÃO      : Lucas Reinert
	 * DATA CRIAÇÃO : 03/04/2016
	...
	if(isset($xmlObj->roottag->tags[0]->name) && strtoupper($xmlObj->roottag->tags[0]->name == 'VALIDAR_EMITEN')){	
		...
		exibirErro('error',$msgErro,'Alerta - Ayllos','bloqueiaFundo($(\'#divRotina\'));$(\'#nrcpfcgc\', \'#frmCadastraEmitente\').focus();',false);
		exit();
	} else {
		if(isset($xmlObj->roottag->tags[0]->name) && strtoupper($xmlObj->roottag->tags[0]->name == 'ERRO')){	
			...
			echo "adicionaChequeI();fechaRotina($('#divRotina'));";
		}
	}
?>
