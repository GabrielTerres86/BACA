...
function controlaOperacaoProcuradores( operacao ){
	...
	if ( (operacao == 'TA') && (flgAlterar   != '1') ) { showError('error','Seu usu&aacute;rio n&atilde;o possui permiss&atilde;o de altera&ctilde;&atilde;o.','Alerta - Aimaro','bloqueiaFundo(divRotina)'); return false; }	
	...
	if ( (operacao == 'TP') && (flgPoderes	 != '1') ) { showError('error','Seu usu&aacute;rio n&atilde;o possui permiss&atilde;o de poderes.'        		  ,'Alerta - Aimaro','bloqueiaFundo(divRotina)'); return false; }
	...
	switch (operacao) {
		...
		case 'AT': 
			showConfirmacao('Deseja cancelar altera&ccedil;&atilde;o?','Confirma&ccedil;&atilde;o - Aimaro','controlaOperacaoProcuradores()','bloqueiaFundo(divRotina)','sim.gif','nao.gif');
			return false;
			break;
		// Formulario em modo inclusão para tabela consulta		
		case 'IT': 
           showConfirmacao('Deseja cancelar inclus&atilde;o?','Confirma&ccedil;&atilde;o - Aimaro','controlaOperacaoProcuradores()','bloqueiaFundo(divRotina)','sim.gif','nao.gif');
			return false;
            break;	
		...
		if ( !campoConta.hasClass('campoTelaSemBorda') ){
			if ( nrdctato == 0 ) { 
				...
				if ( !validaNroConta(nrdctato) ) { 
					showError('error','Conta/dv inválida.','Alerta - Aimaro','bloqueiaFundo(divRotina,\'nrdctato\',\'frmDadosProcuradores\');'); return false; 
				}else{				
			...
			if ( !campoCpf.hasClass('campoTelaSemBorda') ){
				...
				if ( !validaCpfCnpj(cpf,1) ) { 
					showError('error','CPF inv&aacute;lido.','Alerta - Aimaro','bloqueiaFundo(divRotina,\'nrcpfcgc\',\'frmDadosProcuradores\');'); return false; 
				}else{
	...
	// FIELDSET IDENTIFICAÇÃO
		...
		cNrConta.unbind('keydown').bind('keydown', function(e){ 	
			...
			if ( e.keyCode == 13 ) {		
				...
				// Verifica se a conta é válida
				if ( !validaNroConta(nrdctato) ) { showError('error','Conta/dv inválida.','Alerta - Aimaro','bloqueiaFundo(divRotina,\'nrdctato\',\'frmDadosProcuradores\');'); return false; }
		...
		cCPF.unbind('keydown').bind('keydown', function(e){ 	
			...
			if ( e.keyCode == 13 ) {		
				...
				// Verifica se a conta é válida
				if ( !validaCpfCnpj(cpf ,1) ) { showError('error','CPF inv&aacute;lido.','Alerta - Aimaro','bloqueiaFundo(divRotina,\'nrcpfcgc\',\'frmDadosProcuradores\');'); return false; }
