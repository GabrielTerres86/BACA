...
<?	
	...
	if (($msgError = validaPermissao($glbvars['nmdatela'],$glbvars['nmrotina'],$cddopcao)) <> '') exibirErro('error',$msgError,'Alerta - Ayllos','fechaRotina(divRotina)',false);
	...
	if( in_array($operacao,array('AV','IV','EV')) ) {
		if( $operacao == 'AV' ) exibirConfirmacao('Deseja confirmar alteração?','Confirmação - Ayllos','controlaOperacaoDependentes(\'VA\')','bloqueiaFundo(divRotina)',false);		
		if( $operacao == 'IV' ) exibirConfirmacao('Deseja confirmar inclusão?' ,'Confirmação - Ayllos','controlaOperacaoDependentes(\'VI\')','bloqueiaFundo(divRotina)',false);		
		if( $operacao == 'EV' ) exibirConfirmacao('Deseja confirmar exclusão?' ,'Confirmação - Ayllos','controlaOperacaoDependentes(\'VE\')','bloqueiaFundo(divRotina);controlaOperacaoDependentes();',false);
	...
	// Se é Inclusão/Alteração/Exclusão
		...
		if($msgAtCad!='') {					
			if( $operacao == 'VA' ) exibirConfirmacao($msgAtCad,'Confirmação - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0071.p\',\''.$stringArrayMsg.'\')','exibirMensagens(\''.$stringArrayMsg.'\',\'controlaOperacaoDependentes(\"\")\')',false);
			if( $operacao == 'VI' ) exibirConfirmacao($msgAtCad,'Confirmação - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0071.p\',\''.$stringArrayMsg.'\')','exibirMensagens(\''.$stringArrayMsg.'\',\'controlaOperacaoDependentes(\"\")\')',false);
			if( $operacao == 'VE' ) exibirConfirmacao($msgAtCad,'Confirmação - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0071.p\',\''.$stringArrayMsg.'\')','exibirMensagens(\''.$stringArrayMsg.'\',\'controlaOperacaoDependentes(\"\")\')',false);
	...
	function validaDados() {					
		...
		// Valida o Nome
		if( $GLOBALS['nmdepend'] == '' ) exibirErro('error','O Nome do Dependente deve ser informado.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'nmdepend\',\'frmDependentes\')',false);
		
		// Valida a Data Nascimento
		if ( !validaData( $GLOBALS['dtnascto'] )) exibirErro('error','A Data de Nascimento não é uma data válida.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'dtnascto\',\'frmDependentes\')',false);
		
		// Valida o Tipo de Dependente
		if ( !validaInteiro( $GLOBALS['cdtipdep'] )) exibirErro('error','O Tipo de Dependente deve ser informado.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'cdtipdep\',\'frmDependentes\')',false);	
