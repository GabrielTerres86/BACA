 ...
 *			  	  10/07/2014 - Alterações para criticar propostas de cart. cred. em aberto durante exclusão de titulares (Lucas Lunelli - Projeto Bancoob).
 ...
 */
	...
	if ($operacao == 'BC') { //Melhoria 126
		...
		// Monta o xml dinamico de acordo com a operação
		$xml  = '';
		$xml .= '<Root>';
		$xml .= '	<Cabecalho>';
		...
		if( in_array($operacao,array('AV','VT','VG','VS','VE')) ) {
			...
			if($msgAtCad!='' && $flgcadas != 'M') {		
				exibirConfirmacao($msgAtCad,'Confirma&ccedil;&atilde;o - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0074.p\',\'\',controlaOperacao(\'OC\'))','controlaOperacao(\'OC\')',false);
			
			// Se nao existe necessidade de Revisao Cadastral
			} else if ($flgcadas != 'M') {						
	...
	function validaDados() {			
		// No inicio das validacoes, primeiro remove a classe erro de todos os campos
		...
		// Situassao
		if ( $GLOBALS['cdsitdct'] == '' ) exibirErro('error','Situa&ccedil;&atilde;o deve ser selecionada.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'cdsitdct\',\'frmContaCorrente\')',false); 
		
		// Tipo de Conta
		if ( $GLOBALS['cdtipcta'] == '' ) exibirErro('error','Tipo de conta deve ser selecionado.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'cdtipcta\',\'frmContaCorrente\')',false); 
		
		// Banco emi. cheque
		if ( ($GLOBALS['cdbcochq'] == '') ) exibirErro('error','Banco emiss&atilde;o do cheque deve ser informado.','Alerta - Ayllos','bloqueiaFundo(divRotina,\'cdbcochq\',\'frmContaCorrente\')',false);
