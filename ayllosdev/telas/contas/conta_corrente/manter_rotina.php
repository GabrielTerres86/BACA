			...
			if ( $operacao == 'VE' ) {	
				...
				if ( $tipconfi == 0 || $tipconfi == 1 ) {			
					if ($operacao == 'VT') {
						...
						if ( $tipconfi == 0 ) $msgconfi = 'Deseja confirmar alteração?';			
						exibirConfirmacao($msgconfi,'Confirmação - Ayllos','controlaOperacao(\''.$opeconfi.'\')','bloqueiaFundo(divRotina)',false);
					}
				} else if ( $tipconfi == 2 ) {
					echo 'hideMsgAguardo();';
