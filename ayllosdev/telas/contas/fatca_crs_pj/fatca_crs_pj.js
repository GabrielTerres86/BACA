...
function acessaOpcaoAba(nrOpcoes,id,opcao) {
	...
	$.ajax({		
		type: "POST", 
		dataType: "html",
		url: UrlSite + "telas/contas/fatca_crs_pj/principal.php",
		data: {
			nrcpfcgc: nrcpfcgc,
			flgcadas: flgcadas,
...
function controlaOperacao(operacao) {
	...
	switch (operacao) {			
		...
		case 'VA': 
			...
			return false;
			break;
		// Consulta para Alteração do FATCA/CRS relacionado ao Socio
		case 'CAS': 
			msgOperacao = 'abrindo altera&ccedil;&atilde;o';
			break;
		// Alteração para Validação - Validando Alteração - SOCIO
		case 'AVS': 
			manterRotina(operacao);
			return false;
			break;	
		// Validação para Alteração - Salvando Alteração - SOCIO
		case 'VAS': 
			manterRotina(operacao);
			return false;
			break;						
...
function manterRotina(operacao) {
	...
	switch (operacao) {	
		...
		case 'VAS': msgOperacao = 'salvando altera&ccedil;&atilde;o'; break;
		case 'AVS': msgOperacao = 'validando altera&ccedil;&atilde;o'; break;										
		default: return false; break;
	}
	showMsgAguardo('Aguarde, ' + msgOperacao + '...');

	if (operacao == 'VA' || operacao == 'AV') {
		var insocio = 'N';
		var nrCPF   = nrcpfcgc;
	} else if (operacao == 'VAS' || operacao == 'AVS') {
		var insocio = 'S';
		var nrCPF   = nrcpfcgcSocio;
	}
	
	var inobrigacao_exterior = $('#inobrigacao_exterior','#' + nomeForm ).val();
	var insocio_obrigacao    = $('#insocio_obrigacao','#' + nomeForm ).val();
	...
	$.ajax({		
		type: 'POST',
		url: UrlSite + 'telas/contas/fatca_crs_pj/manter_rotina.php', 		
		data: {
			nrcpfcgc: 			  nrCPF,
			inobrigacao_exterior: inobrigacao_exterior,
			insocio_obrigacao:    insocio_obrigacao,
			cdpais:     		  cdpais,
...
function controlaLayout() {	
	...
	var rInsocio_obrigacao    = $('label[for="insocio_obrigacao"]', '#'+nomeForm);	
	...
	var cInsocio_obrigacao 	  = $('#insocio_obrigacao,', '#'+nomeForm);
	...
	$('#divConteudoOpcao').hide(0, function() {
		...
		$('#divConteudoOpcao').css('width','630px');
		$('#divRotina').css('width','630px');		
		$('#frmDadosFatcaCrs').css({'padding-top':'5px'});
		
		// Formatação dos rotulos
		rInobrigacao_exterior.addClass('rotulo').css('width','553px');
		rInsocio_obrigacao.addClass('rotulo').css('width','553px');
		rCdpais.addClass('rotulo').css('width','405px');
		rNridentificacao.addClass('rotulo').css('width','405px');
		rCdpais_exterior.addClass('rotulo').css('width','398px');
		rDscodigo_postal.addClass('rotulo').css('width','60px');
		rNmlogradouro.addClass('rotulo-linha').css('width','40px');
		rNrlogradouro.addClass('rotulo-linha').css('width','30px');
		rDscomplemento.addClass('rotulo').css('width','60px');
		rDscidade.addClass('rotulo').css('width','60px');
		rDsestado.addClass('rotulo-linha').css('width','60px');
		
		// Formatação dos campos
		cInobrigacao_exterior.css('width','50px');
		cInsocio_obrigacao.css('width','50px');
		cCdpais.addClass('alphanum').css('width','35px');
		cDspais.css('width','141px');
		cNridentificacao.addClass('alphanum').css('width','199px').attr('maxlength','30');
		cCdpais_exterior.addClass('alphanum').css('width','35px');
		cDspais_exterior.css('width','141px');
		cDscodigo_postal.css('width','80px');
		cNmlogradouro.addClass('alphanum').css('width','320px');
		cNrlogradouro.addClass('numerocasa').css('width','55px');
		cDscomplemento.addClass('alphanum').css('width','537px');
		cDscidade.addClass('alphanum').css('width','240px');
		cDsestado.addClass('alphanum').css('width','231px');
		...
		if (operacao == 'CA') {
			...
			cInsocio_obrigacao.habilitaCampo();
		}
...
function abrirTelaSocio(tr){
	//Salvar o CPF do socio para alteracao
	nrcpfcgcSocio = normalizaNumero($('#nrcpfcgc', tr).val());
	controlaOperacao('CAS');
}

function controlaLayoutSocio() {	
	...
	$('#divConteudoOpcao').hide(0, function() {
		...
		cCdpais.addClass('alphanum').css('width','35px');
		cDspais.css('width','141px');
		cNridentificacao.css('width','199px').attr('maxlength','30');
		cCdpais_exterior.addClass('alphanum').css('width','35px');
		cDspais_exterior.css('width','141px');
		cDscodigo_postal.css('width','80px');
		cNmlogradouro.addClass('alphanum').css('width','320px');
		...
		if (operacao == 'CAS' && cInobrigacao_exterior.val() == 'S') {
		...
		// if (operacao == 'CAS' && (inacordo != 'FATCA' && inacordo != 'CRS')) {
...
function formatarTabela(){
	...
	$('#fsetTitular').css({ 'clear': 'both', 'border': '1px solid #bbb', 'margin': '3px 0px', 'padding': '0 3px 5px 3px' });
	$('#fsetSocios').css({ 'clear': 'both', 'border': '1px solid #bbb', 'margin': '3px 0px', 'padding': '0 3px 5px 3px' });
	...
	// Caso estiver no modo alteracao, nao permitir duplo click na tabela socio
	if (operacao == 'CA') {
		tabela.formataTabela( ordemInicial, arrayLargura, arrayAlinha, '');
	} else {
		tabela.formataTabela( ordemInicial, arrayLargura, arrayAlinha, 'abrirTelaSocio(this)');
	}
}

function alterarSocioSelecionado(){
