...
function validaDesativar() {
    ...
    if (cCdpacote.val() == 0 || cCdpacote.val() == "") {
        showError("error", "Informe um pacote de tarifas valido.", "Alerta - Ayllos", "blockBackground(parseInt($('#divRotina').css('z-index')))");
    }
    
    // Mostra mensagem de aguardo
    showMsgAguardo("Aguarde, validando pacote de tarifas...");
...
function buscaInformacoesPacote() {
    ...
    if (cCdpacote.val() == 0 || cCdpacote.val() == "") {
        showError("error", "Informe um pacote de tarifas valido.", "Alerta - Ayllos", "cCdpacote.focus();");
        $("#divLupa").css('display', 'block');
        return false;
    }
...
function buscaDados() {
    ...
    if ((cCddopcao.val() == "C" || cCddopcao.val() == "D")  && (cCdpacote.val() == 0 || cCdpacote.val() == "")) {
        showError("error", "Informe um pacote de tarifas v&aacute;lido.", "Alerta - Ayllos", "cCdpacote.focus();");
        return false;
    }
...
function validaIncluirPacote() {
    ...
    if (cCdpacoteInc.val() == "" || cCdpacoteInc.val() == 0) {
        showError("error", "C&oacute;digo do Pacote Inv&aacute;lido.", "Alerta - Ayllos", "estadoInicial();");
        return false;
    } else if (cDspacoteInc.val() == "") {
        showError("error", "Informe a descri&ccedil;&atilde;o do pacote.", "Alerta - Ayllos", "cDspacoteInc.focus();");
        return false;
    ...
    // Mostra mensagem de aguardo
    showMsgAguardo("Aguarde, validando inclus&atilde;o de pacote...");
...
function incluirPacote() {
    ...
    // Mostra mensagem de aguardo
    showMsgAguardo("Aguarde, incluindo pacote de tarifas...");
...
function validaDadosTarifas() {
    ...
    if (!flgServ) {
        showError("error", "Informe o(s) servi&ccedil;o(s) do pacote de tarifas.", "Alerta - Ayllos", "");
        return false;
    }
    ...
    if (flgErro) {
        ...
        showConfirmacao("Confirma a inclus&atilde;o do pacote de tarifas?", "Confirma&ccedil;&atilde;o - Ayllos", "incluirPacote()", "", "sim.gif", "nao.gif");
    }
}
