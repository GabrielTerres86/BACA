...
function carregaTelaEstornar(cNrdconta1, cNrctremp1, cIdtpoest, confirm){
...
function carregaPrejuizoCC(cNrdconta, confirm){
...
function carregaPrejuizoEpr(cNrdconta, cNrctremp, confirm){

	if (cNrctremp == "") {
	  //hideMsgAguardo();
      showError('error', 'Favor preencher o n&uacute;mero do contrato', 'Alerta - Ayllos');
    }else{
		// Mostra mensagem de aguardo
...
function importaArquivo(cNmdarqui, confirm){
...
function ajustaBotaoContinuar(){
	...
	if ( $('#cddopcao','#frmCab').val() == 'F') {
		carregaPrejuizoCC(cNrdconta1.val(), 0);
	}
	
	if ( $('#cddopcao','#frmCab').val() == 'P') {
		carregaPrejuizoEpr(cNrdconta2.val(), cNrctremp2.val(), 0);
	}
	
	if ( $('#cddopcao','#frmCab').val() == 'I') {
		importaArquivo(cNmdarqui, 0);
	}
