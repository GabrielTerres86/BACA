 ...
 * 032: [25/05/2017] Jean	            (Mout´S): Melhoria 324 - permitir gerar (forçar) a transferencia para prejuízo para os produtos PP e TR
 */
?>
...
<?
	...
	if 	(in_array($operacao,array('C_NOVA_PROP'))){
		...
		//$procedure = 'transf_contrato_prejuizo'; -- Melhoria 324 - retirada chamada antiga da transferencia para prejuizo - Jean (Mout´S)
		$procedure = 'TRANSF_PREJUIZO';
	} 
	else if (in_array($operacao,array('C_DESFAZ_PREJU'))) {
		//$procedure = 'desfaz_transferencia_prejuizo'; -- Melhoria 324 - retirada chamada antiga desfazer prejuizo - Jean (Mout´S)
		$procedure = 'DESFAZ_PREJUIZO';
	} else{
	...
	if (in_array($operacao,array('C_EXTRATO','TC','C_NOVA_PROP',''))) {
		...
		if (in_array($operacao,array('C_EXTRATO',''))){
			...
			arrayRegistros['qtdiaatr'] = '<? echo getByTagName($registros,'qtdiaatr'); ?>';
		...
		if (in_array($operacao,array('C_PAG_PREST'))){ 
			...
			for($i=0; $i < count($descontos); $i++) {
	         ...
	         /* 20/05/2017 - Melhoria 324 - transferencia para prejuízo - Jean - Mout´s
			   	  Irá gerar a transferencia forçada de produtos PP e TR de acordo com as novas regras de transferencia */
			...
			$xmlResult = mensageria($xml, "ATENDA", $procedure, $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
	...
	/*
	// 02/06/2017 - Melhoria 324 - Comentado o antigo processo referente a transferencia para prejuizo e desfaz prejuizo
	else if(in_array($operacao,array('C_TRANSF_PREJU','C_DESFAZ_PREJU'))) {
