...
var rCddopcao, rcdsubgru, rCdsubgru, cCddopcao, cDssubgru, cCdsubgru, cTodosCabecalho, cCdfaixav, cCdpartar, glbTabCdpartar,
	glbTabCdfaixav, glbTabCdcomissao, glbTabVlinifvl, glbTabVlfinfvl, glbTabVlcomiss, glbTabDscomissao, glbTabCdhistor, glbTabCdhisest, glbTabDshistor, glbTabDshisest,
...
function LiberaCampos(){
	...
	//Habilita campos
	$('#cdcomissao', '#frmComissao').habilitaCampo();	
	$('#cdcomissao', '#frmComissao').attr('maxlength', '9').setMask('INTEGER', 'zzzzzzzzz', '', '');
	
	$('#cdcomissao', '#frmComissao').focus();
	
	$('#dscomissao', '#frmComissao').desabilitaCampo();
	$('#tpvalor', '#frmComissao').desabilitaCampo();
	$('#flgativo', '#frmComissao').desabilitaCampo();

	
	$('#divBotoes', '#divTelaComissao').css({ 'display': 'block' });
	...
	if ($('#cddopcao', '#frmComissao').val() == 'I') {
		...
		$('#cdcomissao', '#frmComissao').desabilitaCampo();
		$('#dscomissao', '#frmComissao').habilitaCampo();
		$('#tpvalor', '#frmComissao').habilitaCampo();
		$('#flgativo', '#frmComissao').habilitaCampo();
		$('#dscomissao','#frmComissao').focus(); 
		trocaBotao('Incluir');				
	}
...
function trocaBotao(botao) {
	...
	$('#divBotoes', '#divTelaComissao').html('');
	$('#divBotoes', '#divTelaComissao').append('<a href="#" class="botao" id="btVoltar" onclick="btnVoltarComissao(); return false;">Voltar</a>');

	if (botao == 'btnConcluir') {		
		$('#divBotoes', '#divTelaComissao').append('&nbsp;<a href="#" class="botao" onClick="btnConcluir()" return false;" >Prosseguir</a>');
		return false;
	}

	if (botao != '') {
		$('#divBotoes', '#divTelaComissao').append('&nbsp;<a href="#" id="btAcao" class="botao" onClick="realizaOperacao()" return false;" >' + botao + '</a>');
	}
...
function btnConcluir() {
	...
	cdcomissaoTemp = $('#cdcomissao', '#frmComissao').val();
	cdcomissao = normalizaNumero(cdcomissaoTemp);	

	if ((cdcomissao != '') && (!$('#cdcomissao', '#frmComissao').hasClass('campoTelaSemBorda'))) {
		//realizaOperacao();
		consultaComissao();
	}
}
...
function consultaComissao(){
	...
	var cdcomissao = $('#cdcomissao', '#frmComissao').val();
	cdcomissao = normalizaNumero(cdcomissao);
...
function mostraDetalhamentoComissao(value) {
	...
	glbTabCdcomissao = $('#cdcomissao', '#frmComissao').val();
    glbTabDscomissao = $('#dscomissao', '#frmComissao').val();
...
function buscaDetalheComissao() {
	...
	var cdcomissao = normalizaNumero(glbTabCdcomissao);
...
function formataDetalheComissao() {
	...
	var rCdcomissao = $('label[for="cdcomissao"]', '#frmDetalheComissao');
	var rVlinifvl = $('label[for="vlinifvl"]', '#frmDetalheComissao');
	var rVlfinfvl = $('label[for="vlfinfvl"]', '#frmDetalheComissao');
	var rVlcomissao = $('label[for="vlcomissao"]', '#frmDetalheComissao');

	rCdfaixav.addClass('rotulo').css('width', '60px');
	rCdcomissao.addClass('rotulo').css('width', '60px');
	rVlinifvl.addClass('rotulo').css('width', '132px');
	rVlfinfvl.addClass('rotulo').css('width', '132px');
	rVlcomissao.addClass('rotulo').css('width', '132px');
	
	// Campos
	var cTodos = $('input[type="text"],select', '#frmDetalheComissao');
	var cCdfaixav = $('#cdfaixav', '#frmDetalheComissao');
	var cCdcomissao = $('#cdcomissao', '#frmDetalheComissao');
	var cDscomissao = $('#dscomissao', '#frmDetalheComissao');
	var cVlinifvl = $('#vlinifvl', '#frmDetalheComissao');
	var cVlfinfvl = $('#vlfinfvl', '#frmDetalheComissao');
	var cVlcomissao = $('#vlcomissao', '#frmDetalheComissao');
	
	cTodos.habilitaCampo();
	cCdfaixav.css('width', '60px').desabilitaCampo();

	cCdcomissao.css('width', '60px').desabilitaCampo();
	cDscomissao.css('width', '250px').desabilitaCampo();
	cVlinifvl.addClass('moeda').css('width', '100px').setMask('DECIMAL', 'zzz.zzz.zz9,99', '.', '');
	cVlfinfvl.addClass('moeda').css('width', '100px').setMask('DECIMAL', 'zzz.zzz.zz9,99', '.', '');
	cVlcomissao.addClass('moeda').css('width', '100px').setMask('DECIMAL', 'zzz.zzz.zz9,99', '.', '');
	...
	if (cddopdet == 'X') {
		...
		$('#vlcomissao', '#frmDetalheComissao').desabilitaCampo();		
	}
...
function formataDetalhamento() {
	...
	$('table > tbody > tr', divRegistro).click(function () {
		glbTabCdcomissao = normalizaNumero($('#cdcomissao', '#frmComissao').val());
		glbTabDscomissao = $('#dscomissao', '#frmComissao').val();
		...
		glbTabVlcomiss = $(this).find('#vlcomissao > span').text();
	});
...
function controlafrmDetalheComissao() {
	...
	$('#vlfinfvl', '#frmDetalheComissao').unbind('keypress').bind('keypress', function (e) {
		if (e.keyCode == 9 || e.keyCode == 13) {
			$('#vlcomissao', '#frmDetalheComissao').focus();
			return false;
		}
	});
...
function realizaOperacao() {
	...
	if(cddopcao == 'I' || cddopcao == 'A'){
		...
		if ($('#dscomissao', '#frmComissao').val() == '') {
			showError('error', 'Descricao da Comissao deve ser informado.', 'Alerta - Ayllos', "focaCampoErro(\'dscomissao\', \'frmComissao\');");
			return false;
		}
	}
	...
	var cdcomissao = $('#cdcomissao', '#frmComissao').val();
	cdcomissao = normalizaNumero(cdcomissao);
	var dscomissao = $('#dscomissao', '#frmComissao').val();
	...
	$.ajax({
		...
		success: function (response) {
			try {
				...
				eval(response);		
				if (cddopcao == 'I'){
					$('#btAcao', '#divBotoes').desabilitaCampo();
				}
			} catch (error) {
				hideMsgAguardo();
				showError("error", "N&atilde;o foi poss&iacute;vel concluir a requisi&ccedil;&atilde;o. " + error.message, "Alerta - Ayllos", "blockBackground(parseInt($('#divRotina').css('z-index')))");
...
function btnVoltarComissao() {	
	...
	$('#cdcomissao', '#frmComissao').val('');
	$('#dscomissao', '#frmComissao').val('');
...
function controlaFoco() {
	...
	$('#cdcomissao', '#frmComissao').unbind('keypress').bind('keypress', function (e) {
		if (e.keyCode == 9 || e.keyCode == 13) {
			if ($('#cdcomissao', '#frmComissao').val() != '') {
				//realizaOperacao();
				consultaComissao();
			} else {
...
function controlaPesquisa() {
    ...
    if ($("#cdcomissao", "#frmComissao").prop("disabled") == true) { return; }
    ...
    qtReg = '';
    filtros = 'C&oacutedigo;cdcomissao;30px;S;'+cdcomissao+';S|Comiss&atileo;dscomissao;200px;S;'+dscomissao+';S';
    colunas = 'C&oacutedigo;cdcomissao;15%;left|Comiss&atildeo;dscomissao;85%;left';
...
function manterDetalhamento(cddopdet){
	...
	if (cddopdet != 'E'){
		...
		var cdcomissao = $('#cdcomissao', '#frmDetalheComissao').val();
		var vlinifvl = $('#vlinifvl', '#frmDetalheComissao').val();
		var vlfinfvl = $('#vlfinfvl', '#frmDetalheComissao').val();
		var vlcomissao = $('#vlcomissao', '#frmDetalheComissao').val();

		cdfaixa = normalizaNumero(cdfaixa);
		cdcomissao = normalizaNumero(cdcomissao);
		vlinifvl = normalizaNumero(vlinifvl);
		vlfinfvl = normalizaNumero(vlfinfvl);
		vlcomissao = normalizaNumero(vlcomissao);
		vlinifvl = vlinifvl.replace(',', '');
		vlfinfvl = vlfinfvl.replace(',', '');
		vlcomissao = vlcomissao.replace(',', '');

		if (vlinifvl == 0 || vlinifvl == "" || vlinifvl == undefined || vlinifvl == null) {
			showError("error", "Informe o valor inicial.", "Alerta - Ayllos", "bloqueiaFundo(divRotina);$('#vlinifvl', '#frmDetalheComissao').focus();");
			return false;
		}
		...
		if (vlcomissao == 0 || vlcomissao == "" || vlcomissao == undefined || vlcomissao == null) {
			showError("error", "Informe o valor da comiss&atilde;o.", "Alerta - Ayllos", "bloqueiaFundo(divRotina);$('#vlcomissao', '#frmDetalheComissao').focus();");
			return false;
		}
	}else{
