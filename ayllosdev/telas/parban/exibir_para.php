...
<div class="condensado">
	<br/>
	<fieldset>
		<legend><b><? echo utf8ToHtml('Exibir para')?></b></legend>		
		<!--
		...
		-->
			...
			<select id="tpfiltro" name="tpfiltro" class="campo" style="width:300px;" onChange="carregaEnviar(this.value);">
				<option value="0" <?php if($tpfiltro == 0){ echo " selected"; } ?> >Escolher os Filtros</option>
				<option value="1" <?php if($tpfiltro == 1){ echo " selected"; } ?>>Importar arquivo CSV</option>
			</select>
		...
		<?php// if($glbvars["cdcooper"] == 3){ ?>
			...
			<div id="divDireita" name="divDireita" style=" float: left; padding-top: 10px; padding-left: 60px;">
				...
				<br/><br/>
				<label for="dsfiltro_produto" style="clear:both;" ><? echo utf8ToHtml('Outros Filtros:'); ?></label>
				<br/><br/>
				<input type="radio" value="0" name="dsfiltro_produto" id="dsfiltro_produto_0" style="clear:both;" />
				<label for="inoutros_filtros_0">&nbsp;&nbsp;<? echo utf8ToHtml('Sem Filtros') ?></label>
				<br/>
				<input type="radio" value="35" name="dsfiltro_produto" id="dsfiltro_produto_35" style="clear:both;" />
				<label for="inoutros_filtros_1">&nbsp;&nbsp;<? echo utf8ToHtml('Somente Cooperados com Pr&eacute;-Aprovado') ?></label>
				<br/>
			</div>
		</div>
		<script>
