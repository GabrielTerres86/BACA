<?php 
	...
	**/
	...
	if(isset($_FILES['arq_upload']['name'][0]) && trim($_FILES['arq_upload']['name'][0] != "")){
		...
		echo("<script> console.log(' importa imagem  ".$moveImg."');</script> ");
		if($moveImg){

			try {
			  echo("<script> console.log('curl -T  ".$_UP['pasta'] . $nmarqimg . ' -u '.$user.':'.$pass.' '.$_UP['srvImg']."');</script> ");
	...
	if($tpfiltro == 1){
		...
		// Depois verifica se é possível mover o arquivo para a pasta escolhida 
		$nmarquiv = 'parban_'.$nmarquiv.'_'.date('Y-m-d-h-i-s').'.csv';
