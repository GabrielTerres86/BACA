<?php 
	...
	**/
	...
	$cdBanner = (isset($_POST['cdBanner'])) ? $_POST['cdBanner'] : 0; 
 	$cdCanal = (isset($_POST['cdcanal'])) ? $_POST['cdcanal'] : 10; //10 - mobile ou 
	...
	$xmlResultMantemMsgManu = mensageria($xml, 'TELA_PARBAN','TELA_BANNER_REMOVER', $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
	...
	if (isset($xmlObjetoMantemMsgManu->roottag->tags[0]->name) && strtoupper($xmlObjetoMantemMsgManu->roottag->tags[0]->name) == "ERRO") {
		...
		echo "parent.framePrincipal.eval(\"showError('inform','Banner removido com sucesso!','Alerta - Ayllos','hideMsgAguardo(); ');removerBannerDaLista();\");";
		exit;
	}		

	function gerarErro($dserro){
		echo "parent.framePrincipal.eval(\"showError('error','".$dserro . $msgErroArq."','Alerta - Ayllos','hideMsgAguardo();');\");";
		exit;
	}
?>