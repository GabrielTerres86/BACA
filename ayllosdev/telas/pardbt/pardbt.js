...
function validarProcessoExec(checkbox) {
	if ($(checkbox).prop('checked') == false || $('#tipoExecucao').val() == 'E')  {
		return false;
	}

	var cdprocesso = $(checkbox).closest('td').find('input[type="hidden"]').eq(0).val();

	showMsgAguardo('Aguarde, validando programa...');
...
function desabilitarCheckProcesso(cdprocesso) {
	$('.checkboxExecutar').each(function(i, elem) {
		if ($(elem).prop('id') == 'exec_' + cdprocesso) {
			$(elem).prop('checked', false).prop('disabled', true);
		}
	});	
}
...
function formataHistoricos() {
	...
	var arrayLargura = ['120px', '110px', '80px', '471px'];	
