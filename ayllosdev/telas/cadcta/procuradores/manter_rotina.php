...
<?	
	...
	if($procedure == 'valida_dados' ||
	   $procedure == 'grava_dados'){
		...
		$xml .= '       <nmrotina>PROCURADORES</nmrotina>';
	}
	...
	// Se é Inclusão ou Alteração
		...
		$metodo =    "controlaOperacaoProcuradores(\"TC\");";
        if ($operacao == 'E'){
          $metodo = 'hideMsgAguardo();fechaRotina(divRotina,"","encerraRotina(true);");';
        } else if ($operacao == 'I'){
          $metodo =    "controlaOperacaoProcuradores(\"CTP\");";  
        }
	
		// Verificar se existe "Verificação de Revisão Cadastral"
		if($msgAtCad!='') {
		
			if($operacao=='I') exibirConfirmacao($msgAtCad,'Confirmação - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0058.p\',\''.$stringArrayMsg.'\')','exibirMensagens(\''.$stringArrayMsg.'\',\'controlaOperacaoProcuradores(\"CTP\")\')',false);
			if($operacao=='A') exibirConfirmacao($msgAtCad,'Confirmação - Ayllos','revisaoCadastral(\''.$chaveAlt.'\',\''.$tpAtlCad.'\',\'b1wgen0058.p\',\''.$stringArrayMsg.'\')','exibirMensagens(\''.$stringArrayMsg.'\',\'controlaOperacaoProcuradores(\"TC\")\')',false);
