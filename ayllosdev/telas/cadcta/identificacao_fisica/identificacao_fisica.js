...
function acessaOpcaoAba(nrOpcoes,id,opcao) {
	...
	$.ajax({
		type: 'POST',
		dataType: 'html',
		url: UrlSite + 'telas/cadcta/identificacao_fisica/principal.php',
		data: {
			nrdconta: nrdconta,
			idseqttl: idseqttl,
			operacao: operacao,
...
function abrirRotina(nomeValidar,nomeTitulo,nomeScript,nomeURL,ope) {
	...
	var url = UrlSite + "telas/cadcta/" + nomeScript + "/" + nomeURL;
...
function controlaOperacao( novaOp ) {
	...
	switch (operacao) {
		...
		case 'FI':
			...
			idseqttl = $('#idseqttl > option','#frmCabCadcta').length + 1;
			nrcpfcgc = '';
			cdgraupr = '';
			fechaRotina(divRotina,'','encerraRotina(true);');
			break;
...
function liberaOperacao() {
	...
	// INCLUSÃO
		...
		if ( $('#idseqttl > option','#frmCabCadcta').length >= 4 ) {
...
function manterRotina() {
	...
	$.ajax({
		type: 'POST',
		url: UrlSite + 'telas/cadcta/identificacao_fisica/manter_rotina.php',
		data: {
...
function keypressCPF() {
	...
	idseqttl = (operacao == 'CI' || operacao == 'BI') ? $('#idseqttl > option','#frmCabCadcta').length + 1 : $('#idseqttl','#frmCabCadcta').val();
