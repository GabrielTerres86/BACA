   ...
   // echo "ODIRLEI - " . $rotinasTela;
	...
	foreach($tipos_conta as $tipo_conta) {
		if ( getByTagName($tipo_conta->tags,'cdtipo_conta') == $cabecalho[12]->cdata ) { //pos.12 descricao tp.conta
			echo '$("#cdtipcta","#frmCabCadcta").val("' . getByTagName($tipo_conta->tags,'cdtipo_conta') . ' - ' . getByTagName($tipo_conta->tags,'dstipo_conta') .'");';
		}
	}
	...
	foreach($situacoes as $situacao) {
		if ( getByTagName($situacao->tags,'cdsituacao') == $cabecalho[14]->cdata ) { //pos.14 descricao
			echo '$("#cdsitdct","#frmCabCadcta").val("' . getByTagName($situacao->tags,'cdsituacao') . ' - ' . getByTagName($situacao->tags,'dssituacao') .'");';
		}
	}
