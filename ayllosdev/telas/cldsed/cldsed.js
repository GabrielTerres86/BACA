	...
	if ( $('#cddopcao','#frmCab').hasClass('campoTelaSemBorda')  ) {
		...
		if ( (cddopcao == "C") || (cddopcao == "J") ) {
			if($('#dtmvtolt','#frmInfConsultaMovimentacao').hasClass('campoTelaSemBorda') ){
				showConfirmacao("078 - Confirma a opera&ccedil;&atilde;o?","Confirma&ccedil;&atilde;o - Ayllos","realizaOperacao('J');","showError('error','079 - Opera&ccedil;&atilde;o n&atilde;o realizada.','Alerta - Ayllos','unblockBackground()');","sim.gif","nao.gif");			
			} else {
				realizaOperacao("C");
			}
		    ...
		    showConfirmacao("078 - Confirma a opera&ccedil;&atilde;o?","Confirma&ccedil;&atilde;o - Ayllos","realizaOperacao('X');","showError('error','079 - Opera&ccedil;&atilde;o n&atilde;o realizada.','Alerta - Ayllos','unblockBackground()');","sim.gif","nao.gif");						
		} else if (cddopcao == "P") {
			if (cSaidaP.val() == "T") {
				realizaOperacao("P");
...
// rotina de impressão da opção P
function imprimeDetalheMov(){
	
	var sidlogin = $("#sidlogin","#frmMenu").val();
	
	var nrdconta = normalizaNumero(cNrdcontaP.val());
	...
	$('#frmImpressao').append('<input type="hidden" id="dtrefini" name="dtrefini" value="'+dtrefini+'" />');
	$('#frmImpressao').append('<input type="hidden" id="dtreffim" name="dtreffim" value="'+dtreffim+'" />');
	$('#frmImpressao').append('<input type="hidden" id="cdstatus" name="cdstatus" value="'+cdstatus+'" />');
	$('#frmImpressao').append('<input type="hidden" id="cdincoaf" name="cdincoaf" value="'+cdincoaf+'" />');
	$('#frmImpressao').append('<input type="hidden" id="sidlogin" name="sidlogin" value="'+sidlogin+'" />');
	$('#frmImpressao').append('<input type="hidden" id="saida" name="saida" value="'+saida+'" />');

	var action = UrlSite + "telas/cldsed/gera_imprime_arq_pesquisa.php";	
		...
		success: function(response) {
			if(pagina == "nome_arquivo"){
				...
				$('#nmarquivo', '#divNomeArquivoImpresssao').focus().unbind('keypress').bind('keypress', function(e) {
					if ( e.keyCode == 9 || e.keyCode == 13 ) {	
						if(cddopcao == "P") geraArquivo("gera_imprime_arq_pesquisa");
						else if (cddopcao == "T") geraArquivo("gera_imprime_arq_atividade", excel);
						return false;
					}	
				});
			}
			
			if (pagina == "gera_imprime_arq_pesquisa") {
				fechaRotina($('#divRotina'));
				hideMsgAguardo();
				estadoInicial();
			}
			
			if (pagina == "gera_imprime_arq_atividade") {
				fechaRotina($('#divRotina'));
				hideMsgAguardo();
				estadoInicial();
			}			
...
function imprimeListaMovimentacoes(){
	...
	var action = UrlSite + "telas/cldsed/gera_imprime_arq_atividade.php";	
	carregaImpressaoAyllos("frmImpressao",action, 'estadoInicial()');
}