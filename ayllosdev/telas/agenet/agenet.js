/*!
 * FONTE        : agenet.js                             Última altração: 25/04/2016
...
function formataFormularioFiltro(){
	...
	$('label[for="dtiniper"]',"#frmFiltro").addClass("rotulo").css({"width":"120px"}); 
	$('label[for="dtfimper"]',"#frmFiltro").addClass("rotulo-linha").css({"width":"100px"}); 
	...
	$('#dtiniper','#frmFiltro').css({'width':'100px','text-align':'right'}).habilitaCampo().addClass('inteiro').attr('maxlength','14').setMask("DATE","","","");
	$('#dtfimper','#frmFiltro').css({'width':'100px','text-align':'right'}).habilitaCampo().addClass('inteiro').attr('maxlength','14').setMask("DATE","","","");
...
function cancelarAgendamento(dtmvtolt, nrdocmto) {
		...
		if ($('#cddopcao', '#frmCab').val() == 'C') {
		    ...
		    if (dstiptra.indexOf('TED') > -1 && insitlau == 1) {
		        ...
		        $("#btCancelar", "#divBotoes").unbind('click').bind('click', function () {
		            ...
		            showConfirmacao('Deseja confirmar opera&ccedil;&atilde;o?', 'Confirma&ccedil;&atilde;o - Ayllos', 'cancelarAgendamento("' + $('#dtmvtolt', tr).val() + '", "' + $('#nrdocmto', tr).val() + '");', '$(\'#btVoltar\',\'#divBotoes\').focus();', 'sim.gif', 'nao.gif');
		            return false;
