 ...
 * --------------
 * ALTERA��ES   : #273953 Alinhamento das colunas das tabelas (Carlos)
 * --------------
 *                12/07/2016 #451040 Retirar o bot�o "Executar Devolu��o" (Carlos)
 *
...
function estadoInicial() {
	...
	// Inicializa Variaveis do Cabe�alho
	cCdagenci.val( '' );
	cNrdconta.val( '' );
...
function proc_gera_dev() {
	if (altalinea == true){
		showConfirmacao('Confirma Opera&ccedil;&atilde;o?',"Confirma&ccedil;&atilde;o - Ayllos",'verifica_alinea("AL");','fechaRotina($("#divRotina"));',"sim.gif","nao.gif");
	}else{
...
function formataTabelaLancto() {
	...
	*********************/
	...
	$('table > tbody > tr', divRegistro).dblclick( function() {
		...
		// Se tiver alinea e n�o for devolvido e n�o for alineas de contra-ordem(20,21,28,70) e altera alinea para devolu��es 35 somente quando haver craplcm
...
function marcar_cheque_devolu() {
	...
	//Adquire as informa��es das checkbox selecionadas
	...
	$.ajax({
		...
		success: function(response) {
			try {
				hideMsgAguardo();
				eval(response);
				// validar horario para devolu��o de acordo com o parametrizado na TAB055
                if(execucao == 'yes'){                   
...
// Chamada de Formul�rio de Senha - Autorizacao do Coordenador
...
// Chamada de Formul�rio de Alinea
            ...
            try {
                if ( response.indexOf('showError("error"') == -1) {
                    try {						
                        ...
                        showError('inform',mensagemRetorno,'Alerta - Ayllos','hideMsgAguardo();fechaRotina($("#divRotina"));gera_log();iqtSelecao = 0; arrayRegLinha = new Array();arrayRegDados = new Array();'+funcao);
                    } catch(error) {
                        hideMsgAguardo();
                        showError('error','N&atilde;o foi poss&iacute;vel concluir a requisi&ccedil;&atilde;o.','Alerta - Ayllos','unblockBackground()');
                    }
...
// Processo de Execucao de Devolucoes
// Chamada de Formul�rio de Opcoes de Banco - Executar Devolucao
function buscaBanco() {
...
// Processo de Execucao de Devolucoes
// Chamada de Formul�rio de Senha - Autorizacao do Sistema
    ...
    $.ajax({
        ...
        success: function(response) {
            if ( response.indexOf('showError("error"') == -1) {
                try {
                    $('input,select','#'+frmCab).removeClass('campoErro');
                    showError('inform','Devolu&ccedil;&otilde;es Executadas','Alerta - Ayllos','hideMsgAguardo();fechaRotina($("#divRotina"));BuscaDevolu(1,30,"CA");');
                } catch(error) {
                    hideMsgAguardo();
                    showError('error','N&atilde;o foi poss&iacute;vel concluir a requisi&ccedil;&atilde;o.','Alerta - Ayllos','unblockBackground()');
                }
