...
function manter_rotina() {
	...
	if (cddopcao == 'I'){
		var action = UrlSite + 'telas/impsim/manter_rotina.php?keylink=' + milisegundos();
		
		// Configuro o formulário para posteriormente submete-lo
		$('#frmImportaArquivo').attr('method','post');
		$('#frmImportaArquivo').attr('action',action);
		$('#frmImportaArquivo').attr("target",'frameBlank');		
		showMsgAguardo( "Aguarde, enviando arquivo ...<br />&nbsp;&nbsp;&nbsp;Esta operação pode levar alguns minutos ..." );
		//$('#divListErr').html('');
		$('#frmImportaArquivo').append('<input type="hidden" name="cddopcao" id="cddopcao" value="I" />');
		$('#frmImportaArquivo').submit();
	}
	else{	
...
function importaArquivo(){
	...
	$('#sidlogin','#frmImportaArquivo').remove();
	$('#cddopcao','#frmImportaArquivo').remove();
	
	// Insiro input do tipo hidden do formulário para enviá-los posteriormente
	$('#frmImportaArquivo').append('<input type="hidden" id="sidlogin" name="sidlogin" />');
	$('#frmImportaArquivo').append('<input type="hidden" name="cddopcao" id="cddopcao" value="I" />');
	
	// Agora insiro o devido valor no campo (campo necessario para validar sessao)
	$('#sidlogin','#frmImportaArquivo').val($('#sidlogin','#frmMenu').val());	
