...
DECLARE
  ...
  CURSOR cr_crapcop IS
    SELECT *
    FROM crapcop p
    WHERE p.cdcooper = 3; --p.flgativo = 1 and
...
BEGIN
  ...
  FOR rw_crapcop IN cr_crapcop LOOP
    ...
    THEN
      ...
      VALUES
        (rw_crapcop.cdcooper, vr_nmdatela, vr_cdopcoes, vr_dsopcoes, upper(vr_dsprogra), vr_rsprogra, 6, 1, 1, 1, 0, 2,
         1);
    END IF;
  ...
  IF cr_crapaca%NOTFOUND
  THEN

    dbms_output.put_line('Insere CRAPACA - > IMPSIM - > TELA_IMPSIM.pc_importar_arquivo_SIM');
  ...
  THEN
    ...
    dbms_output.put_line('Insere CRAPACA - > IMPSIM - > TELA_IMPSIM.pc_exportar_arquivo_SIM');
