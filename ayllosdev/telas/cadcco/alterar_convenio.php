<?php 
	...
	********************************************************************************/
	...
	$xmlResult = mensageria($xml, "TELA_CADCCO", "ALTERACAO", $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
	...
	if (strtoupper($xmlObjAltera->roottag->tags[0]->name) == "ERRO") {
		...
		if($flgregis == '1'){
			...
			exibirErro('inform','Conv&ecirc;nio alterado com sucesso!','Alerta - Ayllos','buscaTarifas(\'1\',\'30\',\''.$nrconven.'\',\''.$cddbanco.'\');',false);                              
						 
		}else{
			exibirErro('inform','Conv&ecirc;nio alterado com sucesso!','Alerta - Ayllos','estadoInicial();',false); 
		}
	}
