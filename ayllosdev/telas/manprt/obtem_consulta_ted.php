...
<?	
	...
	// Recebe o POST
	$inidtpro 			= (isset($_POST['inidtpro'])) ? $_POST['inidtpro'] : '' ;
	$fimdtpro 			= (isset($_POST['fimdtpro'])) ? $_POST['fimdtpro'] : '' ;
	$inivlpro 			= (isset($_POST['inivlpro'])) ? $_POST['inivlpro'] : 0 ;
	$fimvlpro 			= (isset($_POST['fimvlpro'])) ? $_POST['fimvlpro'] : 0 ;
	$dscartor 			= (isset($_POST['dscartor'])) ? $_POST['dscartor'] : '' ;
	...
	$xml .= "<Root>";
	$xml .= " <Dados>";	
    $xml .= "   <inidtpro>".$inidtpro."</inidtpro>";
	$xml .= "   <fimdtpro>".$fimdtpro."</fimdtpro>";
	$xml .= "   <inivlpro>".$inivlpro."</inivlpro>";
	$xml .= "   <fimvlpro>".$fimvlpro."</fimvlpro>";
	$xml .= "   <indconci>".$indconci."</indconci>";
	$xml .= "   <dscartor>".$dscartor."</dscartor>";
	$xml .= " </Dados>";
	$xml .= "</Root>";

	// Requisicao dos dados de parametrizacao da negativacao Serasa
	$xmlResult = mensageria($xml, "MANPRT", "MANPRT_CONSULTA_TED", $glbvars["cdcooper"], $glbvars["cdagenci"], $glbvars["nrdcaixa"], $glbvars["idorigem"], $glbvars["cdoperad"], "</Root>");
