...
//                Isto e' feito para PJ quando carregada a tela para evitar erros. 
 ...
 function carregaImagem (tipoConsulta) {
	...
	$.ajax({
		type: "GET",
		url:  "../../includes/consulta_rfb/getcaptcha.php",
		data: {
		...
		error: function(objAjax,responseError,objExcept) {	
			hora = data.getTime();
			document.getElementById("imgCaptcha").src = "../../includes/consulta_rfb/getcaptcha.php?tipoConsulta=" + tipoConsulta + "&" + hora;	
 ...
 function efetuarConsulta(flgDispara) {
	...
	var url      = (inpessoa == 1) ? 'processaCPF' : 'processaCNPJ';
	...
	$.ajax({
		type: "POST",
		url: '/includes/consulta_rfb/' + url + ".php",
		dataType: "script",
		data: {
			dtnasctl: dtnasctl,
			nrcpfcgc: nrcpfcgc,
 ...
 function trata_resposta(response , flgDispara) {
	...
	if (objeto[0] == "erro") {
		...
		if (flgDispara == false) { 
			showError("error",objeto[1],"Alerta - Ayllos", "blockBackground(parseInt($('#divRotina').css('z-index')));carrregarCaptcha(false);");
		}
