...
&ANALYZE-RESUME
&Scoped-define WINDOW-NAME w_mensagem
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DEFINITIONS w_mensagem 
/* ..............................................................................
...
&Scoped-define FRAME-NAME f_mensagem
...
DEFINE VAR w_mensagem AS WIDGET-HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE VARIABLE ed_dsmensag1 AS CHARACTER FORMAT "X(30)":U 
      VIEW-AS TEXT 
     SIZE 67 BY .95
...
DEFINE VARIABLE ed_dsmensag3 AS CHARACTER FORMAT "X(30)":U 
      VIEW-AS TEXT 
     SIZE 67 BY .95
     FGCOLOR 12 FONT 11 NO-UNDO.
...
DEFINE VARIABLE ed_dsmensag5 AS CHARACTER FORMAT "X(30)":U 
      VIEW-AS TEXT 
     SIZE 67 BY .95
     FGCOLOR 12 FONT 11 NO-UNDO.

DEFINE VARIABLE ed_tlmensag AS CHARACTER FORMAT "X(20)":U 
      VIEW-AS TEXT 
     SIZE 67 BY 3.1
...
DEFINE FRAME f_mensagem
     ...
     ed_dsmensag4 AT ROW 8.38 COL 1 COLON-ALIGNED NO-LABEL WIDGET-ID 22 NO-TAB-STOP 
     ed_dsmensag5 AT ROW 9.57 COL 1 COLON-ALIGNED NO-LABEL WIDGET-ID 24 NO-TAB-STOP 
     RECT-2 AT ROW 1 COL 1 WIDGET-ID 8
     RECT-3 AT ROW 1.24 COL 2 WIDGET-ID 10
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 45.2 ROW 10.29
         SIZE 71.2 BY 10.05 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w_mensagem ASSIGN
         HIDDEN             = YES
         TITLE              = ""
         HEIGHT             = 28.57
         WIDTH              = 160
...
&ANALYZE-RESUME
ASSIGN w_mensagem = CURRENT-WINDOW.
...
/* SETTINGS FOR WINDOW w_mensagem
  VISIBLE,,RUN-PERSISTENT                                               */
/* SETTINGS FOR FRAME f_mensagem
   FRAME-NAME                                                           */
ASSIGN 
       ed_dsmensag1:READ-ONLY IN FRAME f_mensagem        = TRUE.

ASSIGN 
       ed_dsmensag2:READ-ONLY IN FRAME f_mensagem        = TRUE.

ASSIGN 
       ed_dsmensag3:READ-ONLY IN FRAME f_mensagem        = TRUE.

ASSIGN 
       ed_dsmensag4:READ-ONLY IN FRAME f_mensagem        = TRUE.

ASSIGN 
       ed_dsmensag5:READ-ONLY IN FRAME f_mensagem        = TRUE.
...
&Scoped-define SELF-NAME w_mensagem
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w_mensagem w_mensagem
ON END-ERROR OF w_mensagem
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _MAIN-BLOCK w_mensagem 
...
ASSIGN CURRENT-WINDOW                = {&WINDOW-NAME} 
       ...
       h_mensagem                    = FRAME f_mensagem:HANDLE.
...
IF  par_dsmensag1 <> "" AND LENGTH(par_dsmensag1) > 30  THEN
    RUN quebra_msg (INPUT par_dsmensag1).
ELSE
IF  par_dsmensag2 <> "" AND LENGTH(par_dsmensag2) > 30  THEN
    RUN quebra_msg (INPUT par_dsmensag2).
ELSE
IF  par_dsmensag3 <> "" AND LENGTH(par_dsmensag3) > 30  THEN
    RUN quebra_msg (INPUT par_dsmensag3).
ELSE
IF  par_dsmensag4 <> "" AND LENGTH(par_dsmensag4) > 30  THEN
    RUN quebra_msg (INPUT par_dsmensag4).
ELSE
IF  par_dsmensag5 <> "" AND LENGTH(par_dsmensag5) > 30  THEN
...
ASSIGN par_dsmensag1 = FILL(" ",INT((30 - LENGTH(par_dsmensag1)) / 2)) + par_dsmensag1
       par_dsmensag2 = FILL(" ",INT((30 - LENGTH(par_dsmensag2)) / 2)) + par_dsmensag2
       par_dsmensag3 = FILL(" ",INT((30 - LENGTH(par_dsmensag3)) / 2)) + par_dsmensag3
       par_dsmensag4 = FILL(" ",INT((30 - LENGTH(par_dsmensag4)) / 2)) + par_dsmensag4
       par_dsmensag5 = FILL(" ",INT((30 - LENGTH(par_dsmensag5)) / 2)) + par_dsmensag5.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE disable_UI w_mensagem  _DEFAULT-DISABLE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enable_UI w_mensagem  _DEFAULT-ENABLE
PROCEDURE enable_UI :
/*------------------------------------------------------------------------------
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE quebra_msg w_mensagem 
...
DO  WHILE TRUE:
    ...
    ASSIGN /* comeca do caractere 1 */
           tmp_indice = 1
           /* pega os 30 caracteres */
           tmp_msg    = SUBSTRING(par_msg,tmp_indice,30)
