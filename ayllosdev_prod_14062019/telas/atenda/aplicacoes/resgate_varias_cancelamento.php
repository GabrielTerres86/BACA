<?php 
	...
	/************************************************************************
	 Fonte: resgate_varias_cancelamento.php                           
	 Autor: Jorge                                                     
	 Data : Agosto/2011                 Ultima Alteracao: 03/03/2015
	                                                                  
	 Objetivo  : Mostrar opcao CANCELAMENTO da rotina de Aplicacoes   
			      da tela ATENDA em RESGATAR VARIAS                   
	                                                                   
	 Alteracoes: 21/09/2011 - Adicionado checkbox a coluna, chamando  
	                          a funcao marcaDesmarcaTodos().          
	                          (Fabricio)      

				 30/04/2014 - Ajuste referente ao projeto Captação:
							  - Layout dos botões (Adriano). 		
							  
				 03/03/2015 - Chamada de procedure via OCI e inclusao do campo
							  e variavel "IDTIPAPL" (Jean Michel).
...
?>
<script type="text/javascript">var metodoBlock = "blockBackground(parseInt($('#divRotina').css('z-index')))"; var lstResgates = new Array(); var lstResgatesChecked = new Array();</script>
	...
	<table>
		<thead>
			<tr>
				...
				<th><input type="checkbox" name="marcaTodos" id="marcaTodos" onClick="marcaDesmarcaTodos(<?php echo $qtResgates; ?>);"></th>
				<th onclick="selLinhaCanVarias();" title="Aplica&ccedil;&atilde;o"><?php echo utf8ToHtml('Aplica&ccedil;&atilde;o'); ?></th>
				<th onclick="selLinhaCanVarias();" title="Data de Resgate"><?php echo utf8ToHtml('Dt.Resg.'); ?></th>
				<th onclick="selLinhaCanVarias();" title="Documento"><?php echo utf8ToHtml('Docmto');  ?></th>
				<th onclick="selLinhaCanVarias();" title="Tipo de Resgate"><?php echo utf8ToHtml('Tp.Resg.');  ?></th>
				<th onclick="selLinhaCanVarias();" title="Situa&ccedil;&atilde;o"><?php echo utf8ToHtml('Situa&ccedil;&atilde;o');  ?></th>
				<th onclick="selLinhaCanVarias();" title="Valor"><?php echo utf8ToHtml('Valor');  ?></th>
			</tr>
		</thead>
		<tbody>
			<?php 
			...
			<tr id="trResgateVarias<?php echo $i + 1; ?>" onclick="viewComplementoResgVarias('<?php echo $i; ?>','C');">
				<td style="display:none;"><span><?php echo dataParaTimestamp($dtaplica); ?></span>
					<?php echo $dtaplica; ?>
				</td>
				<td><input type="checkbox" name="appresg<?php echo $i;?>" id="appresg<?php echo $i;?>" onchange="selecionaResgateVarias('<?php echo $i;?>','<?php echo $nraplica;?>','<?php echo $dtresgat; ?>','<?php echo $nrdocmto; ?>','<?php echo $idtipapl; ?>');" ></td>
				<td><span><?php echo $nraplica; ?></span>
					<?php echo formataNumericos("zzz.zzz.zzz",$nraplica,"."); ?>
				</td>
				<td><span><?php echo $dtresgat; ?></span>
					<?php echo $dtresgat; ?>
				</td>
