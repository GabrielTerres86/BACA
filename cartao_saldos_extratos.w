...
ON ANY-KEY OF Btn_A IN FRAME f_saldos_extratos /* SALDOS */
DO:
    RUN tecla.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_A w_saldos_extratos
ON CHOOSE OF Btn_A IN FRAME f_saldos_extratos /* SALDOS */
DO:
    RUN cartao_saldos.w.
END.
...
ON ANY-KEY OF Btn_B IN FRAME f_saldos_extratos /* EXTRATOS */
DO:
    RUN tecla.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_B w_saldos_extratos
ON CHOOSE OF Btn_B IN FRAME f_saldos_extratos /* EXTRATOS */
DO:
    RUN cartao_extrato.w.
END.
...
OCXFile = SEARCH( "cartao_saldos_extratos.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "cartao_saldos_extratos.wrx":U SKIP(1)
...
chtemporizador:t_saldos_extratos:INTERVAL = 0.
    ...
    IF  KEY-FUNCTION(LASTKEY) = "E"                AND
        Btn_A:SENSITIVE IN FRAME f_saldos_extratos THEN
        APPLY "CHOOSE" TO Btn_A. /* Botão Saldo */
    ELSE
    IF  KEY-FUNCTION(LASTKEY) = "F"                AND
        Btn_B:SENSITIVE IN FRAME f_saldos_extratos THEN
        APPLY "CHOOSE" TO Btn_B. /* Botão Extrato */
    ELSE
