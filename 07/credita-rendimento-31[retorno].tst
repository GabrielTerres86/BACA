...
declare 
  ...
  vr_dstitulo        VARCHAR2(500)  := 'estorno-rendimento-3112';
  ...
  vr_cdhistor        craphis.cdhistor%TYPE := 360;
...
begin  
  ...
  WHILE vr_idx_tab IS NOT NULL LOOP
    ...
    IF nvl(vr_cdcritic, 0) > 0 OR vr_dscritic IS NOT NULL THEN
       /* Se for uma critica de negocio, logar e continuar 
          pode ser, por exemplo, sem saldo em conta corrente */
       IF vr_incrineg = 1 THEN
         falha('Cooper: '||vr_tab_lanctos(vr_idx_tab).cdcooper||' Conta: '||vr_tab_lanctos(vr_idx_tab).nrdconta||' Valor: '|| formata(vr_tab_lanctos(vr_idx_tab).vllanmto)||' Critica: '||vr_dscritic);
         vr_cdcritic := 0;
         vr_dscritic := null;
