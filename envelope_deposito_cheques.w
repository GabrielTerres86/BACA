...
Procedure: envelope_deposito_cheques.w
Objetivo : Tela para depositos de cheques
Autor    : Evandro
Data     : Janeiro 2010
...
&Scoped-define FRAME-NAME f_envelope_deposito_cheques

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-127 RECT-128 RECT-142 IMAGE-37 IMAGE-40 ~
IMAGE-46 ed_nrctafav ed_nmtitula ed_vlcheque Btn_G Btn_D Btn_H ed_agctltfn ~
ed_nmcoptfn 
&Scoped-Define DISPLAYED-OBJECTS ed_nrctafav ed_nmtitula ed_vlcheque ~
...
DEFINE VARIABLE ed_vlcheque AS DECIMAL FORMAT "zz,zz9.99":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 41 BY 2.14
     FONT 13 NO-UNDO.
...
DEFINE FRAME f_envelope_deposito_cheques
     ...
     ed_vlcheque AT ROW 17.19 COL 50 COLON-ALIGNED NO-LABEL WIDGET-ID 106
     Btn_G AT ROW 19.1 COL 102.4 WIDGET-ID 86
     Btn_D AT ROW 24.1 COL 6 WIDGET-ID 66
     Btn_H AT ROW 24.1 COL 102.4 WIDGET-ID 74
     ed_agctltfn AT ROW 8.86 COL 50.4 COLON-ALIGNED NO-LABEL WIDGET-ID 228 NO-TAB-STOP 
     ed_nmcoptfn AT ROW 8.86 COL 76.4 COLON-ALIGNED NO-LABEL WIDGET-ID 230 NO-TAB-STOP 
     ...
     RECT-99 AT ROW 5.52 COL 19.6 WIDGET-ID 232
     ...
     IMAGE-46 AT ROW 19.24 COL 156 WIDGET-ID 158
...
/* SETTINGS FOR FRAME f_envelope_deposito_cheques
   FRAME-NAME                                                           */
ASSIGN 
       ed_nmtitula:READ-ONLY IN FRAME f_envelope_deposito_cheques        = TRUE.

/* SETTINGS FOR RECTANGLE RECT-100 IN FRAME f_envelope_deposito_cheques
   NO-ENABLE                                                            */
/* SETTINGS FOR RECTANGLE RECT-98 IN FRAME f_envelope_deposito_cheques
   NO-ENABLE                                                            */
/* SETTINGS FOR RECTANGLE RECT-99 IN FRAME f_envelope_deposito_cheques
   NO-ENABLE                                                            */
...
CREATE CONTROL-FRAME temporizador ASSIGN
       FRAME           = FRAME f_envelope_deposito_cheques:HANDLE
       ROW             = 1.71
       COLUMN          = 6
       HEIGHT          = 1.67
       WIDTH           = 7
       ...
       HIDDEN          = yes
       SENSITIVE       = yes.
/* temporizador OCXINFO:CREATE-CONTROL from: {F0B88A90-F5DA-11CF-B545-0020AF6ED35A} type: t_envelope_deposito_cheques */
...
ON ANY-KEY OF Btn_D IN FRAME f_envelope_deposito_cheques /* CONFIRMAR */
DO:
    RUN tecla.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_D w_envelope_deposito
ON CHOOSE OF Btn_D IN FRAME f_envelope_deposito_cheques /* CONFIRMAR */
DO:
    ...
    RUN procedures/grava_log.p (INPUT "Depósito em cheques identificado para C/C: " +
                                      STRING(ed_nrctafav,"zzzz,zzz,9") + " - " + ed_nmtitula + " - " +
                                      "Valor: " + STRING(ed_vlcheque,"zz,zz9.99") + "...").

    RUN envelope_entrega.w ( INPUT 8,            /* Deposito em C/C */
                             INPUT ed_nrctafav,  /* Conta Destino */
                             INPUT 0,            /* Dinheiro */
                             INPUT ed_vlcheque,  /* Cheque */
...
ON ANY-KEY OF Btn_G IN FRAME f_envelope_deposito_cheques /* CORRIGIR */
DO:
    RUN tecla.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_G w_envelope_deposito
ON CHOOSE OF Btn_G IN FRAME f_envelope_deposito_cheques /* CORRIGIR */
DO:
    ASSIGN ed_nrctafav:SCREEN-VALUE = ""
           ed_nmtitula:SCREEN-VALUE = ""
           ed_vlcheque:SCREEN-VALUE = "".
...
ON ANY-KEY OF Btn_H IN FRAME f_envelope_deposito_cheques /* VOLTAR */
DO:
    RUN tecla.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_H w_envelope_deposito
ON CHOOSE OF Btn_H IN FRAME f_envelope_deposito_cheques /* VOLTAR */
DO:
    ASSIGN par_flgderro = NO
           par_vlretorn = "NOK".
    APPLY "WINDOW-CLOSE" TO CURRENT-WINDOW.  
...
ON ANY-KEY OF ed_nrctafav IN FRAME f_envelope_deposito_cheques
DO:
...
&Scoped-define SELF-NAME ed_vlcheque
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ed_vlcheque w_envelope_deposito
ON ANY-KEY OF ed_vlcheque IN FRAME f_envelope_deposito_cheques
DO:
    RUN tecla.
    ...
    /* se nao digitou numeros, despreza se nao cancelou */
    ...
    ELSE
        DO:
            IF  LENGTH(ed_vlcheque:SCREEN-VALUE) < 9  THEN
                /* controle para permitir centavos */
                ed_vlcheque:SCREEN-VALUE = STRING(DECIMAL(ed_vlcheque:SCREEN-VALUE +
...
PROCEDURE temporizador.t_envelope_deposito_cheques.Tick .
APPLY "CHOOSE" TO Btn_H IN FRAME f_envelope_deposito_cheques.
END PROCEDURE.
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    FRAME f_envelope_deposito_cheques:LOAD-MOUSE-POINTER("blank.cur").

    ASSIGN chtemporizador:t_envelope_deposito_cheques:INTERVAL = glb_nrtempor
    ...
    DISPLAY ed_nmcoptfn
            ed_agctltfn WITH FRAME f_envelope_deposito_cheques.
...
OCXFile = SEARCH( "envelope_deposito_cheques.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "envelope_deposito_cheques.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  /* Hide all frames. */
  HIDE FRAME f_envelope_deposito_cheques.
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY ed_nrctafav ed_nmtitula ed_vlcheque ed_agctltfn ed_nmcoptfn 
      WITH FRAME f_envelope_deposito_cheques.
  ENABLE RECT-127 RECT-128 RECT-142 IMAGE-37 IMAGE-40 IMAGE-46 ed_nrctafav 
         ed_nmtitula ed_vlcheque Btn_G Btn_D Btn_H ed_agctltfn ed_nmcoptfn 
      WITH FRAME f_envelope_deposito_cheques.
  {&OPEN-BROWSERS-IN-QUERY-f_envelope_deposito_cheques}
       ...
       tmp_tximpres = tmp_tximpres +
                      ...
                      "COOPERATIVA/PA/TERMINAL: " + STRING(glb_agctltfn,"9999") + "/" + 
                                                    ...
                                                    STRING(glb_nrterfin,"9999") +
                                                             "         " +
                      "                                                "   +
                      "       COMPROVANTE DE DEPOSITO EM CHEQUE        " +
                      "                                                " +
                      "   DOCUMENTO: " +
...
PROCEDURE tecla :
chtemporizador:t_envelope_deposito_cheques:INTERVAL = 0.

    IF  KEY-FUNCTION(LASTKEY) = "D"                           AND
        Btn_D:SENSITIVE IN FRAME f_envelope_deposito_cheques  THEN
        APPLY "CHOOSE" TO Btn_D.
    ELSE
    IF  KEY-FUNCTION(LASTKEY) = "G"                           AND
        Btn_G:SENSITIVE IN FRAME f_envelope_deposito_cheques  THEN
        APPLY "CHOOSE" TO Btn_G.
    ELSE
    IF  KEY-FUNCTION(LASTKEY) = "H"                           AND
        Btn_H:SENSITIVE IN FRAME f_envelope_deposito_cheques  THEN
        APPLY "CHOOSE" TO Btn_H.
    ELSE
        RETURN NO-APPLY.

    chtemporizador:t_envelope_deposito_cheques:INTERVAL = glb_nrtempor.
