CREATE OR REPLACE TRIGGER CECRED.TRG_TBCADAST_PESSOA_RELAC_HST
  AFTER INSERT OR UPDATE OR DELETE ON TBCADAST_PESSOA_RELACAO
  FOR EACH ROW
  /* ..........................................................................

     Programa : TRG_TBCADAST_PESSOA_RELAC_HST
...
DECLARE
  ...
  --> Retornar descrição do tipo de relacao
  FUNCTION fn_tprelacao(pr_tprelacao tbcadast_pessoa_relacao.tprelacao%type) RETURN VARCHAR2 IS
  BEGIN

    IF pr_tprelacao IS NULL THEN
      RETURN NULL;
    END IF;
    
    RETURN pr_tprelacao ||'-'||
           CADA0014.fn_desc_tprelacao (pr_tprelacao   => pr_tprelacao,
...
BEGIN
  ...
  **************************************/
  ...
  IF INSERTING OR
    ...
    --> NRSEQ_RELACAO
    IF nvl(:new.NRSEQ_RELACAO,0) <> nvl(:OLD.NRSEQ_RELACAO,0) THEN
      Insere_Historico(pr_nmdcampo => 'NRSEQ_RELACAO',
                       pr_dsvalant => (:old.NRSEQ_RELACAO),
                       pr_dsvalnov => (:new.NRSEQ_RELACAO),
                       pr_dsvalor_novo_original => :new.NRSEQ_RELACAO
                      );
    END IF;
    
    --> IDPESSOA_RELACAO
    IF nvl(:new.IDPESSOA_RELACAO,0) <> nvl(:OLD.IDPESSOA_RELACAO,0) THEN
      Insere_Historico(pr_nmdcampo => 'IDPESSOA_RELACAO',
                       pr_dsvalant => :old.IDPESSOA_RELACAO,
                       pr_dsvalnov => :new.IDPESSOA_RELACAO,
                       pr_dsvalor_novo_original => :new.IDPESSOA_RELACAO
                      );
    END IF;

    --> TPRELACAO
    IF nvl(:new.TPRELACAO,0) <> nvl(:OLD.TPRELACAO,0) THEN
      Insere_Historico(pr_nmdcampo => 'TPRELACAO',
                       pr_dsvalant => fn_tprelacao(:old.TPRELACAO),
                       pr_dsvalnov => fn_tprelacao(:new.TPRELACAO),
                       pr_dsvalor_novo_original => :new.TPRELACAO
                      );
    END IF;
  ...
  EXCEPTION
    WHEN vr_exc_erro THEN
      raise_application_error(-20100,vr_dscritic);
    WHEN OTHERS THEN
      raise_application_error(-20100,'Erro TRG_TBCADAST_PESSOA_RELAC_HST: '||SQLERRM);
END TRG_TBCADAST_PESSOA_RELAC_HST;
/
