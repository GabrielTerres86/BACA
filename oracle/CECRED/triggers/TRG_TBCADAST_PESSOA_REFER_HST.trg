CREATE OR REPLACE TRIGGER CECRED.TRG_TBCADAST_PESSOA_REFER_HST
  AFTER INSERT OR UPDATE OR DELETE ON TBCADAST_PESSOA_REFERENCIA

  FOR EACH ROW
  /* ..........................................................................

     Programa : TRG_TBCADAST_PESSOA_REFER_HST
...
BEGIN
  ...
  IF INSERTING THEN
    ...
    vr_nrsequen := :new.NRSEQ_REFERENCIA;    
  ELSIF UPDATING THEN
  ...
  ELSIF DELETING THEN
    ...
    vr_nrsequen := :old.NRSEQ_REFERENCIA;
   ...
   ****** TBCADAST_CAMPO_HISTORICO ******
            204	IDPESSOA
            205	NRSEQ_REFERENCIA
            206	IDPESSOA_REFERENCIA
            207	CDOPERAD_ALTERA
  ...
  **************************************/
  ...
  IF INSERTING OR
    ...
    --> NRSEQ_REFERENCIA
    IF nvl(:new.NRSEQ_REFERENCIA,0) <> nvl(:OLD.NRSEQ_REFERENCIA,0) THEN
      Insere_Historico(pr_nmdcampo => 'NRSEQ_REFERENCIA',
                       pr_dsvalant => (:old.NRSEQ_REFERENCIA),
                       pr_dsvalnov => (:new.NRSEQ_REFERENCIA ),
                       pr_dsvalor_novo_original => :new.NRSEQ_REFERENCIA
                      );
    END IF;
    
    --> IDPESSOA_REFERENCIA
    IF nvl(:new.IDPESSOA_REFERENCIA,0) <> nvl(:OLD.IDPESSOA_REFERENCIA,0) THEN
      Insere_Historico(pr_nmdcampo => 'IDPESSOA_REFERENCIA',
                       pr_dsvalant => :old.IDPESSOA_REFERENCIA,
                       pr_dsvalnov => :new.IDPESSOA_REFERENCIA,
                       pr_dsvalor_novo_original => :new.IDPESSOA_REFERENCIA
                      );
    END IF;        
  ...
  EXCEPTION
    WHEN vr_exc_erro THEN
      raise_application_error(-20100,vr_dscritic);
    WHEN OTHERS THEN
      raise_application_error(-20100,'Erro TRG_TBCADAST_PESSOA_REFER_HST: '||SQLERRM);

END TRG_TBCADAST_PESSOA_REFER_HST;
/
