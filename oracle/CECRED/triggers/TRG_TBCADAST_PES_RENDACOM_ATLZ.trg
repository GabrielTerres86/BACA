CREATE OR REPLACE TRIGGER CECRED.TRG_TBCADAST_PES_RENDACOM_ATLZ
...
DECLARE
  ...
  vr_rendacompl_old   tbcadast_pessoa_rendacompl%ROWTYPE;
  vr_rendacompl_new   tbcadast_pessoa_rendacompl%ROWTYPE;
...
BEGIN
  ...
  vr_rendacompl_old.idpessoa         := :old.idpessoa;       
  vr_rendacompl_old.nrseq_renda      := :old.nrseq_renda;    
  vr_rendacompl_old.tprenda          := :old.tprenda;        
  vr_rendacompl_old.vlrenda          := :old.vlrenda;        
  vr_rendacompl_old.tpfixo_variavel  := :old.tpfixo_variavel; 
  vr_rendacompl_old.cdoperad_altera  := :old.cdoperad_altera;


  --> Armazenar dados novos
  vr_rendacompl_new.idpessoa        := :new.idpessoa;       
  vr_rendacompl_new.nrseq_renda     := :new.nrseq_renda;    
  vr_rendacompl_new.tprenda         := :new.tprenda;        
  vr_rendacompl_new.vlrenda         := :new.vlrenda;        
  vr_rendacompl_new.tpfixo_variavel := :new.tpfixo_variavel; 
  vr_rendacompl_new.cdoperad_altera := :new.cdoperad_altera;
    
  
  CADA0016.pc_atualiza_rendacompl ( pr_idpessoa         => vr_idpessoa        --> Identificador de pessoa                             
                                   ,pr_tpoperacao       => vr_tpoperac        --> Indicador de operacao (1-Inclusao, 2-Alteração, 3-Exclusão)
                                   ,pr_rendacompl_old   => vr_rendacompl_old  --> Dados anteriores
                                   ,pr_rendacompl_new   => vr_rendacompl_new  --> Dados novos
...
END TRG_TBCADAST_PES_RENDACOM_ATLZ;     
/
