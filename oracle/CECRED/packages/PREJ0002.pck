          ...
          FOR r_craplem in c_craplem(pr_cdcooper => pr_cdcooper
            ...
            IF r_craplem.dtmvtolt = rw_crapdat.dtmvtolt THEN
              ...
              BEGIN
                ...
                and   t.cdhistor = decode(pr_idtipo, 'PP',2388,'TR',2388,'CC',2388,2391) --Recuperacao de prejuizo
            ...
            FOR rw_crapepr IN c_crapepr(pr_cdcooper, pr_nrdconta, pr_nrctremp) LOOP
              ...
              --vr_vlsdprej := r_craplem.vllanmto - rw_crapepr.vlsdprej; --rw_crapepr.vlsdprej;
              --
              -- Finalmente após todo o processamento, é atualizada a tabela de empréstimo CRAPEPR
              
              -- JMC recalculo de multa e juros
             /* if  nvl(rw_crapepr.vlttmupr,0) > 0 
              and nvl(rw_crapepr.vlttmupr,0) > 0 then 
                  if (rw_crapepr.vlprejuz + 
                     rw_crapepr.vlttmupr +
                     rw_crapepr.vlttjmpr - 
                     rw_crapepr.vlsdprej) <= r_craplem.vllanmto then
              ...
              IF vr_vlsdprej > 0 THEN
                 ...
                 --jean -- comentada esta regra
                 \*IF nvl(rw_crapepr.vlttmupr,0) > 0 OR nvl(rw_crapepr.vlttjmpr,0) > 0 THEN
                 ...
                 --vr_vlttmupr := 0;*\
              ELSE
                 rw_crapepr.vlsdprej := rw_crapepr.vlprejuz;
                 -- Jean --- comentado esta regra
                 \* vr_vlttmupr := 0;
                  vr_vlttjmpr := 0;
                 if nvl(rw_crapepr.vlttmupr,0) > 0 
