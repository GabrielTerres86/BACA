...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_CONJOB AS
  ...
  BEGIN
    ...
    IF pr_idativo NOT IN ('S','N') THEN
      vr_dscritic := 'Campo Ativo inválido!';
      RAISE vr_exc_erro;
    END IF;

    IF pr_idperiodici_execucao NOT IN ('R','D','S','Q','M','T','E','A') THEN
      vr_dscritic := 'Campo Periodicidade Execução inválido!';
      RAISE vr_exc_erro;
    END IF;

    IF NVL(pr_tpintervalo,' ') NOT IN (' ','H','M') THEN
      vr_dscritic := 'Campo Intervalo inválido!';
      RAISE vr_exc_erro;
    END IF;

    IF pr_tpintervalo = 'H' AND pr_qtintervalo NOT BETWEEN 1 AND 23 THEN
      vr_dscritic := 'Campo Intervalo inválido! Informe quantidade de horas entre 01 e 23';
      RAISE vr_exc_erro;
    ELSIF pr_tpintervalo = 'M' AND pr_qtintervalo NOT BETWEEN 1 AND 59 THEN
      vr_dscritic := 'Campo Intervalo inválido! Informe quantidade de minutos entre 01 e 59';
      RAISE vr_exc_erro;
    END IF;

    IF pr_dsdias_habilitados = '0000000' OR length(pr_dsdias_habilitados) < 7 THEN
      vr_dscritic := 'Campo Dias Habilitados inválido! Selecione pelo menos um dia para Execuçao';
      RAISE vr_exc_erro;
    END IF;

    IF pr_flexecuta_feriado NOT IN (0,1) THEN
      vr_dscritic := 'Campo Executar em Feriados inválido!';
      RAISE vr_exc_erro;
    END IF;

    IF pr_flsaida_email = 'S' AND pr_dsdestino_email IS NULL THEN
      vr_dscritic := 'Campo Destino inválido! Informe destino de Emails para a Saída por Emails';
      RAISE vr_exc_erro;
    ELSIF pr_flsaida_log = 'S' AND pr_dsnome_arq_log IS NULL THEN
      vr_dscritic := 'Campo Nome Arq inválido! Informe Nome do Arquivo para a Saída por LOG';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF pr_idativo = 'S' THEN
      ...
      IF cr_outro_job%found THEN
        CLOSE cr_outro_job;
        vr_dscritic := 'Prefixo ou Nome já utilizados no JOB id '||rw_outro_job.nmjob;
        RAISE vr_Exc_erro;
      ELSE
        CLOSE cr_outro_job;
      END IF;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        vr_dscritic := 'Data e Hora da Próxima Execução Inválidas!';
        RAISE vr_exc_erro;
    END;

    -- Data deve estar no futuro
    IF vr_dtprox_execucao < sysdate THEN
      vr_dscritic := 'Data e Hora da Próxima Execução Inválidas! Favor informar data futura';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF pr_flexecuta_feriado = 0 AND gene0005.fn_valida_dia_util(pr_cdcooper  => 3
      ...
      vr_dscritic := 'Data e Hora da Próxima Execução Inválidas! Favor informar data útil ou selecione para executar em feriados';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF substr(pr_dsdias_habilitados,to_char(vr_dtprox_execucao,'d'),1) <> '1' THEN
      vr_dscritic := 'Data da Próxima Execução Inválida! Você não selecionou uma data '
    ...
    exception
      when others then
        vr_dscritic := 'Código PLSQL Inválido, favor corrigir : '||substr(SQLERRM,1,instr(SQLERRM,':',1,2));
        RAISE vr_exc_erro;
    end;
    ...
    IF pr_tpmantem = 'A' THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          vr_dscritic := 'Erro na atualiação das informações do JOB: '||sqlerrm;
          RAISE vr_exc_erro;
      END;
