...
CREATE OR REPLACE PACKAGE BODY CECRED.SEGU0002 AS
    ...
    BEGIN
      ...
      IF vr_tab_benef.count > 0 THEN    
        ...
        LOOP
          ...
          IF nvl(vr_tab_benef(vr_idx_benef+2).tag,' ') not in('C','F','P') THEN
            -- Retornar erro conforme:
            pc_monta_retorno_ws(402,'Campo cdGrauPar invalido. Informar [C]onjuge, [F]ilhos ou [P]ais.',pr_dsxmlret);
            RETURN;
          END IF;  
  ...
  BEGIN
    ...
    BEGIN
      ...
      --     </Beneficiarios>
      --   </Root> 
      gene0007.pc_itera_nodos('/Root/Beneficiarios/*',pr_array_benef,pr_list_nodos => vr_tab_benef, pr_des_erro => vr_dsmsgret);
      IF vr_dsmsgret IS NOT NULL THEN
        -- Montar retorno
        pc_monta_retorno_ws(402,'Array Beneficiarios invalido --> ' || vr_dsmsgret,pr_dsxmlret);
        RETURN;
      ...
      IF vr_tab_benef.count > 0 THEN    
        ...
        LOOP
          ...
          END IF;          
          -- Validar grau de parentesco
          IF vr_tab_benef(vr_idx_benef+2).tag not in('C','F','P') THEN
            -- Retornar erro conforme:
            pc_monta_retorno_ws(402,'Campo cdGrauPar invalido. Informar [C]onjuge, [F]ilhos ou [P]ais.',pr_dsxmlret);
            RETURN;
          END IF;  
