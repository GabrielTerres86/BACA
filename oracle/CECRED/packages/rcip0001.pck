...
CREATE OR REPLACE PACKAGE BODY CECRED.rcip0001 AS
  ...
  BEGIN    
    ...
    ELSE
      ...
      ELSE
        ...
        vr_calculo := ((pr_vlrbase-rw_parame.vlminimo) / (rw_parame.vlmaximo-rw_parame.vlminimo))*rw_parame.perscore;
  ...
  ---------------------------------------------------------------------------------------------------------------
    ...
    CURSOR cr_indica05 IS
      SELECT sum(epr.vlemprst)
        FROM crapepr epr
       WHERE epr.cdcooper = pr_cdcooper
         AND epr.nrdconta = pr_nrdconta
         AND epr.dtmvtolt BETWEEN vr_dat_ini
                              AND vr_dat_fim; 
    -- Empréstimos BNDES                          
    CURSOR cr_indica05_ebn IS   
      SELECT sum(ebn.vlropepr)
        FROM crapebn ebn
       WHERE ebn.cdcooper = pr_cdcooper
         AND ebn.nrdconta = pr_nrdconta
         AND ebn.dtinictr BETWEEN vr_dat_ini
                              AND vr_dat_fim
         -- DIferente de (C) - Cancelado
         AND ebn.insitctr <> 'C';
    ...
    -- Folha de Pagamento: Lote e Pesquisa
    CURSOR cr_lote13 IS
      SELECT TO_NUMBER(tab.dstextab)
        FROM craptab tab
            ,crapemp emp
       WHERE tab.tpregist        = emp.cdempres
         AND tab.cdcooper        = emp.cdcooper
         AND UPPER(tab.nmsistem) = 'CRED'
         AND UPPER(tab.tptabela) = 'GENERI'
         AND tab.cdempres        = 0
         AND UPPER(tab.cdacesso) = 'NUMLOTEFOL'
         AND emp.cdcooper        = pr_cdcooper
         AND emp.nrdconta        = pr_nrdconta;
    vr_numlote NUMBER;
    
    -- Pesquisa das folhas
    CURSOR cr_indica13(pr_numlote IN NUMBER) IS
      SELECT COUNT(1)
        FROM (SELECT lcm.nrdconta
               ...
               WHERE lcm.cdcooper = his.cdcooper
                 ...
                 AND lcm.nrdolote = pr_numlote+1 /* Folha Email */
                 AND lcm.dtmvtolt >= vr_dat_ini
                 AND lcm.dtmvtolt <= vr_dat_fim
                 AND his.indebcre = 'C'            
              UNION   
              SELECT lcm.nrdconta 
                FROM craplcm lcm
               ...
               WHERE lcm.cdcooper = his.cdcooper
                 AND lcm.cdhistor = his.cdhistor
                 AND lcm.cdcooper = pr_cdcooper
                 AND lcm.nrdolote = pr_numlote /* Folha IB*/
                 AND lcm.dtmvtolt >= vr_dat_ini
                 AND lcm.dtmvtolt <= vr_dat_fim
                 AND his.indebcre = 'C');
  ...
  BEGIN
    ...
    ELSIF pr_idindicador = 5 THEN
      ...
      CLOSE cr_indica05;
      -- Empréstimos BNDES
      OPEN cr_indica05_ebn;
      FETCH cr_indica05_ebn
       INTO vr_retorno_02;
      CLOSE cr_indica05_ebn;
    ...
    ELSIF pr_idindicador = 13 THEN
      -- Folha 
      --   Primeiramente devemos buscar o LOTE de folha da empresa:
      OPEN cr_lote13;
      FETCH cr_lote13
       INTO vr_numlote;
      CLOSE cr_lote13;
      -- Enfim, buscar os pagamentos
      OPEN cr_indica13(vr_numlote);
      FETCH cr_indica13
       INTO vr_retorno;
      CLOSE cr_indica13;  
