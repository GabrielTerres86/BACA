...
CREATE OR REPLACE PACKAGE BODY CECRED.limp0001 IS
  ...
  --
  --            03/06/2019 - Inclus„o do historico 2973 nas validaÁıes de Cheques devolvidos de deposito (Luis Fagundes/AMCOM)                         
  ---------------------------------------------------------------------------------------------------------------
  ...
  /* .............................................................................
   ...
   Alteracoes: 30/09/94 - Alterado para mostrar a alinea na descricao do histo-
               ...
               14/01/2015 - Adicionado ao log da crÌtica 347 tais informaÁıes como:
                            - Tipo de taxa;
                            - ⁄ltimo dia do mÍs;
                            - CÛdigo da linha de crÈdito;
                            - CÛdigo do histÛrico. (Kelvin - SD 242477)
                            
               07/04/2015 - Ajustado critica duplicada. (Daniel).
               
               07/04/2015 - Retirado acentuaÁ„o ao efetuar log de erro (Daniel)     
                        
               03/06/2019 - Incluido Historico 2973 nas validaÁıes de cheques devolvidos de deposito - Luis Fagundes/AMCOM  			   
  ...
  /* .............................................................................
   ...
   Programa: pc_crps046_i                             Antigo: Includes/crps046_1.i at√© crps046_7.i
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Deborah/Edson
  ...
  /* .............................................................................
   ...
   Programa: pc_crps157_i                             Antigo: Includes/crps157_1.i at√© crps157_9.i
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : Deborah/Edson
   ...
   Alteracoes: 03/11/94 - Alterado para criar a variavel com os historicos refe-
     ...
     BEGIN
       ...
       IF vr_caminho IS NULL THEN
         vr_cdcritic:= 0;
         vr_dscritic:= 'Diretorio padrao de microfilmagem n√£o encontrado!';
         --Levantar Excecao
         RAISE vr_exc_saida;
       END IF;
       ...
       --Buscar conta limite para execu√ß√£o nos parametros
...
/*       dbms_xslprocessor.clob2file(vr_des_xml, vr_caminho, vr_nmarquiv ,0);*/
  ...
  PROCEDURE pc_crps038_i( pr_cdcooper IN crapcop.cdcooper%TYPE   --> Cooperativa solicitada
                     ...
                     ,pr_flgfirst IN OUT BOOLEAN             --> indica se √© a primeira chamada da include
  ...
  BEGIN
    /* .............................................................................
       ...
       Alteracao : 21/03/95 - Alterado para ajustar o layout para mostrar os
                   ...
                   1/12/2010 - (001) Altera√ßao de format para x(50)
                               Leonardo Am√©rico (Kbase).
    ...
    DECLARE
      ...
      BEGIN
        --se a tabela n√£o possuir registros, inicializa com registro em branco
    ...
    BEGIN
      ...
      -- Se n√£o existir lancamentos de cotas de capital para o associado
      -- volta para o programa principal
      ...
      -- se n√£o encontrou cotas de capital, retorna o processamento ao programa chamador
      pr_regexist := TRUE;

      --verifica informacoes referentes a cotas e recursos do associado
      --se n√£o existir informacoes de cotas e recursos
      ...
      IF pr_flgfirst THEN
        -- muda para false para n√£o entrar mais nessa condicao nas proximas chamadas
        pr_flgfirst := FALSE;
      ...
      ELSE
        ...
        IF vr_contlinh > 77 THEN
          --inicaliza a variavel indicando totalizador de p√°gina
          vr_indsalto := '1';
      ...
      --seleciona os lan√ßamentos de cotas de capital do associado
      ...
      LOOP
        ...
        --verifica se o numero do contrato do plano √© maior que zero
        ...
        IF pr_tab_grpconta(vr_nrdconta).tabcraplct(vr_indcraplct).cdhistor IN (66) THEN
          ...
          vr_indebcre := '*';
        ELSE
          --sen√£o, busca o indicador de debito/credito na tabela de historicos
        ...
        IF vr_contlinh = 84 THEN
          ...
          IF pr_tab_grpconta(vr_nrdconta).tabcraplct(vr_indcraplct).sqnrdcta = pr_tab_grpconta(vr_nrdconta).tabcraplct(vr_indcraplct).qtnrdcta THEN
            --indica que eh totalizador de p√°gina
            vr_indsalto := '1';
            /* cabecalho 1   */
            pc_gera_cabecalho_1( pr_indsalto => vr_indsalto
                                ,pr_linha    => pr_cabmex01);
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        pr_cdcritic := 0;
        pr_dscritic := 'Erro n√£o tratado no procedimento pc_crps038_i. Conta: '||vr_nrdconta||'. '||SQLERRM;
    END;--BEGIN
  END pc_crps038_i;
  ...
  BEGIN
    DECLARE
      ...
      --tabela temporaria de planos de capitaliza√ß√£o
    ...
    BEGIN
      ...
      -- se o parametro √© diferente de zero, indica que o processo j√° foi executado
      ...
      -- carrega a tabela temporaria de planos de capitaliza√ß√£o
      ...
      LOOP
        ...
        pc_crps038_i( pr_cdcooper    => pr_cdcooper    --> Cooperativa solicitada
                     ...
                     ,pr_tab_grpconta => vr_tab_grpconta --> tabela com informacoes dos lan√ßamentos de cotas
                     ...
                     ,pr_flgfirst    => vr_flgfirst    --> indica se √© a primeira chamada da include
...
/*    DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_clob, vr_patcharq, vr_nmarquiv || vr_nmsufixo, 0);*/
      ...
      IF pr_flgtrans AND vr_regexist THEN
        ...
        -- limpando as vari√°veis de critica
        vr_cdcritic := 0;
        vr_dscritic := NULL;
      END IF;
