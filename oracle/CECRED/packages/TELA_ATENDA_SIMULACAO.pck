CREATE OR REPLACE PACKAGE CECRED.TELA_ATENDA_SIMULACAO IS
  ...
  PROCEDURE pc_valida_simul_consig(pr_nrdconta IN crapass.nrdconta%type --> Conta
...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_ATENDA_SIMULACAO IS
  ...
  --             14/05/2019 - Inclusão procedures pc_busca_dados_soa_fis_calcula
  --             prc_log_erro_soa_fis_calcula -CONSIGNADO-( Jackson Barcellos - AMcom)
  --
  ---------------------------------------------------------------------------
 ...
 PROCEDURE pc_valida_simul_consig(pr_nrdconta IN crapass.nrdconta%type --> Conta
  ...
  BEGIN
   ...
   WHEN vr_saida THEN
      ...
      -- Existe para satisfazer exigência da interface. 
      pr_retxml := xmltype.createxml('<?xml version="1.0" encoding="ISO-8859-1" ?> ' ||
                                     '<Root><dsmensag>' || vr_dscritic || '</dsmensag></Root>');    
   WHEN OTHERS THEN
      -- Retorno não OK
      pr_des_erro := 'NOK';      
      -- Erro
      pr_cdcritic := 0;
      pr_dscritic := 'Erro nao tratado na tela_atenda_simulacao.pc_valida_simul_consig '|| pr_cdlcremp||' : ' || SQLERRM;      
      -- Existe para satisfazer exigência da interface. 
      pr_retxml := xmltype.createxml('<?xml version="1.0" encoding="ISO-8859-1" ?> ' ||
                                     '<Root><Erro>' || pr_cdcritic || '-' ||
                                     pr_dscritic || '</Erro></Root>');  
