...
..............................................................................*/
   ...
   CURSOR C_CRAPRIS(pr_cdcooper in number
             ...
             SELECT  innivris, dtdrisco
               FROM  CRAPRIS
              WHERE crapris.cdcooper = pr_cdcooper     AND
                    crapris.nrdconta = pr_nrdconta     AND
                    crapris.nrctremp = pr_nrctremp     AND
                    crapris.dtrefere = RW_crapdat.dtultdma AND
                    crapris.cdorigem = 3                AND
                    crapris.inddocto = 1;

             R_CRAPRIS C_CRAPRIS%ROWTYPE;
   ...
   BEGIN
        ...
        IF nvl(vr_flgativo,0) = 1 THEN
         ...
         IF C_CRAPEPR%FOUND THEN
             ...
             ELSE
                 ...
                 vr_dtcalcul := rw_crapdat.dtultdma;
                 ...
                 for rw_crappep in c_crappep loop
                  ...
                  if vr_des_reto <> 'OK' then
                     vr_cdcritic := 0;
                     vr_dscritic := 'Erro ao Buscar Dados Parcelas.';
                     raise vr_exc_erro;
                  end if;
                 ...
                 if vr_des_reto <> 'OK' then
                    vr_cdcritic := 0;
                     vr_dscritic := 'Erro ao desativar Rating!';
                     pr_tab_erro(PR_TAB_ERRO.FIRST).dscritic := VR_DSCRITIC;
                     pr_des_reto := 'NOK';
                     raise vr_exc_erro;
                 end if;
                 ...
                 or vr_tab_calculado.count = 0        then
                   ...
                   if nvl(vr_vljura60,0) > 0 then
                    ...
                    BEGIN
                       UPDATE CRAPEPR
                       SET  crapepr.vlprejuz  = vr_tab_calculado(1).vlsderel + nvl(vr_vljura60,0)
                          ,  crapepr.vlsdprej = vr_tab_calculado(1).vlsderel + nvl(vr_vljura60,0)
                    ...
                    BEGIN
                      UPDATE CRAPCYB C
                         SET C.DTPREJUZ = rw_crapdat.dtmvtolt,
                             C.VLSDPREJ = vr_tab_calculado(1).vlsderel + nvl(vr_vljura60,0),
                             C.FLGPREJU = 1
                    ...
                    IF r_craplcr.dsoperac = 'FINANCIAMENTO' THEN /* Financiamento */
                        vr_cdhistor1 := 2396;  /* FINANCIAMENTO PRE-FIXADO TRANSFERIDO PARA PREJUIZO */
                        vr_cdhistor2 := 2411;  /* MULTA MORA FINANC. PRE-FIXADO TRANSF. P/ PREJUIZO */
                        vr_cdhistor3 := 2397;  /* JUROS MORA FINANC. PRE-FIXADO TRANSF. P/ PREJUIZO */
                        vr_cdhistor4 := 2415;
                    ELSE /* Emprestimo */
                        vr_cdhistor1 := 2381;  /* EMPRESTIMO PRE-FIXADO TRANSFERIDO PARA PREJUIZO  */
                        vr_cdhistor2 := 2411;  /* MULTA MORA EMPREST. PRE-FIXADO TRANSF. P/ PREJUIZO */
                        vr_cdhistor3 := 2382;  /* JUROS MORA EMPREST. PRE-FIXADO TRANSF. P/ PREJUIZO */
                        vr_cdhistor4 := 2415;  /* JUROS MORA EMPREST. PRE-FIXADO TRANSF. P/ PREJUIZO */
                    end if;
                 ...
                 if vr_tab_calculado(1).vlsderel > 0 then
                    empr0001.pc_cria_lancamento_lem(pr_cdcooper => pr_cdcooper
                                                   ...
                                                   ,pr_nrctremp => pr_nrctremp
                                                   ,pr_vllanmto => vr_tab_calculado(1).vlsderel
         ...
         ELSE
             vr_cdcritic := 0;
             vr_dscritic := 'Erro ao gerar o emprestimo para prejuizo. ' || sqlerrm;
             pr_tab_erro(PR_TAB_ERRO.FIRST).dscritic := VR_DSCRITIC;
             pr_des_reto := 'NOK';
             raise vr_exc_erro;
         END IF;
    ...
    EXCEPTION
        WHEN vr_exc_erro then
             rollback;
             if pr_tab_erro.first is not null then
                pr_tab_erro(PR_TAB_ERRO.FIRST).dscritic := VR_DSCRITIC;
             end if;
             pr_des_reto := 'NOK';
             ...
             vr_dstransa := 'Erro ao transferir para prejuízo - Conta: ' || pr_nrdconta || 
                             ', Contrato: ' || pr_nrctremp;
        ...
        WHEN others then
             ...
             vr_dstransa := 'Erro geral rotina pc_transfere_epr_PP-Conta: ' || pr_nrdconta || ', Contrato: ' ||
                             pr_nrctremp || '-'|| sqlerrm;
      ...
      BEGIN
         ...
         for r_busca_boleto in c_busca_boleto loop
             ...
             if r_busca_boleto.incobran = 5 then -- boleto pago
                ...
                fetch c_busca_retorno_boleto into r_busca_retorno_boleto;
        ...
        close c_craplcr;
          ...
          IF r_craplcr.dsoperac = 'FINANCIAMENTO' THEN /* Financiamento */
              vr_cdhistor1 := 2401;  /* FINANCIAMENTO PRE-FIXADO TRANSFERIDO PARA PREJUIZO */
              vr_cdhistor2 := 2411;  /* MULTA MORA FINANC. PRE-FIXADO TRANSF. P/ PREJUIZO */
              vr_cdhistor3 := 2406;  /* JUROS MORA FINANC. PRE-FIXADO TRANSF. P/ PREJUIZO */
              vr_cdhistor4 := 2415;
          ELSE /* Emprestimo */
              vr_cdhistor1 := 2401;  /* EMPRESTIMO PRE-FIXADO TRANSFERIDO PARA PREJUIZO  */
              vr_cdhistor2 := 2411;  /* MULTA MORA EMPREST. PRE-FIXADO TRANSF. P/ PREJUIZO */
              vr_cdhistor3 := 2402;  /* JUROS MORA EMPREST. PRE-FIXADO TRANSF. P/ PREJUIZO */
              vr_cdhistor4 := 2415;
          end if;
        ...
        -- Busca próximo numero de lote a ser criado
          ...
          if    not c_busca_prx_lote%found or
                nvl(vr_nrdolote,0) = 0 then
        ...
        empr0001.pc_cria_lancamento_lem(pr_cdcooper => pr_cdcooper
         ...
         if pr_des_reto = 'OK' then 
            BEGIN
               UPDATE CRAPEPR
               SET  crapepr.vlprejuz  = vr_vlsdeved
                  ,  crapepr.vlsdprej = vr_vlsdeved
                  ,  crapepr.inprejuz = 1 /* Em prejuizo */
                  ,  crapepr.inliquid = 1 /* Liquidado   */
  ...
  cursor c_busca_risco_ass is
    select t.cdcooper, t.nrdconta, t.inrisctl, t.dtrisctl, t.dtdemiss, t.cdagenci
     ...
     where t.cdcooper = pr_cdcooper
       and t.nrdconta = pr_nrdconta; -- retirar após o teste
    ...
    BEGIN
      INSERT INTO CRAWEPR(NRDCONTA
                   ...
                   VALUES(PR_NRDCONTA --NRDCONTA
                         ...
                         , PR_VLSDDISP * -1 -- VLEMPRST
                         , 1 --QTPREEMP
                         , PR_VLSDDISP * -1 --VLPREEMP
                         , 100 --CDLCREMP
                         , 66 --CDFINEMP
                         , 0 -- QTDIALIB
    ...
    BEGIN
      INSERT INTO CRAPEPR (DTMVTOLT
           ...
           VALUES       ( RW_CRAPDAT.DTMVTOLT -- DTMVTOLT
                         ...
                         , 0 --VLJURMES
                         , 0 --VLPAGMES
                         , pr_VLSDDISP * -1 -- VERIFICAR VLPREEMP
                         , 0 --VLSDEVED
                         , pr_VLSDDISP * -1 --VLEMPRST
                         ...
                         , pr_VLSDDISP * -1 --VLPREJUZ
                         , pr_VLSDDISP * -1 --VLSDPREJ
                         , 1 --INPREJUZ
                         , 0 --VLJRAPRJ
                         , 0 --VLJRMPRJ
    ...
    BEGIN
     INSERT INTO CRAPPEP (CDCOOPER
           ...
           VALUES     (   pr_CDCOOPER --CDCOOPER
                        ...
                        , vr_nrctremp -- pr_NRDCONTA --NRCTREMP
                        , 1 --NRPAREPR
                        , pr_VLSDDISP * -1 -- VERIFICAR --VLPAREPR
                        , 0 --VLJURPAR
                        , 0 --VLMTAPAR
                        , 0 --VLMRAPAR
                        , 0 --VLMTZPAR
                        ...
                        , 1 --INLIQUID
                        , 0 --VLDESPAR
                        , pr_VLSDDISP * -1 -- VERIFICAR --VLSDVPAR
                        , 0 --VLJINPAR
                        , 0 --VLPAGJIN
                        , 0 --VLSDVATU
                        , 0 --VLJURA60
                        , pr_VLSDDISP * -1 --VLSDVSJI
                       --, PROGRESS_RECID
                        , 1 --INPREJUZ
                        , RW_CRAPDAT.DTMVTOLT); --DTREFATU)          
    EXCEPTION
        ...
        BEGIN
          INSERT INTO crapcyb (cdcooper,cdorigem,nrdconta,nrctremp,cdagenci,cdlcremp,
                       ...
                       values (r_crapcyb.cdcooper
                              ...
                              ,pr_VLSDDISP * -1    -- r_crapcyb.vlsdprej -- rmm
                              ,r_crapcyb.flgjudic
                              ,r_crapcyb.flextjud
                              ,r_crapcyb.flgehvip
  ...
  when others then
             ...
             ELSE
               ...
               if vr_des_reto <> 'OK' then
                ...
                for rw_craplem in cr_craplem2(vr_dtmvtolt) loop
                    ...
                    if  r_crapepr.vlprejuz > 0 
                    and rw_craplem.cdhistor in (1732, 1731) then
                         vr_auxvalor := r_crapepr.vlprejuz;
                    end if;
                    
                    if  r_crapepr.vlttmupr > 0 
                    and rw_craplem.cdhistor in (1734, 1733) then
                         vr_auxvalor := r_crapepr.vlttmupr;
                    end if;
                    
                    if  r_crapepr.vlttjmpr > 0 
                    and rw_craplem.cdhistor in (1736, 1735) then
                ...
                ELSE
                  ...
                  fetch c_craplcr into r_craplcr;
                   ...
                   IF r_craplcr.dsoperac = 'FINANCIAMENTO' THEN /* Financiamento */
                        vr_cdhistor1 := 2398;  /* FINANCIAMENTO PRE-FIXADO TRANSFERIDO PARA PREJUIZO */
                    ELSE /* Emprestimo */
                        vr_cdhistor1 := 2383;  /* EMPRESTIMO PRE-FIXADO TRANSFERIDO PARA PREJUIZO  */
                    end if;
         ...
         IF C_CRAPEPR%FOUND THEN
             ...
             ELSE
                ...
                for rw_craplem in cr_craplem2(vr_dtmvtolt) loop
                    ...
                    if  r_crapepr.vlprejuz > 0 
                    and rw_craplem.cdhistor in (349, 2401,2408,2411) then
                         vr_auxvalor := r_crapepr.vlprejuz;
                    end if;
                ...
                if r_crapepr.dtprejuz = rw_crapdat.dtmvtolt then
                    ...
                    /* Excluir lançamentos da CRAPLCM */
                    BEGIN
                       DELETE FROM CRAPLCM
                       where  craplcm.cdcooper = pr_cdcooper
                       and    craplcm.nrdconta = pr_nrdconta
                       and    craplcm.cdhistor = 2408
                   ...
                   IF r_craplcr.dsoperac = 'FINANCIAMENTO' THEN /* Financiamento */
                        vr_cdhistor1 := 2403;  /* FINANCIAMENTO PRE-FIXADO TRANSFERIDO PARA PREJUIZO */
                    ELSE /* Emprestimo */
                        vr_cdhistor1 := 2403;  /* EMPRESTIMO PRE-FIXADO TRANSFERIDO PARA PREJUIZO  */
                    end if;
   ...
   PROCEDURE pc_transfere_prejuizo_CC_web (pr_nrdconta   IN VARCHAR2  -- Conta corrente
     ...
     /* .............................................................................
      ...
      Objetivo  : Efetua a transferencia de contas correntes para prejuízo (força o envio)
      Observacao: Rotina chamada pela tela Atenda / Ocorrencias / Prejuizo, botão "Prejuízo"
   ...
   EXCEPTION
     WHEN vr_exc_erro THEN
       ...
       GENE0001.pc_gera_log(pr_cdcooper => vr_cdcooper
                           ...
                           ,pr_dscritic => NVL(pr_dscritic,' ')
                           ,pr_dsorigem => 'INTRANET'
                           ,pr_dstransa => 'PREJ0001-Transferencia forçada para prejuizo (CC).'
                           ,pr_dttransa => TRUNC(SYSDATE)
                           ,pr_flgtrans => 0 --> ERRO/FALSE
                           ,pr_hrtransa => TO_NUMBER(TO_CHAR(SYSDATE,'SSSSS'))
                           ,pr_idseqttl => 1
     ...
     WHEN OTHERS THEN
       ...
       GENE0001.pc_gera_log(pr_cdcooper => vr_cdcooper
                           ...
                           ,pr_dscritic => NVL(pr_dscritic,' ')
                           ,pr_dsorigem => vr_dsorigem
                           ,pr_dstransa => 'PREJ0001-Transferência Prejuízo (CC).'
                           ,pr_dttransa => TRUNC(SYSDATE)
                           ,pr_flgtrans => 0 --> ERRO/FALSE
                           ,pr_hrtransa => TO_NUMBER(TO_CHAR(SYSDATE,'SSSSS'))
                           ,pr_idseqttl => 1
     ...
     vr_dstransa := 'Consulta prejuizo: ' || pr_dtprejuz || ', conta: ' ||
      ...
      FOR rw_crapepr IN cr_crapepr(pr_cdcooper => vr_cdcooper,
         ...
         if rw_crapepr.tpemprst = 1 then
          ...
          gene0007.pc_insere_tag(pr_xml => pr_retxml, pr_tag_pai => 'inf', pr_posicao => vr_contador, pr_tag_nova => 'vlemprst', pr_tag_cont => rw_crapepr.vlprejuz, pr_des_erro => vr_dscritic);
     ...
     gene0004.pc_extrai_dados(pr_xml      => pr_retxml
      ...
      if vr_contador > 0 then 
         ...
         gene0003.pc_solicita_email(pr_cdcooper        => 3
                                ...
                                ,pr_des_destino     => 'fernanda.buettgen@cecred.coop.br;jean.calao@mouts.info;'
