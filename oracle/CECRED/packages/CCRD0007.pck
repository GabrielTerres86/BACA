CREATE OR REPLACE PACKAGE CECRED.CCRD0007 is
  ...
  --> Rotina responsável por criar o jason para receber o retorno do Bancoob
  PROCEDURE pc_gera_json_ret_bancoob (pr_cdcooper   IN crapcop.cdcooper%TYPE   --> Codigo da cooperativa
                                     ,pr_nrdconta   IN crawcrd.nrdconta%TYPE   --> Número da conta
                                     ,pr_nrctrcrd   IN crawcrd.nrctrcrd%TYPE   --> Número da proposta do cartão
                                     ---- OUT ----
                                     ,pr_dsjsonan  OUT NOCOPY json             --> Retorno do clob em modelo json com os dados
...
CREATE OR REPLACE PACKAGE BODY CECRED.CCRD0007 IS
  ...
  BEGIN
    ...
    vr_obj_VoReqAltaDeContaCartao.put('agenciaContaVinculada','');
                                        
  END pc_gera_json_ret_bancoob;
  ...
  BEGIN
    ...
    vr_request.api_route := vr_recurso||'/altaDeContaCartao'/*pr_comprecu*/;
...
dbms_output.put_line('vr_idacionamento: '||vr_idacionamento);
          ...
          UPDATE crawcrd
             SET dsprotoc = pr_dsprotocolo
           WHERE cdcooper = pr_cdcooper
             AND nrdconta = pr_nrdconta
             AND nrctrcrd = pr_nrctrcrd;
        ELSE
          -- Gerar erro
          vr_dscritic := 'Nao foi possivel retornar Protocolo do Bancoob!' || ' ' ||pr_des_mensagem;
          RAISE vr_exc_erro;
        END IF;
      EXCEPTION
        WHEN vr_exc_erro THEN
        ...
        WHEN OTHERS THEN
          vr_dscritic := 'Nao foi possivel retornar Protocolo do Bancoob!';
          RAISE vr_exc_erro;
      END;
    END IF;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      pr_dscritic := 'Não foi possivel enviar proposta para o Bancoob: '||SQLERRM;
  ...
  /* ..........................................................................
    ...
    PROCEDURE altera_sit_cartao_bancoob(rw_crawcrd  IN cr_crawcrd%ROWTYPE)IS
      ...
      -- Atualiza registro de Proposta de Cartão de Crédito
        UPDATE crawcrd
           SET insitcrd = 7 -- Aguardando Bancoob
          ...
          IF rw_crawcrd.inpessoa = 1 THEN
            ...
            IF rw_crapttl.idseqttl = 1 THEN
              ...
              IF vr_flupgrad THEN
                ...
                altera_sit_cartao_bancoob(rw_crawcrd);
    ...
    CURSOR cr_crawcrd IS
      select crd.cdcooper
       ...
       where crd.cdcooper = 1
         ...
         and crd.insitcrd = 2  --Solicitado
         and aci.cdcooper = crd.cdcooper
         and aci.cdorigem = 5 --Ayllos
         and aci.nrctrprp = crd.nrctrcrd
         and aci.nrdconta = crd.nrdconta
         and aci.tpproduto = 4; --Cartão de Crédito
    
  BEGIN

    -- Buscar o tempo máximo de espera em segundos pela analise do motor
    vr_qtsegund := gene0001.fn_param_sistema('CRED',pr_cdcooper,'TIME_RESP_BANCOOB');
    ...
    IF rw_crapdat.inproces = 1  THEN
      ...
      FOR rw_crawcrd IN cr_crawcrd LOOP
        ...
        vr_dsprotoc := rw_crawcrd.dsprotocolo;
          
        -- Carregar parametros para a comunicacao com o Bancoob
        pc_carrega_param_bancoob(pr_cdcooper      => rw_crawcrd.cdcooper, -- Codigo da cooperativa
        ...
        vr_obj.put('appOrigem',rw_crawcrd.cdorigem);
        vr_obj.put('protocolo',rw_crawcrd.dsprotocolo);
        ...
        vr_request.content := vr_obj_clob;
        ...
        IF vr_obj_retorno.exist('descErroResult') THEN
          vr_dssitret := gene0007.fn_convert_web_db(UNISTR(replace(RTRIM(LTRIM(vr_obj_retorno.get('descErroResult').to_char(),'"'),'"'),'\u','\')));
        END IF;
        
        IF vr_dssitret IS NULL THEN
          vr_dssitret := 'Cartão Solicitado com sucesso.';
        END IF;
          
        --> Gravar dados log acionamento
        este0001.pc_grava_acionamento(pr_cdcooper              => rw_crawcrd.cdcooper,
                                      pr_cdagenci              => rw_crawcrd.cdagenci,
                                      pr_cdoperad              => 'BANCOOB',
                                      pr_cdorigem              => 5, /*Ayllos*/
                                      pr_nrctrprp              => rw_crawcrd.nrctrcrd,
                                      pr_nrdconta              => rw_crawcrd.nrdconta,
                                      ...
                                      pr_dsresposta_requisicao => vr_resultado||' - '||vr_dssitret,
                                      pr_dsprotocolo           => rw_crawcrd.dsprotocolo,
                                      pr_idacionamento         => vr_idacionamento,
                                      pr_dscritic              => vr_dscritic);
        ...
        -- Liberando a memória alocada pro CLOB
        dbms_lob.close(vr_obj_clob);
        dbms_lob.freetemporary(vr_obj_clob);
      END LOOP;
