...
CREATE OR REPLACE PACKAGE BODY CECRED.EMPR0011 IS
  ...
  BEGIN
    ...
    BEGIN
      ...
      ELSE
        vr_qtparcel := NVL(pr_qtpreemp,0) - NVL(vr_nrparepr,0) + 1;
        vr_dtvencto := pr_dtvencto;
      END IF;
  ...
  BEGIN
    ...
    BEGIN
      ...
      pr_vlatupar              := vr_vlpresente_liquido + NVL(vr_total_juros,0);
      -- Valor de desconto
    ...
    BEGIN
      ...
      FOR rw_crappep IN cr_crappep(pr_cdcooper => pr_cdcooper
        ...
        ELSIF rw_crappep.dtvencto < pr_dtmvtolt THEN
          ...
          pc_calcula_atraso_pos_fixado (pr_dtcalcul => pr_dtmvtolt
    ...
    BEGIN
      ...
      WHILE vr_datafinal <= vr_dtultpag LOOP
        ...
        IF vr_tab_parcelas(vr_nrparepr).dtvencto = vr_datafinal THEN
          ...
          IF vr_tab_parcelas(vr_nrparepr).flcarenc = 0 THEN
            -- Valor Base do IOF
            vr_vlbase_iof := vr_tab_parcelas(vr_nrparepr).vlparepr - vr_tab_total_juros(vr_nrparepr).valor_total_juros - vr_vlbase_iof;
          ...
          ELSE
            -- Valor Base do IOF
            vr_vlbase_iof := vr_vlbase_iof + vr_tab_parcelas(vr_nrparepr).vlparepr;
          END IF;
  ...
  BEGIN
    /* .............................................................................
       ...
       Sigla   : CRED
       Autor   : Jaison Fernando
       Data    : Maio/2017                         Ultima atualizacao: 28/11/2017
       ...
       Alteracoes: Remover o lançamento da tarifa de alienação nas propostas de CDC,
                   Prj. 402 (Jean Michel).
    ............................................................................. */
    ...
    DECLARE
      ...
      CURSOR cr_crawepr(pr_cdcooper IN crapepr.cdcooper%TYPE
        ...
        SELECT qtpreemp
              ...
              ,dtcarenc
              ,idcarenc
              ,DECODE(EMPR0001.fn_tipo_finalidade(pr_cdcooper => crawepr.cdcooper
                                                 ,pr_cdfinemp => crawepr.cdfinemp),3,1,0) AS inlcrcdc
          FROM crawepr
         WHERE cdcooper = pr_cdcooper
           AND nrdconta = pr_nrdconta
           AND nrctremp = pr_nrctremp;
        ...
        IF rw_craplcr.tpctrato = 2 THEN -- Bem Movel
          ...
          FOR rw_crapbpr IN cr_crapbpr(pr_cdcooper => pr_cdcooper
            ...
            OR rw_crapbpr.dscatbem LIKE '%CAMINHAO%') THEN
              ...
              IF rw_crawepr.inlcrcdc = 1 THEN -- Se for CDC não gera tarifa de alienação
                CONTINUE;
              END IF;
  ...
  BEGIN
    ...
    BEGIN
      ...
      vr_vljuros := (NVL(pr_vlsprojt,0) + NVL(vr_vljuros_mensal,0)) * NVL((POWER(1 + pr_txdiaria,vr_qtdedias)-1),0);
  ...
  BEGIN
    ...
    BEGIN
      -- Condicao para verificar se jah foi lancado valor de Juros de Correcao
      IF pr_dtrefjur = pr_dtcalcul THEN
        pr_vljurcor := 0;
        RETURN;
      END IF;  
      ...
      vr_vljuros := (NVL(pr_vlsprojt,0) + NVL(vr_vljuros_mensal,0)) * vr_taxa_periodo;
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        UPDATE crapepr SET crapepr.dtultpag = pr_dtcalcul
