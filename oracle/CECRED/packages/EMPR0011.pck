  ...
  BEGIN
    ...
    BEGIN
      ...
      END IF;  
       ...
       IF UPPER(pr_nmdatela) = 'BLQPREJU' THEN  
           PREJ0003.pc_gera_debt_cta_prj(pr_cdcooper  => pr_cdcooper --> Cooperativa conectada
                                         ,pr_nrdconta => pr_nrdconta --> Número da conta
                                         ,pr_cdoperad => pr_cdoperad --> Código do Operador
                                         ,pr_vlrlanc  => vr_vlpagpar --> Valor da parcela emprestimo
                                         ,pr_dtmvtolt => pr_dtmvtolt --> Movimento atual
  ...
  BEGIN
   ...
   IF UPPER(pr_nmdatela) = 'BLQPREJU' THEN  
           PREJ0003.pc_gera_debt_cta_prj(pr_cdcooper  => pr_cdcooper --> Cooperativa conectada
                                         ,pr_nrdconta => pr_nrdconta --> Número da conta
                                         ,pr_cdoperad => pr_cdoperad --> Código do Operador
                                         ,pr_vlrlanc  => NVL(vr_vlmtapar,0) + 
                                                         NVL(vr_vlmrapar,0) + 
                                                         NVL(vr_vliofcpl,0) +
                                                         NVL(vr_vlpagpar,0)  --> Valor da parcela emprestimo
                                         ,pr_dtmvtolt => pr_dtcalcul --> Movimento atual
                                         ,pr_cdcritic => vr_cdcritic
                                         ,pr_dscritic => vr_dscritic);
