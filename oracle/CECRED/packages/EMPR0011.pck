CREATE OR REPLACE PACKAGE CECRED.EMPR0011 IS
  ...
  PROCEDURE pc_calcula_prox_parcela_pos(pr_cdcooper     IN  crapepr.cdcooper%TYPE --> Codigo da Cooperativa
                                       ...
                                       ,pr_cddindex     IN  crawepr.cddindex%TYPE --> Tipo de Indexador
...
CREATE OR REPLACE PACKAGE BODY CECRED.EMPR0011 IS
        ...
        IF pr_inproces >= 2 THEN
          IF NOT vr_tab_feriado.EXISTS(lpad(pr_cdcooper,10,'0')||LPAD(TO_CHAR(vr_dtmvtolt,'RRRRMMDD'),10,'0')) THEN
            ...
            FOR rw_crapfer IN cr_crapfer (pr_cdcooper => pr_cdcooper
              ...
              vr_tab_feriado(lpad(pr_cdcooper,10,'0')||lpad(LPAD(TO_CHAR(vr_dtmvtolt,'RRRRMMDD'),10,'0'),10,'0')) := rw_crapfer.dtferiad;
            END LOOP;
          END IF;
        END IF;
  ...
  PROCEDURE pc_calcula_dias360(pr_dtvencto   IN crappep.dtvencto%TYPE  --> Data de Vencimento
    ...
    BEGIN
      ...
      BEGIN
        ...
        vr_ano_data_final := to_char(pr_data_final, 'yyyy');
        ...
        EMPR0001.pc_calc_dias360(pr_ehmensal => FALSE
                                ...
                                ,pr_diarefju => to_char(pr_dtrefjur,'dd')   -- Dia da data de referência da última vez que rodou juros
                                ,pr_mesrefju => to_char(pr_dtrefjur,'mm')   -- Mes da data de referência da última vez que rodou juros
                                ,pr_anorefju => to_char(pr_dtrefjur,'yyyy') -- Ano da data de referência da última vez que rodou juros
  ...
  PROCEDURE pc_calcula_fator_price(pr_cdcooper          IN crapepr.cdcooper%TYPE  --> Codigo da Cooperativa
                                  ...
                                  ,pr_txmensal          IN crapepr.txmensal%TYPE  --> Taxa Mensal do Contrato
                                  ,pr_cddindex          IN crawepr.cddindex%TYPE  --> Tipo do Indexador
  ...
  BEGIN
    ...
    BEGIN
      ...
      FOR vr_indice IN 1..pr_qtparcel LOOP
        -- Calcula a diferenca entre duas datas e retorna os dias corridos
        pc_calcula_dias360(pr_dtvencto   => pr_dtvencto
        ...
        pr_tab_price(vr_indice).taxa_periodo         := POWER((1 + (rw_craptxi.vlrdtaxa / 100)),(vr_qtdia_uteis / 252)) - 1;      
  ...
  PROCEDURE pc_calcula_saldo_projetado(pr_cdcooper     IN  crapepr.cdcooper%TYPE  --> Codigo da Cooperativa
                                      ...
                                      ,pr_txdiaria     IN  craplcr.txdiaria%TYPE  --> Taxa Diaria do Contrato
                                      ,pr_vlemprst     IN  crapepr.vlemprst%TYPE  --> Valor do Emprestimo
  ...
  BEGIN                                      
    ...
    DECLARE                  
      ...
      vr_saldo_juros_correcao       NUMBER(25,2);
      vr_saldo_juros_remuneratorio  NUMBER(25,2);
    ...
    BEGIN
      ...
      pc_calcula_dias360(pr_dtvencto   => pr_dtdpagto
      ...
      ELSE
        vr_saldo_juros_correcao      := pr_vlemprst;
        vr_saldo_juros_remuneratorio := pr_vlemprst;
      ...
      IF TO_CHAR(pr_dtdpagto,'DD') = TO_CHAR(pr_datafinal,'DD') THEN
        IF pr_tab_price.EXISTS(vr_indice - 1) THEN
          ...
          vr_saldo_juros_remuneratorio := pr_tab_price(vr_indice - 1).saldo_projetado + 
                                          pr_tab_price(vr_indice - 1).juros_remuneratorio;
                                          
          -- O Saldo do Juros de Correcao será o Saldo da Mensal
          vr_saldo_juros_correcao      := pr_tab_price(vr_indice - 1).saldo_projetado + 
                                          pr_tab_price(vr_indice - 1).juros_correcao;
        END IF;
      END IF;
      -- Calculo do Juros Remuneratorio
      pr_tab_price(vr_indice).juros_remuneratorio := vr_saldo_juros_remuneratorio * ((POWER(1 + pr_txdiaria,vr_qtdias_corridos))-1);
      ...
      --------------------------------------------------------------------------------------------------
      --  Condicao para verificar se estamos calculando o Vencimento da Carência
      --------------------------------------------------------------------------------------------------
      pr_tab_price(vr_indice).vlparepr := 0;
      IF TO_CHAR(pr_dtcarenc,'DD') = TO_CHAR(pr_datafinal,'DD') THEN
        pr_tab_price(vr_indice).vlparepr := pr_tab_price(vr_indice).juros_correcao +
                                            pr_tab_price(vr_indice).juros_remuneratorio;
        -- Condicao para verificar se foi calculado Juros Correcao/Remuneratorio na Mensal
        IF pr_tab_price.EXISTS(vr_indice - 1) THEN
          pr_tab_price(vr_indice).vlparepr := pr_tab_price(vr_indice).vlparepr +
                                              pr_tab_price(vr_indice - 1).juros_correcao +
                                              pr_tab_price(vr_indice - 1).juros_remuneratorio;
        END IF;
      END IF;
      
      -- Quando for calcular o Juros na Mensal, não será calculado o Saldo Projetado
  ...
  BEGIN
    ...
    BEGIN
      ...
      vr_qtparcel := vr_qtparcel + NVL(pr_qtpreemp,0) - NVL(pr_nrparepr,0);
      ...
      vr_nrparepr := NVL(pr_nrparepr,0);
      FOR vr_indice IN 1..pr_qtpreemp LOOP
        pr_tab_parcelas(vr_nrparepr).nrparepr := vr_nrparepr;
        pr_tab_parcelas(vr_nrparepr).vlparepr := NVL(pr_vlsdeved,0) / vr_fator_price_total;
  ...
  BEGIN
    ...
    BEGIN
      ...
      pc_calcula_fator_price(pr_cdcooper          => pr_cdcooper
                            ...
                            ,pr_cddindex          => rw_craplcr.cddindex
  ...
  BEGIN
    ...
    BEGIN
      ...
      END IF;
      -- a ultima ocorrência de juros de mora/vencimento até o par_dtmvtolt
  ...
  BEGIN
    ...
    BEGIN
      ...
      pc_calcula_fator_price(pr_cdcooper          => pr_cdcooper
                            ...
                            ,pr_cddindex          => rw_crawepr.cddindex
                            ,pr_qtparcel          => vr_qtparcel
                            ,pr_tab_price         => vr_tab_price
                            ,pr_fator_price_total => vr_fator_price_total
                            ,pr_cdcritic          => vr_cdcritic
      ...
      */        
      ...
      -- Quantidade de dias corridos entre a data atual e o lancamento de juros remuneratorio
      pc_calcula_dias360(pr_dtvencto   => rw_crappep.dtvencto
  ...
  BEGIN
    ...
    BEGIN
      ...
      FOR rw_crappep IN cr_crappep(pr_cdcooper => pr_cdcooper
        ...
        -- Parcela em Atraso
        IF rw_crappep.dtvencto < pr_dtcalcul THEN
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHILE vr_indice IS NOT NULL LOOP
        vr_vlpreapg := vr_vlpreapg + vr_tab_parcelas(vr_indice).vlatupar;
        vr_vlmtapar := vr_vlmtapar + vr_tab_parcelas(vr_indice).vlmtapar;
        vr_vlmrapar := vr_vlmrapar + vr_tab_parcelas(vr_indice).vlmrapar;

        CASE vr_tab_parcelas(vr_indice).insitpar
          -- Em Dia
          WHEN 1 THEN vr_vlpraven := vr_vlpraven + vr_tab_parcelas(vr_indice).vlatupar;
          -- Em Atraso
          WHEN 2 THEN vr_vlprvenc := vr_vlprvenc + vr_tab_parcelas(vr_indice).vlatupar;
          -- A Vencer
          WHEN 3 THEN vr_vlpraven := vr_vlpraven + vr_tab_parcelas(vr_indice).vlsdvpar;
        END CASE;
  ...
  BEGIN
    ...
    BEGIN
      ...
      pc_calcula_dias360(pr_dtvencto   => pr_dtvencto
                        ,pr_dtrefjur   => NVL(pr_dtrefjur,pr_dtlibera)
                        ,pr_data_final => pr_dtcalcul
      ...
      vr_vljuros := pr_vlsprojt * (POWER(1 + pr_txdiaria,vr_qtdedias)-1);
  ...
  BEGIN
    ...
    BEGIN
      ...
      pc_calcula_qtd_dias_uteis(pr_cdcooper    => pr_cdcooper
                               ...
                               ,pr_datainicial => NVL(pr_dtrefjur,pr_dtlibera)
                               ,pr_datafinal   => pr_dtcalcul
      ...
      -- Condicao para verificar se eh a mensal
      IF NOT pr_ehmensal THEN
        -- Buscar o ultimo lancamento de Juros de Correcao
        ...
        -- Saldo Projetado
        vr_vlsprojt := NVL(vr_vlsprojt,0) + NVL(vr_vllanmto,0);
      END IF;
      
      -- Valor do Juros de Correcao
      vr_vljuros := vr_vlsprojt * vr_taxa_periodo;
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF NVL(pr_vlsdvpar,0) - NVL(pr_vlpagpar,0) <= 0 THEN
        vr_inliquid := 1;
        vr_vljura60 := 0;
      ...
      BEGIN
        UPDATE crappep
           SET crappep.dtultpag = pr_dtcalcul
              ...
              ,crappep.vlsdvpar = NVL(crappep.vlsdvpar,0) - NVL(pr_vlpagpar,0)
...
/* -- JFF
      ...
      ELSE
        ...
        rw_crapepr.vlsdeved := nvl(rw_crapepr.vlsdeved, 0) - nvl(pr_vlpagpar, 0) + nvl(vr_vljurmes, 0);
      END IF;
*/
      ...
      BEGIN
        UPDATE crapepr
           SET crapepr.dtrefjur = pr_dtcalcul
              ,crapepr.dtultpag = pr_dtcalcul
              ,crapepr.qtprepag = vr_qtprepag
              ,crapepr.qtprecal = vr_qtprecal
              --,crapepr.vlsdeved = [A DEFINIR] -- JFF
              ,crapepr.vljuratu = NVL(vljuratu,0) + vr_vljuremu + vr_vljurcor
              ,crapepr.vljuracu = NVL(vljuracu,0) + vr_vljuremu + vr_vljurcor
         WHERE crapepr.cdcooper = pr_cdcooper
           AND crapepr.nrdconta = pr_nrdconta
           AND crapepr.nrctremp = pr_nrctremp;
      EXCEPTION
...
rw_craptxi.vlrdtaxa := 7.83; -- JFF
vr_blnachou := TRUE; -- JFF
      ...
      FOR vr_idx_lst IN 1..vr_parc_lst.COUNT LOOP
        ...
        ELSE
          ...
          -- Parcela Vencida --
          ---------------------
          IF rw_crappep.dtvencto < vr_dtmvtolt THEN
          ...
          END IF; -- Parcela Vencida

        END IF; -- Parcela em dia
