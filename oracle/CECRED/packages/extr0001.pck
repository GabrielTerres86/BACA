CREATE OR REPLACE PACKAGE CECRED.EXTR0001 AS
  ...
  /* -------------------------------------------------------------------------------------------------------------
    ...
    Sigla    : GENE
    Autor    : Mirtes.
    Data     : Dezembro/2012.                   Ultima atualizacao: 20/06/2016
...
..............................................................................*/
  ...
  PROCEDURE pc_obtem_saldo_dia_sd(pr_cdcooper   IN crapcop.cdcooper%TYPE                            
                                 ...
                                 ,pr_tipo_busca IN VARCHAR2 DEFAULT 'U' /*U-nao usa data, I=usa dtrefere, A=Usa dtrefere-1, P=Usa dtrefere+1 */                                
...
..............................................................................*/
  ...
  --> Retornar o ultimo registro com data cfme pr_tipo_busca =
  -- A -> Retornar o ultimo registro com data inferior a data passada
  -- I -> Retornar o ultimo registro com data igual a data passada
  -- U -> Retornar o ultimo registro sem considerar a data
  CURSOR cr_max_sda(pr_cdcooper   IN crapsda.cdcooper%TYPE
                   ,pr_nrdconta   IN crapsda.nrdconta%TYPE
                   ,pr_dtrefere   IN crapsda.dtmvtolt%TYPE
                   ,pr_tipo_busca IN VARCHAR2) IS
    SELECT MAX(sda.progress_recid)
      FROM crapsda sda
     WHERE sda.cdcooper = pr_cdcooper
       AND sda.nrdconta = pr_nrdconta
       AND ( (pr_tipo_busca = 'A' AND sda.dtmvtolt < pr_dtrefere)
            OR
             (pr_tipo_busca = 'I' AND sda.dtmvtolt = pr_dtrefere)
            OR
             (pr_tipo_busca = 'U' AND sda.dtmvtolt >= pr_dtrefere)
           );
  ...
  BEGIN
    ...
    BEGIN
      ...
      /*U-nao usa data, I=usa dtmvtolt, A=Usa dtmvtoan, P=Usa dtmvtopr */
      IF pr_tipo_busca IN ('I','A','P') THEN
        ...
        CLOSE cr_crapsda_pk;
      ELSE
        -- Busca do Saldo da conta tentativa U -> Retornar o ultimo registro ordenado por data
        OPEN cr_max_sda(pr_cdcooper   => pr_cdcooper
                       ,pr_nrdconta   => pr_nrdconta
                       ,pr_dtrefere   => pr_rw_crapdat.dtmvtoan
                       ,pr_tipo_busca => 'U');
        FETCH cr_max_sda INTO vr_progress_recid;
        vr_crapsda:= cr_max_sda%FOUND;
        CLOSE cr_max_sda;
        --Se Encontrou
        IF vr_crapsda THEN
      ...
      IF vr_dtrefere > pr_rw_crapdat.dtmvtoan AND vr_dtrefere < pr_rw_crapdat.dtmvtocd THEN
        ...
        vr_lscdhist_ret := '15,316,375,376,377,450,530,537,538,539,767,771,772,918,920,1109,1110,1009,1011,'||vr_tab_tarifa_transf(vr_tariidx).cdhisint||','||vr_tab_tarifa_transf(vr_tariidx).cdhistaa || vr_cdhishcb; --> Lista com códigos de histórico a retornar         
  ...
  BEGIN
    ...
    --    Sigla   : CRED
    --    Autor   : Marcos (Supero)
    --    Data    : Dez/2012                         Ultima atualizacao: 21/06/2016
    --
    --    Dados referetes ao programa:
    --    Frequencia: Sempre que chamado pelos programas de extrato da conta
    --
  ...
  BEGIN
    /*
        ...
        Sigla   : CRED
        Autor   : Marcos (Supero)
        Data    : Dez/2012                         Ultima atualizacao: 01/04/2015
    ...
    */
    ...
    BEGIN
      ...
      IF rw_craplcm.cdhistor = 508 THEN
    ...
    BEGIN
      ...
      IF pr_dtfimper > pr_rw_crapdat.dtmvtoan AND pr_dtfimper < pr_rw_crapdat.dtmvtocd THEN
        ...
        FOR rw_craplcm_olt IN cr_craplcm_olt(pr_cdcooper => pr_cdcooper            --> Cooperativa conectada
                                    ,pr_nrdconta => pr_nrdconta            --> Número da conta
                                    ,pr_dtmvtolt => pr_rw_crapdat.dtmvtocd --> Data do movimento utilizada no cash dispenser.                                    
                                    ,pr_lsthistor_ret => '15,316,375,376,377,450,530,537,538,539,767,771,772,918,920,1109,1110,1009,1011,'||vr_tab_tarifa_transf(vr_tariidx).cdhisint||','||vr_tab_tarifa_transf(vr_tariidx).cdhistaa || vr_cdhishcb) LOOP --> Lista com códigos de histórico a retornar
          -- Se for uma transferencia agendada, nao compor saldo
          IF NOT( (rw_craplcm_olt.cdhistor IN(375,376,377,537,538,539,771,772) AND NVL(SUBSTR(rw_craplcm_olt.cdpesqbb,54,8),' ') = 'AGENDADO')
                 OR
                  (rw_craplcm_olt.cdhistor IN(1009,1011,vr_tab_tarifa_transf(vr_tariidx).cdhisint,vr_tab_tarifa_transf(vr_tariidx).cdhistaa) AND NVL(SUBSTR(rw_craplcm_olt.cdpesqbb,15,8),' ') = 'AGENDADO')
      ...
      LOOP
        ...
        IF vr_ind_tab = vr_tab_extr.LAST OR vr_tab_extr(vr_ind_tab).dtmvtolt <> vr_tab_extr(vr_tab_extr.NEXT(vr_ind_tab)).dtmvtolt THEN
          ...
          END IF;
          -- Busca do Saldo da conta com tentativa:
          -- I -> Retornar o ultimo registro com data igual a data passada
          OPEN cr_max_sda(pr_cdcooper   => pr_cdcooper
                         ,pr_nrdconta   => pr_nrdconta
                         ,pr_dtrefere   => vr_tab_extr(vr_ind_tab).dtmvtolt
