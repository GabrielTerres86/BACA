...
CREATE OR REPLACE PACKAGE BODY CECRED.GENE0004 IS
  ...
  PROCEDURE pc_valida_acesso_sistema(pr_cdcooper IN crapace.cdcooper%TYPE      --> Código da cooperativa
    ...
    --  Sigla    : GENE
    --  Autor    : Petter R. Villa Real  - Supero
    --  Data     : Maio/2014.                   Ultima atualizacao: 04/09/2015
    --
    --  Dados referentes ao programa:
    --
    ...
    DECLARE
      ...
      CURSOR cr_crapace(pr_cdcooper IN crapace.cdcooper%TYPE
        ...
        WHERE ce.cdcooper          = pr_cdcooper
          ...
          AND ce.nmdatela          = pr_nmdatela
          AND NVL(ce.nmrotina,' ') = NVL(pr_nmrotina,' ')
  ...
  PROCEDURE pc_executa_job(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Codigo da cooperativa
    ...
    --  Sigla    : GENE
    --  Autor    : Tiago
    --  Data     : Julho/2014.                   Ultima atualizacao: 10/08/2015
    --
    --  Dados referentes ao programa:
    --
    --   Frequencia: Sempre que for chamado
      ...
      BEGIN
        ...
        IF cr_job%FOUND THEN
          ...
          gene0001.pc_submit_job(pr_cdcooper  => pr_cdcooper              --> Código da cooperativa
                                ...
                                ,pr_dthrexe   => TO_TIMESTAMP_tz(to_char((SYSDATE + 0.041666),'DD/MM/RRRR HH24:MI'),
      ...
      IF pr_flproces = 1 THEN
         IF rw_crapdat.inproces <> 1 THEN    
            ...
            IF pr_flrepjob = 1 THEN 
              ...
              pr_dscritic := 'Processo noturno nao finalizado pra cooperativa '||to_char(pr_cdcooper)||
                             ', job reagendado para '||to_char((SYSDATE + 0.041666),'DD/MM/RRRR HH24:MI');
              RAISE vr_exc_saida;
            ELSE  
              pr_dscritic := 'Processo noturno nao finalizado pra cooperativa '||to_char(pr_cdcooper);           
              RAISE vr_exc_saida;
    ...
    DECLARE
      ...
      CURSOR cr_craprdr(pr_nmprogra IN craprdr.nmprogra%TYPE      --> Nome da tela de execução
        ...
        WHERE ca.nrseqrdr = cr.nrseqrdr
          AND UPPER(cr.nmprogra) LIKE UPPER(pr_nmprogra)
          AND UPPER(ca.nmdeacao) LIKE UPPER(pr_nmeacao);
