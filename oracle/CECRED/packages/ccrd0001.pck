...
/*..............................................................................
    ...
    Data    : Outubro/2009                       Ultima Atualizacao: 24/10/2013
...
CREATE OR REPLACE PACKAGE BODY CECRED."CCRD0001" AS
  ...
  BEGIN
    ...
    -- CURSOR para craptab que contem os valores dos limites
    CURSOR cr_craptab IS
      SELECT craptab.dstextab
       ...
       WHERE craptab.cdcooper = pr_cdcooper
         ...
         AND craptab.cdacesso = 'VLCONTRCRD'
    ...
    BEGIN
      ...
      IF pr_bradesbb = 1 THEN /* pega vlr credito e deb Bradesco */
        vr_valorbra := TO_NUMBER(SUBSTR(REPLACE(rw_craptab.dstextab,'.',','),1,12));
        vr_debitbra := TO_NUMBER(SUBSTR(REPLACE(rw_craptab.dstextab,'.',','),27,12));
        pr_valorcre := vr_valorbra;
        pr_valordeb := vr_debitbra;
      ELSE
        IF pr_bradesbb = 0 THEN /* pega vlr cred e debito BB  */
          vr_valordbb := TO_NUMBER(SUBSTR(REPLACE(rw_craptab.dstextab,'.',','),14,12));
          pr_valorcre := vr_valordbb;
          vr_debitabb := TO_NUMBER(SUBSTR(REPLACE(rw_craptab.dstextab,'.',','),40,12));
          pr_valordeb := vr_debitabb;
          pr_vltotsaq := TO_NUMBER(SUBSTR(rw_craptab.dstextab,53,12));
        END IF;
      END IF;
      CLOSE cr_craptab;
