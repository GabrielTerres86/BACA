...
BEGIN
	OPEN cr_saldo;
	FETCH cr_saldo INTO vr_qtd_dias_estouro;
	CLOSE cr_saldo;
...
BEGIN
  pr_cdcritic := 0;
	...
	-- Se o lançamento é um débito e a conta está estourada há mais de 60 dias 
	-- e o histórico não permite aumentar o estouro da conta.
	IF vr_reg_historico.indebcre = 'D' AND (NOT fn_pode_debitar(pr_cdcooper, pr_nrdconta, pr_cdhistor)) THEN
		pr_cdcritic := 1139;
