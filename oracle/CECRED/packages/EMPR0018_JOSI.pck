CREATE OR REPLACE PACKAGE CECRED.EMPR0018_JOSI AS
...
/*          vlprecar	number(25,2),
    ...
    PROCEDURE pc_valida_simulacao(pr_cdcooper IN crapcop.cdcooper%TYPE
                                 ,pr_cdagenci IN crapage.cdagenci%TYPE
                                 ,pr_nrdcaixa IN INTEGER
	                               ,pr_cdoperad IN INTEGER 
                                 ,pr_nmdatela IN VARCHAR2
   ...
   PROCEDURE pc_grava_simulacao( pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    PROCEDURE pc_calcula_emprestimo(pr_cdcooper IN crapcop.cdcooper%TYPE
                                   ...
                                   ,pr_dtlibera IN DATE
                                   ,pr_idfiniof IN INTEGER
                                   ,pr_vlparepr IN tbepr_simulacao_parcela.vlparepr%TYPE DEFAULT NULL-- josiane
                                   ,pr_vliofepr IN crapsim.vliofepr%TYPE DEFAULT NULL -- josiane                                   
...
END EMPR0018_JOSI;                                        
/
CREATE OR REPLACE PACKAGE BODY CECRED.EMPR0018_JOSI AS
...
..............................................................................*/
   ...
   PROCEDURE pc_busca_dados_simulacao(pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    BEGIN
      ...
      IF cr_crapsim%FOUND THEN
        ...
        pr_tcrapsim(pr_nrdconta).vlrtotal := rw_crapsim.vlemprst;
        
        IF rw_crapsim.idfiniof = 1 THEN
          pr_tcrapsim(pr_nrdconta).vlrtotal := rw_crapsim.vlemprst + rw_crapsim.vliofepr + rw_crapsim.vlrtarif; 
        END IF;
   ...
   PROCEDURE pc_busca_simulacoes(pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    CURSOR cr_crapsim(pr_cdcooper IN crapsim.cdcooper%TYPE
      ...
      SELECT *
       ...
       WHERE sim.cdcooper = pr_cdcooper
         AND sim.nrdconta = pr_nrdconta         
         AND vr_dt_ini IS NULL
         AND vr_dt_fim IS NULL;
    rw_crapsim cr_crapsim%ROWTYPE;
    ...
    BEGIN
      IF pr_flgerlog THEN
        vr_aux_dsorigem := gene0001.vr_vet_des_origens(pr_cdorigem);
        vr_aux_dstransa := 'Excluir simulacao de emprestimo. ';
      END IF; 
   ...
   PROCEDURE pc_consulta_tarifa_emprst(pr_cdcooper IN INTEGER                    
    ...
    pr_vlrtarif := pr_vlrtarif + vr_vltarifa;
  ...
  rw_craptab cr_craptab%ROWTYPE;
   ...
   PROCEDURE pc_valida_simulacao(pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    BEGIN                             
      IF pr_cddepart NOT IN (14, 18, 20) THEN
        vr_cdcritic := 36;
        vr_dscritic := '';
        RAISE vr_exc_erro;
      END IF;
      ...
      IF pr_flgerlog THEN
        GENE0001.pc_gera_log(pr_cdcooper => pr_cdcooper 
                           ...
                           , pr_dstransa => 'Valida simulação de empréstimo'
                           , pr_dttransa => TRUNC(SYSDATE)
                           , pr_flgtrans => 1 
                           , pr_hrtransa => TO_NUMBER(TO_CHAR(SYSDATE, 'SSSSS'))
                           , pr_idseqttl => pr_idseqttl
   ...
   PROCEDURE pc_grava_simulacao( pr_cdcooper IN crapcop.cdcooper%TYPE
                                ...
                                ,pr_dtcarenc IN DATE
                                ,pr_vlparepr IN tbepr_simulacao_parcela.vlparepr%TYPE -- josiane
                                ,pr_vliofepr IN crapsim.vliofepr%TYPE) IS             -- josiane 
    --CURSORES
    --Cursor para associado
    CURSOR cr_crapass(pr_cdcooper IN crapass.cdcooper%TYPE
                     ,pr_nrdconta IN crapass.nrdconta%TYPE) IS
    ...
    CURSOR cr_craplcr(pr_cdcooper IN craplcr.cdcooper%TYPE
                     ,pr_cdlcremp IN craplcr.cdlcremp%TYPE) IS
      SELECT lcr.tpctrato, lcr.txmensal, lcr.tpmodcon
        FROM craplcr lcr
       WHERE lcr.cdcooper = pr_cdcooper
         AND lcr.cdlcremp = pr_cdlcremp;
    rw_craplcr cr_craplcr%ROWTYPE;
    ...
    pr_retorno := NULL;
      ...
      vr_tpmodcon:= NVL(rw_craplcr.tpmodcon,0);
      ...
      ELSE --POS
        IF pr_tpemprst = 2 THEN
          ...
          IF pr_idfiniof = 1 THEN
             vr_vlemprst := vr_vlemprst + vr_vlrtarif;
           END IF;
          ...
          IF pr_idfiniof = 1 THEN
             vr_vlemprst := vr_vlemprst + vr_valoriof;
           END IF; 
   ...
   PROCEDURE pc_calcula_emprestimo(pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    CURSOR cr_craplcr(pr_cdcooper IN craplcr.cdcooper%TYPE
                     ,pr_cdlcremp IN craplcr.cdlcremp%TYPE) IS
      SELECT lcr.txmensal, lcr.cdmodali, lcr.cdsubmod,lcr.tpmodcon
        FROM craplcr lcr
       WHERE lcr.cdcooper = pr_cdcooper
         AND lcr.cdlcremp = pr_cdlcremp;
    rw_craplcr cr_craplcr%ROWTYPE;
    ...
    BEGIN
      ...
      IF cr_craplcr%NOTFOUND THEN
        ...
        RAISE vr_exc_saida;   
      END IF;
      vr_tpmodcon:= nvl(rw_craplcr.tpmodcon,0); -- josiane
      ...
      IF pr_idfiniof > 0 THEN
        ...
        IF vr_tpmodcon = 0 THEN -- Josiane
          ...
          END IF;  
        ELSE
            vr_valoriof:= pr_vliofepr; -- josiane
        END IF;
      ELSE
        -- considera calendario comercial                         
        pr_txmensal := rw_craplcr.txmensal; 
