...
CREATE OR REPLACE PACKAGE BODY CECRED.RATI0001 IS
  ...
  BEGIN
    ...
    DECLARE
      ...
      vr_idacessaBD varchar2 (3) := 'Não';  
      vr_idachouBD  varchar2 (3) := 'Não';  
      ...
      ELSE        
        ...
        exception
          WHEN NO_DATA_FOUND THEN
            vr_idacessaBD := 'Sim';            
          WHEN OTHERS THEN
            -- Gerar erro não tratado critica 9999
            vr_cdcritic := 9999;
            vr_dscritic := 'RATI0001.pc_param_valor_rating. vr_vet_vlrating. Detalhes: '||sqlerrm;
            RAISE vr_exc_saida;
        END;        
      END IF;

      -- Se a tabela com as informações de valor por coop estiver vazia      
      IF vr_idacessaBD = 'Sim' THEN
        -- Se a tabela com as informações de valor por coop estiver sem a cooperativa da solicitação
        ...
        FOR rw_craptab IN cr_craptab(pr_nmsistem => 'CRED'
          ...
          vr_vet_vlrating(rw_craptab.cdcooper) := gene0002.fn_char_para_number(substr(rw_craptab.dstextab,15,11));
	        ...
	        -- Incluir falha para teste - Chamado 660306 28/06/2017
          --declare
          --  teste number (1) := 0;
          --begin
          --  teste := 0 / 0;
