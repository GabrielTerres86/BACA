CREATE OR REPLACE PACKAGE CECRED.rati0001 is
  ...
  --  Data     : Maio/2013.                   Ultima atualizacao: 06/08/2015
  --
  ...
  TYPE typ_reg_impress_coop IS
    RECORD (nrdconta NUMBER
           ...
           ,nrctrrat crapnrc.tpctrrat%TYPE
           ,tpctrrat crapnrc.nrctrrat%TYPE);
...
CREATE OR REPLACE PACKAGE BODY CECRED.rati0001 IS
  ...
  BEGIN
    ...
    BEGIN
      ...
      FOR rw_crapepr IN cr_crapepr LOOP
        ...
        vr_dsliquid := fn_traz_liquidacoes(pr_rw_crawepr => rw_crawepr);
    ...
    BEGIN
      ...
      IF pr_tpctrato = 90 THEN
        ...
        IF cr_crawepr%FOUND THEN
          CLOSE cr_crawepr;
          -- Usar daqui
          vr_vloperac := rw_crawepr.vlemprst;
        ELSE
          CLOSE cr_crawepr;
          ...
          CLOSE cr_crapprp;
          -- Usar da proposta
          vr_vloperac := rw_crapprp.vlctrbnd;
        END IF;
      ELSE
        ...
        CLOSE cr_craplim;
        -- Usar do limite
        vr_vloperac := rw_craplim.vllimite;
      END IF;
      -- Efetuar o retorno
      RETURN vr_vloperac;
    EXCEPTION
      ...
      CURSOR cr_crapnrc IS
        SELECT nrc.nrdconta
         ...
         WHERE nrc.cdcooper = ass.cdcooper
           ...
           AND nrc.nrdconta = DECODE(pr_nrdconta,0,nrc.nrdconta,pr_nrdconta)
           AND nrc.insitrat = DECODE(pr_insitrat,0,nrc.insitrat,pr_insitrat)
    ...
    BEGIN
      ...
      FOR rw_craprat IN cr_craprat(pr_inpessoa => rw_crapass.inpessoa) LOOP
        ...
        IF rw_craprat.nrsqtopi = 1 THEN
          ...
          pr_tab_impress_rating(vr_sqtb_imprat).nritetop := 0;
          pr_tab_impress_rating(vr_sqtb_imprat).nrseqite := 0;
          pr_tab_impress_rating(vr_sqtb_imprat).dsitetop := rw_craprat.dstopico;
          pr_tab_impress_rating(vr_sqtb_imprat).dspesoit := ' ';
        END IF;
        ...
        IF rw_craprat.nrsqittp = 1 THEN
          ...
          vr_sqtb_imprat := pr_tab_impress_rating.COUNT+1;
          pr_tab_impress_rating(vr_sqtb_imprat).nrtopico := rw_craprat.nrtopico;
          pr_tab_impress_rating(vr_sqtb_imprat).nritetop := rw_craprat.nritetop;
          pr_tab_impress_rating(vr_sqtb_imprat).nrseqite := 0;
          pr_tab_impress_rating(vr_sqtb_imprat).dsitetop := gene0002.fn_mask(rw_craprat.nrtopico,'z9')||'.'||gene0002.fn_mask(rw_craprat.nritetop,'z9')||' '||rw_craprat.dsitetop;
          pr_tab_impress_rating(vr_sqtb_imprat).dspesoit := '('||to_char(rw_craprat.pesoitem,'990d00')||' )';
        END IF;
        ...
        IF vr_vlrtotal > 0 THEN
          -- Criar registros da sequencia do rating
          vr_sqtb_imprat := pr_tab_impress_rating.COUNT+1;
          pr_tab_impress_rating(vr_sqtb_imprat).nrtopico := rw_craprat.nrtopico;
          pr_tab_impress_rating(vr_sqtb_imprat).nritetop := rw_craprat.nritetop;
          pr_tab_impress_rating(vr_sqtb_imprat).nrseqite := rw_craprat.nrseqite;
          pr_tab_impress_rating(vr_sqtb_imprat).dsitetop := gene0002.fn_mask(rw_craprat.nrtopico,'z9')||'.'||gene0002.fn_mask(rw_craprat.nritetop,'z9')||' '||rw_craprat.dsseqite;
          pr_tab_impress_rating(vr_sqtb_imprat).dspesoit := to_char(rw_craprat.pesosequ,'90d00')||'   '|| to_char(vr_vlrdnota,'90d00');
        END IF;
  ...
  *****************************************************************************/
  ...
  BEGIN
    /* ..........................................................................
       ...
       Sigla   : CRED
       Autor   : Marcos Ernani Martini
       Data    : Agosto/2014.                          Ultima Atualizacao: 03/07/2015
  ...
  *****************************************************************************/
  ...
  /* ..........................................................................
     ...
     Sigla   : CRED
     Autor   : Marcos Martini - Supero
     Data    : Setembro/2014.                         Ultima Atualizacao: 03/07/2015
  ...
  ---------------- CURSORES ----------------
    ...
    vr_vet_nrctrliq typ_vet_nrctrliq;
 ...
 BEGIN
    ...
    *************************************************************************/
    ...
    ELSE
      ...
      ELSE
        ...
        IF pr_tpctrato = 90 THEN
          IF vr_fcrapprp THEN
            vr_nrseqite := rw_crapprp.nrinfcad;
          END IF;
        ELSE
          IF vr_fcraplim THEN
            vr_nrseqite := rw_craplim.nrinfcad;
          END IF;
        END IF;
      END IF;
      CLOSE cr_crapnrc;
    ...
    ********************************************************************/
    ...
    IF pr_tpctrato = 90 AND vr_fcrawepr THEN
      ...
      vr_dsliquid := fn_traz_liquidacoes(rw_crawepr);
    END IF;
    ...
    IF pr_tpctrato = 90 THEN
      ...
      IF cr_crapepr%NOTFOUND THEN
        ...
        ELSE -- BNDES
          vr_vlendivi := rw_crapprp.vlctrbnd;
        END IF;
      END IF;
      CLOSE cr_crapepr;
      -- Se houver Valor Total SFN exceto na cooperativa
      IF rw_crapprp.vltotsfn <> 0 THEN
        -- Usá-lo
        vr_vlendivi := nvl(vr_vlendivi,0) + rw_crapprp.vltotsfn;
      ELSE
    ...
    ELSIF pr_tpctrato <> 0 THEN
      ...
      IF pr_tpctrato = 1 THEN
        ...
        END IF;
        -- Se houver Valor Total SFN exceto na cooperativa
        IF rw_craplim.vltotsfn <> 0 THEN
          -- Usá-lo
          vr_vlendivi := nvl(vr_vlendivi,0) + rw_craplim.vltotsfn;
        ELSE
          -- Usar valor do endividamento
          vr_vlendivi := nvl(vr_vlendivi,0) + nvl(vr_vlutiliz,0);
        END IF;
    ...
    ********************************************************************/
    ...
    ELSE
      ...
      ELSE
        ...
        IF pr_tpctrato = 90 THEN
          IF vr_fcrapprp THEN
            vr_nrseqite := rw_crapprp.nrpatlvr;
          END IF;
        ELSE
          IF vr_fcraplim THEN
            vr_nrseqite := rw_craplim.nrpatlvr;
          END IF;
        END IF;
      END IF;
      CLOSE cr_crapnrc;
    ...
    ********************************************************************/
    ...
    IF pr_tpctrato = 90 THEN
      -- Empréstimos Cecred
      IF vr_fcrawepr THEN
        vr_vlpresta := rw_crawepr.vlpreemp;
      ...
      ELSIF vr_fcrapprp THEN
        vr_vlpresta := rw_crapprp.vlctrbnd / rw_crapprp.qtparbnd;
      END IF;
    -- Limite / Cheque especial
    ELSIF pr_tpctrato <> 0 THEN
      vr_vlpresta := rw_craplim.vllimite;
    ELSE
      vr_vlpresta := 0;
    END IF;
    ...
    ********************************************************************/
    ...
    ELSE
      ...
      ELSE
        ...
        IF pr_tpctrato = 90 THEN
          IF vr_fcrapprp THEN
            vr_nrseqite := rw_crapprp.nrperger;
          END IF;
        ELSE
          IF vr_fcraplim THEN
            vr_nrseqite := rw_craplim.nrperger;
          END IF;
        END IF;
      END IF;
      CLOSE cr_crapnrc;
  ...
  BEGIN
    ...
    IF pr_tpctrato = 90 THEN  /* Emprestimo / Financiamento */
      ...
      IF vr_fcrawepr THEN
        ...
        IF rw_crawepr.idquapro > 2 THEN
          vr_nrseqite := 6;
        ELSE
          -- Buscar conforme linha de credito
          CASE rw_craplcr.dsoperac
    ...
    -- Emprestimo / Financiamento
    IF pr_tpctrato = 90 THEN
      vr_nrseqite := rw_crapprp.nrgarope;
    -- Descontos / Limite rotativo
    ELSE
      vr_nrseqite := rw_craplim.nrgarope;
    END IF;
    ...
    -- Emprestimo / Financiamento
    IF pr_tpctrato = 90 THEN
      vr_nrseqite := rw_crapprp.nrliquid;
    -- Descontos / Limite rotativo
    ELSE
      vr_nrseqite := rw_craplim.nrliquid;
    END IF;
    ...
    IF pr_tpctrato = 90 THEN
      ...
      IF vr_fcrawepr THEN
         ...
         vr_qtdiapra := rw_crawepr.qtpreemp * 30; -- Sempre vezes 30
      ELSE
         -- Buscar da proposta
         vr_qtdiapra := rw_crapprp.qtparbnd * 30; -- Sempre vezes 30
      END IF;
    -- Descontos / Limite rotativo
    ELSE
      -- Usar dias de vigencia do limite
      vr_qtdiapra := rw_craplim.qtdiavig;
    END IF;
  ...
  BEGIN
    ...
    IF pr_tpctrato = 90 THEN
      ...
      ELSE
        ...
        vr_dsliquid := fn_traz_liquidacoes(pr_rw_crawepr => rw_crawepr); --> Registro da crawepr);
      END IF;
      CLOSE cr_crawepr;
  ...
  *****************************************************************************/
  ...
  /* ..........................................................................
     ...
     Sigla   : CRED
     Autor   : Odirlei Busana - AMcom
     Data    : Agosto/2014.                          Ultima Atualizacao: 05/11/2014
  ...
  BEGIN
    ...
    *************************************************************************/
    ...
    IF pr_tpctrato = 90  THEN  /* Emprestimo / Financiamento */
      IF vr_fcrawepr AND
         rw_crawepr.idquapro = 3   THEN  /* Renegociacao */
        vr_nrseqite := 3;
      END IF;
    END IF;
    ...
    IF pr_tpctrato = 90  THEN
      ...
      -- Operacao BNDES
      ELSIF vr_fcrapprp THEN
        vr_vlpresta := nvl(rw_crapprp.vlctrbnd,0) / nvl(rw_crapprp.qtparbnd,0);
      END IF;
    ELSIF pr_tpctrato <> 0 THEN
      vr_vlpresta := rw_craplim.vllimite;
    ELSE
    ...
    ELSIF pr_tpctrato <> 0  THEN
      IF pr_tpctrato = 1 THEN /* Ch. Especial */
        IF rw_craplim.insitlim <> 2  THEN /* Diferente de ativo */
          vr_vlendivi := rw_craplim.vllimite;
        END IF;

        IF rw_craplim.vltotsfn <> 0  THEN
          vr_vlendivi := nvl(vr_vlendivi,0) + nvl(rw_craplim.vltotsfn,0);
        ELSE
          vr_vlendivi := nvl(vr_vlendivi,0) + nvl(vr_vlutiliz,0);
        END IF;
      END IF;
    ...
    ELSIF pr_tpctrato = 1 THEN /* Ch. Especial */
      IF rw_craplim.insitlim <> 2  THEN /* Diferente de ativo */
        vr_vlendivi := nvl(vr_vlendivi,0) + nvl(rw_craplim.vllimite,0);
      END IF;
    ...
    IF pr_tpctrato = 90 THEN  /* Emprestimo / Financiamento */
      ...
      IF vr_fcrawepr THEN
        pc_natureza_operacao ( pr_tpctrato => pr_tpctrato          --> Tipo Contrato Rating
                              ,pr_idquapro => rw_crawepr.idquapro  --> Numero Contrato Rating
                              ,pr_dsoperac => rw_craplcr.dsoperac  --> Indicado se deve criar o rating
