...
..............................................................................*/
  ...
  vr_cdhistordsct_multa_atraso        CONSTANT craphis.cdhistor%TYPE := 2764; -- CONTABIL: MULTA
...
..............................................................................*/
  ...
  PROCEDURE pc_transferir_prejuizo (pr_cdcooper IN crapcop.cdcooper%TYPE     --> Coop conectada
    ...
    BEGIN
      ...
      UPDATE
        craptdb
      SET
        vlprejuz = vlsldtit, --Prejuízo original – Valor transferido para prejuízo valor transferido sem incidência de juros, multa e  IOF
      ...
      vr_vllanmto := rw_crapbdt_prej.vlprejuz + rw_crapbdt_prej.vlttjmpr;
      -- Remove do lancamento o juros 60 remuneratório e 60 de mora
      OPEN cr_tdb60(pr_nrdconta => rw_crapbdt.nrdconta, pr_dtmvtolt => rw_crapdat.dtmvtolt, pr_cdcooper => rw_crapbdt.cdcooper, pr_nrborder => rw_crapbdt.nrborder);
      FETCH cr_tdb60 INTO rw_craptdb60;
      IF (cr_tdb60%FOUND) THEN 
        CLOSE cr_tdb60;
        IF (rw_craptdb60.vljurrec > 0) THEN
          vr_vllanmto := vr_vllanmto - rw_craptdb60.vljurrec;
          ...
          DSCT0003.pc_inserir_lancamento_bordero(pr_cdcooper => pr_cdcooper
                                       ...
                                       ,pr_vllanmto => rw_craptdb60.vljurrec
                                       ,pr_dscritic => vr_dscritic );
          IF (TRIM(vr_dscritic) IS NOT NULL) THEN
            RAISE vr_exc_erro;
          END IF;
        END IF;
        IF (rw_craptdb60.vljurmor > 0) THEN
          vr_jurmor60 := rw_craptdb60.vljurmor;
          vr_vllanmto := vr_vllanmto - rw_craptdb60.vljurmor;
          ...
          DSCT0003.pc_inserir_lancamento_bordero(pr_cdcooper => pr_cdcooper
                                       ...
                                       ,pr_vllanmto => rw_craptdb60.vljurmor
                                       ,pr_dscritic => vr_dscritic );
          IF (TRIM(vr_dscritic) IS NOT NULL) THEN
            RAISE vr_exc_erro;
          END IF;
        END IF;
        -- - rw_craptdb60.vljurmor
      ELSE
        CLOSE cr_tdb60;
      END IF;
      ...
      DSCT0003.pc_inserir_lancamento_bordero(pr_cdcooper => pr_cdcooper
                                   ...
                                   ,pr_vllanmto => (rw_crapbdt_prej.vlttjmpr-vr_jurmor60)
                                   ,pr_dscritic => vr_dscritic );
      IF (TRIM(vr_dscritic) IS NOT NULL) THEN
        RAISE vr_exc_erro;
      END IF;
  ...
  /*---------------------------------------------------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
      ...
      IF (pr_vlaboorj < 0 OR pr_vlpagmto < 0 OR  vr_total > vr_tab_prej(0).vlsldatu) THEN
        vr_cdcritic := 0;
      ...
      IF (pr_vlaboorj > 0 AND vr_total <> vr_tab_prej(0).vlsldatu AND pr_cdorigem <> 2) THEN
        vr_cdcritic := 0;
   ...
   /*---------------------------------------------------------------------------------------------------------------------
    ...
    CURSOR cr_crapbdt IS
      ...
      WHERE bdt.cdcooper = pr_cdcooper
        ...
        AND (tdb.insittit = 4 OR (tdb.insittit=3 AND bdt.dtliqprj IS NULL))
        AND bdt.inprejuz = 1
      GROUP BY
  ...
  PROCEDURE pc_pagar_bordero_prejuizo(pr_cdcooper  IN crapbdt.cdcooper%TYPE
     ...
     /*---------------------------------------------------------------------------------------------------------------------
      ...
      -- Cursor para calcular o juros de atualizacao +60
      CURSOR cr_tdb60 (pr_cdcooper crapbdt.cdcooper%TYPE, pr_dtmvtolt crapdat.dtmvtolt%TYPE, pr_nrborder crapbdt.nrborder%TYPE, pr_nrdconta crapbdt.nrdconta%TYPE, pr_nrtitulo craptdb.nrtitulo%TYPE) IS
        SELECT 
           SUM((x.dtvencto-x.dtvenmin60+1)*txdiaria*vltitulo) AS vljurrec, -- valor de juros de receita a ser apropriado
           cdcooper,
           nrborder,
      ...
      BEGIN
        ...
        IF (vr_total > 0) THEN
          ...
          WHILE vr_index IS NOT NULL LOOP
            ...
            IF (vr_tab_craptdb(vr_index).vlsldmora > 0) THEN
              ...
              END IF;
              vr_tab_craptdb(vr_index).vlpgjmpr := vr_tab_craptdb(vr_index).vlpgjmpr + vr_pagamento_atual;
              vr_total := vr_total - vr_pagamento_atual;
              vr_total_pago := vr_total_pago + vr_pagamento_atual;
              vr_tab_craptdb(vr_index).atualizou := 1;
          ...
          WHILE vr_index IS NOT NULL LOOP
            ...
            IF (vr_tab_craptdb(vr_index).vlsdprej > 0) THEN
              ...
              IF (cr_tdb60%FOUND) THEN 
                ...
                vr_saldo_60 := rw_craptdb60.vljurrec - rw_tbdsct_lancamento_bordero.vllanmto;
        ...
        IF ((pr_vlpagmto - vr_pagamento_iof) > 0) THEN
          ...
          DSCT0003.pc_efetua_lanc_cc(pr_dtmvtolt => rw_crapdat.dtmvtolt
                           ...
                           ,pr_cdbccxlt => 100
                           ,pr_nrdconta => rw_crapbdt.nrdconta
                           ,pr_vllanmto => (pr_vlpagmto - vr_pagamento_iof) -- apenas lancamento do que foi realmente pago, tirando o abono
      ...
      ---------------------------------------------------------------------------------------------------------------------*/
        ...
        BEGIN
          ...
          IF (vr_vlsldtit > 0) THEN
            ...
            vr_valorsaldoprj := rw_craptdb.vlprejuz;
            IF rw_craptdb.vlprejuz <> rw_craptdb.vlsdprej THEN -- houve algum pagamento do saldo
      ...
      BEGIN
        ...
        IF (vr_total > 0) THEN
          ...
          IF (rw_craptdb.vlsldmora > 0) THEN
            ...
            END IF;
            rw_craptdb.vlpgjmpr := rw_craptdb.vlpgjmpr + vr_pagamento_atual;
            vr_total := vr_total - vr_pagamento_atual;
            vr_total_pago := vr_total_pago + vr_pagamento_atual;
  ...
  INNER JOIN crapbdt bdt ON bdt.cdcooper = tdb.cdcooper AND bdt.nrborder = tdb.nrborder   
    ...
    BEGIN
      ...
      OPEN cr_vltransfprej;
      FETCH cr_vltransfprej INTO rw_vltransfprej;
      IF (cr_vltransfprej%NOTFOUND) THEN
        CLOSE cr_vltransfprej;
        vr_cdcritic := 0;
        vr_dscritic := 'Lançamento de transferência para prejuízo não encontrado';
        RAISE vr_exc_erro;
      END IF;
      CLOSE cr_vltransfprej;
      ...
      */
      ...
      pr_vlsldprj := rw_vltransfprej.vltransfprej - (nvl(rw_craptdb.vlpagprj,0) - nvl(rw_vljura60remun.vljura60remun,0));
