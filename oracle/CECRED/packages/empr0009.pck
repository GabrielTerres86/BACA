CREATE OR REPLACE PACKAGE CECRED.EMPR0009 IS
  ...
  PROCEDURE pc_calcula_atraso_tr(pr_cdcooper  IN crapcop.cdcooper%TYPE     --> Cooperativa
                                ...
                                ,pr_vlatraso OUT crapepr.vlemprst%TYPE); --> Valor calculado
...
CREATE OR REPLACE PACKAGE BODY CECRED.EMPR0009 IS
  ...
  BEGIN
    ...
    BEGIN
      ...
      -- Se possuir valor a pagar e data do pagamento menor que data atual
      IF pr_vlpreapg > 0 AND pr_dtdpagto < pr_dtmvtolt THEN
  ...
  BEGIN
    ...
    BEGIN
      ...
      -- Calculo para saber quantas parcelas estao em atraso
      IF pr_qtpreemp - pr_qtprecal > 1 THEN
        -- Numero de parcelas que falta pagar
  ...
  BEGIN
    ...
    BEGIN
      ...
      -- Gera LOG
		  ...
		  -- Gera item do LOG
			GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid,
																pr_nmdcampo => 'Valor Pago da Multa',
																pr_dsdadant => '',
																pr_dsdadatu => TO_CHAR(vr_vldmulta,'fm999g999g999g990d00'));

		  -- Gera item do LOG
			GENE0001.pc_gera_log_item(pr_nrdrowid => vr_nrdrowid,
																pr_nmdcampo => 'Valor Pago do Juros de Mora',
																pr_dsdadant => '',
