CREATE OR REPLACE PACKAGE CECRED.SSPC0002 AS
  ...
  PROCEDURE pc_negativa_serasa(pr_cdcooper IN crapcob.cdcooper%TYPE,--> Codigo da cooperativa
                               pr_nrcnvcob IN crapcob.nrcnvcob%TYPE,--> Numero do convenio de cobranca. 
                               pr_nrdconta IN crapcob.nrdconta%TYPE,--> Numero da conta/dv do associado. 
  ...
  -- Rotina para alterar o indicador do Serasa para envio do boleto
  -- ao Serasa no proximo processo
  PROCEDURE pc_cancelar_neg_serasa(pr_cdcooper IN crapcob.cdcooper%TYPE,--> Codigo da cooperativa
                                   pr_nrcnvcob IN crapcob.nrcnvcob%TYPE,--> Numero do convenio de cobranca. 
                                   pr_nrdconta IN crapcob.nrdconta%TYPE,--> Numero da conta/dv do associado. 
                                   pr_nrdocmto IN crapcob.nrdocmto%TYPE,--> Numero do documento(boleto) 
                                   pr_nrremass IN INTEGER,              --> Numero da Remessa
  ...
  PROCEDURE pc_busca_param_negativ(pr_cdcooper             IN  tbcobran_param_negativacao.cdcooper%TYPE,
                                   pr_cduf_sacado          IN  tbcobran_param_exc_neg_serasa.dsuf%TYPE, -- Codigo da UF do Sacado
...
CREATE OR REPLACE PACKAGE BODY CECRED.SSPC0002 AS
  ...
  BEGIN
    ...
    OPEN cr_parneg;
    FETCH cr_parneg INTO rw_parneg;
    ...
    IF rw_crapsab.cdufsaca IS NOT NULL THEN
      ...
      FETCH cr_exc_neg INTO rw_parneg.qtminimo_negativacao;
      CLOSE cr_exc_neg;
    END IF;
    ...
    ELSIF rw_crapcob.vltitulo < rw_parneg.vlminimo_boleto THEN
      vr_dsincons := 'Valor do boleto inferior ao minimo de R$ '||to_char( rw_parneg.vlminimo_boleto,'FM999G999G990D00')||
    ...
    ELSIF rw_crapcob.dtvencto + rw_parneg.qtminimo_negativacao > rw_crapdat.dtmvtolt THEN
      vr_dsincons := 'Vencimento deve ser superior a '||rw_parneg.qtminimo_negativacao||' dias corridos!';
    ELSIF rw_crapcob.inserasa IN (1,2) THEN -- 1-Pendente de envio, 2-Solicitacao Enviada
      vr_dsincons := 'Solicitacao de inclusao ja foi enviada a Serasa.';
    ELSIF rw_crapcob.inserasa IN (3,4) THEN -- 3-Pendente de envio Cancel, 2-Solicitacao Cancel Enviada
      vr_dsincons := 'Solicitacao de cancelamento ja foi enviada a Serasa.';      
    ELSIF rw_crapcob.inserasa = 5 THEN -- Negativado
    ...
    ELSE
      ...
      BEGIN
        UPDATE crapcob
         ...
         RETURNING cdbandoc, nrdctabb 
              INTO vr_cdbandoc, vr_nrdctabb;
      EXCEPTION
        WHEN OTHERS THEN
  ...
  BEGIN
    ...
    OPEN cr_param;
    FETCH cr_param INTO vr_hrenvio_arquivo;
    CLOSE cr_param;
    ...
    IF rw_crapcob.inserasa = 0 THEN -- Nao negativado
      vr_dsincons := 'Boleto nao foi enviado ao Serasa.';
    ELSIF rw_crapcob.inserasa IN (3,4) THEN -- Pendente Envio Sol. Cancel / Solicitacao Cancel. Enviada
      vr_dsincons := 'Solicitacao de cancelamento ja foi enviada ao Serasa. Nova solicitacao nao permitida.';
    ELSIF rw_crapcob.inserasa = 6 THEN -- Recusada na Serasa
    ...
    ELSE
      ...
      paga0001.pc_cria_log_cobranca(pr_idtabcob => rw_crapcob.rowid,
                                    pr_cdoperad => nvl(pr_cdoperad,'1'),
                                    pr_dtmvtolt => trunc(SYSDATE), -- Rotina nao utiliza esta data
                                    pr_dsmensag => 'Serasa - Solicitacao de cancelamento de negativacao',
                                    pr_des_erro => vr_des_erro,
                                    pr_dscritic => vr_dscritic);
      IF vr_dscritic IS NOT NULL THEN
        RAISE vr_exc_saida;
        ...
        VALUES
          ...
          ,(SELECT nvl(MAX(nrsequencia),0)+1 FROM tbcobran_his_neg_serasa 
                                            WHERE cdcooper = pr_cdcooper
                                              ...
                                              AND nrdocmto = pr_nrdocmto)
          ,SYSDATE
          ,decode(rw_crapcob.inserasa,1,0, -- Nao esta no Serasa
      ...
      -- Verifica se o horario atual eh maior que o parametrizado
      IF vr_hrenvio_arquivo IS NOT NULL AND
         vr_hrenvio_arquivo < to_char(SYSDATE,'SSSSS') AND 
         pr_nrremass = 0 AND 
         rw_crapcob.inserasa <> 1 THEN -- Se for 1 ja muda para NAO ESTA NO SERASA AUTOMATICAMENTE
        pr_dscritic := 'Acao sera realizada no proximo dia util';
      END IF;
  ...
  BEGIN
    ...
    OPEN cr_param;
    FETCH cr_param INTO pr_qtminimo_negativacao,
    ...
    OPEN cr_exc_neg;
    FETCH cr_exc_neg INTO pr_qtminimo_negativacao,
                          pr_qtmaximo_negativacao;
    CLOSE cr_exc_neg;
    ...
    ..............................................................................*/
      ...
      CURSOR cr_crapceb(pr_cdcooper crapcop.cdcooper%TYPE) IS
        SELECT b.nmextcop||' - '||b.nmrescop nmcooper,
               ...
               'Sim' flprotes, -- Conforme e-mail da Patricia, devera sempre ser sim para protesto
      ...
      gene0002.pc_escreve_xml(pr_xml            => vr_clob
                             ...
                             ,pr_texto_novo     => '<cooperativa>'||chr(10)||
                                                   ...
                                                   '<cooperado>'||chr(10)||
                                                     ...
                                                     '<inreproc>'||rw_crapceb.inreproc||'</inreproc>'||chr(10)||
