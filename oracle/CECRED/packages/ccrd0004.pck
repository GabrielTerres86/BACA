...
--        EXIT WHEN vr_contador >= nvl(pr_nrregist,99999);
  ...
  BEGIN
    ...
    LOOP
      ...
      IF vr_dscritic IS NOT NULL THEN
        vr_dscritic := 'Erro ao buscar o '||vr_dscritic||' da linha '||vr_nrlinha||' do arquivo '||pr_nmarquivo;
        RAISE vr_exc_saida;
      END IF;
      ...
      BEGIN
        ...
        VALUES
          (vr_nrsequencia
          ,rw_tabela.nrseq_sicoob 
          ,rw_tabela.nmestabelecimento
          ,rw_tabela.nrcpfcgc         
          ,rw_tabela.tppessoa         
          ,rw_tabela.cdagebcb         
          ,rw_tabela.nrdconta         
          ,rw_tabela.tplancamento
          ,rw_tabela.tpproduto                 
          ,rw_tabela.vllancamento     
          ,rw_tabela.dtreferencia     
          ,pr_nmarquivo
          ,0
          ,SYSDATE);
      EXCEPTION
  ...
  BEGIN
    ...
    FOR rw_tabela IN cr_tabela LOOP
      ...
      BEGIN
        ...
        IF rw_tabela.dtreferencia < rw_crapdat.dtmvtolt THEN
          vr_dserro := 'Data do lancamento ('||to_char(rw_tabela.dtreferencia,'DD/MM/YYYY')||') inferior a data do sistema ('||
                                               to_char(rw_crapdat.dtmvtolt,'DD/MM/YYYY')||').';
          RAISE vr_erro;
        ELSIF NOT vr_crapcop.exists(rw_tabela.cdagebcb) THEN
