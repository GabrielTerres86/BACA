...
PROCEDURE pc_confirmar_novo_limite(pr_cdcooper    IN crapcop.cdcooper%TYPE --> Código da Cooperativa
                                  ...
                                  ,pr_insitlim    IN craplim.insitlim%TYPE --> Situação do Limite
                                  ,pr_insitest    IN craplim.insitest%TYPE --> Situação da Análise
                                  ,pr_insitapr    IN craplim.insitapr%TYPE --> Decisão                                  
                                  --------> OUT <--------
                                  ,pr_cdcritic OUT PLS_INTEGER          --> Codigo da critica
                                  ,pr_dscritic OUT VARCHAR2             --> Descricao da critica
                                  );
...
BEGIN
  ...
  BEGIN
    ...
    if  pr_cddopera = 0 then
        ...
        -- vr_mensagem_01 := 'teste mensagem 01';
        -- vr_mensagem_02 := 'teste mensagem 02';
        -- vr_mensagem_03 := 'teste mensagem 03';
        -- vr_mensagem_04 := 'teste mensagem 04';
        -- vr_mensagem_05 := 'teste mensagem 05';
          
        --  Se houver alguma inconsistencia, emitir mensagem para o usuario
        ...
        -- Se não houver nenhuma Mensagem/Inconsistência, efetuar o processo de Confirmação do novo Limite
        ELSE 
           pc_confirmar_novo_limite(pr_cdcooper => vr_cdcooper
                                   ...
                                   ,pr_insitlim => 2 -- Situação do Limite  = ATIVO
                                   ,pr_insitest => 3 -- Situação da Análise = ANALISE FINALIZADA
                                   ,pr_insitapr => NULL
                                   ,pr_cdcritic => vr_cdcritic
                                   ,pr_dscritic => vr_dscritic);
            --  Se ocorrer algum erro
            ...
            pr_retxml := XMLTYPE.CREATEXML('<?xml version="1.0" encoding="ISO-8859-1" ?><Root><Dados>Novo Limite de Desconto de Título Confirmado com Sucesso.</Dados></Root>');
        END IF;    
    ...
    --  Se houver Contigência de Motor e/ou Esteira, será efetuada a Confirmação do novo Limite na situação de Contigência.
        ...
        if  vr_em_contingencia_ibratan then
          ...
          pc_confirmar_novo_limite(pr_cdcooper => vr_cdcooper
                                   ...
                                   ,pr_insitlim => 2    -- Situação do Limite  = ATIVO
                                   ,pr_insitest => 3    -- Situação da Análise = ANALISE FINALIZADA
                                   ,pr_insitapr => NULL -- Decisão             = (Depende do Retorno da Análise IBRATAN)
                                   ,pr_cdcritic => vr_cdcritic
                                   ,pr_dscritic => vr_dscritic);
          --  Se ocorrer algum erro
          IF  vr_dscritic IS NOT NULL THEN
...
BEGIN
  ...
  BEGIN
   ...
   close cr_craplim;
      ...
      BEGIN
        UPDATE craplim
           SET insitlim = pr_insitlim                -- Situação do Limite
              ,insitest = pr_insitest                -- Situação da Análise
...
BEGIN
  ...
  BEGIN
      ...
      BEGIN
        UPDATE craplim
           SET insitlim = 7 -- Situação do Limite  = REJEITADO
              ,insitest = 3 -- Situação da Análise = ANALISE FINALIZADA
    ...
    ELSE
      ...
      BEGIN
        UPDATE craplim
           SET insitlim = 7 -- Situação do Limite  = REJEITADO
              ,insitest = 3 -- Situação da Análise = ANALISE FINALIZADA  
              ,insitapr = 5 -- Decisão             = REJEITADO AUTOMATICAMENTE
         WHERE cdcooper = vr_cdcooper
           AND nrdconta = pr_nrdconta
           AND nrctrlim = pr_nrctrlim
           AND tpctrlim = 3; -- 3 = Título
    ...
    -- Criar cabecalho do XML
    -- pr_retxml := XMLTYPE.CREATEXML('<?xml version="1.0" encoding="ISO-8859-1" ?><Root><Dados>OK</Dados></Root>');
    pr_retxml := XMLTYPE.CREATEXML('<?xml version="1.0" encoding="ISO-8859-1" ?><Root><Dados>Negação da Proposta executada com Sucesso.</Dados></Root>');
