...
procedure pc_incluir_bordero_esteira(pr_cdcooper  in crapbdt.cdcooper%type     --> Codigo da cooperativa
  ...
  cursor cr_aciona_retorno(pr_dsprotocolo varchar2) is
    ...
    and    ac.nrctrprp      = pr_nrborder  --> alterado para ser o bordero, verifivcar
    and    ac.dsprotocolo   = pr_dsprotocolo
    and    ac.tpacionamento = 2; -- Tipo do acionamento /* 1 - Envio, 2 – Retorno */
  --> Somente Retorno
  vr_dsconteudo_requisicao tbgen_webservice_aciona.dsconteudo_requisicao%TYPE;
    
  --> Hora de Envio
  vr_hrenvapr crapbdt.hraprova%TYPE;
...
--                                ,pr_bordero_clob out CLOB -- ANDREW: APENAS PARA TESTES
  ...
  vr_vlborder_lqd craptdb.vlliquid%TYPE;
  ...
  BEGIN
     ...
     if    cr_craptdb%notfound then
       ...
       vr_vlborder_lqd := rw_craptdb.vl_border_lqd; -- valor total líquido do borderô (soma valores líquidos de títulos desse borderô)
   ...
   --> Criar o CLOB para converter JSON para CLOB
   -- ANDREW:Apenas para testes
/*   dbms_lob.createtemporary(vr_obj_bordero_clob, TRUE, dbms_lob.CALL);
   dbms_lob.open(vr_obj_bordero_clob, dbms_lob.lob_readwrite);
   json.to_clob(vr_obj_bordero,vr_obj_bordero_clob);
   pr_bordero_clob := vr_obj_bordero_clob;*/
