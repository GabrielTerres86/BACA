...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_MANPAC AS
    ...
    IF UPPER(pr_cddopcao) <> 'D' THEN
      ...
      IF pr_cddopcao = 'D' THEN
        IF rw_tbtarif_pacotes.flgsituacao = 0 THEN 
          vr_dscritic := 'Pacote de tarifas desativado.'; 
          RAISE vr_exc_erro;    
        END IF;
      END IF;
      ...
      IF cr_crapfco%NOTFOUND THEN
        CLOSE cr_crapfco; 
        vr_dscritic := 'Valor do pacote de tarifas não está cadastrado.';
       RAISE vr_exc_erro;
      ELSE
        CLOSE cr_crapfco; 
    ...
    IF pr_cddopcao = 'D' THEN
      BEGIN
        ...
        SET
          ...
          tbtarif_pacotes_coop.dtcancelamento = SYSDATE
        WHERE 
  ...
  BEGIN
    ...
    IF cr_tbtarif_pacotes_coop%FOUND THEN
      -- Fecha cursor    
      CLOSE cr_tbtarif_pacotes_coop;
      vr_dscritic := 'Operação não efetuada! Pacote de tarifas já está ativo na cooperativa.';
      RAISE vr_exc_erro;
    ELSE
      -- Fecha cursor    
      CLOSE cr_tbtarif_pacotes_coop;  
    ...
    ELSE
      ...
      IF rw_tbtarif_pacotes.flgsituacao = 0 THEN
        vr_dscritic := 'Pacote de tarifas desabilitado.';
        RAISE vr_exc_erro;
      END IF;

      IF rw_tbtarif_pacotes.contador > 1 THEN
        vr_dscritic := 'Não foi possível obter o valor da tarifa ' || TO_CHAR(rw_tbtarif_pacotes.cdtarifa_lancamento) || '. As tarifas incluídas no pacote devem ser cadastradas com uma única faixa de valor.';
        RAISE vr_exc_erro;
      END IF; 
    END IF;
    ...
    IF TRIM(pr_dtvigenc) IS NULL THEN
      vr_dscritic := 'Informe a Data de inicio da vigência do pacote.';
    ...
    IF NVL(rw_tbtarif_pacotes.vlpacote,0) > NVL(rw_vlr_tbtarif.vltarifa,0) THEN
      vr_dscritic := 'Operação não efetuada! O valor da tarifa do pacote não pode ser superior a soma das tarifas individuais dos serviços contemplados no pacote.';
      RAISE vr_exc_erro; 
    END IF;
    ...
    IF pr_cddopcao = 'H' THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          vr_dscritic := 'Erro ao inserir registro de pacote de tarifas. Erro: ' || SQLERRM;
          RAISE vr_exc_erro;
      END;
    END IF;
  ...
  BEGIN
    ...
    IF pr_cdpctant = pr_cdpctnov THEN
      vr_dscritic := 'Código do novo pacote deve ser diferente do pacote atual.';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF cr_tbtarif_pacotes%NOTFOUND THEN
      CLOSE cr_tbtarif_pacotes;
      vr_dscritic := 'Pacote de tarifas não encontrado(Antigo).';
      RAISE vr_exc_erro;
    ELSE
      CLOSE cr_tbtarif_pacotes;
    ...
    IF cr_tbtarif_pacotes%NOTFOUND THEN
      CLOSE cr_tbtarif_pacotes;
      vr_dscritic := 'Pacote de tarifas não encontrado(Novo).';
      RAISE vr_exc_erro;
    ELSE
      CLOSE cr_tbtarif_pacotes;
    ...
    IF vr_tippesnv <> vr_tippesan THEN
      vr_dscritic := 'Operação não efetuada! Tipo de pessoa do novo pacote não confere com o pacote atual.';
      RAISE vr_exc_erro;
    END IF;
  
    -- Verifica se novo pacote tem valor menor ou igual ao antigo
    IF vr_vlrpctnv > vr_vlrpctan THEN
      vr_dscritic := 'Operação não efetuada! Valor do novo pacote é superior ao valor do pacote de tarifas atual.';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF cr_tbtarif_servicos%NOTFOUND THEN
      ...
      vr_dscritic := 'Operação não efetuada! Não existem serviços cadastrados para o pacote informado.';
      RAISE vr_exc_erro;
    ELSE                            
      CLOSE cr_tbtarif_servicos;
    ...
    IF vr_qtdtaran > vr_qtdtarnv THEN
      vr_dscritic := 'Operação não efetuada! O novo pacote não contempla todos os serviços do pacote atual.';
      RAISE vr_exc_erro;
    END IF;
    ...
    FOR rw_tbtarif_servicos_ant IN cr_tbtarif_servicos_ant(pr_cdpacote => pr_cdpctant) LOOP
      ...
      IF cr_tbtarif_servicos_nov%NOTFOUND THEN
        ...
        vr_dscritic := 'Operação não efetuada! Os serviços do novo pacote não contemplam todos os serviços do pacote atual.';
        RAISE vr_exc_erro;        
      ELSE
        CLOSE cr_tbtarif_servicos_nov;
        ...
        IF rw_tbtarif_servicos_nov.qtdoperacoes < rw_tbtarif_servicos_ant.qtdoperacoes THEN
          vr_dscritic := 'Operação não efetuada! Qtd. de operações isentas do novo pacote é inferior a qtd. de operações do pacote atual.';
          RAISE vr_exc_erro;  
        END IF;
      END IF;
    ...
    IF pr_cddopcao = 'D' THEN
      ...
      FOR rw_tbtarif_contas_pacote IN cr_tbtarif_contas_pacote(pr_cdcooper => vr_cdcooper
        ...
        BEGIN
          ...
          SET
            tbtarif_contas_pacote.dtcancelamento = SYSDATE,
            tbtarif_contas_pacote.cdoperador_cancela = vr_cdoperad
          WHERE 
            tbtarif_contas_pacote.rowid = rw_tbtarif_contas_pacote.rowid;
        EXCEPTION
          WHEN OTHERS THEN
            vr_dscritic := 'Erro ao desativar pacote de tarifas do cooperado, conta: ' || TO_CHAR(rw_tbtarif_contas_pacote.nrdconta) || '. Erro: ' || SQLERRM;
            RAISE vr_exc_erro;       
        END;
        ...
        BEGIN
          INSERT INTO 
           ...
           VALUES(
            ...
            ,rw_tbtarif_contas_pacote.qtdmeses_desconto - (TRUNC(MONTHS_BETWEEN(SYSDATE,rw_tbtarif_contas_pacote.dtinicio_vigencia)) + 1)
            ,rw_tbtarif_contas_pacote.cdreciprocidade
            ,vr_cdoperad);
        EXCEPTION
          WHEN OTHERS THEN
            vr_dscritic := 'Erro ao inseir novo pacote de tarifas do cooperado, conta: ' || TO_CHAR(rw_tbtarif_contas_pacote.nrdconta) || '. Erro: ' || SQLERRM;
            RAISE vr_exc_erro;       
        END;

        vr_dsdmensg := 'Caro cooperado, o pacote de servi&ccedil;os ' || TO_CHAR(pr_cdpctant) || '-' || vr_dspctant || ' foi descontinuado pela Cooperativa
                        e substitu&iacute;do pelo pacote ' || TO_CHAR(pr_cdpctnov) || '-' || vr_dspctnov ||'. 
                        O pacote de servi&ccedil;os ' || TO_CHAR(pr_cdpctant) || '-' || vr_dspctant || ' estar&aacute; em vigor at&eacute; o &uacute;ltimo dia do m&ecirc;s corrente.
                        Sua migra&ccedil;&atilde;o para o novo pacote de servi&ccedil;os foi realizada automaticamente. 
        ...
        FOR rw_crapsnh IN cr_crapsnh(pr_cdcooper => vr_cdcooper
          ...
          GENE0003.pc_gerar_mensagem(pr_cdcooper => vr_cdcooper                       -- Cooperativa
                                    ...
                                    ,pr_dsdassun => 'Migração pacote de tarifas'      -- Assunto da mensagem
  ...
  BEGIN
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        pr_cdcritic := vr_cdcritic;
        pr_dscritic := 'Erro geral em Consulta de Pacotes de Tarifas TELA_MANPAC.PC_PESQUISA_MANPAC_CAR: ' || SQLERRM;
    END;
