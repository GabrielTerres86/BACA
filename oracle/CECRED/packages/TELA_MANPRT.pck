  ...
  CURSOR cr_titulos(pr_dtmvtolt IN tbfin_recursos_movimento.dtmvtolt%TYPE,
        ...
        SELECT ret.idretorno,
         ...
         WHERE /*ret.dtconciliacao IS NULL
           ...
           AND (cart.nmcartorio = cart.nmcartorio AND cart.nrcpf_cnpj = cart.nrcpf_cnpj);
...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_MANPRT IS
  ...
  BEGIN
    ...
    vr_conteudo := 'A TED ' || rw_tbcobran_ted.nrdocmto || ' recebida no dia ' || rw_tbcobran_ted.dtmvtolt;

    -- Chamar envio c/ dados do retorno
    COBR0011_AUGUSTO.pc_enviar_ted_IEPTB(pr_cdcooper => pr_cdcooper
    ...
    ' pela TED ' || rw_tbcobran_ted.nrdocmto || '\nMotivo: ' || pr_motivo ||
    '\nDescrição: ' || pr_descricao;
  ...
  PROCEDURE pc_gera_conciliacao(pr_idlancto   IN tbfin_recursos_movimento.idlancto%TYPE -- ID da TED
    ...
    CURSOR cr_titulos (pr_idsretorno IN VARCHAR2) IS
      SELECT ret.idretorno,
             ret.dtconciliacao,
             ret.vltitulo,
             ret.dtocorre
  ...
  BEGIN
    ...
    FOR rw_titulo IN cr_titulos(pr_idsretorno => pr_idsretorno) LOOP
      
      IF rw_titulo.dtconciliacao IS NOT NULL THEN
      ...
      IF rw_titulo.vltitulo > rw_ted.vllanmto THEN
      ...
      IF rw_titulo.dtocorre < rw_ted.dtmvtolt THEN
        vr_dscritic := 'Data de recebimento do título deve ser superior ou igual a TED. Favor verificar!';
        RAISE vr_exc_saida;
      END IF;

      vr_sum_titulos := vr_sum_titulos + rw_titulo.vltitulo;
      ...
      BEGIN
        UPDATE tbcobran_retorno_ieptb
         ...
         WHERE idretorno = rw_titulo.idretorno;
      EXCEPTION
        WHEN OTHERS THEN
...
/*    pr_retxml := xmltype('<?xml version="1.0" encoding="ISO-8859-1"?>
  ...
  BEGIN
    ...
    vr_conteudo := 'Custas estornadas referente ao boleto: ' ||
    '\nCooperativa: ' || rw_tbcobran_retorno.cdcooper ||
    '\nConta: ' || rw_tbcobran_retorno.nrdconta ||
    '\nConvenio: ' || rw_tbcobran_retorno.nrcnvcob ||
    '\nDocumento: ' || rw_tbcobran_retorno.nrdocmto ||
    '\nDevolvidos na TED ' || rw_tbcobran_ted.nrdocmto ||' no ' || ' do cartório ' || rw_tbcobran_ted.cdagenci_debitada;
    vr_email_dest := vr_emails_cobranca;
    --
