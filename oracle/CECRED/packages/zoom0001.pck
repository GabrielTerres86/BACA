...
PROCEDURE pc_busca_liquidacoes_web(pr_xmllog    IN VARCHAR2                --XML com informações de LOG
                                   ,pr_cdcritic  OUT PLS_INTEGER            --Código da crítica
                                   ,pr_dscritic  OUT VARCHAR2               --Descrição da crítica
                                   ,pr_retxml    IN OUT NOCOPY XMLType      --Arquivo de retorno do XML
                                   ,pr_nmdcampo  OUT VARCHAR2               --Nome do Campo
...
/*---------------------------------------------------------------------------------------------------------------
  ...
  /*---------------------------------------------------------------------------------------------------------------
    ...
    Programa : pc_busca_liquidacoes_web
    Sistema  : Conta-Corrente - Cooperativa de Credito
    Sigla    : CRED
    Autor    : Diego Simas
    Data     : Janeiro/2018                          Ultima atualizacao:
    ...
    Frequencia: -----
    Objetivo   : Pesquisa que retorna os tipos de liquidações do empréstimo.
  ...
  BEGIN
    ...
    gene0002.pc_escreve_xml(pr_xml            => vr_clob
      ...
      -- Carrega os dados -- Tipos de liquidação
      gene0002.pc_escreve_xml(pr_xml            => vr_clob
                             ,pr_texto_completo => vr_xml_temp
                             ,pr_texto_novo     => '<linha>'||
                                                   '  <idtipliq>1</idtipliq>'||
                                                   '  <dstipliq>Sem Liquidações</dstipliq>'||
                                                   '</linha>');
      gene0002.pc_escreve_xml(pr_xml            => vr_clob
                             ,pr_texto_completo => vr_xml_temp
                             ,pr_texto_novo     => '<linha>'||
                                                   '  <idtipliq>2</idtipliq>'||
                                                   '  <dstipliq>Empréstimo</dstipliq>'||
                                                   '</linha>');
      gene0002.pc_escreve_xml(pr_xml            => vr_clob
                             ,pr_texto_completo => vr_xml_temp
                             ,pr_texto_novo     => '<linha>'||
                                                   '  <idtipliq>3</idtipliq>'||
                                                   '  <dstipliq>Limite de Crédito/ADP</dstipliq>'||
                                                   '</linha>');
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      -- Erro
      pr_cdcritic:= 0;
      pr_dscritic:= 'Erro na pc_busca_liquidacoes_web --> '|| SQLERRM;

      -- Existe para satisfazer exigência da interface.
      pr_retxml := XMLType.createXML('<?xml version="1.0" encoding="ISO-8859-1" ?> ' ||
                                     '<Root><Erro>' || pr_cdcritic||'-'||pr_dscritic || '</Erro></Root>');

  END pc_busca_liquidacoes_web;                       
