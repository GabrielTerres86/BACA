CREATE OR REPLACE PACKAGE CECRED.TELA_GRAVAM AS
  ...
  PROCEDURE pc_processa_retorno(pr_cdcoptel   IN INTEGER            --> 0- Não traz a opção TODAS / 1 - Traz a opção TODAS      
                               ,pr_tparquiv   IN VARCHAR2           --> Tipo do arquivo                      
                               ,pr_cddopcao   IN VARCHAR2           --> Opção da tela
                               ,pr_dsdepart   IN VARCHAR2           --> Departamento do operador
                               ,pr_xmllog     IN VARCHAR2           --> XML com informações de LOG
...
/*---------------------------------------------------------------------------------------------------------------
  ...
  PROCEDURE pc_processa_retorno(pr_cdcoptel   IN INTEGER            --> 0- Não traz a opção TODAS / 1 - Traz a opção TODAS      
    ...
    Frequencia: Sempre que for chamado
    Objetivo  : Rotina para Importacao de arquivo RETORNO

    Alteracoes: 14/07/2016 - Ajuste para retirar o parametro pr_cdoprad (Andrei - RKAM).                          
    ............................................................................. */
    -- Variaveis de locais
    vr_cdcooper crapcop.cdcooper%TYPE;
    vr_cdoperad VARCHAR2(100);
  ...
  BEGIN
    ...
    GRVM0001.pc_gravames_processa_retorno(pr_cdcooper  => vr_cdcooper -- Cooperativa conectada
                                         ,pr_cdcoptel  => pr_cdcoptel -- Opção selecionada na tela
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN 
      ...
      -- Erro
      pr_cdcritic:= 0;
      pr_dscritic:= 'Erro na pc_processa_retorno --> '|| SQLERRM;
      pr_des_erro:= 'NOK';
