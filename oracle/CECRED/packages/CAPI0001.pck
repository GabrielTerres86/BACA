...
---------------------------------------------------------------------------------------------------------
  ...
  PROCEDURE pc_cancela_integralizacao(pr_nrdrowid IN ROWID
...
CREATE OR REPLACE PACKAGE BODY CECRED.capi0001 IS
  ...
  cd_craplst_cdhist          CONSTANT INTEGER := 61;
  cd_craplcm_cdhist          CONSTANT INTEGER := 127;
  cd_craplst_cdhist_internet CONSTANT NUMBER(5) := 2138;
  ...
  BEGIN
    ...
    IF pr_idorigem = vc_internet THEN
      ...
      IF pr_vlintegr > vr_vlmaximo_integralizacao THEN
        ...
        RAISE vr_exc_erro;
      END IF;
      vr_craplct_cdhist := cd_craplst_cdhist_internet;
      vr_craplcm_cdhist := cd_craplcm_cdhist_internet;
    ELSE
      vr_craplct_cdhist := cd_craplst_cdhist;
  ...
  PROCEDURE pc_cancela_integralizacao(pr_nrdrowid IN ROWID
    ...
    -- Cursor para encontrar a conta/corrente
    CURSOR cr_craplct(pr_nrdrowid IN ROWID) IS
      SELECT cdcooper
            ,nrdconta
            ,dtmvtolt
            ,nrdocmto
  ...
  BEGIN
     ...
     CLOSE cr_craplct;
      ...
      IF cr_craplct%NOTFOUND THEN
         CLOSE btch0001.cr_crapdat;
         vr_dscritic := 'Não foi possível consultar lançamento.';
         RAISE vr_exc_erro;
      ELSE
          CLOSE cr_craplct;
      END IF;     
    ...
    END IF;     
     ...
     -- Valida o valor das cotas
      EXTR0001.pc_ver_capital(pr_cdcooper => pr_cdcooper -- Código da cooperativa
                             ,pr_cdagenci => vc_cdagenci         -- Código da agência
     ...
     -- Verifica o histórico a ser utilizado de acordo com a origem (IB/Ayllos)     
     IF pr_idorigem = vc_internet THEN
    ...
    -- Log de item
    GENE0001.pc_gera_log_item (pr_nrdrowid => pr_nrdrowid
                              ,pr_nmdcampo => 'Documento'
                              ,pr_dsdadant => ' '
