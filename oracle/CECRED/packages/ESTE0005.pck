...
CREATE OR REPLACE PACKAGE BODY CECRED.ESTE0005 IS
  ...
  BEGIN  
    ...
    pc_verifica_regras_esteira(pr_cdcooper  => pr_cdcooper,  --> Codigo da cooperativa                                        
                               ...
                               pr_nrctremp  => pr_nrctrcrd,  --> Numero da proposta de cartão
  ...
  BEGIN
    ...
    pc_carrega_param_ibra(pr_cdcooper      => pr_cdcooper,                   -- Codigo da cooperativa
                          pr_nrdconta      => pr_nrdconta,                   -- Numero da conta do cooperado
                          pr_nrctrcrd      => pr_nrctrcrd,                   -- Numero da proposta de emprestimo
  ...
  BEGIN
    ...
    BEGIN
      ...
      vr_obj_generic2.put('limiteCartaoCredit',este0001.fn_decimal_ibra(vr_vltotccr));
...
GENE0001.pc_set_modulo(pr_module => 'ESTE0005', pr_action => 'OI '||rw_crapepr.nrctremp||' data '||rw_crapepr.dtmvtolt);
 ...
 GENE0001.pc_set_modulo(pr_module => 'ESTE0005', pr_action => 'fim Risco '||pr_nrdconta);
      ...
      ELSE
        CLOSE cr_rencomp;
        vr_obj_generic2.put('teveRenegociacao',rw_rencomp.temrenegociacao);
        vr_obj_generic2.put('teveComposicao',rw_rencomp.temcomposicao);
      END IF;
      ...
      --Tabela para tratar Cartões de Crédito
      vr_obj_generic2.put('limiteAtualCartaoCecred',0);
      vr_obj_generic2.put('codigoCatAualCartaoCecred',' ');
      vr_obj_generic2.put('descricaoCatAtualCartaoCecred',' ');
      vr_obj_generic2.put('contaCartao',' ');
      ...
      -- Adicionar o array seguro no objeto informações adicionais
      vr_obj_generic2.put('categoriasCartaoCecred', vr_lst_generic3);
  ...
  BEGIN
    ...
    vr_obj_generico.put('valorEmprest',' ');
    vr_obj_generico.put('valorParcela',' ');
    vr_obj_generico.put('quantParcela',' ');
    ...
    vr_obj_generico.put('qualificaOperacaoCodigo',' ');
  ...
  BEGIN
    ...
    IF cr_crapass%FOUND THEN
      ...
      IF gene0001.fn_param_sistema('CRED',pr_cdcooper,'CONTIGENCIA_ESTEIRA_IBRA') = 1 THEN
        vr_dscritic := 'Motor está em contingência.';
        raise vr_exc_erro;
      END IF;
      ...
      pc_gera_json_motor(pr_cdcooper  => pr_cdcooper,         --> Codigo da cooperativa
                         ...
                         pr_nrdconta  => pr_nrdconta,         --> Numero da conta do cooperado
                         pr_nrctrcrd  => null,         --> Numero da proposta do cartão
                         pr_tpproces  => 'I',--pr_tpproces,         --> Tipo Processo (I-Inclusão, A-Alteração)
                         pr_cdoperad  => pr_cdoperad,
                         ---- OUT ----
      ...
      ELSE
        ...
        pr_dsmensag := 'Resultado da Avaliação: <b>Finalizada</b> com situação indefinida!';
  ...
  ..........................................................................*/
    ...
    vr_codcateg      VARCHAR2(100);
    vr_descateg      VARCHAR2(100);
    vr_vllimite      VARCHAR2(100);
    ...
    FOR rw_crapcop IN cr_crapcop LOOP
      ...
      IF rw_crapdat.inproces = 1 THEN
        ...
        FOR rw_sugret IN cr_sugret LOOP
          ...
          pc_carrega_param_ibra(pr_cdcooper      => pr_cdcooper, -- Codigo da cooperativa
                                pr_nrdconta      => pr_nrdconta, -- Numero da conta do cooperado
                                pr_nrctrcrd      => NULL, -- Numero da proposta de emprestimo
          ...
          ELSE
            ...
            -- Buscar Sugestões de Cartão Cecred
            IF vr_obj_retorno.exist('sugestaoCartaoCecred') THEN
              
              vr_obj_indicadores := json(vr_obj_retorno.get('sugestaoCartaoCecred'));
              
              -- Código da Categoria -- 
              IF vr_obj_indicadores.exist('codigoCategoria') THEN
                vr_codcateg := ltrim(rtrim(vr_obj_indicadores.get('codigoCategoria').to_char(),'"'),'"');
              END IF;
              
              -- Descrição Categoria -- 
              IF vr_obj_indicadores.exist('descricaoCategoria') THEN
                vr_descateg := ltrim(rtrim(vr_obj_indicadores.get('descricaoCategoria').to_char(),'"'),'"');
              END IF;
              
              -- Valor do Limite -- 
              IF vr_obj_indicadores.exist('vlLimite') THEN
                vr_vllimite := ltrim(rtrim(vr_obj_indicadores.get('vlLimite').to_char(),'"'),'"');
              END IF;
  ...
  /* ..........................................................................
    ...
    CURSOR cr_crawcrd (pr_cdcooper crawcrd.cdcooper%TYPE,
      ...
      SELECT crd.nrctrcrd,
        ...
        FROM crawcrd crd,
             crapope ope
       WHERE crd.cdcooper = ope.cdcooper
         AND upper(crd.cdoperad) = upper(ope.cdoperad)
  ...
  BEGIN
    ...
    vr_data_aux := to_date(to_char(rw_crawcrd.dtmvtolt,'DD/MM/RRRR HH24:MI:SS'),'DD/MM/RRRR HH24:MI:SS');
    ...
    vr_obj_proposta.put('produtoCreditoSegmentoCodigo','');
    vr_obj_proposta.put('produtoCreditoSegmentoDescricao','');
    ...
    vr_obj_proposta.put('tipoProduto'           ,'');
    ...
    vr_obj_proposta.put('parecerPreAnalise',2);
...
--> Verificar com o Anderson      
  ...
  BEGIN    
    ...
    BEGIN
      UPDATE crawcrd wpr 
         SET wpr.dsprotoc = nvl(vr_dsprotoc,' ')
