...
/*
    ...
    Data    : Outubro/2009                    Ultima Atualizacao: 07/11/2017
    ...
    Alteracoes: 26/06/2010 - Alterada BO para tratar conta com digito verificador
				...
				07/11/2017 - Ajuste para atualizar IFs inativas na procedure 
							 pc_proc_pag0101. (Reinert)
..............................................................................*/
    ...
    DECLARE 
      ...
      rw_crapban_bb cr_crapban_bb%ROWTYPE;
		  ...
		  CURSOR cr_situacao_if IS
			...
			-- Verificar IFs ativas
			CURSOR cr_ver_ban IS
			  SELECT ban.nrispbif
				      ,ban.cdbccxlt
				  FROM crapban ban
				 WHERE ban.flgoppag = 1; -- Ativa
			
			-- Verificar se Número ISPB existe no arquivo
			CURSOR cr_ver_if(pr_nrispbif crapban.nrispbif%TYPE) IS
				SELECT * 
				  FROM (SELECT extractvalue(column_value, '/dados/nrispbif') nrispbif
							        ,extractvalue(column_value, '/dados/cdsitope') cdsitope
						      FROM TABLE(xmlsequence(xmltype(pr_clobxml).extract('/root/dados'))) t) a
			   WHERE a.nrispbif = pr_nrispbif; 
			rw_ver_if cr_ver_if%ROWTYPE;
...
/*					IF SQL%ROWCOUNT = 0 THEN
      ...
      -- Devemos percorrer as IFs para verificar IFS inativas
      FOR rw_ver_ban IN cr_ver_ban LOOP
				
			  -- Devemos desprezar IFs com ISPB = 0 que não sejam do BB
			  IF rw_ver_ban.nrispbif = 0  AND 
					 rw_ver_ban.cdbccxlt <> 1 THEN
					-- Buscar próxima IF
			    CONTINUE;
        END IF;
				
			  -- Verificar se IF está no arquivo
			  OPEN cr_ver_if(pr_nrispbif => rw_ver_ban.nrispbif);
				FETCH cr_ver_if INTO rw_ver_if;
				
				-- Se não encontrar, devemos inativar IF
				IF cr_ver_if%NOTFOUND THEN
					-- Atualizar registro da IF
					UPDATE crapban ban
						 SET ban.dtaltpag = trunc(SYSDATE)
						    ,ban.flgoppag = 0
					 WHERE ban.nrispbif = rw_ver_ban.nrispbif;
				END IF;
				-- Fechar cursor
				CLOSE cr_ver_if;				
      END LOOP;
