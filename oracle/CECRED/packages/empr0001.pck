      ...
      BEGIN
        ...
        FOR rw_crappep IN cr_crappep LOOP
          ...
          IF NOT pr_dtmvtolt <= rw_crawepr.dtlibera THEN
            ...
            pr_tab_pgto_parcel(vr_ind_pag).vlatrpag := NVL(pr_tab_pgto_parcel(vr_ind_pag).vlatupar
                                                          ,0) +
                                                       NVL(pr_tab_pgto_parcel(vr_ind_pag).vlmtapar
                                                          ,0) +
                                                       NVL(pr_tab_pgto_parcel(vr_ind_pag).vlmrapar
                                                          ,0);
            -- Saldo para relatorios
            vr_vlsderel := vr_vlsderel + pr_tab_pgto_parcel(vr_ind_pag).vlatupar;
            -- Saldo devedor total do emprestimo
            vr_vlsdeved := vr_vlsdeved + pr_tab_pgto_parcel(vr_ind_pag).vlatrpag;
          END IF;
        END LOOP;
        ...
        /*** FIM - PORTABILIDADE - Verifica se eh uma proposta de portabilidade **/
          ...
          IF pr_tab_dados_epr(vr_indadepr).inprejuz > 0 THEN
            ...
            pr_tab_dados_epr(vr_indadepr).vlsdprej := nvl(rw_crapepr.vlsdprej
                                                         ,0) +
                                                      (pr_tab_dados_epr(vr_indadepr).vlttmupr - pr_tab_dados_epr(vr_indadepr)
                                                       .vlpgmupr) +
                                                      (pr_tab_dados_epr(vr_indadepr).vlttjmpr 
                                                      - pr_tab_dados_epr(vr_indadepr).vlpgjmpr);
