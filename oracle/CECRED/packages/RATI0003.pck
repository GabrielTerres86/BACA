...
---------------------------- ESTRUTURAS DE REGISTRO ---------------------
 ...
 /* Gravar o Rating da operação que retornou do Ibratan. */
  ...
  /* Atualizar o Rating da operação para a Ibratan. */
...
CREATE OR REPLACE PACKAGE BODY CECRED.RATI0003 IS
     ...
     BEGIN
       ...
       *****************************************************/
       ...
       IF pr_cdcooper <> 3 THEN
         ...
         IF vr_inconting = 1 AND NVL(pr_strating,0) > 0 THEN --- em contigencia (1-Sim 0-Não) (P450 - Rating)
  ...
  /* Atualizar o Rating da operação na Ibratan ou conforme a Central de Risco do dia anterior. */
  PROCEDURE pc_atualiza_rating_naousar(pr_cdcritic OUT crapcri.cdcritic%TYPE  --> Critica encontrada no processo
                              ,pr_dscritic OUT VARCHAR2) IS           --> Descritivo do erro
  ...
  Frequencia: Sempre que chamado.
  Objetivo  : Atualizar o Rating da operação na Ibratan ou conforme a Central de Risco do dia anterior.
...
BEGIN
  IF pr_rating_sugerido IS null then
    vr_inorigem_rating := 1; -- SE NÃO SUGERIR NADA ELE VAI PARA AUTOMATICO(MOTOR) ;
  ELSE
...
/*
    pc_verifica_endivid_param(pr_cdcooper     => pr_cdcooper        --> Código da Cooperativa
      ...
      */
      IF pr_tpctrato IN (1,2,91,92) THEN
        ...
        IF pr_tpctrato = 91 THEN
          ...
          vr_obj_indcliente.put('produtoCreditoSegmentoDescricao', 'Borderôs de descontos de Títulos');
        ELSIF pr_tpctrato = 92 THEN
          vr_obj_indcliente.put('produtoCreditoSegmentoCodigo', 92);
          vr_obj_indcliente.put('produtoCreditoSegmentoDescricao', 'Borderôs de descontos de Cheques');
        END IF;
...
--crapepr  -- tabela emprestimo
 ...
 --    ww_teste := vr_tab_risco_op.count;
  ...
  -- Parametro de Flag Rating Lote Ativo: 0-Não Ativo, 1-Ativo
  ...
  BEGIN
    ...
    FOR rw_crapcop IN cr_crapcop LOOP
      ...
      -- Limpa atualização SAS para processamento
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            GENE0001.pc_gera_log(pr_cdcooper => rw_crapcop.cdcooper
                                ,pr_cdoperad => 'MOTOR'
                                ,pr_dscritic => 'Erro ao Limpar atualização SAS para processamento. '||sqlerrm
                                ,pr_dsorigem => 'SAS'
      ...
      LOOP
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            GENE0001.pc_gera_log(pr_cdcooper => rw_crapcop.cdcooper
                                ,pr_cdoperad => 'MOTOR'
                                ,pr_dscritic => 'Erro ao atualizar Flag Risco Operações (Parte 1). '||sqlerrm
                                ,pr_dsorigem => 'SAS'
                                ,pr_dstransa => 'Processa Rating Operações'
                                ,pr_dttransa => TRUNC(SYSDATE)
                                ,pr_flgtrans => 1 --> FALSE
