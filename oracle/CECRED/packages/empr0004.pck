...
CREATE OR REPLACE PACKAGE BODY CECRED.EMPR0004 AS
  ...
  /* -------------------------------------------------------------------------------------------------------------
    ...
    Programa: EMPR0004                     Antigo: sistema/generico/procedures/b1wgen0084.p
    Autor   : Irlan
    Data    : Fevereiro/2011               ultima Atualizacao: 08/04/2015
...
..............................................................................*/
  ...
  ---------------------------------------------------------------------------------------------------------------
    ...
    --Subrotina para gerar as parcelas da proposta de emprestimo 
    PROCEDURE pc_gera_parcelas_emprest    (pr_cdcooper    IN crapcop.cdcooper%TYPE       --Codigo Cooperativa
                                          ,pr_cdagenci    IN crapass.cdagenci%TYPE       --Codigo Agencia
                                          ,pr_nrdcaixa    IN INTEGER                     --Numero do Caixa
                                          ,pr_cdoperad    IN VARCHAR2                    --Codigo Operador
                                          ,pr_nmdatela    IN VARCHAR2                    --Nome da Tela
                                          ...
                                          ,pr_flgerlog    IN BOOLEAN                     --Imprimir log
                                          ,pr_nrctremp    IN crapepr.nrctremp%TYPE       --Contrato Emprestimo
                                          ,pr_parcela_epr IN typ_tab_parcela_epr         -->Tipo de tabela com parcelas emprestimo
                                          ,pr_des_reto    OUT VARCHAR2                   --Retorno OK ou NOK
                                          ,pr_tab_erro    OUT gene0001.typ_tab_erro) IS  --Tabela de Erros

  ---------------------------------------------------------------------------------------------------------------
  --
  --  Programa : pc_gera_parcelas_emprest              Antigo: procedures/b1wgen0084.p/gera_parcelas_emprestimo
  --  Sistema  : 
  --  Sigla    : CRED
  --  Autor    : Alisson C. Berrido - Amcom
  --  Data     : Julho/2014                           Ultima atualizacao: 14/07/2014
  ...
  --
  -- Frequencia: -----
  -- Objetivo   : Procedure para gerar as parcelas da proposta de emprestimo  
  --
  -- Alterações : 14/07/2014 - Conversão Progress -> Oracle (Alisson - AMcom)
  --              
  ---------------------------------------------------------------------------------------------------------------
    ...
    BEGIN
      ...
      --Inicializar transacao
      vr_dsorigem:= gene0001.vr_vet_des_origens(pr_idorigem);
      vr_dstransa:= 'Gerar parcelas de emprestimo.';
      --Excluir parcelas do Emprestimo
      BEGIN
        DELETE FROM crappep
        WHERE crappep.cdcooper = pr_cdcooper   
        AND   crappep.nrdconta = pr_nrdconta   
      ...
      EXCEPTION
        WHEN OTHERS THEN
          vr_dscritic:= 'Erro ao excluir parcelas do emprestimo. '||sqlerrm;
          --Levantar Excecao
          RAISE vr_exc_erro;
      END;  
      --Percorrer parcelas da tabela de memoria
      vr_index:= pr_parcela_epr.FIRST;
      WHILE vr_index IS NOT NULL LOOP
        --Inserir na tabela de parcelas do emprestimo
        BEGIN
          INSERT INTO crappep
              (cdcooper
              ,nrdconta
          ...
          VALUES 
              ...
              ,pr_nrdconta                  
              ,pr_nrctremp                  
              ,pr_parcela_epr(vr_index).nrparepr 
              ,pr_parcela_epr(vr_index).vlparepr
              ,pr_parcela_epr(vr_index).vlparepr
              ,pr_parcela_epr(vr_index).vlparepr
              ,pr_parcela_epr(vr_index).dtparepr
              ,0);  
        EXCEPTION
          WHEN DUP_VAL_ON_INDEX THEN
            ...
            NULL;
          WHEN OTHERS THEN
            vr_dscritic:= 'Erro ao inserir parcela do emprestimo. '||sqlerrm;
            --Levantar Excecao
            RAISE vr_exc_erro;
        END;    
        --Proxima parcela
        vr_index:= pr_parcela_epr.NEXT(vr_index);  
      END LOOP;
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        pr_des_reto := 'NOK';
        -- Chamar rotina de gravação de erro
        vr_dscritic := 'Erro na pc_gera_parcelas_emprestimo --> '|| sqlerrm;
        gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper
                             ,pr_cdagenci => pr_cdagenci
                             ,pr_nrdcaixa => pr_nrdcaixa
                             ,pr_nrsequen => 1 --> Fixo
        ...
        IF pr_flgerlog THEN
          ...
          gene0001.pc_gera_log(pr_cdcooper => pr_cdcooper
                              ...
                              ,pr_nrdrowid => vr_nrdrowid);
        END IF;    
    END pc_gera_parcelas_emprest; 


    --Subrotina para calcular o valor do emprestimo
    PROCEDURE pc_calcula_emprestimo (pr_cdcooper    IN crapcop.cdcooper%TYPE       --Codigo Cooperativa
                                    ,pr_cdagenci    IN crapass.cdagenci%TYPE       --Codigo Agencia
                                    ,pr_nrdcaixa    IN INTEGER                     --Numero do Caixa
                                    ,pr_cdoperad    IN VARCHAR2                    --Codigo Operador
                                    ,pr_nmdatela    IN VARCHAR2                    --Nome da Tela
                                    ...
                                    ,pr_dtmvtolt    IN DATE                        --Data Movimento
                                    ,pr_dtdpagto    IN DATE                        --Data pagamento                                       
                                    ,pr_flggrava    IN BOOLEAN                     --Indicador gravacao
                                    ,pr_dtlibera    IN DATE                        --Data Liberacao
                                    ,pr_qtdiacar    OUT INTEGER                    --Quantidade Dias Carencia
                                    ,pr_vlajuepr    OUT NUMBER                     --Ajustado ao valor presente
                                    ,pr_txdiaria    OUT NUMBER                     --Taxa Diaria Juros
                                    ,pr_txmensal    OUT NUMBER                     --Taxa Mensal Juros                                    
                                    ,pr_parcela_epr OUT typ_tab_parcela_epr         -->Tipo de tabela com parcelas emprestimo
                                    ,pr_des_reto    OUT VARCHAR2                   --Retorno OK ou NOK
                                    ,pr_tab_erro    OUT gene0001.typ_tab_erro) IS  --Tabela de Erros
  ...
  ---------------------------------------------------------------------------------------------------------------
  --
  --  Programa : pc_calcula_emprestimo            Antigo: procedures/b1wgen0084.p/calcula_emprestimo
  --  Sistema  : 
  --  Sigla    : CRED
  --  Autor    : Alisson C. Berrido - Amcom
  --  Data     : Julho/2014                           Ultima atualizacao: 14/07/2014
  ...
  --
  -- Frequencia: -----
  -- Objetivo   : Procedure para calcular o valor do emprestimo
  --
  -- Alterações : 14/07/2014 - Conversão Progress -> Oracle (Alisson - AMcom)
  --              
  ---------------------------------------------------------------------------------------------------------------
      ...
      --tabela de Memoria para controlar parcelas
      vr_tab_datas_parcelas typ_tab_datas_parcelas;
      --Variaveis Locais
      ...
      vr_vlparepr NUMBER;
      vr_vlpreemp NUMBER;
      vr_txdiaria NUMBER(35,10); 
      vr_vlexpone NUMBER(35,10);
      vr_diapagto INTEGER;
      vr_mespagto INTEGER;
      vr_anopagto INTEGER;
      vr_dstransa VARCHAR2(100);
    ...
    BEGIN
      ...
      OPEN cr_crawepr (pr_cdcooper => pr_cdcooper
                      ...
                      ,pr_nrctremp => pr_nrctremp);
      FETCH cr_crawepr INTO rw_crawepr;
      vr_crawepr:= cr_crawepr%FOUND; 
      --Se Encontrou
      IF vr_crawepr AND rw_crawepr.tpemprst <> 1 THEN
        --Fechar Cursor
        CLOSE cr_crawepr;  
        --mensagem Critica
        vr_cdcritic:= 363;
      ...
      BEGIN
        ...
        vr_diapagto:= to_number(to_char(pr_dtdpagto,'DD')); 
        vr_mespagto:= to_number(to_char(pr_dtdpagto,'MM'));  
        vr_anopagto:= to_number(to_char(pr_dtdpagto,'YYYY'));  
        ...
        empr0001.pc_calc_dias360(pr_ehmensal => FALSE                       -- Indica se juros esta rodando na mensal
                                ...
                                ,pr_diarefju => to_char(pr_dtlibera,'dd')   -- Dia da data de referência da última vez que rodou juros
                                ,pr_mesrefju => to_char(pr_dtlibera,'mm')   -- Mes da data de referência da última vez que rodou juros
                                ,pr_anorefju => to_char(pr_dtlibera,'yyyy') -- Ano da data de referência da última vez que rodou juros
                                ,pr_diafinal => vr_diapagto                 -- Dia data final
                                ,pr_mesfinal => vr_mespagto                 -- Mes data final
                                ,pr_anofinal => vr_anopagto                 -- Ano data final
                                ,pr_qtdedias => pr_qtdiacar);               -- Quantidade de dias calculada
        ...
        --Taxa Diaria
        vr_txdiaria:= apli0001.fn_round( (100 * (POWER ((pr_txmensal / 100) + 1 , (1 / 30)) - 1)), 10);
        --Colocar o valor calculado no parametro saida
        ...
        pc_calcula_data_parcela (pr_cdcooper     => pr_cdcooper            --Codigo Cooperativa
                                ,pr_dtvencto     => pr_dtdpagto            --Data de Pagamento
                                ,pr_nrparcel     => pr_qtparepr            --Numero de parcelas
                                ,pr_data_parcela => vr_tab_datas_parcelas  --Tabela com datas das parcelas emprestimo
        ...
        --Percorrer todas as parcelas
        FOR idx IN 1..pr_qtparepr LOOP
          --Verificar se a prestacao está na tabela 
          IF vr_tab_datas_parcelas.EXISTS(idx) THEN
            --Incrementar Indice 
            vr_index:= pr_parcela_epr.count + 1;
            pr_parcela_epr(vr_index).cdcooper:= pr_cdcooper;
            pr_parcela_epr(vr_index).nrdconta:= pr_nrdconta;
            pr_parcela_epr(vr_index).nrparepr:= vr_nrparepr;
            pr_parcela_epr(vr_index).vlparepr:= vr_vlparepr;
            pr_parcela_epr(vr_index).dtparepr:= vr_tab_datas_parcelas(idx); --dtparepr                   
          END IF;  
        END LOOP;
        --Transacao OK
        vr_flgtrans:= TRUE;  
      ...
      --Se nao ocorreu sucesso na transacao
      ...
      --Se for para gravar
      IF pr_flggrava THEN
        --Se existe emprestimo
        IF vr_crawepr THEN
          --Gerar parcelas Emprestimo
          pc_gera_parcelas_emprest    (pr_cdcooper    => pr_cdcooper       --Codigo Cooperativa
                                      ...
                                      ,pr_parcela_epr => pr_parcela_epr    -->Tipo de tabela com parcelas emprestimo
                                      ,pr_des_reto    => vr_des_reto       --Retorno OK ou NOK
          ...
          IF vr_des_reto = 'NOK' THEN
            ...
            END IF;  
          END IF;
          --Se possui parcela na tabela memoria
          IF pr_parcela_epr.COUNT > 0 THEN
            --Buscar valor parcela emprestimo
            vr_vlpreemp:= pr_parcela_epr(pr_parcela_epr.FIRST).vlparepr;
            BEGIN
              UPDATE crawepr SET crawepr.vlpreemp = vr_vlpreemp
                                ,crawepr.txdiaria = pr_txdiaria
                                ,crawepr.txmensal = pr_txmensal
              WHERE crawepr.rowid = rw_crawepr.rowid;                  
            EXCEPTION
              WHEN OTHERS THEN
                vr_dscritic:= 'Erro ao atualizar parcela do emprestimo. '||sqlerrm;
                --Levantar Excecao
                RAISE vr_exc_erro;
            END;      
          END IF;    
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        ...
        pr_des_reto := 'NOK';
        -- Chamar rotina de gravação de erro
        vr_dscritic := 'Erro na pc_calcula_emprestimo --> '|| sqlerrm;
        gene0001.pc_gera_erro(pr_cdcooper => pr_cdcooper
                             ,pr_cdagenci => pr_cdagenci
                             ,pr_nrdcaixa => pr_nrdcaixa
                             ,pr_nrsequen => 1 --> Fixo
