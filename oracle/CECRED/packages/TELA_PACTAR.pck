...
CREATE OR REPLACE PACKAGE BODY CECRED.TELA_PACTAR AS
  ...
  BEGIN
    ...
    IF pr_cddopcao = 'I' THEN
      ...
      IF NVL(pr_cdpacote,0) <= 0 THEN
        vr_dscritic := 'Código do Serviço Cooperativo informado é inválido.';
      ELSE
        -- Consulta codigo na tabela para se certificar que codigo e valido e nao havera registro duplicado
        ...
        ELSE
          ...
          CLOSE cr_tbtarif_pacotes;
          vr_dscritic := 'Código de Serviço Cooperativo já cadastrado.';
          RAISE vr_exc_erro;
        END IF;
      ...
      IF TRIM(pr_dspacote) IS NULL THEN
        vr_dscritic := 'Descrição do Serviço Cooperativo informada inválida.';
      END IF;      
  ...
  BEGIN
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        pr_cdcritic := vr_cdcritic;
        pr_dscritic := 'Erro geral em Consulta de Serviços Cooperativos TELA_PACTAR.PC_CONSULTA_PCT_TAR_CAR: ' || SQLERRM;
    END;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        vr_cdcritic := 0;
        vr_dscritic := 'Erro ao inserir Serviço Cooperativo. Erro: ' || SQLERRM;
        RAISE vr_exc_erro;
    END;
    ...
    FOR ind_registro IN vr_tarifa_geral.FIRST..vr_tarifa_geral.LAST LOOP
      ...
      EXCEPTION
        WHEN OTHERS THEN
          vr_cdcritic := 0;
          vr_dscritic := 'Erro ao inserir registro de Serviço Cooperativo, Serviço: ' || TO_CHAR(pr_strtarif) || ' . Erro: ' || SQLERRM;
          RAISE vr_exc_erro;
      END;
  ...
  BEGIN
    ...
    IF cr_tbtarif_pacotes%NOTFOUND THEN
      CLOSE cr_tbtarif_pacotes;
      vr_dscritic := 'Serviço Cooperativo não cadastrado.'; 
      RAISE vr_exc_erro;
    ELSE
      CLOSE cr_tbtarif_pacotes;
  ...
  BEGIN
    ...
    IF rw_tbtarif_pacotes.flgsituacao = 0 THEN
      vr_dscritic := 'Serviço Cooperativo já esta cancelado.';
      RAISE vr_exc_erro;
    END IF;
    ...
    IF cr_tbtarif_pacotes_coop%FOUND THEN
      ...
      vr_dscritic := 'Operação não efetuada! Serviço Cooperativo encontra-se Ativo na(s) singular(es).';
