...
/*..............................................................................
   ...
   Alteracoes: 27/06/2018 - P450 - Criação de procedure para consulta de saldos - pc_consulta_sld_cta_prj (Daniel/AMcom)
               ...
               15/08/2018 - 9318   Pagamento de contrato de emprestimo de contas em prejuizo. Rangel Decker (AMcom)
..............................................................................*/
...
/* Rotina para liquidar prejuizo de Conta Corrente*/
  ...
  FUNCTION fn_sld_cta_prj(pr_cdcooper   IN NUMBER --> Código da Cooperativa
                         ,pr_nrdconta   IN NUMBER --> Número da conta
												 ,pr_exclbloq   IN NUMBER DEFAULT 0) --> Flag que indica se devem ser excluídos (subtraídos) do saldo os créditos bloqueados (cheques ainda não compensados)
                          RETURN NUMBER;
...
/*..............................................................................
   ...
   Sigla   : CRED
   Autor   :Rangel Decker - AMCom
   Data    : Maio/2018                      Ultima atualizacao:15/08/2018
...
..............................................................................*/
  ...
  --Retorna o valor dos créditos disponíveis na CC em prejuízo
	FUNCTION fn_cred_disp_prj(pr_cdcooper IN crapris.cdcooper%TYPE
	  ...
	  -- Calcula valor de créditos disponíveis para uso no pagamento
	  CURSOR cr_creddisp IS
		select nvl(SUM(decode(his.indebcre, 'D', tr.vllanmto*-1, tr.vllanmto)), 0)
			from craplcm tr
				 , craphis his
		 where tr.cdcooper = pr_cdcooper
			 and tr.nrdconta = pr_nrdconta
			 ...
			 and tr.cdhistor = his.cdhistor
			 and tr.cdcooper = his.cdcooper
			 AND his.cdhistor <>  2718 -- Exclui débitos referentes aos juros remuneratórios
			 AND his.indebprj <> 1;    -- Exclui os débitos que ocorrem mesmo após a transf. para prejuízo
			 
		vr_vlcreddisp NUMBER; -- Valor dos créditos disponíveis para uso no pagamento                		           
	BEGIN
		 OPEN cr_creddisp;
		 FETCH cr_creddisp INTO vr_vlcreddisp;
		 CLOSE cr_creddisp;
...
--
  ...
  BEGIN
     ...
     BEGIN
				...
				IF vr_des_erro = 'NOK' THEN
						-- Verifica se retornou erro no vetor
						IF vr_tab_erro.COUNT > 0 THEN
							vr_cdcritic:= vr_tab_erro(vr_tab_erro.FIRST).cdcritic;
							vr_dscritic:= vr_tab_erro(vr_tab_erro.FIRST).dscritic|| ' Conta: ' || pr_nrdconta;
						ELSE
							vr_cdcritic:= 0;
							vr_dscritic:= 'Retorno "NOK" na extr0001.pc_obtem_saldo_dia e sem informação na pr_tab_erro, Conta: ' || pr_nrdconta;
						END IF;
				 ...
				 --Verificar se retornou informações do saldo
				 IF vr_tab_saldo.Count = 0 THEN 
					 ...
					 RAISE vr_exc_saida;
				 ELSE
					 -- Monta valor do saldo devedor
					 vr_vlslddev := abs(nvl(vr_tab_saldo(vr_tab_saldo.FIRST).vlsddisp,0) +
     ...
     pc_lanca_transf_extrato_prj(pr_idprejuizo => vr_idprejuizo
		                           ...
		                           , pr_vlsldprj   => vr_vlslddev
															 , pr_vljur60_ctneg => vr_vlhist37 + vr_vlhist57
															 , pr_vljur60_lcred => vr_vlhist38
															 , pr_dtmvtolt   => rw_crapdat.dtmvtolt
															 , pr_tpope      => pr_tpope
															 , pr_cdcritic   => vr_cdcritic
															 , pr_dscritic   => vr_dscritic);
 ...
 BEGIN
    ...
    FOR rw_conta_liquida IN cr_conta_liquida(pr_cdcooper) LOOP
			BEGIN
				-- Preenche a data de liquidação do prejuízo
...
/*
			...
			UPDATE tbcc_prejuizo
			   SET vlsldlib = vlsldlib + pr_vlrlanc
			 WHERE cdcooper = pr_cdcooper
			   AND nrdconta = pr_nrdconta
...
/* .............................................................................
        ...
        Dados referentes ao programa:
        Frequencia: Sempre que for chamado
        Objetivo  : Rotina para efetuar lançamento de debito em conta Prejuízo
        Observacao: -----
        Alteracoes:
      ..............................................................................*/
  ...
  BEGIN   			
      ...
      vr_saldodisp := fn_sld_cta_prj(pr_cdcooper => pr_cdcooper
                                    ,pr_nrdconta => pr_nrdconta
																	  ,pr_exclbloq => 1); --> Desconta créditos bloqueados
																		
			vr_cdhistor := 2739; -- Débito da conta transitória (desbloqueio de valor)

      IF vr_saldodisp < pr_vlrlanc THEN
        vr_cdcritic := 0;
        vr_dscritic := 'A conta' || pr_nrdconta || ' não possui saldo para esta operação!';
...
PROCEDURE pc_ret_saldo_dia_prej ( pr_cdcooper  IN crapcop.cdcooper%TYPE         --> Coop conectada
    ...
    CURSOR cr_crapsda( pr_cdcooper IN crapass.cdcooper%TYPE,
     ...
     SELECT * FROM (SELECT sda.vlblqprj
        FROM crapsda sda
  ...
  BEGIN
    ...
    ELSE
      ...
      ELSE
        CLOSE cr_crapsda;
        vr_vldsaldo := rw_crapsda.vlblqprj;
      END IF;
    END IF;
  ...
  EXCEPTION
     WHEN OTHERS THEN
			  pr_cdcritic := 0; 
        pr_dscritic := 'Erro calc. Juro prejuizo mensal: ' || SQLERRM;  
 ...
 BEGIN
	  ...
	  CLOSE cr_craplauiof;
		...
		IF  vr_vlsddisp > 0 AND rw_contaprej.vljuprej > 0 THEN           
			 ...
			 -- Incluir lançamento na TBCC_PREJUIZO_DETALHE
			 -- Juros remuneratórios (Hist 2718)
				INSERT INTO tbcc_prejuizo_detalhe (
					dtmvtolt
				 ,nrdconta
				 ,cdhistor 	
		...
		IF vr_vlsddisp > 0 THEN		
			...
			IF vr_vljupre > 0 THEN 
				IF vr_vlsddisp < vr_vljupre THEN
		...
		-- Lançar total pago na CRAPLCM
				 
		 /*
		 -- Desconta o valor total do pagamento efetuado do saldo disponível para operações na C/C
		 UPDATE tbcc_prejuizo_lancamento
				SET vlsldlib = vlsldlib - vr_valrpago
			WHERE cdcooper = pr_cdcooper
				AND nrdconta = pr_nrdconta
				AND dtliquidacao IS NULL;
		*/
...
PROCEDURE pc_pagar_IOF_conta_prej(pr_cdcooper  IN craplcm.cdcooper%TYPE        -- Código da Cooperativa
 ...
 END pc_resgata_cred_bloq_preju;
  ...
  PROCEDURE pc_devolve_chq_prej(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Coop conectada
    ...
    Objetivo  : Se existir devolução de cheques depositados, realizar débito na conta transitória de prejuízo.
    ...
    rw_chqdev cr_chqdev%ROWTYPE;
     ...
     -- Cursor para buscar as informações de depósito do cheque devoldido
    ...
    rw_crapchd cr_crapchd%ROWTYPE;
		...
		-- Verifica se há lançamento na TBCC_PREJUIZO_LANCAMENTO para um cheque devolvido
  ...
  BEGIN
    ...
    FOR rw_chqdev IN cr_chqdev(rw_crapdat.dtmvtolt) LOOP      
      -- Leitura das informações dos cheques devolvidos
      FOR rw_crapchd IN cr_crapchd(pr_nrdconta => rw_chqdev.nrdconta
			  ...
			  -- Verifica se há depósito na Conta Transitória (TBCC_PREJUIZO_LANCAMENTO) para o cheque em questão
  ...
  -- Pagamento de contratos de emprestimos de contas em prejuizo
 PROCEDURE pc_pagar_contrato_emprestimo(pr_cdcooper  IN crapepr.cdcooper%TYPE       -- Código da Cooperativa
                                        ,pr_nrdconta  IN crapepr.nrdconta%TYPE       -- Número da Conta
                                        ,pr_cdagenci  IN NUMBER                      -- Código da agencia
                                        ,pr_nrctremp  IN crapepr.nrctremp%TYPE       -- Número do contrato de empréstimo 
 ...
 /* .............................................................................
    ...
    Dados referentes ao programa:
    Frequencia: Sempre que for chamado
    Objetivo  : Pagamento de emprestimos do tipo TR, PP ou POS de contas em prejuizo
    Alteracoes:
    ..............................................................................*/
