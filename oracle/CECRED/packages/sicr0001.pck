...
create or replace package body cecred.SICR0001 is
  ...
  -- Criada padronização de erro mas continuando a devolver o problema da mesma forma - 15/05/2018 - Chd REQ0014479 
  EXCEPTION
    WHEN OTHERS THEN
      ...
      -- Monta descrição do erro nao tratado
      vr_cdcrimsg := 9999;      
      vr_sqlerrm  := SQLERRM;
    ...
    BEGIN
      ...
      FOR rw_craplau IN cr_craplau(pr_cdcooper => pr_cdcooper
        ...
        pc_identifica_crapatr(pr_cdcooper  => rw_craplau.cdcooper --> Código da coopertiva
                             ...
                             ,pr_cdcritic  => vr_cdcritic --> Codigo da critica de erro
                             ,pr_dscritic  => vr_dscritic); --> descrição do erro se ocorrer
        -- Não parar o processo caso retorne critica pois o Log já foi gravado para averiguações - 15/05/2018 - Chd REQ0014479
        -- Inclui nome do modulo logado - 15/12/2017 - Ch 788828
        GENE0001.pc_set_modulo(pr_module => NULL ,pr_action => 'SICR0001.pc_efetua_debito_automatico');
        rw_crapatr := NULL;
...
--------------------------------------------------------------------------------------------------------------------*/
    -------------> CURSOR <--------------
     ...
     -- Busca o último horário de execução no debitador único
      ...
      SELECT MAX(to_date(to_char(/*pr_dtmvtopg*/SYSDATE,'ddmmyyyy')||to_char(tdeb.dhprocessamento, 'hh24:mi'),'ddmmyyyyhh24:mi:ss')) dtultexec 
