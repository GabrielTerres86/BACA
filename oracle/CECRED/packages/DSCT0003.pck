CREATE OR REPLACE PACKAGE DSCT0003 AS
  ...
  vr_cdhistordsct_credpagmaior   CONSTANT craphis.cdhistor%TYPE := 2758; --CREDITO DESCONTO DE TITULO PAGO A MAIOR
 ...
 PROCEDURE pc_pagar_titulo( pr_cdcooper    IN crapcop.cdcooper%TYPE           --Codigo Cooperativa
  ...
  PROCEDURE pc_buscar_borderos_liberados (pr_nrdconta  IN crapass.nrdconta%TYPE --> Numero da Conta
                                         ,pr_nrctremp  IN craptdb.nrborder%TYPE --> Numero do bordero
                                         ,pr_xmllog        IN VARCHAR2             --> XML com informações de LOG
                                         ,pr_cdcritic     OUT PLS_INTEGER          --> Código da crítica
                                         ,pr_dscritic     OUT VARCHAR2             --> Descrição da crítica
                                         ,pr_retxml        IN OUT NOCOPY XMLType   --> Arquivo de retorno do XML
...
END  DSCT0003;
/
CREATE OR REPLACE PACKAGE BODY DSCT0003 AS
      ...
      IF cr_craplot%NOTFOUND THEN
        ...
        BEGIN
          lanc0001.pc_gerar_lancamento_conta(
             ...
             pr_cdhistor => vr_cdhistordsct_iofspriadic,
             pr_nrseqdig => rw_craplot.nrseqdig,
             pr_vllanmto => vr_vltotiof,
             pr_nrdctabb => pr_nrdconta,
             pr_cdpesqbb => 'Bordero ' || pr_nrborder || ' - ' ||  TO_CHAR(gene0002.fn_mask(pr_vltotliq,'999.999.999999')),
             pr_cdcooper => pr_cdcooper,
          ...
          rw_craplcm.cdhistor := vr_cdhistordsct_iofspriadic;
          rw_craplcm.cdcooper := pr_cdcooper;
          rw_craplcm.dtmvtolt := rw_craplot.dtmvtolt;
          rw_craplcm.hrtransa := TO_NUMBER(TO_CHAR(SYSDATE,'SSSSS'));
          rw_craplcm.nrdconta := pr_nrdconta;
          rw_craplcm.nrdocmto := rw_craplot.nrseqdig;
          rw_craplcm.vllanmto := vr_vltotiof;
    ...
    BEGIN
      ...
      BEGIN
        ...
        lanc0001.pc_gerar_lancamento_conta(
           ...
           pr_nrdctabb => pr_nrdconta, 
           pr_cdpesqbb => 'Desconto do Borderô ' || pr_nrborder, 
           pr_nrautdoc => 0, 
           pr_cdcooper => pr_cdcooper, 
           pr_tab_retorno => vr_tab_retorno, 
           pr_incrineg => vr_incrineg, 
        ...
        vr_rowid            := vr_tab_retorno.rowidlct;
        rw_craplcm.cdhistor := vr_cdhistordsct_credito;
        rw_craplcm.cdcooper := pr_cdcooper;
        rw_craplcm.dtmvtolt := rw_craplot.dtmvtolt;

        begin
           select hrtransa
           into rw_craplcm.hrtransa
           from craplcm
           where rowid = vr_rowid;
        exception
           when no_data_found then
              vr_dscritic := 'Erro ao obter hora transação: ' || SQLERRM;
              RAISE vr_exc_erro;
        end;

        rw_craplcm.nrdconta := pr_nrdconta;
        rw_craplcm.nrdocmto := rw_craplot.nrseqdig +1;
    ...
    WHILE NOT vr_flg_criou_lot LOOP  
      ...
      BEGIN
        ...
        lanc0001.pc_gerar_lancamento_conta(
           ...
           pr_nrseqdig => vr_nrdolote,
           pr_nrdctabb => pr_nrdconta,
           pr_nrautdoc => 0,
           pr_cdcooper => pr_cdcooper,
           pr_cdpesqbb => 'Desconto de Título do Borderô ' || pr_nrborder,
           pr_tab_retorno => vr_tab_retorno, 
           pr_incrineg => vr_incrineg, 
           pr_cdcritic => vr_cdcritic, 
           pr_dscritic => vr_dscritic);
        ...
        exception
           when no_data_found then
              vr_dscritic := 'Erro ao obter hora transação (2): ' || SQLERRM;
              RAISE vr_exc_erro;
        end;

        rw_craplcm.nrdconta := pr_nrdconta;
        rw_craplcm.nrdocmto := pr_nrdocmto;
        rw_craplcm.vllanmto := pr_vllanmto;
    ...
    CURSOR cr_craptdb (pr_cdcooper craptdb.cdcooper%TYPE) IS
      SELECT
         ...
         WHERE tdb.nrdconta = pr_nrdconta
           ...
           AND tdb.nrborder = DECODE(pr_nrctremp, NULL, tdb.nrborder, pr_nrctremp)
         GROUP BY tdb.nrdconta, tdb.nrborder, tdb.dtlibbdt
