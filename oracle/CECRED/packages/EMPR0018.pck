CREATE OR REPLACE PACKAGE CECRED.EMPR0018 AS
  ...
  /* -------------------------------------------------------------------------------------------------------------
    ...
    Data    : 18/12/2018               ultima Atualizacao: -- 14/03/2019
    ...
    Alteracoes: 01/03/2019 - Conversão das alterações do PRJ298. (Douglas Pagel / AMcom)
                14/03/2019 - Inclusão dos parametros pr_vlparepr e pr_vliofepr
                na procedure pc_grava_simulacao e tratativas do valor da parcela 
                e do IOF quando for consignado (Josiane Stiehler - AMcom)
  ...
  rw_craptab cr_craptab%ROWTYPE;
   ...
   PROCEDURE pc_grava_simulacao( pr_cdcooper IN crapcop.cdcooper%TYPE
    ...
    pr_retorno := NULL;
      ...
      IF vr_tpmodcon > 0 and
         pr_tpemprst <> 1 then
         vr_cdcritic:= 0;
         vr_dscritic:= 'Linha nao permitida para este produto!';
         RAISE vr_exc_erro;  
      END IF;
      ...
      IF pr_tpemprst = 1 THEN
         IF vr_tpmodcon = 0 THEN 
            ...
            TIOF0001.pc_calcula_iof_epr(pr_cdcooper => pr_cdcooper
                                      ...
                                      , pr_dscritic => vr_dscritic);
          ELSE
             vr_valoriof:= pr_vliofepr; 
             vr_vlpreclc:= pr_vlparepr; 
          END IF;
