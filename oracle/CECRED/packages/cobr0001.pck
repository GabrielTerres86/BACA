...
............................................................................ */
  ...
  /* Procedure para separar os registros DDA dos arquivos 29999*.RET */
  PROCEDURE pc_separar_registros_dda(pr_cdcooper IN crapcop.cdcooper%TYPE);
...
CREATE OR REPLACE PACKAGE BODY CECRED.cobr0001 AS
  ...
  --  Data     : Novembro/2013.                   Ultima atualizacao: 19/08/2016
  --
  -- Dados referentes ao programa:
  --
  -- Frequencia: -----
  ...
  --
  --             19/08/2016 - #400556 Procedure para separar os registros DDA dos arquivos 29999*.RET.
  --                          e salvar no diretório usr/coop/cecred/arq, que será acessado pelo script 
  --                          perl ftpabbc_copia_dda.pl para fazer a cópia para a cabine.
  --                          Executado em job jb_dda_separar_registros_dda. (Carlos)
	--  
  ---------------------------------------------------------------------------------------------------------------
  ...
  /* Procedure para separar os registros DDA dos arquivos 29999*.RET. e salvar no diretório
     usr/coop/cecred/arq, que será acessado pelo script perl ftpabbc_copia_dda.pl para fazer 
     a cópia para a cabine.
     Executado em job jb_dda_separar_registros_dda. */
  PROCEDURE pc_separar_registros_dda(pr_cdcooper IN crapcop.cdcooper%TYPE) IS
    BEGIN
    ...
    DECLARE
      ...
      CURSOR cr_cco (pr_nrconven IN crapcco.nrconven%TYPE) IS
        SELECT cco.cdcooper, cco.dsorgarq, cco.nrconven
          FROM crapcco cco
         WHERE cco.nrconven = pr_nrconven;
      rw_cco cr_cco%ROWTYPE;

      CURSOR cr_cco2 (pr_nrconven IN crapcco.nrconven%TYPE) IS
        SELECT cco.cdcooper
          FROM crapcco cco
         WHERE cco.nrconven = pr_nrconven
           AND cco.dsorgarq NOT IN ('MIGRACAO', 'INCORPORACAO');
      rw_cco2 cr_cco2%ROWTYPE;

      CURSOR cr_tco (pr_cdcooper IN craptco.cdcooper%TYPE
      ...
      CURSOR cr_ceb (pr_cdcooper IN crapceb.cdcooper%TYPE
      ...
      vr_idx             VARCHAR2(100);
      vr_dirarqui        VARCHAR2(500);
      -- Variáveis para armazenar as informações em XML
      vr_des_xml         CLOB;
      -- Variável para armazenar os dados do XML antes de incluir no CLOB
      vr_texto_completo  VARCHAR2(32600);
      -- diretorio de geracao do relatorio
      vr_caminho_dest      VARCHAR2(100);
      
      --Tabela para receber arquivos lidos no unix
      vr_tab_nmarqtel GENE0002.typ_split;
      
      TYPE typ_tab_arqord is table of varchar2(500)
       index by varchar2(100);
      vr_tab_Arqord typ_tab_Arqord;

      vr_cdprogra   crapprg.cdprogra%TYPE;           --> Código do programa
      vr_nomdojob  CONSTANT VARCHAR2(100) := 'jb_dda_separar_registros_dda';
    ...
    BEGIN
      ...
      vr_nmarquiv := '29999%.RET';
--    vr_nmarquiv := '29999815.RET';

      vr_dirarqui     := gene0001.fn_diretorio(pr_tpdireto => 'C' , 
                                               pr_cdcooper => pr_cdcooper, 
                                               pr_nmsubdir => 'integra');

      vr_caminho_dest := gene0001.fn_diretorio(pr_tpdireto => 'C' , 
                                               pr_cdcooper => pr_cdcooper, 
                                               pr_nmsubdir => 'arq');
      ...
      --Listar arquivos no diretorio integra
      gene0001.pc_lista_arquivos (pr_path     => vr_dirarqui
                                 ,pr_pesq     => vr_nmarquiv
      ...
      IF vr_dscritic IS NOT NULL THEN
        pc_controla_log_batch(pr_dstiplog => 'E',
                              pr_dscritic => 'Erro ao localizar arquivos: '||vr_dscritic);
        RAISE vr_exc_saida;
      END IF;
      ...
      --Se nao encontrou arquivos no dir integra ...
      IF vr_tab_nmarqtel.COUNT <= 0 THEN
        ...
        --Se nao encontrou arquivos tbm no salvar, reporta erro e sai do programa
        IF vr_tab_nmarqtel.COUNT <= 0 THEN
                
          --DBMS_OUTPUT.put_line('Nenhum arquivo encontrado!');
          pc_controla_log_batch(pr_dstiplog => 'E',
                                pr_dscritic => 'Nenhum arquivo encontrado!');
          RAISE vr_exc_saida;
        END IF;

      END IF;
         
      -- ordenar temptable dos arquivos
      FOR idx IN 1..vr_tab_nmarqtel.COUNT LOOP
        vr_tab_arqord(vr_tab_nmarqtel(idx)) := vr_tab_nmarqtel(idx);
      END LOOP;

      vr_idx := vr_tab_arqord.first;
        
      --Percorrer todos os arquivos
      WHILE vr_idx IS NOT NULL LOOP

        vr_nom_arquivo := vr_tab_arqord(vr_idx);

        gene0001.pc_abre_arquivo(pr_nmdireto => vr_dirarqui        --> Diretório do arquivo
        ...
        BEGIN
          ...
          gene0001.pc_le_linha_arquivo(vr_ind_arqlog, vr_linha);

          IF substr(vr_linha, 1, 10) = '9999999999' THEN
          ...
          vr_cdbandst:= TO_NUMBER(TRIM(SUBSTR(vr_linha,01,3)));
          vr_cdmotdev:= TO_NUMBER(TRIM(SUBSTR(vr_linha,51,2)));
          vr_cdbcoaco:= TO_NUMBER(TRIM(SUBSTR(vr_linha,54,3)));
          vr_cdageaco:= TO_NUMBER(TRIM(SUBSTR(vr_linha,57,04)));
          vr_dscodbar:= SUBSTR(vr_linha,20,25);
          vr_dtmvtolt:= TO_DATE(TRIM(SUBSTR(vr_linha,71,8)),'YYYYMMDD');
          vr_vlliquid:= TO_NUMBER(TRIM(SUBSTR(vr_linha,85,12))) / 100;
          vr_nrcnvcob:= TO_NUMBER(TRIM(SUBSTR(vr_linha,20,6)));
          vr_nrdconta:= TO_NUMBER(TRIM(SUBSTR(vr_linha,26,8)));
          vr_nrdocmto:= TO_NUMBER(TRIM(SUBSTR(vr_linha,34,9)));

          vr_tco := FALSE;

          OPEN cr_cco(pr_nrconven => vr_nrcnvcob);
          LOOP
            FETCH cr_cco INTO rw_cco;
            EXIT WHEN cr_cco%NOTFOUND;

            IF rw_cco.dsorgarq IN ('MIGRACAO','INCORPORACAO') THEN

              OPEN cr_cco2 (pr_nrconven => vr_nrcnvcob);
              FETCH cr_cco2 INTO rw_cco2;
              CLOSE cr_cco2;

              OPEN cr_tco (pr_cdcooper => rw_cco.cdcooper
                          ,pr_cdcopant => rw_cco2.cdcooper
                          ,pr_nrctaant => vr_nrdconta);
              FETCH cr_tco INTO rw_tco;
              IF cr_tco%FOUND THEN
                vr_tco := TRUE;
                vr_nrdconta := rw_tco.nrdconta;
              ELSE
                CLOSE cr_tco;
                continue;
              END IF;
            ...
            OPEN cr_ceb (pr_cdcooper => rw_cco.cdcooper
                        ,pr_nrconven => vr_nrcnvcob
                        ,pr_nrdconta => vr_nrdconta);
            FETCH cr_ceb INTO rw_ceb;

            IF cr_ceb%FOUND THEN

              IF rw_cco.dsorgarq = 'IMPRESSO PELO SOFTWARE' AND
                 rw_ceb.flgcebhm = 0 THEN
            ...
            OPEN cr_cob (pr_cdcooper => rw_cco.cdcooper
                        ,pr_nrdconta => vr_nrdconta
                        ,pr_nrcnvcob => rw_cco.nrconven
                        ,pr_nrdocmto => vr_nrdocmto);
            FETCH cr_cob INTO rw_cob;

            /* Se achou cobrança, registra no arquivo novo */
            IF cr_cob%FOUND THEN
              IF rw_cob.flgcbdda = 1 THEN
                pc_escreve_xml(vr_linha || chr(10));
              END IF;
          ...
          EXCEPTION
            ...
            WHEN OTHERS THEN
              ...
              pc_escreve_xml(vr_nom_arquivo||' - Erro linha ' || to_char( i ) || ' - ' || vr_linha);
              vr_dscritic:= vr_nom_arquivo||' - Erro na leitura do arquivo. '||sqlerrm;
              pc_escreve_xml(vr_dscritic);
        ...
        vr_idx := vr_tab_arqord.next(vr_idx);
      
        -- descarregar buffer
        pc_escreve_xml(nvl(vr_dscritic,''),TRUE);    
        
        vr_nom_arquivo := replace(vr_nom_arquivo, '.RET', '.DDA.RET');

        -- gerar informações no arquivo
        DBMS_XSLPROCESSOR.CLOB2FILE(vr_des_xml, 
                                    vr_caminho_dest,                                     
                                    vr_nom_arquivo, 
                                    NLS_CHARSET_ID('UTF8'));
                                    
        -- json0001.pck Exemplo de execução de .pl
        --Obtem o arquivo perl no diretório ROOT da Cecred
        vr_nmarqcmd := gene0001.fn_param_sistema(pr_nmsistem => 'CRED'
                                                ,pr_cdcooper => 3
                                                ,pr_cdacesso => 'ROOT_CECRED_BIN') || 'ftpabbc_copia_dda.pl';
