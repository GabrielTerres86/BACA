CREATE OR REPLACE PACKAGE CECRED.NPCB0001 is
  ...
  PROCEDURE pc_linha_codigo_barras(pr_titulo1  IN OUT NUMBER     --> Campo 1 da linha digitável
                                  ...
                                  ,pr_codbarra IN OUT NUMBER     --> Código de barras completo
  ...
  FUNCTION fn_valor_calc_titulo_npc (pr_dtmvtolt     IN crapdat.dtmvtolt%TYPE --> Data de movimento                                                                   
                                    ,pr_tab_CalcTit  IN typ_tab_CalcTit       --> Regras de calculo de juros                                 
                                    ) RETURN INTEGER ; --> Retornar valor que deve ser cobrado conforme a data de pagamentp                                  
  ...
  PROCEDURE pc_valid_pagamento_npc (pr_cdcooper     IN crapcop.cdcooper%TYPE --> Codigo da cooperativa
                                   ...
                                   ,pr_nrctrlcs     IN tbcobran_consulta_titulo.nrctrlcs%TYPE --> Numero de controle da consulta no NPC
...
CREATE OR REPLACE PACKAGE BODY CECRED.NPCB0001 is
  ...
  BEGIN
     ...
     RETURN TO_CHAR(SYSTIMESTAMP, 'YYYYMMDDHH24MISSFF6')||vr_dssufixo;
  ...
  BEGIN   
     ...
     ELSE
        ...
        FOR idx IN 1..3 LOOP
           ...
           -- Se não calculou o dígito verificador
           IF NOT vr_retorno THEN
              -- Retornar a crítica 
              pr_cdcritic := 8;
              pr_dscritic := NULL;
  ...
  /* ..........................................................................
    ...
    CURSOR cr_cons_titulo (pr_nrctrlcs IN tbcobran_consulta_titulo.nrctrlcs%TYPE ) IS
      SELECT con.dsxml
        FROM tbcobran_consulta_titulo con
       WHERE con.nrctrlcs = pr_nrctrlcs;
  ...
  BEGIN     
    ...
    --> Buscar dados da consulta
    OPEN cr_cons_titulo (pr_nrctrlcs => pr_nrctrlcs);
    FETCH cr_cons_titulo INTO rw_cons_titulo;
    
    IF cr_cons_titulo%NOTFOUND THEN
      CLOSE cr_cons_titulo;
      vr_dscritic := 'Não foi possivel localizar consulta NPC '||pr_nrctrlcs;    
      RAISE vr_exc_erro;  
    ELSE
      CLOSE cr_cons_titulo;
    END IF;
  ...
  FUNCTION fn_valor_calc_titulo_npc( pr_dtmvtolt     IN crapdat.dtmvtolt%TYPE --> Data de movimento                                                                   
                                    ,pr_tab_CalcTit  IN typ_tab_CalcTit       --> Regras de calculo de juros                                 
                                    ) RETURN INTEGER IS --> Retornar valor que deve ser cobrado conforme a data de pagamentp  
  ...
  BEGIN    
    ...
    IF pr_tab_CalcTit.count <> 0 THEN
      FOR idx IN pr_tab_CalcTit.first..pr_tab_CalcTit.last LOOP
        ...
        IF pr_dtmvtolt <= pr_tab_CalcTit(idx).dtValiddCalc AND 
           pr_tab_CalcTit(idx).dtValiddCalc < vr_dtValiddCalc 
           THEN 
          vr_VlrTotCobrar := pr_tab_CalcTit(idx).VlrTotCobrar;
          vr_dtValiddCalc := pr_tab_CalcTit(idx).dtValiddCalc; 
        END IF;
  ...
  /* ..........................................................................
    ...
    vr_tab_TituloCIP  typ_tab_TituloCIP;
  ...
  BEGIN     
    ...
    pc_valid_titulo_npc ( pr_cdcooper   => pr_cdcooper   --> Codigo da cooperativa
                         ...
                         ,pr_nrctrlcs   => pr_nrctrlcs   --> Numero de controle da consulta no NPC
    ...
    FOR idx IN vr_tab_TituloCIP.first..vr_tab_TituloCIP.last LOOP
      ...
      CASE vr_tab_TituloCIP(idx).TpAutcRecbtVlrDivgte
        ...
        WHEN 2 THEN --> 2 - entre o mínimo e o máximo   
          ...
          IF 0 = fn_valid_max_min_valor 
                                  (pr_vltitulo => vr_tab_TituloCIP(idx).VlrTit             --> Valor do titulo
                                  ,pr_vldpagto => pr_vldpagto                              --> Valor do pagamento
                                  ,pr_tpmaxpgt => vr_tab_TituloCIP(idx).IndrVlr_PercMaxTit --> Tipo de calculo valor maximo(P-Percentual, V-Valor)
                                  ,pr_vlmaxpgt => vr_tab_TituloCIP(idx).Vlr_PercMaxTit     --> Valor maximo de pagamento
                                  ,pr_tpminpgt => vr_tab_TituloCIP(idx).IndrVlr_PercMinTit --> Tipo de calculo valor maximo(P-Percentual, V-Valor)
                                  ,pr_vlminpgt => vr_tab_TituloCIP(idx).Vlr_PercMinTit     --> Valor maximo de pagamento
                                   ) THEN
        ...
        WHEN 3 THEN --> 3 -- não aceita pagar valor divergente;
          ...
          vr_vltitcal := fn_valor_calc_titulo_npc
                                   ( pr_dtmvtolt     => pr_dtmvtolt                        --> Data de movimento                                                                   
                                    ,pr_tab_CalcTit  => vr_tab_TituloCIP(idx).TabCalcTit); --> Regras de calculo de juros                                 
        ...
        WHEN 4 THEN --> 2 - Somente valor mínimo
          ...
          IF 0 = fn_valid_max_min_valor 
                                  ...
                                  ,pr_vlmaxpgt => vr_tab_TituloCIP(idx).VlrTit             --> Valor maximo de pagamento
                                  ,pr_tpminpgt => vr_tab_TituloCIP(idx).IndrVlr_PercMinTit --> Tipo de calculo valor maximo(P-Percentual, V-Valor)
                                  ,pr_vlminpgt => vr_tab_TituloCIP(idx).Vlr_PercMinTit     --> Valor maximo de pagamento
                                   ) THEN
            vr_dscritic := 'Valor não permitido para pagamento.';
      ...
      IF vr_tab_TituloCIP(idx).VlrSldTotAtlPgtoTit > 0 THEN
 
        IF pr_vldpagto >= vr_tab_TituloCIP(idx).VlrSldTotAtlPgtoTit THEN
          IF substr(vr_tab_TituloCIP(idx).NumCodBarras,1,3) <> 85 THEN
