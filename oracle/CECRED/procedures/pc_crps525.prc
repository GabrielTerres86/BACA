CREATE OR REPLACE PROCEDURE CECRED.
         pc_crps525 (pr_cdcooper IN crapcop.cdcooper%TYPE   --> Cooperativa solicitada
  ...
  BEGIN
    ...
    Sigla   : CRED
    Autor   : Fernando
    Data    : Abril/2009                      Ultima Atualizacao: 15/01/2013
...
..............................................................................*/
    ...
    DECLARE
      ...
      -- Busca dos dados referente a limites de desconto
      CURSOR cr_craptab (pr_cdcooper crapcop.cdcooper%TYPE) IS
        ...
        FROM
          craptab tab
        WHERE
          tab.cdcooper = pr_cdcooper  AND -- Codigo da cooperativa
          tab.nmsistem = 'CRED'       AND -- Nome do sistema
          tab.tptabela = 'USUARI'     AND -- Tipo da tabela
          tab.cdempres = 11           AND -- Codigo da empresa
          tab.cdacesso = 'LIMDESCONT' AND -- Codigo de acesso
          tab.tpregist = 0;               -- Tipo de registro
    ...
    BEGIN
      ...
      -- Verifica se existe limites de desconto
      ...
      OPEN cr_crapass(pr_cdcooper => pr_cdcooper);
        ...
        LOOP
          ...
          vr_dtvencdc := rw_crapass.dtinivig + (rw_crapass.qtdiavig * SUBSTR(rw_craptab.dstextab, 19, 2));
          ...
          IF vr_dtvencdc > rw_crapdat.dtmvtoan AND vr_dtvencdc <= rw_crapdat.dtmvtolt THEN
            ...
            pc_escreve_xml('<nrdconta id="' || gene0002.fn_mask_conta(rw_crapass.nrdconta) || '">' ||
                             ...
                             '<qtrenova>' || TO_CHAR(SUBSTR(rw_craptab.dstextab, 19, 2),'9999999') || '</qtrenova>' ||
                           '</nrdconta>');
