...
BEGIN
  DECLARE
   ...
   /* TYPE typ_tab_crapscd IS TABLE OF typ_reg_crapscd INDEX BY PLS_INTEGER;
    ...
    CURSOR cr_crapepr(pr_cdcooper IN craptab.cdcooper%TYPE) IS    --> Código da cooperativa
    ...
    CURSOR cr_crawepr(pr_cdcooper IN craptab.cdcooper%TYPE) IS    --> Código da cooperativa
      SELECT cp.nrdconta
            ,cp.nrctremp
            ,cp.dtmvtolt
      FROM crawepr cp
    ...
    CURSOR cr_craplim(pr_cdcooper IN craptab.cdcooper%TYPE) IS    --> Código da cooperativa
      SELECT ci.nrdconta
            ,ci.vllimite
            ,ci.nrctrlim
            ,ci.dtpropos
    ...
    CURSOR cr_crappla(pr_cdcooper IN crappla.cdcooper%TYPE) IS      --> Código da cooperativa
     ...
     BEGIN
       ...
       BEGIN
        ...
        WHILE vr_index_crapsdv IS NOT NULL LOOP 
           ...
           IF vr_cont = 10000 THEN -- a cada 10000 registros, eh atualizado a crapsdv
