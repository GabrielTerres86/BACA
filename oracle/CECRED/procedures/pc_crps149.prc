CREATE OR REPLACE PROCEDURE CECRED.pc_crps149(pr_cdcooper IN crapcop.cdcooper%TYPE  --> Coop conectada
  ...
  /* ............................................................................
   ...
   Sigla   : CRED
   Autor   : Odair
   Data    : Marco/96.                       Ultima atualizacao: 13/08/2018
   ...
   Alteracoes: 26/08/96 - Alterado para nao gerar aviso para os associados que
               ...
               13/08/2018 - Gravar valores de contratos refinanciados com valor de Juros+60 da 
                            central de riscos (CRAPRIS) para TBEPR_LIQUIDADO_FINANCIADO.
  ...
  -- Seleciona os contratos que foram refinanciados na CRAPRIS
  CURSOR  cr_crapris (pr_cdcooper IN crapris.cdcooper%TYPE
                      ,pr_nrdconta IN crapris.nrdconta%TYPE
                      ,pr_nrctremp IN crapris.nrctremp%TYPE) IS
   --Busca informações dos contratos selecionados na crapris
     select  ris.cdcooper,
             ris.nrdconta,
             ris.inpessoa,
             ris.cdmodali,
             ...
             ris.vljura60
      from crapris ris
      where ris.nrctremp   =pr_nrctremp
      and   ris.cdcooper   =pr_cdcooper
      and   ris.nrdconta   =pr_nrdconta
      and   ris.qtdiaatr   >=60
      and   ris.vljura60   >0
      and   ris.dtrefere  = (select max(r.dtrefere)
                             from crapris r
                             where ris.nrctremp =pr_nrctremp
                             and   r.nrctremp   =ris.nrctremp
                             and   r.nrdconta   =ris.nrdconta
                             and   r.cdcooper   =ris.cdcooper);
  ...
  --Verifica se o contrato liquidado vem de refinanciamento
  vr_verif_refi NUMBER;
  ...
  --Verifica se o contrato foi liquidado para refinacimento
  FUNCTION fn_liquid_refi(par_cdcooper IN crawepr.cdcooper%TYPE
                         ,par_cdagenci IN crawepr.cdagenci%TYPE
                         ,par_nrdconta IN crawepr.nrdconta%TYPE
                         ,par_nrctremp IN crawepr.nrctremp%TYPE) return number is
  vr_return numeric;
  vr_verifica numeric;
  begin
   vr_verifica :=0;

   select count(*) into vr_verifica
   from tbepr_liquidado_financiado tlf
   where tlf.cdcooper = par_cdcooper
   and   tlf.nrdconta = par_nrdconta
   and   tlf.nrctremp = par_nrctremp;

   if vr_verifica = 0 then
    select count(*) into vr_return
    from   crawepr crwpr,
           crapepr epr
    where  (crwpr.nrctrliq##1   = par_nrctremp
        or    crwpr.nrctrliq##2 = par_nrctremp
        or    crwpr.nrctrliq##3 = par_nrctremp
        or    crwpr.nrctrliq##4 = par_nrctremp
        or    crwpr.nrctrliq##5 = par_nrctremp
        or    crwpr.nrctrliq##6 = par_nrctremp
        or    crwpr.nrctrliq##7 = par_nrctremp
        or    crwpr.nrctrliq##8 = par_nrctremp
        or    crwpr.nrctrliq##9 = par_nrctremp
        or    crwpr.nrctrliq##10= par_nrctremp)
        ...
        and   crwpr.cdcooper = par_cdcooper
        and   crwpr.nrdconta = par_nrdconta
        and   crwpr.cdagenci = par_cdagenci;
   end if;
      ...
      IF vr_tab_nrctrliq(vr_contador) > 0 THEN
        ...
        IF rw_crapepr.tpemprst = 0 THEN  -- Somente emprestimo antigo
          ...
          IF rw_crapepr.inliquid = 1 THEN -- Liquidado
            ...
            --Verifica se o contrato liquidado foi refinanciado
             IF fn_liquid_refi(rw_crapepr.cdcooper,
                               rw_crapepr.cdagenci,
                               rw_crapepr.nrdconta,
                               rw_crapepr.nrctremp) >0  THEN


              FOR rw_crapris in  cr_crapris (rw_crapepr.cdcooper
                                             ,rw_crapepr.nrdconta
                                             ,rw_crapepr.nrctremp)   LOOP

                   BEGIN
                     insert into tbepr_liquidado_financiado
                      ( cdcooper ,
                        nrdconta ,
                        qtdia_atraso,
                        cdmodali,
                  ...
                  EXCEPTION
                   WHEN OTHERS THEN
                     vr_dscritic := 'Erro ao inserir na tabela tbepr_liquidado_financiado. '||SQLERRM;
                     --Sair do programa
                    RAISE vr_exc_saida;
        ...
        ELSIF rw_crapepr.tpemprst = 2 THEN  -- Pos-Fixado
          ...
          IF fn_liquid_refi(rw_crapepr.cdcooper,
           ...
           END IF; --Fim Verifica Contrato Refinanciado
        END IF;
