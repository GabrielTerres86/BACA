...
BEGIN
   ...
   DECLARE
     ...
     CURSOR cr_sqarquiv(pr_dtrefere DATE) IS
       SELECT nvl(MAX(nrseqarq),0)
         FROM tbted_control_arq arq
        WHERE arq.dtrefere = pr_dtrefere;
     vr_nrseqarq tbted_control_arq.nrseqarq%TYPE;
     ...
     IF vr_listaarq IS NOT NULL THEN
       ...
       WHILE vr_idxtexto IS NOT NULL LOOP
         ...
         BEGIN
           ...
           OPEN cr_sqarquiv(vr_dtarquiv);
           FETCH cr_sqarquiv
            INTO vr_nrseqarq;
           CLOSE cr_sqarquiv;

           IF vr_nrseqhead <> vr_nrseqarq + 1 THEN
             vr_dsassunt := 'TEDs SICREDI - ARQUIVO COM SEQUENCIA INVALIDA';
             vr_dscorpoe := 'Arquivo '||vr_idxtexto||' com problema Header - Sequencia recebida --> '||vr_nrseqhead||', Sequencia esperada --> '||vr_nrseqarq+1;
             -- Direcionar para a saida com envio de email e encerramento
             vr_flgencer := TRUE;
             RAISE vr_exc_email;
           END IF;
           ...
           -- Enfim, efetuar a cópia do arquivo
           gene0001.pc_osCommand_Shell('mv '||vr_dir_sicredi_teds||'/'||vr_idxtexto||' '||vr_dir_backup_teds);
