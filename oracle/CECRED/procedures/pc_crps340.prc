...
BEGIN
  ...
  Sigla   : CRED
  Autor   : Margarete
  Data    : Abril/2003.                     Ultima atualizacao: 30/10/2013
  ...
  DECLARE
    ...
    --Busca o valor limite parametrizado para microfilmagem
    CURSOR cr_craptab IS
    SELECT gene0002.fn_char_para_number(tab.dstextab)
      FROM craptab tab
     WHERE tab.cdcooper = pr_cdcooper
       AND tab.nmsistem = 'CRED'
       AND tab.tptabela = 'GENERI'
       AND tab.cdempres = 0
       AND tab.cdacesso = 'VLMICFLIMI'
       AND tab.tpregist = 0;
  ...
  BEGIN
    ...
    --Busca o valor limite dos microfilmes parametrizados na craptab.
    OPEN cr_craptab;
    FETCH cr_craptab INTO vr_vlrmifil;
    -- Se nao encontrar
