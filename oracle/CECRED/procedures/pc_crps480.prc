CREATE OR REPLACE PROCEDURE CECRED.PC_CRPS480(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Cooperativa solicitada
  ...
  /*..............................................................................
     ...
     Sigla   : CRED
     Autor   : David
     Data    : Maio/2007                       Ultima atualizacao: 03/12/2015
  ...
  ..............................................................................*/
    ...
    DECLARE
      ...
      -- Tipo para armazenar as situações das aplicações
      -- E evitar muitas leituras na craptab
      TYPE typ_tab_blqrgt IS
        TABLE OF VARCHAR2(16)
          INDEX BY VARCHAR2(16); --> Chave composta [Conta(10) + Aplica(6)]
      -- Vetor para armazenar os dados de detalhe
          ...
          FROM (SELECT SUM(vlsldmed) vlsldmed
                  FROM (SELECT SUM(vlsrdcpo) vlsldmed
                        ...
                        SELECT SUM(vlslfmea) --Select abaixo para buscar valores do ultimo dia do mês, pois não contem dados na sda
                         ...
                         WHERE cdcooper = pr_cdcooper
                           AND nrdconta = pr_nrdconta
                           AND dtsdfmea = last_day(pr_dtmvtoan))); --Busca do último dia do mês
      ...
      BEGIN
        ...
        IF NOT vr_tab_detalhe.EXISTS(vr_des_chave_det) THEN
          ...
          IF vr_tab_blqrgt.EXISTS(LPAD(pr_nrdconta,10,'0')||LPAD(pr_nraplica,6,'0')) THEN
            -- Situação bloqueada
    ...
    BEGIN
      ...
      FOR rw_craptab IN cr_craptab(pr_cdcooper => pr_cdcooper
        ...
        vr_tab_blqrgt(lpad(rw_craptab.cdacesso,10,'0')||LPAD(SUBSTR(rw_craptab.dstextab,2,6),6,'0')) := lpad(rw_craptab.cdacesso,10,'0')||LPAD(SUBSTR(rw_craptab.dstextab,2,6),6,'0');
      END LOOP;
