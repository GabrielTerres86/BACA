   ...
   gl_nrdolote    craplot.nrdolote%type;
    ...
    Procedure pc_gera_prejuizo_emprestimo IS
       ...
       cursor c_busca_epr(pr_cdcooper number
            ...
            /*rocedure pc_grava_log_transferencia (pr_cdcooper in number
                                             ,
        begin
           begin
                               insert into  tabela
                               ...
                               ,nrdconta -- conta
                               ,nrctremp -- contrato emprestimo
                               ,dttransf -- data de transferencia
                               ,idtiptrf -- indicador de tipo transferencia 1 - Emprestimo ou 2 - Conta
                               ,tpemprst -- 0 - TR; 1 - PP ou nulo se for conta
                               ,vlslddev -- saldo devedor original
                               ,vlrmulta -- valor multa anterior a transferencia
                               ,vlrjuros -- valor dos juros anterior a transferencia
                               ,vllimcre -- limite de credito original (para contas)
                               ,qtdiaatr -- dias em atraso anterior à transferencia
                               ,qtpresta -- quantidade de prestacoes do contrato
                               ,qtpreliq -- quantidade de prestacoes pagas (liquidadas)
                               ,idsitcar -- indicador da situacao do cartão magnetico antes transferencia)
                               ,idsitint -- indicador situacao liberação internet (antes transferencia)
                               ,idsitcta -- indicador da situacao da conta antes da transferencia
                               ,idsitccr -- indicador situacao cartao de credito antes transferencia
                               ,nrctrnov -- novo contrato gerado na transferencia para prejuizo
                               ) values
          exception
      ...
      begin
        for rw_busca_cooper in c_busca_cooper loop
            ...
            for rw_busca_epr in c_busca_epr(rw_busca_cooper.cdcooper
                          ...
                          /* Se for emprestimo PP
                             - grava trilha de auditoria
                             - gera transferencia em conformidade com a bw1gen0084.p (convertida para PC_transfere_contrato_prejuizo)

                          */
                          if rw_busca_epr.tpemprst = 1 then /* emprestimo PP */
                            --pc_grava_log_transferencia
                             ...
                             PREJ0001.pc_transfere_epr_prejuizo_PP(pr_cdcooper => rw_busca_cooper.cdcooper
                              ...
                              if vr_des_reto <> 'OK' then
                                 vr_cdcritic := 0;
                                 vr_dscritic := 'Erro execucao da rotian transfere_prejuizo: ' || sqlerrm;
