CREATE OR REPLACE PROCEDURE CECRED.PC_CRPS280_I_1(pr_cdcooper   IN crapcop.cdcooper%TYPE --> Coop. conectada
                                                 ,pr_dtrefere   IN DATE                      --> Data de referência para o cálculo
                                                 ,pr_cdprogra   IN crapprg.cdprogra%TYPE) IS     --> Codigo programa conectado
                                            --     ,pr_dsdircop   IN crapcop.dsdircop%TYPE     --> Conta da cooperativa conectada
                                            --     ,pr_nrctactl   IN crapcop.nrctactl%type) IS --> Diretório da cooperativa
...
BEGIN
  /* ..........................................................................
     ...
     Alteracoes: 16/02/2006 - Alterado p/ listar Riscos por PAC (Diego).
                 ...
                 16/05/2018 - Criação da rotina que é chamado por um JOB, cópia da parte da geração dos
                              retórios da crps280_i e retirada do paralelismo
                              Projeto Ligeirinho - Josiane Stiehler - AMcom
  ............................................................................. */
  ...
  DECLARE
    ...
    pr_vltotprv NUMBER(14,2); --> Total acumulado de provisão
    pr_vltotdiv NUMBER(14,2); --> Total acumulado de dívida
    ...
    -- Código do programa
    vr_cdprogra   varchar2(15) ; --crapprg.cdprogra%type;
  ...
  CURSOR cr_craptco_credimilsul_scrcred IS
   ...
   -- Busca de registro de transferência entre cooperativas
    ...
    BEGIN
      BEGIN
        FOR r_dados_epr IN c_dados_epr LOOP
          ...
          vr_tab_dados_epr(r_dados_epr.vr_indice).dtdpagto := to_date(r_dados_epr.dtdpagto,'mm/dd/yyyy');
          ...
          vr_tab_dados_epr(r_dados_epr.vr_indice).dtmvtolt := to_date(r_dados_epr.dtmvtolt,'mm/dd/yyyy');
          ...
          vr_tab_dados_epr(r_dados_epr.vr_indice).dtinictr := to_date(r_dados_epr.dtinictr,'mm/dd/yyyy');
          vr_tab_dados_epr(r_dados_epr.vr_indice).dtdpagto := to_date(r_dados_epr.dtdpagto,'mm/dd/yyyy');
        END LOOP;
      EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
      BEGIN
        FOR r_crapris IN c_crapris LOOP
          ...
          vr_tab_crapris(r_crapris.vr_indice).dtrefere := to_date(r_crapris.dtrefere,'mm/dd/yyyy');
          ...
          vr_tab_crapris(r_crapris.vr_indice).dtinictr := to_date(r_crapris.dtinictr,'mm/dd/yyyy');
    ...
    -----------------------------------------
    -- Inicio Bloco Principal pc_crps280_I_1
    -----------------------------------------
  ...
  BEGIN
    ...
    pc_controla_log_programa('O', 'Programa CRPS280_I_1 iniciado');
    -- Inicializar pl-table
    pc_inicializa_pltable;

    vr_cdprogra:= 'PC_CRPS280_I_1';
    ...
    END IF;
      ...
      pc_controla_log_programa('O', 'Programa CRPS280_i_1 Inicio Loop vr_tab_crapris ');
      ...
      pc_controla_log_programa('O', 'Programa CRPS280_i_1 Fim Loop vr_tab_crapris ');
      ...
      -- Atualiza somente quando o programa que chamou ´´e o crps280
      -- projeto ligeirinho
      IF pr_cdprogra = 'CRPS280' THEN 
             ...
             BEGIN
                   INSERT INTO crapbnd(cdcooper
                    ...
                    VALUES(3                      --> Cooperativa conectada
                          ,pr_rw_crapdat.dtmvtolt    --> Data atual
                          ,pr_nrctactl    --> Conta da cooperativa conectada
                          ,pr_vltotprv            --> Total acumulado de provisão
                          ,pr_vltotdiv);          --> Total acumulado de dívidas
            EXCEPTION
              WHEN DUP_VAL_ON_INDEX THEN
                BEGIN
                  UPDATE crapbnd
                     SET vltotprv = pr_vltotprv       --> Total acumulado de provisão
                        ,vltotdiv = pr_vltotdiv      --> Total acumulado de dívidas
                   WHERE cdcooper = 3                    --> Cooperativa Gravar sempre na Cecred
                     AND dtmvtolt = pr_rw_crapdat.dtmvtolt  --> Data atual
                     AND nrdconta = pr_nrctactl; --> Conta da cooperativa conectada
             EXCEPTION
                WHEN OTHERS THEN
                    --Montar mensagem de erro
                    vr_cdcritic := 1035;
                    vr_dscritic := gene0001.fn_busca_critica(vr_cdcritic)||' crapbnd: '||
                                  'vltotprv:'  ||pr_vltotprv||
                                  ', vltotdiv:'||pr_vltotdiv||
                                  ' com cdcooper:3, dtmvtolt:'||pr_rw_crapdat.dtmvtolt||
              ...
              WHEN OTHERS THEN
                ...
                vr_cdcritic := 1034;
                vr_dscritic := gene0001.fn_busca_critica(vr_cdcritic)||' crapbnd: '||
                              'cdcooper:3, dtmvtolt:'||pr_rw_crapdat.dtmvtolt||
                              ', nrdconta:'||pr_nrctactl||
                              ', vltotprv:'||pr_vltotprv||
                              ', vltotdiv:'||pr_vltotdiv||
                              '. '||sqlerrm;
      ...
      -- limpa tabela de memória (tbgen_batch_relatorio_wrk)
      pc_clear_memoria;
      --Salvar informacoes no banco de dados
      commit;

 
      pc_controla_log_programa('O', 'Programa CRPS280_i_1 finalizando');
  ...
  EXCEPTION
    WHEN vr_exc_erro THEN
      ...
      pr_dscritic := 'Erro na rotina PC_CRPS280_I_1 (vr_exc_erro). Detalhes: '||pr_dscritic;
    ...
    WHEN vr_exc_saida THEN
	 ...
	 WHEN OTHERS THEN
      ...
      pr_cdcritic := 0;
      pr_dscritic := vr_dscritic ||'.Erro não tratado na rotina PC_CRPS280_I_1.(others) Detalhes: ' ||SQLERRM;
