...
BEGIN
  /* ............................................................................
    ...
    Data    : Abril/2015                      Ultima atualizacao: 06/07/2016
    ...
    14/04/2016 - Ajuste para validar se o arquivo que foi recebido pelo webservice
                 nao possui erro. Caso contenha erro deve aguardar 5 segundos
                 e buscar o arquivo novamente. Se nas 10 tentativas nao conseguir
                 um arquivo valido, envia email para a cobranca.
                 (Douglas - Chamado 424471)

    06/07/2016 - Ajustar a mensagem enviado por e-mail (Douglas - Chamado 480243)
  .......................................................................... */
  ...
  DECLARE
    ...
    -- Define se encontrou um arquivo valido
    vr_arq_valido BOOLEAN;
    -- Exception para quando nao consegui arquivo
    vr_arq_invalido EXCEPTION;
    -- Mensagem de erro no arquivo
    vr_erro_arquivo VARCHAR2(32600);
  ...
  BEGIN
    ...
    vr_arq_valido := FALSE;
    -- Tentar 10 vezes a busca de um arquivo valido
    FOR i IN 1..10 LOOP
      ...
      IF  utl_file.IS_OPEN(vr_ind_arquivo) THEN
        ...
        -- Verificar se ocorreu erro na geracao do arquivo
        IF UPPER(vr_des_linha) LIKE '%OCORREU%ERRO%ARQUIVO%' THEN
          -- Aramazena a critica do erro que estao no arquivo
          vr_erro_arquivo:= vr_des_linha;
          -- Fechar o arquivo
          gene0001.pc_fecha_arquivo(pr_utlfileh => vr_ind_arquivo); --> Handle do arquivo aberto
          -- Encontrou um arquivo valido
          vr_arq_valido:= FALSE;
        ...
        ELSE 
          ...
          -- Sair do LOOP
          EXIT;
        END IF; -- Validacao de arquivo valido
      END IF; -- Arquivo aberto
    END LOOP; -- LOOP de busca de arquivos validos
    
    -- Verificar se encontrou arquivo valido
    IF NOT vr_arq_valido THEN
      RAISE vr_arq_invalido;
    END IF;
...
--             '<vltitulo>'||  vr_vlrtotal    ||'</vltitulo>'||
  ...
  WHEN vr_arq_invalido THEN
    -- Enviar e-mail para a cobranca informando erro
    GENE0003.pc_solicita_email(pr_cdcooper        => 3 /* cecred */
                              ,pr_cdprogra        => vr_cdprogra
                              ,pr_des_destino     => vr_des_destino
                              ,pr_des_assunto     => 'Cobrança - ARQUIVO DE RETORNO PG NAO PROCESSADO'
                              ,pr_des_corpo       => '<b>ATEN'||Chr(38)||'Ccedil;'||Chr(38)||'Atilde;O!</b><br><br>'||
                                                     'Voc'||Chr(38)||'ecirc; est'||Chr(38)||'aacute; recebendo este ' ||
                                                     'e-mail para lhe informar que o arquivo de retorno da PG n'||Chr(38)||'atilde;o foi processado. <br><br>' ||
                                                     'Cr'||Chr(38)||'iacute;tica recebida no arquivo: <b>'  || vr_erro_arquivo || '</b><br><br>'||
                                                     'Voc'||Chr(38)||'ecirc; deve entrar em contato com a PG ' ||
                                                     ' e verificar o motivo do erro na gera'||Chr(38)||'ccedil;'||Chr(38)||'atilde;o do arquivo.'
                              ,pr_des_anexo       => NULL
                              ,pr_des_erro        => vr_dscritic);
    ...
    BTCH0001.pc_gera_log_batch(pr_cdcooper     => 3
                              ...
                              ,pr_des_log      => to_char(SYSDATE,'hh24:mi:ss')||' - '|| vr_cdprogra || ' --> '||
                                                  'Arquivo de retorno da PG nao foi processado. ' ||
                                                  'Critica recebida no arquivo: ' || vr_erro_arquivo ||
                                                  ' . Entrar em contato com a PG para verificar o ' ||
                                                  'motivo do erro na geracao do arquivo');
