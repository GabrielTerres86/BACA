...
BEGIN
  ...
  /* .............................................................................
     ...
     Sigla   : CRED
     Autor   : Ze Eduardo
     Data    : Marco/2005.                     Ultima atualizacao: 31/03/2016
     ...
     Alteracoes: 01/07/2005 - Alimentado campo cdcooper das tabelas craplot,
                 ...
                 22/10/2012 - Quando der critica 258 enviar email para
                              Diego, David, Mirtes, Z√© e cpd@cecred (Ze).
                 ...
                 18/10/2013 - Convers„o Progress -> Oracle - Alisson (AMcom)

                 25/11/2013 - Ajustes na passagem dos par‚metros para restart;
                            - TambÈm limpeza de variaveis n„o utilizadas;
                            - E por fim, chamada ao encerramento de elimina_restart
                            (Marcos-Supero)

                 17/12/2013 - Adaptacao para migracao VIACREDI/ACREDI (Ze/Rodrigo).

                 19/12/2013 - GeraÁ„o de arquivo com registros rejeitados para
                              lanÁamento em cooperativa migrada (Lucas).

                 20/12/2013 - Totalizadores separados por DÈbito e CrÈdito para
                              o crrl414_99 (Lucas).

                 27/12/2013 - GravaÁ„o da Data de LiberaÁ„o no arquivo de registros
                              rejeitados para a criaÁ„o da crapdpb (Lucas).

                 13/03/2014 - Incluido tratamento no relatÛrio 414 para listar
                              TEC salario nao integrada na relacao de criticas,
                              devido alteracao do historico deste lancamento no
                              arquivo deb558 (Diego).

                 19/03/2014 - Implementada manutenÁ„o ref. 201403 tratando TEC salario
                 ...
                 06/05/2014 - CorreÁ„o no tratamento para historico TEC SALARIO,
                              foi colocado a verificaÁ„o nos pontos conforme fonte progress.
                              (Tiago Castro - RKAM)

                 23/05/2014 - Ajustado leitura do craplcm2 e ajustado cdcritic(Odirlei/Amcom)

                 12/09/2014 - Ajustes devido a migraÁ„o das coop. Concredi e Credimilsul
                              e inclus„o da geraÁ„o temporaria do arquivo RegistrosRejeitadosMigracao
                              (Odirlei/Amcom)

                 26/09/2014 - Ajustes devido a incorporaÁ„o da coop Concredi, ser· necessario
                              ler os arquivos da concredi no processamento da viacredi, porÈm todos os
                              registros ser„o rejeitados, apenas sendo apresentados nos relatorios.(Odirlei/Amcom)

                 30/09/2014 - Ajustes devido a incorporaÁ„o da coop credimilsul, ser· necessario
                              ler os arquivos da credimilsul no processamento da scrcred,
                              e identificar a nova conta atraves da craptco para gerar os lanÁamentos
                              na nova coop.(Odirlei/Amcom)

                 29/10/2014 - #185818 HistÛrico 0500 (cheque) retirado (Carlos)

                 02/12/2014 - Descomentado regras da incorporaÁ„o da credimilsul
                 ...
                 31/08/2015 - Apos definiÁ„o dos historicos que devem ser processados para o hist 444
  ...
  DECLARE
     ...
     CURSOR  cr_craprej_tot (pr_cdcooper IN craprej.cdcooper%type
       ...
       -- serem migrados
       -- n„o È necessario exibir as criticas 999 no relatorio
     ...
     BEGIN
       ...
       FOR rw_rej IN cr_craprej_999 (pr_cdcooper => pr_cdcooper
           ...
           IF INSTR(vr_dshstblq,SUBSTR(vr_dshistor,01,04)) > 0 THEN /* Deposito Bloqueado */
            ...
            ELSE
              ...
              WHILE vr_contador > 1 loop
                 ...
                 --verificar se È feriado ou sabado ou domingo
         ...
         ELSE
           ...
           /* Gerar arquivo damigraÁ„o da 4-concredi para 1-viacredi e
       ...
       IF vr_flimprel THEN
         ...
         IF vr_email_dest IS NULL THEN
           ...
           vr_dscritic:= 'N√o foi encontrado destinatario para relatorio TCO.';
           --Levantar Excessao
           RAISE vr_exc_saida;
         END IF;
         ...
         gene0002.pc_solicita_relato (pr_cdcooper  => pr_cdcooper                  --> Cooperativa conectada
                                     ...
                                     ,pr_dsassmail => vr_des_assunto               --> Assunto do e-mail que enviar√° o relat√≥rio
         ...
         IF pr_cdcooper in (1,13) AND
           ...
           /*geraÁ„o temporaria do arquivo devido a migraÁ„o da concredi e credimilsul*/
   ...
   BEGIN
     ...
     /* para a cooperativa viacredi, È necessario importar tambÈm os arquivos
     ...
     /* para a cooperativa SCRCRED, È necessario importar tambÈm os arquivos
     ...
     IF vr_inrestar = 0 THEN
       ...
       IF vr_typ_saida = 'ERR' THEN
         vr_dscritic:= 'N„o foi possÌvel executar comando unix. '||vr_comando;
         RAISE vr_exc_saida;
       END IF;

       -- Buscar o diret√≥rio padrao da cooperativa conectada
       ...
       IF pr_cdcooper = 1 THEN
         ...
         IF vr_tab_arquivo.Count > 0 THEN
           ...
           END LOOP;
         END IF; --vr_tab_arquivo.count
       END IF; --  fim verificaÁ„o arquivos concredi
       ...
       IF pr_cdcooper = 13 THEN
         ...
         IF vr_tab_arquivo.Count > 0 THEN
           ...
           END LOOP;
         END IF; --vr_tab_arquivo.count
       END IF; --  fim verificaÁ„o arquivos credimilsul

       -- Sen„o encontrou nenhum arquivo, deve abortar programa
     ...
     FOR vr_contaarq IN 1..9 LOOP
       ...
       gene0001.pc_abre_arquivo(pr_nmdireto => vr_caminho_integra  --> Diret√≥rio do arquivo
       ...
       LOOP
         ...
         ELSE
           vr_nrdctabb:= fn_ver_contaitg(SUBSTR(vr_setlinha,33,09));
           --Verificar se È conta integracao
           ...
           -- N„o deve atualizar o saldo, do contrario atualizar·
         ...
         IF pr_cdcooper = 1 AND
            vr_tab_nmarqint(vr_contaarq) like '%_CONCREDI%' THEN
           -- verificar se È uma conta migrada da coop 4
           --Localizar se È uma conta migrada pela conta itg
           OPEN cr_craptco_itg (pr_cdcooper => pr_cdcooper
         ...
         /* tratar migraÁ„o da 4-concredi para 1-viacredi e
                              15-credimilsul para 13-SCRCRED */
         IF pr_cdcooper in (1,13) AND
           (rw_crapdat.dtmvtolt >= to_date('11/18/2014','MM/DD/RRRR')   AND
            rw_crapdat.dtmvtolt <= to_date('11/30/2014','MM/DD/RRRR'))  THEN
           --Localizar assosiado pelo numero integraÁaı
           IF vr_tab_nrdctitg.EXISTS(lpad(vr_nrdctitg_arq,10,'0'))  THEN
         ...
         IF vr_cdcritic = 0 THEN
           ...
           IF vr_flgdepos OR vr_flgestor OR vr_flgdebit THEN
         ...
         BEGIN
               ...
               ELSE
                 ...
                 WHILE vr_contador > 1 LOOP
                   ...
                   vr_dtferiado:= gene0005.fn_valida_dia_util(pr_cdcooper => pr_cdcooper
                                                             ...
                                                             ,pr_tipo => 'P');
                   --Se a data for diferente √© feriado ou fim semana
           ...
           IF vr_flgentra AND vr_flgchequ THEN
             ...
             IF vr_tab_craphis(rw_craplcm.cdhistor) = 'D' THEN
               ...
               IF vr_rowid_debito IS NULL THEN
                 ...
                 -- Se cursor est· aberto -- Renato - supero - 25/04
           ...
           ELSIF vr_flgentra AND vr_flgdepos THEN
             ...
             IF vr_tab_craphis(vr_cdhistor) = 'C' THEN
               ...
               END;
             END IF;
           ELSIF vr_flgentra AND (vr_flgestor OR vr_flgdebit) THEN
             --Verificar se o Historico Existe
             IF NOT vr_tab_craphis.EXISTS(vr_cdhistor) THEN
               vr_cdcritic:= 80;
               vr_dscritic:= gene0001.fn_busca_critica(vr_cdcritic);
             ...
             IF vr_cdhistor = 297 THEN
               vr_cdhistor:= 290;
             ELSIF vr_cdhistor NOT IN (611,613,614,658,668,661,471,662) THEN
               vr_cdhistor:= 290;
             END IF;
             --Inserir Lancamento
             BEGIN
       ...
       IF NOT vr_flgarqvz THEN
         ...
         FOR rw_craprej_tot IN cr_craprej_tot (pr_cdcooper => pr_cdcooper
           ...
           IF vr_cdcritic <> rw_craprej_tot.cdcritic or
             ...
             END IF;
             IF rw_craprej_tot.cdcritic IN (96,137,172,257,287,439,508,608) THEN
               vr_dscritic:= '* '||vr_dscritic;
             END IF;
           END IF;
           IF rw_craprej_tot.cdcritic NOT IN (96,137,172,257,287,439,508,608,762) THEN
           ...
           --tratando a tec salario n„o integrada na relacao de criticas
           IF vr_cdcritic = 0 THEN
       ...
       -- limpar temptables que ser„o utilizadas
    ...
    END IF;
     ...
     EXCEPTION
       WHEN OTHERS THEN
         vr_dscritic := 'N„o foi possivel limpar tabela craprej: '||SQLerrm;
         raise vr_exc_saida;
     END;
