CREATE OR REPLACE PROCEDURE CECRED.pc_crps495(pr_cdcooper  in crapcop.cdcooper%type,  --> Cooperativa
...
/* ..........................................................................
   ...
   Programa: PC_CRPS495 (Antigo Fontes/crps495.p)
   Sistema : Conta-Corrente - Cooperativa de Credito
   Sigla   : CRED
   Autor   : David
   Data    : Maio/2007                       Ultima atualizacao: 14/06/2016

   Dados referentes ao programa:

   Frequencia : Diario.
   Objetivo   : Calcular o saldo das aplicacoes RDCPRE e RDCPOS com resgate
                programado e efetuar os creditos em conta, usando glb_dtmvtopr.
                Emite relatorio 462. Solicitacao 5 e ordem 39.
   ...
   Alteracoes : 21/09/2007 - Desmembrar em dois, um rodando com olt e outro
                ...
                07/02/2008 - Melhorar critica 913, valor resgatado (Magui).

                08/02/2008 - Acerto na data de geracao dos lotes (Magui).
                ...
                05/06/2008 - Corrigir lote na listagem feitos (Magui).
               ...
               15/09/2009 - Acerto qtde dias em carencia para o RDCPOS (Magui).

               25/08/2011 - Nao zerar vlslfmes quando ultimo dia do mes.
                            Campos sera zerado no crps249 (Magui).

               30/07/2013 - Conversão Progress >> Oracle PL/SQL (Daniel - Supero), baseado na pc_crps478.
               ...
               14/06/2016 - Ajustado a leitura da craptab para o MXRENDIPOS e a leitura das contas
                            bloqueadas para que seja utilizado o indice da tabela
...
............................................................................. */
  ...
  vr_dstextab   craptab.dstextab%TYPE;       --> Texto da Tabela
  ...
  CURSOR cr_craplrg (pr_cdcooper IN craplrg.cdcooper%TYPE           --> Código cooperativa
                    ,pr_dtmvtopr IN craplrg.dtresgat%TYPE) IS       --> Data movimento atual
    ...
    WHERE cg.cdcooper  = pr_cdcooper
      AND cg.dtresgat <= pr_dtmvtopr
      AND cg.inresgat  = 0
      AND cg.tpresgat in (1, 2)
    order by cg.nrdconta,
             cg.nraplica,
             cg.dtmvtolt,
             cg.nrseqdig;
  ...
  CURSOR cr_craptab_ctablq (pr_cdcooper  IN crapdat.cdcooper%TYPE          --> Código da cooperativa
                    ...
                    ,pr_cdacesso  IN craptab.cdacesso%TYPE          --> Código de acesso
                           ,pr_nraplica  IN craptab.dstextab%TYPE) IS          --> Tipo da aplicação
    SELECT ct.dstextab
    FROM craptab ct
    WHERE ct.cdcooper = pr_cdcooper
       ...
       AND upper(ct.cdacesso) = upper(pr_cdacesso)
       AND ct.tpregist > 0
       AND substr(ct.dstextab, 1, 7) = pr_nraplica;
  rw_craptab cr_craptab_ctablq%rowtype;
  ...
  CURSOR cr_crapdtc (pr_cdcooper IN crapdtc.cdcooper%TYPE           --> Código cooperativa
    ...
    WHERE cc.cdcooper = pr_cdcooper
      ...
      AND (cc.tpaplrdc = 1 OR cc.tpaplrdc = 2);
  ...
  CURSOR cr_craplap (pr_cdcooper IN crapdtc.cdcooper%TYPE       --> Código cooperativa
                    ...
                    ,pr_dtmvtopr IN craprda.dtmvtolt%TYPE) IS   --> Data movimento atual
    SELECT cl.txaplica
          ,cl.txaplmes
          ,cl.cdhistor
          ,cl.vllanmto
  ...
  CURSOR cr_craplaplast (pr_cdcooper   IN craplrg.cdcooper%TYPE      --> Codigo cooperativa
                        ...
                        ,pr_dtmvtopr   IN craplap.dtmvtolt%TYPE      --> Data de movimento atual
  ...
  CURSOR cr_craplcm (pr_cdcooper   IN craplrg.cdcooper%TYPE        --> Código cooperativa
                    ,pr_dtmvtopr   IN craplot.dtmvtolt%TYPE        --> Data movimento atual
  ...
  CURSOR cr_craprej (pr_cdcooper   IN craplrg.cdcooper%TYPE        --> Código cooperativa
                    ,pr_dtmvtopr   IN craplot.dtmvtolt%TYPE) IS    --> Data movimento atual
    SELECT cj.ROWID,
           cj.cdcritic,
           cj.nrdconta,
           cj.nraplica,
...
--    PRAGMA AUTONOMOUS_TRANSACTION;
  ...
  PROCEDURE pc_cria_wcraprej (pr_dtmvtopr      IN DATE
                             ,pr_nrdconta      IN craplrg.nrdconta%TYPE
                             ,pr_nraplica      IN craplrg.nraplica%TYPE
  ...
  BEGIN
    ...
    pr_tab_craprej(vr_craprej).dtmvtolt := pr_dtmvtopr;
  ...
  PROCEDURE pc_gera_lancamentos_craplci(pr_flgctain  IN craprda.flgctain%TYPE
    ...
    CURSOR cr_crapsli (pr_cdcooper  IN crapcop.cdcooper%TYPE
      ...
      WHERE ci.cdcooper  = pr_cdcooper
        ...
        AND to_char(ci.dtrefere, 'MM') = to_char(pr_dtmvtopr, 'MM')
        AND to_char(ci.dtrefere, 'RRRR') = to_char(pr_dtmvtopr, 'RRRR');
  ...
  BEGIN
    IF pr_flgctain = 1 AND pr_flgcreci = 0 THEN
      ...
      -- Buscar lote
      OPEN cr_craplot(pr_cdcooper, pr_dtmvtopr, 1, 100, 10111);
      ...
      OPEN cr_craplot(pr_cdcooper, pr_dtmvtopr, 1, 100, 10112);
      FETCH cr_craplot INTO rw_craplot;
      ...
      UPDATE craplot ct
         SET ct.qtinfoln = ct.qtinfoln + 1,
             ct.qtcompln = ct.qtcompln + 1,
             ct.vlinfocr = ct.vlinfocr + nvl(pr_vlresgat,0),
             ct.vlcompcr = ct.vlcompcr + nvl(pr_vlresgat,0),
             ct.nrseqdig = rw_craplot.nrseqdig + 1
       WHERE ct.rowid = rw_craplot.rowid
      RETURNING nrseqdig INTO rw_craplot.nrseqdig;
      EXCEPTION
    ...
    IF pr_flgcreci = 1 THEN
      ...
      OPEN cr_craplot(pr_cdcooper, pr_dtmvtopr, 1, 100, 10113);
      FETCH cr_craplot INTO rw_craplot;
      ...
      OPEN cr_crapsli(pr_cdcooper, pr_nrdconta, pr_dtmvtopr);
      ...
      IF cr_crapsli%notfound OR rw_crapsli.retorno > 1 THEN
        ...
        vr_dtrefere := last_day(pr_dtmvtopr);
      ...
      ELSE
        ...
        BEGIN
          UPDATE crapsli ci
          SET ci.vlsddisp = ci.vlsddisp + nvl(pr_vlresgat,0)
  ...
  EXCEPTION
    WHEN others THEN
      pr_cdcritic := 0;
      pr_des_erro := 'Problemas na Rotina apli0001.pc_rendi_apl_pos_com_resgate ' || pr_cdcooper || '. Erro: ' || sqlerrm;
...
BEGIN
  ...
  gene0001.pc_informa_acesso(pr_module => 'PC_CRPS495',
                             pr_action => null);
  ...
  FOR rw_craplrg IN cr_craplrg(pr_cdcooper, rw_crapdat.dtmvtopr) LOOP
    ...
    OPEN cr_craptab_ctablq (pr_cdcooper => pr_cdcooper
                           ,pr_nmsistem => 'CRED'
                           ,pr_tptabela => 'BLQRGT'
                           ,pr_cdempres =>  0
                           ,pr_cdacesso => lpad(to_char(rw_craplrg.nrdconta), 10, '0')
                           ,pr_nraplica => lpad(to_char(rw_craplrg.nraplica),  7, '0'));
    FETCH cr_craptab_ctablq INTO rw_craptab;

    -- Caso a busca encontre registros
    IF cr_craptab_ctablq%FOUND THEN
      vr_cdcritic := 640;
      CLOSE cr_craptab_ctablq;
      ...
      pc_cria_craprej (pr_cdcooper  => pr_cdcooper
                      ...
                      ,pr_cdcritic  => vr_cdcritic
                      ,pr_gdtmvtol  => rw_crapdat.dtmvtopr
    ...
    vr_txapllap := rw_craplap.txaplica;
    ...
    OPEN cr_craplot (pr_cdcooper, rw_crapdat.dtmvtopr, 1, 100, 8496);
    FETCH cr_craplot INTO rw_craplot;
    ...
    BEGIN
      UPDATE craplot co
      SET co.vlinfodb = co.vlinfodb + nvl(vr_vlresgat,0)
         ,co.vlcompdb = co.vlcompdb + nvl(vr_vlresgat,0)
    ...
    IF rw_crapdtc.tpaplrdc = 1 THEN
      ...
      IF vr_vlestprv > 0 THEN
        ...
        IF vr_des_reto NOT IN ('OK') THEN
          vr_cdcritic := 0;
          vr_dscritic := TO_CHAR(SYSDATE, 'HH24:MI:SS') || ' --> ' || vr_cdprogra || ' - ' || vr_tab_erro(vr_tab_erro.last).dscritic;
    ...
    ELSIF rw_crapdtc.tpaplrdc = 2 THEN
      ...
      IF (rw_crapdat.dtmvtopr - rw_craprda.dtmvtolt) < rw_craprda.qtdiauti
      ...
      apli0001.pc_saldo_rgt_rdc_pos (pr_cdcooper => pr_cdcooper
                                    ...
                                    ,pr_dtaplrgt => rw_crapdat.dtmvtopr
                                    ,pr_vlsdorgt => 0
                                    ,pr_flggrvir => FALSE
      ...
      IF vr_des_reto = 'NOK' THEN
        ...
        vr_dscritic := TO_CHAR(SYSDATE,'HH24:MI:SS') ||
                       ' - ' || vr_cdprogra || ' --> ' || vr_tab_erro(vr_tab_erro.last).dscritic ||
      ...
      IF vr_vlrrgtot < rw_craplrg.vllanmto THEN
        ...
        pc_cria_wcraprej (pr_dtmvtopr => rw_crapdat.dtmvtopr
                         ,pr_nrdconta => rw_craplrg.nrdconta
                         ,pr_nraplica => rw_craplrg.nraplica
      ...
      IF rw_crapdat.dtmvtopr - rw_craprda.dtmvtolt < rw_craprda.qtdiauti THEN
      ...
      ELSE
        ...
        apli0001.pc_valor_original_resgatado(pr_cdcooper => pr_cdcooper
                                            ...
                                            ,pr_nrctaapl => rw_craprda.nrdconta
                                            ,pr_nraplres => rw_craprda.nraplica
                                            ,pr_dtaplrgt => rw_craplrg.dtresgat
      ...
      IF vr_vlbasrgt > 0 OR rw_crapdat.dtmvtopr - rw_craprda.dtmvtolt < rw_craprda.qtdiauti THEN
      ...
      IF rw_craplrg.tpresgat = 2 THEN
        ...
        BEGIN
          UPDATE craplot co
          SET co.vlinfodb = co.vlinfodb - nvl(vr_vlresgat,0) + nvl(vr_vlsolrgt,0)
             ,co.vlcompdb = co.vlcompdb - nvl(vr_vlresgat,0) + nvl(vr_vlsolrgt,0)
          WHERE co.ROWID = rw_craplot.ROWID
      ...
      IF to_char(rw_craprda.dtmvtolt, 'MM') != to_char(rw_crapdat.dtmvtopr, 'MM')
         OR to_char(rw_craprda.dtmvtolt, 'yyyy') !=  to_char(rw_crapdat.dtmvtopr, 'yyyy') THEN
        vr_dtiniper := rw_craprda.dtatslmm;
        vr_dtfimper := rw_crapdat.dtmvtopr;
        vr_dtrefere := rw_craprda.dtfimper;
      ...
      -- Não é resgate total
      IF rw_craplrg.tpresgat <> 2
         AND (to_char(rw_crapdat.dtmvtopr, 'MM') != to_char(rw_craprda.dtmvtolt, 'MM')
            OR to_char(rw_crapdat.dtmvtopr, 'RRRR') != to_char(rw_craprda.dtmvtolt, 'RRRR')) THEN
        ...
        apli0001.pc_rendi_apl_pos_com_resgate (pr_cdcooper => pr_cdcooper
                                              ...
                                              ,pr_nrctaapl => rw_craprda.nrdconta
                                              ,pr_nraplres => rw_craprda.nraplica
                                              ,pr_dtaplrgt => rw_crapdat.dtmvtopr /* Data resgate */
                                              ,pr_dtinitax => vr_dtinitax
                                              ,pr_dtfimtax => vr_dtfimtax
    ...
    BEGIN
      IF nvl(vr_vllan534,0) != 0 THEN
        UPDATE craprda cp
        SET cp.vlrgtacu = cp.vlrgtacu + nvl(vr_vllan534,0)
        WHERE cp.rowid = rw_craprda.ROWID
         RETURNING cp.vlrgtacu INTO rw_craprda.vlrgtacu;
      ELSE
        UPDATE craprda cp
        SET cp.vlrgtacu = cp.vlrgtacu + nvl(vr_vlresgat,0)
        WHERE cp.rowid = rw_craprda.ROWID
         RETURNING cp.vlrgtacu INTO rw_craprda.vlrgtacu;
      END IF;
      ...
      IF rw_crapdtc.tpaplrdc = 1 THEN
        UPDATE craprda cd
        SET cd.vlsdrdca = cd.vlsdrdca - nvl(vr_vlresgat,0)
        WHERE cd.rowid = rw_craprda.ROWID
         RETURNING cd.vlsdrdca INTO rw_craprda.vlsdrdca;
      ELSE
        UPDATE craprda cd
        SET cd.vlsdrdca = cd.vlsdrdca - nvl(vr_vlbasrgt,0)
        WHERE cd.ROWID = rw_craprda.ROWID
         RETURNING cd.vlsdrdca INTO rw_craprda.vlsdrdca;
      END IF;
    EXCEPTION
    ...
    IF nvl(vr_vllan534, 0) != 0 AND rw_craplrg.tpresgat != 2 THEN
      ...
      IF to_char(rw_crapdat.dtmvtopr, 'MM') = to_char(rw_craprda.dtmvtolt, 'MM')
         AND to_char(rw_crapdat.dtmvtopr, 'RRRR') =  to_char(rw_craprda.dtmvtolt, 'RRRR') THEN
    ...
    IF nvl(vr_vlresgat, 0) > 0 THEN
      ...
      IF rw_crapdtc.tpaplrdc = 2 AND rw_craplrg.vllanmto = 0 THEN
        ...
        IF vr_typ_tab_extr_rdc.count() > 0 THEN
          ...
          IF vr_typ_tab_extr_rdc(vr_lastextr).vlsdlsap != 0
            ...
            OPEN cr_craplaplast(pr_cdcooper, rw_craplrg.nrdconta, rw_craplrg.nraplica, rw_crapdat.dtmvtopr, vr_vlresgat);
            ...
            IF cr_craplaplast%found THEN
              ...
              BEGIN
                UPDATE craplot cl
                SET cl.vlinfodb = cl.vlinfodb - rw_craplaplast.vllanmto + nvl(vr_vlresgat,0)
                   ,cl.vlcompdb = cl.vlcompdb - rw_craplaplast.vllanmto + nvl(vr_vlresgat,0)
      ...
      IF rw_craplrg.flgcreci = 0 THEN
        ...
        OPEN cr_craplot(pr_cdcooper, rw_crapdat.dtmvtopr, 1, 100, 8495);
        FETCH cr_craplot INTO rw_craplot;
          ...
          IF cr_craplcm%NOTFOUND OR rw_craplcm.retorno > 1 THEN
            ...
            BEGIN
              ...
              SET cl.qtinfoln = cl.qtinfoln + 1
                 ...
                 ,cl.vlinfocr = cl.vlinfocr + nvl(vr_vlresgat,0)
                 ,cl.vlcompcr = cl.vlcompcr + nvl(vr_vlresgat,0)
    ...
    -- Se não for fim de mês
    IF rw_crapdat.dtmvtolt <> rw_craprda.dtsdfmes THEN
      -- Limpar os campos
      rw_craprda.vlslfmes := 0;
      rw_craprda.dtsdfmes := NULL;
    END IF;
    ...
    IF rw_craplrg.vllanmto = 0 THEN
      BEGIN
        ...
        SET cp.inaniver = 1
           ...
           ,cp.vlsltxmx = 0
           ,cp.vlsltxmm = 0
           ,cp.vlslfmes = rw_craprda.vlslfmes
           ,cp.dtsdfmes = rw_craprda.dtsdfmes
        WHERE cp.rowid = rw_craprda.rowid;
      EXCEPTION
        WHEN others THEN
          vr_cdcritic := 0;
  ...
  BEGIN
    FOR idx IN 1..vr_tab_craprej.count() LOOP
      INSERT INTO craprej(dtmvtolt
                         ,cdagenci
                         ,cdbccxlt
                         ,nrdolote
  ...
  EXCEPTION
    WHEN others THEN
      vr_cdcritic := 0;
      vr_dscritic := 'Erro ao inserir em CRAPREJ. ' || sqlerrm;
      RAISE vr_exc_saida;
  END;
  ...
  vr_bfclob := '<?xml version="1.0" encoding="utf-8"?><resgates>';
  ...
  END IF;
  vr_bfclob := vr_bfclob||'<mensagem>'||vr_mensag||'</mensagem>';

  -- Busca informação de rejeitados
  FOR vr_craprej IN cr_craprej(pr_cdcooper, rw_crapdat.dtmvtopr) LOOP
  ...
  -- Efetuar chamada de geração do relatório
  gene0002.pc_solicita_relato(pr_cdcooper  => pr_cdcooper,         --> Cooperativa conectada
                              ...
                              pr_dsjasper  => 'crrl462.jasper',    --> Arquivo de layout do iReport
                              pr_dsparams  => null,
  ...
  BEGIN
    FOR vr_craprej IN cr_craprej(pr_cdcooper, rw_crapdat.dtmvtopr) LOOP
      DELETE craprej ca
      WHERE ca.rowid = vr_craprej.rowid;
    END LOOP;
  EXCEPTION
    WHEN others THEN
      vr_cdcritic := 0;
