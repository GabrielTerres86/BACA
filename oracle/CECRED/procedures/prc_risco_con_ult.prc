CREATE OR REPLACE PROCEDURE CECRED.prc_risco_con_ult IS

  aux_cdcooper_old crapass.cdcooper%TYPE := 4;
  aux_cdcooper_new crapass.cdcooper%TYPE := 1;
  aux_data_de      DATE := to_date('31/10/2014','dd/mm/yyyy');
  aux_data_ate     DATE := to_date('30/11/2014','dd/mm/yyyy');
  ...
  CURSOR cur_crapris IS
    SELECT crapris.*
     ...
     WHERE craptco.cdcopant = crapris.cdcooper
       ...
       AND TRUNC(crapris.dtrefere) >= TRUNC(last_day(aux_data_de))
...
BEGIN
  ...
  IF aux_data_de > aux_data_ate THEN
    dbms_output.put_line('ERRO: "Data Até" deve ser maior que "Data De"');
    RETURN;
  END IF;
