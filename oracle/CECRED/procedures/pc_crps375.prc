CREATE OR REPLACE PROCEDURE CECRED.pc_crps375 ( pr_cdcooper IN crapcop.cdcooper%TYPE   --> Codigo da cooperativa
  ...
  /* ............................................................................
     ...
     Sigla   : CRED
     Autor   : Mirtes
     Data    : Dezembro/2003.                  Ultima atualizacao: 12/11/2015
  ...
  ............................................................................ */
    ...
    DECLARE
      ...
      vr_cdhistor_pf INTEGER;
      vr_vltarifa_pf NUMBER;
      vr_cdfvlcop_pf INTEGER;

      vr_cdhistor_pj INTEGER;
      vr_vltarifa_pj NUMBER;
      vr_cdfvlcop_pj INTEGER;
      ...
      *************************************************************************/
        DECLARE
          ...
          CURSOR cr_crapceb2 ( pr_cdcooper IN crapcob.cdcooper%TYPE
            ...
            WHERE  crapceb.cdcooper = pr_cdcooper
            AND    crapceb.nrconven = pr_nrconven
            -- retirar tratamento de ceb com 5 digitos(invalida)
            --AND    substr(crapceb.nrcnvceb,1,4) = substr(pr_nrcnvceb,1,4)
          ...
          -- Cadastro de emissao de bloquetos
          CURSOR cr_crapceb4 ( pr_cdcooper IN crapcob.cdcooper%TYPE
                              ,pr_nrconven IN crapceb.nrconven%TYPE
                              ,pr_nrcnvcob IN crapcob.nrcnvcob%TYPE
                              ,pr_nrdocmto IN crapcob.nrdocmto%TYPE
                              --16012014 - inicio
                              ...
                              ,pr_nrcnvceb IN INTEGER) IS
            SELECT crapceb.inarqcbr
                  ,crapceb.nrdconta --busca a conta da crapcob
                  ,crapceb.flgcruni
                  ,crapceb.cddemail
                  ,crapceb.nrcnvceb
                  ,crapcob.nrdconta crapcob_nrdconta --busca a conta da crapcob
                  ,COUNT(*) OVER() qtde_reg
            FROM   crapceb, crapcob
            WHERE  crapceb.cdcooper = pr_cdcooper
            AND    crapceb.nrconven = pr_nrconven
            -- retirar tratamento de ceb com 5 digitos(invalida)
            --AND    substr(crapceb.nrcnvceb,1,4) = substr(pr_nrcnvceb,1,4)
            ...
            AND    crapcob.nrdctabb = pr_nrdctabb
            AND    crapcob.cdbandoc = pr_cdbandoc
            AND    crapcob.vltitulo = pr_vltitulo
            ORDER BY crapceb.cdcooper
                    ,crapceb.nrconven
                    ,crapceb.nrcnvceb;
/*
          -- Cadastro de bloquetos de cobrança pelo valor do titulo
          CURSOR cr_crapcob2( pr_cdcooper IN crapcob.cdcooper%TYPE
                             ...
                             ,pr_cdbandoc IN crapcob.cdbandoc%TYPE
                             --16012014 - fim
                             ,pr_vltitulo IN crapcob.vltitulo%TYPE) IS
...
*/
          -- Cadastro de bloquetos de cobrança pelo documento
          CURSOR cr_crapcob3( pr_cdcooper IN crapcob.cdcooper%TYPE
                             ,pr_nrdconta IN crapcob.nrdconta%TYPE
                             ,pr_nrcnvcob IN crapcob.nrcnvcob%TYPE
                             ,pr_nrdctabb IN crapcob.nrdctabb%TYPE
                             ,pr_cdbandoc IN crapcob.cdbandoc%TYPE
          ...
          CURSOR cr_crapcob_ceb(pr_cdcooper  crapcob.cdcooper%TYPE,
                                ...
                                pr_vltitulo  crapcob.vltitulo%TYPE) IS                    
            SELECT ceb.nrcnvceb
                   ,COUNT(*) OVER() qtd_reg
            FROM crapcob cob,
                 crapceb ceb
          ...
          WHILE vr_ind IS NOT NULL LOOP
            ...
            IF substr(vr_setlinha,8,1) = '0' THEN  /* Header do Arquivo */
              ...
              IF cr_crapcco_global%FOUND THEN
                ...
                -- Carregando os dados de tarifa de cobranca para pessoa fisica
                tari0001.pc_carrega_dados_tarifa_cobr ( pr_cdcooper  => pr_cdcooper            --Codigo Cooperativa
                                                       ...
                                                       ,pr_cdmotivo  => '31' /* Outras instituicÅ‘es financeiras (Correspondente)*/ --Codigo Motivo
                                                       ,pr_inpessoa  => 1  /* Fisica */        --Tipo Pessoa
                                                       ,pr_vllanmto  => 1                      --Valor Lancamento
                                                       ,pr_cdprogra  => NULL                   --Nome do programa
                                                       ,pr_cdhistor  => vr_cdhistor_pf         --Codigo Historico
                                                       ,pr_cdhisest  => vr_cdhisest            --Historico Estorno
                                                       ,pr_vltarifa  => vr_vltarifa_pf         --Valor Tarifa
                                                       ,pr_dtdivulg  => vr_dtdivulg            --Data Divulgacao
                                                       ,pr_dtvigenc  => vr_dtvigenc            --Data Vigencia
                                                       ,pr_cdfvlcop  => vr_cdfvlcop_pf         --Codigo Cooperativa
                ...
                IF trim(pr_dscritic) IS NOT NULL THEN
                  ...
                  IF pr_tab_erro.count() > 0 THEN
                    ...
                    FOR vr_ind IN pr_tab_erro.first .. pr_tab_erro.last LOOP
                      btch0001.pc_gera_log_batch(pr_cdcooper     => pr_cdcooper
                                                ,pr_ind_tipo_log => 2 -- Erro tratato
                                                ,pr_nmarqlog     => vr_nmarqlog
                                                ,pr_des_log      => to_char(sysdate,'hh24:mi:ss')||' - '
                                                                 || vr_cdprogra || ' --> '
                                                                 || pr_tab_erro(vr_ind).dscritic);
                    END LOOP;
                  END IF;
                ...
                -- Carregando os dados de tarifa de cobranca para pessoa juridica
                tari0001.pc_carrega_dados_tarifa_cobr ( pr_cdcooper  => pr_cdcooper            --Codigo Cooperativa
                                                       ,pr_nrconven  => vr_nroconve            --Numero Convenio
                                                       ,pr_dsincide  => 'RET'                  --Descricao Incidencia
                                                       ,pr_cdocorre  => 0                      --Codigo Ocorrencia
                                                       ,pr_cdmotivo  => '31' /* Outras instituicoes financeiras (Correspondente)*/ --Codigo Motivo
                                                       ,pr_inpessoa  => 2  /* Fisica */        --Tipo Pessoa
                                                       ,pr_vllanmto  => 1                      --Valor Lancamento
                                                       ,pr_cdprogra  => NULL                   --Nome do programa
                                                       ,pr_cdhistor  => vr_cdhistor_pj         --Codigo Historico
                                                       ,pr_cdhisest  => vr_cdhisest            --Historico Estorno
                                                       ,pr_vltarifa  => vr_vltarifa_pj         --Valor Tarifa
                                                       ,pr_dtdivulg  => vr_dtdivulg            --Data Divulgacao
                                                       ,pr_dtvigenc  => vr_dtvigenc            --Data Vigencia
                                                       ,pr_cdfvlcop  => vr_cdfvlcop_pj         --Codigo Cooperativa
                                                       ,pr_cdcritic  => pr_cdcritic            --Codigo Critica
                                                       ,pr_dscritic  => pr_dscritic            --Descricao Critica
                                                       ,pr_tab_erro  => pr_tab_erro);          --Tabela erros
                -- Se retornar com erro
            ...
            LOOP
              ...
              ELSIF substr(vr_setlinha,8,1)  = '3' AND substr(vr_setlinha,14,1) = 'T' THEN
                ...
                IF vr_inpessoa = 1 THEN /* 1-Pessoa fisica */
                  vr_vlrtarif := vr_vltarifa_pf;
                  vr_cdtarhis := vr_cdhistor_pf;
                  vr_cdfvlcop := vr_cdfvlcop_pf;
                ELSE
                  vr_vlrtarif := vr_vltarifa_pj;
                  vr_cdtarhis := vr_cdhistor_pj;
                  vr_cdfvlcop := vr_cdfvlcop_pj;
                END IF; /* 2-Pessoa juridica */
...
--                CLOSE cr_crapass;
      ...
      *************************************************************************/
        DECLARE
          ...
          -- Cursor para saber qual a cooperativa origem do convenio 'INCORPORACAO'
          CURSOR cr_crapcco_inc( pr_nrconven IN crapcco.nrconven%TYPE) IS
            SELECT crapcco.cdcooper
            FROM   crapcco
            WHERE  crapcco.cdcooper > 0
            AND    crapcco.nrconven = pr_nrconven
            AND    crapcco.dsorgarq <> 'INCORPORACAO';
          rw_crapcco_inc cr_crapcco_inc%ROWTYPE;
          ...
          -- cadastro de contas transferidas entre cooperativas
          CURSOR cr_craptco_ant( pr_cdcooper IN craptco.cdcopant%TYPE
        ...
        BEGIN
          ...
          WHILE vr_indmigracob IS NOT NULL LOOP
            ...
            IF vr_inpessoa <> 3 THEN
              ...
              -- se retornar erro
              IF trim(pr_dscritic) IS NOT NULL THEN
                -- verifica se o erro foi inserido na tabela temporária de erros
                IF pr_tab_erro.Count() > 0 THEN
