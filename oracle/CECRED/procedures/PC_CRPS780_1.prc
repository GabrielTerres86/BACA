  ...
  DECLARE
    ...
    PROCEDURE pc_gera_pagamento(pr_cdcooper IN crapcop.cdcooper%TYPE  --> Codigo Cooperativa
      ...
      cursor c_crapris (pr_cdcooper craplem.cdcooper%type 
         ...
         and    ris.dtrefere = rw_crapdat.dtultdia;
    ...
    BEGIN
     ...
     IF vr_flgprc = 1 THEN
      ...
      FOR rw_crapepr IN cr_crapepr LOOP
        ...
        END IF;
        --
        -- Se foi passado valor de pagamento, assume o pagamento
         if nvl(pr_vlpagmto,0) > 0 then
           vr_vldescto := pr_vlpagmto; 
           
           if vr_vldescto > vr_vlsldtot then
              IF vr_vlsldtot > 0 THEN
                -- Aplicar a taxa de CPMF
                if nvl(pr_vlpagmto ,0) =0 then
                   vr_vldescto := TRUNC(vr_vlsldtot * vr_txrdcpmf,2);
        ...
        IF vr_vldescto > 0 THEN
            ...
            if vr_vlsldtot < vr_vldescto then -- nao tem saldo suficiente na conta corrente para o desconto
        ...
        IF rw_crapepr.vlsdprej +  -- saldo devedor atualizado
          (rw_crapepr.vlttmupr - rw_crapepr.vlpgmupr) + -- valor residual de multa
          (rw_crapepr.vlttjmpr - rw_crapepr.vlpgjmpr) = (vr_vldescto + nvl(pr_vldabono,0)) -- valor residual do juros
        THEN
        ...
        ELSIF (rw_crapepr.vlsdprej +  -- saldo devedor atualizado
              (rw_crapepr.vlttmupr - rw_crapepr.vlpgmupr) + -- valor residual de multa
              (rw_crapepr.vlttjmpr - rw_crapepr.vlpgjmpr)) - (vr_vldescto + nvl(pr_vldabono,0)) > 0 THEN
          -- Indicar que o emprestimo não está liquidado
          rw_crapepr.inliquid := 0;
          -- rmm
          rw_crapepr.vlsdprej := rw_crapepr.vlsdprej - vr_vldescto - nvl(pr_vldabono,0);
          vr_vlttjmpr := 0;
          vr_vlttmupr := 0;
        ELSE
     ...
     /*   vr_vlsdprej := nvl(rw_crapepr.vlsdprej,0) - (nvl(vr_vldescto,0) + nvl(pr_vldabono,0));
        ...
        ELSE
          vr_dscritic := 'Valor de pagamento superior ao valor do saldo do prejuizo: '||vr_vldescto;
          --pr_des_reto := 'NOK';
           raise vr_exc_erro;
        end if;*/
        -- Valor do abono deve também considerar na multas e juros
        /*IF NVL(pr_vldabono,0) > 0 THEN
           if nvl(rw_crapepr.vlttmupr,0) > 0 
           or nvl(rw_crapepr.vlttjmpr,0) > 0 then
              vr_vlttmupr := nvl(vr_vlttmupr,0) + ( abs(pr_vldabono) * rw_crapepr.vlttmupr /( rw_crapepr.vlttmupr + rw_crapepr.vlttjmpr));
              vr_vlttjmpr := nvl(vr_vlttjmpr,0) + ( abs(pr_vldabono) * rw_crapepr.vlttjmpr /( rw_crapepr.vlttmupr + rw_crapepr.vlttjmpr));                      
           
            -- pode ter um abono maior que a multa e juros, neste caso assume o valor de multa e juros
             if vr_vlttmupr > rw_crapepr.vlttmupr then 
                vr_vlttmupr := rw_crapepr.vlttmupr;
             end if;
             if vr_vlttjmpr > rw_crapepr.vlttjmpr then 
                vr_vlttjmpr := rw_crapepr.vlttjmpr;
             end if;
        ...
        BEGIN
          UPDATE crapepr
            SET dtdpagto  = rw_crapepr.dtdpagto
                ...
                ,vlsdprej = rw_crapepr.vlsdprej  --vlsdprej - vr_vldescto - nvl(pr_vldabono,0)
                ,vlpgjmpr = nvl(rw_crapepr.vlpgjmpr,0) + vr_vlttjmpr
                ,vlpgmupr = nvl(rw_crapepr.vlpgmupr,0) + vr_vlttmupr
