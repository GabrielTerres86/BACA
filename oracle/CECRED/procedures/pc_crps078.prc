...
--      vr_exc_erro EXCEPTION;
      ...
      -- Buscar as informac?es para restart e Rowid para atualizac?o posterior
      ...
      CURSOR cr_crapepr IS
        SELECT epr.rowid
         ...
         WHERE epr.cdcooper = pr_cdcooper
           ...
           AND epr.nrdconta >= vr_nrctares  -- Conta de restart
    ...
    BEGIN
     ...
     -- Leitura do calendario da cooperativa
      ...
      btch0001.pc_valida_restart(pr_cdcooper  => pr_cdcooper   --> Cooperativa conectada
                                ...
                                ,pr_dsrestar  => vr_dsrestar   --> String generica com informac?es para restart
      ...
      -- Se ainda houver indicac?o de restart
      IF vr_inrestar > 0 THEN
        -- Buscar o ultimo contrado de emprestimo antes do restart
        vr_nrctremp := SUBSTR(vr_dsrestar,1,8);
        -- Buscar as linhas processadas antes do restart
        vr_dslinhas := SUBSTR(vr_dsrestar,9,500);
      END IF;
      ...
      FOR rw_crapepr IN cr_crapepr LOOP
        BEGIN
          ...
          -- Se ha controle de restart e Se e a mesma conta, mas de uma aplicac?o anterior
          IF vr_inrestar > 0  AND rw_crapepr.nrdconta = vr_nrctares AND rw_crapepr.nrctremp <= vr_nrctremp THEN
            -- Ignorar o registro pois ja foi processado
            RAISE vr_exc_next;
          END IF;
          ...
          IF pr_flgresta = 1 THEN
            ...
            -- gravar tbm o controle de restart, pois qualquer rollback que sera efetuado
            -- vai retornar a situac?o ate o ultimo commit
            -- Obs. Descontamos da quantidade atual, a quantidade que n?o foi processada
            ...
            IF Mod(cr_crapepr%ROWCOUNT-vr_qtregres,10000) = 0 THEN
              -- Incluir nas 7 primeiras posic?es o numero do contrato de emprestimo
              -- e um espaco e o e utilizado para armazenar as linhas de credito
              vr_dsrestar := LPAD(rw_crapepr.nrctremp,8,'0')||' '||vr_dslinhas;
              -- Atualizar a tabela de restart
              BEGIN
                UPDATE crapres res
                   SET res.nrdconta = rw_crapepr.nrdconta  -- conta do emprestimo atual
                      ,res.dsrestar = vr_dsrestar          -- descric?o generica montada acima
        ...
        EXCEPTION
          WHEN vr_exc_next THEN
            -- Exception criada para desviar o fluxo para ca e
            -- n?o processar o restante das instruc?es apos o RAISE
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          RAISE vr_exc_saida;
      END;
      -- Chamar rotina para eliminac?o do restart para evitarmos
      -- reprocessamento das aplicac?es indevidamente
      btch0001.pc_elimina_restart(pr_cdcooper => pr_cdcooper   --> Cooperativa conectada
                                 ,pr_cdprogra => vr_cdprogra   --> Codigo do programa
                                 ,pr_flgresta => pr_flgresta   --> Indicador de restart
                                 ,pr_des_erro => vr_dscritic); --> Saida de erro
      -- Testar saida de erro
      IF vr_dscritic IS NOT NULL THEN
        -- Sair do processo
        RAISE vr_exc_saida;
      END IF;
