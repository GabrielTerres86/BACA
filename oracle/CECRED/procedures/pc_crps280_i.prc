...
BEGIN
  ...
  DECLARE
    ...
    CURSOR cr_craptab(pr_cdcooper IN craptab.cdcooper%TYPE,
      ...
      SELECT tab.dstextab, tab.tpregist
        FROM craptab tab
    ...
    TYPE typ_reg_risco IS RECORD(
      dsdrisco VARCHAR2(2) -- "x(02)"
    ...
    TYPE typ_reg_total IS RECORD(
       vlfinanc_pf NUMBER -- Valor Financiamento Pessoa Fisica
    ...
    TYPE typ_reg_crapris IS RECORD(
      cdagenci crapage.cdagenci%TYPE --> Chave Asc
    ...
    TYPE typ_tab_crapris IS TABLE OF typ_reg_crapris INDEX BY VARCHAR2(150);
    ...
    TYPE typ_reg_totris IS RECORD(
      qtdabase INTEGER -- "zzz,zz9"
    ...
    TYPE typ_tab_totris IS TABLE OF typ_reg_totris INDEX BY PLS_INTEGER;
    ...
    TYPE typ_reg_contabi IS RECORD(
      rel1731_1      NUMBER(14, 2) DEFAULT 0 -- Financiamentos Pessoais [Provisao]
    ...
    TYPE typ_tab_craptco IS TABLE OF crapass.nrdconta%TYPE INDEX BY PLS_INTEGER;

    vr_tab_craptco typ_tab_craptco;

    -- Definição de registro para totalização por origem de microcrédito
    TYPE typ_reg_microcredito IS RECORD(
      idgrumic INTEGER -- Id do grupo para separação das informações - 1 - CECRED / 2 - BNDES
    ...
    TYPE typ_tab_microcredito IS TABLE OF typ_reg_microcredito INDEX BY craplcr.dsorgrec%type;
    ...
    TYPE typ_reg_miccred_fin IS RECORD(
      vllibctr    NUMBER -- Valor acumulado liberação de contratos
    ...
    TYPE typ_reg_miccred_nivris IS RECORD(
      vlslddev NUMBER); -- Valor acumulado saldo devedor

    -- Definicao do tipo de tabela totalização por nível de risco de microcrédito
    TYPE typ_tab_miccred_nivris IS TABLE OF typ_reg_miccred_nivris INDEX BY crawepr.dsnivris%type;
    ...
    TYPE typ_reg_finame_nivris IS RECORD(
      vlslddev NUMBER); -- Valor acumulado saldo devedor

    -- Definicao do tipo de tabela totalização por nível de risco de FINAME
    TYPE typ_tab_finame_nivris IS TABLE OF typ_reg_finame_nivris INDEX BY crawepr.dsnivris%type;
    ...
    CURSOR cr_ris_liminutz IS
      SELECT ris.nrdconta, ris.vldivida, ris.inpessoa
        FROM crapris ris
    ...
    CURSOR cr_crapvri(pr_dtrefere IN crapris.dtrefere%TYPE) IS
      SELECT nrdconta,
             ...
             nrctremp,
             nrseqctr,
             SUM(DECODE(greatest(cdvencto, 110),
                        cdvencto,  DECODE(LEAST(cdvencto, 290), cdvencto, vldivida, 0),
                        0)) vldivida,
             SUM(DECODE(greatest(cdvencto, 205),
                        cdvencto,  DECODE(LEAST(cdvencto, 290), cdvencto, vldivida, 0),
                        0)) vltotatr,
             COUNT(DISTINCT
                   DECODE(greatest(cdvencto, 205),
                          cdvencto,  DECODE(LEAST(cdvencto, 290), cdvencto, 1, NULL),
                          NULL)) qtpreatr,
             SUM(DECODE(cdvencto,
                        310, vldivida,
                        320, vldivida,
    ...
    type typ_reg_crapvri is record(
    ...
    CURSOR cr_crapris_last(pr_nrdconta IN crapris.nrdconta%TYPE,
      ...
      SELECT /*+ INDEX (ris CRAPRIS##CRAPRIS1) */
       'S' ind_achou, ris.innivris, ris.dtdrisco, ris.vldivida
        FROM crapris ris
       WHERE ris.cdcooper = pr_cdcooper
         AND ris.nrdconta = pr_nrdconta
         AND ris.dtrefere = pr_dtrefere
    ...
    CURSOR cr_craplem(pr_nrdconta IN craplem.nrdconta%TYPE,
      ...
      SELECT lem.cdhistor, NVL(SUM(lem.vllanmto), 0) vllanmto
        FROM craplem lem
    ...
    CURSOR cr_crapris_reccaixa IS
      SELECT cop.nmrescop,
             ...
             epr.vlemprst,
             epr.vlsdeved
        FROM crapris ris, crapepr epr, crapcop cop
       WHERE ris.nrdconta = epr.nrdconta
         ...
         AND ris.dtrefere = pr_dtrefere
         AND ris.cdcooper = pr_cdcooper
       ORDER BY cop.nmrescop, epr.nrctremp;
    ...
    FUNCTION fn_set_gerencial(pr_cdagenci in number, pr_vlagenci in number)
      RETURN VARCHAR2 IS
    BEGIN
    ...
    PROCEDURE pc_cria_node_contab(pr_des_xml    IN OUT VARCHAR2,
                                  ...
                                  pr_flcessao   IN INTEGER DEFAULT 0) IS
    BEGIN
      -- Escreve no XML abrindo e fechando a tag de contab
      pr_des_xml := pr_des_xml || '<contab>' || '  <idcessao>' ||
                    pr_flcessao || '</idcessao> ' || '  <dscontab>' ||
                    pr_des_contab || '</dscontab>' || '  <vlprovis>' ||
                    to_char(pr_num_valor1, 'fm999g999g999g990d00') ||
                    '</vlprovis>' || '  <vldivida>' ||
                    to_char(pr_num_valor2, 'fm999g999g999g990d00') ||
                    '</vldivida>' || '  <vlprvfis>' ||
                    to_char(pr_num_vlpvpf, 'fm999g999g999g990d00') ||
                    '</vlprvfis>' || '  <vlprvjur>' ||
                    to_char(pr_num_vlprpj, 'fm999g999g999g990d00') ||
                    '</vlprvjur>' || '  <vldivfis>' ||
                    to_char(pr_num_vldvpf, 'fm999g999g999g990d00') ||
                    '</vldivfis>' || '  <vldivjur>' ||
                    to_char(pr_num_vldvpj, 'fm999g999g999g990d00') ||
                    '</vldivjur>' || '</contab>';
    END;
    ...
    BEGIN
      ...
      pr_des_xml := pr_des_xml || '<microcredito>' || '  <idgrumic>' ||
                    pr_idgrumic || '</idgrumic>' || '  <dsorgrec>' ||
                    pr_ds_microcredito || '</dsorgrec>' || '  <vlreprec>' ||
                    to_char(pr_vlreprec, 'fm999g999g999g990d00') ||
                    '</vlreprec>' || '  <vlempatr>' ||
                    to_char(pr_vlempatr, 'fm999g999g999g990d00') ||
                    '</vlempatr>' || '  <vlrepfis>' ||
                    to_char(pr_vlreprec_pf, 'fm999g999g999g990d00') ||
                    '</vlrepfis>' || '  <vlrepjur>' ||
                    to_char(pr_vlreprec_pj, 'fm999g999g999g990d00') ||
                    '</vlrepjur>' || '  <vlaci59d>' ||
                    to_char(pr_vlaci59d, 'fm999g999g999g990d00') ||
                    '</vlaci59d>' || '  <vlate59d>' ||
                    to_char(pr_vlate59d, 'fm999g999g999g990d00') ||
                    '</vlate59d>' || '</microcredito>';
    ...
    BEGIN
      ...
      IF (nvl(vr_tot_vltttlcr_dim, 0) + nvl(vr_tot_vltttlcr_dim_outros, 0) +
         nvl(vr_totatraso59_dim, 0) + nvl(vr_totatraso59_dim_outros, 0)) > 0 THEN

        -- Nome do arquivo a ser gerado
        vr_nmarquiv := vr_dtmvtolt_yymmdd || '_' || lpad(pr_cdcooper, 2, 0) ||
                       '_MICROCREDITO_COMPENSACAO.txt';
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            vr_dscritic := 'Problema ao fechar o arquivo <' ||
                           vr_nom_diretorio || '/' || vr_nmarquiv || '>: ' ||
                           SQLERRM;
            RAISE vr_exc_erro;
        END;
    ...
    BEGIN
      ...
      -- Nome do arquivo a ser gerado
      vr_nmarqfin := vr_dtmvtolt_yymmdd || '_' || lpad(pr_cdcooper, 2, 0) ||
                     '_AJUSTE_FINAME.txt';
      ...
      LOOP
        ...
        IF vr_tab_finame_nivris(vr_chave_nivris).vlslddev > 0 THEN
          ...
          vr_setlinha := fn_set_cabecalho('20',
                                          ...
                                          9302,
                                          vr_destino,
                                          vr_tab_finame_nivris(vr_chave_nivris)
                                          .vlslddev,
      ...
      -- Testar erro
      if vr_typ_said = 'ERR' then
        vr_dscritic := 'Erro ao copiar o arquivo ' || vr_nmarqfin || ': ' ||
                       vr_dscritic;
        raise vr_exc_erro;
      end if;
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        pr_dscritic := 'Erro geral na procedure pc_gera_arq_miccred: ' ||
                       SQLERRM;
    ...
    BEGIN
      ...
      -- Nome do arquivo a ser gerado
      vr_nmarqmic := vr_dtmvtolt_yymmdd || '_' || lpad(pr_cdcooper, 2, 0) ||
                     '_AJUSTE_MICROCREDITO.txt';
      ...
      LOOP
        ...
        IF vr_tab_miccred_fin(vr_chave_finalidade).vldebpar441 > 0 THEN
          ...
          vr_setlinha := fn_set_cabecalho('20',
                                          ...
                                          '"EMPRESTIMOS EFETUADOS PARA ASSOCIADOS - (0441) JUROS SOBRE EMPRESTIMOS ' ||
                                          vr_descricao);
      ...
      -- Testar erro
      if vr_typ_said = 'ERR' then
        vr_dscritic := 'Erro ao copiar o arquivo ' || vr_nmarqmic || ': ' ||
                       vr_dscritic;
        raise vr_exc_erro;
      end if;
  ...
  BEGIN
    ...
    END IF;
		...
		if pr_idparale = 0 then
			...
			ELSIF pr_cdcooper = 16 AND
						pr_dtrefere <= TO_DATE('31/12/2012', 'DD/MM/RRRR') THEN
				-- Vindas da Via
				FOR regs IN cr_craptco_via_alto LOOP
					vr_tab_craptco(regs.nrdconta) := regs.nrdconta;
				END LOOP;
				-- Incorporação da Credimil >> SCR
			ELSIF pr_cdcooper = 13 AND
						pr_dtrefere <= TO_DATE('30/11/2014', 'DD/MM/RRRR') THEN
				-- Vindas da Credimil
			...
			pr_vltotprv := 0;
			 ...
			 pc_log_programa(PR_DSTIPLOG     => 'O',
											...
											pr_tpexecucao   => vr_tpexecucao, -- Tipo de execucao (0-Outro/ 1-Batch/ 2-Job/ 3-Online)
											pr_tpocorrencia => 4,
											pr_dsmensagem   => 'crapvri Início AGENCIA: [' ||
																				 pr_cdagenci || '] - INPROCES: ' ||
																				 pr_rw_crapdat.inproces,
											PR_IDPRGLOG     => vr_idlog_ini_ger);
				...
				LOOP
					FETCH cr_crapvri BULK COLLECT
						INTO vr_tab_crapvri LIMIT 100000; -- carrega de 100 em 100 mil registros
					...
					IF vr_tab_crapvri.COUNT > 0 THEN
						...
						FOR idx IN vr_tab_crapvri.FIRST .. vr_tab_crapvri.LAST LOOP
							-- monta o indice
							vr_chave_index := vr_tab_crapvri(idx).nrdconta ||
				...
				pc_log_programa(PR_DSTIPLOG     => 'O',
											...
											pr_tpexecucao   => vr_tpexecucao, -- Tipo de execucao (0-Outro/ 1-Batch/ 2-Job/ 3-Online)
											pr_tpocorrencia => 4,
											pr_dsmensagem   => 'crapvri Fim AGENCIA: [' ||
																				 pr_cdagenci || '] - INPROCES: ' ||
																				 pr_rw_crapdat.inproces,
											PR_IDPRGLOG     => vr_idlog_ini_ger);
		end if; --pr_idparale = 0 then
...
--*/
    ...
    if (pr_idparale = 0) then
      ...
      LOOP
        ...
        IF vr_des_chave_crapris = vr_tab_crapris.FIRST
        OR vr_tab_crapris(vr_tab_crapris.PRIOR(vr_des_chave_crapris)).cdagenci <> vr_tab_crapris(vr_des_chave_crapris).cdagenci THEN
          -- Buscar nome da agência
            ...
            IF vr_tab_dados_epr.exists(vr_indice) THEN
              IF vr_tab_dados_epr(vr_indice).dsorgrec IS NOT NULL THEN
                ...
                IF NOT vr_tab_microcredito.exists(vr_tab_dados_epr(vr_indice).dsorgrec) THEN
                  ...
                  IF vr_tab_dados_epr(vr_indice).dsorgrec LIKE '%BRDE%' OR vr_tab_dados_epr(vr_indice)
                     .dsorgrec LIKE '%BNDES%' THEN
                    -- BNDEs
                ...
                IF vr_tab_crapris(vr_des_chave_crapris).inpessoa = 1 THEN
                  vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlpesfis := vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlpesfis + vr_vldivida;
                ELSE
                  vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlpesjur := vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlpesjur + vr_vldivida;
                END IF;

                -- Se o atraso for até 59 dias
                IF vr_tab_crapris(vr_des_chave_crapris).qtdiaatr <= 59 THEN
                  -- Acumular no período até 59
                  vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlate59d := vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlate59d + vr_vldivida;
                ...
                ELSE
                  ...
                  vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlaci59d := vr_tab_microcredito(vr_tab_dados_epr(vr_indice).dsorgrec).vlaci59d + vr_vldivida;
        ...
        IF pr_cdprogra = 'CRPS280' THEN
          ...
          IF vr_qtpreatr <= 0 
					AND vr_tab_crapris(vr_des_chave_crapris).innivris <> 2 
					AND vr_vldivida > 0 
					AND vr_dsorigem = 'E' THEN
            -- So emprestimo
        ...
        FOR rw_crapris_last IN cr_crapris_last(pr_nrdconta => vr_tab_crapris(vr_des_chave_crapris).nrdconta,
                                               pr_dtrefere => pr_rw_crapdat.dtultdma) LOOP
        ...
        IF vr_vldivida > 0 THEN
          ...
          IF pr_cdprogra <> 'CRPS184' THEN
            ...
            IF vr_dsorigem = 'E' THEN
              vr_cdlcremp_354 := vr_tab_crapris(vr_des_chave_crapris)
                                 .cdlcremp;
            ...
            ELSIF vr_dsorigem = 'C'
              AND vr_tab_crapris(vr_des_chave_crapris).cdmodali = 0101 THEN
        ...
        IF vr_des_chave_crapris = vr_tab_crapris.LAST OR vr_tab_crapris(vr_tab_crapris.NEXT(vr_des_chave_crapris))
          .cdagenci <> vr_tab_crapris(vr_des_chave_crapris).cdagenci THEN
        ...
        IF pr_cdcooper = 3 AND (TRUNC(pr_rw_crapdat.dtmvtolt, 'mm') <> TRUNC(pr_rw_crapdat.dtmvtopr, 'mm')) THEN
          ...
          IF ( (vr_tab_crapris(vr_des_chave_crapris).cdfinemp IN (1, 4) AND vr_tab_crapris(vr_des_chave_crapris).cdusolcr = 1)
               OR
               (vr_tab_crapris(vr_des_chave_crapris).cdfinemp IN (2, 3))
               AND vr_tab_crapris(vr_des_chave_crapris).cdusolcr = 0
             )
          AND vr_tab_crapris(vr_des_chave_crapris).dsorgrec <> ' ' THEN
        ...
        LOOP
          ...
          IF vr_tab_microcredito(vr_chave_microcredito).idgrumic = vr_cont THEN
            pc_cria_node_miccre(pr_des_xml         => vr_des_xml_gene,
                                ...
                                pr_vlreprec        => vr_tab_microcredito(vr_chave_microcredito).vlpesfis + vr_tab_microcredito(vr_chave_microcredito).vlpesjur,
                                pr_vlempatr        => vr_tab_microcredito(vr_chave_microcredito).vlate59d + vr_tab_microcredito(vr_chave_microcredito).vlaci59d,
  ...
  EXCEPTION
    WHEN vr_exc_erro THEN
      ...
      pr_dscritic := 'Erro na rotina PC_CRPS280_I. Detalhes: ' ||
                     pr_dscritic;
