CREATE OR REPLACE PROCEDURE CECRED.PC_CRPS652 (pr_cdcooper IN crapcop.cdcooper%TYPE   --> Codigo Cooperativa
  ...
  /* .............................................................................
   ...
   Sigla   : CRED
   Autor   : Lucas Reinert
   Data    : AGOSTO/2013                      Ultima atualizacao: 07/11/2018
   ...
   Alteracoes: 26/09/2013 - Ajuste para gerar pagamento e baixa para os
		 ...
		 02/05/2017 - Prj 432 - retirar regra de não enviar baixa se contrato VIP, está gerando conflitos no Cyber 
               ...
               07/11/2018 - P442 - Inclusão de Score Comportamental (Marcos-Envolti)
     ...
     TYPE typ_tab_contlinh IS VARRAY(9) OF PLS_INTEGER;
     TYPE typ_tab_telcoop  IS VARRAY(5) OF VARCHAR2(1000);
     TYPE typ_tab_nmclob   IS VARRAY(9) OF VARCHAR2(100);
     TYPE typ_tab_linha    IS VARRAY(9) OF VARCHAR2(5000);
     ...
     vr_tab_contlinh typ_tab_contlinh:= typ_tab_contlinh(0,0,0,0,0,0,0,0,0);
     vr_tab_telcoop  typ_tab_telcoop:= typ_tab_telcoop(NULL,NULL,NULL,NULL,NULL);
     vr_tab_nmclob   typ_tab_nmclob:= typ_tab_nmclob(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
     vr_tab_linha    typ_tab_linha:= typ_tab_linha (NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
     ...
     CURSOR cr_crapcop (pr_cdcooper IN crapcop.cdcooper%TYPE) IS
       ...
       WHERE crapcop.cdcooper = 16 -- <> pr_cdcooper
     ...
     CURSOR cr_crapcyb (pr_cdcooper IN crapcop.cdcooper%type) IS
       ...
       -- Cursor com dados de Score por CPF/Raiz CNPJ
       CURSOR cr_score(pr_cdcooper crapcop.cdcooper%TYPE) IS
         SELECT sco.tppessoa
               ,sco.nrcpfcnpjbase
       ...
       BEGIN
         ...
         CASE pr_nro_clob
           ...
           WHEN 9 THEN dbms_lob.writeappend(vr_des_xml9,length(vr_linha),vr_linha);
         END CASE;
       EXCEPTION
         WHEN OTHERS THEN
           vr_cdcritic:= 0;
     ...
     BEGIN
       ...
       FOR idx IN 1..9 LOOP
         ...
         CASE idx
           ...
           WHEN 8 THEN
             ...
             vr_tparquiv:= 'acordo';  
           WHEN 9 THEN
             vr_setlinha:= vr_setlinha ||'_score_in.txt';       /* Score Comportamental */
             vr_tparquiv:= 'score';
         END CASE;
       ...
       FOR rw_crapcop IN cr_crapcop (pr_cdcooper => pr_cdcooper) LOOP
         ...
         -- Carregar scores por CPF/CNPJ Raiz 
         FOR rw_score IN cr_score(pr_cdcooper => rw_crapcop.cdcooper) LOOP
           -- Montar o IDX com TpPessoa e CPF/CNPJ Raiz
           vr_idx_score := rw_score.tppessoa || lpad(rw_score.nrcpfcnpjbase,22,'0');
           -- Se já existe e sua tabela interna possui menos de 10 scores
           IF NOT vr_tab_score.exists(vr_idx_score) OR vr_tab_score(vr_idx_score).qtdscore < 10  THEN
             -- Criar registro chave caso não exista
             IF NOT vr_tab_score.exists(vr_idx_score) THEN
               vr_tab_score(vr_idx_score).qtdscore := 0;
             END IF;
             -- Incrementar quantidade
             vr_tab_score(vr_idx_score).qtdscore := vr_tab_score(vr_idx_score).qtdscore + 1;
             -- Posicionar a nota no vetor interno de notas
             vr_tab_score(vr_idx_score).nrdscore(vr_tab_score(vr_idx_score).qtdscore) := rw_score.nrscore_alinhado;
           END IF;
         END LOOP;
         ...
         FOR rw_crapcyb IN cr_crapcyb (pr_cdcooper => rw_crapcop.cdcooper) LOOP
           ...
           -- Caso o Cooperado tenha Score Behaviou
           vr_idx_score := vr_tab_crapass(rw_crapcyb.nrdconta).inpessoa || lpad(vr_tab_crapass(rw_crapcyb.nrdconta).nrcpfcnpj_base,22,'0');
           -- Se já existe e sua tabela interna possui menos de 10 scores
           IF vr_tab_score.exists(vr_idx_score) AND vr_tab_score(vr_idx_score).qtdscore > 0  THEN
             -- Incrementar Contador linha
             pc_incrementa_linha(pr_nrolinha => 9);
             -- Gerar campos obrigatórios
             pc_monta_linha('1',4,9);
             pc_monta_linha(gene0002.fn_mask(rw_crapcyb.cdcooper,'9999'),5,9);
             pc_monta_linha(gene0002.fn_mask(rw_crapcyb.cdorigem,'9999'),9,9);
             pc_monta_linha(gene0002.fn_mask(rw_crapcyb.nrdconta,'99999999'),13,9);
             pc_monta_linha(gene0002.fn_mask(rw_crapcyb.nrctremp,'99999999'),21,9);           
             
             -- Enviar cada Score com 4 posições a partir da posição 30
             FOR idx IN 1..vr_tab_score(vr_idx_score).qtdscore LOOP
               -- Calcular posições conforme idx do loop
               pc_monta_linha(gene0002.fn_mask(vr_tab_score(vr_idx_score).nrdscore(idx),'ZZZZ'),30+(idx-1)*4,9);
             END LOOP;
             -- Enviar ao CLOB
             pc_escreve_xml(NULL,9);
           END IF;  
       ...
       --Escrever Trailer nos 8 arquivos, fechar e liberar os 9 Clobs.
       FOR idx IN 1..9 LOOP
         ...
         --Escrever linha no arquivo
         pc_escreve_xml(vr_setlinha,idx);
         --Gerar os 9 arquivos fisicamente
         CASE idx
           WHEN 1 THEN
