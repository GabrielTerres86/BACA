CREATE OR REPLACE PROCEDURE CECRED.PC_CRPS652(pr_cdcooper IN crapcop.cdcooper%TYPE   --> Codigo Cooperativa
  ...
  /* .............................................................................
   ...
   Sigla   : CRED
   Autor   : Lucas Reinert
   Data    : AGOSTO/2013                      Ultima atualizacao: 24/07/2018
   ...
   DECLARE
     ...
     CURSOR cr_crapcop (pr_cdcooper crapcop.cdcooper%TYPE              DEFAULT 0
     ...
     cursor cr_work_arquv(pr_cdcooper    tbgen_batch_relatorio_wrk.cdcooper%type  
      ...
      SELECT dsxml
       ...
       ORDER BY cdagenci,nrdconta,nrctremp; -- Cdagenci é Coop, Nrdconta é o Arquivo e Nrctremp é a Linha                         
     ...
     TYPE vr_typ_texto IS TABLE OF VARCHAR2(5000) INDEX BY PLS_INTEGER;
     ...
     BEGIN
       ...
       CASE pr_cod_info
         WHEN 1 THEN vr_tab_texto1(vr_tab_texto1.count()) := vr_linha;
         WHEN 2 THEN vr_tab_texto2(vr_tab_texto2.count()) := vr_linha;
         WHEN 3 THEN vr_tab_texto3(vr_tab_texto3.count()) := vr_linha;
         WHEN 4 THEN vr_tab_texto4(vr_tab_texto4.count()) := vr_linha;
         WHEN 5 THEN vr_tab_texto5(vr_tab_texto5.count()) := vr_linha;
         WHEN 6 THEN vr_tab_texto6(vr_tab_texto6.count()) := vr_linha;
         WHEN 7 THEN vr_tab_texto7(vr_tab_texto7.count()) := vr_linha;
         WHEN 8 THEN vr_tab_texto8(vr_tab_texto8.count()) := vr_linha;
       END CASE; 
       ...
       BEGIN
         ...
         IF pr_opccarga = 'F' THEN
           ...
           IF pr_idarquivo NOT IN (1,3) THEN
             ...
             pc_escreve_dado(NULL,pr_idarquivo);
           END IF;
         ELSIF pr_opccarga = 'R' THEN /*Relations*/
   ...
   BEGIN
     ...
     IF vr_qtdjobs > 0 and pr_cdcoppar = 0 then  
       ...
       FOR rw_crapcop IN cr_crapcop(pr_cdprogra => pr_cdprogra
         ...
         vr_jobname := pr_cdprogra ||'_'|| rw_crapcop.cdcooper || '$';  
       
         -- Cadastra o programa paralelo
         gene0001.pc_ativa_paralelo(pr_idparale => vr_idparale
                                   ,pr_idprogra => LPAD(rw_crapcop.cdcooper,3,'0') --> Utiliza coop como id programa
     ...
     ELSE
       ...
       FOR rw_crapcop IN cr_crapcop (pr_cdcooper => pr_cdcoppar) LOOP
         ...
         IF pr_cdcoppar > 0 THEN
           ...
           FOR idx IN 1..8 LOOP 
             ...
             FOR idx2 IN 0..vr_tab_texto_Generi.count()-1 LOOP
               ...
               IF idx2 != vr_tab_texto_Generi.last OR trim(vr_tab_texto_Generi(idx2)) IS NOT NULL THEN 
     ...
     IF pr_idparale = 0 THEN
       ...
       IF vr_idparale > 0 THEN
         -- Laço para buscar dados dos 8 arquivos
         FOR vr_idx IN 1..8 LOOP         
           ...
           FOR rw_work IN cr_work_arquv(pr_cdcooper    => pr_cdcooper
             ...
             pc_escreve_dado(rw_work.dsxml,vr_idx);
           END LOOP;
         END LOOP;
       END IF;
       ...
       FOR idx IN 1..8 LOOP 
         ...
         FOR idx2 IN 0..vr_tab_texto_Generi.count()-1 LOOP
           ...
           IF idx2 != vr_tab_texto_Generi.last OR trim(vr_tab_texto_Generi(idx2)) IS NOT NULL THEN 
             pc_escreve_clob(vr_tab_texto_Generi(idx2)||chr(10),idx);
           END IF;
         END LOOP;
