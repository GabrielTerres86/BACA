...
CREATE OR REPLACE PACKAGE BODY PROGRID.WPGD0191 IS
  ...
  --  Sigla    : WPGD
  --  Autor    : Jonathan Cristiano da Silva - RKAM
  --  Data     : Setembro/2015.                   Ultima atualizacao: 19/04/2016
  --
  -- Dados referentes ao programa:
  --
  -- Frequencia: -----
 ...
 PROCEDURE pc_recebe_cursos_aprovados(pr_dscritic OUT VARCHAR2) IS --> Retorno de crítica
  ...
  -- Buscar a informação da CRAPADP - Sequence
  CURSOR cr_crapadp_seq(pr_idevento IN NUMBER) IS
   SELECT NVL(MAX(nrseqdig),0) AS nrseqdig
    FROM crapadp
   WHERE idevento = pr_idevento;
  rw_crapadp_seq cr_crapadp_seq%ROWTYPE;
  ...
  CURSOR cr_crapidp_seq(pr_idevento IN NUMBER,
                        pr_cdevento IN NUMBER, 
                        pr_cdcooper IN NUMBER,
                        pr_cdagenci IN NUMBER, 
                        pr_dtanoage IN NUMBER) IS
  SELECT NVL(MAX(nrseqdig),0) AS nrseqdig
    FROM crapidp 
   WHERE idevento = pr_idevento;
  rw_crapidp_seq cr_crapidp_seq%ROWTYPE;
    ...
    BEGIN
      ...
      LOOP
        ...
        IF cr_crapadp%NOTFOUND THEN
          ...
          OPEN cr_crapadp_seq(pr_idevento => vr_idevento);                 
          FETCH cr_crapadp_seq INTO rw_crapadp_seq;
          ...
          INSERT INTO crapadp (
            ...
            1,
            vr_dtmesevt,
            NVL(rw_crapadp_seq.nrseqdig, 0) + 1
          );
      ...
      LOOP
        ...
        IF cr_tbeadip%FOUND THEN
          ...
          IF cr_crapidp%NOTFOUND THEN 
            ...
            OPEN cr_crapidp_seq(pr_idevento => vr_idevento,
                                pr_cdevento => vr_cdevento,
                                pr_cdcooper => vr_cdcooper,
                                pr_cdagenci => vr_cdagenci,
                                pr_dtanoage => vr_dtanoage);                 
            FETCH cr_crapidp_seq INTO rw_crapidp_seq;
            ...
            INSERT INTO crapidp (
              ...
              1,
              vr_cdagenci,
              NVL(rw_crapidp_seq.nrseqdig, 0) + 1,
