CREATE OR REPLACE PACKAGE PROGRID."AJUSTA_OBJETO" is

procedure move_objeto_tablespace;

procedure grava_comando_indice;

procedure recria_indice;
...
CREATE OR REPLACE PACKAGE BODY PROGRID."AJUSTA_OBJETO" is
...
procedure move_objeto_tablespace is
  ...
  cursor c0002(pwd_owner varchar2, pwd_table_name varchar2, pwd_tablespace_name varchar2) is
    select a.owner, a.index_name, pwd_tablespace_name||'_I' tablespace_name
    from dba_indexes a
  ...
  cursor c0003 is
    select 'alter table "'||a.owner||'"."'||a.table_name||'" move tablespace '||a.tablespace_name||'_D' ww_comando
    from cecred.tabela_tablespace a
    where not exists
    (select 1
     from dba_tables b
  ...
  cursor c0004 is
    select 'alter index "'||a.owner||'"."'||a.index_name||'" rebuild tablespace '||a.tablespace_name ww_comando
...
begin
  ...
  for r0001 in c0001 loop
    ...
    for r0002 in c0002(r0001.owner,r0001.table_name,r0001.tablespace_name) loop

      insert into cecred.index_tablespace
      values (r0002.owner,r0002.index_name,r0002.tablespace_name);
...
procedure grava_comando_indice is
  ...
  cursor c0001 is
    select
    to_char(dbms_metadata.get_ddl('INDEX',a.index_name,a.owner)) ww_comando,
    a.tablespace_name,
    a.owner,
    a.index_name
    from dba_indexes a
  ...
  cursor c0002 is
    select
    'drop index "'||a.owner||'"."'||a.index_name||'"' ww_comando
    from dba_indexes a
        ,tabela_tablespace b
    where a.owner = 'CECRED'
      and a.owner = b.owner
...
begin
  ...
  for r0001 in c0001 loop
    ...
    values
    (r0001.owner,r0001.index_name,r0001.ww_comando,r0001.tablespace_name,null,null);
...
exception
  when others then
    raise_application_error(-25002,'Erro Geral GRAVA_COMANDO_INDICE: '||sqlerrm);
end grava_comando_indice;

procedure recria_indice is

  cursor c0003 is
    select owner, index_name, tablespace_name, ds_comando
    from cecred.create_index
    where tablespace_name = 'TBS_GERAL_I'
      and (status is null
           or
           status = 'NOK')
    order by 1,2;
...
begin
  ...
  for r0003 in c0003 loop
    ...
    begin
      ...
      execute immediate r0003.ds_comando;
      ...
      exception
        when others then
          raise_application_error(-25033,'UPDATE CECRED.CREATE_INDEX OK: '||sqlerrm);
      end;
    ...
    exception
      when others then
        ...
        exception
          when others then
            raise_application_error(-25333,'UPDATE CECRED.CREATE_INDEX NOK: '||sqlerrm);
        end;
...
begin
  ...
  for r0004 in c0004 loop
    ...
    sys.dbms_job.submit(ww_cont,'CECRED.AJUSTA_OBJETO.RECRIA_INDICE('''||R0004.TABLESPACE_NAME|| ''');', sysdate, null);
...
exception
  when others then
    raise_application_error(-25004,'Erro Geral RECRIA_INDICE_PARALELO: '||sqlerrm);
...
exception
  when others then
    raise_application_error(-25003,'Erro para dropar a tabela: '||sqlerrm);
...
exception
  when others then
    raise_application_error(-25003,'Erro para dropar a sequence: '||sqlerrm);
